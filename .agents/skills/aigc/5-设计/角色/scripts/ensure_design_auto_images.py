#!/usr/bin/env python3
"""Ensure 5-设计 domain Markdown files have same-stem built-in imagegen requests."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="扫描 5-设计 域级输出目录，补齐每个 Markdown 设计文件的同 stem 自动图。"
    )
    parser.add_argument("--project", help="项目名；用于自动发现 projects/aigc/<项目名>/4-设计/")
    ...
    parser.add_argument("--design-dir", action="append", default=[], help="直接指定 5-设计 输出目录，可重复传入")
...
def discover_design_dirs(args: argparse.Namespace) -> list[Path]:
    ...
    if args.project:
        root = REPO_ROOT / "projects" / "aigc" / args.project / "5-设计"
        if root.exists():
            dirs.append(root)
    deduped: list[Path] = []
    seen: set[str] = set()
