...
| 启用 subagents 后只问物品酷不酷，没有请教剧情杠杆 | advisor consultation gap | 按 `team.yaml` 顾问专长追问归属、启用、代价、专属适配、线索功能与不可替代性 | 显式启用 subagents 时先生成 `advisor_consultation_packet`，再进入物品卡创作 | 物品卡能说明顾问建议如何落到 `ownership_links / usage_rules / costs / exclusive_fit` |
...
2. 若不清楚为何是物品问题，先回父技能。
3. 先修结构，再补材质与风格。
4. 如果显式启用 subagents，顾问问题要逼近“为什么非它不可”和“谁因为它付出什么代价”，不要停留在外观风格。
...
- 顾问请教在物品卡里最适合用来防止“道具装饰化”：一条建议必须能转成归属链、启用条件、代价、线索动作或专属适配，才算进入正式设计。
