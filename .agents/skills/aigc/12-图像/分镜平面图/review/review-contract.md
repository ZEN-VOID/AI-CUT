...
本 review gate 只裁决 `分镜平面图` 的 prompt、manifest、imagegen 计划、成图和项目持久化，不改写 `10-分组` 主真源。
...
| --- | --- | --- | --- |
| `G1-SOURCE` | 每个 `group_id` 可回指 `10-分组` 源标题、组正文和 YAML，且连接件未进入生图任务 | `FAIL-FLOOR-PLAN-PANELS` | `N2-SOURCE-LOCK` |
...
| `G7-IMAGEGEN` | 已加载并调用 `.agents/skills/cli/imagegen`；imagegen plan 一组一任务，默认 4K，输出项目内路径，不越权 fallback，不静默覆盖，不得以 plan-only 通过 | `FAIL-FLOOR-IMAGEGEN` | `N6-PROMPT-PAYLOAD` / `N7-IMAGEGEN` |
