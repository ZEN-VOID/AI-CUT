...
- 必需输入：
  - 一个目标根目录，例如 `.agents/skills/story/stages/3-初稿`
...
- `0-初始化 -> 1-Cards -> 2-卷章规划 -> 3-初稿 -> 4-Polish -> 5-Validation -> 6-Loopback`

### `1-设定`

- `1-设定` 是组级路由器，不应把卡族强行串成线性流水线。
- 叶子卡族技能若缺显式下游，默认优先提供返回组级路由器，或进入当前任务真正需要的下一个卡族。

### `2-卷章规划`

- `2-卷章规划` 默认只有一个对外入口：组级 `SKILL.md`。
- 所有 leaf 子目录默认视为内部 planning pass，不应在用户只说“进入 2-卷章规划”时被直接当成首入口。
...
- `story2026` 的 `1-设定`、`2-卷章规划`、`3-初稿`、`4-Polish`、`5-Validation` 与 `6-Loopback` 没有被误写成过期阶段链或旧中文 stage。
