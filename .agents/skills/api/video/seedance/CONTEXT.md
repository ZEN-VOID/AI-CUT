...
| `TM-SEEDANCE-ALIAS-DRIFT` | 本地技能仍按旧版本/旧冲突口径解释 `seedance` 与 `seedance-fast` | 真源同步层 | 回到官方 AnyFast 文档，恢复 `seedance=最新质量优先`、`seedance-fast=最新速度优先` | 在 `SKILL.md`、`references/api.md`、入口元数据和脚本注释里统一写明滚动别名语义 | 默认运行 `run` 时不显式传 `--model` 仍落到最新质量档 |
...
- 对存在滚动别名的模型接口，默认值不应写成易过期的长版本号；优先跟随官方稳定别名，并在真源文档中写清“别名代表的版本语义”。
