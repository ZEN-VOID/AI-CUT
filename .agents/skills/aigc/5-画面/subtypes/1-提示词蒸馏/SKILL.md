...
description: Use when the `5-画面` stage needs a parent prompt-distillation contract that routes work to `分镜故事板`、`分镜帧`、or `漫画` before downstream consistency or image-generation subtypes run.
...
- 需要进入 `5-画面` 的第一层执行，但用户还没明确说是 `分镜故事板`、`分镜帧` 还是 `漫画`。
- 需要把已有 `projects/<项目名>/编导/第N集.json` 内容先蒸馏成图像请求 JSON。
...
- 上游 `编导/第N集.json` 尚未形成可消费的 `分镜组列表[]` 或明确 `分镜ID`。
...
- 若上游对象不成立，应停止在本层并回退到 `5-画面` 父级或更上游阶段补齐输入。

## Visual Maps

```mermaid
flowchart TD
    A["读取 编导/第N集.json"] --> B{"判定蒸馏对象"}
    B -->|"组级多格"| C["分镜故事板"]
    B -->|"单一分镜ID"| D["分镜帧"]
    B -->|"漫画页/气泡文字"| E["漫画"]
    C --> F["输出请求 JSON"]
