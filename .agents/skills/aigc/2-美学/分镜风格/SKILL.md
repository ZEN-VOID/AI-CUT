...
`分镜风格` 是 AIGC 影片项目的分镜层级风格协议技能。它从上游剧本、画面基调、摄影风格、参考图、参考视频或候选分镜资料中提取分镜组织规律，研究节奏、景别流转、镜头组合、转场逻辑、注意力/视点路由、动作/情绪承接、信息揭示/声画节拍、段落推进和 AI 视频下游可执行的分镜连接方式。
...
- 默认上游来源包括 `1-分集/`、`2-美学/画面基调/全局风格协议.md`、`2-美学/摄影风格/`、用户粘贴文本、参考图/视频说明和已有候选分镜风格协议；用户显式指定来源时，以用户输入为本轮来源并标注来源类型。
- 若输入来源或任务目标明确为 `第N集`，正式输出对象是该集分镜风格覆盖，写入 `projects/aigc/<项目名>/2-美学/第N集/分镜风格/`；只有整季、多集汇总或用户明确项目级基线时才写入非逐集 `分镜风格/`。
...
- `projects/aigc/<项目名>/1-分集/第N集.md`、整季 `1-分集/`、`2-编导/`、项目设定或用户指定剧本文本。
- `projects/aigc/<项目名>/2-美学/画面基调/全局风格协议.md`、摄影风格协议、项目 `MEMORY.md` 中长期节奏偏好或禁区。
...
| --- | --- | --- | --- | --- | --- |
| `script_storyboard_analysis` | 指定 `1-分集`、`2-编导` 或粘贴剧本文本 | `Script Storyboard Style Path` | `N1,N2,N4,N5,N6,N7,N8,N9` | `CONTEXT.md` | `FAIL-SBS-TYPE-SCRIPT` |
...
| --- | --- | --- |
| `single_episode_seed` | 基于单集剧本或单段资料建立分镜风格覆盖 | `projects/aigc/<项目名>/2-美学/第N集/分镜风格/分镜风格协议.md` |
| `series_storyboard_protocol` | 基于多集、整季或项目资料建立正式项目级协议 | `projects/aigc/<项目名>/2-美学/分镜风格/分镜风格协议.md` |
...
| `N2-STORY-EVIDENCE` | 从剧本/设定抽取分镜节奏证据 | `1-分集`、`2-编导` 或文本 | 提取段落推进、信息揭示、冲突密度、情绪转折、场景转换压力；禁止生成具体镜头或动作 | `storyboard_source_evidence_map`，至少 5 条证据 | `N4` / `N3` | 每条证据能回指输入片段或项目资料 |
...
| `N4-RHYTHM-PROFILE` | 汇流分镜节奏画像 | N2/N3 证据 | 建立 8 个解析维度：节奏密度、景别流转、镜头组合、转场逻辑、注意力/视点路由、动作/情绪承接、信息揭示/声画节拍、段落推进与下游容错 | `storyboard_rhythm_profile` | `N5` | 只保留分镜组织属性；剧情和资产内容必须删除 |
...
| `N6-DENSITY-FLOW-CALIBRATION` | 校准信息密度和流畅性 | N4/N5 输出 | 平衡建立信息、推进信息、反应信息和尾钩信息；明确对白、音效、静默、情绪反应等声画节拍如何影响慢/中/快段落切换 | `density_flow_matrix`，至少 4 类信息功能 | `N7` | 不得把节奏策略写成具体剧情动作 |
| `N7-PROMPT-DISTILL` | 蒸馏分镜风格 prompt | N4-N6 输出 | 生成约 100 字中文 `storyboard_style_prompt`；包含节奏、景别流转、镜头组合、注意力/视点路由、连接方式、声画节拍、信息密度、下游容错；过滤镜头编号、剧情动作、资产设计、摄影参数 | `candidate_storyboard_style_prompt`、`boundary_scan` | `N8` | 字数 80-130；禁用类别残留 0 个；只保留风格层级 |
...
| 是否说明信息密度、流畅性、连贯性和声画节拍？ | `GATE-SBS-07-DENSITY-FLOW` | `FAIL-SBS-DENSITY-MISSING` | `N6-DENSITY-FLOW-CALIBRATION` | `density_flow_matrix` |
...
- 默认禁止摄影参数：最终 prompt 不写焦段、光圈、机位关系、视角距离、相机运动参数、灯位或具体构图法；如需继承摄影风格，只转译为分镜连接和节奏约束。
- 默认禁止参考照搬：参考图/视频只能抽取节奏、连接、信息密度和段落推进规律，不复制具体镜头顺序或动作段落。
- 可保留的分镜风格类别：节奏密度、景别流转倾向、镜头组合语法、转场逻辑、注意力/视点路由、动作/视线/情绪承接、信息揭示/声画节拍、段落推进、下游容错和负面特征。

## Output Contract

正式写回路径：

- `episode_scoped`: `projects/aigc/<项目名>/2-美学/第N集/分镜风格/分镜风格协议.md`
- `episode_scoped`: `projects/aigc/<项目名>/2-美学/第N集/分镜风格/执行报告.md`
- `series_baseline`: `projects/aigc/<项目名>/2-美学/分镜风格/分镜风格协议.md`
- `series_baseline`: `projects/aigc/<项目名>/2-美学/分镜风格/执行报告.md`
...
当 `2-美学` 父级调度多个同级风格子技能时，`分镜风格` 参与父级并发 fan-out，不要求等待 `画面基调` 或 `摄影风格` 实时先跑；若已有上游协议则继承底层视觉和摄影约束，若缺失则输出候选分镜风格并标记 `dependency_gap`。它只输出分镜组织、节奏、连接和信息密度协议；不拥有画面基调、摄影参数、具体分镜正文、角色、场景、道具或资产真源，不得反向补写上游结论。
