    ...
    def get_review_trend_stats(self, last_n: int = 5) -> Dict[str, Any]:
        ...
        if not records:
            return {
                ...
                "dimension_avg": {},
                "risk_avg": {},
                "anti_ai_force_check": {"pass": 0, "fail": 0, "pending": 0},
                "severity_totals": {},
                "recent_ranges": [],
            }
        ...
        risk_scale = {"low": 1.0, "medium": 2.0, "high": 3.0, "critical": 4.0}
        risk_totals: Dict[str, float] = {}
        risk_counts: Dict[str, int] = {}
        anti_ai_counts = {"pass": 0, "fail": 0, "pending": 0}
        ...
        for record in records:
            ...
            anti_ai = str(record.get("anti_ai_force_check") or "pending").strip().lower()
            if anti_ai not in anti_ai_counts:
                anti_ai = "pending"
            anti_ai_counts[anti_ai] += 1

            for risk_field in ("spoiler_risk", "contrivance_risk", "cold_commentary_risk"):
                raw_level = str(record.get(risk_field) or "").strip().lower()
                if raw_level not in risk_scale:
                    continue
                risk_totals[risk_field] = risk_totals.get(risk_field, 0.0) + risk_scale[raw_level]
                risk_counts[risk_field] = risk_counts.get(risk_field, 0) + 1
        ...
        risk_avg = {
            key: round(risk_totals[key] / risk_counts[key], 2)
            for key in risk_totals
            if risk_counts.get(key, 0) > 0
        }
        recent_ranges = [
            {
                ...
                "anti_ai_force_check": record.get("anti_ai_force_check", "pending"),
                "spoiler_risk": record.get("spoiler_risk", "low"),
                "contrivance_risk": record.get("contrivance_risk", "low"),
                "cold_commentary_risk": record.get("cold_commentary_risk", "low"),
            }
            for record in records
        ]
        ...
        return {
            ...
            "risk_avg": risk_avg,
            "anti_ai_force_check": anti_ai_counts,
