...
本合同定义 `10-审片` 的核心审片维度。所有判断必须基于真实视频证据、prompt / 分镜组证据或用户提供的示例证据，避免空泛审美口号。
...
审查视频是否与用户 prompt、`6-分组` 真源和 `9-视频` 生成证据一致。
...
| --- | --- | --- |
| `prompt_problem` | prompt 缺关键信息、互相矛盾、单条塞入太多 beat、动作不可拍、审美词空泛、负面约束不清 | 修 `6-分组` 或 `9-视频` prompt 组装 |
...
若 `6-分组` / `9-视频` prompt 已明确要求低角度、贴地前景、遮挡慢拉或发现路径，但视频没有执行，优先归因 `model_problem / rerun_only`；若上游 prompt 只有“人物向前走”等动作摘要，归因 `prompt_problem`，回到 `5-摄影` 或 `6-分组` 补观看选择。
