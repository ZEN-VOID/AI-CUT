...
description: Use when `2-Global` needs to turn `1-Planning/3-分组` grouped scripts plus `0-Init` project seeds into a series-level visual style design under `projects/aigc/<项目名>/2-Global/全局风格/全局风格设计.md`.
...
- 附加上下文真源是 `0-Init`
- canonical 输出固定为 `projects/aigc/<项目名>/2-Global/全局风格/全局风格设计.md`
- 若未来父级 `2-Global` 需要额外投影 `projects/aigc/<项目名>/2-Global/全局风格.md`，该文件只能是派生摘要，不得反向充当第二真源
...
- 上游还没有稳定的 `projects/aigc/<项目名>/1-Planning/3-分组/第N集.md`
...
| slot | content |
| --- | --- |
| `business_goal` | 以 grouped script 为主证据，为整部影片生成一份系列级全局画面风格设计，并输出可复用的风格词汇、风格句与 150 字内通用前缀提示词。 |
| `business_object` | `projects/aigc/<项目名>/1-Planning/3-分组/第N集.md`、`执行报告.md`、`episode-split-plan.json`、`north_star.yaml`、`init_handoff.yaml`、最终 `全局风格设计.md`。 |
| `constraint_profile` | `3-分组` 是主输入，`0-Init` 只作附加约束；灵感可借大师机制，不得抄单一作品表层；输出必须面向全剧而非单集碎片。 |
| `success_criteria` | 产出中同时具备全剧视觉命题、上游证据摘要、大师灵感映射、风格词汇造句、150 字内通用前缀提示词、下游继承边界，并能直接服务后续阶段。 |
| `topology_fit` | 固定主干为：输入锁定 -> 全剧归纳 -> 大师映射 -> 风格造句 -> 通用前缀提炼 -> Markdown 写回 -> QA 汇流。 |
...
flowchart TD
    ...
    E --> F["提炼 150 字内通用前缀"]
    F --> G["按模板写入 全局风格设计.md"]
    G --> H["QA: 证据/范围/继承性/非临摹"]
    H --> I{"Pass?"}
    I -->|"Yes"| J["Emit canonical output"]
    I -->|"No"| K["回到取证 / 映射 / 造句节点"]
```
...
3. `secondary init context`
   ...
   - `projects/aigc/<项目名>/0-Init/story-source-manifest.yaml`（若存在）
4. `optional context`
   - 用户显式风格偏好
   - 已有 `2-Global` 层草稿
   - 用户额外提供的参考影片或参考图册
...
| `N4-STYLE-SENTENCE` | `S4` | `FIELD-GSTYLE-05` `FIELD-GSTYLE-06` | 生成全局风格主陈述、风格词汇造句与通用前缀 | 产出全剧视觉命题、风格母句、词汇组句、稳定 guardrails，并压出 150 字内通用前缀 | 风格主陈述、风格句、词汇包、前缀草案 | 成功 -> `N5`；句法空泛或前缀过长/过私有 -> 回 `S4` | 句子与前缀都必须可被下游继承 |
| `N5-WRITEBACK` | `S5` | `FIELD-GSTYLE-07` | 按模板写回 Markdown 真源 | 动态读取模板，写入 `全局风格设计.md` 各分区，并把通用前缀固定在文末最后一节 | 文件落盘证据 | 成功 -> `N6`；结构错位 -> 回 `S5` | 输出路径、章节完整且末尾前缀就位后方可验收 |
...
5. 文档最后已沉淀一条全局可复用的通用全局风格前缀提示词，且正文不超过 `150` 字
6. canonical 输出已写入 `projects/aigc/<项目名>/2-Global/全局风格/全局风格设计.md`
7. 输出可以直接被 `类型元素 / 设计元素 / 3-Detail / 4-Design / 5-Image` 读取而不必重新解释
...
- 若未来父阶段需要平铺摘要到 `projects/aigc/<项目名>/2-Global/全局风格.md`，只能从本文件派生，不得双向编辑。
...
1. 第 `9` 节必须是文档最后一节，不得再在其后追加新正文段落。
2. 通用前缀必须是全局可复用的视觉提示词前缀，不得写入项目专有剧情细节、角色专名或单集事件。
3. 通用前缀总字数不得超过 `150` 字。
...
| `FIELD-GSTYLE-07` | 稳定继承法则、边界与通用前缀 | `全局风格设计.md > 稳定继承法则 / 下游使用边界 / 通用全局风格提示词前缀（150字内）` | `FIELD-GSTYLE-05 + 下游消费场景` | `N5` |
...
| `S4-style-sentence` | `FIELD-GSTYLE-05` `FIELD-GSTYLE-06` | 生成可继承的风格主陈述、风格句与通用前缀 | 风格句华丽但不可执行，或前缀超过 `150` 字、带入项目私货 | 回 `N4-STYLE-SENTENCE` |
| `S5-writeback` | `FIELD-GSTYLE-07` | 按模板写回结构化 Markdown | 章节缺失、路径错误、无继承法则或文末缺少通用前缀 | 回 `N5-WRITEBACK` |
...
| `FIELD-GSTYLE-07` | 输出结构完整性 | `FAIL-OUTPUT-STRUCTURE-DRIFT` | 模板章节缺失、路径错误，或缺少 `150` 字内通用前缀 | `S5-writeback` |
