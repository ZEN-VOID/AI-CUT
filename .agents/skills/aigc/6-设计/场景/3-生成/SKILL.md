...
- 必须读取目标场景的上游设计文档：`projects/aigc/<项目名>/6-设计/场景/2-设计/S###-<场景名>.md`。
...
本阶段拥有 `projects/aigc/<项目名>/6-设计/场景/3-生成` 下场景生成资产与提示词 JSON 的交付权。它不拥有上游 `2-设计` 设计文档的业务真源权，不新增或重命名场景主体，不改 registry、父级技能、角色/道具生成技能或其他 worker 的输出。
...
- 已存在的上游 `projects/aigc/<项目名>/6-设计/场景/2-设计/S###-<场景名>.md`。
...
3. 读取目标上游场景设计文档和可选 `projects/aigc/<项目名>/6-设计/场景/design-manifest.yaml`，只抽取主体名称、来源文件、`4. 解构` 与必要的设计约束，不重新设计主体；不得再把 `提示词设计` 的英文整合 prompt 作为导入给 gpt-image-2 的源文本。
...
7. 所有项目交付资产写入 `projects/aigc/<项目名>/6-设计/场景/3-生成`；不得只停留在 `$CODEX_HOME/generated_images`；可更新 `design-manifest.yaml` 的 `generation_assets` 与 `generation_gaps`。
...
6. 可选更新 `projects/aigc/<项目名>/6-设计/场景/design-manifest.yaml`，记录 `generation_assets` 和剩余 `generation_gaps`；manifest 不替代生成资产真源。
...
| --- | --- |
| `OUTPUT-SCENE-MAIN-IMAGE` | `projects/aigc/<项目名>/6-设计/场景/3-生成/<主体ID>-<主体名称>-主图.<ext>` |
| `OUTPUT-SCENE-MAIN-PROMPT` | `projects/aigc/<项目名>/6-设计/场景/3-生成/<主体ID>-<主体名称>-主图.json` |
| `OUTPUT-SCENE-MULTIVIEW-IMAGE` | `projects/aigc/<项目名>/6-设计/场景/3-生成/<主体ID>-<主体名称>-多视图.<ext>` |
| `OUTPUT-SCENE-MULTIVIEW-PROMPT` | `projects/aigc/<项目名>/6-设计/场景/3-生成/<主体ID>-<主体名称>-多视图.json` |
| `OUTPUT-SCENE-GENERATION-REPORT` | `projects/aigc/<项目名>/6-设计/场景/3-生成/执行报告.md` |
...
- 项目交付图片已持久化到 `projects/aigc/<项目名>/6-设计/场景/3-生成`。
