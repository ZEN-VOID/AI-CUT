...
本文件是 `aigc/2-Global -> 3-Detail` 之间 `分镜组壳 + 组间设计` 写入 shared episode root 的单一真源。
...
| --- | --- | --- |
| `2-Global` | `aigc-global` | 创建或 patch shared episode root 的 `metadata / 分镜组列表[].分镜组壳` |
| `3-Detail` | `aigc-detail` | 继承已存在的 `组间设计`，并补全 `分镜明细[]` 与 detail 级字段 |

硬规则：

1. `2-Global` 只拥有 `分镜组壳`、`组间设计` seed 与相关 metadata 的写入权，不得在本阶段发明 shot-level `分镜明细[]`。
2. `3-Detail` 默认继承 `组间设计`，不得在无明确返工理由时重写其含义。
3. `2-Global/*.md` 是长文本解释侧车；下游第一结构化 handoff 以 shared episode root 为准。
...
| `组间设计.类型元素` | `2-Global/类型元素.md` 中 `第N集 -> 【组ID】` 命中段落里的字段标题 `类型元素` | 当前组 | 50 | 直接引用 Markdown 中已确认的一段组级类型信号，不在 JSON 现场改写 |
...
3. `2-Global` 写回 shared root 时，必须同时形成完整分镜组壳：`分镜组ID / 总时长 / 剧本正文 / 组间设计 / 分镜明细=[]`。
...
- 重点检查：
  1. `组间设计` 是否稳定继承。
  2. `分镜明细[]` 是否只在 `3-Detail` 阶段扩展。
  3. phase 推进是否仍保持同一 canonical root，而不是另起第二份终稿。
