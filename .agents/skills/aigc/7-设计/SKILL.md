...
metadata:
  short-description: Route fused 7-设计 domain packages
---
...
- 若命中的域级或叶子设计阶段显式要求执行顾问与复核流程，父级必须把 `../_shared/team-advisor-consultation-contract.md` 作为共享机制传递给对应域，并要求其优先消费 `team.yaml.roles.supervision.stage_profiles."7-设计"` 或对应 leaf 的阶段 profile；真实顾问请教与 `advisor_consultation_packet` 由命中的域级/叶子技能执行和消费，父级只在阶段验收中检查是否执行或是否有本地 checklist 结果。
...
- 无序号同级子技能包默认全选并发执行；因此整体调用 `7-设计` 且未指定域时，默认同时调度 `场景`、`角色`、`道具` 三个域级包，再由本父级汇总验收。
...
- Accepted input: 7-设计 阶段任务、场景/角色/道具清单-设计-生成一体化任务、旧 `1-清单 / 2-设计 / 3-面板` 路径迁移后的路由修复。
- Required input: 项目名与集数，或可定位到 `projects/aigc/<项目名>/6-分组/第N集.md` / 既有 7-设计 输入。
...
当前 `7-设计` 只保留三个域级 Skill 2.0 子技能包：

| domain | skill package | final required outputs |
| --- | --- | --- |
| `场景` | `.agents/skills/aigc/7-设计/场景` | `场景/1-清单/场景清单.md`、`场景/2-设计/S###-<场景名>.md`、`场景/3-生成/<场景名>-*.json` |
| `角色` | `.agents/skills/aigc/7-设计/角色` | `角色/1-清单/角色清单.md`、`角色/2-设计/<角色名>.md`、`角色/3-生成/<角色名>-*.json` |
| `道具` | `.agents/skills/aigc/7-设计/道具` | `道具/1-清单/道具清单.md`、`道具/2-设计/<道具名>.md`、`道具/3-生成/<道具名>-*.json` |
...
| `multi_domain` | 用户要求一次处理多个域，或整体调用 `7-设计` 且未指定单一域 | 无序号域级包默认全选并发调度；若用户显式给出域集合但未给顺序，也按并发汇总处理 |
| `incremental_reconcile` | `6-分组` 只完成部分集数后曾执行过 7-设计，随后又新增或更新 `第N集.md` | 先按域执行增量对账，再进入缺口对应的 `1-清单 -> 2-设计 -> 3-生成` |
...
2. 锁定 7-设计 输出根：`projects/aigc/<项目名>/7-设计/`。
3. 根据用户请求、输入文件或现有产物判定命中域。
4. 若检测到 `6-分组` 新增、更新、只覆盖部分集数，或既有 `7-设计` 产物已存在，必须进入 `incremental_reconcile`：读取 `references/incremental-reconciliation-contract.md`，并要求每个命中域先产出本轮 `reconcile_delta`。
5. 只调度命中的域级子技能包；未命中的域不得补空清单、补占位主体或伪造面板 JSON。
6. 域级子技能内部固定处理顺序为 `清单 -> 设计 -> 生成`，但每步只处理增量对账后的缺口；不得静默覆盖既有清单、设计稿或生成资产。
7. 当命中 `角色/2-设计`、`道具/2-设计`、`场景/2-设计` 且其合同要求 顾问与复核流程 时，父级验收必须检查域级是否按共享团队顾问合同读取 `team.yaml`、优先解析 `roles.supervision.stage_profiles."7-设计"` 或 leaf 专属 profile、请教项目监制顾问并形成 `advisor_consultation_packet`，或是否记录了本地流程。
...
10. 所有命中域通过复审后，写入 `projects/aigc/<项目名>/7-设计/validation-report.md`，并记录本轮上游范围、新增主体、跳过既有产物、顾问请教/本地复核、repair actions、复审结果和遗留风险。

## Stage-Closeout Review-Repair Contract

`7-设计` 父级不拥有场景、角色、道具业务主稿写权，因此阶段末闭环采用“父级聚合，域级修复”的模式。
...
4. 若失败源于上游 `6-分组`、旧路径迁移或增量对账，父级可以修路由/对账合同，但业务主体仍回对应域级 leaf。
...
6. 设计阶段显式执行顾问与复核流程 但没有请教 `team.yaml` 监制顾问，或没有优先消费 `roles.supervision.stage_profiles."7-设计"`：回到 `../_shared/team-advisor-consultation-contract.md` 和对应叶子 `SKILL.md`。
...
| `DESIGN-FIELD-06` | `projects/aigc/<项目名>/7-设计/validation-report.md` | 阶段级验收摘要 |
| `DESIGN-FIELD-07` | `../_shared/team-advisor-consultation-contract.md` | 显式执行顾问与复核流程 的设计叶子已执行 team advisor 请教或使用本地流程 |
...
| --- | --- | --- | --- |
| `DESIGN-PASS-01` | 判断项目根与输出根 | 锁定 `projects/aigc/<项目名>/7-设计/` | runtime path |
...
| `DESIGN-REWORK-INCREMENTAL` | 新增 `6-分组` 后清单/设计/生成未对账或覆盖既有产物 | `references/incremental-reconciliation-contract.md` |
| `DESIGN-REWORK-DOMAIN` | 域级输出失败 | 对应域级 leaf `review/review-contract.md` -> direct repair -> re-review |
| `DESIGN-REWORK-ADVISOR` | 显式执行顾问与复核流程 的设计叶子缺少 team advisor 请教或本地流程 | `../_shared/team-advisor-consultation-contract.md` |
...
- Output path: 父级阶段报告写到 `projects/aigc/<项目名>/7-设计/validation-report.md`；域级业务真源写到 `projects/aigc/<项目名>/7-设计/{场景,角色,道具}/{1-清单,2-设计,3-生成}/`；域级状态 sidecar 可写到 `projects/aigc/<项目名>/7-设计/{场景,角色,道具}/design-manifest.yaml`。
- Naming convention: 父级报告固定名 `validation-report.md`；域级命名由各域 `SKILL.md` 的 Output Contract 裁决。
- Completion gate: 路由只命中 active 域级包；旧 tranche 路径不再作为 active skill 入口；命中域的 Skill 2.0 结构和输出合同验证通过；若存在分批上游或既有产物，已完成增量对账且未静默覆盖；显式执行顾问与复核流程 的设计叶子已完成 `team.yaml` 顾问请教或使用本地流程；阶段 closeout 中发现的域级阻断项已回到对应 leaf 直接修复并复审，或在 `validation-report.md` 中明确使用本地流程和未通过域。
