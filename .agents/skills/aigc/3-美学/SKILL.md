...
Completion gate: `overall_parallel` 模式必须有 6 行 dispatch、6 行 result、父级 cross-check、`anti_scripted_suite_audit`、父级 handoff 和完整报告证据；任一子技能 `blocked` 或 `FAIL-AES-SCRIPTED-SUITE` 时父级不得判定为 `pass`。
