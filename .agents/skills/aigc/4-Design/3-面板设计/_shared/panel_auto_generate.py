...
def _load_manifest_packet_paths(manifest_path: Path) -> list[Path]:
    ...
    if not packet_files:
        packet_files = [
            ...
            if str(item).strip().endswith("-layout.json")
        ]
...
def _discover_continuity_refs(packet: dict[str, Any], packet_path: Path, repo_root: Path) -> list[str]:
    ...
    for root in _continuity_roots(packet, repo_root):
        ...
        for candidate in root.rglob("*"):
            ...
            if score <= 0:
                continue
            if candidate.parent == packet_path.parent:
                score -= 20
            candidates.append((score, depth, candidate.resolve().as_posix()))

    candidates.sort(key=lambda item: (-item[0], item[1], item[2]))
    ordered = [item[2] for item in candidates[:MAX_DISCOVERED_REFS]]
...
def _extract_prompt(packet: dict[str, Any]) -> tuple[str, str]:
    ...
    if isinstance(image_generation, dict):
        ...
        if prompt_text:
            return prompt_text, str(image_generation.get("prompt_field", "image_generation.prompt_text")).strip()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dry-run", action="store_true", help="只生成 request sidecar 并打印 nano payload")
    parser.add_argument("--print-payload", action="store_true", help="打印 nano 最终 payload")
    parser.add_argument(
        "--pipeline-context",
        choices=("panel-stage", "direct-request"),
