...
DEFAULT_GPT_IMAGE_2_SIZE = "2048x1152"
DEFAULT_LEGACY_SIZE = "auto"
DEFAULT_SIZE = None
...
def _resolve_size(size: Optional[str], model: str) -> str:
    if size:
        return size
    if model == GPT_IMAGE_2_MODEL:
        return DEFAULT_GPT_IMAGE_2_SIZE
    return DEFAULT_LEGACY_SIZE
...
def _validate_generate_payload(payload: Dict[str, Any]) -> None:
    ...
    size = _resolve_size(payload.get("size"), model)
    payload["size"] = size
...
def _generate(args: argparse.Namespace) -> None:
    ...
    size = _resolve_size(args.size, args.model)
    payload = {
        "model": args.model,
        "prompt": prompt,
        "n": args.n,
...
def _edit(args: argparse.Namespace) -> None:
    ...
    size = _resolve_size(args.size, args.model)
    _validate_size(size, args.model)
