...
description: Use when the 4-Design stage needs one role-domain Skill 2.0 package that runs role list, role design, and role panel prompt generation in order, while writing final role artifacts directly under `projects/aigc/<项目名>/4-Design/`.
...
- 本技能是 `.agents/skills/aigc/4-Design/` 下的角色域子技能包；清单、设计、面板不再作为独立 skill 入口展开。
...
| --- | --- | --- |
| `ROLE-PASS` | 清单、设计文档、面板 JSON 都在 4-Design 根目录且同 stem 对齐 | done |
