...
`4-表演` 可以有自己的表演工艺重心，但不得把 `2-编剧` 和 `3-导演` 已经完成的画面化成果重新抽象化。心理反应、演员动机、台词处理、潜台词、场面调度、顾问意见、执行报告和最终落盘，都必须使用具像表演语言：身体哪里变、声音哪里断、呼吸如何变化、手和重心怎么动、对手如何接不到、空间距离如何改变。不得把输出写回“紧张感增强/关系变化/内心崩塌/权力压迫/情绪复杂/演员要克制”等概念标签或解释性总结。
...
| 跨阶段具像画面/表演语言、反抽象、反概念、反解释、禁止把上游画面化成果重新抽象化 | `../_shared/concrete-visual-language-contract.md` |
...
| 角色演技五层控制与台词声线控制：触发点、情绪动机、微表情、身体联动、语气情绪、气口、断句、长对白节拍交付、环境声、非对称瑕疵、情绪切换瞬间和微动态限制 | `references/actor-performance-control-contract.md` |
...
7. 每段对白必须完成台词表演标注：`对白（角色，语气/情绪/状态短语）` 第二项不得缺失，且 `对白画面` 或相邻字段必须就近承托气口、断句、停顿、声线、重音、尾音或对手反应；若上游含 `long_dialogue_beat_map`，必须逐 beat 形成 `long_dialogue_delivery_map`，不得把长对白演成同一口气、同一状态、同一反应；不得改写引号内原对白。
...
- 上游存在 `long_dialogue_beat_map` 时，已形成 `long_dialogue_delivery_map`：每个原文节拍都有气口/连续气息、停顿或无停顿依据、重音、尾音、身体联动和对手反应；没有改写、合并吞并或重新断开上游原文节拍。
...
- 表演层所有 planning evidence、顾问意见、执行报告和终稿字段已通过 `../_shared/concrete-visual-language-contract.md`：心理、情绪、潜台词、关系和权力判断必须落到可见身体、可听声音、可演停顿、空间距离、道具互动或对手反应；不得把上游画面化成果重新抽象成概念标签或解释性心理/表演总结。执行报告包含 `concrete_visual_language_evidence`。
...
| `FIELD-PERF-03B` | 长对白交付链 | 消费上游 `long_dialogue_beat_map`，为每个长对白节拍补气口/连续气息、停顿、重音、尾音、身体联动和对手反应；不改写或重切原文节拍 | `FAIL-LONG-DIALOGUE-DELIVERY` |
...
| `FIELD-PERF-25` | 具像表演语言 | 心理、情绪、潜台词、关系、权力、演员任务、顾问意见、报告和终稿字段均落到可见身体、可听声音、可演停顿、空间距离、道具互动或对手反应；没有把上游画面化成果重新抽象成概念标签、情绪论文或表演意图总结 | `FAIL-CONCRETE-PERFORMANCE-LANGUAGE` |
...
| `PASS-PERF-04` | 五层表演控制 | 关键情绪 beat、上游 `long_dialogue_beat_map`、`performance_task_map`、`actor-performance-control-contract.md`、`stanislavski-method-reference.md` | 触发点是否明确；情绪动机是否至少两层（表层/压制/隐藏）；微表情变量和非面部身体变量是否到位；环境声或微动态限制是否承托；台词是否写清语气、情绪、气口、断句和声线；长对白是否逐 beat 形成交付链；调用三层情绪执行模板和情感瞬间与微动态控制进行 micro_expression、micro_dynamics、dialogue_delivery 处理 | `actor_performance_control_evidence`、`dialogue_performance_evidence`、`long_dialogue_delivery_map` |
...
| `PASS-PERF-11` | LLM 表演工艺投影 | `psychological_reaction_evidence`、`actor_performance_control_evidence`、`dialogue_performance_evidence`、`scene_dramatic_map`、`performance_task_map`、`blocking_power_map`、`protagonist_inner_voice_evidence`、`integration_targets`、`../_shared/concrete-visual-language-contract.md`、上游正文 | 是否把全部表演规划证据嵌入既有字段，形成完整表演稿；是否保持原文保真；是否把心理、情绪、潜台词、关系和权力判断全部转成具像表演语言而非概念标签 | `candidate_performance`、`concrete_visual_language_evidence` |
| `PASS-PERF-12` | 审查修复闭环 | candidate 表演稿、review gate、上游真源、`concrete_visual_language_evidence` | 是否覆盖心理反应可感知化、五层表演控制、台词表演、潜台词行为化、场景状态差、场面调度内嵌、沉默反应、内心独白人称、动作纯度、具像表演语言和原文保真；阻断项是否已在本阶段最小修复并复审通过 | review result、repair result |
...
| `PASS-PERF-04` | 关键情绪 beat 有五层证据；不只有情绪标签或模板化表情；每段对白有语气情绪和气口断句等台词表演锚点；长对白逐 beat 有交付链 | `FAIL-PERF-03` / `FAIL-PERF-03A` / `FAIL-LONG-DIALOGUE-DELIVERY` | `references/actor-performance-control-contract.md`、`references/performance-and-scene-craft-contract.md`、`references/stanislavski-method-reference.md`（三层情绪模板、情感瞬间与微动态控制） |
...
| `PASS-PERF-11` | 全部表演证据嵌入既有字段；原文保真；心理、情绪、潜台词、关系和权力判断不抽象化，均落成可见/可听/可演锚点 | `FAIL-PERF-11` / `FAIL-CONCRETE-PERFORMANCE-LANGUAGE` | `references/performance-and-scene-craft-contract.md`、`../_shared/concrete-visual-language-contract.md` |
...
| `PASS-PERF-04` | `N4-PERF-ACTOR-CONTROL` | 关键情绪 beat 有五层表演证据，且对白表演有语气情绪、气口断句和声线控制；长对白逐 beat 有交付链 | `performance_task_map`、上游 `long_dialogue_beat_map`、`actor-performance-control-contract.md`、`performance-and-scene-craft-contract.md` | `actor_performance_control_evidence`、`dialogue_performance_evidence`、`long_dialogue_delivery_map`、`integration_targets` |
...
| `PASS-PERF-11` | `N7-PERF-DRAFT` | 全部表演证据嵌入既有字段，原文保真，且所有心理/情绪/潜台词/关系/权力判断落成具像表演语言 | 所有 planning evidence、`performance_style_consumption_evidence`、`audience_psychology_performance_evidence`、`emotional_register_performance_evidence`、`monologue_budget_evidence`、`advisor_consultation_packet`、`../_shared/concrete-visual-language-contract.md` | candidate `第N集.md`、`faithful_performance_trace`、`concrete_visual_language_evidence` |
| `PASS-PERF-12` | `N8-PERF-REVIEW` / `N8R-PERF-REPAIR` / `N8R-REVIEW-AGAIN` / `N9-PERF-WRITEBACK` | 阻断项已最小修复并复审通过；具像表演语言通过；未通过时不写 canonical 终稿 | candidate 表演稿、review gate、上游真源、`concrete_visual_language_evidence` | `review_result`、`repair_result`、final path |
...
- 表演层把上游已经具像化的画面、心理或导演判断重新写成“情绪复杂/关系变化/权力压迫/内心崩塌/演员克制/张力增强”等概念标签、心理解释或表演意图总结，而不是落成可见身体、可听声音、可演停顿、空间距离、道具互动或对手反应。
...
| `GATE-PERF-14` | 长对白交付链 | `long_dialogue_delivery_map`，上游长对白节拍已转为气口、停顿、重音、尾音、身体联动和对手反应链，且未改写或重切原文节拍 | blocking |
| `GATE-PERF-15` | 具像表演语言 | `concrete_visual_language_evidence`，心理、情绪、潜台词、关系、权力、演员任务、顾问意见、报告和终稿字段均落到身体、声线、呼吸、停顿、空间距离、道具互动或对手反应，没有重新抽象化 | blocking |
