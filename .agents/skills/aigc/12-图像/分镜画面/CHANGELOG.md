...
- 组词逻辑改为直接引用 `10-分组` 对应 `## x-y-z` 的完整组稿；组内普通 `分镜N` 数量决定 `n/image_count`，并映射为四段式 `x-y-z-N` 图片。
...
- 固化 step1-step4：`10-分组` 镜级提取、英文 prompt 组装、主体图片参照绑定、`.agents/skills/cli/imagegen` 批量生成 handoff。
