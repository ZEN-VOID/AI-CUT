...
| `2-Global / 3-Detail / 4-Design` 把首次落盘后的收尾继续挂在 `roles.supervision` 名下 | 角色边界层 | 在 shared `module-spec.md` 明确 `监制` 只负责前置 advisory，落盘后收尾回到阶段审计/验收层 | 共享运行时不再平行承载 post-write refine，避免把顾问机制和审计机制混层 | `2-4` 阶段不会再从 shared runtime 触发 `监制 refine` |
...
- `0-Init` 之外的共享运行时不再默认调度 `策划`；这里的前置顾问以 `监制` 为主，`评审` 只负责当前 `5-Image / 6-Video` 收口。
- 对跨阶段顾问运行时，当前更稳的分层是：`0-Init` 负责策划 kickoff，`2-Global / 3-Detail / 4-Design` 只让监制做前置 advisory，首次落盘后的收尾交回阶段审计/验收层，`5-Image / 6-Video` 仍由评审卡最终闸门，后续再单独调整。
