...
`Original/ -> 1-分集 -> 2-剧本 -> 3-分组 -> validation-report.md -> 2-Global`

口径说明：`2-剧本` 是当前业务显示名；内部 mode id 仍为 `script_format`，历史 runtime 路径仍为 `projects/aigc/<项目名>/1-Planning/2-格式/`，不得因显示名调整而重命名目录或破坏既有引用。

硬规则：

1. `.agents/skills/aigc/1-规划/1-分集`、历史 `2-格式`（业务口径 `2-剧本`）、`3-分组` 不再作为独立 `SKILL.md` 包存在。
...
`1-Planning` 的入口只咬住“项目源、上游阶段种子、规划阶段已发生产物”。具体判断、拆分、剧本化、分组过程全部导向 `references/`、`steps/`、`types/` 与 `review/`。
...
| `INPUT-STORY-SOURCE` | `projects/aigc/<项目名>/Original/` | `episode_split`、`full_chain` | 故事正文真源，优先读取 `Original/Story/` 中的原文/章节/剧本正文 | 不得用治理文档、项目 `CONTEXT/` 或预设参照替代正文 |
| `INPUT-PRESET-CONTEXT` | `projects/aigc/<项目名>/CONTEXT/` | `episode_split`、`script_format`、`grouping`、`full_chain` | 项目预设、改编参考、机制参照与补充事实 | 只作参照和辅证，不拥有分集边界真源权 |
...
| `INPUT-SPLIT-SOURCE` | `projects/aigc/<项目名>/1-Planning/1-分集/第N集.md` | `script_format`、`full_chain` | 逐集原文真源 | 不得回退到 `Original/` 或 `CONTEXT/` 自由重切 |
...
3. `story-source-manifest.yaml` 是输入索引与 readiness 证据，不替代故事正文；若 manifest 指向 `CONTEXT/` 中的预设参照，必须降级为 reference，不得抬升为分集真源。
4. 项目 `CONTEXT/` 是预设参照层，只能辅助理解设定、风格、机制和补充材料；除非用户显式指定其中某个文件就是本轮正文真源，否则不得按 `CONTEXT/` 的场次、场景或建议集数重切 `Original/`。
5. `script_format`（`2-剧本`）只消费 `1-分集` 输出物；`grouping` 只消费 `2-剧本` 主稿。
6. 未命中模式的输入不得被硬凑、补空或伪造成 ready。
...
| `OUTPUT-SCRIPT-REPORT` | `projects/aigc/<项目名>/1-Planning/2-格式/执行报告.md` | `script_format` | 全部已执行集的剧本策略、validator、返工入口与 handoff；不得为每集另建执行报告 | 父级验收 |
...
| 剧本主稿投影执行（2-剧本） | 本 `SKILL.md` 的 `script_format` 模式 | `references/script-format-contract.md` | 统一走标准剧主稿投影、保真转换和 validator 闭环 |
| 分组与节奏复核 | 本 `SKILL.md` 的 `grouping` 模式 | `references/grouping-contract.md` | 内化组边界、量化、尾钩借焰与 reviewer gate |
| 阶段共享 I/O | `references/planning-io-contract.md` | `references/planning-io-contract.md` | 锁定 `Original -> 1-分集 -> 2-剧本（runtime: 2-格式） -> 3-分组` 的项目 runtime 真源关系，并把项目 `CONTEXT/` 限定为预设参照层 |
...
| 2-剧本、标准剧主稿投影、对白旁白独白整理 | `references/script-format-contract.md`、`templates/script-format-report.template.md`、`scripts/validate_script_output.py`、`knowledge-base/script-format-heuristics.md` |
...
| 迁移追溯与旧三包去向 | `references/legacy-migration-matrix.md`、`CHANGELOG.md` |

## Mode Selection

| mode | 触发信号 | 入口输入 | 出口输出 | 过程细则 |
| --- | --- | --- | --- | --- |
| `episode_split` | 需要从 `Original/` 或 manifest 登记源生成逐集原文 | `INPUT-STORY-SOURCE`、`INPUT-STORY-MANIFEST`、按需 `INPUT-PRESET-CONTEXT` | `OUTPUT-SPLIT-SOURCE`、`OUTPUT-SPLIT-REPORT`、`OUTPUT-SPLIT-PLAN` | `references/episode-splitter-contract.md` |
...
未命中的模式不得补空目录、补默认思维链、伪造全链完成或写入未发生的 handoff。若用户只是宽泛要求“执行/继续 `.agents/skills/aigc/1-规划`”，且没有显式说“只做分集/只做格式/先停在分组前”，则三步子路径只是内部处理边界，不是用户交互断点；应一气呵成执行 `full_chain` 到阶段验收。
...
| 规划主稿执行报告 | `projects/aigc/<项目名>/1-Planning/2-格式/执行报告.md` | 全部已执行集的剧本策略、validator、返工入口 |
...
```mermaid
flowchart TD
    A["Original/ + 0-Init seeds + CONTEXT/ reference"] --> B["episode_split"]
    B --> C["script_format"]
    C --> D["grouping"]
...
| 路径 | 职责 |
| --- | --- |
| `scripts/validate_script_output.py` | 校验 `2-剧本` 标准剧结构、对白冻结、声画配对与字数回填；runtime 输入路径仍为 `2-格式/` |
...
脚本不得生成核心创作正文、不得替代 LLM 做分集、剧本主稿投影、组界审美与叙事判断。
...
| `FIELD-PLAN-04` | 剧本输出 | 主稿、标准剧本策略、validator 与 handoff 一致 | `S4` | 剧本结构稳定性 | `FAIL-PLAN-04` |
...
| `S4` | `FIELD-PLAN-04` | 主稿是否完成标准剧剧本投影 | 执行 `2-剧本` mode 并运行 validator | 跳过保真投影直接改写正文 |
...
| `FIELD-PLAN-04` | `2-剧本` 主稿（runtime: `2-格式/第N集.md`）与唯一 `2-格式/执行报告.md` 通过对应 validator 或有明确返工入口 | `FAIL-PLAN-04` | `S4` |
...
- `Original -> 1-分集 -> 2-剧本（runtime: 2-格式） -> 3-分组` 项目 runtime 真源关系混写，或把 `CONTEXT/` 参照层误当分集真源
...
- `.agents/skills/aigc/1-规划` 已具备完整 Skill 2.0 目录结构。
- 原 `1-分集 / 2-格式 / 3-分组` 细则均已保留并增强于 `references/`。
- 旧子技能入口不再作为 `SKILL.md` 包存在。
- 父包 `SKILL.md` 只承担入口、路由、动态引用、关键门禁和输出合同。
- `scripts/`、`templates/`、`agents/openai.yaml`、`README.md`、`CONTEXT.md` 与审计脚本均已同步。
