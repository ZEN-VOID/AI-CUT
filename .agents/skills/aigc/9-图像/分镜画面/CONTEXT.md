...
| --- | --- | --- | --- | --- |
| prompt 只用了单镜片段，没有完整组稿 | group source layer | 回到 `8-分组` 目标 `## x-y-z`，把完整组块作为 `group_full_content` | `G1-GROUP-SOURCE` 阻断摘要替代完整源 | prompt/report 能定位完整组稿来源 |
| 把连接件生成了图片 | connector boundary layer | 从 group index 和 plan 中删除 `## x-y-z~x-y-z` 任务 | 连接件只进 `skipped_connectors[]` | plan 中连接件任务数为 0 |
| `shot_id` 数量和组内 `分镜N` 不一致 | shot point mapping layer | 重新统计普通 `分镜N` 并生成 `x-y-z-N` | `shot_count == prompt sections == expected_image_count == returned_count` | report 中四项计数一致 |
| 多图任务缺少组级 package | handoff topology layer | 合并为每组一个 group imagegen package，并为每个 `shot_id` 建 task spec | plan schema 固定 `task_id=<group_id>.group_imagegen_package` | 同一 group 只有一个 package，task specs 覆盖所有 shot |
...
| 主体参照图错绑 | YAML reference binding layer | 回到组底 YAML 和 `3-主体/*/3-生成` 精确匹配；多视图优先 | 禁止泛词、子串和 JSON 伪图绑定 | manifest bound paths 存在且 selected_variant 正确 |
...
| imagegen 批量并发超上限 | imagegen batch layer | 拆成不超过 10 并发的批次或报告 blocked；主线程串行仅在用户显式要求时使用 | 默认最大并发 10，执行形态必须写入 plan/report | `max_concurrency <= 10` 或 blocked |
...
| 上游上下文没有转成分镜画面导向 | upstream visual direction layer | 补 `Image Upstream Visual Direction Matrix`，把美学、主体、分组稿、已有图像和项目上下文落到组级一致性、Image section、参照保真、比例/输出和 handoff | review gate 固化 `G5C-UPSTREAM-DIRECTION`，父级 `9-图像` 检查叶子报告 | 矩阵有 source anchor、stage decision、prompt/output anchor 和 boundary check |
...
3. 任何数量问题先检查 `group-index.json.shot_count`，再对照 prompt Image section 数、expected_image_count、results returned_count。
...
10. 若组内分镜数或目标组数量超过一次并发上限，按 `.agents/skills/cli/imagegen` 的最大并发 10 分批调度；主线程串行只在用户显式要求时使用。
...
13. 如果报告只列出“读取了 2-美学 / 3-主体 / 8-分组”，但没有说明这些上游信号如何改变一致性策略、单张 Image prompt、参照保真或输出 handoff，按 `FAIL-FRAME-UPSTREAM-DIRECTION` 返工；上游电影风格词只能作为证据或禁用覆盖边界。
...
- 同组多图保持一致的关键不是复用上一张生成图，而是让所有 imagegen task specs 共享参照图、组级空间模型和风格锁，并由父级汇总检查一致性。
...
- 对用户只给四段式 `shot_id` 的请求，默认回到该 `shot_id` 所在 `group_id`，因为本技能的多图一致性任务单位是组。
