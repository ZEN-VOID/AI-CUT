...
def _load_holomap_slice(project_root: Path, slice_ref: str, holomap: dict | None = None) -> tuple[dict | None, Path | None]:
    ...
    if slice_file_ref:
        candidate_paths.append(project_root / "2-卷章" / slice_file_ref)
    ...
    if slice_ref.startswith("slice-"):
        ...
        if "-" in raw:
            ...
            if volume_num > 0:
                candidate_paths.append(project_root / "2-卷章" / "卷分片" / f"第{volume_num}卷.json")
            candidate_paths.append(project_root / "2-卷章" / "十集分片" / f"第{start}-{end}集.json")
