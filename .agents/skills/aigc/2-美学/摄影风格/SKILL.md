...
`摄影风格` 是 AIGC 影片项目的摄影层级协议制定技能。它以摄影指导（Director of Photography）的身份，从上游剧本、`画面基调`、参考图、参考视频或参考作品中提取摄影语法，研究构图秩序、景别体系、机位关系/视角距离、透视/景深纪律、焦点行为、镜头运动、运动连招、镜头间流畅性、连续性、节奏承托和组合实现方式。
...
- 默认上游剧本真源为 `projects/aigc/<项目名>/1-分集/第N集.md` 或 `projects/aigc/<项目名>/1-分集/` 下的剧本；默认视觉上游为 `projects/aigc/<项目名>/2-美学/画面基调/全局风格协议.md`。
- 若输入来源或任务目标明确为 `第N集`，正式输出对象是该集摄影风格覆盖，写入 `projects/aigc/<项目名>/2-美学/第N集/摄影风格/`；只有整季、多集汇总或用户明确项目级基线时才写入非逐集 `摄影风格/`。
...
- 根据参考图、参考视频或参考作品，分析构图、景别、机位关系、视角距离、透视/景深纪律、焦点行为、镜头运动、运动连招、流畅性、连续性和镜头组合实现。
...
- `projects/aigc/<项目名>/1-分集/第N集.md`、整季 `1-分集/`、`2-编导` 目录或用户指定剧本文本。
- `projects/aigc/<项目名>/2-美学/画面基调/全局风格协议.md` 或用户指定的视觉基调文本。
- 项目初始化资料、世界观设定、用户摄影偏好、禁用运镜、平台或模型限制。
- 参考图、参考视频、参考作品名称、导演/摄影师/影片段落说明。
- 已有逐集 `projects/aigc/<项目名>/2-美学/第N集/摄影风格/摄影风格协议.md`、项目级 `projects/aigc/<项目名>/2-美学/摄影风格/摄影风格协议.md` 或候选摄影 prompt。
...
| --- | --- | --- | --- | --- | --- |
| `script_camera_analysis` | 指定 `1-分集` 文件/目录或粘贴剧本文本 | `Script Cinematography Path` | `N1,N2,N4,N5,N6,N7,N8,N9` | `CONTEXT.md` | `FAIL-CS-TYPE-SCRIPT` |
...
| --- | --- | --- |
| `single_episode_seed` | 基于单集剧本建立摄影风格覆盖 | `projects/aigc/<项目名>/2-美学/第N集/摄影风格/摄影风格协议.md` |
| `series_camera_protocol` | 基于多集、整季或项目资料建立正式项目级摄影协议 | `projects/aigc/<项目名>/2-美学/摄影风格/摄影风格协议.md` |
...
| `N2-SCRIPT-CAMERA-EVIDENCE` | 从剧本/设定抽取摄影需求证据 | `1-分集` 或文本 | 提取叙事节奏、空间压力、动作密度、情绪曲线、视角距离和段落转换需求；禁止写具体镜头正文 | `script_camera_evidence_map`，至少 5 条证据 | `N4` / `N3` | 每条证据能回指输入片段或项目资料 |
...
| `N4-CAMERA-GRAMMAR` | 汇流摄影语法 | N2/N3 证据 | 建立 8 个解析维度：构图秩序、景别体系、机位关系/视角距离、透视/景深纪律、焦点行为、镜头运动、节奏/速度密度、稳定性 | `camera_grammar_profile` | `N5` | 只保留摄影属性；具象内容必须删除 |
...
| `N7-PROMPT-DISTILL` | 蒸馏摄影风格 prompt | N4-N6 输出 | 生成约 100 字中文 `cinematography_style_prompt`；包含构图秩序、景别/机位关系、视角距离、透视/景深纪律、焦点行为、运动方式、连贯性和负面限制；过滤剧情、角色、道具、场景、单镜头正文 | `candidate_cinematography_style_prompt`、`boundary_scan` | `N8` | 80-130 个汉字为默认合格区间；无禁用类别；摄影语法明确 |
...
| `pass_threshold` | P0 gate 全部通过；`cinematography_style_prompt` 默认 80-130 个汉字；禁用类别残留 0 个，除非用户明确要求候选单镜头化且标记非正式；`GATE-CS-10-ANTI-SCRIPTED-CAMERA` 阻断项为 0 | `N7/N8.gate` | `FAIL-CS-QUANT-THRESHOLD` |
...
- `anti_scripted_style_audit` 证明摄影语法、运动连招、连续性规则和 prompt 不是模板句轮换、锚点替换或同义改写批量生成。
...
- 参考图/视频内容被照搬为项目设定。
- 摄影风格协议或 prompt 呈现脚本化生成、批量插入、正则套句、映射投影、模板句式复用、关键词锚点替换、句式轮换或同义改写批量痕迹。
...
| 摄影语法是否覆盖构图、景别、机位关系/视角距离、透视/景深、焦点行为、运动、节奏和稳定性？ | `GATE-CS-03-GRAMMAR-COVERAGE` | `FAIL-CS-GRAMMAR-MISSING` | `N4-CAMERA-GRAMMAR` | `camera_grammar_profile` |
...
| 摄影语法、运动连招、连续性规则和 prompt 是否无脚本化生成、批量插入、正则套句、映射投影、模板句式复用、关键词锚点替换、句式轮换或同义改写批量生成痕迹？ | `GATE-CS-10-ANTI-SCRIPTED-CAMERA` | `FAIL-CS-SCRIPTED-CAMERA-STYLE` | `N4-CAMERA-GRAMMAR` / `N5-MOVEMENT-COMBO` / `N7-PROMPT-DISTILL` | `anti_scripted_style_audit` |
...
- 可写摄影层级：构图秩序、景别体系、机位关系、视角距离、透视/景深纪律、焦点行为、镜头运动类型、运动速度、稳定性、节奏密度、连续性规则和负面摄影禁区。
...
- `episode_scoped`: `projects/aigc/<项目名>/2-美学/第N集/摄影风格/摄影风格协议.md`
- `episode_scoped`: `projects/aigc/<项目名>/2-美学/第N集/摄影风格/执行报告.md`
- `series_baseline`: `projects/aigc/<项目名>/2-美学/摄影风格/摄影风格协议.md`
- `series_baseline`: `projects/aigc/<项目名>/2-美学/摄影风格/执行报告.md`
...
| `FIELD-CS-02` | camera grammar | `Camera Grammar Profile` | 8 个摄影解析维度，覆盖构图、景别、机位/视角、透视/景深、焦点、运动、节奏/速度和稳定性，且只含摄影属性 | `FAIL-CS-GRAMMAR-MISSING` |
...
| `PASS-CS-07` | 摄影风格协议无模板句轮换、锚点替换或同义改写批量痕迹 | `FAIL-CS-SCRIPTED-CAMERA-STYLE` | `N4-CAMERA-GRAMMAR` / `N5-MOVEMENT-COMBO` / `N7-PROMPT-DISTILL` |
...
| 参考图/视频的构图、景别、机位关系、视角距离、透视/景深、焦点、运动和衔接 | `reference_camera_evidence_map` | `Camera Grammar Profile.evidence` |
...
当 `2-美学` 父级调度多个同级风格子技能时，`摄影风格` 默认应继承 `画面基调` 的底层视觉协议，并向后游 `分镜风格`、图像生成和视频生成阶段提供摄影层约束。它不拥有剧情、角色、场景、道具或具体镜头正文真源。下游可以继承 `Cinematography Style Prompt`、`Movement Combo Map` 和 `Continuity Rule Set`，但必须在自身合同内生成具体镜头或分镜内容。
