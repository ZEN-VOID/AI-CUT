...
This script only serves projects still using `2-卷章/全息地图.json` +
...
def main() -> int:
    ...
    planning_root_path = project_root / "2-卷章" / "全息地图.json"
    ...
    for idx, entry in enumerate(manifest):
        ...
        slice_path = project_root / "2-卷章" / file_ref
        if not slice_path.exists():
            alt_path = project_root / "2-卷章" / "卷分片" / Path(file_ref).name
            slice_path = alt_path
        if not slice_path.exists():
