...
| `N9-SB-REVIEW-REPAIR` | 审查并最小修复候选稿 | candidate、review gates | 执行 `GATE-SB-01..23`；阻断项回到 N2-N8 或 R2 最小修复，最多 3 轮；无法修复时进入阻断收束 | `review_verdict`、`repair_log`、`reference_execution_matrix`、`rule_evidence_map`、`beat_calculation_audit` | `N10` / `R1` | review 未通过不得写回 canonical |
...
| 画面点匹配、心理反应、系统画面、音效/旁白承托、分镜数量裁决、beat 计算漂移、`FAIL-SB-VISUAL-POINT`、`FAIL-SB-NONVISUAL`、`FAIL-SB-BEAT`、`FAIL-SB-SHOT-COUNT`、`FAIL-SB-BEAT-QUANT-LINE`、`FAIL-SB-BEAT-CALCULATION-DRIFT` | `references/visual-point-and-beat-contract.md` | `N3/N4` | `GATE-SB-02..04`,`GATE-SB-21`,`GATE-SB-23` | `N3/N4` | 检查 visual_point_inventory、candidate_trigger_set、state_change_cluster_map、beat_count_formula、beat_map、beat_quant_line、shot_count_decision |
...
| `CP-SB-REPORT` | 正式写回时，执行报告包含 `Execution Decision Trace`、`Beat Calculation Audit`、`Scene Axis Dictionary`、`Reference Execution Matrix`、`Rule Evidence Map`、`N/A Justification`、`Repair Log`、`Coverage Stats` 和 `Source Override Note` | 报告证据缺少 beat calculation audit、scene axis dictionary、reference matrix、rule map、N/A 或 repair log | `execution_report` | `N9-SB-REVIEW-REPAIR` |
