...
本合同监督 `场景/2-设计` 的顾问、reviewer 汇流路径。它不替代 `SKILL.md`、`steps/` 或 `review/`，只记录外部 provider 调度、不可用、本地 review 和 slot bundle 结论，避免 reviewer 路径变成口头声明。
...
| `dispatch_mode`、`blocking_layer`、`advisor_roster_source` 是否能说明外部 provider 可用性、上层阻断层级和项目 `team.yaml` 顾问来源？ | `GATE-SCENE-DESIGN-12` | `FAIL-SCENE-DESIGN-WORKFLOW` | `N7-REVIEW` | 留下 provider 状态、阻断层级、roster 来源和降级理由。 |
| 默认顾问路径启用时，`advisor_consultation_packet` 与 `advisor_node_coverage` 是否绑定当前 `node_ref / pass_ref / gate_ref`，并包含顾问 lens 对节点判断、执行取舍、局部 patch 或风险提示的影响？ | `GATE-SCENE-DESIGN-11` | `FAIL-SCENE-DESIGN-11` | `N5-RESEARCH` | 报告记录顾问问题、节点引用、gate 引用、顾问建议和进入研究/设计草稿的采纳点。 |
...
| `SCENE-BUNDLE-01.required_slots` 中的 `scene_id`、`deconstruction_subject_id`、`period_region_anchor`、`research_brief`、`source_posture`、`visual_translation`、`prompt_evidence_chain`、`deconstruction_coverage` 是否都有证据位置？ | `GATE-SCENE-DESIGN-SLOT-01` | `FAIL-SCENE-DESIGN-SLOT-01` | `N7-REVIEW` | 留下 slot evidence 表、缺槽名、证据位置和对应返工节点。 |
