...
- 关键镜级字段：目标 `分镜明细` 下的 `分镜ID`、`时间段`、`角色背景面`、`角色站位走位`、`角色表现`、`分镜表现` 及其他镜级事实
...
- `actions`:
  ...
  3. 将 `类型元素 / 导演意图 / 出场角色及穿搭 / 目标镜级字段` 压成自然句、短语或关键词串
  4. 显式消费目标镜级 `角色背景面 / 角色站位走位`，不得回退成旧字段心智
  5. 明确 `budget_state` 并登记例外说明
...
| --- | --- | --- | --- | --- | --- |
| V-VID-FFR-01 | 输入 | 目标分镜结构是否完整 | `ready/incomplete` | 检查 `分镜组ID / 剧本正文 / 组间设计（含出场角色及穿搭） / 目标分镜明细（含角色背景面/角色站位走位）` | P0 |
...
5. `prompt` 必须覆盖目标分镜所属组的上下文与该目标分镜的镜级内容，且不得漏掉组级 `出场角色及穿搭` 与镜级 `角色背景面 / 角色站位走位`。
6. `全局风格` 必须原文保留，不得改写。
...
| FIELD-VID-FFR-02 | `prompt / prompt_char_count` | prompt 必须覆盖目标分镜的剧情桥段、全局风格和压缩后的上下文；压缩块需显式覆盖 `类型元素 / 导演意图 / 出场角色及穿搭 / 目标镜级字段`，且隐藏字段标题 | `剧本正文`、`组间设计.*`、目标镜级字段 | N4-N7 | Prompt 蒸馏稳定性 | FAIL-VID-FFR-02 |
...
- `角色背景面 / 角色站位走位` 是否已被消费，而非继续按旧字段名理解
...
- prompt 里仍然残留旧字段标题，或因 schema 升级后漏掉 `出场角色及穿搭 / 角色背景面 / 角色站位走位`
