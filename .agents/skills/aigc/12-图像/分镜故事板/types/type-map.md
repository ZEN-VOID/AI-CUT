...
默认加载 `types/storyboard-sheet-default.md`。生成、修复或审查时再按 mode 额外读取 `SKILL.md` 主节点、`review/review-contract.md` 或 provider handoff 合同。
...
| `imagegen_route` | `built_in / cli_confirmed / blocked` | imagegen 路由；无论内置还是显式确认 fallback，都必须直接调用 `.agents/skills/cli/imagegen` 生成图片，不能停在 plan |
...
5. `imagegen_route == built_in`，除非用户显式确认 CLI/API fallback；必须进入 `N7-IMAGEGEN` 并产生项目内图片路径，不得 plan-only 结束。
