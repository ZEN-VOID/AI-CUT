...
def _load_context_return_module():
    _ensure_scripts_on_path()
    import context_return_manager as context_return_module

    return context_return_module
...
def test_context_return_manager_blocks_non_pass_validation(tmp_path, monkeypatch):
    module = _load_context_return_module()
...
def test_context_return_manager_applies_projection_refresh_modes(tmp_path, monkeypatch):
    ...
    assert state["runtime_markers"]["context_return_state_revision"] == 1


def test_context_return_manager_writes_artifact_and_applies_writebacks(tmp_path, monkeypatch):
    module = _load_context_return_module()
    project_root = (tmp_path / "book").resolve()
    _build_project(project_root)
    ...
    assert state["runtime_markers"]["context_return"]["last_actualized_episode"] == "第12章"
    assert state["runtime_markers"]["context_return"]["last_commit_manifest"]["phase"] == "committed"
    assert state["runtime_markers"]["context_return_state_revision"] == 1


def test_context_return_manager_writes_slice_actualization_and_root_indexes(tmp_path, monkeypatch):
    module = _load_context_return_module()
    project_root = (tmp_path / "book").resolve()
    _build_project(project_root)
...
def test_context_return_manager_rolls_back_on_commit_failure(tmp_path, monkeypatch):
    module = _load_context_return_module()
    project_root = (tmp_path / "book").resolve()
    _build_project(project_root)
    ...
    def flaky_atomic_write_json(path, payload, use_lock=True, backup=True):
        path_obj = Path(path)
        if path_obj.name == "第2卷.context-return.json":
...
def test_context_return_manager_writes_nested_card_schema_state(tmp_path, monkeypatch):
    ...
    assert nested["context_return_revision"] == 1
    assert "current_state" not in card
    assert "history" not in card


def test_context_return_manager_rejects_revision_drift(tmp_path, monkeypatch):
    module = _load_context_return_module()
    project_root = (tmp_path / "book").resolve()
    _build_project(project_root)
...
def test_context_return_manager_rejects_non_whitelisted_delta_fields(tmp_path, monkeypatch):
    module = _load_context_return_module()
    project_root = (tmp_path / "book").resolve()
    _build_project(project_root)
