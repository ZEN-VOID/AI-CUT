...
| `constraint_profile` | 当前 root 必须已经有 `分镜构图`；摄影只能强化观看压力和材料气候，不能反向改镜头骨架、动作事实或镜序。 |
| `success_criteria` | 每个命中镜头都获得可消费的 `摄影美学`；branch sidecar 保留思考过程；输出包含 `visual_control_line + cinematography_strategy_note + lighting_strategy + group_lighting_note + color_strategy + texture_strategy`。 |
| `non_goals` | 不生成第二份 episode 主稿；不单独创造统一导演 prose；不把 `光影/色彩/质感` 各写成互相竞争的总稿。 |
| `complexity_source` | 复杂度来自“必须先锁控制线，再让三个叶子沿同一控制线串行推进，再汇流成单一摄影判断”，而不是来自字段数量。 |
| `topology_fit` | 固定为 `当前 root 锁定 -> 摄影底座回收 -> 控制线 -> 策略锁定 -> 光影 -> 色彩 -> 质感 -> 汇流 patch -> 自检与 sidecar`。 |
| `step_strategy` | 先锁 branch 主张和控制线，再让叶子按固定顺序吸收，不允许叶子倒逼主线。 |
...
| `CG-S2-BASELINE` | `S2` | 回收摄影底座与风格走廊 | `分镜构图`、`组间设计`、项目风格承诺 | 提炼必须继承的摄影承诺、禁区和戏剧焦点 | `cinematography_brief` | 底座发虚 -> `FAIL-CG-02` / 通过 -> `CG-S3` | 能说清“必须继承什么、禁止什么” |
| `CG-S3-CONTROL-LINE` | `S3` | 锁定 `visual_control_line` | `cinematography_brief`、当前镜头阅读需求 | 明确主体控制、空间剥离、质感方向、观看压力 | `visual_control_line` | 控制线不完整 -> `FAIL-CG-03` / 通过 -> `CG-S4` | 四项必须同时成立 |
| `CG-S4-STRATEGY` | `S4` | 压一句稳定贯穿的摄影美学 | `visual_control_line`、`分镜构图` | 写 `cinematography_strategy_note`，记录稳定判断与放弃项 | `cinematography_strategy_note` | 仍是漂亮话 -> `FAIL-CG-04` / 通过 -> `CG-S5` | 句子必须能驱动叶子顺序执行 |
| `CG-S5-LIGHTING` | `S5` | 先落光影主线 | `visual_control_line`、`cinematography_strategy_note`、光影叶子合同 | 生成 `lighting_strategy` 与关键光影证据 | `lighting_strategy` | 光源无来源或压没信息 -> `FAIL-CG-05` / 通过 -> `CG-S6` | 光影必须服务可读性与压力 |
| `CG-S6-COLOR` | `S6` | 在既有控制线下补色彩关系 | `visual_control_line`、`cinematography_strategy_note`、色彩叶子合同 | 生成 `color_strategy` | `color_strategy` | 只有感受词 -> `FAIL-CG-06` / 通过 -> `CG-S7` | 色相/明度/饱和度/色温至少可执行 |
| `CG-S7-TEXTURE` | `S7` | 在既有控制线下补质感收益 | `visual_control_line`、质感叶子合同 | 生成 `texture_strategy` | `texture_strategy` | 质感喧宾夺主 -> `FAIL-CG-07` / 通过 -> `CG-S8` | 质感只能强化，不得抢主线 |
| `CG-S8-CONVERGE` | `S8` | 汇流为单一摄影判断 | `lighting_strategy`、`color_strategy`、`texture_strategy` | 先写 `group_lighting_note`，再压成统一 `摄影美学` patch | `group_lighting_note`、`cinematography_patch` | 汇流后仍像三段摘要 -> `FAIL-CG-08` / 通过 -> `CG-S9` | 必须保留单一主收益 |
...
| `FIELD-CG-02` | `cinematography_brief` | 能说清摄影底座、风格走廊、禁区与戏剧焦点 | `组间设计`、项目风格承诺 | `S2` | 约束继承 | `FAIL-CG-02` | `CG-S2-BASELINE` |
| `FIELD-CG-03` | `视觉控制线` | 同时回答主体控制、空间剥离、质感方向、观看压力 | `cinematography_brief` | `S3` | 控制线完整性 | `FAIL-CG-03` | `CG-S3-CONTROL-LINE` |
| `FIELD-CG-04` | `cinematography_strategy_note` | 一句可驱动后三叶子的稳定判断 | `visual_control_line` | `S4` | 策略可执行性 | `FAIL-CG-04` | `CG-S4-STRATEGY` |
| `FIELD-CG-05` | `光影策略` | 光影有来源、有戏剧用途，并形成组内流动基础 | 光影叶子产物 | `S5` | 光影可读性 | `FAIL-CG-05` | `CG-S5-LIGHTING` |
| `FIELD-CG-06` | `色彩策略` | 色相/明度/饱和度/色温可执行，不只是感受词 | 色彩叶子产物 | `S6` | 色彩执行度 | `FAIL-CG-06` | `CG-S6-COLOR` |
| `FIELD-CG-07` | `质感策略` | 表面行为可见，且不抢人物与动作主线 | 质感叶子产物 | `S7` | 质感节制 | `FAIL-CG-07` | `CG-S7-TEXTURE` |
| `FIELD-CG-08` | `组内光影流动 + 摄影美学` | 汇流后保留单一摄影主收益，不退回三段并列摘要 | 汇流结果 | `S8` | 收束稳定性 | `FAIL-CG-08` | `CG-S8-CONVERGE` |
...
4. 若最终 `摄影美学` 仍像三段字段摘要，必须回退到 `CG-S4` 或 `CG-S8`，重新压策略和汇流。
