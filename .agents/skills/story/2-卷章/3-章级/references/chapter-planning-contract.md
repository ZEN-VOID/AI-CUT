...
| `success_criteria` | drafting 读取 `第N卷/第N章.md` 时，可以清楚知道这一章在故事世界里何时发生、事件按什么顺序推进、章末造成什么状态变化、该推进什么、留下什么、避开什么，并知道本章支流任务是当章汇聚、延后转挂，还是继续保留开放。 |
...
5. `本章节奏曲线` 必须写明 `selected_pack / selected_mode / mode_selection_reason / payoff_type / rhythm_intensity / previous_next_contrast / 七步职责映射 / 规划义务 / 义务段位 / 建议写法`，并附 Mermaid 图。
6. `payoff_type` 与 `micro_payoff` 必须消费 `本章爽点设计`，不得另起一套读者满足判断。
7. `本章任务线` 必须至少写清 `上承卷级任务 / 主线 / 支线 / 支流角色 / 汇聚动作 / 未汇聚任务去向`。
8. `本章线索` 负责当前章可见信息推进。
9. `本章伏笔` 必须拆成 `铺设 / 兑现`，允许某一项为空，但段落必须存在。
10. `规划义务` 中必须显式锁定 `entry_promise / conflict_axis / micro_payoff / exit_hook`。
11. `义务段位` 只写必须兑现的结构义务，不得把建议写法偷渡成硬法律。
12. `建议写法` 只提供推荐编排，不得直接写正文句段。
13. 章级文件只能写规划，不得直接写对白、叙述段落或正文桥段。
14. 任何章级局部修订都必须以上游卷级文档为直接上下文、部级文档为总上下文；缺任何一层都不得落盘。
