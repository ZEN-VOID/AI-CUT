...
8. `references/character-design-assembly.md`
9. `templates/character_masterprompt.structured.v2.md`
10. `projects/aigc/<项目名>/4-Design/角色/1-清单/第N集/role_design_bridge.json`（若存在）
11. `projects/aigc/<项目名>/4-Design/角色/1-清单/第N集/角色研究.json`（若存在）
12. `projects/aigc/<项目名>/4-Design/角色/1-清单/第N集/角色清单.json`
13. `projects/aigc/<项目名>/0-Init/{north_star,init_handoff,story-source-manifest}.yaml`
14. `projects/aigc/<项目名>/2-Global/全局风格.md`
15. `projects/aigc/<项目名>/2-Global/全集类型元素.md`
16. `projects/aigc/<项目名>/2-Global/导演意图.md`
...
| `FIELD-CHAR-11` | `[角色名].<ext> / _manifest.json.auto_image` | 自动生图使用含全局风格前缀的完整 prompt，图片与设计文件同目录同 stem | `design-output-contract.md`、`全局风格.md`、`[角色名].md` | `S10` | auto image completeness | `FAIL-CHAR-AUTO-IMAGE` |
...
| `S10` | 单主体自动生图 | 读取 `[角色名].md` 与全局风格前缀；自动生图前复验纯色背景锚句，再调用 nano-banana general 输出同目录同名图片 | `auto_image_packet + reference_cleanliness_note` | `done` | `S7-S10` |
...
| `N10-AUTO-IMAGE` | 生成单角色概念图 | `[角色名].md`、`全局风格.md`、`prompt_integration` | 生成 `full_generation_prompt`；复验纯色背景锚句与背景污染禁令后调用 nano-banana general，输出 `[角色名].<ext>` | `auto_image_packet + reference_cleanliness_note` | `done` | 不得只传局部 prompt；图片必须同目录同名；纯色背景门禁失败不得生图 |
...
| `FIELD-CHAR-11` | `[角色名].<ext>` 已由 `full_generation_prompt` 生成，且 stem 与 `[角色名].md` 一致 | `FAIL-CHAR-AUTO-IMAGE` | `S10` |
...
6. 已使用含统一全局风格前缀的 `full_generation_prompt` 自动生成 `[角色名].<ext>`。
