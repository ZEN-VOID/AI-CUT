...
本合同为 `2-编剧` 提供新增细则：题材类型解析、叙事情节解析、编剧大师改编逻辑、爆款短剧节奏表现手法、匹配机制、套用逻辑、高潮强化和尾钩设计。它补充 copied references，不覆盖 `SKILL.md`、保真边界或输出路径。
...
| principle | usable rule in `2-编剧` | forbidden misuse |
| --- | --- | --- |
...
| 陈述转行动 | 重要陈述优先转成对白、独白、喊出式台词、道具证据、动作结果或场内声音 | 把信息全塞旁白或字幕 |
