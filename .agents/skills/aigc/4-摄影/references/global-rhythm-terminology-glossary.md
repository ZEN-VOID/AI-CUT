...
本词汇表为 `4-摄影` 的全局节奏术语标准。所有 reference 文件中的节奏相关术语必须与本词汇表保持一致；若本词汇表与引用文件出现不一致，以本词汇表为准，并在更新引用文件后同步更新本词汇表。
...
| 节拍触发与时间段数 | `beat-analysis-contract.md` | 单个 `visual_unit` 内的切换点 | `beat_trigger`、`shot_count_decision`、`BT-01~BT-15` |
...
描述整段内每个 phase 的时间段密度倾向。

| 状态词 | 含义 | 适用场景 |
| --- | --- | --- |
| `conserve` | 收敛：每画面 1 段，低信息、低运动 | 日常建立、动作承接、读秒停顿、边界交出 |
| `measured` | 标准：每画面 1-2 段，信息与节奏平衡 | 常规叙事、标准对话、空间交代 |
| `build` | 建构：每画面 2-3 段，密度渐升 | 威胁逼近、情绪积累、规则显影准备 |
| `burst` | 爆发：每画面 3-4+ 镜，高密度冲击 | 动作高点、惊吓、惩罚、群像恐慌扩散 |
| `hold` | 停顿：峰值后收束，1-2 段读秒或反应 | 高点后反压、结果钉镜、余波吸收 |
...
| `recovery_slots` | 爆点后恢复位置 | 2 段（结果钉镜/普通人反应） |
...
| --- | --- | --- |
| `hold` | 静止或极慢，停顿感 | standard / held，内部 1.5-3.5 秒，落盘为 `[起始秒-结束秒]`；低信息交出可压短 |
| `slow_burn` | 极慢推进/拉，悬疑感 | standard / held，内部 2-3.5 秒，落盘为 `[起始秒-结束秒]` |
| `steady` | 中速稳定，观察感 | short / standard，内部 1.5-3 秒，落盘为 `[起始秒-结束秒]` |
| `quick` | 快速运动，冲击感 | short，1-2 秒 |
| `rupture` | 断裂跳切，急停/光变 | instant，<1 秒 |

### 3.3 Density（信息密度感）对应

| rhythm_profile.density | 描述广度 | 时间段数量倾向 |
| --- | --- | --- |
| `sparse` | 一行完成，单一运动 | 1 段 |
| `measured` | 标准展开，单一主体 | 1-2 段 |
| `rich` | 组合运镜，多重信息 | 2-3 段 |
| `maximal` | 多时间段高密度，复杂运动 | 3-4 段 |
...
`节拍` 在 `4-摄影` 中等价于“有效时间段触发点”。快节奏短视频平台默认采用 `trigger-first` 口径：`BT-01~BT-15` 命中的有效触发点默认 1:1 落为一个 `[起始秒-结束秒]`。只有当多个触发点能在同一时间段内清楚完成，且不损失平台节奏、观看结果、下游 payload 或人物动作连续性时，才合并。

| 术语 | 含义 | 落盘关系 |
| --- | --- | --- |
| `beat_trigger` | 有效触发点，说明哪里需要换观看策略、平台刺激、信息可读性或声画打点 | 默认 1 个触发点 = 1 个 `时间段` |
| `trigger_merge_exception` | 合并例外，说明多个触发点可在同一时间段内完成 | 只在不损失观看结果时使用 |
| `shot_count_decision` | 对当前 `visual_unit` 最终时间段数的裁决 | 必须说明每个 `时间段` 对应哪个触发点或合并例外 |

### 4.1 Beat Trigger Matrix（BT-01~BT-15）
...
### 4.2 Shot Count Decision（时间段数裁决）

| 条件 | 允许时间段数 | 必须满足 |
| --- | --- | --- |
| 低信息/单一观看 | 1 段 | 一个镜头完成观看策略，不得硬补 |
| 存在第二个有效触发点 | 2 段 | 建立后揭示、动作后反应、环境后可读细节、平台钩子后结果等 |
| 关键规则显影/动作分相/群像恐慌/高潮承托/空间重置/平台钩子连续推进 | 3-4 段 | 每段必须有新触发、新主体、新动作相位、新信息、新交接或新叙事节奏价值 |
| 连续动作/声画打点（set-piece-chain 例外） | 5-6 段 | 每段独立起点/撞点/结果/声音，删掉任一段少一节拍 |
...
| `short` | 短镜，内部 1-2 秒，落盘为 `[起始秒-结束秒]` | 用于动作通过、快速切入、hold 节奏 |
| `standard` | 标准镜，内部 2-4 秒，落盘为 `[起始秒-结束秒]` | 用于标准叙事、观察、空间交代 |
| `held` | 读秒镜，内部 3-6 秒，落盘为 `[起始秒-结束秒]` | 用于关键文字/道具/微表情可读、表演停顿、高点读秒 |
| `long_hold` | 长停顿，>6 秒 | 用于情绪峰值、关系暖点、规则显影、slow_burn |

**短剧·AIGC 压缩规则**：
- 默认优先 `short / standard`，普通氛围段、过场动作、常规反应不得沿用传统影视宽停顿
- 超过 3 秒的时间段必须有台词、读秒、表演变化、复杂调度、空间重置或高点证据
- AIGC 工具片段时长不得反推拉长单段叙事时值
...
→ 术语：`beat_trigger`、`BT-01~BT-15`、`trigger_merge_exception`、`shot_count_decision`
