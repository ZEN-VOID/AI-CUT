...
3. `镜头类型`
   最后回答观众怎么被带着看。这里的 `镜头类型` 是上位叶子，内部统领 `镜头类型 / 镜头框架 / 镜头视角` 等 shot descriptor、POV、观看姿态与镜头角度概念，不指摄影器材型号。

注意：

- 外部教程里常把 `zoom out / close-up / wide shot / off-center / low angle` 一起叫“镜头”。
- 落到本技能树时必须拆分：`zoom out / close-up / wide shot` 先归 `景别景深`，`off-center` 先归 `构图形式`，`low/high/overhead/OTS/POV/dutch` 这类观看立场才归 `镜头类型`。
- `镜头类型` 叶子吸收这些外部教程时，主要负责的是“观众站位”和 descriptor 协调，而不是接管所有镜头术语。
...
- 这组更适合哪种景别曲线
- 哪些镜头需要逼近，哪些镜头需要保持距离，哪些镜头需要纳入环境或压缩关系
- 前中后景的层级怎样服务主冲突和主视线
- 是否需要前景遮挡、主体重叠、相对尺寸差异来制造 Z 轴深度提示
- 视觉焦点如何唯一或形成主次排序
- 当前镜更适合开放景框还是封闭景框；若开放，观众被引向哪侧画外
- 心理距离如何随着情绪升降改变

这里的 `景深` 是前中后景、焦点层级与观看深度，不是去决定具体镜头焦段或光圈值；这里的“广/窄”也是纳入/排除信息的空间取向，不是器材表。
...
flowchart TD
    ...
    C --> D["视觉焦点 + 开放/封闭景框"]
    D --> E["心理距离 + 误读校验"]
    E --> F["shot_size_rhythm_preview"]
```

### 5. 最后用 `镜头类型` 锁戏剧任务、角度、POV 与 descriptor

`镜头类型` 在本模块里回答的是：

- 这组镜头先要让观众感到什么，是力量、弱势、观察、进入、关系压制还是危险失衡
- 低角度、高角度、侧面、背身、顶视、过肩、POV 或荷兰角中，哪类角度最适合这个任务
- 观众是旁观、贴身、主观、压视还是窥视
- POV 站在哪一边，是否允许偏置
- 是否需要 `过肩 -> POV` 的关系链，或 `低角度 + 荷兰角` 的异常时刻组合
- `镜头类型 / 镜头框架 / 镜头视角` 这些专门镜头概念怎样被 `镜头类型` 叶子统领并锁住
- `景别 / 镜头框架 / 镜头类型 / 镜头视角` 这些 descriptor 槽位怎样锁住
...
```mermaid
flowchart TD
    A["锁戏剧任务"] --> B{{"要抬高 / 压弱 / 观察 / 进入 / 失衡?"}}
    B -->|"抬高"| C["低角度"]
    B -->|"压弱"| D["高角度 / 顶视"]
    B -->|"观察"| E["侧面 / 背身"]
    B -->|"关系"| F["过肩"]
    B -->|"进入主观"| G["POV"]
    C --> H["锁 POV 与 descriptor"]
    D --> H
    E --> H
    F --> H
    G --> H
    H --> I["收束 focus/spatial logic"]
```
...
| `SHOT-N3-SIZE-DEPTH` | 锁 `景别景深` | 这组更适合怎样的景别曲线、空间偏置、观看深度、视觉焦点与画外策略 | 汇总 `shot_size_rhythm_preview`，明确景别曲线、空间纳入/压缩倾向、Z 轴层级、焦点/景框策略与心理距离 | `shot_size_rhythm_preview` | 若把景深写成摄影参数、开放景框没有画外引导、或焦点无法解释情感距离，不得继续 |
| `SHOT-N4-TYPE` | 锁 `镜头类型` | 这组先要让观众感到什么；观众站在哪边看；该用哪类角度与 descriptor 锁稳 | 汇总 `pov_strategy_preview + shot_descriptor_lock + focus_spatial_logic`，并先锁戏剧任务与角度家族 | `focus_spatial_logic` | 若镜头类型漂成器材、运镜路线或空泛“电影感”话术，不得汇流 |
...
- `镜头类型` 是否已锁戏剧任务与角度家族，而不是只停留在观看姿态的抽象空话。
- `镜头类型` 是否仍停留在观看姿态、角度与 descriptor，而没有漂成器材或运镜说明。
...
- 若开放景框却说不清观众该看哪侧画外，说明景别景深还没完成阅读控制。
- 若前景遮挡没有偷窥、压迫、引导或深度提示任务，说明层级设计是假动作。
- 若把 `镜头类型` 写成摄影机型号或镜头运动方案，说明 descriptor 边界失真。
- 若只写“有故事感、很电影、很有压迫感”，却没有角度任务，说明镜头类型还没锁住。
