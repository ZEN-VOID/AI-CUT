...
| --- | --- | --- | --- |
| `business_goal` | 在保留初稿事实、骨架、人物气口和文本分布的基础上，修掉明显表达坏处并输出可交付润色稿。 | `repair_plan`、`polished_markdown` | `FAIL-POLISH-TYPE` |
...
| `P3-REPAIR-PLAN` | 确定最小修补范围 | `source_lock`、`context_pack`、`repair_brief`、授权 types | 标注坏点、影响段落、可动范围和禁止改动事实；按需判定 `character_reaction_repair`、`prose_texture_repair`、`visual_readability_repair`、`genre_scene_repair` 或 `wuxia_blade_qi_repair`，命中类型场面时建立 `repair_type_package_manifest` 与 `genre_scene_route` | `repair_plan`、`repair_type_profile`、`repair_type_package_manifest` 或 N/A、`genre_scene_repair_profile` 或 N/A | `P3A` | 默认不扩大为整章重写；不得把局部质感或类型场面问题扩大成全章加料 |
...
| `P4-CREATIVE-POLISH` | LLM-first 润色正文 | 源章 affected span、repair plan、`live_quality_brief`、`repair_type_package_manifest`、授权 reference/types | 当前执行 LLM 按源章逐段理解后做最小修补；只在 affected span 修复人物反应、场景压力、视觉可读性、类型化场面或中文表达坏点；白刃剑气流命中时只补出刃路径、实体接触、材质响应、人物代价和余波留痕，不改胜负或能力规则 | `polished_markdown`、`character_reaction_patch`、`prose_texture_patch`、`visual_readability_patch`、`genre_scene_patch` 或 N/A、`wuxia_blade_qi_patch` 或 N/A、`creative_engine_note` | `P5` | 禁止脚本主创和模板灌字；不得为表演、氛围、光影或类型场面新增无源装饰 |
...
11. 输出必须是完整章节 prose，不得变成点评、建议、差异说明或多个版本。
...
| `genre_texture_density` | 保留并强化题材质感、场景密度、信息延迟、心理暗流、对白锋利度、人物在场反应、视觉可读性和句群节奏 | `FAIL-POLISH-TEXTURE` | `P4-CREATIVE-POLISH` |
...
| 是否保留并强化题材质感、场景密度、人物在场反应、视觉可读性、句群节奏和追读力？ | `genre_texture_density` / `reader_pull` | `FAIL-POLISH-TEXTURE` / `FAIL-POLISH-READER-PULL` | `P4-CREATIVE-POLISH` | before/after evidence |
...
- Completion gate: 源初稿已读取；LLM-first 润色完成；命中类型化场面修复时 `repair_type_package_manifest`、`genre_scene_repair_profile` 与 before/after evidence 通过；内置自动验收通过；写回 canonical path 和验收包；已记录或说明 `record-skill-completion` 状态 hook。
