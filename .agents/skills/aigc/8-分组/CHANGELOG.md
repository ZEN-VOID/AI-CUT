...
## 2026-06-15 (Skill 2.0 Delivery Compliance Upgrade)

- 按最新版 `skill-2.0` runtime-spine delivery gate 补齐 `Context Processing Contract`、`LLM-First Creative Authorship Contract`、`Multi-Subskill Continuous Workflow`、`Field Mapping`、S20 注意力协议、S20 检查点和 `Runtime Guardrails`。
- 同步 `Module Loading Matrix` 根目录授权与 `Module Trigger Matrix` fail-code 覆盖，补齐 `episode_range`、`all_ready_episodes` 类型路由，并修复旧 `N5/N7` reference 回流落点。
- 新增 `types/type-map.md`，将 `test-prompts.json` 改为顶层数组 schema；模板 YAML 示例改为 subject registry `id + name` 形状。
- 同步 README、review verdict、output template 和 CONTEXT 经验层；验证以 `validate_skill_2_0.py --mode delivery` 与 `smoke_test_skill_2_0.py --mode delivery` 为完成门。
...
- 将分镜组目标时长调整为约 14.5 秒；通常 10-11.5 秒可接受，14.5 秒为硬上限。
...
- 将 `8-分组` 升级为 Skill 2.0 runtime-spine 主合同：补齐 Business Requirement、Type Routing、Thinking-Action Node Map、Module Loading / Trigger、Convergence、Review Gate、Quantifiable Criteria、Attention、Checkpoint、Evaluation Prompt 与 Learning Writeback。
- 默认上游从旧分组/摄影路径收束为 `projects/aigc/<项目名>/7-摄影/第N集.md` 摄影稿；用户显式指定其他文稿时优先 source override，并在报告中声明 skipped camera-source-specific checks。
- 删除 `steps/grouping-workflow.md`，节点真源回收到 `SKILL.md`；补充 `test-prompts.json` 作为回归评估资产。
- 同步模板、README、类型包、references、review、入口元数据、validator 和根路由/registry 中的旧 `5-分组` 引用到 `8-分组`。

## 2026-06-01 (Direct Screenplay Intake)

- 新增 `source_state=direct_screenplay` 源层规则：当用户要求 `8-分组` 直接接手剧本/编导稿，或项目缺少 `7-摄影` 但存在可读剧本源时，不再阻断到 `7-摄影`。
- 明确 direct screenplay 路径由 LLM 按剧本声画 atomic unit 自动规划约 15 秒/组，补写连续 `[N-N秒]` 时间码、组级 `画面风格：`、首帧衔接和 YAML 统计；脚本仍只做机械校验。
- 要求 frontmatter 与执行报告声明剧本源路径、`source_state=direct_screenplay`、时间码由本阶段规划，以及 `7-摄影` 字段级 diff 不适用。
...
- 将 `8-分组` 的正文继承口径同步为直接保留 `7-摄影` 的原画面性字段标题，不再把画面字段替换或包裹为 `分镜画面：`。
...
- 同步入口合同、boundary/statistics/entry/exit/north-star/review/workflow/type/template/validator 口径；legacy 旧式时长不再计入 canonical 分组时长，validator 会要求回到 `7-摄影` 迁移。

## 2026-06-01 (Init-Only Team Synthesis)

- 接入冻结初始化综合消费：读取 `team.yaml.init_synthesis.stage_seed_summary."8-分组"`、`init_handoff.grouping_seed` 与 `north_star.yaml.创作阶段不变量.分组`，形成 `init_team_synthesis_context`。
...
- 在 workflow 中新增 `N5E-SUBJECT-EVIDENCE` 取证节点，从 `7-摄影` 的画面、角色动作、镜头设计、分镜明细、对白主体和道具特写建立主体/空间/镜头证据表，再进入入场落盘。
...
- 明确跨组连续分镜序列感只能通过入场镜头和画面属性说明“同一尾帧状态换景别/机位/焦点/构图/观看距离进入”，不得改写 `7-摄影` 原正文或原有 `分镜明细`。
...
- 将单个分镜组显式时长累计 `>18s` 从语义 warning / 可声明完整性例外，升级为硬性 error；`8-分组` 不再允许用同画面完整、对白承托、动作完成或场景完整性放行超 18 秒组。
- 若单个 atomic unit 自身超过 18 秒，新增回退口径：必须返回 `7-摄影` 拆分画面单位、压缩镜头时值或重裁分镜数量后再重新分组。
...
- 将 15 秒/组的边界裁决从字数中心切换为显式时长累计中心：优先累计 `7-摄影` 的 `分镜N（约X秒）` 接近 15 秒，允许约 12-18 秒弹性，不为精确卡点拆断同一画面。
...
- 调整字数统计口径：1680 常规目标和 1980 硬上限只约束从 `7-摄影` 划定的纯分镜剧本正文；组标题、north_star、YAML 与组间连接件均不计入。
