...
`1-分集` 是 `1-Planning` 单包内的 `episode_split` mode，负责把 `projects/aigc/<项目名>/Original/` 中的故事正文真源收束成逐集原文真源，并为 `2-剧本` 与下游 `2-Global` 生成稳定 handoff。项目 `CONTEXT/` 只作为预设参照、机制参照和补充事实层，不默认拥有分集边界真源权。旧版 direct leaf skill 的完整执行细则在本文件中保留为 mode contract。

本技能在内容与机制上继承当前 DREAMER 规划链既有配置：

- 默认故事源真源根是 `projects/aigc/<项目名>/Original/`，优先扫描 `Original/Story/`
- 默认预设参照根是 `projects/aigc/<项目名>/CONTEXT/`
...
| `business_object` | `projects/aigc/<项目名>/Original/` 下的故事正文、manifest 索引与 init 种子；`CONTEXT/` 仅作预设参照 |
...
3. `.agents/skills/aigc/1-规划/SKILL.md + CONTEXT.md`
4. 本 `SKILL.md + CONTEXT.md`
5. `.agents/skills/aigc/1-规划/references/planning-io-contract.md`
...
10. `projects/aigc/<项目名>/Original/` 相关内容，优先 `Original/Story/`
11. `projects/aigc/<项目名>/CONTEXT/` 中与规划阶段相关的预设参照（只作辅证）
12. `projects/aigc/<项目名>/0-Init/story-source-manifest.yaml`（若存在）
13. `templates/episode-split-plan.template.json`
...
1. 默认故事源真源根必须是 `projects/aigc/<项目名>/Original/`，优先 `Original/Story/`；若旧项目仍使用 `Story/`，必须在执行报告中标记 legacy fallback。
...
- `projects/aigc/<项目名>/Original/` 相关内容，优先 `Original/Story/`
...
- `projects/aigc/<项目名>/CONTEXT/` 中与规划阶段相关的预设参照、机制参照或补充事实
  - 默认只作边界辅证、设定补充和风格/机制约束
  - 不得抬升为 `primary_story_source`
- `projects/aigc/<项目名>/Original/` 或 manifest 登记的 `development_briefs`
...
- 如果执行案 / 提案 / 结构蓝图 / 项目 `CONTEXT/` 参照文件已经被 `story-source-manifest.yaml` 显式登记为当前项目的 `development_briefs`，且 `readiness` 结论是“可以开发式/增量分集”，则这些材料不再视为“无关输入”，但仍只拥有辅证地位。
- 辅证地位不等于正文真源：不得据此宣称整季正文 ready，不得覆盖 `primary_story_source` 的权属。

### 输入处理原则

1. 用户显式指定路径或范围时，用户指定优先。
2. 用户未指定时，只能从 `projects/aigc/<项目名>/Original/` 扫描有效正文，优先 `Original/Story/`。
3. manifest 只承担索引与证据，不替代故事正文主源；若 manifest 指向 `CONTEXT/`，而 `Original/` 有正文，则必须报告路径漂移并降级为参照。
4. 若 manifest 显式登记了 `development_briefs` 或项目 `CONTEXT/` 且当前是 `incremental`，可把这些材料作为“开发式分集”的辅证输入，但必须在执行报告和 `episode-split-plan.json` 中显式保留 coverage 缺口。
...
```mermaid
flowchart TD
    A["锁定 Original 故事源与 init 种子"] --> B["判定 readiness 与 coverage"]
...
```mermaid
graph LR
    A["Original/ + north_star + init_handoff + CONTEXT/ reference"] --> B["FIELD-SPLIT-01 输入清单"]
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| N1-INPUT-LOCK | S1 | `FIELD-SPLIT-01` | 锁定唯一故事输入范围 | 读取 `Original/`、`north_star`、`init_handoff`、manifest，并把 `CONTEXT/` 标为参照层 | 输入清单、coverage 范围、参照清单 | 成功 -> N2；输入漂移 -> 返工 S1 | 输入真源唯一后方可继续 |
...
| --- | --- | --- | --- | --- | --- |
| FIELD-SPLIT-01 | 输入清单 | 列出 `projects/aigc/<项目名>/Original/` 命中内容、有效覆盖范围与 `CONTEXT/` 参照清单 | S1 | 输入真源一致性 | FAIL-SPLIT-01 |
...
| --- | --- | --- | --- | --- |
| S1 | FIELD-SPLIT-01 | 输入范围是否唯一且可追溯 | 读取 `Original/` 与 manifest（若有），按需读取 `CONTEXT/` 参照 | 临时猜路径或把参照层当真源 |
...
| --- | --- | --- | --- |
| FIELD-SPLIT-01 | 输入来自 `Original/`，`CONTEXT/` 仅作参照，且 coverage 可追溯 | FAIL-SPLIT-01 | S1 |
...
- 读错 `projects/aigc/<项目名>/Original/` 输入范围
- 把 `projects/aigc/<项目名>/CONTEXT/` 或其他非故事正文材料当主故事源
...
- 已从 `projects/aigc/<项目名>/Original/` 锁定输入范围，并在 manifest 存在时完成索引对齐；项目 `CONTEXT/` 参照已被标明为 reference，不参与覆盖正文真源。
