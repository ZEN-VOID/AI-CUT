...
class TestIndexManager:
    ...
    def test_review_metrics_and_trends(self, temp_project):
        ...
        manager.save_review_metrics(
            ReviewMetrics(
                ...
                critical_issues=[],
                report_file="Validation/第1-1章审查报告.md",
            )
        )
        manager.save_review_metrics(
            ReviewMetrics(
                ...
                report_file="Validation/第2-2章审查报告.md",
            )
        )

        recent = manager.get_recent_review_metrics(limit=2)
        assert len(recent) == 2
        assert recent[0]["anti_ai_force_check"] == "fail"
        assert recent[0]["spoiler_risk"] == "high"
        assert recent[0]["contrivance_risk"] == "medium"
        assert recent[0]["cold_commentary_risk"] == "high"
    ...
    def test_review_metrics_schema_migration_adds_risk_columns(self, temp_project):
        IndexManager(temp_project)

        with sqlite3.connect(temp_project.index_db) as conn:
            conn.execute("DROP TABLE review_metrics")
            conn.execute(
                """
        ...
        with sqlite3.connect(temp_project.index_db) as conn:
            cols = {row[1] for row in conn.execute("PRAGMA table_info(review_metrics)")}
    ...
    def test_index_manager_cli(self, temp_project, monkeypatch, capsys):
        ...
        manager = IndexManager(temp_project)
        temp_project.webnovel_dir.mkdir(parents=True, exist_ok=True)
        temp_project.state_file.write_text(
            json.dumps({"progress": {"current_chapter": 1}, "chapter_meta": {}}, ensure_ascii=False),
            encoding="utf-8",
        )
