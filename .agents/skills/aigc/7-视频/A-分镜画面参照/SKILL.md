...
`A-分镜画面参照` 负责把 `projects/aigc/<项目名>/4-分组/` 中的分镜组转为 Dreamina 组级视频生成任务：step1 直接使用现有分镜组内容作为生视频提示词主体，并把组内 `分镜N` 转为四段式 `分镜ID`；step2 检查 `6-图像/A-分镜画面/` 是否存在对应四段式 `分镜ID` 的图像，有则在对应 `分镜ID` 后标注 `@路径` 并写入参照清单，没有则移除空槽位；step3 调用 `.agents/skills/cli/dreamina-cli`，以分镜组为单位组织“完整组内容 + 多张分镜画面参照”的 Dreamina 视频任务，默认后台多线程批量并发执行。
...
- 分镜组视频 prompt 主体直接采用 `4-分组` 的现有分镜组正文；LLM 只负责保真组织、Dreamina 指令化封装、参照映射说明、缺口说明和审查，不得扩写或改写剧情事实。
...
- 已有 `7-视频/A-分镜画面参照/` 的 prompt、参照 manifest、Dreamina batch YAML、queue ledger 或生成结果需要 repair / review / rerun / query。
- 用户要求“在对应分镜ID后添加 `@路径`”“按分镜组多图参照出视频”“后台并发跑 Dreamina”等任务。

Required input:

- 可定位的 `projects/aigc/<项目名>/4-分组/第N集.md`。
- 每个目标分镜组必须有可解析的 `## x-y-z` 标题和非空组正文；组内分镜应能从 `分镜N`、`分镜 N` 或等价标签映射到 `x-y-z-N`。
- 调用 Dreamina 前必须能确定项目内输出目录，默认 `projects/aigc/<项目名>/7-视频/A-分镜画面参照/第N集/`。
- 执行生成前必须能运行 `dreamina user_credit`；若失败，停止提交并进入 Dreamina 登录/环境排障。

Optional input:

- `prompt_only`：只生成 shot index、prompt 包、参照 manifest 和 Dreamina batch YAML，不执行 Dreamina。
...
本技能是 `7-视频` 阶段的组级多图分镜画面参照入口，向上承接 `4-分组`，横向读取 `6-图像/A-分镜画面` 的已生成单帧图，向下调用 `.agents/skills/cli/dreamina-cli`。它拥有视频 prompt 包、镜级参照 manifest、Dreamina batch YAML、queue ledger、结果下载记录和执行报告的裁决权；它不拥有上游分组改写权，也不拥有分镜画面生成权。

## LLM-First Creative Authorship Contract

- 视频 prompt 的 Dreamina 指令化组织、镜级参照解释、运动/声音约束、节奏保真说明和失败诊断必须由 LLM 直接完成。
...
- 参照图路径属于机械绑定；是否使用参照图、如何在 prompt 中把 `分镜ID@路径` 转换为 Dreamina `@图N` 映射，仍须由本技能合同和 LLM 审查裁决。
...
| `repair` | prompt 缺组、ID 映射错、图像错绑、YAML 漂移、submit_id 缺失、下载不完整 | 按 review gate 定位返工节点 |
...
flowchart TD
    ...
    H --> J
    I --> J
    J --> K["Dreamina batch YAML"]
    K --> L["step3 dreamina-cli backend worker pool"]
    L --> M["submit_id queue ledger"]
...
flowchart TD
    ...
    C --> D["--image <shot_1_path> ... --image <shot_n_path>"]
...
stateDiagram-v2
    ...
    BindFrameImages --> BuildDreaminaYaml
...
2. 按 `types/type-map.md` 锁定 mode、集号范围、目标分镜组集合、目标四段式 `分镜ID` 集合、是否执行 Dreamina、是否查询下载。
...
5. step1 组装 prompt 时添加 Dreamina 视频约束前缀：`根据以下完整分镜组内容生成一条连续视频。保持分镜顺序、角色动作、镜头运动、场景与情绪连续；不生成字幕，不生成BGM，保留物理互动音效与环境音。`
...
8. step3：根据 YAML 转换为 Dreamina CLI 提交格式。一组一个 job：有一张或多张分镜画面图时走 `dreamina multimodal2video --image <path> ... --prompt "<@图N 映射 + 完整组内容>"`；无图时走 `dreamina text2video --prompt "<完整组内容>"`。
9. Dreamina `multimodal2video` 当前图片输入上限按 `.agents/skills/cli/dreamina-cli` 执行；若某组可用分镜画面图超过当前 CLI 上限，必须记录 `reference_over_limit`，按用户策略选择阻断、分段提交或降级为文字 prompt，不得静默丢图。
10. 默认后台多线程批量并发执行：提交前生成 `第N集-dreamina-batch.yaml` 和 queue ledger；worker 数默认 `min(4, job_count)`，每个 worker 只写自己的临时结果，最终由主流程汇流更新 `第N集-dreamina-results.json` 与 `执行报告.md`。
11. 每次生成前必须运行 `dreamina user_credit`；若失败，停止提交并按 `dreamina-cli` 技能进入登录或环境排障。
12. `--poll` 只作短等待；超时后必须保存 `submit_id`，把状态写入 queue ledger，并使用 `dreamina query_result --submit_id=<id>` 后续查询。下载默认写入 `projects/aigc/<项目名>/7-视频/A-分镜画面参照/第N集/videos/`。
13. 交付前执行 `review/review-contract.md`：组 ID 追溯、四段式 ID 映射、prompt 完整性、`分镜ID@路径` 参照路径、Dreamina 子命令选择、queue ledger、并发写入边界和本地视频结果状态必须可复核。
...
| `FIELD-FVID-03` | prompt package | Dreamina 视频约束前缀 + 现有完整组内容 + `分镜ID@路径` 映射，镜头未缺失乱序 | `FAIL-FVID-PROMPT` |
| `FIELD-FVID-04` | frame reference manifest | 只绑定真实 `6-图像/A-分镜画面` 图片；缺图移除空槽位 | `FAIL-FVID-REF` |
| `FIELD-FVID-05` | Dreamina YAML / commands | 有图走 `multimodal2video`，无图走 `text2video`，参数符合 dreamina-cli | `FAIL-FVID-DREAMINA` |
| `FIELD-FVID-06` | queue ledger / submit ids | 多任务均有 queue row、submit_id 或明确失败原因、next_action | `FAIL-FVID-QUEUE` |
...
| `FIELD-FVID-03` | prompt assembly | `第N集-video-prompts.md` | Dreamina 前缀、完整组正文、`shot_id@path` 与 `@图N` 映射说明 | `FAIL-FVID-PROMPT` |
...
| `FIELD-FVID-05` | Dreamina handoff | `第N集-dreamina-batch.yaml` | command type、prompt、reference_images、output path、poll | `FAIL-FVID-DREAMINA` |
| `FIELD-FVID-06` | queue tracking | `第N集-dreamina-queue.md` | queue_id、group_id、submit_id、remote_status、next_action | `FAIL-FVID-QUEUE` |
...
| `PASS-FVID-05` | `FIELD-FVID-05` | YAML 如何转换为 Dreamina CLI 命令 | 有图选 `multimodal2video`，无图选 `text2video` | batch YAML / command preview |
| `PASS-FVID-06` | `FIELD-FVID-06` | 批量任务如何后台并发且可追踪 | 建 ledger、提交、记录 submit_id | queue ledger |
...
| `PASS-FVID-05` | Dreamina YAML 可转为合法 `multimodal2video` 或 `text2video` 命令 | `FAIL-FVID-DREAMINA` | `references/dreamina-handoff-contract.md` |
| `PASS-FVID-06` | 每个提交任务都有 queue row、submit_id 或明确失败原因 | `FAIL-FVID-QUEUE` | `.agents/skills/cli/dreamina-cli/SKILL.md` |
...
3. YAML 无法转换为 Dreamina 命令、子命令选择错误、参照图超过 CLI 上限未处理或参数越权：回到 `references/dreamina-handoff-contract.md` 与 `.agents/skills/cli/dreamina-cli/SKILL.md`。
...
- Required output: 组级视频 prompt 包、镜级分镜画面参照 manifest、Dreamina batch YAML、Dreamina queue ledger、submit/result JSON、逐集执行报告；若执行下载，还应包含本地视频文件。
...
- Naming convention: prompt 文档命名 `第N集-video-prompts.md`；索引命名 `第N集-group-shot-index.json`；参照清单命名 `第N集-reference-manifest.json`；Dreamina 配置命名 `第N集-dreamina-batch.yaml`；队列命名 `第N集-dreamina-queue.md`；结果命名 `第N集-dreamina-results.json`；执行报告命名 `执行报告.md`；视频命名 `<分镜组ID>.mp4`，例如 `1-1-1.mp4`。
- Completion gate: 目标分镜组均可从 `4-分组` 回指；四段式 `分镜ID` 均可回指组内分镜；每条 prompt 完整保留组正文主体；YAML 参照图只绑定真实分镜画面图片并移除缺图空槽位；生成前已通过 `dreamina user_credit`；批量任务均有 queue ledger；执行 Dreamina 时遵循 `.agents/skills/cli/dreamina-cli` 的提交、轮询、查询与下载门禁；审查结果为 `pass` 或 `pass_with_todo`。
