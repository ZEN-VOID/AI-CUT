...
当前技能采用 hybrid 拓扑：前段串行锁源与判型，中段先启动 team supervision subagents；若用户显式要求 subagents 模式，则追加按 `story/review` 维度子技能拆分的审计点 subagents，并把 findings 汇成 provider repair brief；随后按起草/续写/重写/修复分支汇流到 DeepSeek provider，后段接 writeback 与卷级 review handoff。
...
| `N6-DEEPSEEK-DRAFT` | 通过 DeepSeek 生成完整最小局部修补章节文件 | provider prompt、系统提示、输出模板、supervision packet | 调用 `polish_chapter_via_deepseek.py` 及 DeepSeek bridge | provider report、raw output | `N7-VALIDATE-WRITEBACK` | provider 真实命中 |
...
| `FAIL-DSD-REVIEW-SUBAGENTS` | 显式 subagents 模式未按 `story/review` 维度子技能拆分审计，或只有审计包没有通过 DeepSeek 直接优化 | `N3R-REVIEW-SUBAGENT-AUDIT`、`N5D-REPAIR-PROMPT` |
...
- 显式 subagents 模式还必须产生 `story/review` 维度 packets 或降级报告、review repair brief，以及 DeepSeek 输出已按 findings 直接优化的证据。
