...
| `design_detail_culture` 是否有证据位置，并能回指 `Design Appeal Target`、`Signature Detail`、`Cultural Element Strategy`、`Craft / Ornament Detail` 或 prompt token？ | `GATE-PROP-DESIGN-SLOT-01` / `GATE-PROP-DESIGN-13` | `FAIL-PROP-DESIGN-SLOT-01` / `FAIL-PROP-DESIGN-DETAIL-CULTURE` | `N7-REVIEW` / `N5-RESEARCH-CHAIN` / `N6-DESIGN` | 设计细节与文化元素证据位置、signature detail、时代语境说明 |
| `prop_corpus_usage_trace` 是否有证据位置；触发时是否记录 `knowledge-base/prop-design-corpus.md`、语料种子、原创转译、时代/文化 guardrail 与 prompt 证据？ | `GATE-PROP-DESIGN-SLOT-01` / `GATE-PROP-DESIGN-14` | `FAIL-PROP-DESIGN-SLOT-01` / `FAIL-PROP-DESIGN-CORPUS-MISSING` | `N7-REVIEW` / `N5-RESEARCH-CHAIN` / `N6-DESIGN` | `Prop Corpus Usage Trace`、语料种子、原创转译说明、period/culture guardrail |
