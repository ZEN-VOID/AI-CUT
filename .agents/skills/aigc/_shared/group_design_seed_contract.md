...
| `3-Detail` | `aigc-detail` | 继承已存在的 `组间设计`，并补全 `正文切分参考[]`、`分镜明细[].正文回指`、`分镜明细[]` 与 detail 级字段 |
...
6. `分镜切换` 只负责锁定组级固定镜数；former `镜花/1-切换` 的 fixed-shot-count 接受逻辑已在 `2-Global` 内化为阶段侧车说明，真实 `分镜ID / 时间段 / 正文切分参考[] / 分镜明细[].正文回指.beat_refs[]` 仍由 `3-Detail` 的 `分镜构图` 落镜与父层 merge 完成。
...
3. `正文切分参考[]` 与 `分镜明细[].正文回指` 是否只在 `3-Detail` 阶段扩展。
4. `分镜明细[]` 是否只在 `3-Detail` 阶段扩展。
5. phase 推进是否仍保持同一 canonical root，而不是另起第二份终稿。
