...
- Canonical input: `projects/aigc/<项目名>/7-设计/角色/2-设计/<角色名>.md`。
- 必需区块：`4. 解构`。
...
| 真实生成模式下，主图是否以 `<主体ID>-<主体名称>-主图.<ext>` 存在于 `projects/aigc/<项目名>/7-设计/角色/3-生成/`，并可作为 continuity anchor？ | `GATE-CHAR-GEN-03` | `FAIL-MAIN-IMAGE` | `N4-MAIN-IMAGE` | 报告 `main_image_path`、文件存在检查、主图 JSON 的 `output_image_path`。 |
...
| 本阶段是否没有修改上游 `2-设计`、registry、父级 skill、场景/道具/视频/分镜目录，也没有把多视图做成 3x3 分镜、战斗动作集或泛化海报？ | `GATE-CHAR-GEN-08` | `FAIL-WRITE-BOUNDARY` | `N7-REVIEW` | 报告 `changed_files`，并确认全部位于 `projects/aigc/<项目名>/7-设计/角色/3-生成/` 或允许的执行报告路径。 |
