...
long_dialogue_visual_policy: beat_delivery_to_multi_focus_visual_plan
...
<从上游 3-运动 保留的环境描写/画面描写/运动强化等原始字段；回退模式为 2-编导 原始字段>

环境描写：<原文画面句子>
分镜明细：
分镜1（约X秒）: <先按 L0/L1/L2/L3 判断当前镜头应有多完整，再用自然中文写当前节拍的影视功能、可见主体、动作相位，以及画面中自然存在的维度信息点（角色情绪/肢体语言/语气/镜头意识等表演维度、陪体/前景/背景动态、景别/视角/景深/焦点/镜头运动等技术维度、光影变化与反射、节奏同步等——应有则有，没有不必强制），以及镜头起点、景别/视角/景深/焦点/镜头类型中的关键变化、镜头先行的运镜方式/方向/速度曲线/停点、显式时长对应的读秒/停顿/快速通过理由、构图锚点、方向参照、光线可见结果、必要表演微动态、落点、连续性交接和下游可消费点；L0 可精准短写，L1 需可拍可接，L2 需承托表演/信息/关系，L3 需证明峰值或 set-piece 的时值、反应余波和下游稳定性；也必须能抽出 entry/action_anchor/exit/handoff。道具、倒影、反射、涟漪、餐具/杯子/纸张/桌面等物件细节只有在角色互动、关键信息/规则/证据/危险源或必要环境交代时才写成焦点；删除上游原句已有主体、动作、道具和事实后，仍应保留可执行摄影决策，不能只是画面内容拆写/复述；短剧·AIGC 模式下优先用 short/standard 成立，非 slow_burn/hold 的约3秒以上必须有台词、读秒、表演变化、复杂调度、空间重置或高点证据，情绪类 slow_burn/hold 则必须有可见微动态、静止压力、极慢运动或框内变化；只显式写当前画面和当前梯度需要的信息，不虚构不存在的维度，不机械填满 15 项，不粘贴完整视频提示词分栏或维度标签列表。>
<可选：只有存在第二个有效触发点时才写 分镜2（约X秒）；快节奏平台默认一个有效触发点可成一镜。若一个镜头已完成观看策略，不补第二镜。关键揭示、动作分相、群像扩散、对白承托、平台钩子、AIGC 执行重置或高点承托可继续写 分镜3/分镜4，但每一镜都必须提供新的观看策略、时值理由和与前后镜相接的物理因果链。>

<!-- 若相邻画面单位共享空间、道具链、声音链、动作链、记忆插入或视觉母题，段落级 `sequence_profile` 只作为内部计划；道具链必须有互动、关键信息或必要环境理由；此处仍只写正上方画面句子拥有的信息，不能吞入后文画面点。跨分镜和跨块时必须继承人物姿态、位置、朝向、身体接触、手部状态、轴线、运动方向、光色或声音余波；需要改变时必须写出可见动作变化或过渡桥。 -->
<!-- 若上游存在 long_dialogue_beat_map / long_dialogue_delivery_map，必须先内部形成 long_dialogue_visual_plan：每个长对白节拍回指 beat_id，焦点可落在说话者、听者、手部/道具、空间压力、群像、画外声源或沉默余波；不要用一个正面说话者镜头吞完整段长对白。 -->
```
