...
本规格包为 `4-导演` stage 提供"高质量但不越过保真"的质量标准参照。它定义了什么样的导演创作可以被视为好莱坞级质量，同时明确了哪些越权行为是禁止的。
...
- `.agents/skills/aigc/3-美学/SKILL.md` 负责单场画面美学；本文件负责判断"画面密度"和"节奏控制"是否达标。
- `.agents/skills/aigc/6-氛围/references/atmosphere-and-mood-contract.md` 负责五感氛围和意境技法；本文件负责判断意境是否可拍可感而非抽象。
...
以下 5 个质量维度是 `4-导演` stage 必须同时满足的好莱坞级质量基线。
...
以下 8 类越权行为是 `4-导演` stage 的绝对禁止项，任何一个发生即为 quality failure。
...
| **摄影越权** | 越界到下游 8-摄影 | 检查是否出现机位、景别、镜头运动、分镜编号或图像提示词 | "写分镜编号、机位、景别、镜头运动" |
