...
1. Skill 2.0 结构校验：
   `python3 /Users/vincentlee/.codex/skills/meta/构建/技能/skill-工作车间/scripts/validate_skill_2_0.py .agents/skills/aigc/1-Planning`
2. sibling context 校验：
   `python3 scripts/skill_context_audit.py --root .agents/skills/aigc/1-Planning --strict`
3. AIGC 技能树审计：
   `python3 scripts/aigc_skill_audit.py --strict`
4. 引用同步检查：
   `rg -n "\\.agents/skills/aigc/1-Planning/(1-分集|2-格式|3-分组)|aigc-planning-(episode-splitter|script|grouping)"`
...
- `find .agents/skills/aigc/1-Planning -name SKILL.md` 只返回父级 `SKILL.md`。
