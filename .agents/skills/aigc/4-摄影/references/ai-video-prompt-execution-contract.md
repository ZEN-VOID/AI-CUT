...
本文件把 `reports/transcripts/audio/AI视频镜头与提示词学习稿.md` 中可复用的方法论转译为 `4-摄影` 的摄影细则。它不要求本阶段直接生成最终视频提示词；它要求每条 `时间段` 在成稿时具备可被下游 `8-视频` 稳定改写为提示词的执行顺序、方向参照、光线结果和表演微动态控制。
...
AI 视频生成更像顺序执行提示词，而不是自动理解导演意图。因此 `4-摄影` 的分镜画面必须采用“镜头包裹动作”的设计思路：先建立镜头、运动、构图和参照方向，再让人物动作、表演变化、光线结果和环境声在该镜头内部发生。
...
若某条 `时间段` 读起来像“动作先发生，镜头最后补上”，必须回到 `N6.5-SHOT-PLAN` 重排为镜头先行的执行顺序。
...
当方向信息会影响下游运动生成时，最终 `分镜画面` 应显式写出“朝镜头、远离摄像机、画面左侧、画面右侧三分之一、前景、后景、门框内侧、课桌通道”等参照词；不能只写文学化的“向前”“后退”“走过去”。
...
每条可被下游独立消费的候选 `时间段`，只要 screen left/right 会影响理解，就必须在内部 `ai_video_prompt_execution_profile` 中重复以下信息：
...
“主角在左、反派在右”只在第一镜出现不算稳定。AI 视频模型可能在后续镜头随机反转位置；因此需要在 `shot_design_plan` 和下游 payload 中逐段携带 `axis_continuity_anchor`。
...
**非独白场景的情绪词转译（强制）**：`4-摄影` 的分镜画面不得将”紧张””愤怒””压抑””心痛””痛苦”等抽象心理标签直接写入正文——AI 视频生成器无法理解这些标签，也不会将其转化为可见动作。必须将每个抽象情绪词转译为以下可见物理动作之一：
- 面部肌肉具体变化（咬肌收紧鼓起、眉心竖纹加深、鼻翼微张、嘴角下拉）
- 身体动作（手指抓紧衣角/桌沿、指节发白、肩膀内收、胸腔起伏变浅、喉结滚动）
- 呼吸节奏（屏住呼吸、呼气变浅变短、叹息）
- 视线转移（眼神从对方身上移开、目光变冷、呆住不动）

转译时每条时间段必须保留至少 1-2 个具体可见动作，不能只写”他看起来很紧张”却不交代具体怎么表现紧张。

## Prompt-Ready Shot Payload

每条 `时间段` 的内部计划应形成 `ai_video_prompt_execution_profile`：
...
`ai_video_prompt_execution_profile` 不替代 `functional_projection_plan`，而是其视频执行稳定性扩展。若最终 `分镜画面` 不能反推出这些关键项，下游视频提示词会退化为关键词列表，应回到 `N6.4-FUNCTIONAL-PROJECTION` 或 `N6.5-SHOT-PLAN` 修复。

## Output Boundary

- `4-摄影` 仍输出自然中文 `分镜画面`，不输出完整视频提示词模板。
- 分栏模板如“镜头与构图 / 人物与动作 / 情绪动机 / 面部肌肉 / 身体联动 / 光线 / 声音与环境 / 画面质感”只作为下游提示词结构参考，不直接粘进 `分镜画面`。
- 情绪原因只能来自上游编导稿或项目上下文；不得为了补“情绪动机”新增剧情事实。
- 声音与环境只能作为镜头承托和交出锚点，不能在本阶段越权替代音效或视频阶段的完整声音设计。
- 负向约束默认留在内部或执行报告，不把”不要夸张表情、不要卡通感”等命令式提示词写进每条时间段正文。
...
1. **慢镜头**：极慢推轨（内部 3-5 秒，落盘为 `[起始秒-结束秒]`）、长停顿（内部 3-4 秒，落盘为 `[起始秒-结束秒]`）、焦点极慢拉移——让观众有时间读取面部肌肉变化。
...
| `时间段` 能否还原“镜头类型/运动/构图先包裹动作”的执行顺序，而不是动作发生后才补一句镜头推进或跟拍？ | `GATE-CINE-15A`、`GATE-CINE-15` | `FAIL-SHOT-IDENTITY-01`、`FAIL-CINE-05N` | `steps/cinematography-workflow.md#N6.4-FUNCTIONAL-PROJECTION`、`steps/cinematography-workflow.md#N6.5-SHOT-PLAN` | `ai_video_prompt_execution_profile.camera_first_opening` 抽样；动作与镜头割裂修复记录 |
...
11. 双人/多人轴线漂移：回到 `shot-continuity-contract.md#Two-Person-Axis-And-180-Degree-Rule`，补 `axis_line / screen_position_lock / middle_spatial_anchor / camera_half_space / axis_change_bridge`，并在每条关键候选时间段的 `axis_continuity_anchor` 中重复；直接左右反转必须改为中性、主观或运动桥接。
