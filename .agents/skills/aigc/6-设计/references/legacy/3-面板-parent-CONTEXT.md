...
- 本文件是 `.agents/skills/aigc/6-设计/3-面板` 的经验层知识库，不是过程日志。
...
| --- | --- | --- | --- | --- |
| `3-面板` 父目录为空导致父层无法路由 | tranche source-layer | 补齐 `SKILL.md + CONTEXT.md + _shared bridge` | 父 tranche 只声明 active leaf 与 pending sibling | `6-设计/SKILL.md` 能条件读取 `3-面板/SKILL.md` |
...
- active leaf 每新增一个，都要同步父 `3-面板`、`6-设计` 父层与 registry；否则批量路由会出现“文件存在但入口仍 pending”的断层。
