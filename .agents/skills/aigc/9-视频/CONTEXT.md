...
| --- | --- | --- | --- |
| libtv canvas plan | 用户只说视频阶段、生视频、LibTV、画布流、按 `6-分组` 组级出视频 | `libTV画布流` | 默认路线；生成计划和 CLI handoff，不在父级直接执行 |
...
- `9-视频` 与 `.agents/skills/cli/libTV` 的关系应类似 `photoGPT` 与 `.agents/skills/cli/imagegen`：前者做计划、类型判断和质量门禁，后者拥有实际 provider / CLI 执行边界。
...
- 视频文件名是 `9-视频` 和 `10-审片` 之间的主接口；不要把审片依赖绑到旧会话 ID、node id 或下载顺序上。
