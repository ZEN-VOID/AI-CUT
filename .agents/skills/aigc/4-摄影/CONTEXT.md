...
| 双人轴线漂移或左右反复 | 180 度轴线 / AI 提示词继承层 | 回到 `shot-continuity-contract.md#Two-Person-Axis-And-180-Degree-Rule`，先锁定 line of action、screen left/right、中间空间锚点和 camera_half_space；再在每条关键候选分镜补 `axis_continuity_anchor` | `GATE-CINE-27` + `FAIL-CINE-05V`；直接越轴必须有中性、主观、运动镜头或角色换位桥接 | 同一对峙/追逐/动作关系中，观众始终知道谁在左谁在右；AI 下游逐镜消费时不会随机把主角和对手调换 |
...
| 改写了 2-编导 原文 | 保真层 | 恢复上游原句，只保留下方新增分镜明细块 | diff 中除 frontmatter 和分镜明细外无正文改写 |
| 输出中途截断但结构校验通过 | 源文保真/validator 脱节层 | 先补回缺失的上游正文，再为缺失画面句子补 `分镜明细：`，最后更新报告统计 | 机械校验必须剥离 `分镜明细` 后与 `source_motion_path` 或回退 `source_writing_directing_path` 做正文保真比对；只看输出稿内部覆盖率不够 |
...
13. 在写任何 `分镜N` 前先走完节点链：`N2-MATCH -> N3-TYPE -> N3.5-SEQUENCE-ALIGN -> N3.6-DENSITY-CURVE -> N4-BEAT -> N5-RHYTHM -> N5.2-DURATION -> N5.5-PEAK-SHOT -> N5.6-INIT-SYNTHESIS -> N6-CONTINUITY -> N6.1-HANDOFF -> N6.2-CAMERA-GRAMMAR -> N6.4-FUNCTIONAL-PROJECTION -> N6.5-SHOT-PLAN`；缺任一层都不能直接成稿。
...
15.51. 对每条候选分镜做镜头叙事功能检查：必须能说明这条镜头新增了什么信息、关系变化、动作结果、情绪压力、观看发现或交出价值。删掉无损失时，删并或重写。
...
16.05. 对双人/多人对峙、追逐、动作、逼问或谈判做 180 度轴线检查：先建立 `axis_line`，再锁定谁在 screen left/right、中间隔着什么空间锚点、摄影机在哪个同侧 180 度半区；后续每条关键候选分镜都要带 `axis_continuity_anchor`。若需要换轴，必须给中性镜头、主观视角、可见运动镜头或明确角色换位，不能硬切左右反转。
...
26. 最后做一次机械检查：覆盖率、连续编号、输出路径、分镜数量分布、显式秒数、逐画面点归属、源文保真和 AI 视频执行稳定性。源文保真必须剥离输出稿中的 `分镜明细：` 与 `分镜N` 后，与 `source_motion_path` 或回退 `source_writing_directing_path` 的 `【剧本正文】` 非空行逐行比对；若上游后半段被漏写，即使 validator 内部覆盖通过也必须判为阻断项。
...
- 物理遮挡可以作为转场锚点，但 `4-摄影` 只负责当前画面内部的遮挡交出点：柱子、门框、树干、墙面、衣摆或黑场前景如何吞没画面。完整组间连接件的创意裁决仍交给 `5-分组`，不要在单条 `分镜明细` 中越权写完整跨组转场。
...
- AI 不会自动记住“主角在左、反派在右”。双人对峙、追逐和动作场要把这件事当成每条分镜的 payload，而不是第一镜的背景设定；逐镜重复 screen left/right、中间空间锚点和摄影机同侧半区，比事后解释“保持轴线”更稳定。
...
- 分镜不是“把动作拍下来”，而是让观众在这一镜里获得某个新东西：信息、关系、压力、结果、发现或交出锚点。删掉无损失的镜头通常不是克制，而是冗余。
- 快节奏平台采用 trigger-first：`BT-01~BT-16` 命中的有效触发点默认可以成镜。合并是例外，只在同镜完成不损失观看结果、平台节奏、下游 payload 或动作连续性时使用。
