...
- 需要在 `projects/<项目名>/1-规划/` 下生成规划阶段产物与阶段验收，并按需写出 `Init/` bootstrap 产物。
...
flowchart TD
    ...
    D1 -->|"默认表演优先"| D2["标准剧"]
    D1 -->|"显式旁白主导"| D3["解说剧"]
    D2 --> G["落盘 projects/<项目名>/1-规划/2-格式/"]
    D3 --> G
    E --> H["落盘 projects/<项目名>/1-规划/3-分组/"]
    F --> I["落盘 projects/<项目名>/1-规划/4-节奏/"]
```

```mermaid
flowchart LR
    A["0-Init/north_star.yaml"] --> B["1-规划"]
    C["0-Init/init_handoff.yaml"] --> B
    B --> D["2-组间"]
    B --> E["3-明细"]
```
...
- 阶段根目录：`projects/<项目名>/1-规划/`
- 阶段验证报告：`projects/<项目名>/1-规划/validation-report.md`
...
- `2-格式 / 3-分组 / 4-节奏` 的 canonical 产物统一落在 `projects/<项目名>/1-规划/`
...
4. 在 `projects/<项目名>/1-规划/validation-report.md` 写作前后，若 `roles.review.members` 非空，则调用 `评审` 顾问团给出 PASS/返工意见。
...
| --- | --- | --- | --- | --- | --- |
| `1-分集` | 串行第 1 步 | 已建合同 | 需要把长故事、剧本原文或混合分镜文本按集切分 | `projects/<项目名>/Init/` + `projects/<项目名>/编导/第N集.json` | 例外型 bootstrap 子路径；负责首次创建编导根文件 |
| `2-格式` | 串行第 2 步 | 已建父子合同 | 需要统一文档模板、文本层级、场景标题与变体格式 | `projects/<项目名>/1-规划/2-格式/` | 先进入父技能，再在 `标准剧/解说剧` 间唯一裁决 |
| `3-分组` | 串行第 3 步 | 已建合同 | 需要按章节、任务批次、镜头包或制作波次做结构分组 | `projects/<项目名>/1-规划/3-分组/第N集.md` | 集粒度沿用 `1-分集`，仅在集内执行分组 |
| `4-节奏` | 串行第 4 步 | 已建合同 | 需要在分组结果上收束主驱动、七步、峰值与节奏重排边界 | `projects/<项目名>/1-规划/4-节奏/第N集.md` | 仅在 `original_adherence=false` 且 `3-分组` 稳定时进入 |
...
2. 读取 `projects/<项目名>/0-Init/north_star.yaml` 与 `projects/<项目名>/0-Init/init_handoff.yaml`。
3. 若顾问团启用且 `roles.planning.members` 非空，先调用 `策划` 顾问团，再进入阶段路由。
4. 判断当前请求属于 `1-分集`、`2-格式`、`3-分组`、`4-节奏` 中哪一个唯一主入口。
5. 若目标是 `2-格式`，先进入父技能，再在 `标准剧/解说剧` 间完成唯一变体裁决。
6. 若目标是 `4-节奏`，先确认 `3-分组` 结果已稳定，且 `0-Init.original_adherence=false`。
7. 若目标子路径合同缺失，停止向下伪造，返回缺口与补建落点。
8. 若目标子路径合同存在，则进入对应子技能执行。
9. 在阶段级 `validation-report.md` 前后按需调用 `评审` 顾问团。
10. 将阶段级验收结论落到 `projects/<项目名>/1-规划/validation-report.md`；若命中 `1-分集`，同时在 `Init/` 写 bootstrap 产物，并首次创建 `projects/<项目名>/编导/第N集.json`。
11. 返回唯一推荐的下一阶段入口，而不是模糊候选集合。
...
- 规划产物落点漂移到 `projects/<项目名>/1-规划/` 之外
- `0-Init` 的种子无法映射到规划阶段
...
| FIELD-PLAN-LAND-03 | Canonical Landing | 锁定 `projects/<项目名>/1-规划/` 及各子路径产物落点 | S3 | 落点一致性 | FAIL-PLAN-LAND-03 |
