...
当前技能采用 hybrid 拓扑：前段串行锁源与判型，中段先启动 team supervision subagents，再按起草/续写/重写/修复分支汇流到 Doubao provider，后段接 writeback 与卷级 review handoff。
...
| `N6-PROVIDER-DRAFT` | 通过豆包生成完整润色章节文件 | provider prompt、系统提示、输出模板、supervision packet | 调用 `polish_chapter_via_doubao.py` 及豆包 bridge | provider report、raw output | `N7-VALIDATE-WRITEBACK` | provider 真实命中 |
