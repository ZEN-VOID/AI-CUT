...
| `duration_signal=missing / mixed` | 回到 `4-摄影` 补显式秒数，临时兼容时只能做风险估算 | group duration band | 报告缺秒数来源 |
| `style_payload=long_north_star` | 不影响组内时长或计入场景标题行/入场镜头/画面构图/构图分区/画面属性/出场画面后的 1680 字风险口径；组头第 1 行应从全局风格母稿抽取当前组匹配部分并压到 300 字以内，第 2、3 行直引类型元素和画面风格 | north-star projection | 禁止完整照抄全局风格母稿到每个组；禁止凭空新增当前组无证据风格 |
...
- 若正文含 `stage: 4-摄影`、`source_directing_path`、`分镜明细：`，通常为 `complete_cinematography`。
