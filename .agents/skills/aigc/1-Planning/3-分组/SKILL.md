...
description: Use when `1-Planning` needs to turn `2-剧本/第N集.md` into a grouped script with stable group boundaries, quantized duration/text-window evidence, and an internalized grouping/rhythm review contract inside one stage-local parent SKILL.
...
flowchart TD
    ...
    B --> C["量化裁决内核"]
    C --> D["可选节奏复核 gate"]
    D --> E["写回 grouped script + 执行报告"]
    E --> F["validate_grouping_output.py"]
    F --> G{"Pass?"}
    G -->|"Yes"| H["Emit parent handoff"]
    G -->|"No"| I["Return to boundary or quantizer node"]
```
...
6. 写回 grouped script 与执行报告。
7. 运行 validator，失败则回退到边界或量化节点。
8. 生成父级 handoff。
...
| `FIELD-GROUP-05` | 主稿落盘 | 写 grouped script 与执行报告 | `S5` | 输出完整性 | `FAIL-GROUP-05` |
| `FIELD-GROUP-06` | validator | 运行结构/量化校验并收敛返工入口 | `S6` | 闭环完整性 | `FAIL-GROUP-06` |
...
| `S5` | `FIELD-GROUP-05` | grouped script 如何落盘 | 写 grouped script + 执行报告 | 只返摘要稿 |
| `S6` | `FIELD-GROUP-06` | 输出是否通过质量门 | 跑 validator 并返工 | validator 失败仍宣告完成 |
...
| `FIELD-GROUP-05` | grouped script 与执行报告都已落盘 | `FAIL-GROUP-05` | `S5` |
| `FIELD-GROUP-06` | validator 通过或失败项已明确返工 | `FAIL-GROUP-06` | `S6` |
