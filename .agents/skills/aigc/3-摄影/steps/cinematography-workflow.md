...
| `N6.4-FUNCTIONAL-PROJECTION` | 建立功能性影视投影与下游 payload | camera_grammar_plan、beat_map、shot_duration_decision、dialogue_time_budget、continuity_profile、handoff_profile、`references/functional-cinematic-projection-contract.md`、`references/ai-video-prompt-execution-contract.md`、下游图像/视频消费要求 | 为每个计划分镜锁定影视功能、可见主体、动作相位、运镜计划、显式时长、对白/旁白承托、构图锚点、光色/材质、空间接口、连续性交接、交出锚点和 AIGC 下游消费点；同步形成 `ai_video_prompt_execution_profile`，确认镜头先行、动作发生在镜头内部、方向参照明确、光线写结果、必要表演微动态可见 | functional_projection_plan、ai_video_prompt_execution_profile | `N6.5-SHOT-PLAN` | 每个候选分镜能指导生图和视频；删掉或缩短后会损失明确观看信息或台词承托；不存在动作与运镜割裂、方向含混或光线空泛 |
| `N6.5-SHOT-PLAN` | 汇流 references 形成逐分镜计划 | visual_unit、type_profile、sequence_density_curve、beat_map、rhythm_profile、shot_duration_decision、dialogue_time_budget、peak_shot_profile、sequence_profile、unit_ownership_map、forbidden_bleed、continuity_profile、handoff_profile、camera_grammar_plan、functional_projection_plan、ai_video_prompt_execution_profile、`advisor_consultation_packet`、`references/shot-planning-integration-contract.md`、`references/visual-sequence-alignment-contract.md`、`references/sequence-density-curve-contract.md`、`references/ai-video-prompt-execution-contract.md`、动态运镜合同、自然成稿合同 | 为每个 visual_unit 形成内部 `shot_design_plan`；裁决分镜数量、顺序、入口、单镜显式时长、对白台词量承托、运镜路径、速度曲线、停点、落点、边界交出锚点、摄影语法、显式参数取舍、功能 payload、AI 视频镜头先行执行顺序、方向参照、光线结果、表演微动态、交出点和 `unit_ownership_check`；若命中 `sequence_density_curve`，必须说明当前分镜属于 `density_ramp` 的哪个槽位，是否为 `peak_slot/recovery_slot/set_piece_chain_slot`，并删除没有新观看策略、时值理由、台词承托、下游消费价值、当前画面点归属或视频执行稳定性的随机分镜 | shot_design_plan | `N7-INJECT` | 每个计划分镜可回指 beat/rhythm/duration/dialogue/sequence density/sequence ownership/continuity/handoff/camera/function/downstream/ai-video execution/naturalness；不存在跨画面点外溢、动作镜头割裂、方向含混或光线结果缺失 |
| `N7-INJECT` | 执行 step3 运镜摄影设计注入 | `shot_design_plan`、`sequence_profile`、`unit_ownership_map`、`forbidden_bleed`、`advisor_consultation_packet`、`references/visual-sequence-alignment-contract.md`、`references/functional-cinematic-projection-contract.md`、`references/ai-video-prompt-execution-contract.md`、`references/shot-duration-decision-contract.md`、`references/dynamic-lens-language-contract.md`、`references/cinematic-technique-library.md`、`references/natural-shot-detail-writing-contract.md` | LLM 直写 `分镜明细：` 与 `分镜N（约X秒）`，严格投影 `shot_design_plan`，把参数、时值裁决、AI 视频执行稳定性和段落连续性压成自然画面文字；每个块只写当前画面点拥有的主体/动作/道具/对白承托，吸收顾问参谋上下文但不改写上游真源；字段内容不得输出抽象主题、心理结论、世界观解释、导演阐释、不可执行气氛口号、随机好看句、参数清单、提示词分栏模板、命令式负向词、模板句法或跨块失主镜头 | enriched episode draft | `N8-REVIEW` | 原文保留，注入块紧跟命中句子；字段语义纯度、逐画面点归属、摄影语法变化、功能性投影、AI 视频执行稳定性、显式时长、对白承托和自然成稿通过；每个分镜能反推出起点、运镜路径、速度、时值等级、停点、落点、动机、交出点、所属 visual_unit、下游消费 payload、方向参照、光线结果和必要微动态；顾问上下文未越权 |
| `N8-REVIEW` | 执行质量与机械门禁 | candidate enriched draft、`review/review-contract.md`、可选 validator | 检查覆盖、连续编号、节奏张弛、镜头时值、摄影语法变化、shot_design_plan 投影、功能性投影、AI 视频执行稳定性、连续性、保真、专业性和自然成稿，定位 repair target | review result、repair targets | `N8R-DIRECT-REPAIR` 或 `N9-WRITE` | 无阻断项才可写回；`GATE-CINE-15A` 失败不得降级为 followup |
| `N8R-DIRECT-REPAIR` | 阶段内直接修复阻断项 | repair targets、candidate enriched draft、上游编导稿 | 最小修复 `分镜明细`、`分镜N`、连续性、节奏张弛、镜头时值、AI 视频执行稳定性、峰值分镜、专业可执行或报告证据；不改上游原文 | repaired draft、repair actions | `N8R-REVIEW-AGAIN` | 修复范围不越权 |
...
- review 返工必须回到最早责任节点：归属错回 `N2-MATCH` 或 `N3.5-SEQUENCE-ALIGN`，类型错回 `N3-TYPE`，段落密度错回 `N3.6-DENSITY-CURVE`，数量错回 `N4-BEAT` / `N5-RHYTHM`，时值错回 `N5.2-DURATION`，语法错回 `N6.2-CAMERA-GRAMMAR`，payload 或 AI 视频执行稳定性错回 `N6.4-FUNCTIONAL-PROJECTION`，计划缺环或视频执行稳定性未汇流回 `N6.5-SHOT-PLAN`，正文模板腔回 `N7-INJECT`。
