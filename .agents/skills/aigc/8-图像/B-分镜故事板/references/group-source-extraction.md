...
本文件定义 step1 的输入锁定：以 `projects/aigc/<项目名>/6-分组` 为主要信息来源，获取每个分镜组的完整内容。
...
- `8-图像/B-分镜故事板` 默认完全忽略连接件块：不进入 `group_body`、storyboard prompt、YAML 主体基准、shot_count、reference manifest 或 imagegen plan。
...
group_id: "1-1-1"
episode_id: "第1集"
source_file: "projects/aigc/<项目名>/6-分组/第1集.md"
heading: "## 1-1-1"
...
| fail signal | rework |
| --- | --- |
| 找不到 `## x-y-z` | 确认集号或 group_id，必要时回上游 `6-分组` 修复 |
