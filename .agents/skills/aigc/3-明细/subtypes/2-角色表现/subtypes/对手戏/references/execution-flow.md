...
3. 锁定攻防主轴、说与不说的边界、主导角色与逆转节点。
4. 用停顿、视线、姿态、手部反馈与空间逼压外化潜台词，不改写对白原文。
5. 将稳定结果写入 evidence sidecar，并返回本镜级 `角色表现` patch 供父技能聚合。
