...
- 必须同时读取 `../_shared/chapter-board-locating-contract.md`，先把当前章解析成唯一 `chapter_board`，再解码本章义务。
- 若项目存在续作 / 前作 / 旧 IP 余波信号，必须同时读取 `../_shared/sequel-continuity-contract.md` 与 `0-Init/story-source-manifest.yaml`。
- 正式处理前，必须读取当前 `第N章.md`；若是新集首轮，可由父层先用 template bootstrap。

## Parent Positioning

本 child 负责：

- 把 `2-Planning/第V卷/第N章.md` 的本章功能债、事件骨架、任务/线索/伏笔债务翻译成可读的首轮叙事结构
- 生成本章第一版完整正文
- 锁定本章必须兑现的义务、连续性锚与首轮推进主干，但不抢占 Step 2 对节奏 mode 与脉冲编排的 owning 权
...
| analysis_slot | 当前结论 |
| --- | --- |
| `business_goal` | 先让本章有完整、可读、可继续加工的叙事底座，而不是直接追求完美文笔。 |
| `business_object` | `2-Planning/整体规划.md`、当前卷 `卷规划.md`、当前章 `第N章.md`、当前 `第N章.md`。 |
| `constraint_profile` | 必须执行规划义务；必须优先回应当前卷 continuity pack；若前序集终稿已存在，可用作增强校准；不得提前替后续工序做过度装饰。 |
| `success_criteria` | 当前章已经是一篇章节级、完整可读的小说初稿，能回答“发生了什么、谁在做什么、为什么要继续看”，而不是压缩剧情稿。 |
...
- 必需输入：
  ...
  - 当前 `chapter_num / episode_id`
  - 当前 `第N章.md`
  - 当前卷 `第V卷.写作日志.yaml`
- 条件必需输入：
  - 若当前卷前序集已完成，可选读取上一章终稿
- 硬规则：
  - 必须先按 shared locating contract 命中唯一 `第N章.md`，必要时再命中兼容 `chapter_board`。
  - 禁止用 `chapter_boards` 数组顺序或标题文案猜测“哪一个是本章”。
  - 先锁本章功能和承接义务，再写 scene chain。
  - 起盘必须显式消费 `chapter_goal + action_beat_plan.turning_point + emotion_beat`，并把它们压成最少三拍：开场局面、局势改向、当前章承诺的终端碰撞。
...
- 起盘阶段不得把 planning 的外部总结语言原样写进正文；chapter goal / must happen / pressure 等字段必须翻译成人物视角中的局面、动作、代价或预感。
  ...
  - 若本章依赖续作 continuity 才能成立，必须在首轮正文里至少落下一处“旧事正在改变当前选择”的戏内证据，不得把关键旧债拖到后续工序才补。

## Output Contract

- `manuscript_patch`
  - 第一版完整正文
  - 若当前章 root 由本 step 首次 bootstrap 或重写，必须确保 frontmatter 至少显式包含：
    - `story_name`
    - `rhythm_type`
...
| --- | --- | --- | --- | --- | --- | --- |
| `N1-ROOT-REREAD` | `FIELD-DR1-01` | 读取当前 root 与日志 | 回读正文、日志、当前 `chapter_num / episode_id` | `input_note` | -> `N2` | root 最新 |
| `N2-BOARD-LOCATE` | `FIELD-DR1-02` | 命中本章唯一 board | 按 shared locating contract 解析 `chapter_board` | `board_resolution_note` | -> `N3` | board 唯一 |
| `N3-BOARD-DECODE` | `FIELD-DR1-03` | 锁本章功能与债务 | 抽取事件、冲突、任务、线索、伏笔 | `board_note` | -> `N4` | 功能明确 |
| `N4-CONTINUITY-BRIDGE` | `FIELD-DR1-04` | 接上上一章停点 | 识别情绪、动作、信息停点 | `continuity_note` | -> `N5` | 承接成立 |
...
| `FIELD-DR1-02` | 本章 board 定位 | 当前章已命中唯一 `chapter_board` | `FAIL-DR1-02` | `N2` |
| `FIELD-DR1-03` | 本章功能锁定 | 功能债与 threads 债务清楚 | `FAIL-DR1-03` | `N3` |
...
- `process_log_entry` 已能说明本章 board 如何被定位。
