---
name: story-drafting-dialogue-optimization
description: Use when `3-Drafting` needs the governed child skill that optimizes dialogue so it stays in character, sounds natural, and serves the episode's narrative movement.
...
- 把对话写成角色个性的外显动作，同时确保它为当前叙事推进、关系变化与局势变化服务
- 让说话方式同时体现人物性格、身份位置、关系冷热与当下策略
- 强化潜台词、试探、回避、施压等对话意图
- 处理断句、气口、信息密度与自然感，让台词像真人会在当场说出来
- 校准语气、口吻、称呼与停顿，使之符合角色设定而不过火、不生硬、不唐突
- 先区分对白类型，再决定优化重写重点；至少要区分自白/辩解、试探/施压、劝阻/诱导、冲突对峙、关系回暖与信息遮掩几类常见对话功能
...
- `./references/dialogue-optimization-execution-playbook.md`
- `../../1-Cards/角色卡/`
...
| analysis_slot | 当前结论 |
| --- | --- |
| `business_goal` | 让不同角色一开口就带出自己的性格、处境和关系张力，使对话自然、鲜活，并真正服务叙事。 |
| `business_object` | Step 4 后正文、角色卡、关系变化上下文、当前项目的 `类型卡 / 题材走廊`，以及本地对白执行细则中声明的对白功能分流与镜片接入规则。 |
| `constraint_profile` | 对话必须服务叙事、人物与关系，而不是变成说明渠道；说话差异必须建立在角色设定、关系冷热、身份高低与情境压力上；台词要像真人会说的话，避免啰嗦、生硬、唐突；若 pack 要求“情绪升级必须伴随行动或代价”，必须在本 step 兑现；若调用外部文学镜片，只能强化角色声口和冲突机制，不得直接模仿名家表层措辞。 |
| `success_criteria` | 对手戏读起来能分辨谁是谁、谁在试探谁、谁在回避什么，也能感到每个人为什么会用这样的方式开口；台词不拖沓、不硬拗、不跳戏，既符合真人说话习惯，也能鲜活地反映角色个性。 |
| `topology_fit` | `root reread -> dialogue hotspot sampling -> dialogue-function routing -> persona extraction -> relationship/intention mapping -> optional literary-lens bind -> optimized rewrite -> anti-mimic guard` |
...
- 硬规则：
  - 对话不能替代动作与场景独自承担全部信息。
  - 对话必须服务叙事推进、关系变化或局势变化；若一句话既不推动局面，也不显影人物，则默认应重写或删减。
  - 对白优化必须避免所有角色都说“作者的普通话”。
  - 对话必须像真人会在当场说出来的话；禁止出现只为补设定、补观点或补总结而存在的硬塞句。
  - 进入重写前，必须先给关键对白场打 `dialogue_function`，至少区分：自白/辩解、试探/施压、劝阻/诱导、冲突对峙、关系回暖、信息遮掩；不同类型不得用同一种“顺口化”手法统一处理。
  - 优化不能只靠口头禅或表面词汇差异；必须同时落实到句长、停顿、信息密度、称呼习惯、进攻/防守方式与回避路径。
  - 角色的说话风格必须和其卡面事实一致；寡言者不能突然长篇发言，机敏者不能一直只会直白说明，身份高低也不能失去应有的压迫感或自保感。
  - 语气口吻必须符合角色设定与当下关系温度；强硬的人不能忽然温吞，谨慎的人不能忽然像作者代言，亲疏变化也必须有触发条件。
...
- `process_log_entry`
  - `step_id: 5`
  - `focus_dimension: dialogue_optimization`
...
| `FIELD-DR5-06` | 对话优化版正文 | 对话自然、鲜活、符合真人习惯且具意图层 | `FAIL-DR5-06` | `N6` |
| `FIELD-DR5-07` | 反模仿 guard | 无明显作者说明腔、无名家 cosplay、无 Step 4/6 越权 | `FAIL-DR5-07` | `N7` |

## Completion Contract

- 当前正文中的关键对话已经具备角色个性、关系压力与潜台词。
- 当前正文中的关键对话已经完成基本 `dialogue_function` 分流，不再用同一种润句手法处理所有对手戏。
- 若命中羞耻自白或证词冲突等高压场面，当前对话已经能听见人物的防御、体面或羞耻，但不会滑成外部文学人物模仿。
- 当前对话不存在明显啰嗦、生硬或唐突的解释性台词。
