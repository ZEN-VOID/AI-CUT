...
本技能不改写剧情事实、对白、场景顺序、分镜编号含义或上游摄影/光影内容。核心裁决是组边界、组级 `全局风格：`、组级 `画面风格：`、首帧衔接/回龙帧内化、组底 YAML 统计和唯一 canonical 落盘。
...
- 核心分组边界、首帧衔接、组级风格和 YAML 语义统计必须由 LLM 直接完成；`scripts/` 只能做读取、格式扫描、时间码连续性、ID、旧字段、YAML 与机械统计校验。
...
| `complexity_source` | 复杂度来自 source 格式分型、15 秒组边界、光影稿保真、声画连续、组级风格和 YAML 统计汇流 | Type Routing、Node Map、Review Gate | `FAIL-GROUP-COMPLEXITY` |
| `topology_fit` | 先锁 source 与 north_star，再建场景/分镜清单，再裁边界，再写组级风格和回龙帧，再统计和审查：1) 防止误改上游；2) 保证组边界可追踪；3) 保证下游生成可消费；4) 支持用户指定 source 跳过光影稿专属检查 | Visual Maps、Node Map、报告证据 | `FAIL-GROUP-TOPOLOGY-FIT` |
...
- 用户要求“10-分组”“分组”“分镜组”“从 9-光影 到 10-分组”“把光影稿切分镜组”“直接用指定文稿分组”“修复 15 秒分组断裂”“首帧衔接”“回龙帧”等任务。
...
- 指定文稿路径：source 中存在可识别场景、动作、对白、音效、转场或分镜信息；若缺少显式时长，由 LLM 按约 15 秒/组规划连续时间码并在报告声明。
...
| `direct_screenplay` | 用户指定剧本/编导稿，或缺少逐分镜时长但存在可读逐集剧本源 | LLM 规划约 15 秒/组，报告声明时间码来源 |
...
| --- | --- | --- | --- | --- | --- |
| `single_episode_lighting_source` | 默认 `9-光影/第N集.md` 或单集光影稿 | `Lighting Source Path` | `N1,N2,N3,N4,N5,N6,N7,N8,N9` | `CONTEXT.md`, `types/grouping-type-map.md`, `references/group-boundary-contract.md`, `references/north-star-projection-contract.md`, `references/group-visual-tone-contract.md`, `references/statistics-yaml-contract.md`, `review/review-contract.md` | `FAIL-GROUP-TYPE-LIGHTING` |
| `specified_source_override` | 用户指定非默认 source | `Override Source Path` | `N1,N2,N3,N4,N5,N6,N7,N8,N9` | `CONTEXT.md`, `types/grouping-type-map.md`, `references/group-boundary-contract.md`, `references/north-star-projection-contract.md`, `references/statistics-yaml-contract.md`, `review/review-contract.md` | `FAIL-GROUP-TYPE-OVERRIDE` |
| `direct_screenplay` | 剧本/编导稿直入或无分镜时长 | `Direct Screenplay Path` | `N1,N2,N3,N4,N5,N6,N7,N8,N9` | `CONTEXT.md`, `types/grouping-type-map.md`, `references/group-boundary-contract.md`, `references/north-star-projection-contract.md`, `references/statistics-yaml-contract.md`, `review/review-contract.md` | `FAIL-GROUP-DIRECT-SCREENPLAY` |
...
| `global_style_projection_map` | `3-美学/画面基调/全局风格协议.md` 的 `Global Style Prompt`、`Visual Slogan`、`Design Principle`、`Visual Gene Profile`、`Negative Traits`，以及必要的 `north_star.yaml` 项目禁区/不变量 | 每组 `全局风格：` 三行 | `N4-GLOBAL-STYLE` | `GATE-GROUP-02` |
| `group_visual_style_map` | 组内摄影、光影、构图、色彩、观看位置证据 | 每组 `画面风格：` 自然语句 | `N5-VISUAL-STYLE` | `GATE-GROUP-12` |
...
| `N3-GROUP-BOUNDARY` | 裁决约 15 秒分镜组边界 | N2 清单、boundary reference | 按完整 shot line / 声画 atomic unit 累计时长；通常 12-18 秒；不得超过 18 秒；跨场景重置组序 | `group_boundary_plan`, `duration_table`, `scene_group_index` | `N4` / `R1` | 不拆 atomic unit；超 18 秒必须重裁或回退 source owner |
| `N4-GLOBAL-STYLE` | 写每组 `全局风格：` | `3-美学/画面基调/全局风格协议.md`、north_star 项目禁区、当前组证据 | 按 `north-star-projection-contract.md` 的当前画面基调投影口径写三行风格；第 1 行固定前置词 + 300 字以内当前组整理句，第 2/3 行引用或压缩画面基调指定字段 | `global_style_projection_map` | `N5` / `R1` | 每组均有三行；不能完整照抄母稿 |
| `N5-VISUAL-STYLE` | 写每组 `画面风格：` | 当前组分镜、摄影/光影信息 | 从组内镜头、构图、光影、色彩、观看位置和运动关系提炼自然语句 | `group_visual_style_map` | `N6` / `R1` | `画面风格：` 位于 `全局风格：` 后、第一个分镜行前 |
...
| `P2-boundary` | 分组边界是否以完整分镜行和约 15 秒为主 | `N3-GROUP-BOUNDARY` | `group_boundary_plan`, `duration_table`, `atomic_unit_table` | `N3` / `R1` |
| `P3-style` | `全局风格：` 与 `画面风格：` 是否各自取证且位置正确 | `N4-GLOBAL-STYLE` / `N5-VISUAL-STYLE` | `global_style_projection_map`, `group_visual_style_map` | `N4` / `N5` |
...
| `references/north-star-projection-contract.md` | 任意正式生成、repair、review | `全局风格：` 三行投影 | 完整照抄全局母稿或新增风格真源 | `N4-GLOBAL-STYLE` |
| `references/group-visual-tone-contract.md` | 默认光影稿或有镜头/光影信息 source | 组级 `画面风格：` 细则 | 替代 9-光影或摄影阶段创作 | `N5-VISUAL-STYLE` |
...
| `templates/` | 需要输出样板或报告对齐时 | 输出投影样板 | 另立输出路径或完成门 | `Output Contract` |
...
| --- | --- | --- | --- | --- |
| `default_lighting_grouping` | `types/grouping-type-map.md`, `references/group-boundary-contract.md`, `references/north-star-projection-contract.md`, `references/group-visual-tone-contract.md`, `references/statistics-yaml-contract.md`, `review/review-contract.md` | `N1-N8` | `GATE-GROUP-01` 到 `GATE-GROUP-16` | source line coverage、time ranges、YAML、旧字段 |
...
| `FAIL-GROUP-02, FAIL-GROUP-10, FAIL-GROUP-11` | `references/north-star-projection-contract.md`, `references/group-visual-tone-contract.md` | `R1/R2` | style gates | style order scan |
...
| `evidence_count` | 每集至少 1 个 `source_manifest`、`scene_map`、`shot_line_inventory`、`group_boundary_plan`、`global_style_projection_map`、`group_visual_style_map`、`stats_yaml`、`review_verdict` | `Thinking-Action Node Map.evidence` | `FAIL-GROUP-QUANT-EVIDENCE` |
| `pass_threshold` | 阻断 gate 为 0；组内时长 `<=18s`；ID 错误 0；旧连接件 0；旧字段 0；YAML 缺字段 0；source 无授权改写 0 | `N8.gate` / `Convergence Contract` | `FAIL-GROUP-QUANT-THRESHOLD` |
...
| `C3-boundary` | 每组边界完整，通常 12-18 秒，硬上限 18 秒 | 拆断 atomic unit、超时、跨场景混组 | `group_boundary_plan`, `duration_table` | `N3-GROUP-BOUNDARY` |
| `C4-assembly` | 组头、风格、回龙帧、YAML、保真均可检查 | 旧字段、连接件、风格缺失、YAML 缺失、source 改写 | `candidate_groups`, `source_preservation_diff` | `N4-N7` |
...
| 每组是否有场景标题行和三行 `全局风格：`？ | `GATE-GROUP-02` | `FAIL-GROUP-02` | `N4-GLOBAL-STYLE` | `global_style_projection_map` |
| 分镜组 ID 是否为 `x-y-z` 且跨场景重置？ | `GATE-GROUP-05` | `FAIL-GROUP-04` | `N3-GROUP-BOUNDARY` | `scene_group_index` |
| 边界是否以完整 shot line / atomic unit 和约 15 秒为主，且任一组不超过 18 秒？ | `GATE-GROUP-06` | `FAIL-GROUP-05` | `N3-GROUP-BOUNDARY` | `duration_table`, `boundary_reason` |
...
| `画面风格：` 是否来自组内摄影/光影信息且位于第一个分镜行前？ | `GATE-GROUP-12` | `FAIL-GROUP-10` | `N5-VISUAL-STYLE` | `group_visual_style_map` |
...
| `boundary_layer` | 拆断 atomic unit、跨场景混组、单组超过 18 秒 | `shot_line_inventory -> group_boundary_plan -> duration_table` | `N3-GROUP-BOUNDARY` | 同一 atomic unit 自身超过 18 秒时回退 source owner |
| `style_layer` | `全局风格：` 或 `画面风格：` 缺失、位置错误、证据不明 | `north_star/source evidence -> style map -> output position` | `N4-GLOBAL-STYLE` / `N5-VISUAL-STYLE` | 缺 north_star 或等价风格文本时停止 |
...
| `scripts/validate_storyboard_groups.py` | 检查分镜组标题、场景标题行、`全局风格：`、`画面风格：`、禁用旧字段、YAML、时间码连续累加、编号连续性和连接件残留；不能替代语义判断 |
...
- Completion gate: 输入、边界、`全局风格`、`画面风格`、首帧衔接/回龙帧、YAML、source 保真、机械校验和执行报告证据均通过；旧字段 `增补首帧：`、`入场镜头：`、`出场画面：`、`画面属性：`、`画面构图：`、六类位置细节字段、`回龙帧：` 和 `## A~B` 连接件块不得出现在新产物中。
...
- `Rule Evidence Map`：把 source、边界、风格、回龙帧、YAML、保真、下游 handoff 等规则映射到输出位置或报告证据。
...
| `ATTE-GROUP-02` | 注意力转移规则 | source 锁定后转 inventory；inventory 完成后转 boundary；boundary 完成后转风格与回龙帧；审查失败回到最近 source owner | `Thinking-Action Node Map` |
