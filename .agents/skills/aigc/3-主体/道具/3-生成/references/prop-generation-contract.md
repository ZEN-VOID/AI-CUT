...
| Step2 多视图 | 已取消；不套用道具多视图模板，不生成多视图主体设计图 | cancellation evidence |

Step2 多视图已取消；不得因 reference image、同画布主图节点或 prompt_only 状态恢复多视图生成。主图只在本地且当前画布缺同名节点时，上传动作只服务主图复用或后续显式参考，不服务默认多视图。
...
- 导入给 Midjourney V8.1 或 Lib Image 的主图提示词不得再以旧“提示词设计”英文整合 prompt 为主源。
...
- 项目绑定资产必须最终持久化到 `projects/aigc/<项目名>/3-主体/道具/3-生成/`；第 N 集画布上生成或复用的道具主图节点必须用 `libtv download` 同步到该目录，或证明本地 canonical 文件已存在；多视图已取消，不再作为本合同的同步对象。
...
| 道具主图是否已确保存在于项目 `道具/3-生成/`？ | `REV-PROP-GEN-14` | `FAIL-PROP-GEN-LOCAL-SYNC` | `P4-MAIN-IMAGE` | 报告 `local_sync_required`、`local_sync_action`、`local_sync_status`、`local_asset_path`、下载分支的 `download_command` / `download_stdout_path`，并检查本地文件 stem 与 libTV 节点名一致；本地 canonical 已有时允许 `already_present`。 |
...
| 是否遵守多视图取消合同，未生成、补齐、下载或验收 `-多视图`？ | `REV-PROP-GEN-03` | `FAIL-PROP-GEN-REFERENCE` | `Multiview Cancellation Contract` | 报告 cancellation evidence；历史多视图缺失不计失败。 |
| 是否没有把多视图 reference 缺失当作失败？ | `REV-PROP-GEN-07` | `FAIL-PROP-GEN-REFERENCE-CONTEXT` | `Multiview Cancellation Contract` | 报告多视图 reference gate 已取消。 |
| 真实生成模式下主图是否产出同 stem 的 `<主体ID>-<主体名称>-主图.<ext>` 与 `<主体ID>-<主体名称>-主图.json`？ | `REV-PROP-GEN-10` | `FAIL-PROP-GEN-MAIN-OUTPUT` | `P4-MAIN-IMAGE` | 报告主图图片路径、主图 JSON 路径、同一主体/材质/比例/识别点复核。 |
| 主图 JSON 是否能回指设计文档、`subject_id`、`subject_id_source` 和输出图路径，保证图像可复跑、可审查？ | `REV-PROP-GEN-11` | `FAIL-PROP-GEN-JSON` | `N3-MAIN-PROMPT` | 报告主图 JSON 路径、必填字段检查、`source_design_doc -> prompt -> output_image` 证据链。 |
...
| `prompt_only` 模式是否只落盘主图 JSON 和 planned path，并明确阻断原因，没有声称主图已经真实生成？ | `REV-PROP-GEN-09` | `FAIL-PROP-GEN-PROMPT-ONLY-CLAIM` | `N7-REVIEW` | 报告 `mode: prompt_only`、`blocked_reason`、已落盘 JSON、空图片路径或 planned output path。 |
