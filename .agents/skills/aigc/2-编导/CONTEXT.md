...
| --- | --- | --- | --- | --- |
| 编导稿仍像旧三阶段拼接 | 阶段整合层 | 回到 `steps/directing-workflow.md`，把 script/director/performance 证据内嵌到同一正文 | Output Contract 固定唯一 `2-编导` 真源，`2-编剧` 只作为上游剧本真源读取 | 不生成新的 `3-导演`、`4-表演` 主稿，也不在本阶段改写 `2-编剧` |
...
| 客观叙事被随意加成新台词 | 小说转译层 | 回到 `narration-to-voice-adaptation-contract.md`，先确认非引号来源、voice owner、知识依据和信息差安全；不合格则改回画面化、旁白或留白 | 派生语音只作为 source-grounded 表达手段，不作为新增剧情工具 | `narration_to_voice_adaptation_map` 完整，且不改写上游已有对白 |
| 跨度性衔接叙事被整段旁白化 | 剧情衔接层 | 拆成 `bridge_payload_units`，只把当前行动必需的 1-2 个信息簇转对白/独白，其余落到音效、道具、现场痕迹、动作或留白 | `continuity_bridge` 不等于信息倾倒；它要把衔接信息变成场内压力和下一步动作 | 派生语音删掉后剧情事实不变，但动作目标、场外状态或威胁方向仍清楚 |
...
| 旧 `3-导演` / `4-表演` 仍被当 active stage | 路由层 | 根路由、registry、sword6、3-运动和 4-摄影改为串接 `2-编剧 -> 2-编导 -> 3-运动 -> 4-摄影` | 旧名称只做兼容触发词并回接 `2-编导` | 主链为 `1-分集 -> 2-编剧 -> 2-编导 -> 3-运动 -> 4-摄影` |
...
9. 检查 `narration_to_voice_adaptation_map`：若终稿存在非原文对白/独白，必须确认其来源是非引号客观叙事，有合法 voice owner、知识依据、语音预算和就近画面承托；否则删回画面化或旁白。
10. 检查跨度性剧情衔接：若原文一段压缩上一轮结果、场外行动、角色分工、威胁升级和下一 beat，必须能看到 `bridge_payload_units`，且不是整段旁白复述。
11. 检查 `motion_enrichment_handoff`：`visual_unit_candidate_map` 应列出可被 `3-运动` 消费的角色动作或画面句；如保留 `cinematography_handoff`，不得含机位、景别、运镜、分镜编号或 prompt。
12. 检查旧 stage 路径泄露：本阶段新生产任务不得写 `3-导演/`、`4-表演/`，也不得改写 active 上游 `2-编剧/`；历史项目产物只做 legacy readback。
13. 交付前把 review finding 当修复输入，不把阻断项留成报告附件。
...
- 客观叙事转对白/独白的好用标准不是“更会说”，而是“这句话删掉后剧情事实不变，但场景更可演、信息更可感、节奏更利落”；一旦它承担新事实或替角色解释作者意思，就应删除。
- 跨度性衔接段最稳的处理是“一句确认场外状态，一句发出当前动作命令”，再用音效和现场痕迹补齐未说的信息；不要让角色像复述剧情梗概。
...
- `2-编剧` 已恢复为 active 上游剧本阶段；`2-编导` 可读取但不改写它。旧 `3-导演`、`4-表演` 目录仍是历史产物，不要继续追加 canonical 主稿。
