...
- outcome: 将 `0-Init` 从“父 skill + 初始组外置合同”回收成真正的单技能真源，路由、三种模式与充分性审计全部内收到父 `SKILL.md`。
- root_cause_or_design_decision: 虽然上一轮已经把 `0-Init` 重编为思行网络，但仍残留对外置初始组文档的强依赖，导致父 skill 不是唯一可执行真源；这与 `知行合一` 要求的“单技能内同时承载思考链与执行链”不一致。
- final_fix_or_heuristic: 把 `internal_router / advisor_council_engine / fast_draft_engine / autonomous_qa_engine / sufficiency_audit_engine` 全部显式写入父 `SKILL.md`，同时删除 `.codex/agents/aigc/初始组/*.md`，并让审计脚本反向检查 `0-Init` 不得再回指这些外置合同。
- prevention_or_replication_checklist:
  - [x] `0-Init/SKILL.md` 已新增 `Internal Capability Fusion Contract`
  - [x] `0-Init` 规范文件已移除对初始组文档的引用
  - [x] `scripts/aigc_skill_audit.py` 已改为拦截外置初始组依赖
  - [x] `.codex/agents/aigc/初始组/*.md` 已删除
- evidence_paths:
...
- user_feedback_or_constraint: 用户明确要求“不再需要 `.codex/agents/aigc/初始组/*` 这一堆 subagents，相关能力也要完全消化融合于 `SKILL.md` 中”。

### Case-20260412-AIGC-INIT-RUNTIME-MAPPING-CLARIFICATION

- milestone_type: source_contract_change
- outcome: 明确了 `0-Init` 里的“active 子路径骨架”说的是项目 runtime 预建落盘目录，不是技能树的执行入口目录；并为 `5-Image / 6-Video` 增加了技能树到 runtime 的映射表。
- root_cause_or_design_decision: 当前仓在 `5-Image` 与 `6-Video` 同时存在两套命名层：技能树里有 `1-提示词蒸馏/…`、`2-视频生成`，而 runtime 落盘又使用 `分镜故事板 / 分镜帧 / 漫画 / 全能参照 / 首帧参照 / 生成任务`。原合同虽然选择了正确的 runtime 口径，但没有把这层区别说透，读起来会误以为初始化目录与“当前状态”不一致。
- final_fix_or_heuristic: 把 `_shared/project-runtime-layout.md` 升级为显式 `Skill Tree To Runtime Mapping` 真源，并在 `0-Init/SKILL.md` 的 bootstrap skeleton 章节同步标注：为什么 `5-Image/2-图像生成` 不在默认预建列表、为什么 `6-Video/2-视频生成` 在 runtime 里落成 `生成任务/`。
- prevention_or_replication_checklist:
  - [x] `_shared/project-runtime-layout.md` 已新增技能树到 runtime 的映射表
  - [x] `0-Init/SKILL.md` 已标注 `5-Image / 6-Video` 的两套命名区别
  - [x] `0-Init/CONTEXT.md` 已收录该混层问题与修复策略
- evidence_paths:
  - `.agents/skills/aigc/_shared/project-runtime-layout.md`
  - `.agents/skills/aigc/0-Init/SKILL.md`
  - `.agents/skills/aigc/0-Init/CONTEXT.md`
- user_feedback_or_constraint: 用户指出“初始化目录结构与当前状态似乎并不完全匹配”，要求核对并修正口径。

### Case-20260411-AIGC-INIT-LIGHTWEIGHT-BOOTSTRAP

- milestone_type: source_contract_change
- outcome: 将 `0-Init` 的初始化输出从“首次起盘默认全量治理工件”收敛为“核心真源立即生成，治理载体按需惰性生成”。
- root_cause_or_design_decision: 先前合同把 `governance-state / mandate / mission-brief / route-plan / preflight / validation / learning` 与 `north_star` 同时前置，导致创作起盘被治理复杂度绑死。
- final_fix_or_heuristic: 把首次必出固定为 `north_star / init_handoff / story-source-manifest / team / project_state`，其余治理载体只在深治理、复杂执行、高风险任务或卫星技能真正需要时再补。
- evidence_paths:
  - `.agents/skills/aigc/0-Init/SKILL.md`
  - `.agents/skills/aigc/_shared/project-runtime-layout.md`
...
- root_cause_or_design_decision: `1-分集` 已要求读取故事源集合，但 `0-Init` 之前没有项目级故事源落点与 manifest。
- final_fix_or_heuristic: 把故事目录固定为 `projects/<项目名>/Story/`，把登记真源固定为 `projects/<项目名>/0-Init/story-source-manifest.yaml`，并把“增量进入分集”和“整季正式分集完成”拆成两层 readiness。
- evidence_paths:
