...
| `fantasy_xuanhuan` | `types/action-destruction-fx/fantasy-xuanhuan.md` | 玄幻、奇幻、仙侠、仪式、法阵、灵光、超自然力量被上游和 `3-美学` 授权。 |
...
- 类型裁决优先级：用户显式信号 > `3-美学` > `5-表演` 场景/动作/材质证据 > 项目 `MEMORY.md` > 物理氛围知识库。
...
- 本目录没有统一禁词。`灵光`、`法阵`、`能量波`、`护盾`、`粒子`、`科技光效` 等是否允许，由 `destruction_type_route.type`、上游 source、`3-美学` 和项目 `MEMORY.md` 共同裁决。
- 对不支持法术/科技光效的类型，相关表达属于类型越界。
- 对玄幻、奇幻、仙侠、科幻或赛博类型，只要上游和 `3-美学` 已授权，对应光效/能量/法阵/科技故障可以作为力量来源，但仍必须落到受击材质和剧情保真边界。
