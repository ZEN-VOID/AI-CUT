---
name: comic
description: Use when 漫画项目需要从任意来源或小说进入完整四段链：漫画小说改编、九刀流 9 页漫画提示词 JSON、Seedream 一次生成 9 张连续竖版漫画页，以及当前剧集海报设计 JSON，并需要统一落盘到 projects/comic/[项目名]/。
...
本技能是 `.agents/skills/comic/` 的父级总入口，负责把漫画项目路由到四段受治理子技能，并统一项目落点：
...
- `task_intent`
  - `adapt_novel | make_prompts | generate_images | design_posters | full_pipeline | full_pipeline_with_poster | inspect`
...
flowchart LR
    ...
    D -->|"nine_blade + bridge pack"| E
    D -->|"9 images + reports"| E
    E -->|"第N集-剧集海报.json"| F["交付证据"]
```
...
erDiagram
    ...
    COMIC_PROJECT ||--|| EPISODE_POSTER_JSON : stage_4
...
| 2 | [2-九刀流漫画提示词](2-九刀流漫画提示词/SKILL.md) | 漫画小说主稿、桥接包或用户小说 | 单集默认 `nine_blade_comic_prompts.json`；多集默认 `第N集-nine_blade_comic_prompts.json` | `projects/comic/[项目名]/2-九刀流漫画提示词/` |
| 3 | [3-漫画生成](3-漫画生成/SKILL.md) | `nine_blade_comic_prompts.v1` JSON | 9 张漫画页、Seedream 报告、生成报告 | `projects/comic/[项目名]/3-漫画生成/` |
| 4 | [4-剧集海报](4-剧集海报/SKILL.md) | 当前集小说/桥接包/九刀流 JSON，及可选生成页 | `第N集-剧集海报.json` | `projects/comic/[项目名]/4-剧集海报/` |
...
| `N2-ROUTE` | 判断进入哪一段 | 按输入类型和任务目标路由 1/2/3/4 | 输入文件/文本类型 | 对应子技能 | 路由唯一 |
...
- 用户要求“做完整漫画项目”或明确要海报：走 `full_pipeline_with_poster`，依次走 1 -> 2 -> 3 -> 4。
- 用户给小说并要求“出 9 张图提示词”：走 2。
- 用户给 `nine_blade_comic_prompts.json` 并要求“生成漫画”：走 3。
- 用户要求“给这集做海报 / 海报 JSON / 剧集 poster”：走 4。
...
- 下游不得把图片或报告回写到上游目录。
- 4 号技能只产 JSON，不直接回写图片。
- 若用户显式指定其他输出根，必须在交付中说明偏离原因。

## 8. Root-Cause 合同

若出现路由错段、输出落到 `output/comic`、找不到上游 artifact、九页 JSON 与图片目录脱节，或剧集海报脱离当前集事实，按以下链路上溯：
