...
| Completion gate | `3-初稿` 源章已读取；DeepSeek provider 真实命中；显式 subagents 模式下已按 `story/review` 维度子技能完成审计并把 findings 注入 DeepSeek repair brief 直接优化正文；输出通过 frontmatter、heading、最小修补、中文表达与题材质感门禁；正式正文已写回 canonical path。 |
