...
- 上层 SKILL 语境：`story2026/0-初始化` 已完成模式锁定，但 `project_contract.creative_mandate`、`planning_seed`、商业定位或反套路约束仍存在真实缺口。
- 当前模块负责对象/范围：治理 `0-初始化` 内所有创意相关 leaf references 的统一路由，包括约束包、卖点、复合题材、灵感救援、平台定位、反套路映射与趋势校准。
- 模块归属类型：多种情况触发之一。
- 进入触发信号：
...
- target_object: `0-初始化/references/creative-seed-routing/module-spec.md`
...
- 如果不是这个结果，会不会有更好的答案：
  ...
  - 成立轴判断：若 provenance 不清，`0-初始化` 下游就无法区分用户承诺和助手推断。
...
- 动作：
  ...
  - 若缺口属于对象层或 worldbuilding，立即回退到共享 `templates/worldbuilding/` 或下游 `1-设定` 路由，不在本模块硬补。
- 产出：
...
- 类型：非内容输出型
- 模式：为 `0-初始化` 的创意相关缺口提供统一的 leaf reference 路由与结构化写回。
- 正式落点：
  - `0-初始化/north_star.yaml.project_identity.target_reader / platform`
  - `0-初始化/north_star.yaml.story_kernel`
  - `0-初始化/north_star.yaml.reader_promise`
  - `0-初始化/init_handoff.yaml.project_contract.creative_mandate / promise_surface`
  - `0-初始化/init_handoff.yaml.stage_entry_seeds.planning_seed.anti_trope / hard_constraints / story_engine / opening_hook`
  - `0-初始化/init_handoff.yaml.unknowns.deferred_to_planning`
  - `0-初始化/init_handoff.yaml.risk_notes`
- 上层消费方式：
...
- acceptance checklist：
  ...
  - [x] `0-初始化` 与 planning 固定题包直答 都只依赖本模块入口，不再散点点名 leaf references。
