...
def test_load_dotenv_migrates_legacy_global_env(monkeypatch, tmp_path):
    claude_root = tmp_path / ".claude-home"
    legacy_env = claude_root / "webnovel-writer" / ".env"
    legacy_env.parent.mkdir(parents=True, exist_ok=True)
    legacy_env.write_text("RERANK_BASE_URL=https://legacy.example.com\n", encoding="utf-8")
    ...
    assert os.environ.get("RERANK_BASE_URL") == "https://legacy.example.com"
    assert (claude_root / "story2026" / ".env").is_file()
