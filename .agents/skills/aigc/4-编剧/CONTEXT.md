...
5. 若已有 `3-主体/场景/2-设计`、`3-主体/场景/3-生成` 或 `design-manifest.yaml`，先建立 `scene_asset_context_manifest`；只把场景资产用于场景标题、环境白描、空间连续性和 handoff，不能覆盖 source 事实、注册表或写 prompt/镜头。
6. 检查题材画像是否能解释节奏，不解释就回 `N2`。
7. 检查场景标题是否包含天气后缀；未知天气用 `天气待定` 并报告 followup。
8. 对新增对白/独白逐条查 source anchor、voice owner、知识依据和语音预算。
9. 对每个节奏机制查承托字段；无承托就删机制或补正文落点。
10. 检查相邻画面字段是否其实是同一拍摄单位：同一主体、同一停顿、同一手部动作、同一道具状态、同一声源反应要合并；只有主体/空间/时间/信息/节奏功能变化时才保留连续字段。
11. 如果“画面化”变成比喻或概念，先删掉“像/仿佛/宿命/高级/压迫感”等词，再补可见身体动作、道具状态、空间距离、声源、光照和时间变化。
12. 用户指出“脚本化/偷懒/未思考/未差异化”时，不接受逐句润色；直接废弃候选稿，回 `N2-N6` 重建证据和正文。
13. 高潮只强化已有事件的声画/情绪/行动，不改结果。
14. 尾钩必须落在最后可感对象上，不接受“悬念拉满”类总结。
15. 报告服务修复和下游交接，必须能回指正文位置。
16. 报告不得写自由散文式“思考过程”；必须写可审计的 `Execution Decision Trace`、`Reference Execution Matrix` 和 `Upstream Creative Direction Matrix`。
