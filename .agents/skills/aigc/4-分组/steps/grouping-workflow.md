...
| `constraint_profile` | 对白 4-6 句、纯分镜剧本正文目标 <= 1680 字且硬上限 <= 1980 字、画面句子多分镜不可截断、相邻组具备 3-4 秒首尾帧连接件 |
| `success_criteria` | 每组 ID 真实、风格投影含置顶于第 1 行最前的全局固定前置词、正文保真、组间连接自然、统计 YAML 可复查 |
| `non_goals` | 不改剧情、不改对白、不重写原有分镜明细、不生成图像/视频提示词 |
| `complexity_source` | 边界裁决、组间首尾帧连接件、字数与完整性汇流 |
...
| `N3-SCENE-MAP` | 建立集/场/atomic unit 映射 | 摄影稿正文 | 提取场景标题、字段、分镜明细块和对白数 | scene unit table | `N5-GROUP-PLAN` | atomic unit 不跨场景 |
| `N5-GROUP-PLAN` | 裁决组边界 | scene unit table、style projection | 按三重约束形成组计划；1680/1980 只约束纯分镜剧本正文，超过 1680 字时先拆分复核，1980 仅作硬上限 | group boundary plan | `N6-CONNECTOR` | 每组纯正文目标 <= 1680，例外 <= 1980 且完整 |
| `N6-CONNECTOR` | 设计组间首尾帧连接件 | 相邻组首尾 atomic unit、style projection | 在第 N+1 组完成后回看第 N 组原尾帧与第 N+1 组原首帧，逐对设计 3-4 秒连接件并判断同场景/跨场景连接，内部选择依赖型/流动型/变形型/复合型/无连接方法论，落盘为三项 north_star 风格行和具体画面连接办法；不复述端点，改写为变化过程、主体运动、运镜设计和透视适应，末尾用 `避免元素` 写负面约束，连接件 ID 固定为 `上一个分镜组ID~下一个分镜组ID` | connector list | `N7-ASSEMBLE` | 相邻组都有连接件，且不新增剧情、不使用旧入场/出场尾钩、不输出 `连接件提示：` |
| `N7-ASSEMBLE` | 组装分组稿 | group plan、connector list、style projection | 写组头、原正文、YAML 统计，并把组间连接件物理夹放在对应上下两个相邻分镜组之间 | episode group draft | `N8-REVIEW` | 正文同步原换行 |
...
flowchart TD
    ...
    F1 -->|"组超 1680 字未复核、超 1980 字或分镜明细被截断"| R5["返回 N5-GROUP-PLAN"]
...
| 纯正文超 1680 字且无例外理由，或纯正文超 1980 字 | `N5-GROUP-PLAN`，移动完整 atomic unit |
