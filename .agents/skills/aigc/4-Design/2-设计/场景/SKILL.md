...
7. `.agents/skills/aigc/4-Design/1-清单/_shared/list-output-contract.md`
8. `.agents/skills/aigc/4-Design/1-清单/场景/SKILL.md + CONTEXT.md`
9. 本 `SKILL.md + CONTEXT.md`
10. `references/scene-design-assembly.md`
11. `templates/scene_masterprompt.structured.v2.md`
12. `projects/aigc/<项目名>/4-Design/场景/1-清单/第N集/场景清单.json`
13. `projects/aigc/<项目名>/4-Design/场景/1-清单/第N集/场景研究.json`（若存在）
14. `projects/aigc/<项目名>/4-Design/场景/1-清单/第N集/scene_design_bridge.json`（若存在）
...
| `FIELD-SCN-DES-12` | `[场景名].<ext> / _manifest.json.auto_image` | 自动生图使用含全局风格前缀的完整 prompt，图片与设计文件同目录同 stem | `design-output-contract.md`、`全局风格.md`、`[场景名].md` | `S10` | auto image completeness | `FAIL-SCN-AUTO-IMAGE` |
...
| `S10` | 单主体自动生图 | 读取 `[场景名].md` 与全局风格前缀；自动生图前复验空镜锚句，再调用 nano-banana general 输出同目录同名图片 | `auto_image_packet + reference_cleanliness_note` | `done` | `S7-S10` |
...
| `N10-AUTO-IMAGE` | 生成单场景概念图 | `[场景名].md`、`全局风格.md`、`design_prompt` | 生成 `full_generation_prompt`；复验空镜锚句与污染主体禁令后调用 nano-banana general，输出 `[场景名].<ext>` | `auto_image_packet + reference_cleanliness_note` | `done` | 不得只传局部 prompt；图片必须同目录同名；空镜门禁失败不得生图 |
...
| `FIELD-SCN-DES-12` | `[场景名].<ext>` 已由 `full_generation_prompt` 生成，且 stem 与 `[场景名].md` 一致 | `FAIL-SCN-AUTO-IMAGE` | `S10` |
...
7. 已使用含统一全局风格前缀的 `full_generation_prompt` 自动生成 `[场景名].<ext>`。
