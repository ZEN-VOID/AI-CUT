...
- `projects/aigc/<项目名>/6-设计/道具/2-设计/<主体名称>.md`
...
- 项目绑定资产必须最终持久化到 `projects/aigc/<项目名>/6-设计/道具/3-生成/`。
...
| Step1 是否产出同 stem 的 `<主体ID>-<主体名称>-主图.<ext>` 与 `<主体ID>-<主体名称>-主图.json`，且资产最终持久化在项目 `6-设计/道具/3-生成/`？ | `REV-PROP-GEN-04` | `FAIL-PROP-GEN-NAMING` | `N4-MAIN-IMAGE` | 报告主图图片路径、主图 JSON 路径、basename 对照、canonical path 存在检查。 |
...
| 所有项目绑定资产是否最终持久化到 `projects/aigc/<项目名>/6-设计/道具/3-生成/`，没有只停留在临时目录、`$CODEX_HOME` 或 imagegen provider 输出区？ | `REV-PROP-GEN-05` | `FAIL-PROP-GEN-PERSISTENCE` | `N4-MAIN-IMAGE` | 报告最终 workspace 路径、临时源路径迁移说明、文件存在检查。 |
