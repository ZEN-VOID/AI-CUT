...
- 调用 `.agents/skills/aigc/4-Design/角色/SKILL.md` 时，必须同时加载本文件。
...
| --- | --- | --- | --- |
| `ROLE-TM-01` | 角色输出仍散落在 `角色/1-清单`、`角色/2-设计`、`角色/3-面板` | 收束到 `projects/aigc/<项目名>/4-Design/` 根，并保留旧 JSON 为兼容侧车 | 根目录存在 `角色清单.md`、`[角色名].md`、`[角色名].json` |
