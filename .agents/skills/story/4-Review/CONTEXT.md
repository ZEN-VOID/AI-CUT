...
- 本文件是 `4-Review` 父技能的经验层知识库，不是第二份阶段合同。
- 每次调用 `4-Review` 时，应与 `SKILL.md` 一起加载，用于识别卷级 fact pack 缺口、并发聚合错误、`code-reviewer` findings 汇流问题，以及 review/loopback 接驳问题。
...
| --- | --- | --- | --- | --- |
| 仍把 `4-Review` 当成单章终验 | stage unit contract | 把父层 aggregate sink 改成卷级 `第V卷.validation.json` | 固定“卷级 gate，章级 issue index”双层模型 | review/loopback 只消费卷级 aggregate |
...
| `code-reviewer` 已跑完，但 findings 没并入 aggregate JSON | reviewer aggregation gap | 把外部 findings 映射回 `issues / severity_counts / rework_targets / source_trace` 再写 aggregate | 固定 `4-Review` 父层拥有外部 reviewer 汇流权，不能只留 sidecar | `4-Review/*.validation.json` 与审计 sidecar 同时可追溯 |

## Repair Playbook

1. 先确认这是卷级验收，不要先回到单章思维。
...
- 卷级终验最值钱的不是“把 10 章分别打分”，而是回答“这整卷现在是否能作为一个被放行的创作单元”。
