# 5-设计 Incremental Reconciliation Contract

## Purpose

`5-设计` 必须支持上游 `4-分组` 分批完成的生产节奏。项目可以先完成若干集分组并执行设计，之后继续追加新的 `第N集.md`；后续执行不得把既有清单、设计稿或生成资产当作一次性全量覆盖对象。
...
```text
projects/aigc/<项目名>/5-设计/<域>/design-manifest.yaml
```
...
1. 锁定当前可读上游：`projects/aigc/<项目名>/4-分组/第N集.md`，或用户指定的集号子集。
