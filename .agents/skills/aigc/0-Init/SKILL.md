---
name: aigc-init
description: Use when initializing or reinitializing an AIGC film project through smart-advisor mode, with auto/custom lineup selection from `.agents/skills/team/`, `team.yaml` as the team manifest, and planning-led subagent direct-answer packets before `north_star` synthesis.
...
- If the task is bound to `projects/aigc/<项目名>/`, load project `MEMORY.md` first, then relevant files under project `CONTEXT/`, before executing initialization nodes.
...
- Build `0-Init/`, project `MEMORY.md`, project `CONTEXT/`, `team.yaml`, `STATE.json`, and the core initialization artifacts.
- Lock a project-level `north_star` before entering `1-Planning`, `2-Global`, or later AIGC stages.
...
All canonical business outputs must land under `projects/aigc/<项目名>/`, with initialization-owned outputs under `projects/aigc/<项目名>/0-Init/` and project-root carriers at the project root.
...
| --- | --- | --- | --- |
| project root carriers | `projects/aigc/<项目名>/MEMORY.md`, `CHANGELOG.md`, `CONTEXT/`, `STATE.json` | project memory, trace, shared context, live route truth | `references/scope-and-runtime.md` |
| north star | `projects/aigc/<项目名>/0-Init/north_star.yaml` | long-lived creative and production constraints | `references/artifacts-and-sources.md` |
| init handoff | `projects/aigc/<项目名>/0-Init/init_handoff.yaml` | next-stage seeds, unknowns, source breakdown | `references/artifacts-and-sources.md` |
| story source manifest | `projects/aigc/<项目名>/0-Init/story-source-manifest.yaml` | source readiness and coverage truth | `references/artifacts-and-sources.md` |
...
`0-Init` keeps routing, mode locking, lineup decision, planning direct-answer dispatch, synthesis, and sufficiency audit under the parent skill. Partition files expand the contract but do not become independent entry points.

- `0-Init` 现只允许 `init_mode == smart_advisor`；旧的 `主创会诊模式 / 快速成案模式 / 自主问答模式` 全部失效。
...
4. In `N2`, create the canonical project runtime skeleton, project `MEMORY.md`, project `CONTEXT/`, and 同步创建项目根 `CHANGELOG.md` 作为时间序记录入口。
...
| --- | --- | --- | --- |
| `FIELD-INIT-01` | `N5` | `0-Init/north_star.yaml` | Long-term constraints only; no route truth. |
| `FIELD-INIT-02` | `N5` | `0-Init/init_handoff.yaml` | Stage-entry seeds, unknowns, source breakdown. |
...
| `N2` | `FIELD-INIT-05` | are root carriers and skeleton ready? | create runtime roots and project support files | missing `CHANGELOG.md`, `MEMORY.md`, or `CONTEXT/` |
...
| --- | --- | --- | --- |
| `FIELD-INIT-01` | `north_star.yaml` only contains durable constraints | `FAIL-INIT-01` | `N4/N5` |
...
For Skill 2.0 maintenance of this package, also keep `README.md`, `TODO.md`, `CHANGELOG.md`, `CONTEXT.md`, and `agents/openai.yaml` synchronized with any contract move.
