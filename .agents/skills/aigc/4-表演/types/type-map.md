...
| `performance_style_dependency` | `required` / `optional` / `not_applicable` | 上游 `3-导演` 提供 `performance_style_directive`，或角色表演外放度、身体性、声线、面具/真实轴需要被风格限制 | `N2-PERF-TYPE` / `N4-PERF-ACTOR-CONTROL` |
| `emotional_register_dependency` | `required` / `optional` / `not_applicable` | 上游提供 `emotional_rhythm_map`，或场景存在压抑、爆发、冷却、反高潮、类型底色情绪差 | `N2-PERF-TYPE` / `N4-PERF-ACTOR-CONTROL` |
...
| `audience_psychology_dependency` | `required` / `optional` / `not_applicable` | 上游提供 `audience_psychology_map` / `conflict_legacy_transfer`，或存在观众与角色知情差、期待/恐惧/渴望、误判、隐瞒、反应延迟 | `N5-PERF-SCENE-CRAFT` |
...
| `monologue_budget_need` | `required` / `optional` / `not_applicable` | 内心独白、旁白、心理解释或抽象动机文字可能挤占可外显表演 | `N3-PERF-PSYCHOLOGICAL` / `N5-PERF-SCENE-CRAFT` |
...
| `advisor_need` | `required` / `optional` / `not_applicable` | 用户要求、项目 team 配置或 顾问与复核流程启动 | `N6.5-PERF-ADVISOR` |
...
performance_type_profile:
  ...
  performance_style_dependency: required | optional | not_applicable
  emotional_register_dependency: required | optional | not_applicable
  actor_control_need: required | optional | not_applicable
  dialogue_performance_need: required | optional | not_applicable
  audience_psychology_dependency: required | optional | not_applicable
  subtext_behavior_need: required | optional | not_applicable
  scene_turn_need: required | optional | not_applicable
  monologue_budget_need: required | optional | not_applicable
