...
2. `init_handoff.yaml` carries stage-entry seeds, source layers, and unknowns.
3. Session-only information does not belong in `north_star.yaml`.
4. `north_star.yaml` must not contain route truth or `rebootstrap` process state.
...
4. After the project leaves `0-Init`, `init_handoff.yaml` remains historical handoff seed and no longer owns live current-stage truth.
...
Conflicting assistant-inferred story fields must be rewritten before entering `1-Planning`, `2-Global`, or later stages.
