...
1. `1-规划` 只负责登记 `bootstrap_output` 目标路径与 `source_profile` handoff，不在规划阶段默认创建 `projects/<项目名>/编导/第N集.json`。
2. `2-组间` 首次进入且根文件缺失时，必须先基于 shared bootstrap template 自动创建 `第N集.json`。
3. `2-组间` 每次执行前都要完整读取现存或刚初始化的 `第N集.json` 作为上下文，而不是只读单个组。
4. `2-组间` 只 patch 组级字段，不创建第二份 episode/group 真源。
5. `3-明细` 继续在同一根文件上 patch 镜级字段。
