...
| 设计包目录完整但新版必需控制块不可解析 | Skill 2.0 runtime-spine 层 | 保留旧正文，在 `SKILL.md` 追加 Business、Type Routing、Node Map、Module Loading/Trigger、Convergence、Review、Quant、Attention、Checkpoint 和 Evaluation prompts | README/CHANGELOG/test-prompts 同步，legacy workflow 降为 reference | validator/smoke 能定位控制块，旧语义仍可由 references 和正文保留 |
...
- runtime-spine 升级不要为了新版表格删除旧设计语义；先追加可解析主脊柱，再逐步把旧细则纳入授权 reference、review 或模板边界。
