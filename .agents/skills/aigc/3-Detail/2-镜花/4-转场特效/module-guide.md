...
- 本分支负责在人物和画面主轴已成立后，克制式补充组内、组间转场和必要特效。
- 它在 `运镜手法` 之后按当前序号执行；若已有 `camera_patch` 或 `cinematography_patch`，只作为兼容性 side input。
- 它拥有“要不要补”而不是“必须补”的判断权。
...
- 只有在直切不足时，再决定命中哪些叶子：
  - `组内` 负责相邻分镜切接是否需要被显化。
  - `组间` 负责当前组结尾怎样基于 `尾出帧 -> 首入锚` 把观众送入下一组。
...
- `组间`：先锁 `尾出帧 -> 首入锚` 的主导 delta，再在依赖型 / 流动型 / 变形型 / 复合型里选最轻的送入语法。
