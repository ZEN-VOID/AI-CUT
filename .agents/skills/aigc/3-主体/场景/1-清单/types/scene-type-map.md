...
| --- | --- | --- |
| `source_specificity` | registry 场景名是明确空间、泛称、别名还是组合地点？ | 决定是否需要证据回查与待核标记。 |
| `alias_evidence` | source anchor 或项目上下文能否证明两个名称指向同一空间？ | 决定是否可归并为 canonical 名称。 |
...
- 代称归并必须能回查 source anchor 或项目上下文，不能只凭语感。
