...
| --- | --- | --- | --- |
| `G1-SOURCE` | 每个 `group_id` 可回指 `8-分组` 源标题、组正文和 YAML | `FAIL-SHEET-GROUP` | `references/group-source-extraction.md` |
...
| `G3C-UPSTREAM-DIRECTION` | 报告包含 `Image Upstream Visual Direction Matrix`，说明上游美学、主体、分组稿、可选平面图和项目上下文如何导向 source comprehension、frame units、layout、visual atoms、参照保真和 handoff；上游电影风格只作 evidence-only | `FAIL-SHEET-UPSTREAM-DIRECTION` | `../../_shared/upstream-context-application-contract.md` |
| `G4-CONTENT` | prompt 主体直接引用 `8-分组` 对应分镜组完整内容，源分镜顺序与底部 YAML 完整 | `FAIL-SHEET-PROMPT` | `references/prompt-assembly-contract.md` |
...
| `G8A-LAYOUT-ASPECT` | `layout_aspect_decision` 先按实际 panel 数和目标单格比例反推候选行列、整图比例、`panel_geometry_blueprint` 与适配 imagegen 4K 目标的尺寸策略；每个 image box 锁定 16:9 且 `panel_image_box_ratio_error <= 0.06`；不得固定整图比例后挤压 panel | `FAIL-SHEET-LAYOUT-ASPECT` | `references/imagegen-handoff.md` |
...
| `G9-HANDOFF` | 已加载并调用 `.agents/skills/cli/imagegen`；imagegen mode 合法，批量执行符合最大并发 10，不走 CLI/API/provider 专属控制，不得以 plan-only 通过 | `FAIL-SHEET-IMAGEGEN` | `references/imagegen-handoff.md` |
