...
| --- | --- | --- | --- | --- |
| GPT 原生流只读章级 planning，漏读north_star 全局/风格/类型契约 | context pack contract | 回补 `0-初始化/north_star.yaml` 的 `global_contract`、`style_contract` 与 `genre_contract` | 把完整上下文加载与 sidecar 证据链作为硬门槛，YAML 头只保留 `润色模型` | sidecar 中三类上下文齐备，正文能读出吸收痕迹 |
...
| GPT 原生正文缺少 sidecar，无法证明读过哪些源 | evidence chain gap | 运行 `polish_chapter_gpt_native.py --dry-run` 生成 context pack | 在 Completion gate 中固定 context pack / authored draft / writeback 证据链 | `reports/4-润色/gpt-native/...` 能追溯输入和输出 |
...
2. 若 YAML 头缺字段，只检查并补正 `润色模型: GPT`；上下文缺口回到 context pack / sidecar，而不是补进正文头。
