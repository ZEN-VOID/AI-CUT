...
- `prompt.md` 必须采用 source-first YAML 两阶段处理：draft 阶段直接保留 `4-分组` 原文和原始 fenced YAML，不提前写死 `reference_index / uploaded_url`、空 URL 或占位 URL；final 阶段只在 fenced YAML 的 `分镜画面参照` 中注入最终 `reference_index`、真实 `uploaded_url` 和可选 `image_token`。
