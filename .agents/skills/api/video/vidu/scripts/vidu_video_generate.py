#!/usr/bin/env python3
"""
Vidu video generation create CLI.

Current scope:
- submit/create: create a Vidu task via POST /v1/video/generations
...
STATUS_VALUES = {"queued", "processing", "succeeded", "failed"}


def _vidu_model_sort_key(model: str) -> tuple[int, int, str]:
    match = re.fullmatch(r"Vidu-q(?P<major>\d+)(?:-(?P<variant>[A-Za-z0-9-]+))?", model, re.IGNORECASE)
    if not match:
        return (-1, -1, model.lower())
...
def _safe_name(text: str, max_len: int = 64) -> str:
    ...
    return (normalized or "vidu_video")[:max_len]
...
def _default_output_dir(project_name: str) -> Path:
    ...
    return Path("output") / "影片" / project / "5-API" / "video" / "vidu"
...
def _make_report_path(output_dir: Path, prefix: str, command: str, explicit: Optional[str]) -> Path:
    ...
    stem = _safe_name(prefix or "vidu")
...
def _build_validation_notes(
    *,
    ...
    if response_body:
        text_parts: List[str] = []
        pending: List[Any] = [response_body]
        while pending:
            current = pending.pop()
            if isinstance(current, dict):
                for key, value in current.items():
                    if isinstance(value, str) and key.lower() in {"message", "error", "detail", "msg", "code", "type"}:
        ...
        if any(signal in merged.lower() for signal in auth_signals):
        ...
        if "model_not_found" in merged.lower() or "no available channel for model" in merged.lower():
            notes.append(
                "当前更像网关通道未开通该模型；若公共文档已出现更高版本，请先区分“官方已发布”与“当前 AnyFast/FineAPI key 已开通渠道”这两个层级。"
            )
    return notes
...
def build_parser() -> argparse.ArgumentParser:
    ...
    common_parser.add_argument("--filename-prefix", default="vidu", help="报告文件名前缀")
    common_parser.add_argument("--report-json", help="显式指定报告 JSON 路径")

    parser = argparse.ArgumentParser(
        description="Submit Vidu video generation create tasks",
    ...
    submit_parser.add_argument(
        ...
        help=f"模型 ID；不传时自动取当前最高通用 Vidu 型号（当前 {DEFAULT_MODEL}）",
    )
...
def main() -> int:
    ...
    if not api_key:
        raise ValueError(
            "缺少 API Key，请在 .env 中配置 ANYFAST_VIDEO_API_KEY / VIDU_API_KEY / ANYFAST_API_KEY / FINEAPI_API_KEY"
        )
    if not args.base_url:
        raise ValueError(
            "缺少 Base URL，请配置 VIDU_API_BASE_URL / ANYFAST_API_BASE_URL / FINEAPI_API_BASE_URL 或显式传 --base-url"
        )
