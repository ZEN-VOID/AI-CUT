...
本技能不是摄影分镜、灯光设计图、舞台执行单、VFX 参数表、图像 prompt 或视频生成技能。它只把烟雾、打灯、鼓风机、自然元素、天气模拟、水火尘雪、投影影纹、气味/温度、动作破坏点材质响应等可见可感的氛围手段，写成服务叙事、动作和场景的白描式画面化正文。
...
- 必须读取可用的 `2-美学` 产物，优先级为 `画面基调`、当前集 `角色风格`、当前集 `场景风格`。`画面基调` 始终读取全局 singleton；若能从 source 推断 `第N集`，角色/场景风格必须先读 `2-美学/第N集/<风格>/...`，缺失时回退项目级 `2-美学/<风格>/...`。若任一仍缺失，可使用用户提供的等价风格文本，但必须在报告记录降级来源和 N/A 理由。
- 正式生成、repair 或 review 时，必须加载 `../_shared/upstream-context-application-contract.md`，并在执行报告中记录 `Upstream Context Application Map`：说明 `5-表演` 触发点、表演焦点和 `2-美学` 三类协议如何被投影为选择性 `氛围画面`、物理氛围手段、时间锚点和保真边界。
...
- 冲突优先级：用户显式请求 > 根 `AGENTS.md` / meta 规则 > 本 `SKILL.md` > 本 `Module Loading Matrix` 授权模块 > 上游 `5-表演` 原稿 > `2-美学` 产物 > 项目 `MEMORY.md` > 项目 `CONTEXT/` > 本 `CONTEXT.md` > 知识库或网络资料。
...
| `B5` | `Module Loading Matrix` | 授权 references、types、knowledge-base、agents 和 test prompts 的职责 |
| `B5A` | `Module Trigger Matrix` | 将任务信号和 `FAIL-*` 映射到 reference/type 组合、加载阶段和回流门 |
...
- 输入是 `5-表演/第N集.md`、演员表演稿、经过剧本化处理 + 导演批注 + 演员表演画面化描述后的稿件，且需要结合 `2-美学` 风格上下文。
...
- 对动作强触发点先建立 `destruction_type_route`：显式挂钩用户类型信号、`2-美学`、`5-表演` 场景/动作/材质证据和项目 `MEMORY.md`，裁决类型分支或回退到 `generic_material_response`。再按路由加载 `types/action-destruction-fx/<type>.md` 并建立 `action_destruction_fx_profile`：把动作来源、受击材质、破坏材料、节奏强度和风格边界绑定起来；类型模块提供各自的材质库和专属边界，但不得替代本 `SKILL.md` 的触发、门禁或输出合同。
- 在触发点附近新增 `氛围画面：XXX` 字段。`XXX` 必须按白描式画面化直写，具体、具备时间属性，并与相邻动作或画面点绑定，例如“与此同时……”“在他转身之后……”“话音落下的两秒里……”。
...
- 不得用明喻、隐喻、象征或概念解释替代氛围正文，例如“空气像被按了暂停”“光像刀一样”“仿佛命运降临”。删除这些词后，字段仍必须保留清楚的物理氛围、感官通道、空间/材质状态和时间变化。
- 动作破坏点不得写成当前类型、上游 source、`2-美学` 或项目 `MEMORY.md` 不支持的力量来源、法术规则、科技光效、UI 参数或无源爆炸；玄幻/奇幻/科幻等已授权类型可以使用灵光、法阵、能量、护盾、粒子或科技光效，但仍必须落到现场材质、力量来源、碰撞路径、材料响应和节奏余波上。

## Business Requirement Analysis Contract

| field | requirement | evidence | fail_code |
| --- | --- | --- | --- |
| `business_goal` | 将单集表演稿增写为带选择性物理氛围特效字段的逐集氛围稿 | 用户请求、`5-表演` source、`2-美学` source、输出路径 | `FAIL-ATM-BUSINESS-GOAL` |
...
| `complexity_source` | 复杂度来自选择性增写、物理特效真实感、题材适配、场景节奏、2-美学继承和不过度新增的平衡 | 类型路由、节点证据、reference execution matrix | `FAIL-ATM-COMPLEXITY` |
...
| --- | --- | --- | --- | --- | --- |
| `single_episode_atmosphere_enrichment` | 单个集号、单个表演稿或单集文本 | `Single Episode Path` | `N1,N2,N3,N4,N5,N6,N7` | `CONTEXT.md`, `../_shared/anti-abstract-language-contract.md`, `references/atmosphere-and-mood-contract.md`, `references/scene-rhythm-contract.md`, `types/action-destruction-fx/index.md`, `knowledge-base/physical-atmosphere-index.md` | `FAIL-ATM-TYPE-SINGLE` |
| `episode_range_atmosphere_enrichment` | 多集范围或全量可读表演稿 | `Batch Episode Path` | `N1,N2,N3,N4,N5,N6,N7` | `CONTEXT.md`, `../_shared/anti-abstract-language-contract.md`, `references/atmosphere-and-mood-contract.md`, `references/scene-rhythm-contract.md`, `types/action-destruction-fx/index.md`, `knowledge-base/physical-atmosphere-index.md` | `FAIL-ATM-TYPE-RANGE` |
| `specified_script_override` | 用户指定 source 或粘贴文本 | `Override Source Path` | `N1,N2,N3,N4,N5,N6,N7` | `CONTEXT.md`, `../_shared/anti-abstract-language-contract.md`, `references/atmosphere-and-mood-contract.md`, `types/action-destruction-fx/index.md` | `FAIL-ATM-TYPE-OVERRIDE` |
| `atmosphere_pack_research` | 指名题材/特效手段但 knowledge-base 无匹配 | `Atmosphere Pack Research Path` | `N1,N2,N3,N4,N5,N6,N7` | `CONTEXT.md`, `../_shared/anti-abstract-language-contract.md`, `knowledge-base/physical-atmosphere-index.md`, `references/atmosphere-and-mood-contract.md` | `FAIL-ATM-TYPE-RESEARCH` |
| `repair` | 既有稿件需修复 | `Repair Path` | `N1,R1,R2,N6,N7` | `CONTEXT.md`, `../_shared/anti-abstract-language-contract.md`, `references/atmosphere-and-mood-contract.md`, `references/scene-rhythm-contract.md`, `types/action-destruction-fx/index.md` | `FAIL-ATM-TYPE-REPAIR` |
| `review_only` | 只审查候选氛围稿 | `Review Path` | `N1,V1,N7` | `CONTEXT.md`, `../_shared/anti-abstract-language-contract.md`, `references/atmosphere-and-mood-contract.md`, `references/scene-rhythm-contract.md`, `types/action-destruction-fx/index.md` | `FAIL-ATM-TYPE-REVIEW` |
...
| `destruction_fx` | `动作破坏点强化` | 任意行动、力量或余波会让现场材质产生可见状态变化，且该变化能增强动作重量、危机、空间压力或余波 | 力量来源、受击材质、破坏材料、节奏属性、类型边界；必要时加载 `types/action-destruction-fx/index.md` 和路由后的类型模块 | 无源爆炸、材质不明、类型越界、破坏结果改写剧情 |
...
| `N2-ATM-UNDERSTAND` | 理解题材、情节、表演稿正文和 2-美学上下文 | 表演稿、美学协议、项目上下文、references | 摘要题材机制、主要冲突、场景节奏、情绪曲线、画面基调、大师/作品参照、角色风格和场景风格；识别已有环境条件；若存在动作破坏触发，建立 `destruction_type_route`，记录 user/aesthetic/source_scene/source_action/material/memory 六类信号、confidence、boundary_profile 和 fallback_reason | `atmosphere_context_profile`、`aesthetic_context_map`、`scene_rhythm_map`、`existing_environment_condition_map`、`destruction_type_route` | `N3` / `R1` | 不能只写类型标签；必须说明氛围方向、禁用手段和美学继承边界；动作破坏类型不得只靠单个关键词猜测 |
...
| `fallback_evidence` | 若某项 `2-美学` 缺失，使用用户指定等价资料并标记降级；若知识库缺题材氛围包，记录 `pretrained_atmosphere_inference` 或网络来源；若某画面点语义不可判定，保持原文不增写并在报告列 N/A | `Review Gate Binding.report_evidence` | `FAIL-ATM-QUANT-FALLBACK` |
...
| `types/action-destruction-fx/wuxia-hk-cold-weapons.md` | `destruction_type_route.type=wuxia_hk_cold_weapons` | 港风武侠/冷兵器动作破坏表现库和专属边界 | 泛化为所有动作破坏类型；禁止玄幻/科幻已授权能力 | `N4/N5/N6` |
| `types/action-destruction-fx/fantasy-xuanhuan.md` | `destruction_type_route.type=fantasy_xuanhuan` | 玄幻/奇幻/仙侠/仪式动作破坏表现库，含已授权灵光、法阵、能量等表达 | 新造法术机制、升级战力或替代上游世界观设定 | `N4/N5/N6` |
| `types/action-destruction-fx/war-ruins.md` | `destruction_type_route.type=war_ruins` | 战争、废墟、灾后、爆破余波和坍塌材质响应 | 新增无源爆炸或套用武侠/玄幻爆点 | `N4/N5/N6` |
| `types/action-destruction-fx/thriller-chase.md` | `destruction_type_route.type=thriller_chase` | 惊悚追逐、门窗玻璃、杂物和短促破坏节奏 | 写成大规模动作爆破 | `N4/N5/N6` |
| `types/action-destruction-fx/naval-water.md` | `destruction_type_route.type=naval_water` | 海战/水域/船上湿木、水汽、缆绳和浪声破坏质感 | 照搬陆地尘土爆点或新增海难事件 | `N4/N5/N6` |
| `types/action-destruction-fx/interior-court.md` | `destruction_type_route.type=interior_court` | 宫廷/室内/文戏动作中的屏风、瓷器、纸墨、烛烟和秩序破坏 | 把室内细碎动作写成大爆破 | `N4/N5/N6` |
| `types/action-destruction-fx/realism-street.md` | `destruction_type_route.type=realism_street` | 现实主义街巷、铁门、玻璃、积水和可信小破坏 | 追求港式爆点、玄幻光效或超出日常材质尺度 | `N4/N5/N6` |
| `types/action-destruction-fx/sci-fi-cyber.md` | `destruction_type_route.type=sci_fi_cyber` | 科幻/赛博金属、管线、屏幕、蒸汽、电子故障和已授权科技光效 | 输出 UI 指令、设备参数或无源炫光 | `N4/N5/N6` |
| `types/action-destruction-fx/generic-material-response.md` | `destruction_type_route.type=generic_material_response` 或类型证据不足 | 保守材质响应和低类型色彩破坏 | 强行套入某个强类型样例 | `N4/N5/N6` |
...
| `抽象氛围 / 诗意 / 电影感 / 宿命感 / 比喻 / 明喻 / 隐喻 / 概念化氛围 / FAIL-ATM-ABSTRACT / FAIL-ATM-PLAIN-VISUALIZATION` | `../_shared/anti-abstract-language-contract.md`, `references/atmosphere-and-mood-contract.md` | `N5/N6` | `GATE-ATM-10-CONCRETE-SENSORY` | 抽象词/比喻残留扫描 + 感官通道证据 + `plain_visualization_audit` |
...
| --- | --- | --- | --- | --- |
| 是否正确读取 `5-表演` source 和 `2-美学` 三类上下文？ | `GATE-ATM-01-SOURCE-CONTEXT` | `FAIL-ATM-SOURCE-CONTEXT` | `N1` | `source_manifest`、`aesthetic_manifest` |
...
| 氛围细节是否符合题材、情节、场景条件和 2-美学协议？ | `GATE-ATM-06-STYLE-FIT` | `FAIL-ATM-STYLE-FIT` | `N2/N4/N5` | `aesthetic_context_map`、`style_source_matrix` |
...
| 是否避免抽象审美词、明喻、隐喻、象征和概念标签，至少使用一个白描式具体感官通道？ | `GATE-ATM-10-CONCRETE-SENSORY` | `FAIL-ATM-ABSTRACT` / `FAIL-ATM-CONCRETE` / `FAIL-ATM-PLAIN-VISUALIZATION` | `N5` | 感官通道抽样、`plain_visualization_audit`、比喻/概念残留 |
...
| 动作破坏点是否先建立 `destruction_type_route`，并以用户信号、2-美学、5-表演场景/动作/材质和项目记忆为证据，而不是只靠单个关键词裁决类型？是否按路由加载了对应 `types/action-destruction-fx/<type>.md`？破坏正文是否有动作来源、受击材质、破坏材料、节奏属性和风格边界，且没有类型越界、无源爆炸或剧情结果越权？ | `GATE-ATM-19-ACTION-DESTRUCTION-FX` | `FAIL-ATM-DESTRUCTION-FX` / `FAIL-ATM-DESTRUCTION-TYPE-ROUTE` | `N2/N3/N4/N5` | `destruction_type_route`、`action_destruction_type_module`、`destruction_trigger_inventory`、`action_destruction_fx_map`、`style_fit_matrix`、`boundary_check` |
| `5-表演` 和 `2-美学` 上下文是否明确投影为氛围触发与新增字段决策，并记录 source anchor、local decision 和 preservation check，而非只写“已读取/已参考”？ | `GATE-ATM-20-UPSTREAM-CONTEXT` | `FAIL-ATM-UPSTREAM-CONTEXT` | `N1-ATM-INTAKE` / `N3-TRIGGER-INVENTORY` / `N5-ATM-ENRICH` | `upstream_context_application_map` |
...
| `ATTE-S20-03` | 漂移检测 | 出现每点硬加、无时间属性、抽象氛围、比喻/象征替代感官材料、新剧情事实、设备参数、摄影术语、prompt 术语或特效抢戏时视为漂移 | `Review Gate Binding` |
...
- `FAIL-ATM-PLAIN-VISUALIZATION` 必须为 0；若 `氛围画面` 靠比喻、象征、概念标签或抽象解释承托主信息，必须回到 N5 重写为白描式物理手段、感官通道、材质状态和时间变化。
- 原字段结构漂移 0。
- 剧情事实越权 0。
- 无时间属性新增字段 0。
- 无触发理由新增字段 0。
...
| 画面化失真为比喻或概念 | 共享反抽象合同未进入 N5，或把白描误当文采化修辞 | `candidate_atmosphere_episode` / `../_shared/anti-abstract-language-contract.md` | `N5` | `plain_visualization_audit` 通过；删除比喻/概念词后仍有物理氛围、感官通道和时间变化 |
| 新增剧情或天气事实 | 物理手段无上游条件 | `physical_fx_selection_map` | `N3/N4/N5` | 保真 diff 通过 |
| 特效抢戏 | 增强点未回到人物焦点 | `trigger_point_inventory` | `N3/N5` | 表演焦点抽样通过 |
| 动作破坏点类型误判 | 没有建立 `destruction_type_route`，或只靠关键词把战争、惊悚、海战、室内、科幻等套成某个强样例类型 | `destruction_type_route` / `types/action-destruction-fx/index.md` | `N2/N4` | `FAIL-ATM-DESTRUCTION-TYPE-ROUTE` 为 0，类型证据含 user/aesthetic/source_scene/source_action/material/memory 六类信号 |
| 动作破坏点生硬、轻飘或越界 | 没有建立动作源、受击材质、破坏材料和风格边界，或未加载路由后的类型模块，或把破坏写成当前类型未授权的法术/科技光效 | `destruction_trigger_inventory` / `action_destruction_type_module` / `action_destruction_fx_map` / `types/action-destruction-fx/<type>.md` | `N3/N4/N5` | `FAIL-ATM-DESTRUCTION-FX` 为 0，破坏点有 source anchor、target material、effect material、boundary check |
...
| --- | --- | --- | --- |
| `画面` / `动作画面` / `角色动作` | 动作路径、空间变化、场景进入、危机升级、武器风压、撞击、坍塌、余劲、现场材质破坏 | 在相邻点后新增 `氛围画面：`，写空气介质、光线、风、水汽、尘土、雨雪、投影等与动作同步或先后关系；动作破坏点先建立 `destruction_type_route`，再写动作源、受击材质、湿泥/尘土/石粉/木屑/水汽/火星等材料响应和边界 | `atmosphere_insert_map`、`time_anchor_map`、`destruction_type_route`、`action_destruction_fx_map` |
...
| `环境描写` / `场景描述` | 空间质感、天气/季节、场景换气、已有石/木/泥/水/金属/玻璃/布纸等材质条件 | 只在已有场景条件内加物理氛围；不得新增无源天气或地理条件；可把已有材质作为动作破坏点的承托目标，并参与 `destruction_type_route.source_scene_signal/material_signal` | `existing_environment_condition_map`、`destruction_type_route`、`action_destruction_fx_map` |
| `音效画面` / `旁白画面` / `系统画面` | 声音层次、沉默、余韵、信息接收、撞击回声、碎裂声、尘土落声、爆点余声 | 声景与视觉特效配合，例如雷声后的频闪、钟声余震、风声距离变化、碎石二次落地、木屑擦过地面、断链余声推远；声音证据可参与 `destruction_type_route.source_action_signal` | `soundscape_map`、`scene_rhythm_map`、`destruction_type_route`、`action_destruction_fx_map` |
...
- 动作破坏点的稳定类型分流规则和类型专属材质边界写入 `types/action-destruction-fx/`；单次执行经验、失败模式和修复打法仍写入 `CONTEXT.md`。
