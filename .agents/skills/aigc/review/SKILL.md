...
`aigc-review` 是 `.agents/skills/aigc` 的 review 卫星技能包。它不进入主阶段串行链，不替代 `0-初始化` 到 `14-审片` 的业务产物，只负责把 checkpoint、stage acceptance 与 package release 的审计事实收束为唯一 aggregate review packet。
