...
| 启用 subagents 后只问技能是否酷炫，没有请教机制压力 | advisor consultation gap | 按 `team.yaml` 顾问专长追问启用条件、限制代价、成长路径、反制方式与失败模式 | 显式启用 subagents 时先生成 `advisor_consultation_packet`，再进入技能卡创作 | 技能卡能说明顾问建议如何落到 `activation_rules / limits_and_costs / progression_model / counterplay` |
...
6. 如果显式启用 subagents，顾问问题要围绕“能力如何失败、被反制、付代价或逼出成长”，不要让顾问只扩写能力效果。
...
- 顾问请教在技能卡里最有用的是约束而不是加料；把顾问脑洞转成限制、代价、反制和失败模式，才能避免万能能力。
