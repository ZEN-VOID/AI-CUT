...
3. `2-Global`
   - 负责全项目共用的全局风格、类型元素与按集按组展开的导演意图三份 Markdown 真源，并在阶段末段把 `组间设计` seed 写入 shared `第N集.json`，作为 `3-Detail` 的导演前置结构化合同
4. `3-Detail`
...
| `2-Global` | 是 | 已建阶段合同，采用单技能知行合一并发链，内部融合 `全局风格 / 类型元素 / 导演意图` 三条能力链并消费 `1-Planning/3-分组` handoff | 写入 `全局风格.md`、`类型元素.md`、`导演意图.md`，并 seed shared `第N集.json` 的 `组间设计` |
