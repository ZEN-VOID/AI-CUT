...
- `7-分镜` 是新增文本分镜拆分阶段，和 `7-图像` 共享数字前缀但不共享业务真源；显式分镜拆分进入 `7-分镜`，显式生图或故事板生成仍进入 `7-图像`。
...
当前 AIGC 影视流水线以根 `SKILL.md` 的 Stage Status Table 为真源。本经验层保留旧 `3-运动 / 4-摄影 / 5-分组` 制作链的兼容经验，但新增文本分镜链路时，默认主链应先走 `3-美学 -> 4-导演 -> 5-表演 -> 6-氛围 -> 7-分镜`；只有用户显式要求 legacy 摄影稿、分组稿或下游生成阶段时，再回接对应旧制作链。

```text
0-初始化 → 1-分集 → 2-编剧/2-编导 → 3-美学 → 4-导演 → 5-表演 → 6-氛围 → 7-分镜 → 6-设计/7-图像 → 8-视频 → 9-审片
legacy/并行制作链：2-编导 → 3-运动 → 4-摄影 → 5-分组 → 6-设计/7-图像
```
...
| `2-编剧/2-编导` | 把逐集原文转成可拍、可演、可听的剧本稿 | 逐集原文 | slugline、声画配对、对白冻结、小说转译、导演判断、视觉主轴、心理反应、台词交付、潜台词行为、场面调度、画面化语言 | `2-编剧/第N集.md` 或 `2-编导/第N集.md` |
| `3-美学` | 建立画面基调、分镜风格和大师/作品参照 | 项目设定 + 剧本稿 | 全局风格协议、画面基调、分镜节奏、美学参照边界 | `3-美学/.../风格协议.md` |
...
| `3-运动` | legacy/并行：强化角色运动连续性 | 编导稿 | 起点、路径、终点、参照系、上一画面最终位置回顾 | `3-运动/第N集.md`（运动强化稿） |
| `4-摄影` | legacy/并行：把画面描述翻译成镜头语言 | 运动强化稿 | `分镜明细：`（景别、运镜、焦点、光影、时长、连续性锚点） | `4-摄影/第N集.md`（摄影稿） |
| `5-分组` | legacy/并行：把逐镜切成可生产的分镜组 | 摄影稿 | ~15秒分镜组、组间 3-4 秒连接件、风格投影、统计数据 | `5-分组/第N集.md`（分镜组稿） |
| `6-设计` | 提取并设计场景/角色/道具资产 | 分镜稿或分镜组稿 | 资产清单、设计规格、生成请求 JSON | 设计资产（场景/角色/道具） |
| `7-图像` | 生成分镜画面或故事板 | 分镜稿/分镜组稿 + 设计资产 | AI 生成的帧图像或故事板 | 图像文件 |
...
flowchart LR
    ...
    S0["0-初始化"] --> S1
    S1 --> S2 --> S3A --> S4A --> S5A --> S6A --> S7A --> S7 --> S8 --> S9 --> S10
    S10 -.->|"修复回写"| S7A
    S10 -.->|"修复回写"| S7
```
...
flowchart TD
    ...
    A -->|"2-编剧/2-编导"| B["剧本稿<br/>字段化·对白冻结·声画配对"]
    ...
    K -->|"6-设计"| G["设计资产<br/>场景规格·角色造型·道具清单"]
    G -->|"7-图像"| H["帧画面/故事板"]
    H -->|"8-视频"| I["MP4 视频"]
...
9-审片 说”第 3 镜焦点偏移，建议回到 7-分镜 调整景深参数”         → 质量闭环
```
