...
- `scripts/migrate_legacy_dirs.py`：批量把旧 `book_id` 目录收敛到书名目录
...
- 即使目录是通过番茄官网书页回退获得，正文下载也必须先探测第三方 `api/content` 候选节点；只有正文节点全部失败后，才允许回退到番茄官网 `reader` 页 / 官方 reader 接口。
- 当第三方正文接口不可用时，必须自动回退到番茄官网 `reader` 页 / 官方 reader 接口，并对私有区字体正文做字符表解码。
- 若官网 `reader` 持续返回“验证码中间页”，必须先尝试“书页预热浏览器会话 -> 同源 `fetch /api/reader/full`”自动兜底，并输出 challenge 专用诊断；若操作者已有通过验证的浏览器会话，仍可通过环境变量 `FANQIE_COOKIE_HEADER` 或 `FANQIE_NOVEL_WEB_ID` 注入后重跑。
- 若样章探测已确认“第三方正文接口全挂 + 官网正文仍被 challenge 拦截”，必须在整书下载启动前直接转 blocker 失败，不得继续切到官网正文优先或进入多轮补缺重跑。
...
- 下载失败时应同时落盘 `download_failures.json`，用于补缺重跑与证据回看。
...
- 目录输出：
  - `input/番茄小说/<书名>/metadata.json`
- 正文章节输出：
  - `input/番茄小说/<书名>/chapters/*.txt`
- 下载失败清单输出（仅失败时）：
  - `input/番茄小说/<书名>/download_failures.json`
- 内嵌章节拆分输出：
  - `input/番茄小说/<书名>/chapters/_split/*.txt`
- 短篇分享输出：
...
- 小说目录默认优先按 `metadata.data.book_name` 落盘，而不是按 `book_id` 命名。
- 若已存在旧的 `input/番茄小说/<book_id>/` 目录，脚本会优先兼容读取；在可安全迁移时，会收敛到书名目录。
- 若同名书名目录已被其他 `book_id` 占用，脚本会自动追加冲突后缀，避免覆盖已有内容。
- 若需要批量收敛历史目录，执行：

```bash
python3 .agents/skills/fanqie-download/scripts/migrate_legacy_dirs.py --all
python3 .agents/skills/fanqie-download/scripts/migrate_legacy_dirs.py <book_id> [<book_id> ...]
```
...
- `catalog.py` 进入 `official_page_fallback` 只代表目录来源切换，不代表正文必须强制走官网；`download.py` 必须先探测至少 1 个样章的正文通道，再决定是官网优先、API 优先，还是以 challenge blocker 立即失败
- 当官网 `reader` 被验证码中间页拦截时，`download.py` 应优先启动 `playwright-cli` 打开同源书页，再在浏览器上下文内 `fetch /api/reader/full`；该链路默认强制 `TMPDIR=/tmp`，避免 macOS 下 unix socket 路径过长导致 `playwright-cli` 报 `EINVAL`
- challenge blocker 一旦在样章阶段成立，就必须立即结束整书执行并返回 blocker 诊断；不能再以“可能零星成功”为由继续整书盲打
...
- 官网 `reader` challenge 场景下，`download.py` 支持读取环境变量 `FANQIE_COOKIE_HEADER`（完整 Cookie 头）或 `FANQIE_NOVEL_WEB_ID`（仅注入 `novel_web_id`）后重跑；若仍失败，应按 challenge 阻塞态结案，不要伪装成普通空正文。
