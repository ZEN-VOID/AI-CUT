...
本文件承接 `aigc/2-编导/references/climax-visual-treatment-contract.md`，用于把上游已经强化出的高潮画面成分，进一步转化为 `4-摄影` 的分镜表现、分镜画面和运镜手法。

`4-摄影` 不重新设计高潮，不新增剧情事实、对白、胜负结果、道具效果或人物动机。它只在上游画面句子已有的高点基础上，提高观看路径的戏剧强度、节拍清晰度、镜头稀缺性和运镜辨识度。
...
`peak_shot_profile` 只作为内部判断，不作为输出标题写入成稿。最终仍落入原画面性字段标题下的连续时间段。
...
| `emotional_slow_burn` | 崩溃/震惊/突然醒悟/强忍情绪场景；此类情绪镜头 `tempo` 标记为 `slow_burn` 或 `hold`，时长由情绪节奏决定，不受 Short Drama AIGC Bias 的 3 秒证据门槛约束 | 正面多角度切换（正面中景→正面近景→正面双眼特写，每个角度 1-2 秒，硬切或焦点跳切串联）或极慢推轨（内部 3-5 秒，落盘为 `[起始秒-结束秒]`）；不用快速运镜或复杂环绕，以免破坏表演可读性；正面完整捕捉面部肌肉变化；双眼特写拍摄正面眉骨到鼻尖区域，不得只拍单眼侧面 |
...
- **时长**：由情绪节奏决定，不受 Short Drama AIGC Bias 的 3 秒证据门槛约束；若单段超过 3 秒，正文必须通过对白承托、读秒、静止、极慢运动或框内变化证明它在短剧·AIGC 节奏里成立
...
- `peak_shot_profile` 的结果体现在 `时间段` 中，但没有外露为冗余标签。
...
| 每个高点是否形成内部 `peak_shot_profile`，并把 `peak_mode / emphasis_goal / beat_amplification / aftershock_handoff` 投影到 `时间段`？ | `GATE-CINE-14` | `FAIL-CINE-05E` | `N5.5-PEAK-SHOT` + `N6.5-SHOT-PLAN` | 峰值镜头策略、余波交出、被压平高点的修复记录 |
| 高点密度是否来自新注意力、新戏剧信息、反应/结果或余波，而不是一律增加时间段？ | `GATE-CINE-04A` | `FAIL-CINE-03A` | `N4-BEAT` + `N5-RHYTHM` + `N5.5-PEAK-SHOT` | 时间段数量分布与高点抽样复核、删并或加密依据 |
