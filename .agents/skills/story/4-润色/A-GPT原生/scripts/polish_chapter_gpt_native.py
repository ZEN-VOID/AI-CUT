#!/usr/bin/env python3
"""
Mechanical helper for `story/4-润色/A-GPT原生`.

It assembles a polishing context pack from the canonical 3-初稿 chapter and
validates/writes back a polished Markdown chapter already authored by the
current GPT/LLM session. It does not rewrite prose by rule or template.
"""
...
POLISHING_DIR = SCRIPT_PATH.parents[1]
STORY_ROOT = SCRIPT_PATH.parents[3]
STORY_SCRIPTS_DIR = STORY_ROOT / "scripts"
SYSTEM_PROMPT_PATH = POLISHING_DIR / "templates" / "gpt-native-system-prompt.md"
CHAPTER_TEMPLATE_PATH = POLISHING_DIR / "templates" / "chapter-root.template.md"

if str(STORY_SCRIPTS_DIR) not in sys.path:
    sys.path.insert(0, str(STORY_SCRIPTS_DIR))

from chapter_paths import drafting_root_md_path, polishing_root_md_path, extract_chapter_title, find_chapter_file
...
def _render_output_template(
    *,
    ...
    rendered = rendered.replace("{{draft_source_ref}}", previous_chapter_ref)
    rendered = rendered.replace("{{chapter_body}}", "[由当前 GPT/LLM 会话润色后的完整章节正文]")
    return rendered
...
def _build_messages(
    *,
    ...
    output_template = _render_output_template(
        ...
        previous_chapter_ref=_rel(source_draft_path, project_root),
    )
    ...
    if current_path.exists():
        sections.append(
            f"当前润色稿现稿（必须先吸收后再按当前模式处理，{_rel(current_path, project_root)}）：\n"
...
def _split_frontmatter(text: str) -> tuple[dict, str]:
    ...
    if not match:
        raise ValueError("GPT 原生润色输出缺少完整 YAML frontmatter。")
    try:
        payload = yaml.safe_load(match.group(1)) or {}
    except yaml.YAMLError as exc:
        raise ValueError(f"GPT 原生润色输出 YAML frontmatter 无法解析：{exc}") from exc
    if not isinstance(payload, dict):
        raise ValueError("GPT 原生润色输出 YAML frontmatter 必须是映射对象。")
...
def _validate_generated_markdown(text: str, chapter_num: int) -> None:
    for marker in REQUIRED_OUTPUT_MARKERS:
        if marker not in text:
            raise ValueError(f"GPT 原生润色输出缺少必需字段：{marker}")
    ...
    for field in FORBIDDEN_FRONTMATTER_FIELDS:
        if field in frontmatter:
            raise ValueError(f"GPT 原生润色输出 frontmatter 不应重复写入 `{field}`，该信息由上下文加载与 sidecar 承载。")

    heading = f"# 第{chapter_num}章｜"
    if heading not in body_with_heading:
        raise ValueError(f"GPT 原生润色输出缺少章节标题行：{heading}")
    if "[由当前 GPT/LLM 会话润色后的完整章节正文]" in text or "[请填充完整章节正文]" in text:
        raise ValueError("GPT 原生润色输出仍包含模板占位正文，禁止写回。")

    body = body_with_heading.split(heading, 1)[-1].strip()
    if len(body) < 800:
        raise ValueError("GPT 原生润色输出正文过短，疑似不是完整章节。")


def _resolve_polishing_mode(requested_mode: str, chapter_path: Path) -> str:
    if requested_mode != "auto":
        return requested_mode
    return "polish_rewrite" if chapter_path.exists() else "chapter_polish"


def _default_output_dir(project_root: Path, volume_num: int, chapter_num: int) -> Path:
    return project_root / "reports" / "4-润色" / "gpt-native" / f"第{volume_num}卷" / f"第{chapter_num}章"


def _load_authored_polish(args: argparse.Namespace) -> str:
    if args.from_stdin:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate and write a GPT-native polished chapter manuscript")
    ...
    parser.add_argument(
        ...
        default="auto",
        help="polishing mode; auto polishes from 3-初稿 or rewrites an existing 4-润色 chapter",
    )
    parser.add_argument("--dry-run", action="store_true", help="only emit the context pack without writing a chapter")
    parser.add_argument("--supervision-packet", help="team supervision packet file produced by polishing subagents")
    parser.add_argument("--draft-file", help="Markdown file already polished by the current GPT/LLM session")
    parser.add_argument("--from-stdin", action="store_true", help="read the GPT-polished Markdown from stdin")
...
def main() -> int:
    ...
    polishing_mode = _resolve_polishing_mode(args.mode, chapter_path)
    supervision_packet_text = _read_text(Path(args.supervision_packet)).strip() if args.supervision_packet else ""

    required_files = [book_plan_path, volume_plan_path, chapter_plan_path, north_star_path, source_draft_path]
    ...
    generated = _strip_code_fence(_load_authored_polish(args))
    _validate_generated_markdown(generated, chapter_num)

    raw_path = output_dir / f"chapter_{chapter_num}_gpt_polished.md"
