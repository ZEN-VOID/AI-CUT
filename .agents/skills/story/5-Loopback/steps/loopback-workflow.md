...
| `business_object` | validation aggregate, Cards, planning sidecars, story_map, STATE, loopback artifact |
...
| `N3-DELTA` | normalize validated delta | aggregate, evidence refs, template | build `card_deltas`, `map_deltas`, `projection_refresh` | normalized `loopback_delta` | `N4-STAGE` | no non-whitelisted fields |
...
| `N5-PENDING` | mark run as pending | `STATE.json` | write pending manifest | `loopback_pending` marker | `N6-WRITEBACK` | pending marker durable |
