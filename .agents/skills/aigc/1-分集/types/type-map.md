...
Load `source-type-map.md` by default for every invocation of this skill. If future subtype packages are added, select only the packages needed by the current source material and let `SKILL.md` consume the resulting source type through `Type Routing Matrix` and `Thinking-Action Node Map`.
