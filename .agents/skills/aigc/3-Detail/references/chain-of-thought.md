...
| FIELD-DETAIL-05 | 业务字段.空间与构图 | 锁定 `场景及方位`、`分镜表现` 与 shot-level 镜头描述子槽，并用量化密度机制明确主陪背景层级、构图布局与构图方式 | skeleton、导演意图、全局风格、参考锚点、`format_density_profile` | S4 | 空间可读性 | FAIL-DETAIL-05 |
| FIELD-DETAIL-06 | 业务字段.站位与道具 | 锁定 `角色及站位和穿搭`、`道具及状态` 的几何与叙事功能 | skeleton、结构链、角色任务 | S5 | 可拍性 | FAIL-DETAIL-06 |
| FIELD-DETAIL-07 | 业务字段.角色表现 | 把情绪/动作/关系翻译成镜头可见表演，并显化角色个性、习惯动作、下意识、对手戏高命中参考转译与可被捕捉的眉眼/身体信号 | skeleton、导演意图、结构 draft、条件命中的电影学院派对话场景知识 | S6 | 表演可见性 | FAIL-DETAIL-07 |
| FIELD-DETAIL-08 | 业务字段.场景氛围 | 把时间感、空气感、压迫/抒缓气候写成可感知条件 | 组间设计.全局风格、组间设计.导演意图、空间 draft | S7 | 氛围可感知性 | FAIL-DETAIL-08 |
| FIELD-DETAIL-09 | 业务字段.运镜手法 | 先给默认叙事路由，再判断电影学院派分镜脚本中是否存在高度命中的运动语法母型，最后在同目标内比较更强镜头变体并按需升格挑战方案 | 组间设计.类型元素、组间设计.导演意图、core draft、`academy_hit_note`、条件命中的电影学院派分镜脚本知识 | S8 | 叙事优先度 | FAIL-DETAIL-09 |
| FIELD-DETAIL-10 | 业务字段.摄影美学 | 先回看上游 `组间设计` 摄影底座与命中的学院派摄影知识，再把光位/组级光影推进/色彩心理子补丁合成 single-line final look | 组间设计.全局风格、组间设计.类型元素、组间设计.导演意图、氛围、运镜、摄影知识库、light/group/color 子补丁 | S9 | 摄影合成度 | FAIL-DETAIL-10 |
...
| S4 | FIELD-DETAIL-05 | 空间、构图、节拍、景别、镜头属性/框架/类型/视角、主陪背景层级、构图布局/方式与焦点是否成立；候选节拍、节奏系数、格式档位、拆镜加权与合镜折减是否共同支撑当前镜数 | 锁 `动作阶段点 / 台词气口点 / 焦点切换点 / 结构转折点`，再锁密度、主陪背景关系、构图布局、构图方式、镜头描述子槽、焦点、空间锚点、主体重心 | 画面主重心、层级关系和空间关系模糊，或镜数无法解释为何不是更少/更多 |
| S5 | FIELD-DETAIL-06 | 站位与道具是否支撑动作和观看路径 | 补角色站位、穿搭显著特征、道具状态 | 结构字段彼此打架 |
| S6 | FIELD-DETAIL-07 | 表演字段由哪种子模式负责，且该角色会以什么个性化方式显露/压住情绪、用什么行为推动叙事、把强情绪落到哪些眉眼与身体信号上；若为对手戏，电影学院派一流对话场景知识是否存在高命中母型 | 选择内心 / 动作 / 对手戏一路或多路，并锁角色化表达通道、叙事性行为、微表情承载点与必要的对手戏高命中参考转译 | 情绪、动作、关系被混写成空话，表演失去角色辨识度，或把知识库只当抽象致敬未转成表演策略 |
| S7 | FIELD-DETAIL-08 | 氛围来源是否具体可感知 | 补空气、温度、密度、压迫等可体验条件 | 只剩形容词口号 |
| S8 | FIELD-DETAIL-09 | 运镜是否先服务叙事清晰度；在默认路线与动机成立后，电影学院派分镜脚本是否存在可被转译的高命中运动母型；并在同目标内完成“默认路线 vs 更强变体”的比较 | 先锁默认叙事路线与运动动机，再做条件性的高命中检测，把可用知识转成当前镜头的运动策略，随后比较同目标酷法，条件时追加挑战对照 | 直接默认走挑战方案，用知识库重写 core 事实，或以更酷为名偷换表现目标 |
...
| FIELD-DETAIL-05 | 空间可读性 | 构图、节拍、景别、镜头属性/框架/类型/视角、主陪背景层级、构图布局/方式与空间锚点明确，且镜数已通过量化密度机制与组时长窗口校正，可支撑后续链路 | FAIL-DETAIL-05 | S4 |
| FIELD-DETAIL-06 | 可拍性 | 站位、穿搭显著特征、道具状态可直接消费 | FAIL-DETAIL-06 | S5 |
| FIELD-DETAIL-07 | 表演可见性 | 情绪/动作/关系都已落成镜头可见表演，且带有角色个性痕迹、能推动叙事、有明确的眉眼/身体承载点；对手戏高命中时已把知识库母型转译成攻守与距离策略 | FAIL-DETAIL-07 | S6 |
| FIELD-DETAIL-08 | 氛围可感知性 | 氛围来源具体，可被摄影和画面阶段理解 | FAIL-DETAIL-08 | S7 |
| FIELD-DETAIL-09 | 叙事优先度 | 运镜先服务信息与情绪递送；若命中学院派分镜脚本，必须已转成当前镜头可执行的运动语言；若提出更酷变体，必须证明其仍服务同一表现目标且收益明确 | FAIL-DETAIL-09 | S8 |
