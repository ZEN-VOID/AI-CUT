...
8. 如果“画面化”变成比喻或概念，先删掉“像/仿佛/宿命/高级/压迫感”等词，再补可见身体动作、道具状态、空间距离、声源、光照和时间变化。
9. 用户指出“脚本化/偷懒/未思考/未差异化”时，不接受逐句润色；直接废弃候选稿，回 `N2-N6` 重建证据和正文。
10. 高潮只强化已有事件的声画/情绪/行动，不改结果。
11. 尾钩必须落在最后可感对象上，不接受“悬念拉满”类总结。
12. 报告服务修复和下游交接，必须能回指正文位置。
13. 报告不得写自由散文式“思考过程”；必须写可审计的 `Execution Decision Trace` 和 `Reference Execution Matrix`。
