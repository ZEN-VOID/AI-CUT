...
- 将旧英文 runtime 口径调整为新版中文 runtime：`0-初始化 / 1-分集 / 2-编导 / 3-运动 / 4-摄影 / 8-分组 / 3-主体 / 9-图像 / 10-画布 / 11-审片`，并把 `4-设计` 标为 transition/compat 输入。
- 根据外部 reviewer provider 审计优化：恢复项目 `CONTEXT/` 加载合同、`3-主体` canonical 恢复口径、`blocked_safety_stop` 模式、`7-Cut` 搁浅语义与 `knowledge-base/` owner 边界。
