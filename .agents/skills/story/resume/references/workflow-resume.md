...
<context>
本文件只定义 `resume/` 的恢复原则与安全边界，不替代 `3-初稿`、`review/`、`5-上下文回流` 自身的 stage 合同。
</context>
...
1. `5-上下文回流/第V卷.context-return.json`
...
- ❌ 自动替用户选择恢复策略
- ❌ 未备份即删除正文
- ❌ 把 `resume/` 冒充 `5-上下文回流` actualization 主流程
...
<example>
<input>workflow detect 显示无中断，但项目里已有 `review/第1章.validation.json` 和 `review/第1-1章审查报告.md`，还没有 `5-上下文回流/第1章.context-return.json`</input>
<output>
说明：
