...
flowchart LR
    ...
    F --> M["运镜手法"]
    F --> N["转场特效"]
    F --> O["legacy compatibility projection"]
```
...
3. `分镜明细[]` 的第一真相是 branch-owned 八字段：
   - `角色表现`
   - `运动表现`
   - `氛围表现`
   - `视觉强化`
   ...
   - `运镜手法`
   - `转场特效`
   兼容旧名 `人物表演锚点 / 动作路径 / 空间氛围 / 视觉抓手 / 构图骨架` 时，应按 alias 读取；更老的 `人物表演 / 动作调度 / 视觉焦点 / 构图策略 / 摄影策略 / 运镜策略 / 转场策略` 已退出 shared schema 主兼容层，不得再把它们当规范主名。
4. legacy 字段如 `角色背景面 / 角色站位走位 / 道具及状态 / 分镜表现` 只作 fallback，不得盖过 branch-owned canonical。
   其中 `分镜表现` 视为 deprecated alias，必须具像、细致、可见，不得写抽象总结句。
5. `剧本正文` 仅作回退与歧义解消，不得盖过已经存在的结构化镜级事实。
...
| --- | --- | --- | --- | --- | --- |
| `角色` | `组间设计.出场角色及穿搭`、`分镜明细[].角色表现`、`分镜明细[].运动表现` | `分镜明细[].视觉强化`、`分镜明细[].氛围表现`、`剧本正文` | `角色站位走位`、`角色背景面`、`分镜表现` | 只凭代词、无锚亲属称谓、纯背景描写臆造角色 | `4-Design/角色/1-清单/第N集/{角色清单.json,角色研究.json,role_design_bridge.json}` |
| `场景` | `分镜明细[].氛围表现`、`分镜明细[].分镜构图`、`分镜明细[].摄影美学` | `分镜明细[].运动表现`、`分镜明细[].转场特效`、`分镜明细[].视觉强化`、`组间设计.导演意图`、`剧本正文` | `角色背景面`、`分镜表现`、`时间段` | 把角色动作误写成场景描述；把无证据的研究设定升格成场景真相 | `4-Design/场景/1-清单/第N集/...` |
| `道具` | `分镜明细[].运动表现`、`分镜明细[].视觉强化`、`分镜明细[].角色表现` | `分镜明细[].氛围表现`、`分镜明细[].分镜构图` | `道具及状态`、`分镜表现` | 把服装部件误当成独立道具 | `4-Design/道具/1-清单/第N集/...` |
| `服装` | `组间设计.出场角色及穿搭`、`分镜明细[].角色表现`、`分镜明细[].视觉强化` | `分镜明细[].运动表现`、`分镜明细[].分镜构图` | `角色站位走位`、`分镜表现` | 把环境色光或道具材质误写进服装字段 | pending-migration；暂不声明 active runtime，不作为初始化预建目录 |
...
| P1 | `分镜明细[].角色表现` | 表演意图、对手关系、情绪兑现 | 角色主体性的首选镜级证据 |
| P2 | `分镜明细[].运动表现` | 出场、走位、动作链、同镜角色关系 | 角色 presence 与 blocking 的首选证据 |
| P3 | `分镜明细[].视觉强化` | 视觉识别锚点、镜头先看什么 | 服装与主体识别的强化证据 |
| P4 | `分镜明细[].氛围表现` | 角色所处场域、空间压力、材质支持 | 只在显式关涉角色时回收为人物证据 |
...
| --- | --- | --- | --- |
| P0 | `分镜明细[].氛围表现` | 空间类型、空气、温度、压迫、材质承载 | 场景设计的第一镜级真相 |
| P1 | `分镜明细[].分镜构图` | 观看关系、画面骨架、主体与背景组织 | 直接影响场景 framing |
| P2 | `分镜明细[].摄影美学` | 光影、色彩、质感方向 | 场景光感与表面行为真源 |
| P3 | `分镜明细[].运动表现` | 人与空间的使用方式 | 只用来补场景被怎样“占用” |
| P4 | `分镜明细[].转场特效` | 镜间连续性与组间衔接 | 只在需要镜间连续设计时使用 |
...
- `4-Design/1-清单/角色`、`场景`、`道具` 都应优先回链本文件。
