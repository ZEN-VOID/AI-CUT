...
本文件展开 F2 的审查问题，不替代 `SKILL.md` 的 `Review Gate Binding`。执行时必须回接 `SKILL.md` 中的 fail code 和返工节点。

## Review Checklist

| area | question | blocking_when | return_to |
| --- | --- | --- | --- |
| HyperFrames-only | 是否存在 F1 script、F1 validator、MoviePy/ffmpeg 主链依赖，或把 `_shared/video-to-manifest` 当作必需真源/runtime？ | 任一出现即阻断；共享 manifest 仅作可选证据输入不阻断 | `N2-HYPERFRAMES-LOAD` |
...
| Preview | snapshot 是否非空、叠层不挡字幕和核心 UI？ | 空画面、遮挡或 validate 失败 | `N7-PREVIEW-VALIDATE` |
...
- `artifact_paths`: work root、composition root、snapshot、render、report。
...
| --- | --- | --- |
| `pass` | F2 route、证据、composition、preview/render 和报告门禁均满足对应完成层级。 | close |
| `conditional_pass` | project 或 plan 可交付，但 render、CLI、素材或授权存在明确阻断。 | report residual risk |
| `fail` | 存在阻断性 gate 失败，必须回到对应 F2 node 返工。 | rework |

## Review Gate Mapping

| review_question | review_gate | fail_code | rework_target | report_evidence |
| --- | --- | --- | --- | --- |
| Is the F2 review tied back to SKILL.md gates? | Review finding without fail code and node target fails | `FAIL-REVIEW-BINDING` | `Review Gate Binding` | finding table |
