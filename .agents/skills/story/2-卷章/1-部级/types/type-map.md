...
- 若未来拆出题材、修订范围或审查 provider 子类型包，必须先在 `Package Index` 中登记 `context_files` 后才允许被 `SKILL.md` 的 `Thinking-Action Node Map` 消费。
...
4. 将 `type_profile` 交给 `SKILL.md` 的 `Thinking-Action Node Map` 消费。
