...
| `frontmatter` | 必需字段、`写作模型: GPT`、引用路径、global/style/north-star 摘要是否齐备且非照抄 |
...
finding:
  ...
  dimension: context_loading | source_alignment | continuity | frontmatter | prose_quality | gpt_native_evidence | path_contract | script_boundary
  symptom: ""
  direct_cause: ""
  source_contract: ""
  rework_target: ""
...
- 输出不是 canonical path。
- frontmatter 缺任一必需字段，或 `写作模型` 不等于 `GPT`。
