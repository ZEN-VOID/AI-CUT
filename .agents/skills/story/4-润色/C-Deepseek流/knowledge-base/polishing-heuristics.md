...
- DeepSeek 流适合让模型先综合大量上下文后判断哪里必须修，但必须给出清晰输出 schema，否则容易把推理优势用在解释流程上。
- 当前润色默认目标是最小局部修补：保留初稿句群骨架、长短不齐、段落密度和人物气口，只修明显坏点。
- 若 AI 检测或人工成分指标显示润色稿变差，优先怀疑整章重排、短句化清洗和通用顺滑化，而不是继续追加“去 AI 味”提示。
