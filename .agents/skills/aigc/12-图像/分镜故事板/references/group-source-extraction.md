...
- 枚举候选行列时，应优先考虑：`panel_image_box_ratio_error <= 0.06`、单格图片区面积足够、空白格少、阅读顺序自然、单格文字可读、整体比例满足 provider 约束。不得把固定行列数当成默认真源。
- `selected_sheet_size` 必须在 `gpt-image-2` 合法范围内：最大边 `<=3840px`、宽高均为 `16px` 的倍数、长短边比例 `<=3:1`、总像素 `655360..8294400`。4K storyboard 优先靠近上限像素预算，但不得违反这些约束。
- 若最佳候选仍导致 `panel_image_box_ratio_error > 0.06`、图片框面积过小、文字区过小或整图比例超过 `3:1`，必须选择 `pagination_or_multi_sheet_decision: paginate | multiple_sheets`，并在报告中说明受影响 `group_id`、推荐拆分和原因。
...
8. `layout_aspect_decision` 非空，`panel_count` 等于 frame-unit 数量，`selected_sheet_size` 满足 `gpt-image-2` 尺寸约束，且包含 `panel_geometry_blueprint`；若需要分页或多 sheet，必须记录原因。
...
| 整体 sheet 比例固定导致 panel 被压扁，或 image box 没有 16:9 几何锁定 | 重算 `layout_aspect_decision`：先用 frame-unit 数量枚举行列，再建立 `panel_geometry_blueprint` 并选 `gpt-image-2` 合法 sheet size |
...
| `layout_aspect_decision` 是否先用 frame-unit 数量反推候选行列和整图比例，再为每格生成固定 16:9 `image_box` 几何蓝图并选择 `gpt-image-2` 合法尺寸，没有固定画布后挤压 panel？ | `G8A-LAYOUT-ASPECT` | `FAIL-SHEET-LAYOUT-ASPECT` | `N3B-LAYOUT-ASPECT` / `references/group-source-extraction.md#layout-aspect-decision` | `group-index.json.layout_aspect_decision` 记录 panel_count、candidate_grids、selected_grid、selected_sheet_size、provider constraints、panel_geometry_blueprint、panel_image_box_ratio_error 和分页/多 sheet 决策 |
