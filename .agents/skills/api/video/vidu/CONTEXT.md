...
| `TM-VIDU-DEFAULT-MODEL-DRIFT` | 默认模型长期停留在旧版本，如 `Vidu-q2-pro` | 真源治理层 | 把默认模型改回父级 `../runbooks/default-model-policy.md` 的 `highest-available-general` 规则族 | 共享算法骨架只允许放在 `../shared/default_model_policy.py`；Vidu 子技能只声明“只取 `Vidu-*` 通用模型且排除 `mix`”这一过滤条件，文档/元数据仅引用当前解析结果 | `--help` 与 dry-run 中默认模型随已知型号集合同步更新 |
...
- 默认模型如果需要“总是跟最高版本走”，就不能在 `SKILL.md`、参考摘要和入口元数据里分别写死字面量；应先回到父级共享 runbook，再由共享 helper + provider 本地过滤条件解析，其他载体只引用当前解析结果。
