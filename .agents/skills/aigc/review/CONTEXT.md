...
| `TM-REVIEW-02` | child dimensions 对同一 scope 给出互相冲突结论 | fact pack covenant layer | 重建同一份 `review_fact_pack` 后重跑 | 所有 child 只消费同一 pack | sidecar 中 `scope_ref` 与 `checkpoint_id` 一致 |
...
3. 若 child 输出不一致，优先检查 `review_fact_pack` 是否同一份，而不是先裁掉某个 reviewer。
4. 若 aggregate packet 与 child sidecar 冲突，以 aggregate packet 为 gate 真源，再修 child output contract。
...
- child skills 应报告维度问题和默认返工建议，但不能写最终 `routing_decision`。
- “自动修复”在 review 父层默认指生成 repair route 与治理桥接，不是替创作阶段改稿。
- 新 `aigc` 阶段名以 `0-初始化 / 1-分集 / 2-编导 / 3-摄影 / 4-分组 / 5-设计 / 6-图像 / 7-视频` 为主；旧英文 stage 只作为兼容线索。
