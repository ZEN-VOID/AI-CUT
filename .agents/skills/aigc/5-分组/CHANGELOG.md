...
- 明确跨组连续分镜序列感只能通过入场镜头和画面属性说明“同一尾帧状态换景别/机位/焦点/构图/观看距离进入”，不得改写 `4-摄影` 原正文或原有 `分镜明细`。
...
- 将单个分镜组显式时长累计 `>18s` 从语义 warning / 可声明完整性例外，升级为硬性 error；`5-分组` 不再允许用同画面完整、对白承托、动作完成或场景完整性放行超 18 秒组。
- 若单个 atomic unit 自身超过 18 秒，新增回退口径：必须返回 `4-摄影` 拆分画面单位、压缩镜头时值或重裁分镜数量后再重新分组。
...
- 将 15 秒/组的边界裁决从字数中心切换为显式时长累计中心：优先累计 `4-摄影` 的 `分镜N（约X秒）` 接近 15 秒，允许约 12-18 秒弹性，不为精确卡点拆断同一画面。
...
- 调整字数统计口径：1680 常规目标和 1980 硬上限只约束从 `4-摄影` 划定的纯分镜剧本正文；组标题、north_star、YAML 与组间连接件均不计入。
