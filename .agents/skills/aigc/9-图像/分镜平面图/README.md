...
`分镜平面图` 是 `9-图像` 阶段的组级空间调度图入口。它从 `projects/aigc/<项目名>/8-分组` 直接引用分镜组内容，调用 `.agents/skills/cli/imagegen` 生成一张多 panel 顶视图平面图 sheet，用黑白建筑平面图底图表达场景边界、角色站位、动线、摄影机位置、运镜方向和上下组空间连续性。
...
```text
使用 $分镜平面图，为 projects/aigc/诡校-测试版/8-分组/第1集.md 生成第1集全部分镜平面图，并保持同一集上下分镜组空间连续。
```

## Runtime Output

```text
projects/aigc/<项目名>/9-图像/分镜平面图/第N集/
