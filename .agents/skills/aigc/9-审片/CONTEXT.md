...
本文件是 `9-审片` 的经验层知识库。调用同目录 `SKILL.md` 时必须同时加载本文件。它不改写审片主合同，只保存实际素材审查、落盘和源层升级中的可复用经验。
...
| --- | --- | --- | --- | --- |
| 视频把单组拉成多 beat 蒙太奇 | `5-分组` 组正文过载或焦点不稳 | 把组内容改成单一可生成目标，后续 beat 留给下一组 | 审片后把“15 秒素材承载不住的多 beat”写入 `5-分组` 修复经验 | 新组正文有唯一主体、唯一镜头意图和清楚出场接口 |
| 文件名无法回推分镜组 | `8-视频` 下载命名漂移 | 重命名或在审片报告标注命名异常，先不改内容 | `8-视频` 输出合同固定 `<group_id>.mp4` / `<group_id>-a.mp4` | 审片入口能从文件名直接定位 `5-分组` |
| 单次瑕疵被误升源层 | 源层升级门过松 | 降级为 rerun-only finding | 源层升级必须多例、可复现、能定位 source owner | 报告中有重复证据和直接原因 |
| 审片报告替代分组真源 | 落盘边界错误 | 把可执行修复写回 `5-分组`，报告只留证据与决策 | landing contract 固定 report 与 canonical source 分工 | 后续 `8-视频` 不需要读审片报告才能消费修复 |
...
| prompt 本身同时要求多个主体/动作/风格 | `prompt_problem` 或分组过载 | 修 `5-分组`，降低内容密度和审美词空泛度 | prompt alignment pass 先查矛盾、过载和不可拍动作 | 新 prompt 有唯一焦点、可拍动作和明确停点 |
...
2. 找到 `5-分组/第N集.md` 中对应组，再看视频；否则容易把生成偏差当成剧情事实。
3. 审片描述分三层：实际画面、与分组预期差异、可执行修复。
4. 如果视频素材好但分组文档不适合 15 秒生成，优先修 `5-分组`，不是把视频判死。
5. 如果只是手、脸、伪字、运动抖动等局部生成瑕疵，通常建议 rerun，不改分组。
6. 只有当同类问题跨多个素材反复出现，且能指向 `8-视频` 命名/输出、`5-分组` 字数/桥接或 `4-摄影` 可动性合同，才进入源层候选。
...
- `5-分组` 是最常见修复落点，因为它直接决定单条 15 秒视频的内容密度、焦点和首尾状态。
