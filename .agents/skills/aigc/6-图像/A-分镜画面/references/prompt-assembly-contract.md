...
每个 `Integrated AIGC image prompt` 必须按完整镜级设计体系组织。体系的作用是把 `4-分组` 的单镜真相、参照图视觉证据、空间连续性和 imagegen 执行约束合成一个可生成的英文 prompt。
...
1. 组织 `Integrated AIGC image prompt` 前，先从 `5-设计/场景/3-生成` 预绑定当前场景参照图。
...
6. 当前 prompt 必须在不改写 `4-分组` 核心内容的前提下延续这些空间事实；若源镜头明确发生换位、转身、移动或镜头反打，prompt 应写清变化路径，使走位逻辑可追溯。
7. 若上一图缺失或上一分镜尚未生成，记录 `previous_image_missing` 或 `previous_shot_not_generated`，只能依赖 `4-分组` 与设计图，不得臆造上一画面的实际生成结果。
