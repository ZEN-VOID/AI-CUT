...
- **专有名词统一写法**：人名/势力/境界/技能名，统一一个版本（写入 `Cards/` 并在 `state.json` 追踪）。
...
9. 章末是否有钩子
10. 新实体首次出现是否写清楚（名字/身份/记忆点）；如需手动标注（可选）：使用隐藏 XML 标签（见 `.agents/skills/story/query/references/tag-specification.md`）
