...
| Project Root Guard | 先解析 `PROJECT_ROOT` | `SKILL.md#Thinking-Action Node Map` | split | low | field `FIELD-QUERY-01` |
...
| old `1-Planning` mixed planning/grouping/report facts | 旧规划目录同时承载规划、格式、分组、节奏和报告线索 | `references/project-runtime-layout.md`、`types/query-type-map.md`、对应阶段执行报告 | split and annotate | medium | query maps episode split to `1-分集`、grouping to `8-分组`、validation/report facts to `执行报告.md` 或 legacy fallback |
| old `3-Detail` / `2-编导` 主文件查询 | 旧编导/detail root | `references/project-runtime-layout.md` | adapt by intent to `4-编剧` / `5-导演` / active `6-分镜` or explicit archived `5-表演` with legacy fallback | medium | output marks current owner, legacy fallback, and archived status |
| old `4-Design/5-Image/6-Video/7-Cut` | 旧资产阶段 | `references/project-runtime-layout.md` | adapt to `3-主体/9-图像/10-画布` | medium | no default answer points to legacy |
| Workflow Checklist | 查询执行顺序 | `SKILL.md#Thinking-Action Node Map` | rewrite as nodes | low | nodes have route, evidence, gate |
...
| 旧 `3-Detail` / `2-编导` 主文件查询是否按意图改查 `4-编剧`、`5-导演`、active `6-分镜` 或 explicit archived `5-表演`，只有证据需要时才标注 legacy fallback？ | `GATE-QUERY-06` | `FAIL-QUERY-LEGACY` | `N3-carrier-read` | 输出 current owner 路径、legacy 路径、archived 状态及 fallback 原因。 |
| 旧 `4-Design/5-Image/6-Video/7-Cut` 是否只作为 legacy compatibility，而不冒充当前 `3-主体/9-图像/10-画布` 的默认产物根？ | `GATE-QUERY-06` | `FAIL-QUERY-LEGACY` | `N3-carrier-read` | 报告当前资产 carrier 优先级，并列出被降级的旧英文阶段名。 |
| 旧 Workflow Checklist 是否已被 `SKILL.md#Thinking-Action Node Map` 的节点网络承接，查询回答能回溯到具体读取、验收或治理节点？ | `GATE-QUERY-04` | `FAIL-QUERY-EVIDENCE` | `N6-answer` | 报告每条结论对应的节点、读取路径和缺口来源。 |
...
| 旧 `CONTEXT.md` 经验是否只进入当前 `CONTEXT.md` / `knowledge-base`，没有反向覆盖 `SKILL.md`、references 或运行时 carrier 真源？ | `GATE-QUERY-01` | `FAIL-QUERY-CONTEXT` | `N0-load-contract` | 报告经验层只作为查询策略参考，并列出规范真源仍来自 `SKILL.md`、`references/`、`types/`、`review/`。 |
