...
def _risk_label(avg_value: float) -> str:
    if avg_value >= 3.5:
        return "critical"
    if avg_value >= 2.5:
        return "high"
...
def _build_review_rows(records: List[Dict[str, Any]]) -> List[str]:
    ...
    for row in sorted_records:
        ...
        anti_ai_force_check = str(row.get("anti_ai_force_check") or "pending")
        spoiler_risk = str(row.get("spoiler_risk") or "low")
        contrivance_risk = str(row.get("contrivance_risk") or "low")
        cold_commentary_risk = str(row.get("cold_commentary_risk") or "low")
        rows.append(
            f"| {range_text} | {score:.1f} | {critical} | {high} | {medium} | {low} | {anti_ai_force_check} | {spoiler_risk} | {contrivance_risk} | {cold_commentary_risk} |"
        )
    return rows
...
def _build_risk_flags(
    ...
    anti_ai_counts = review_trend.get("anti_ai_force_check") or {}
    anti_ai_fail = _to_int(anti_ai_counts.get("fail"))
    anti_ai_pending = _to_int(anti_ai_counts.get("pending"))
    if anti_ai_fail > 0:
        flags.append(f"近期待复核区间中有 {anti_ai_fail} 条 `anti_ai_force_check=fail`，需优先清理 AI 痕迹。")
    elif anti_ai_pending > 0:
        flags.append(f"近期待复核区间中仍有 {anti_ai_pending} 条 `anti_ai_force_check=pending`，说明终检闭环还没补完。")

    risk_avg = review_trend.get("risk_avg") or {}
    for field, label in (
        ("spoiler_risk", "剧透风险"),
        ("contrivance_risk", "刻意感风险"),
        ("cold_commentary_risk", "冷评风险"),
    ):
        avg_value = _to_float(risk_avg.get(field))
        if avg_value >= 2.5:
            flags.append(f"{label}均值达到 {_risk_label(avg_value)}（{avg_value:.2f}），建议专项回看表达层与揭晓节奏。")
...
def build_quality_report(
    ...
    *,
    ...
    lines.append("## 审查区间趋势")
    lines.append("")
    lines.append("| 区间 | 总分 | Critical | High | Medium | Low | Anti-AI | 剧透风险 | 刻意感风险 | 冷评风险 |")
    lines.append("|---|---:|---:|---:|---:|---:|---|---|---|---|")
    ...
    lines.append("## 风险雷达")
    lines.append("")
    lines.append("| 指标 | 当前窗口 |")
    lines.append("|---|---|")
    lines.append(
        f"| anti_ai_force_check | pass={_to_int(anti_ai_counts.get('pass'))}, fail={_to_int(anti_ai_counts.get('fail'))}, pending={_to_int(anti_ai_counts.get('pending'))} |"
    )
    for field, label in (
        ...
        avg_value = _to_float(risk_avg.get(field))
        if avg_value > 0:
            summary = f"{avg_value:.2f} ({_risk_label(avg_value)})"
        else:
            summary = "-"
