...
本文件是 `aigc/7-摄影` 的经验层知识库，不是规范真源。调用同目录 `SKILL.md` 时必须同时加载本文件；它只沉淀可复用判断经验、失败模式和修复打法，不改写 `SKILL.md` 的输入、输出、节点或门禁。
...
| --- | --- | --- | --- |
| 把 `7-摄影` 当成旧阶段延续 | 阶段边界层 | 回到 `Core Task Contract`，只处理 `6-分镜` 行后的运镜注入 | source 为 `6-分镜` 或用户指定稿，输出到 `7-摄影` |
...
1. 先锁定 source 是 `6-分镜` 还是用户指定稿，不从旧阶段路径反推。
...
7. 一镜到底不要逐条设计“漂亮镜头”，先设计摄影机身体如何连续穿过 6-分镜 已定义的空间；不能写上游没有的前景、遮挡、后景或入口。
