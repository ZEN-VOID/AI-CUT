...
| dimension | checks |
| --- | --- |
| input | 项目、集号、`分镜组ID` 或 source request 是否唯一 |
| prompt | LLM 主创、固定英文前缀、组级边界、多镜融写列、字段覆盖、压缩层级、模板骨架 |
...
7. 原三个技能包是否未被删除或移动。
8. `request-distillation.md` 与 `steps/storyboard-sheet-workflow.md` 是否保留旧 `分镜故事板` 完整蒸馏方法，而不是只写摘要。
...
- prompt 正文由脚本主创。
- 组内镜头顺序丢失或被摘要化。
- `from_detail + storyboard_group` 路由跳过 `S2/S3`。
- 引用绑定存在未处理歧义。
