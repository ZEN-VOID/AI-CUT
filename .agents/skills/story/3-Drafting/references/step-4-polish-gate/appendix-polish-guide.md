...
> Canonical module route: `references/step-4-polish-gate/module-spec.md`
> 当前角色：Step 4 appendix。根 `SKILL.md` 不再直接路由到本文件。
...
| 观察项 | 可能风险 | 较稳信号 |
|-----|-------|------|
| 停顿词 | 全文完全没有停顿，语气像统一播报 | 关键对白或高压段偶尔有自然停顿 |
| 不确定表达 | 所有人都像结论机器，没有试探和迟疑 | 在需要犹疑、试探、回避时能出现不确定表达 |
| 短句占比 | 全文长句均匀铺开，爆点不加速 | 爆点、判断、冲突处能看见短句提速 |
| 口语词 | 全员标准书面语，缺人物活气 | 角色在合适场景里有自身口头习惯或生活气 |

> **提示**：这些信号只用于发现模板腔，不得反向变成新的配额；角色声口、题材修辞和叙事策略优先于表面计数。

### 执行协议（症状优先）

- 必须执行 Anti-AI 检查，但检查目标是**模板症状**，不是机械消灭词表命中。
- 高频词库与 7 层规则默认作为扫描入口和修复工具，不是逐项清零指标。
- 必查部位：
  - 章首抓手
  - 章末钩子
  - 关键对白
  - 审查报告命中的问题段落
- 若以上关键部位仍存在明显模板腔、说明腔或机械腔，才继续向外扩展全文排查。
- Step 4 完成后必须输出 `anti_ai_force_check: pass/fail`。
- 若结果为 `fail`，必须继续留在 Step 4 重写，不得进入 Step 5。
- 若命中高风险表达但因角色声口、题材惯例或叙事策略需要保留，必须写入 `deviation`（位置 + 原因 + 代价）。

### 检查范围

- 必查：章首、章末、关键对白、问题段落
- 扩查触发：当关键部位暴露出重复模板句群、统一播报腔或严重词表堆积时，再向相邻段落或全文扩查
- 任何命中项都应做“保留 / 改写 / deviation”三选一裁决，禁止只因命中单词就自动改写
...
- 关键部位不得保留未处理的三段式枚举模板句。
- 已识别的高风险症状均已完成“改写或记录 deviation”。
- 抽象句向行为句的改写要覆盖**真正卡顿的句群**，不设固定数量配额。
- 章末存在可感知的下一步压力或未闭合问题。
- 若仍保留高风险表达，需在 deviation 说明保留理由与收益。
