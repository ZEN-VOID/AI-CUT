...
description: Use when `aigc/review` needs the governed child skill that audits detail-root completeness, shot continuity, and storyboard execution readiness.
...
- `3-Detail/第N集.json` 是否具备稳定可消费的导演事实
- 组镜 continuity、镜级字段完整度、handoff readiness
...
```mermaid
flowchart TD
    A["读取 detail root"] --> B["检查组镜字段与 continuity"]
...
| --- | --- | --- | --- | --- | --- |
| `N1-DETAIL-READ` | 锁 detail root | 读取第N集 JSON 与 validator evidence | `detail_note` | `N2` | root 明确 |
...
若本维度失效，先修 `3-Detail` 的结构完整性与 handoff readiness，不要把 detail 问题伪装成 design/image/video 问题。

## Completion Contract

- 已指出 detail root 的 continuity 或 readiness 问题
- 已给出回退到 `3-Detail` 的建议
