...
| `N6-NODE-PLAN` | 规划唯一视频节点实例、参数和破坏性动作 | source_group_id、active registry、remote node list、用户覆盖项 | 为每个组选择 `vid__<source_group_id>__bNNN__rNN__vNNN`；默认新建不覆盖；二修记录 parent；删除旧 video 节点只在用户明确授权时执行；默认规格为 `star-video2/mixed2video/16:9/720p/enableSound=on`；`duration` 必须读取分镜组 `时长估算` 或用户覆盖值并保留小数，`.5` 不得因 LibTV schema `step=1` 被取整、向上取整或 `parseInt`；若需要半秒尾帧缓冲，只在结果仍处于模型 `4–15` 秒范围内追加 `0.5`，否则保持边界值并记录原因；用户显式覆盖时记录 | instance identity map、delete authorization、settings plan、duration_source/normalization、legacy node count | `N7-ORDER-LOCK` / `R1-REWORK` | `GATE-LTVCTRL-NODE-IDENTITY`：同组多实例不覆盖、不跳过；节点名唯一且参数可审计 |
...
| `duration_fidelity` | 每个节点 `settings.duration` 必须等于分镜组 `时长估算`、用户显式覆盖值或记录过的半秒尾帧归一结果；`.5` 秒必须作为数值小数写入 queue、submit plan、summary/report 和远端节点，不得整数化 | `N6-NODE-PLAN`、`N8-PROMPT-AND-FINAL-QUERY`、`N9-EVIDENCE` | `FAIL-LTVCTRL-DURATION-FIDELITY` |
