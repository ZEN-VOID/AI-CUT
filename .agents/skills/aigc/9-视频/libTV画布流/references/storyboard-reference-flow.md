...
- 基本输入对象：故事板图、分镜图或 `8-图像/B-分镜故事板` 工件。
- 图像参照绑定表：故事板图与分镜组 ID 的关系。
- LibTV CLI handoff plan 和 source node 映射。
- 时长、分辨率、比例、画布沉淀和下载策略。
- review gate 与输出目录。
...
| 用户显式选择分镜参照流时，是否返回 `not_implemented_placeholder`，而不是伪造提交计划或生成任务？ | `REV-LIBTVCANVAS-01` | `FAIL-ROUTE` | `N1 Intake` | `PLACEHOLDER` verdict、blocked queue/report、未调用 LibTV 执行命令的证据 |
| 占位响应是否说明未来最小输入对象：故事板图、分镜图或 `8-图像/B-分镜故事板` 工件？ | `REV-LIBTVCANVAS-01` | `FAIL-ROUTE` | `references/storyboard-reference-flow.md` | placeholder report 中的 missing input list |
