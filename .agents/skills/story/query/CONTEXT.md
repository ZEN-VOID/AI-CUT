...
| 实绩查询忽略 `5-上下文回流` 的 PASS-only gate | stage contract | 把 `actualization` 与 `第V卷.context-return.json` 设为必须校验项 | 在 query 规则中写明“无 PASS 证据只能回答尚无 validated actual” | 实绩类回答会带 `validation_ref` 或明确缺口 |
