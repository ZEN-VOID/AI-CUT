...
本文件是 `story-polishing-deepseek` 的类型包索引。执行时先加载本索引，再加载命中的类型包；`knowledge-base/` 只做按需检索，不替代固定类型上下文。
...
| `guardrail-setup` | 每次执行前、provider 调用前与写回前 | `types/guardrail-setup.md` | required | `security` / `runtime_behavior` |
...
- 若用户显式要求多路线对照，本索引只负责当前 `C-Deepseek流` 路线；英文序号分流由父级 `4-润色/SKILL.md` 裁决。

## Loading Flow

1. 读取 `SKILL.md + CONTEXT.md`，锁定输入、输出、provider 边界和 guardrails。
