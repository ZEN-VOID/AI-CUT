...
3. 若缺 `3-美学/画面基调/全局风格协议.md`、当前集优先/项目级回退的 `3-美学/道具风格/道具风格协议.md`、`north_star.yaml` 或 `team.yaml.init_synthesis`，保留道具设计可执行部分，但在文件或报告中标注画面基调、道具风格、fallback 状态或初始化综合缺口。
