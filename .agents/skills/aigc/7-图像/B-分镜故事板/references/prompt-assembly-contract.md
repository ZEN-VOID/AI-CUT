...
固定开头之后先进入 `Storyboard Frame Units`，再进入 `5-分组` 的组正文主体；不插入解释性说明、provider 参数、执行日志或额外任务指令。
...
<5-分组中该组现有正文，不含底部 YAML>

### Reference Subjects

Characters:
- 林寂: projects/aigc/<项目名>/6-设计/角色/3-生成/林寂-多视图.png

Scene:
- 永夜私立中学二年级A班教室: projects/aigc/<项目名>/6-设计/场景/3-生成/永夜私立中学二年级A班教室-多视图.png
...
4. `group_body` 直接来自 `5-分组`，没有上游剧情改写。
5. 所有 `分镜N:` 标签按原顺序保留。
...
| 固定开头之后是否先写 `Storyboard Frame Units`，再进入 `5-分组` 组正文主体，没有插入 provider 参数、执行日志或解释性说明？ | `G2-PREFIX` | `FAIL-SHEET-PROMPT` | `N4-PROMPT` / `references/prompt-assembly-contract.md#fixed-prefix` | prompt package 结构顺序为 fixed prefix -> frame units -> group body -> reference subjects |
