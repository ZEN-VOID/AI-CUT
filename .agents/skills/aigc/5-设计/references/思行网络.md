...
| --- | --- | --- | --- | --- |
| `D-N1-INTAKE` | 用户请求、项目名、集数、候选输入文件 | 锁定项目根与 5-设计 输出根 | `design_scope` | `projects/aigc/<项目名>/4-设计/` 可定位或可创建 |
...
| `D-N4-DOMAIN-GATE` | 域级输出 | 检查 `[域]清单.md + [主体名].md + [主体名].json` | `domain_verdicts` | 输出位于 5-设计 根 |
...
| 输出不在 5-设计 根 | `D-N4-DOMAIN-GATE -> 对应域 SKILL.md Output Contract` |
