...
| `scene_id`、`deconstruction_subject_id`、`period_region_anchor`、`space_style_token` 是否分别能回指上游场景主体、解构主体 ID、时间/地域锚点和按场景类型选择的空间风格锚点，且 ID 不发生漂移、非建筑场景不被强行建筑化？ | `GATE-SCENE-DESIGN-SLOT-01` | `FAIL-SCENE-DESIGN-SLOT-01` | `N6-DESIGN` | 留下 slot evidence 表，包含场景 ID、解构 ID、prompt 前缀、时间 token、地域 token、space_style_token 与类型选择依据的位置。 |
