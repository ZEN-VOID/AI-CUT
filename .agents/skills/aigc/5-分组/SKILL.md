...
当用户显式要求 `5-分组` 直接接手剧本、编导稿、无 `4-摄影` 逐集稿的 `第N集.md`，或项目当前缺少 `4-摄影/第N集.md` 但已提供可读剧本真源时，本技能自动进入 `direct_screenplay` 源层兼容路径：由 LLM 直接把剧本拆成约 15 秒/组的 production-ready 分镜组，补写普通 `[N-N秒]` 时间码行、组级 `画面风格：`、首帧衔接和 YAML 统计；不得要求用户先手动补 `4-摄影`，不得用脚本生成正文。该路径必须在 frontmatter 与执行报告中声明 `source_state: direct_screenplay`、记录剧本源路径，并说明无法执行 `4-摄影` 字段级保真 diff。

术语定义：`回龙帧` 是本阶段对首帧衔接的内部执行口径，指下一分镜组第一个普通 `[0-N秒]` 分镜行完整代入上一分镜组结尾状态画面点内容，再通过景别、机位、镜头角度、焦距、观看距离或焦点路径调整进入本组开始画面。若被回龙的画面点来自 `对白画面`、`独白画面`、`旁白画面`、`音效画面` 等声音承托画面，必须把对应声音内容一并代入首行画面描述，保持声画关系不断裂。`回龙帧` 不是输出字段、不是独立连接件、不是复制上一组完整画面块或完整原画面性字段；落盘正文不得出现 `回龙帧：`、来源说明或规则说明。

新版组头不再输出 `入场镜头：`、`出场画面：`、`画面构图：`、位置细节字段、`分镜画面：`、`增补首帧：` 或 `回龙帧：`。组内连续性的主要机制改为：每组第一个原画面性字段标题下方直接从普通 `[0-N秒]` 时间码行开始；首组自然承接本组开始画面，第二组起按回龙帧口径承接上一组尾帧状态，但不写任何“增补首帧”字段、来源说明或规则说明。
...
- 上游正文真源默认固定为 `projects/aigc/<项目名>/4-摄影/第N集.md`。若用户显式指定剧本/编导稿作为输入，或 `4-摄影/第N集.md` 缺失但存在可读剧本真源，则切换为 `direct_screenplay`：以用户指定文件或最近上游剧本文件为源，自动按约 15 秒/组拆组并落盘到 canonical `5-分组/第N集.md`。
...
- 项目名、项目路径、单个 `projects/aigc/<项目名>/4-摄影/第N集.md` 文件、单个剧本/编导稿文件，或多个集号范围。
- 用户要求“分组”“分镜组”“从 4-摄影 到 5-分组”“给摄影稿按分镜组切分”“直接用剧本分组”“从 2-编导 执行 5-分组”“首帧衔接”“缝纫线头”“修复 15 秒分组之间断裂”等任务。
- 已完成或部分完成的 `4-摄影` 逐集稿；或用户显式指定 / 项目仅有的 `2-编导`、剧本、分集稿等可读逐集源。默认以集为单位处理 `第N集.md`。

Required input:

- 可定位、可读取的 `projects/aigc/<项目名>/4-摄影/第N集.md`；或在 `direct_screenplay` 路径中，可定位、可读取的用户指定剧本/编导稿逐集文件。
- 可定位、可读取的 `projects/aigc/<项目名>/0-初始化/north_star.yaml`。
- 至少一个目标集号，或允许默认处理 `4-摄影/` / 用户指定源目录中全部 `第N集.md`。
- 标准摄影路径：输入正文中存在可识别的场景标题、剧本字段、对白字段、非画面字段或原画面性字段标题及其 `[起始秒-结束秒]` 时间段。
- `direct_screenplay` 路径：输入正文中存在可识别的场景标题、角色动作、环境描写、对白、音效或转场等剧本字段；可没有原画面性字段时间段，由 LLM 按约 15 秒/组补写可验证的连续时间码。
...
- `0-初始化/north_star.yaml` 不存在或不可读。
- 上游 `4-摄影/第N集.md` 不存在、不可读，且用户没有提供替代真源，也无法在项目上游阶段中定位可读剧本/编导稿。
...
| `direct_screenplay` | 用户显式指定剧本/编导稿，或缺少 `4-摄影` 但存在可读逐集剧本源 | 按约 15 秒/组直接生成 `5-分组/第N集.md`，报告声明源层兼容路径 |
| `episode_range` | 指定多个集号或集号范围 | 多个逐集分镜组稿与更新后的执行报告 |
| `all_ready_episodes` | 未指定集号但 `4-摄影/` 下有 `第N集.md` | 全部可读逐集分镜组稿 |
| `repair` | 已有分组稿 ID 错误、组过长、`全局风格` 拼接缺失、首帧衔接/回龙帧断裂、旧连接件块残留或 YAML 统计不完整 | 最小修复后的逐集分组稿与问题报告 |
...
| 反抽象语言、全局风格、画面风格和首帧衔接/回龙帧具象化落盘 | `../_shared/anti-abstract-language-contract.md`，并回接 `references/north-star-projection-contract.md`、`references/group-visual-tone-contract.md` |
...
```mermaid
flowchart TD
    A["projects/aigc/<项目名>/4-摄影/第N集.md 或 显式剧本源"] --> B["场景与集号取证"]
...
flowchart TD
    ...
    P3V --> P3F["PASS-GROUP-03F 首帧衔接/回龙帧裁决"]
    ...
    P6 -->|"FAIL-GROUP-12 首帧衔接/回龙帧错误"| P3F
...
2. 锁定上游源文件。标准路径读取 `4-摄影/第N集.md`，提取集号、场景标题、正文块、对白字段、原画面性字段标题、字段下方 `[起始秒-结束秒]` 时间段和场景顺序；不得改写原正文。`direct_screenplay` 路径读取用户指定剧本/编导稿，提取场景标题、剧本字段、对白、音效、动作与转场，按声画完整性拆成约 15 秒/组，并由 LLM 补写连续 `[N-N秒]` 普通分镜行。
...
4. 按 `references/group-boundary-contract.md` 执行边界裁决：标准摄影路径中，上游每个原画面性字段以最后一条 `[起始秒-结束秒]` 的结束秒作为该字段画面总时长，先用这些字段总时长裁决分组；落盘到当前分镜组后，原本在各字段内各自从 0 开始的时间段必须统一改写为当前分镜组基准下的连续累计 `[N-N秒]`，后一个时间段起点必须等于前一个时间段终点，组内 `时长估算` 取当前组最后一个时间段的结束秒。`direct_screenplay` 路径中，LLM 以剧本字段、对白承托、动作落点、物件证据和场景转折作为 atomic unit，直接规划约 15 秒/组的连续时间码。每组优先接近约 15 秒，通常允许约 12-18 秒弹性，单组时长硬上限为 18 秒。
...
6. 按普通分镜画面写法内化首帧衔接；本阶段将这种内化机制称为 `回龙帧`：
   - 每组第一个原画面性字段标题下方直接写普通 `[0-N秒]` 时间码行，不输出 `分镜画面：`、`增补首帧：` 或任何可识别特殊字段。
   - 首组第一条时间码行自然整理本组开始画面；第二组起第一条时间码行必须完整代入上一组最后分镜结尾状态画面点内容，例如主体姿态、动作落点、道具位置、空间关系、门缝光线、烟雾余波、视线落点或动作完成瞬间，并通过景别和镜头视角调整接入本组开始画面。
   - 若回龙帧锚点来自 `对白画面`、`独白画面`、`旁白画面`、`音效画面` 或其他声音承托画面，首行必须同步代入对应对白、独白、旁白或音效内容；不得只保留静态视觉而切断声画承托。
   - 回龙帧只调整景别、机位、镜头角度、焦距、观看距离或焦点路径，不新增剧情、不改对白、不改变人物状态，不复制上一组完整画面块或完整原画面性字段；这些规则不得写入输出正文。
   - 回龙帧作为普通分镜正文计入 `字数统计` 和当前组时间码，不输出 `回龙帧：` 字段或独立组间连接件。
7. 标准摄影路径保持从 `4-摄影` 划定的分镜剧本正文、字段标题和字段下方时间段完整同步原换行；不得删改上游字段、对白、时间段或场景顺序；不得把原画面性字段标题替换为 `分镜画面：`。`direct_screenplay` 路径必须保留剧本事实、场景顺序、英文对白原文和声画承托关系，可将剧本字段投影为带时间码的画面字段，但不得改写对白、增删剧情事实或把估算时间伪装成 `4-摄影` 真源。
...
| `scripts/validate_storyboard_groups.py` | 必跑机械校验：检查分镜组标题、场景标题行、`全局风格：` 字段、`画面风格：` 位于第一个原画面性字段标题和第一个时间码上方、禁用 `分镜画面：` / `增补首帧：` / `回龙帧：` / `入场镜头：` / `出场画面：` / `画面属性：` / `画面构图：` / 六类位置细节字段 / `## A~B` 连接件块、YAML 统计、组内时间码连续累加和编号连续性；不能替代分组边界、首帧衔接/回龙帧语义、声音承托同步、原文保真和统计证据判断 |
...
| --- | --- | --- | --- |
| `FIELD-GROUP-01` | 输入取证 | source cinematography episode 或 direct screenplay source、north_star、项目记忆、相关上下文、目标集号明确 | `FAIL-GROUP-01` |
...
| `FIELD-GROUP-03` | 边界裁决 | 标准摄影路径按原画面性字段时间段总时长接近 15 秒；direct screenplay 路径按剧本声画 atomic unit 直接规划约 15 秒/组；通常 12-18 秒，且不得超过 18 秒；同一 atomic unit 不跨组；落盘后组内时间码从 0 连续累加 | `FAIL-GROUP-05` / `FAIL-GROUP-06` |
...
| `FIELD-GROUP-06` | 首帧衔接 / 回龙帧 | 每组第一个原画面性字段标题下方第一行是普通 `[0-N秒]` 分镜行；首组承接本组开始画面，第二组起以回龙帧口径完整代入上一组结尾状态画面点内容，并只调整景别和镜头视角进入本组；若锚点来自对白/独白/旁白/音效画面，必须一并带入对应声音内容；不输出 `分镜画面：`、`增补首帧：`、`回龙帧：` 或规则说明 | `FAIL-GROUP-12` |
| `FIELD-GROUP-07` | 连接件禁用 | 不输出 `## <上一个分镜组ID>~<下一个分镜组ID>` 连接件块，也不输出连接件字段 | `FAIL-GROUP-07` |
| `FIELD-GROUP-08` | 原文保真 | 标准摄影路径同步 `4-摄影` 划定正文；direct screenplay 路径保留剧本事实、场景顺序、对白原文和声画承托关系，并在报告中声明时间码由本阶段 LLM 规划 | `FAIL-GROUP-09` |
...
| `PASS-GROUP-03F` | 首帧衔接 / 回龙帧裁决 | 上一组最后原画面性字段、本组第一个原画面性字段 | 是否把上一组结尾状态画面点内容完整内化到第一个普通 `[0-N秒]` 分镜行，并只通过景别和镜头视角调整进入本组；若画面点承托声音，是否同步带入声音内容；不写特殊字段或规则说明 | `first_storyboard_line_continuity` |
| `PASS-GROUP-05` | 统计抽取 | 完整分镜组正文 | 角色、场景、重要叙事道具是否准确；道具是否已完成同物识别、归一合并和去重 | `stats_yaml` |
| `PASS-GROUP-06` | 落盘与审查 | 分组稿 | ID、全局风格、画面风格、首帧衔接/回龙帧、声音承托同步、时长、连接件禁用和 YAML 是否通过 | `review_result` |
...
| `PASS-GROUP-03F` | 写首帧衔接 / 回龙帧 | 第一个原画面性字段标题下方普通 `[0-N秒]` 分镜行、自然画面描述 | `steps/grouping-workflow.md#N5F-FIRST-STORYBOARD-LINE` |
...
- 第一个原画面性字段标题下方第一行不是普通 `[0-N秒]` 分镜行，首帧衔接/回龙帧断裂，未完整代入上一组结尾状态画面点内容，回龙到对白画面、独白画面、旁白画面或音效画面时遗漏对应声音内容，写成规则说明或模板化来源说明，输出 `分镜画面：`、`增补首帧：` 或 `回龙帧：` 字段，复制上一组完整画面块或完整原画面性字段，或借回龙帧新增剧情、改对白、改变人物状态。
...
- Completion gate: 输入、边界、`全局风格`、`画面风格`、首帧衔接/回龙帧、YAML、原文保真与机械校验均通过；旧字段 `增补首帧：`、`入场镜头：`、`出场画面：`、`画面属性：`、`画面构图：`、六类位置细节字段、`回龙帧：` 和 `## A~B` 连接件块不再出现在新产物中。
