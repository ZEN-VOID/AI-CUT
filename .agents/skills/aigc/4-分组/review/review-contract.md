...
| mode | trigger | action |
| --- | --- | --- |
| `mechanical_check` | 每次落盘前或修复后 | 运行 `scripts/validate_storyboard_groups.py`，检查分镜组标题、必填标题、字数、YAML、组间桥接一致性；warning 进入语义复核，不直接等于 PASS |
| `boundary_review` | 每次阶段 PASS 前 | LLM 或人工检查对白数、1680 字目标上限、1980 字硬上限、atomic unit 完整性、过长组拆分复核和短组例外理由 |
| `bridge_review` | 每次阶段 PASS 前 | LLM 或人工检查入场/出场画面是否同画面、短镜头、非新增剧情、可自然接续首尾帧 |
...
| `GATE-GROUP-06` | 字数 | 每组完整构成常规目标 <= 1680 字；1681-1980 字必须有拆分复核后的例外理由；任何组不得超过 1980 字；均不含 YAML |
...
| `FAIL-GROUP-05` | 组超 1680 字未复核、组超 1980 字硬上限或对白过载 | `references/group-boundary-contract.md` |
