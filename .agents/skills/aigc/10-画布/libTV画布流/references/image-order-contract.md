...
4. 按 `图片N` 顺序逐张执行 `libtv node <video_node_instance_id> --left-add <node_key>`。
5. 连线后再次写入 `imageList/mixedList/imageListOrder/mixedListOrder`，因为单靠 `--left-add` 不保证远端 `params.imageList` 重新排序。
6. 最后查询 `libtv node <video_node_instance_id>`，以返回的 `data.params.imageList[] + data.params.prompt` 为唯一完成真源。
...
1. 原分镜组正文，保持不改写。
2. 底部完整 fenced YAML；提交 prompt 时将已有 `图片N 主体名 UUID` 主体行重排为 `图片N 主体名 {{Image N}} UUID`。

示例：

```yaml
角色:
  - 图片1 罗曼·沃斯 {{Image 1}} 4222b19c-b712-478e-ad33-9ce68b8fb61a
  - 图片2 艾娃·沃斯 {{Image 2}} 7baf0914-3d12-4bac-9fbb-366d5d1bb2b5
场景:
  - 图片3 沃斯庄园书房 {{Image 3}} 37515350-d9ff-4951-9d7c-97b7cc449f4d
```
...
| prompt 是否在 YAML 主体行使用 `图片N 主体名 {{Image N}} UUID`，且没有 `{{Portrait N}}`？ | `GATE-LTVCTRL-PROMPT` | `FAIL-LTVCTRL-PROMPT-POLLUTION` | `N7-PROMPT-HYGIENE` | final prompt query |
