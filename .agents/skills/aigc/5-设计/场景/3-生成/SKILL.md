...
- 可读取的上游单场景设计文档，且包含 `提示词设计` 与可识别的英文提示词。
- 每个目标场景至少有一个 canonical 主体名称。
- 可用的 `$imagegen` 执行路径；默认使用内建 `image_gen`，生成后需把项目交付资产持久化到 workspace。
...
| `FIELD-SCENE-GEN-03` | 主图生成 | 每个目标场景生成 `主体名称-主图` | `FAIL-SCENE-GEN-03` |
| `FIELD-SCENE-GEN-04` | 多视图生成 | 以对应主图为参照生成 `主体名称-多视图` | `FAIL-SCENE-GEN-04` |
...
1. 每个目标场景输出一张单主体主图：`主体名称-主图`。
2. 每个目标场景输出一张多视图主体设计图：`主体名称-多视图`。
