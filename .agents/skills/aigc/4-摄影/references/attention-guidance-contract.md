...
`attention-guidance-contract.md` defines how `4-摄影` actively controls what the audience notices first, second and last inside each shot. It is consumed by `N6.4-FUNCTIONAL-PROJECTION`, `N6.5-SHOT-PLAN`, `N7-INJECT` and `N8-REVIEW`.
...
| Is the attention path written into `时间段` as camera action, composition, focus or blocking rather than explanatory text about what the audience should feel? | `GATE-CINE-10` / `GATE-CINE-18` | `FAIL-CINE-05A` / `FAIL-CINE-05G` | `steps/cinematography-workflow.md#N7-INJECT` | before/after lines showing explanatory attention text rewritten as visible camera behavior |
