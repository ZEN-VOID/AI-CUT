...
| `N5-REF-BIND` | 保守绑定 YAML 主体参照 | prompt package、5-设计生成目录 | 多视图优先、主图次之、缺图移除槽位 | reference manifest | `N6` | 无猜测路径 |
| `N6-REVIEW` | 执行生成前审查 | prompt、manifest | 检查 ID、固定开头、组正文、路径、mode | review note | `N7` / `N9` / repair | 必需项通过 |
| `N7-IMAGEGEN` | 批量调用 imagegen | imagegen plan | 每组独立任务，默认顺序或受控批量执行；更高吞吐执行方式必须由工具能力和用户显式要求共同支持 | plan/result json | `N8` | 不覆盖、不越权 |
