...
| `N5-REF-BIND` | 保守绑定 YAML 主体参照 | prompt package、5-设计生成目录 | 多视图优先、主图次之、缺图保留原主体名，已上传主体注入 YAML `uploaded_url` | reference manifest | `N6` | 无猜测路径 |
...
| `N8-DISPATCH` | 批量调用 LibTV | submit plan | 每组独立任务，按 `duration_hint=clamp(duration_estimate_seconds, 4, 15)` 写入远端提交，默认后台多线程并发提交 | session ids | `N9` | 不覆盖、不越权、不固定 15 秒 |
