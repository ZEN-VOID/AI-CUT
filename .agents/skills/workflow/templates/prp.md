...
| `dialogue_alignment.json` | supplied transcript, HyperFrames media transcribe, or per-cue manual timing review with audio anchors plus sortable script order | `C3-DIALOGUE-CLOCKED` |
| `dialogue_sync_validation.json` | run `scripts/validate_dialogue_sync.py --strict-final` against project root and HTML caption cue-id timeline | `C3-DIALOGUE-CLOCKED` / `C7-RENDER-VERIFIED` |
