...
- 必须回读父层 `2-Planning/SKILL.md`、`../_shared/planning-slice-layout-contract.md`、`../_shared/planning-branch-output-contract.md`、`references/volume-rhythm-framework.md`、当前 `2-Planning/全息地图.json` 与受影响十集分片。
...
| analysis_slot | 当前结论 |
| --- | --- |
| `business_goal` | 把体量判断翻译成稳定章节容器与整书节奏骨架，为后续 3-7 提供“可挂内容、可控波形、可查拍点”的 planning 基座。 |
| `business_object` | global root 的 `volume_boards / episode_slice_manifest / thin episode_sequence_axis`，目标 slice 的 `chapter_boards / episode_sequence_axis / chapter_boards[].bundled_elements.characters / chapter_boards[].planned_state.character_focus`，以及 `story_promise.type_stack_ref / genre_corridor.type_pack_projection / 主题线索`。 |
| `constraint_profile` | 只负责容器，不代写主干与长线。 |
| `success_criteria` | chapter/volume blocks 稳定，`macro_rhythm_scaffold` 已锁住整书波形与关键拍点，board 已能通过角色/关系投影锁定角色焦点，后续 child 可以直接在 board 上挂内容与事件。 |
...
flowchart TD
    ...
    B --> C["锁 Save the Cat 节奏骨架"]
    C --> D["锁 function slots / density / 爽点梯度"]
    D --> E["生成 rhythm-aware chapter_boards skeleton"]
    E --> F["写 evidence artifact + patch"]
```
...
| `N3-MACRO-RHYTHM-SCAFFOLD` | `FIELD-CHP-03` | 锁整书波形与关键拍点 | 基于 `references/volume-rhythm-framework.md` 生成 `macro_rhythm_scaffold`，明确 `theme_carrier / b_story_carrier / midpoint_shift / all_is_lost_corridor / finale_acceleration_rules` | `macro_rhythm_note` | -> `N4` | 拍点不是死百分比，且能回答整书改向 |
| `N4-DENSITY-CONTRACT` | `FIELD-CHP-04` | 锁密度、爽点梯度与节奏窗口 | 结合 volume wave 生成 `scale_density_contract / rhythm_windows / episode_rhythm_roles` | `density_note` | -> `N5` | 负荷成立，且中段不过平 |
| `N5-PATCH-WRITE` | `FIELD-CHP-05` | 写 skeleton patch | 生成 `volume_boards / episode_slice_manifest / thin axis` 与 `chapter_boards skeleton`，确保 board/axis 含节奏职责摘要 | `patch_note` | done | 只命中 owned slots |
...
| `FIELD-CHP-05` | `slice chapter_boards skeleton` | skeleton 可供后续挂载，且 board/axis 已带节奏职责摘要、角色/关系焦点引用成立 | `FAIL-CHP-05` | `N5` |
