...
description: "Use when running an AIGC project episode chain from 2-美学 through 8-分组 with 3-主体 registry preflight, per-stage subagents, and isolated stage handoffs."
...
`sword10` 是 AIGC 影视项目的多阶段 subagent 编排技能。它面向 `projects/aigc/<项目名>/` 下已完成 `1-分集` 的特定分集集合，依次调度 `2-美学 -> 3-主体 -> 4-编剧 -> 5-导演 -> 6-分镜 -> 7-摄影 -> 8-分组`。主窗口只负责派发、追踪、汇流和触发下一阶段；阶段正文、风格协议、主体注册表、设计稿和分组稿由后台隔离 subagent 按对应阶段技能完成。
...
- 针对 `projects/aigc/<项目名>/` 下指定分集，真实启用后台隔离 subagents，按 `2-美学 -> 3-主体 -> 4-编剧 -> 5-导演 -> 6-分镜 -> 7-摄影 -> 8-分组` 推进；`3-主体` 建立主体注册表和三域主体资产，后续阶段只读继承。
- 主窗口只做 preflight、dispatch packet、run ledger、stage merge、failure route 和 completion report，不生成或改写任何阶段 canonical 正文。
- `2-美学` 作为阶段级套件执行，按其父级合同 fan-out 到 6 个风格 subagents；其他逐集阶段默认一集一个后台隔离 subagent。

Non-goals:

- 不跨入 `9-图像` 或 `10-画布`；`3-主体` 已是本链前置主体注册表阶段。
...
| --- | --- | --- | --- |
| `business_goal` | 把 AIGC 2-8 主链变成可追踪、可续跑、可审计的 subagent 编排链，并在编剧前完成主体注册表 | 用户请求、阶段链、workflow ledger | `FAIL-SWORD10-BUSINESS-GOAL` |
...
| `success_criteria` | 目标分集到达 `8-分组` 且通过 gate，或 completion report 明确 partial/blocked 与 retry route | run ledger、completion report、stage verdict table | `FAIL-SWORD10-SUCCESS` |
| `complexity_source` | 复杂度来自跨 6 个阶段的串行汇流、`2-美学` 套件并发、逐集并发、失败续跑和额外输入校验 | Type Routing、Stage Chain、Node Map | `FAIL-SWORD10-COMPLEXITY` |
...
整体调用 `$aigc-workflow-sword10` 时，视为用户已授权本编排器在输入、安全门和 subagent runtime 成立后连续完成 `2-美学` 到 `8-分组`。阶段之间不再逐步询问“是否继续下一步”。

- 数字序号阶段按 `2-美学 -> 3-主体 -> 4-编剧 -> 5-导演 -> 6-分镜 -> 7-摄影 -> 8-分组` 执行；上一阶段所有目标分集或阶段级输出通过汇流门后，才启动下一阶段。`3-主体` 可在内部并发角色/场景/道具三域，但 `4-编剧` 必须等 `subject-registry.yaml` 可读后启动。
- 无序号同级子技能包默认全选并发执行；`2-美学` 内部 6 个风格子技能按其父级合同并发执行，父级汇流后进入 `3-主体`；`3-主体` 内部三域并发汇流后进入 `4-编剧`。
- 英文序号子技能包或路线默认按用户意图、父级路由或输入类型单选分流；本技能没有英文路线时，不自动并跑旁路路线。
- 逐集阶段内部按分集并发：一集一个后台隔离 subagent；同一阶段的 subagents 只写各自 `第N集.md` 和阶段允许的执行报告片段，不互相改写。`2-美学` 作为阶段级套件调用，按其父级合同 fan-out 到 6 个风格 subagents 并汇流为风格协议集合。
- 阶段 subagent 的输入固定为上一阶段 canonical 产物和 `stage-handoff-contract.md` 声明的额外输入；例如 `3-主体` 消费 `1-分集`、`2-美学/类型风格.md` 和三域风格协议，`5-导演` 消费 `4-编剧/第N集.md` 与 `2-美学/画面基调/全局风格协议.md`，`8-分组` 消费 `7-摄影/第N集.md`、`2-美学` 风格协议与 `3-主体/subject-registry.yaml`。
...
- 指定 AIGC 影视项目和一个或多个分集，要求从 `2-美学` 连续推进到 `8-分组`。
- 指向 `projects/aigc/<项目名>/1-分集/第N集.md` 的分集源文件，或已存在的中间阶段产物并要求从某个阶段续跑到 `8-分组`。
...
- `start_stage`: 默认 `2-美学`；续跑时允许为本链任一阶段：`3-主体`、`4-编剧`、`5-导演`、`6-分镜`、`7-摄影` 或 `8-分组`。
- `end_stage`: 默认且通常固定为 `8-分组`。
- `subagent_runtime`: 必须可真实启动后台隔离 subagents；否则阻断或降级报告，不执行主创正文。

Reject or clarify when:

- 项目根无法唯一定位，或分集选择会匹配到多个项目。
- 请求跨越 `9-图像` 或 `10-画布`，但仍要求使用 `sword10` 作为唯一编排器。
...
| mode | trigger | route |
| --- | --- | --- |
| `bounded_episode_chain` | 单集或少量明确集数从 `2-美学` 推到 `8-分组` | 加载 `types/bounded-episode-chain/bounded-episode-chain.md` |
| `episode_batch_chain` | 多集批处理，每阶段一集一个 subagent 并发 | 加载 `types/episode-batch-chain/episode-batch-chain.md` |
| `retry_from_stage` | 某阶段失败后从指定阶段续跑到 `8-分组` | 加载 `types/retry-from-stage/retry-from-stage.md` |
...
| --- | --- | --- | --- | --- | --- |
| `bounded_episode_chain` | 单集或 1-3 集，从 `2-美学` 或明确阶段推进到 `8-分组` | `Bounded Episode Path` | `N1,N2,N3,N4,N5,N6/N7` | `types/bounded-episode-chain/bounded-episode-chain.md`, `references/stage-handoff-contract.md`, `references/subagent-dispatch-contract.md`, `review/review-contract.md`, `templates/run-ledger-template.md`, `templates/stage-dispatch-packet-template.md`, `templates/output-template.md` | `FAIL-SWORD10-TYPE-BOUNDED` |
...
3. 运行 `N1-PREFLIGHT`：确认每集上游输入存在，确认 `2-美学` 到 `8-分组` 阶段技能可加载。
...
| `FIELD-SWORD10-03` | stage handoff | `references/stage-handoff-contract.md` | 2-8 输入输出链与额外输入 | `FAIL-SWORD10-HANDOFF` |
...
| `C2-STAGE-MERGE` | 当前阶段目标分集或 stage-suite 均有 canonical 输出与 stage verdict | 输出缺失、消费非 canonical 输入、`2-美学` 协议缺失 | stage verdict table、input/output path | `N4-STAGE-MERGE` / owning stage |
...
| `C4-COMPLETE` | `8-分组` 目标产物存在且通过 gate，或 partial/blocked 已有失败边界 | completion report 缺失败表、缺 retry route、复制阶段正文 | completion report、run ledger | `N6-COMPLETE` / `N7-FAILURE-ROUTE` |
...
| `CHK-SWORD10-SEMANTIC` | 定稿 2-8 阶段链、`2-美学` stage-suite 与 `3-主体` registry 语义、量化口径 | 确认 handoff、node map、review gate 都有返工入口 | Stage Chain、Node Map、Quant Table | `FAIL-SWORD10-CHECKPOINT-SEMANTIC` |
...
- Required output: workflow run ledger、每阶段 dispatch packet、completion report；阶段 subagents 另按阶段合同写入 `2-美学`、`3-主体`、`4-编剧`、`5-导演`、`6-分镜`、`7-摄影`、`8-分组` 的 canonical 产物。
...
- Completion gate: 所有目标分集在 `8-分组` 产物存在且通过对应阶段 gate；阶段正文、风格协议、分镜/摄影/分组正文均由 owning stage subagent 按阶段合同主创，未命中 `FAIL-SWORD10-SCRIPTED-STAGE-BODY`；若未全部通过，completion report 必须列出失败集、失败阶段、阻断原因和 retry/repair route。
