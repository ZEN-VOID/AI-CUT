...
4. 启用 subagents 时，向项目监制顾问问“这个角色的身份压力如何变成身体和服装”“纯色背景定妆照里最该保留的辨识点是什么”“哪些文化/职业 token 有证据风险”，不要只问风格评价。
...
9. 英文提示词最后生成，必须融合全局风格和服装风格，而不是把中文段落直译堆叠。
10. 批量执行时，每个角色先独立成稿，再做跨角色一致性审查，避免所有角色共享同一套形容词。
11. subagent 并行时，主 agent 只采纳可回指清单、项目风格和字段要求的内容。
12. 若发现清单字段错误，不在本技能直接修 `角色清单.md`；输出上游修复建议。
...
- 顾问请教的最佳产物不是“某大师会怎么想”，而是三到五条能改变角色轮廓、材质、姿态、摄影或 prompt token 的执行指令。
