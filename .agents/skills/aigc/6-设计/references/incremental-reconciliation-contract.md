# 6-设计 Incremental Reconciliation Contract

## Purpose

`6-设计` 必须支持上游 `5-分组` 分批完成的生产节奏。项目可以先完成若干集分组并执行设计，之后继续追加新的 `第N集.md`；后续执行不得把既有清单、设计稿或生成资产当作一次性全量覆盖对象。
...
```text
projects/aigc/<项目名>/6-设计/<域>/design-manifest.yaml
```
...
1. 锁定当前可读上游：`projects/aigc/<项目名>/5-分组/第N集.md`，或用户指定的集号子集。
...
| --- | --- | --- | --- | --- |
| 本轮是否先锁定了可读上游范围、用户指定集号子集和既有 6-设计输出范围，而不是默认全量重跑或凭记忆判断？ | `GATE-DESIGN-INC-01` source scope lock | `FAIL-DESIGN-INC-SOURCE-SCOPE` | `D-N3-RECONCILE`；本文件 `Reconciliation Pass` 第 1-2 条 | `source_scope`、`previous_scope`、已读取的 `5-分组/第N集.md` 列表和既有 `1-清单/2-设计/3-生成/design-manifest.yaml` 摘要 |
