# 5-设计 / 2-设计 Shared Output Contract

本文件是 `5-设计/{场景,角色,道具}` 的共享输出真源，负责统一三类设计主体的 prompt、图片快路径与派生产物边界。
...
| --- | --- | --- |
| `角色` | `.agents/skills/aigc/5-设计/角色/templates/character_masterprompt.structured.v2.md` | `validate_character_design_projection.py` 必须校验同结构；新增 renderer 时只能读取本模板填槽 |
| `道具` | `.agents/skills/aigc/5-设计/道具/templates/prop_masterprompt.structured.v2.md` | `build_prop_design_packets.py` 必须读取本模板填槽 |
| `场景` | `.agents/skills/aigc/5-设计/场景/templates/scene_masterprompt.structured.v2.md` | `build_scene_design_packets.py` 必须读取本模板填槽 |
...
```bash
python3 .agents/skills/aigc/5-设计/{场景,角色,道具}/scripts/run_design_auto_image.py \
  --design-file "projects/aigc/<项目名>/4-设计/<主体>/2-设计/第N集/<主体文件>.md"
```
...
```bash
python3 .agents/skills/aigc/5-设计/{场景,角色,道具}/scripts/ensure_design_auto_images.py \
  --project "<项目名>" \
  --domain "场景" \
  --episode "第1集"
```
