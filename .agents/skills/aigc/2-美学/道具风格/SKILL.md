...
`道具风格` 是 AIGC 影片项目的道具视觉风格协议技能。它从上游剧本、`画面基调`、参考图/视频或项目资料中提取道具层面的风格规则，研究文化语境、形制语言、表面/材料行为、制造逻辑、工艺痕迹、使用/维护痕迹、尺度/交互可供性、符号系统和制作边界，并蒸馏为可被后续道具清单、单件道具设计和图像生成阶段继承的道具风格提示词。
...
- 默认上游来源包括 `projects/aigc/<项目名>/1-分集/`、`projects/aigc/<项目名>/2-美学/画面基调/全局风格协议.md`、用户指定参考图/视频、项目设定或已有候选道具风格协议。
- 若输入来源或任务目标明确为 `第N集`，正式输出对象是该集道具风格覆盖，写入 `projects/aigc/<项目名>/2-美学/第N集/道具风格/`；只有整季、多集汇总或用户明确项目级基线时才写入非逐集 `道具风格/`。
...
- 根据参考图、参考视频或参考作品，分析道具的文化语境、形制逻辑、表面/材料行为、制造逻辑、工艺痕迹、使用/维护痕迹、尺度/交互可供性和符号系统。
...
| `success_criteria` | 输出包含来源清单、风格维度、文化语境、形制语言、表面/材料行为、制造逻辑、工艺痕迹、使用/维护痕迹、尺度/交互可供性、符号系统、制作边界、负面特征、约 100 字 prompt 和审计报告 | Output Contract、Review Gate Binding | `FAIL-PS-SUCCESS` |
...
- `projects/aigc/<项目名>/1-分集/第N集.md`、整季 `1-分集/` 目录、`2-编导` 目录或用户指定剧本文本。
- `projects/aigc/<项目名>/2-美学/画面基调/全局风格协议.md` 或用户提供的全局视觉风格描述。
- 项目初始化资料、世界观设定、题材说明、用户道具审美偏好、禁区说明。
- 参考图、参考视频、参考作品名称、文化/时代/工艺参考资料。
- 已有逐集 `projects/aigc/<项目名>/2-美学/第N集/道具风格/道具风格协议.md`、项目级 `projects/aigc/<项目名>/2-美学/道具风格/道具风格协议.md` 或候选 prompt。
...
| --- | --- | --- | --- | --- | --- |
| `script_prop_style_analysis` | 指定 `1-分集` 文件/目录或粘贴剧本文本 | `Script Prop Style Path` | `N1,N2,N4,N5,N6,N7,N8,N9` | `CONTEXT.md` | `FAIL-PS-TYPE-SCRIPT` |
...
| --- | --- | --- |
| `single_episode_seed` | 基于单集剧本建立道具风格覆盖 | `projects/aigc/<项目名>/2-美学/第N集/道具风格/道具风格协议.md` |
| `series_prop_style_protocol` | 基于多集、整季或项目资料建立正式项目级协议 | `projects/aigc/<项目名>/2-美学/道具风格/道具风格协议.md` |
...
| `N2-SCRIPT-EVIDENCE` | 从剧本/设定抽取道具风格证据 | `1-分集`、`2-编导` 或文本 | 提取与道具风格相关的文化、时代、使用环境、秩序、磨损、仪式、技术水平和生产方式信号；删除具体物件名称、持有人和剧情用途 | `script_prop_style_evidence_map`，至少 5 条证据 | `N4` / `N3` | 每条证据能回指输入片段或项目资料，且不形成道具清单 |
| `N3-TONE-REFERENCE-EVIDENCE` | 从画面基调和参考资料提取道具风格事实 | `画面基调`、图片、视频、作品说明 | 只提取媒介纪律、形制倾向、表面/材料行为、制造逻辑、工艺痕迹、使用/维护痕迹、尺度/交互可供性、符号系统和负面特征；剔除具体物件、品牌和构图 | `tone_reference_prop_evidence_map`，画面基调至少 3 条继承约束；每个参考至少 3 条风格事实 | `N4` | 不得复制参考中的具体道具或品牌符号 |
| `N4-PROP-STYLE-DIMENSIONS` | 汇流道具风格维度 | N2/N3 证据 | 建立 8 个解析维度：文化语境、形制语言、表面/材料行为、制造逻辑、工艺痕迹、使用/维护痕迹、尺度/交互可供性、符号系统；另列制作边界 | `prop_style_dimension_profile` | `N5` | 只保留风格层级；具体道具名和剧情用途必须删除 |
| `N5-CULTURE-CRAFT-MAPPING` | 形成文化、工艺和制作因果链 | `prop_style_dimension_profile` | 写 `Prop Style Slogan`、2-4 句设计原则、`[设定/画面基调/参考 -> 道具风格]` 映射；每个主要决策说明触发原因，并标明资源经济或生产方式如何影响制造逻辑 | `prop_style_slogan`、`design_principle`、`prop_style_causal_chain`，至少 5 条因果链 | `N6` | 每条道具风格决策可追溯，不能只写审美偏好 |
...
| `N7-PROMPT-DISTILL` | 蒸馏道具风格 prompt | N4-N6 输出 | 生成约 100 字中文 `prop_style_prompt`；包含文化语境、形制语言、表面/材料行为、制造逻辑、工艺/使用维护痕迹、尺度/交互可供性、符号系统和制作边界；过滤道具名、用途、单件设定和生成指令 | `candidate_prop_style_prompt`、`contamination_scan` | `N8` | 字数约 80-130 中文字；禁用类别残留 0 个 |
...
| 文化语境、形制、表面/材料行为、制造逻辑、工艺、使用/维护痕迹、尺度/交互可供性和符号系统是否保持风格层级？ | `GATE-PS-06-STYLE-LEVEL` | `FAIL-PS-STYLE-LEVEL` | `N4-PROP-STYLE-DIMENSIONS` | 8 维 profile 和降级记录 |
...
- 可保留的道具风格类别：文化语境、形制趋势、表面/材料行为、制造逻辑、工艺痕迹、使用/维护痕迹、尺度/交互可供性、符号系统、制作边界和负面特征。

## Output Contract

正式写回路径：

- `episode_scoped`: `projects/aigc/<项目名>/2-美学/第N集/道具风格/道具风格协议.md`
- `episode_scoped`: `projects/aigc/<项目名>/2-美学/第N集/道具风格/执行报告.md`
- `series_baseline`: `projects/aigc/<项目名>/2-美学/道具风格/道具风格协议.md`
- `series_baseline`: `projects/aigc/<项目名>/2-美学/道具风格/执行报告.md`
...
| `FIELD-PS-02` | prop style dimensions | `Prop Style Dimension Profile` | 8 个道具风格解析维度，含文化语境、形制、表面/材料、制造、工艺、使用/维护、尺度/交互和符号系统，且只含风格属性 | `FAIL-PS-DIMENSION` |
...
当 `2-美学` 父级调度多个同级风格子技能时，`道具风格` 默认继承 `画面基调` 的全局媒介和渲染纪律，并与 `角色风格`、`场景风格`、`摄影风格`、`分镜风格` 保持边界。它只提供道具视觉风格协议，不拥有具体道具清单、单件道具设定、剧情用途或图像生成真源。后续道具清单和单件设计阶段可继承 `Prop Style Prompt`，但必须在自身合同内追加具体物件事实和生成约束。
