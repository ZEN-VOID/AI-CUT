...
```bash
python3 .agents/skills/cli/libTV/scripts/query_session.py "<sessionId>" --project-id "<projectUuid>"
python3 .agents/skills/aigc/7-视频-backup/C-主体参照/scripts/detect-libtv-stall.py "<query-json>"
python3 .agents/skills/aigc/7-视频-backup/C-主体参照/scripts/validate-post-submit-reference-order.py "<package_dir>" "<query-json>"
```
...
| 平台审核拦截是否触发安全重提，而不是改写 `4-分组` 或把空画布视为完成？ | `G14-POST-SUBMIT` | `FAIL-VIDSUBJ-LIBTV-STALL` | `N9-QUEUE` / `N8-DISPATCH` | `platform_policy_rejected`、`moderation_repair`、安全版提交路径和结果 |
