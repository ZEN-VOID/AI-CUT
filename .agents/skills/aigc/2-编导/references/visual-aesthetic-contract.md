...
本细则定义 `2-编导` director layer 的画面美学层：在编剧保真、导演创作内核、高潮承托和受控增强边界成立之后，进一步组织画面的审美品质、景境层次和情绪承托。
...
| `心理反应` | 写内心状态，但必须消费 `psychological-reaction-contract.md` 的外部化锚点规则。不得写无表演支撑的纯内心描述。 |
| 声音字段与对应画面字段 | 声音只写可听本体；画面可用空间余波、静物震动、众人反应或自然声后的空白承托声音。 |

## 心理反应字段约束

本文件不再重复定义 `心理反应` 字段语义。单场美学只检查心理反应是否有可见、可听或可演的外部化锚点；完整准入、禁止写法、投影阶梯和验收映射统一回到 `psychological-reaction-contract.md`。

单场美学层只能补充外部化承托，例如视线、手部、停顿、空间距离、声音消失或道具状态；不得用美学判断替代表演任务，也不得把心理反应写成剧情解释。
...
- 把摄影、分镜或图像提示词提前写入 `2-编导` director layer。
- `心理反应` 字段没有外部化锚点。
