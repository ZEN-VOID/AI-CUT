...
- 初始化任务必须能锁定项目名；`0-初始化` 当前只创建 0-14 目录骨架、项目根 `MEMORY.md` 与项目根 `CONTEXT/`，不要求 `auto/custom`、north-star 或团队输入。
...
| ---------- | ---------------------------------------------------- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `12-图像` | `.agents/skills/aigc/12-图像/分镜故事板/SKILL.md` | 用户只说进入 `12-图像`、生成图像阶段、下一步生图，且未明确指定单镜分镜画面                        | 用户点名 `分镜画面`、四段式 `分镜ID`、单镜图、生图 prompt                                                |
| `13-画布` | `.agents/skills/aigc/13-画布/libTV画布流/SKILL.md`  | 用户只说进入 `13-画布`、生成视频阶段、下一步生视频、LibTV 画布、创建画布项目、上传主体参照图、按 `10-分组` 建视频节点 | 用户明确指定其他当前存在的视频叶子或要求多路线对比 |

该默认只负责根入口初始路由；进入 `12-图像` 或 `13-画布` 后，仍必须加载目标阶段和目标叶子的 `SKILL.md + CONTEXT.md`，并遵循叶子自身输入、输出和审查合同。
...
flowchart TD
    ...
    F --> F10A["10-分组"]
    F --> F6["11-主体"]
    F --> F7["12-图像 default: 分镜故事板"]
    F --> F8["13-画布 default: libTV画布流"]
    F --> F9["14-审片"]
...
| `8-摄影`   | `.agents/skills/aigc/8-摄影/`   | `projects/aigc/<项目名>/8-摄影/`   | active；默认消费 `7-分镜` 或用户指定分镜稿，并结合 `3-美学/画面基调` 与 `3-美学/摄影风格` 在每条分镜原文后内联注入综合运镜手法 |
| `9-光影`   | `.agents/skills/aigc/9-光影/`   | `projects/aigc/<项目名>/9-光影/`   | active；默认消费 `8-摄影` 或用户指定文稿，并结合 `3-美学/画面基调`、`3-美学/场景风格` 与 `3-美学/摄影风格` 在每条分镜原文后内联注入电影光影美学描述 |
| `10-分组`   | `.agents/skills/aigc/10-分组/`   | `projects/aigc/<项目名>/10-分组/`   | active                                                                       |
| `11-主体`   | `.agents/skills/aigc/11-主体/`   | `projects/aigc/<项目名>/11-主体/`   | active                                                                       |
| `12-图像`   | `.agents/skills/aigc/12-图像/`   | `projects/aigc/<项目名>/12-图像/`   | active；默认叶子 `分镜故事板`                                            |
| `13-画布`   | `.agents/skills/aigc/13-画布/`   | `projects/aigc/<项目名>/13-画布/`   | active；默认叶子 `libTV画布流` |
| `14-审片`  | `.agents/skills/aigc/14-审片/`  | `projects/aigc/<项目名>/14-审片/`  | active                                                                       |

旧 `2-编导`、`3-运动`、旧 `4-摄影`、旧 `5-分组`、旧 `3-导演`、旧 `4-表演`、`5-Image` 与旧 `6-Video` 只作为 legacy 自然语言兼容触发词或旧产物回读线索；显式 `2-编剧`、`4-导演`、`5-表演`、`6-氛围` 分别路由到当前同名阶段目录。

Supporting project roots may be created by later owning workflows as needed. `0-初始化` only creates the current 0-14 stage directories, `projects/aigc/<项目名>/MEMORY.md`, and `projects/aigc/<项目名>/CONTEXT/`.
