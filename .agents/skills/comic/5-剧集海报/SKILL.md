...
本技能是 `.agents/skills/comic/` 系列的第 5 段，也是最后一步。
...
进入 `5-剧集海报` 时，不得把海报当作独立创作物处理。必须优先回读同项目的上游输出物：
...
- `projects/comic/[项目名]/2-九刀流漫画提示词/page-group-*.json`
  ...
  - 仅当项目仍停留在 legacy 旧产物时，才回退读取 `nine_blade_comic_prompts.json` 或 `第N集-nine_blade_comic_prompts.json`。
...
- `output_path`
  - 默认：`projects/comic/[项目名]/5-剧集海报/第N集-剧集海报.json`
...
| `N11-HANDOFF` | 交付最终真源 | 已验证 JSON | 写入 stage-5 canonical output | 最终 JSON | 完成 | 只有一个 canonical JSON |
...
```text
projects/comic/[项目名]/5-剧集海报/第N集-剧集海报.json
```
...
```bash
python3 .agents/skills/comic/5-剧集海报/scripts/validate_episode_poster_json.py path/to/poster.json
```
