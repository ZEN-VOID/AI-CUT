...
| --- | --- |
| Required output | 单个普通 `group_id` 的 GPT-IMAGE-2 多图 prompt block |
...
| Completion gate | `shot_count == Image section count == plan.n`；默认 `aspect_ratio=16:9`；任务前缀禁止拼图；每个分镜点有独立 prompt；同组一致性约束存在 |
