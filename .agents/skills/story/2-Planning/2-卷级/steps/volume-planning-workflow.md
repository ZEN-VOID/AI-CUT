...
flowchart TD
    ...
    C -->|"audit_volume_plan"| F["N7 Review Gate"]
    D --> G["N4 六拍节奏"]
    E --> G
    G --> H["N5 资源与任务线"]
    H --> I["N6 卷末闭合"]
...
| --- | --- | --- | --- | --- |
| `N1-UPSTREAM-REREAD` | 是否已定位项目根、目标卷和 `整体规划.md` | 完整读取总纲，摘出目标卷职责、整书节奏位置、主任务从属 | `volume_duty`、`upstream_constraints` | 不缺上游真源 |
...
| `N4-SIX-BEAT-RHYTHM` | 本卷六拍是否支撑章节职责 | 生成本卷 promise、六拍职责、章节职责分配与 Mermaid 图 | `six_beat_map` | 不套用部级 15 步 |
| `N5-RESOURCE-MISSION` | 人物/场景/道具/任务线是否足够章级消费 | 写最小资源投影与 `上承 / 主线 / 支线 / 支流角色 / 下钻 / 汇聚` | `resource_mission_map` | 任务能回主线 |
| `N6-CLOSURE-AVOIDANCE` | 卷尾是否真的收束 | 写卷末达成与规避 | `closure_packet` | 卷尾完成度清楚 |
| `N7-REVIEW-GATE` | 输出是否可交付 | 按 `review/review-contract.md` 检查 headings、六拍、任务线、planning-only | `verdict` | pass 或带返工目标 |
...
| 节奏错用部级框架 | `N4-SIX-BEAT-RHYTHM` |
| 资源列表堆叠但不可执行 | `N5-RESOURCE-MISSION` |
| 卷末达成空泛 | `N6-CLOSURE-AVOIDANCE` |
| 质量门禁失败 | 对应 owner 节点 |
