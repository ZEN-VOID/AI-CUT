...
flowchart TD
    ...
    B --> C{"每行可回指 registry 场景条目?"}
    C -->|"否"| D["rework: N2-REGISTRY-SCAN"]
    C -->|"是"| E{"表头是否固定三列?"}
...
| --- | --- | --- | --- |
| `GATE-SCENE-LIST-01` | Source lock | 最终清单任一场景主体不能回指 registry `subjects.scenes` 条目 | `N2-REGISTRY-SCAN` |
| `GATE-SCENE-LIST-02` | Evidence boundary | source anchor、项目记忆或上下文被当作新增主体来源，而不是消歧证据 | `N3-EVIDENCE` |
| `GATE-SCENE-LIST-03` | Candidate record completeness | 候选记录缺少 registry ID、canonical name、source anchor 或必要消歧关键词 | `N2-REGISTRY-SCAN` / `N3-EVIDENCE` |
...
| --- | --- | --- |
| `FAIL-SCENE-SOURCE` | 场景主体来自 registry `subjects.scenes` 之外，或无法回指 source anchor | `N2-REGISTRY-SCAN` |
| `FAIL-SCENE-EVIDENCE` | 命名、首次登场或归并消歧缺少 source anchor/项目上下文证据 | `N3-EVIDENCE` |
...
| --- | --- | --- |
| `REV-SCENE-01` | 每个场景主体都能回指至少一个 registry `subjects.scenes` 条目 | blocking |
...
| `evidence` | yes | registry 来源、source anchor、表格行或缺失证据说明。 |
| `route` | yes | 返工目标节点：`N2-REGISTRY-SCAN`、`N3-EVIDENCE`、`N5-MERGE` 或 `N6-RENDER`。 |
