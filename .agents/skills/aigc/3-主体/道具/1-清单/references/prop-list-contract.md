...
- 唯一候选来源：`projects/aigc/<项目名>/3-主体/subject-registry.yaml` 的 `subjects.props` 条目。
- 证据回查范围：必要时只回查 registry `source_anchors` 指向的 `1-分集` 故事源与 `2-美学/道具风格` 协议，用于确认别名、代称、首次登场和是否背景杂物。
- 已有 `8-分组` 稿只作为后置命名对齐证据，不能新增道具主体。
- 禁止来源：角色目录、场景目录、后续设计稿、脚本猜测、项目外资料或 LLM 凭空补充。

## Canonical Output

- 路径：`projects/aigc/<项目名>/3-主体/道具/1-清单/道具清单.md`
- 形态：Markdown table
...
- 归并裁决必须由 LLM 结合 registry 字段与 source anchor 证据完成。
...
| --- | --- | --- | --- | --- |
| 每个最终道具主体是否都能回指 `subject-registry.yaml` 的 `subjects.props` 条目，而不是角色目录、场景目录、后续设计稿、脚本猜测、项目外资料或 LLM 凭空补充？ | `REV-PROP-01` | `FAIL-PROP-SOURCE` | `N2-REGISTRY-CANDIDATES` | `prop_candidates` 记录 `registry_id`、`canonical_name`、`source_anchor`；执行报告列出删除或阻断的 registry 外主体。 |
| source anchor 回查是否只服务别名、代称、首次登场和背景杂物判断，没有越源取证或把正文/项目资料变成新增候选来源？ | `REV-PROP-01` | `FAIL-PROP-EVIDENCE` | `N3-EVIDENCE` | `evidence_notes` 标明正文摘录所属 source anchor、用途和未新增候选的说明。 |
| canonical 输出是否写入 `projects/aigc/<项目名>/3-主体/道具/1-清单/道具清单.md`，且可选 `执行报告.md` / `design-manifest.yaml` 没有替代主体清单？ | `REV-PROP-07` | `FAIL-PROP-RENDER` | `N6-RENDER` | 输出路径检查、写入文件清单，以及报告/manifest 的 sidecar 边界说明。 |
...
| `原文描述（关键词式）` 是否只压缩 registry 原词、source anchor 可见词和动作关系词，没有扩写成道具造型、材质细节、三视图、生成提示词或设计说明？ | `REV-PROP-08` | `FAIL-PROP-DESIGN-OVERREACH` | `N6-RENDER` | 表格抽样、扩写删除记录和关键词来源说明；后续设计需求移交 `2-设计`。 |
| 归并、过滤、canonical 命名和重要性判断是否由 LLM 主判，脚本只做读取、registry 定位、路径枚举、表格格式检查或机械校验？ | `REV-PROP-06` | `FAIL-PROP-LLM-FIRST` | `N4-MERGE` / `N5-FILTER` | 执行报告记录 LLM 裁决摘要、脚本辅助范围；若使用脚本，列明其未生成 canonical 名称或过滤判断。 |
