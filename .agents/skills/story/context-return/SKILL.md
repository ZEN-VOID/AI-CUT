...
description: Use when a PASS-validated volume has been explicitly handed off to `context-return` for validated actualization into Cards, planning actualization sidecars, story_map actualization, project CONTEXT carryover notes, runtime projections, and the canonical context-return artifact.
governance_tier: full
---

# context-return

`context-return` is the validated actualization and project-context carryover checkpoint for the `story2026` workflow. It is a sibling skill to `review/`, not a numbered mainline writing stage. It only writes truth after `review` has produced a volume-level PASS aggregate and explicitly granted handoff to both `review/` and `context-return`.
...
- `handoff_targets` does not contain both `review/` and `context-return`
...
| mode | trigger | route |
| --- | --- | --- |
| `actualize_volume` | PASS aggregate explicitly grants `review/` and `context-return` handoff, and locks accepted manuscript refs | run `steps/context-return-workflow.md` |
...
flowchart TD
    ...
    B -->|"Yes"| C{"Handoff to review/ and context-return?"}
    ...
    E --> F["Compute staged patches"]
    F --> G["Write Cards current_state/history"]
    G --> H["Write planning actualization sidecars"]
...
| 初稿态被直接 actualize | accepted manuscript contract | `references/context-return-spec.md` + `types/type-map.md` |
