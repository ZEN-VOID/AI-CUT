...
def check_expression_dna(content: str) -> tuple[bool, str]:
    """检查表达DNA辨识度"""
    dna_section = bool(re.search(r'表达\s*DNA|Expression DNA|表达风格', content, re.IGNORECASE))
    if not dna_section:
...
def check_tensions(content: str) -> tuple[bool, str]:
    ...
    if section_match:
        items = re.findall(r'^[-*]\s+', section_match.group(1), re.MULTILINE)
        if items:
            passed = len(items) >= 2
            return passed, f"内在张力: {len(items)}对 {'✅' if passed else '❌ (应≥2对)'}"
