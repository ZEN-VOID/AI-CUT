...
description: Use when story2026 needs GPT-native chapter drafting from chapter-level planning truth into `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`.
...
`A-GPT原生` 是 `story2026` 主链 `3-Drafting` 阶段的 GPT 原生路径。它负责根据 `projects/story/<项目名>/2-Planning` 中已经规划好的章级资料，结合整书/卷级规划、全局卡、风格卡、`north_star.yaml`、项目记忆、项目上下文与上一章承接，由当前 GPT/LLM 会话直接创作为中文小说章节。
...
2. 当前 GPT/LLM 会话负责实际生成完整章节 Markdown 文件。
3. `scripts/write_chapter_gpt_native.py` 只做 context pack 输出、草稿校验、sidecar 记录与 canonical writeback。
...
- YAML 头必须显式包含 `写作模型`、`global_context`、`style_context` 与 `north_star_chapter_brief`；`写作模型` 固定为 `GPT`。
- 正文主体必须是小说 prose，不得把 planning 中的标题、任务线或规避条目原样复制成正文段落。
- 输出路径固定为 `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`。
- GPT 原生输出缺完整 YAML frontmatter、必需字段、`写作模型: GPT` 或 `# 第N章｜章标题` 标题行时，禁止写回业务真源。
- GPT-native artifacts 可落到项目 `reports/3-Drafting/gpt-native/.../`，但它们不是业务真源。
...
| `FIELD-GPTDRAFT-04` | `types/` | 类型策略层 | drafting mode、type variables、route matrix | `FAIL-GPTDRAFT-TYPES` |
| `FIELD-GPTDRAFT-05` | `review/` | 质量门禁层 | verdict model、finding shape、GPT-native evidence gate | `FAIL-GPTDRAFT-REVIEW` |
...
| `N3-CONTEXT-PACK` | planning/cards/north_star/MEMORY/CONTEXT/上一章 | 组装 GPT 原生创作上下文 | `context_pack` | `N4-DRAFT-BRANCH` |
| `N4-DRAFT-BRANCH` | `type_profile` 与 context pack | 选择新章/重写/续写/修复分支 | `branch_prompt` | `N6-GPT-NATIVE-DRAFT` |
| `N6-GPT-NATIVE-DRAFT` | branch prompt 与完整上下文 | 当前 GPT 原生生成完整章节 | `gpt_native_output` | `N7-VALIDATE-WRITEBACK` |
| `N7-VALIDATE-WRITEBACK` | GPT 原生输出 | 校验并写回 canonical path | `chapter_file` | done |
...
| `FAIL-GPTDRAFT-TYPES` | 起草、续写、重写、修复误判 | `types/drafting-type-map.md` |
...
```bash
python3 .agents/skills/story/3-Drafting/A-GPT原生/scripts/write_chapter_gpt_native.py \
...
| Output path | 业务真源固定写入 `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`；GPT-native artifacts 可写入 `projects/story/<项目名>/reports/3-Drafting/gpt-native/.../`。 |
| Naming convention | 卷目录使用 `第N卷`，章节文件使用 `第N章.md`；不得降格为平铺旧路径、`正文/` 或临时 sibling 文件。 |
| Completion gate | 当前 GPT/LLM 会话完成正文主创；输出通过 frontmatter、必需字段、标题行与正文完整度校验；正式正文已写回 canonical path；必要 sidecar 能追溯 context pack、GPT-authored draft 与 writeback。 |
