...
| `planning` 顾问团未成为初始化 kickoff owner | planning interview topology | 明确 `roles.planning.members` 必须先执行 interview，再允许综合 `north_star` | 在 `0-Init/SKILL.md` 固化 `team -> planning interview -> synthesis` 顺序 | 初始化不会再先写 handoff 再补 interview 说明 |
...
- 初始化阶段最多只产出 legacy 兼容骨架，不产出规划真源；正式规划入口必须留给 `2-Planning` 收敛出的 `Planning/全息地图.json`。
...
- 如果一组创意资料会被父技能与 team interview 共享消费，就不该让多个入口散点点名 leaf docs，而应先升格成 `references/<module-name>/module-spec.md + CONTEXT.md` 统一路由。
...
- 单模式初始化最稳的落地顺序是：先锁 `team_lineup_mode`，再锁 `team.yaml`，随后先做 `planning interview`，最后综合 `project_contract / cards_seed / planning_seed / unknowns`。
...
- team 代入模式的价值不在“顾问更像角色扮演”，而在把稳定立场转成 `planning interview` 可吸收的 patch；因此必须保留来源分层，而不是只给一段混合结论。
