...
description: Use when the 4-Design stage needs one prop-domain Skill 2.0 package that runs prop list, prop design, and prop panel prompt generation in order, while writing final prop artifacts directly under `projects/aigc/<项目名>/4-Design/`.
...
- 本技能是 `.agents/skills/aigc/4-Design/` 下的道具域子技能包；清单、设计、面板不再作为独立 skill 入口展开。
...
| --- | --- | --- |
| `PROP-PASS` | 清单、设计文档、面板 JSON 都在 4-Design 根目录且同 stem 对齐 | done |
