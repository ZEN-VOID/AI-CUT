...
从 `projects/aigc/<项目名>/5-分组/` 提取镜级分镜，生成四段式 `分镜ID` 的英文生图 prompt，绑定 `6-设计/*/3-生成` 中的主体图片，并按需调用 `.agents/skills/cli/imagegen` 批量生成分镜画面。
...
```text
使用 $aigc-image-storyboard-frame，为 projects/aigc/诡校-测试版/5-分组/第1集.md 生成第1集全部分镜画面 prompt，并绑定已有角色、场景、道具参照图。
```
