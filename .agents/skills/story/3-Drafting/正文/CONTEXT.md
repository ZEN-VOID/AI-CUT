...
| --- | --- | --- | --- | --- |
| 明明是按章直写，却仍回写到平铺 `3-Drafting/第N章.md` | runtime path contract | 改回 `projects/story/<项目名>/3-Drafting/第N卷/第N章.md` | 在 `正文/SKILL.md`、父层 `3-Drafting/SKILL.md` 与 registry 路由同时固定 chapter-native 输出路径 | 正文不会再漂到平铺旧路径 |
...
| 项目有 `CONTEXT/`，但正文 skill 完全没加载 | project context loading | 补读相关 `CONTEXT` 文件并记录到 `project_context_refs` | 在本地合同中固定“存在则按相关性加载，不存在才留空” | frontmatter 中 `project_context_refs` 与真实上下文一致 |
...
| `正文/` 看起来命中正确，但实际创作仍由本地 GPT 会话直接写出 | provider route drift | 改成通过 `write_chapter_via_doubao.py` 调用 AnyFast 豆包，再写回业务根稿 | 在 child 合同里写死“actual creative step = Doubao provider”，并为脚本增加 dry-run / 校验 / writeback 路径 | provider 报告、messages pack 与写回文件能互相对上 |
...
- 对当前 child 来说，“真正命中豆包”不是口头说明，而是能落出 messages pack、provider report、raw model output 和最终 `第N卷/第N章.md` 的同轮证据链。
