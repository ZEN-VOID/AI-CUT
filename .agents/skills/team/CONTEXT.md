...
| TM-TEAM-10 | 自动组队每次都全树扫描 team 子技能，选人慢且容易漏掉新增成员 | 根层索引缺失 | 先在根 `SKILL.md` 建立 `成员 -> 适配场景` 快路径索引，再让上游只深读 shortlist | 把 “先读根索引、后读候选子技能” 固定为 team 树的自动选人合同 | `0-Init` 等上游能明确说明 shortlist 来自根层索引而不是全树盲扫 |
...
7. 若问题出在自动配队选人效率或漏选，先检查根 `SKILL.md` 的成员索引是否已与当前 team 树同步，而不是直接改 `0-Init` 的挑选文案。
...
- 当 team 树成员发生变化时，先同步根层成员索引，再让 `0-Init` 之类的上游消费；这比在上游重复维护名单更稳。
