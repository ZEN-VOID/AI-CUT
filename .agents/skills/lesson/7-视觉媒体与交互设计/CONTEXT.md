...
| HTML artifact 生成停留在第 7 阶段 | html executor route drift | 删除 `.html`/站点落地动作，补 `8/html -> .agents/skills/claude-design` handoff | 第 7 阶段只写设计约束；真实 HTML 实现由第 8/html 叶子调度 `claude-design` | `downstream-handoff.md` 含 executor route 且无 `.html` 写回 |
...
2. 如果用户在第 7 阶段要求真实 HTML、`index.html`、静态站点或现有 HTML 改造，只写 HTML 设计约束和 `8/html -> .agents/skills/claude-design` 调用说明，不在第 7 阶段落地文件。
3. 正式写回前检查 3-6 阶段四类输入；缺目标、结构、正文或活动测评时，不要凭想象定稿。
4. 视觉系统先服务学习体验，再谈色彩和风格；避免只做装饰。
5. 媒体资产 brief 必须区分已提供、需制作、需采购、可生成、待授权和不可用素材。
6. 图解规划优先覆盖复杂概念、流程、对比、数据证据、案例拆解和学习路径，不为每页强行配图。
7. 交互模型要写清动作、反馈、状态和失败路径；否则第 8 阶段只能做静态页面。
8. 无障碍要求要可执行；如果法规或组织标准不明，采用保守要求并列待确认项。
9. 若用户明确说“以后都按这个品牌/视觉禁区/无障碍标准”，同步更新项目 `MEMORY.md`；一次性素材和设计结论写第 7 阶段输出。
...
- 第 7 阶段给 HTML 的产物是设计 brief 和执行边界；高保真 HTML artifact 的视觉实现、交互 polish 和浏览器验证交给 `.agents/skills/claude-design`，但必须经由 `8-多端交付生成/html` 保持课程真源和 manifest 边界。
