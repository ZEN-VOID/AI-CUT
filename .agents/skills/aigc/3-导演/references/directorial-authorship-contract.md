...
├── consumed_by: N5-DIR-ADVISOR (顾问提问)
...
| `N5-DIR-ADVISOR` | 使用 `dramatic_question` 和 `audience_position` 向顾问提出节点级创作判断问题 |
...
`lived_in_behavior_seed` 是 `performance_engine` 的上游动机种子。它不要求导演阶段写满演员微表情，但必须让下游 `4-表演` 知道角色不是“闲着等待表演”，而是被当前生活压力、目标或阻碍推着走。
...
- 这个 beat 结束时，人物状态应该落在哪里，方便 `4-表演` 和 `5-摄影` 承接。
