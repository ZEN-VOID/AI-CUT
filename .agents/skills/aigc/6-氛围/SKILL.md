...
- 必须读取可用的 `3-美学` 产物，优先级为 `画面基调`、当前集 `角色风格`、当前集 `场景风格`。`画面基调` 始终读取全局 singleton；若能从 source 推断 `第N集`，角色/场景风格必须先读 `3-美学/第N集/<风格>/...`，缺失时回退项目级 `3-美学/<风格>/...`。若任一仍缺失，可使用用户提供的等价风格文本，但必须在报告记录降级来源和 N/A 理由。
...
- 对动作强触发点先建立 `destruction_type_route`：显式挂钩用户类型信号、`3-美学`、`5-表演` 场景/动作/材质证据和项目 `MEMORY.md`，裁决类型分支或回退到 `generic_material_response`。再按路由加载 `types/action-destruction-fx/<type>.md` 并建立 `action_destruction_fx_profile`：把动作来源、受击材质、破坏材料、节奏强度和风格边界绑定起来；类型模块提供各自的材质库和专属边界，但不得替代本 `SKILL.md` 的触发、门禁或输出合同。
...
- 动作破坏点不得写成当前类型、上游 source、`3-美学` 或项目 `MEMORY.md` 不支持的力量来源、法术规则、科技光效、UI 参数或无源爆炸；玄幻/奇幻/科幻等已授权类型可以使用灵光、法阵、能量、护盾、粒子或科技光效，但仍必须落到现场材质、力量来源、碰撞路径、材料响应和节奏余波上。
...
- `3-美学/画面基调/全局风格协议.md`、当前集优先的 `3-美学/第N集/角色风格/角色风格协议.md` 与 `3-美学/第N集/场景风格/场景风格协议.md`；缺失时分别回退 `3-美学/角色风格/角色风格协议.md`、`3-美学/场景风格/场景风格协议.md`。
