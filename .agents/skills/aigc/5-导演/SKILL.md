...
`5-导演` 负责在 `4-编剧` 逐集剧本基础上做导演执导批注。它默认消费 `projects/aigc/<项目名>/4-编剧/第N集.md`，或用户显式指定的剧本文件；同时读取 `2-美学` 产物，优先是 `projects/aigc/<项目名>/2-美学/画面基调/全局风格协议.md`，把其中的画面基调、大师参照、作品锚点和全局风格约束作为导演理解上下文。

导演批注的首要下游用途是和原剧本一起传递给后续表演技能包与演员阅读：演员读到批注后，应能把剧本中的心理、关系、信息差和导演意图转成具体、显式、可画面化的表演方式，例如视线落点、呼吸变化、停顿时长、手部动作、身体距离、重心变化、声线收放、话前话后反应和与道具/空间的接触。这里的“可画面化”默认按文学白描式直写处理，不用比喻、象征或概念标签承担主信息。批注不是影评，也不是摄影说明；它必须帮助演员知道“这一画面该怎么演出来”。
...
批注应紧贴被批注的画面性字段或画面性句子之后，覆盖 `画面`、`动作画面`、`对白画面`、`音效画面`、`旁白画面`、`系统画面`、`心理反应`、`表演提示`、`环境描写`、`角色动作`、`场面调度`、`群像画面`、`表情特写`、`道具特写`、`独白画面`、`内心独白画面`、`规则显影`、`现实灾难画面`、`转场` 等画面点。批注只解释导演会如何理解、调度、拍摄和表演这个画面点；不得改写剧情事实、对白正文、场景顺序、字段标题或 `2-美学` 的全局风格真源。
...
- 当 `4-编剧`、`2-美学`、导演风格来源、项目 `MEMORY.md/CONTEXT/` 共同影响导演批注方向时，执行报告必须追加 `Director Direction Inheritance Matrix`，逐项说明每类上游上下文的 `direction_role`、`used_as`、`stage_decision`、`stage_landing`、`boundary_check` 和 `evidence_map`；只写“已读取剧本/风格”不得 pass。
...
- 任意涉及表演交接、心理外显、画面化批注或抽象/比喻转写的任务，必须加载 `../_shared/anti-abstract-language-contract.md`，并按白描式主体、动作、空间、光声、道具和时间关系审查批注。
...
- 用户要求“5-导演”“导演批注”“导演执导批注”“在剧本里加导演批注”“以内联方式注入导演意见”“代入某导演风格批注剧本”。
- 输入是 `4-编剧/第N集.md`、用户指定剧本、粘贴剧本文本，且需要结合 `2-美学` 画面基调或大师参照进行导演理解。

Core task:

- 先充分理解题材、情节、人物关系、剧本正文、画面点分布和 `2-美学` 画面基调，并建立整集导演意图规划。
...
- 对每个命中画面点生成 1 条紧贴原文的 `（导演批注：XXX）`，说明作为该导演会如何理解这个画面、如何安排观看重点、表演压力、节奏、空间、光影或声音关系，并把其中的心理/关系/信息差转译为演员可执行的白描式显式表演动作。
- 保持原剧本文字、字段标题、场景顺序和对白不变，只新增导演批注行或批注段。

Non-goals:

- 不生成摄影分镜时间段、镜头清单、图像 prompt、视频任务、完整演员表演稿或设计稿；但每条批注必须给后续表演技能包足够的可执行表演种子。
- 不改写 `4-编剧` 剧本 canonical truth，不改写 `2-美学` 风格协议。
...
- 不得把 `2-美学` 的大师锚点照搬成每条批注口号；每条批注必须回到当前画面点。
- 不得把“像被命运压住”“宿命感很强”“灵魂碎裂”等明喻、隐喻、象征或概念标签当作表演交接材料；删除这些词后，批注仍必须能落到演员身体、空间、道具、光声或时间变化。
...
| `constraint_profile` | 原剧本保真、内联注入、画面点全覆盖、导演风格有证据、`2-美学` 只作上下文 | 用户限制、本 SKILL 禁止项、上游合同 | `FAIL-DIR-CONSTRAINT` |
| `success_criteria` | 输出保留原剧本并在每个命中画面点后新增合格批注；批注能被表演技能包直接消费为视线、呼吸、停顿、手部、距离、重心、声线等白描式可见/可听表演任务；执行报告含来源、风格画像、覆盖统计和审查结果 | `annotated_episode`、`performance_handoff_map`、`execution_report` | `FAIL-DIR-SUCCESS` |
...
| `topology_fit` | 先取源、再建立整集导演意图规划、再锁导演风格、再逐点注入、再 review：1) 防止未理解剧情就套导演口头禅；2) 防止注入破坏原剧本；3) 防止 `2-美学` 被反向改写；4) 让每条批注都有整集主轴和画面点归属 | Visual Maps、节点表、覆盖报告 | `FAIL-DIR-TOPOLOGY-FIT` |

## Input Contract

Accepted input:

- 项目名、项目路径、单个或多个 `projects/aigc/<项目名>/4-编剧/第N集.md`。
- 用户指定剧本文件、粘贴剧本文本、已有导演批注稿或候选稿。
- 用户指定导演、导演作品、导演风格、参考影片、禁用表达、注入密度或批注长度。
- `2-美学/画面基调/全局风格协议.md`、其他 `2-美学` 子产物、项目 `MEMORY.md` 和项目 `CONTEXT/`。
...
| --- | --- | --- |
| `single_episode_annotation` | 指定单个 `第N集.md`、单个集号或单集剧本文本 | `projects/aigc/<项目名>/5-导演/` 下的单集导演批注稿 |
...
| `N4-DIR-POINT-MAP` | 建立画面点清单和注入计划 | 剧本正文、`4-编剧` 字段体系、N2 规划证据 | 识别全部画面性字段/句子；每个注入点记录字段名、原文摘要、相邻声音/动作语境、批注意图、整集主轴归属、信息差状态、场景节奏角色、表演/声画取舍、演员可执行表演种子、白描式画面化交接目标和保真风险；不得把纯对白正文单独作为画面点，除非有 `对白画面` 或相邻可见承托 | `visual_point_inventory`、`injection_plan`、`episode_spine_binding_map`、`asymmetry_rhythm_binding_map`、`performance_handoff_plan`、`plain_visualization_plan` | `N5` / `R1` | 命中画面点覆盖率必须为 100%；关键心理、对白、表演和动作画面点必须有至少 1 个表演外化种子；删除比喻/概念词后仍须可交接；无法处理点列入 blocked/followup |
| `N5-DIR-INJECT` | LLM 逐点生成内联导演批注 | N2-N4 证据 | 在原剧本每个命中画面点后新增 `（导演批注：...）`；批注体现导演对该画面点的戏剧问题、人物压力、观众信息位置、节奏取舍、高点兑现/反高潮策略、表演/空间/声音重点；关键批注必须把心理和关系外化为视线、呼吸、停顿、手部、身体距离、重心、声线、话前/话后反应或道具/空间接触等演员可执行动作；抽象、比喻和概念判断必须转成白描式可演材料；保留原文和字段顺序 | `candidate_annotated_episode`、`annotation_binding_map`、`reference_application_map`、`performance_handoff_map`、`plain_visualization_audit` | `N6` / `R1` | 每条批注必须绑定一个画面点、至少一个整集或场景级导演证据；关键心理/对白/表演画面批注必须能被演员直接执行；格式完全匹配固定样式 |
| `N6-DIR-REVIEW-REPAIR` | 审查并最小修复候选稿 | candidate、review contract | 执行 `GATE-DIR-01..21`；阻断项回到 N2-N5 或 R2 最小修复，最多 3 轮；无法修复时进入阻断收束 | `review_verdict`、`repair_log`、`coverage_stats` | `N7` / `R1` / `N8` | review 未通过不得写回 canonical |
| `N7-DIR-WRITEBACK-CLOSE` | 写回唯一输出并生成报告 | passed candidate、output contract | 写入 `projects/aigc/<项目名>/5-导演/` 下的单集文件与 `执行报告.md`；指定剧本非项目 source 时按用户指定输出或只返回候选；报告记录来源、覆盖、风格证据、网络来源、N/A 与修复 | `output_manifest`、`execution_report` | done | 输出路径唯一；报告证据完整 |
...
```mermaid
flowchart TD
    A["4-编剧/第N集.md or user script"] --> B["N1-DIR-INTAKE"]
    C["2-美学/画面基调/全局风格协议.md"] --> B
...
| `pass_threshold` | `GATE-DIR-01..21` 阻断项为 0；比喻、象征、概念标签或抽象解释替代表演交接材料 0；非阻断 followup 不超过 3 项且不得影响保真、覆盖、格式、导演风格证据、白描式表演交接、作者性完整性、上游上下文应用、导演方向继承或输出路径 | `N6.gate` / `Convergence Contract` | `FAIL-DIR-QUANT-THRESHOLD` |
...
| `fallback_evidence` | 若 `2-美学` 缺失，使用用户指定风格资料并标记降级；若知识库缺导演资料，记录 `pretrained_style_inference` 或网络来源；若某画面点语义不可判定，保守不注入并在报告列 blocked/followup | `Review Gate Binding.report_evidence` | `FAIL-DIR-QUANT-FALLBACK` |
...
- 卫星技能只承担查询、恢复、审查承接或辅助动作，不因连续调度自动改写 `5-导演` canonical 输出。
...
| `../_shared/anti-abstract-language-contract.md` | 表演交接、心理外显、可画面化批注、抽象情绪、比喻/象征/概念标签转写、`FAIL-DIR-PERFORMANCE-HANDOFF` | 跨阶段反抽象合同，定义白描式画面化和可拍/可演材料审查 | 替代导演风格判断、整集意图规划、批注主创或原剧本保真规则 | `N4/N5/N6` |
| `../_shared/upstream-context-application-contract.md` | 任意正式生成、repair、review，或 `FAIL-DIR-UPSTREAM-CONTEXT` | 规定剧本与美学上下文如何被导演批注应用、保真和举证，要求 `Upstream Context Application Map` | 替代导演批注主创、改写剧本/美学真源、把风格锚点机械套进每条批注 | `N1-DIR-INTAKE` / `N2-DIR-UNDERSTAND` / `N7-DIR-WRITEBACK-CLOSE` |
...
| `upstream_context_application` / `FAIL-DIR-UPSTREAM-CONTEXT` | `../_shared/upstream-context-application-contract.md`, `review/review-contract.md` | `N1 -> N7` | `GATE-DIR-20-UPSTREAM-CONTEXT` | `Upstream Context Application Map` links script/style anchors to annotation decisions |
| `upstream_direction_matrix` / `FAIL-DIR-UPSTREAM-DIRECTION-MATRIX` | `../_shared/upstream-context-application-contract.md`, `review/review-contract.md`, `templates/output-template.md` | `N2 -> N7` | `GATE-DIR-21-UPSTREAM-DIRECTION` | `Director Direction Inheritance Matrix` maps script/style/memory/director source to director intent, annotation decisions and boundaries |
| `single_episode_annotation` / `episode_range_annotation` | `../_shared/anti-abstract-language-contract.md`, `references/episode-visual-spine-contract.md`, `references/directorial-authorship-contract.md`, `references/information-asymmetry-contract.md`, `references/scene-rhythm-contract.md`, `references/anticlimax-strategy-contract.md`, `references/director-annotation-contract.md`, `templates/output-template.md`, `review/review-contract.md` | `N2 -> N7` | `C4-REVIEW-PASS` | 整集导演意图、注入点覆盖、白描式表演交接、批注格式和报告结构 |
...
| `FAIL-DIR-PERFORMANCE-HANDOFF` | `../_shared/anti-abstract-language-contract.md`, `references/directorial-authorship-contract.md`, `references/director-annotation-contract.md`, `review/review-contract.md` | `N4-DIR-POINT-MAP -> N5-DIR-INJECT` | `C4-REVIEW-PASS` | performance handoff audit + `plain_visualization_audit` |
...
| `C2A-UPSTREAM-CONTEXT-APPLIED` | 剧本画面点和画面基调已投影为整集导演意图、逐点批注和表演交接约束 | 批注只复述导演名/美学口号，或无法回指具体剧本字段、画面点和保真检查 | `upstream_context_application_map` | `N1-DIR-INTAKE` / `N2-DIR-UNDERSTAND` / `N5-DIR-INJECT` |
...
| `C4-REVIEW-PASS` | `GATE-DIR-01..21` 阻断项为 0，修复日志收束 | 格式、覆盖、保真、风格、上游上下文应用、导演方向继承、表演交接、作者性完整性、报告任一阻断失败 | `review_verdict`、`repair_log`、`authorship_integrity_audit`、`upstream_context_application_map`、`director_direction_inheritance_matrix` | `N6-DIR-REVIEW-REPAIR` |
...
| 批注是否能被表演技能包和演员直接消费为具体、显式、白描式可画面化的表演方式，且不靠比喻、象征或概念标签承托主信息？ | `GATE-DIR-18-PERFORMANCE-HANDOFF` | `FAIL-DIR-PERFORMANCE-HANDOFF` | `N4-DIR-POINT-MAP` / `N5-DIR-INJECT` | `performance_handoff_map`、`plain_visualization_audit`、表演动作抽样 |
...
| `4-编剧` 与 `2-美学` 上下文是否明确投影为导演批注决策，并记录 source anchor、local decision 和 preservation check，而非只写“已参考剧本/风格”？ | `GATE-DIR-20-UPSTREAM-CONTEXT` | `FAIL-DIR-UPSTREAM-CONTEXT` | `N1-DIR-INTAKE` / `N2-DIR-UNDERSTAND` / `N5-DIR-INJECT` | `upstream_context_application_map` |
| `4-编剧`、`2-美学`、导演风格来源与项目记忆是否分别说明如何引导本集导演方向，且落到导演意图、信息差、节奏、表演交接或批注取舍？ | `GATE-DIR-21-UPSTREAM-DIRECTION` | `FAIL-DIR-UPSTREAM-DIRECTION-MATRIX` | `N2-DIR-UNDERSTAND` / `N3-DIR-STYLE` / `N5-DIR-INJECT` / `N7-DIR-WRITEBACK-CLOSE` | `director_direction_inheritance_matrix` |
...
| `ATTE-DIR-03` | 注意力漂移检测 | 出现改对白、改剧情、泛泛复述导演名、只写摄影参数、批注脱离画面点、比喻/象征替代表演交接、复制 `2-美学` 口号、输出路径漂移即漂移 | `Review Gate Binding` |
...
- 保留原 `4-编剧` 剧本结构、frontmatter、场景标题、字段标题、对白和顺序。
- 在每个命中画面点后新增固定格式批注：`（导演批注：XXX）`。
- 追加或生成结构化执行报告，包含 `Source Manifest`、`Visual Tone Context Map`、`Upstream Context Application Map`、`Director Direction Inheritance Matrix`、`Director Style Source Matrix`、`Visual Point Coverage`、`Annotation Binding Map`、`Review Verdict`、`Repair Log`、`N/A Justification`。
...
- 正式写回时 `GATE-DIR-01..21` 阻断项为 0。
- `FAIL-DIR-SCRIPTED-PROJECTION` 必须为 0；若候选批注呈现脚本化生成、批量插入、正则套句、映射投影、句式复用或锚点替换伪差异，候选稿不得表层润色通过，必须废弃并回到 `N2/N5` 重新由 LLM 逐画面主创。
- `FAIL-DIR-PERFORMANCE-HANDOFF` 必须检查白描式画面化：若批注靠比喻、象征、概念标签或抽象解释承托主信息，必须回到 `N4/N5` 重写为可演动作、空间、光声、道具或时间关系。
...
8. 批注无法被演员转成具体表演动作，或用比喻/概念替代表演材料：回 `N4-DIR-POINT-MAP` 建白描式表演种子，再回 `N5-DIR-INJECT` 改写批注。
...
| `FIELD-DIR-10` | 输出路径 | `5-导演/第N集.md` 与 `执行报告.md` 可复查 | `FAIL-DIR-10` |
...
| `FIELD-DIR-12` | Performance handoff | 关键批注能把心理、关系、信息差和导演意图外化为白描式视线、呼吸、停顿、手部、距离、重心、声线、话前/话后反应或道具/空间接触；删除比喻/概念词后仍可交接 | `FAIL-DIR-12` |
...
- `2-美学` 只提供风格上下文，不允许被本技能更新、覆盖或重命名。
- 目标 skill 维护任务可写 `.agents/skills/aigc/5-导演/`；正式运行任务只写 `projects/aigc/<项目名>/5-导演/`。
