...
| `legacy_compat` | 明确点名 legacy `5-Image` 或旧产物 | 只做兼容回读或迁移说明 |
...
| current image stage | `6-图像/SKILL.md + CONTEXT.md` |
| current video stage | `7-视频/SKILL.md + CONTEXT.md` |
...
3. 选择唯一主入口；未命中的阶段、叶子或卫星技能不得补占位输出。
4. 阶段技能完成后，根入口只汇流下一入口、治理证据与失败回接，不改写阶段业务主稿。
5. 若遇到 legacy `5-Image`、`6-Video` 或 `7-Cut`，只允许兼容读取、迁移说明或明确的搁浅状态，不得把旧路径写成新 runtime。
...
- Completion gate: route 唯一，runtime 不漂移，legacy/shelved 状态明确，未命中单元不参与聚合。
