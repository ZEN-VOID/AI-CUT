...
主体参照流是本技能默认路线。它只负责把 `6-分组` 分镜组转成 LibTV CLI handoff plan；真实画布执行由 `.agents/skills/cli/libTV` 完成。
...
- `主体绑定表` 是参考绑定唯一真源。
- 最新版 CLI 的稳定引用方式是左侧输入连线 + `{{Image N}}`；`N` 必须来自查询验证后的左侧输入顺序。
- 若 `Image N`、`imageList`、UI 排列、节点框体顺序或缩略图顺序与绑定表不一致，以 `yaml_name + node_key + URL + image_index` 为准。
- `subject_bindings`、`left_input_edges[]`、`image_placeholder_map[]`、`source_node_keys`、`source_node_url_mapping`、`imageList/mixedList` 必须按 canonical order 的选中子集排列；不得按上传顺序、画布创建时间、本地文件扫描顺序或 `Portrait N` 排列。
- 执行层必须用 `--left` 或 `--left-add` 把主体图节点连到视频节点左侧；prompt 中只使用已登记的 `{{Image N}}` 占位，不使用不可验证的 UI `@` 文本。
- `video_node.params.prompt` 中必须保留原主体名，不得只写图片序号。
- 每个进入 LibTV 的参考图都必须在 handoff plan 中以真实 `image_index + {{Image N}} + URL + node_key + yaml_name + 用途` 出现，且和 `left_input_edges[] / libtv_images[] / mixedList` 一致。
...
- 未 opt-in 时，若查询发现实际节点 prompt 被压缩改写，或混入执行锁/生成参数/主体绑定表/诊断说明，应记为 `needs_rework / remote_prompt_rewritten_or_polluted`。
...
- `<分镜组ID>-queue-record.json`：组级提交状态，必须记录 `executor_skill`、`projectUuid`、`groupNodeKey`、`video_node_key`、下载状态和阻断原因。
