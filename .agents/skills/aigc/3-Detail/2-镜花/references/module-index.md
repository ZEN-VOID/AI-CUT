...
- 汇流顺序固定为：`光影 -> 色彩 -> 质感`。
- `光影` 产出后必须补 `group_lighting_note`，明确主光源 / 辅助光 / 逆光 / 照明类型 / 组内光影流动，再进入摄影总协调。
- 汇流原则：先锁视觉控制线与摄影执行策略，再确定照明和影调，再叠色彩策略，最后压入材料与表面质感，并收束成单一组级摄影判断。
...
8. 做 `1800` 字预算裁剪，再写回 canonical 文件。
