...
`4-分组` 负责把 `3-摄影` 的逐集摄影稿切分为可供后续设计、图像和视频阶段消费的完整分镜组。它不改写上游剧本正文和原有分镜明细，只裁决组边界、补入组间衔接用的入场/出场画面、拼接项目 `north_star.yaml` 的风格字段，最后在组底部附加统计 YAML。
...
4. 按 `references/group-boundary-contract.md` 执行边界裁决：每组同时满足对白 4-6 句弹性上限、完整组构成常规目标不超过约 1680 字、硬上限不超过 1980 字、同一画面句子及其多分镜不被截断，并通过过长拆分复核与短组回填复核；不得仅因情绪、话题或危险信息转折切出低密度短组。
...
7. 按 `references/statistics-yaml-contract.md` 在每组底部附加 YAML 统计；统计 YAML 本身不计入 1680 目标上限、1980 硬上限，也不计入 `字数统计`。
8. 写入 `projects/aigc/<项目名>/4-分组/第N集.md`，并生成或更新 `projects/aigc/<项目名>/4-分组/执行报告.md`。
9. 按 `review/review-contract.md` 执行验收；可运行 `scripts/validate_storyboard_groups.py` 做机械检查，但脚本不得替代 LLM 分组、补位画面和统计判断。
...
| `scripts/validate_storyboard_groups.py` | 必跑机械校验：检查分镜组标题、风格字段、入场/出场标题、YAML 统计、字数上限和编号连续性；不能替代分组边界、补位画面、原文保真和统计证据判断 |
...
| `FIELD-GROUP-03` | 边界裁决 | 对白 4-6 句、总字数常规目标 <= 1680、硬上限 <= 1980、画面句子及多分镜完整、过长组已拆分复核、短组已回填复核，不以情绪/话题/危险信息转折作为切组原则 | `FAIL-GROUP-05` |
| `FIELD-GROUP-04` | 分镜组 ID | `x-y-z` 与真实集、场、组匹配，场内组号连续 | `FAIL-GROUP-04` |
| `FIELD-GROUP-06` | 入场/出场画面 | 组间补位同画面、1-2 秒、通常非对白，每集首组省略入场画面段 | `FAIL-GROUP-07` |
...
- 完整组构成超过 1680 字却未做拆分复核，或超过 1980 字硬上限，或把 YAML 统计/补位画面漏算、错算。
...
- YAML 统计缺角色、场景、道具，或把统计块计入 1680 目标上限 / 1980 硬上限。
...
3. 每个分镜组必须以 `## x-y-z` 作为分镜组标题，并包含组头三项 north_star 风格纯内容、从 `3-摄影` 划定的分镜剧本正文、`出场画面：`、组底 YAML 统计；第 2 组起还必须包含 `入场画面：`。
4. 每集第 1 个分镜组直接省略 `入场画面：` 段，不写 `无`，不占用字数；后续组的 `入场画面：` 必须与上一组 `出场画面：` 为同一画面内容。
5. 分镜剧本正文必须同步原换行，不改写 `3-摄影` 的字段、对白、原有分镜明细或场景顺序。
6. 每组正文构成总字数常规目标不超过约 1680 字，硬上限不超过 1980 字；统计 YAML 不计入 1680 目标上限、1980 硬上限，也不计入 `字数统计`。
...
- 每组满足对白 4-6 句弹性上限、完整组构成常规目标 <= 1680 字、硬上限 <= 1980 字、画面句子及其多分镜不截断，并通过过长拆分复核与短组回填复核。
