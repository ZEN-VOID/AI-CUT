...
- 五段链完整交付时，最稳的节奏是：先把小说母稿和桥接包落盘，再把九刀流 JSON 落盘，再让生图报告引用 JSON 和图片，再由 4 号技能按页提炼 sora prompt 并生成页视频，最后由 5 号技能收束成单集海报设计 JSON。
...
- 当需要动画时，最稳的节奏是：先锁 2 号 group JSON 为 prompt 真源，再锁 3 号的 `page01..page09` 为首帧真源，4 号只做页级 prompt 提炼和 Sora 执行，不重写剧情。
