...
| `N2-TYPE-PROFILE` | 判定起草、续写、重写、修复或 dry-run | 目标章是否存在、用户意图、`types/drafting-type-map.md` | 生成 `type_profile`；目标章已存在时要求显式 mode，正式写回要求 `--force` | `type_profile` | `N3-CONTEXT-PACK` | mode 唯一且不冲突 |
