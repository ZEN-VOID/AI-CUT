...
本文件定义 step1 之后的组级视频 prompt 组装规则。目标是把 `4-分组` 的现有组正文保真转换为 Dreamina 可提交文本，而不是重新创作分镜。
...
```text
横屏真人实拍感视频，保持平行当代校园规则怪谈质感，低饱和冷白灯光，黑白红警告层与金色真相层分离。请根据以下完整分镜组内容生成一条连续视频，保留镜头顺序、入场画面、出场画面、角色动作、对白、音效、镜头语言与氛围约束。
...
- 不得承诺 Dreamina 不支持的参数或能力；模型、时长、分辨率以 `.agents/skills/cli/dreamina-cli` 当前约束为准。

## Reference Path Suffix Rule

- 当使用 `multimodal2video` 时，prompt 必须在对应主体信息后追加 `@<图片路径>`，例如 `角色：林寂 @projects/.../林寂-多视图.png`。
...
- 默认保留完整 `group_body`。
- 若 Dreamina prompt 长度或人工策略要求压缩，必须在 submit plan 中记录：
