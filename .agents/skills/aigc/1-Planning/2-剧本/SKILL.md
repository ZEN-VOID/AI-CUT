...
`2-剧本` 是 `1-Planning` 下负责“逐集原文真源 -> 规划阶段 canonical 主稿”的单技能真源。

本次口径固定为：

- 内容与机制继续全面继承现有 `2-剧本` 配置与 `AIGC-ZEN-VOID/.agents/skills/aigc2026/1-编剧/2-对白·独白·旁白` 的高价值规则
- 但所有原本散落在旧规划组文档中的格式判模、标准剧、解说剧与 team/gating 规则，统一内化回本 `SKILL.md`
- `2-剧本` 不再依赖外部 planning agent 文档才能完成主变体裁决、文本层规则选择、主稿落盘、validator 闭环与下游 handoff
...
| 载体 | 位置 | 作用 |
| --- | --- | --- |
| 剧本主稿 | `projects/<项目名>/1-Planning/2-剧本/第N集.md` | 规划阶段逐集 canonical 主稿 |
| 单集执行报告 | `projects/<项目名>/1-Planning/2-剧本/第N集-执行报告.md` | 记录变体裁决、结构重排、validator 与返工结论 |
| 变体决策证据 | `projects/<项目名>/1-Planning/2-剧本/agents-plan/第N集.<variant>.md` | 可选证据，不与主稿竞争 |
...
| 共享校验器 | `.agents/skills/aigc/1-Planning/2-剧本/scripts/validate_script_output.py` | 校验 `标准剧 / 解说剧` 主稿结构与高频硬门禁 |
...
3. 本阶段写回对象只有 `2-剧本/第N集.md` 与 `第N集-执行报告.md`。
...
| `FIELD-SCRIPT-06` | 主稿落盘 | 写出 `2-剧本/第N集.md` 与执行报告 | S6 | 输出完整性 | `FAIL-SCRIPT-06` |
...
- 已能从 `1-分集` 输出物生成 `2-剧本/第N集.md` 与 `第N集-执行报告.md`
