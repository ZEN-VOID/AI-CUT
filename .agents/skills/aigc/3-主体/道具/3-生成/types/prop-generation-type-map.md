...
| `libtv_canvas_mode` | `canvas_image_node` / `external_provider_explicit` | `.agents/skills/cli/libTV` 画布节点合同与用户显式 provider 选择 |
...
| `repair_scope` | `main_prompt` / `main_image` / `json_path` / `canvas_node_link` / `multiview_cancelled` | `review/review-contract.md` findings |

## Type Matrix

| type_id | trigger | route | required templates | review focus |
| --- | --- | --- | --- | --- |
| `TYPE-PROP-GEN-SINGLE` | 用户指定单个道具或单份设计文档 | `single_prop_generation` | `single-subject-prompt.json` | 单主体忠实度、主图证据链 |
| `TYPE-PROP-GEN-BATCH` | 用户指定项目或全部 `2-设计` 文档 | `batch_from_designs` | 同上，每个主体独立实例化 | 不合并主体、命名不冲突 |
| `TYPE-PROP-GEN-PROMPT-ONLY` | 用户要求只输出 JSON / dry-run | `prompt_only` | 同上 | 主图 JSON 可复跑，预期路径完整 |
...
| --- | --- | --- |
| `single_prop_generation` | `N1 -> N2 -> N3 -> N4 -> N7` | 写入一个主体的主图、主图 JSON 与可选报告 |
| `batch_from_designs` | `N1 -> N2 -> per subject N3-N4-N7 -> batch verdict` | 每个主体独立输出，不合并 prompt |
| `prompt_only` | `N1 -> N2 -> N3 -> N7` | 只写主图 JSON 或按用户要求只返回 JSON 内容 |
