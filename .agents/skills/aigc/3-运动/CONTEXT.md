...
| --- | --- | --- | --- | --- |
| 运动扩写只写“走过去/跑过去” | 五要素层 | 在原有运动字段内补主体、参照系、起点、路径和终点 | review 固定 `GATE-MOTION-03` | 任取一个被扩写的原有运动字段能回答从哪里、沿哪里、到哪里、相对什么 |
| 新增独立 `运动强化：` 字段或改名成新字段导致动作重复 | 字段落点层 | 删除独立字段，恢复 source 原字段名，把五要素并回原有 `画面`、`动作`、`表演`、`调度` 等运动承载字段值 | review 固定 `GATE-MOTION-08A` | 正文中没有新增/改名字段，motion ledger 仍能回指 source anchor |
...
| 同一场景或连续动作段参照系漂移 | 场景/动作段参照层 | 先建 `group_reference_profile`，选一个参照簇主参照；局部切换补 `reference_switch_reason` | review 固定 `GATE-MOTION-04A` 与 `GATE-MOTION-04B` | 同一场景/动作段的 motion units 能回指同一 `primary_reference_frame`，或每次切换都有源内理由 |
...
4. 默认按同一场景或连续动作段先选 `primary_reference_frame`；只有 source 显式已有分镜组或 group_id 时才继承源内组边界。参照簇内 motion units 默认沿用主参照，只有参照不可见、动作重心转移或身体微动作时才切换。
...
- 同一场景或连续动作段最好像共用一张小地图：先选门、桌、墙、台阶、车门、床沿等主锚点，再让局部手部或眼神动作挂到这个主锚点上。
