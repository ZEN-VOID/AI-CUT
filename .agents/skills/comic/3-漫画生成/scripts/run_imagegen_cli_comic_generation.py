#!/usr/bin/env python3
"""Compile and optionally execute CLI imagegen jobs for a 9-page comic group.

This runner is mechanical: it validates the stage-2 JSON, projects each page
into one CLI imagegen job, writes evidence files, and optionally invokes the
shared `.agents/skills/cli/imagegen/scripts/image_gen.py` CLI. It does not
rewrite story, panel, character, or scene truth.
"""
...
def _output_dir_for(args: argparse.Namespace, json_path: Path, data: dict[str, Any]) -> Path:
    ...
    return project_root / "3-漫画生成" / group_slug / "imagegen-cli"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate a 9-page comic group with .agents/skills/cli/imagegen"
    )
    ...
    if not os.getenv("OPENAI_API_KEY"):
        _write_json(
            comic_report_path,
            {
                **pending_report,
                "status": "blocked",
                "error": "OPENAI_API_KEY is not set; CLI imagegen execute requires it.",
