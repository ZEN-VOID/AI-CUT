...
对具体画布里的节点做**查询 / 新建 / 更新 / 改边 / 触发生成**。画布由 **`-p` / `--project`** 指定，或读取当前目录 **`.libtv/project.json`** 的 **`projectUuid`**（见 [commands/project.md](./project.md)）。这里的 `projectUuid` 是具体画布 UUID，不是新版上层 `projectSpaceId` / `folderId`；限定到某个普通分组时通过 **`-g` / `--group`** 或 [`libtv group use`](./group.md) 绑定（见 [commands/group.md](./group.md)）。

> Current CLI compatibility note（2026-06-20 observed）：当前 `libtv node --help` 明确说明默认子命令不接受 `-t`；新建节点必须使用 `libtv node create <node> -t <type> ...`。如果旧文档、旧示例或上游计划写成 `libtv node <node> -t image ...`，执行前必须改写为 `libtv node create <node> -t image ...`；已存在节点的查询、改参、连线和 `--run` 仍使用默认子命令。
...
| 子命令                          | 作用                                                                           |
| ------------------------------- | ------------------------------------------------------------------------------ |
| **（默认）`libtv node <node>`** | **查询 / 更新 / 改边 / `--run`**；当前 CLI 不接受 `-t`，不能用于新建。       |
...
| 选项                                       | 可重复 | 说明                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------ | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `-p, --project <project>`                  | 否     | 具体画布 **UUID**。省略时读当前目录 `.libtv/project.json` 的 `projectUuid`（需已 `libtv project use`）。**勿与** [`libtv project list -p`](./project.md)（**页码**）或列表输出的 `projectSpaceId` / `folderId` 混淆。                                                                                                                                                                                                                                           |
| `-g, --group <group>`                      | 否     | 父级**普通分组**的节点 id 或展示名。限定 `<node>` 与 `--left/--right` 端点解析范围；未传时若已 `libtv group use` 则自动套用；带分组新建时 `--x/--y` 为**组内相对坐标**。                                                                                                                                                                                                                                             |
| `-t, --type <type>`                        | 否     | **当前 CLI 默认子命令不接受此选项；新建节点请使用 `libtv node create <node> -t <type>`。** 本行保留为旧版行为说明的兼容提醒；执行时以 `libtv node --help` 为准。                                                                                                                                                                        |
...
**视频默认模型**：当 `-t video` 创建/更新视频生成节点且命令或上游计划未指定模型时，执行层默认应写入 `-s model=star-video2`。不要把已有节点、历史任务或示例中的 `star-video2-fast` 继承为默认值；fast 仅在用户或计划明确要求 fast / 极速 / 草稿预览时使用。
...
| 有 `-t` 且非「仅改边」独占分支                        | 旧版行为；当前 CLI 默认子命令会报 `unknown option '-t'`，必须改用 `node create` |
