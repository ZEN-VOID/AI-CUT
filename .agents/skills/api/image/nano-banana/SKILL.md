...
7. **调用方技能包驱动的输出路径**
   ...
   - `aigc/4-Design/2-主体设计` 系调用方，默认输出应进入对应 `projects/aigc/<项目名>/.../generated/` 运行时路径；若带 `episode_id`，必须补齐到分集级目录。
...
- 默认输出目录由调用方技能包决定：
  ...
  - `aigc/4-Design/2-主体设计`：`projects/aigc/[项目名]/4-Design/.../generated/`
...
5. 确认输出目录是否符合：
   ...
   - `4-Design/2-主体设计` 是否进入 `projects/aigc/[项目名]/.../generated/`
