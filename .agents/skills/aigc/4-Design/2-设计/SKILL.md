...
6. 设计文件生成后必须继续调用 `.agents/skills/api/image/nano-banana/general` 的单主体快路径，图片与设计文件同目录同 stem。
...
8. 命中 `场景` 时，加载 `场景/SKILL.md + CONTEXT.md`
9. 命中 `角色` 时，加载 `角色/SKILL.md + CONTEXT.md`
10. 命中 `道具` 时，加载 `道具/SKILL.md + CONTEXT.md`
11. `projects/aigc/<项目名>/0-Init/{north_star,init_handoff,story-source-manifest}.yaml`
12. `projects/aigc/<项目名>/2-Global/全局风格.md`
13. `projects/aigc/<项目名>/2-Global/全集类型元素.md`
14. `projects/aigc/<项目名>/2-Global/导演意图.md`
