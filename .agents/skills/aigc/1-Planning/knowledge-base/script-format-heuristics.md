...
- 本文件是 `aigc/1-Planning/2-格式` 的经验层知识库，不是过程日志。
- 调用 `.agents/skills/aigc/1-Planning/references/script-format-contract.md` 时，应自动预加载本文件。
...
| --- | --- | --- | --- | --- |
| 又把 `格式判模 / 标准剧 / 解说剧` 拆成外部 agent 依赖 | 真源治理层 | 收回到 `references/script-format-contract.md` 的内化能力面 | 在 `SKILL.md + audit` 固化 `Internal Capability Fusion Contract` | `2-格式` 不再引用旧规划组文档 |
...
- `2-格式` 最稳的形态不是“skill + 一堆外部规划组 agent”，而是“一个 skill 内化判模、变体与执行闭环”。
