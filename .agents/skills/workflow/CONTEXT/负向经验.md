...
| `TM-WORKFLOW-28` | 素材池只有粗粒度 `视频/图片`，选材时难以快速定位开头、收益、漫剧、大字报、工作流、引流、转场或关键词素材 | 素材预处理分类缺失，导致每次 workflow 都从原始素材重做筛选 | 先按 `projects/素材/` 预处理树补齐空目录，把后续素材逐步放入对应分支；保留 `视频/图片/文案/音频` 作为兼容原始入口 | `Project Asset Pool Contract` + registry `shared_asset_material_branches` / `shared_asset_keyword_root` | `find projects/素材` 能看到素材分支和 `核心关键词/`；空目录不被当作真实素材证据 |
