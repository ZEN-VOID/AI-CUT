...
`角色/1-清单` 负责把 `5-分组` 逐集分镜组底部 YAML 的 `角色` 字段汇总为角色设计阶段的第一份 canonical 清单。它只建立“哪些角色需要进入后续设计”的表格真源，不生成角色设计稿、外貌方案、服装方案或镜头提示词。
...
- 上游唯一准确信息来源固定为 `projects/aigc/<项目名>/5-分组/第N集.md` 中每个分镜组底部 YAML 的 `角色` 字段；必要时只允许回查同一分镜组正文作为证据。
...
- 用户要求“角色清单”“从分组 YAML 生成角色列表”“角色 1-清单”“进入 6-设计/角色清单”等任务。
- 已完成或部分完成的 `5-分组` 逐集分镜组稿。
...
- 上游 `5-分组/第N集.md` 不存在、不可读，且用户没有提供替代可核验证据。
- 用户要求直接从剧情印象、摄影稿或外部设定跳过 `5-分组` YAML 生成清单；本技能 canonical 上游必须是组底 YAML。
...
| mode | 触发信号 | 输出 |
| --- | --- | --- |
| `project_all` | 给定项目且未限制集号 | `6-设计/角色/1-清单/角色清单.md` 与可选执行报告 |
| `episode_range` | 指定一个或多个 `第N集.md` | 覆盖指定范围证据的角色清单更新 |
| `incremental_merge` | 既有 `角色清单.md` 存在，且 `5-分组` 新增/更新了部分 `第N集.md` | merge 更新清单、执行报告与可选 `design-manifest.yaml` |
...
2. 锁定上游 `projects/aigc/<项目名>/5-分组/第N集.md`，按集号和文档顺序遍历所有分镜组；若既有 `角色清单.md` 或 `design-manifest.yaml` 存在，先读取并建立本轮 `reconcile_delta`。
...
7. 写入 canonical 输出 `projects/aigc/<项目名>/6-设计/角色/1-清单/角色清单.md`；如生成执行报告，写入同目录 `执行报告.md`；可同步更新 `projects/aigc/<项目名>/6-设计/角色/design-manifest.yaml` 的 source/subject 映射。
...
| --- | --- | --- | --- |
| `FIELD-CHAR-LIST-01` | 输入取证 | 项目路径、上游 `5-分组/第N集.md`、分镜组 ID 和 YAML `角色` 字段明确 | `FAIL-CHAR-LIST-01` |
...
| --- | --- | --- | --- | --- |
| `PASS-CHAR-LIST-01` | 输入锁定 | 项目路径、目标集号、`5-分组/第N集.md` | 是否具备组底 YAML `角色` 字段和分镜组 ID | `input_manifest` |
...
| --- | --- | --- | --- |
| `PASS-CHAR-LIST-01` | 读取本技能与项目上下文，锁定 `5-分组` 输入 | input manifest | `references/source-and-merge-contract.md` |
...
- 从非 `5-分组` YAML 的来源直接创建 canonical 清单。
...
1. 角色清单固定写入 `projects/aigc/<项目名>/6-设计/角色/1-清单/角色清单.md`。
2. 可选执行报告写入 `projects/aigc/<项目名>/6-设计/角色/1-清单/执行报告.md`。
3. 可选增量状态索引写入 `projects/aigc/<项目名>/6-设计/角色/design-manifest.yaml`；它只是 sidecar，不替代 `角色清单.md`。
...
| --- | --- |
| `OUTPUT-CHARACTER-LIST` | `projects/aigc/<项目名>/6-设计/角色/1-清单/角色清单.md` |
| `OUTPUT-CHARACTER-REPORT` | `projects/aigc/<项目名>/6-设计/角色/1-清单/执行报告.md` |
...
- 上游 `5-分组/第N集.md` 可回指，候选角色均来自组底 YAML `角色` 字段。
