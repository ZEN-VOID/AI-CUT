...
3. 检查 prompt 是否为 source-first enriched YAML：以原 `## group_id` 起笔，完整保留组正文，只在 fenced YAML 内注入故事板和主体 uploaded_url。
4. 检查故事板参照是否来自 `6-图像/B-分镜故事板`，且只作为 `storyboard_total_reference`。
5. 检查主体参照是否只来自组底 YAML 与 `5-设计/*/3-生成` 的真实图片。
6. 检查每个已绑定主体是否在 fenced YAML 对应 `角色 / 场景 / 道具` 列表项中出现 `name + uploaded_url`，不得使用空 URL 或人工 `参照图N`。
...
11. 检查远端 `*-libtv-submission.txt` 是否只在 `【分镜组源文本】` fenced YAML 内写入进入 `mixedList` 的故事板身份/主体名 + uploaded URL，不预设 `参照图1/2/N` 人工编号。
12. 检查远端 `*-libtv-submission.txt` 的 `【直接生成请求】` 是否使用 source-first enriched YAML 形态的 `【分镜组源文本】` 作为生成 prompt 完整体；不得另起 `【混合参照说明】`，不得出现裸图片 token 丢失故事板身份或主体名绑定。
...
15. 检查生成前是否能验证 `create_generation_task.params.enableSound` 开启；生成后或下载后是否通过音频证据 / `ffprobe`。
