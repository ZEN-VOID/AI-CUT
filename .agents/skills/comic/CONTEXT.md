...
| `TM-COMIC-ROOT-09` | 3 号仍默认走 CLI/API、Seedream、Dreamina 或散装 provider | 3 号 runtime policy 层 | 回到 3 号 built-in imagegen 合同，默认生成 handoff plan / prompt set / page prompts | 父级明确 3 号默认 provider 为 `.agents/skills/cli/imagegen` built-in `image_gen` | 3 号 report 记录 `provider=built-in-imagegen` |
...
5. 若内容质量问题来自故事切分、角色锁或漫画语法，回到 2 号；若来自 built-in imagegen handoff、文件命名、项目持久化或执行环境，回到 3 号。
...
- 四段链完整交付时，最稳的节奏是：先把 `第N组.md` 分组漫剧剧本落盘，再把九刀流 group JSON 落盘，再让 3 号 built-in imagegen 生成 handoff plan/prompts/pages/report，最后由 4 号技能收束成单集海报设计 JSON。
