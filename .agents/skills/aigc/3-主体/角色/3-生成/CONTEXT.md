...
| 生成包需要 Skill 2.0 升级但旧正文较长 | runtime-spine 追加层 | 保留旧正文，在末尾追加 `Skill 2.0 Runtime-Spine Upgrade` 小节和必需控制块 | README/CHANGELOG/test-prompts 同步；legacy workflow 作为 reference | 控制块可被 validator 发现，旧执行器锁和 view_image 规则不丢失 |
...
- 当生成技能包需要快速升级时，优先保持 Executor Lock、prompt_only、view_image 和 anti-pseudo-diff 四个门不变，再补 runtime-spine 表格。
