...
| `style_payload=long_north_star` | 不影响组内时长或计入场景标题行/定场镜头/画面构图/构图分区/画面属性后的 1680 字风险口径；组头第 1 行应从全局风格母稿抽取当前组匹配部分并压到 300 字以内，第 2、3 行直引类型元素和画面风格 | north-star projection | 禁止完整照抄全局风格母稿到每个组；禁止凭空新增当前组无证据风格 |
