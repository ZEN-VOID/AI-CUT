...
| 文档末尾缺少通用前缀，或前缀超过 150 字、夹带项目私货 | 交接收束层 | 把全剧视觉命题压成 1 条通用视觉前缀，只保留可跨任务复用的世界、光影、材质与情绪约束 | 在模板和 `SKILL.md` 固定文末 `通用全局风格提示词前缀（150字内）` 为必备最后一节 | 文末存在一条 <= 150 字的通用前缀，且不含项目专名/单集事件 |
| 输出路径漂移回 `projects/aigc/<项目名>/2-Global/全局风格.md` | 真源治理层 | 改回子目录 canonical 输出 | 在 `SKILL.md` 固化 child-local canonical rule | 真源只存在于 `全局风格/全局风格设计.md` |

## Repair Playbook

1. 先确认 `projects/aigc/<项目名>/1-Planning/3-分组/` 是否已有完整 grouped script。
...
- 通用前缀不是把整份文档缩写一遍，而是把“怎么生成这个项目的视觉世界”压成一条可跨阶段复用的起手约束；一旦带入角色专名或剧情事件，就不再通用。
