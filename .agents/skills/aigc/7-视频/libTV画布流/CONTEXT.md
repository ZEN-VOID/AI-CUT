...
本文件是 `aigc/7-视频/libTV画布流` 的经验层知识库，不重定义 `SKILL.md` 的入口合同。
...
| `TM-LIBTVCANVAS-07` | 远端 Agent 重写、压缩或优化 `4-分组` 正文 | 未锁定 `allow_libtv_prompt_optimization=false` | 重提并明确禁止远端优化，除非用户显式 opt-in | submit plan、queue、report 必须记录 opt-in 状态 |
...
3. 主体参照流先读 `4-分组/第N集.md`，不要回到 `3-摄影` 或重新编写组内容。
...
- `4-分组` 组正文已经是 prompt 主体；本技能只做运输层组织、主体绑定和参数投影。
