...
| `constraint_profile` | 对白 4-6 句、完整组构成目标 <= 1680 字且硬上限 <= 1980 字、画面句子多分镜不可截断、补位画面成对一致 |
...
| `N5-GROUP-PLAN` | 裁决组边界 | scene unit table、style projection | 按三重约束形成组计划，预留补位画面字数；超过 1680 字时先拆分复核，1980 仅作硬上限 | group boundary plan | `N6-BRIDGE` | 每组候选目标 <= 1680，例外 <= 1980 且完整 |
...
| 组超 1680 字且无例外理由，或组超 1980 字 | `N5-GROUP-PLAN`，移动完整 atomic unit |
