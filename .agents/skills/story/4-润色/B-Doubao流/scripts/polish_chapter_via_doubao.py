#!/usr/bin/env python3
"""
Bridge script for `story/4-润色/B-Doubao流`.

It assembles the polishing context pack from the canonical 3-初稿 chapter,
then delegates the actual prose polish to AnyFast `doubao-seed-2.0-pro`.
"""
...
SYSTEM_PROMPT_PATH = POLISHING_DIR / "templates" / "doubao-system-prompt.md"
...
EXPECTED_POLISHING_MODEL = "Doubao"
...
def _render_output_template(
    *,
    ...
    rendered = rendered.replace('初稿来源: ""', f'初稿来源: "{previous_chapter_ref}"')
    rendered = rendered.replace("{{draft_source_ref}}", previous_chapter_ref)
    rendered = rendered.replace("{{chapter_body}}", "[请填充完整润色正文]")
    return rendered
...
def _build_branch_instruction_section(
    *,
    ...
    lines: list[str] = []
    if polishing_mode == "polish_rewrite":
        lines.append("重润分支：必须先吸收 3-初稿与既有 4-润色稿中已经成立的事实，再按中文表达和题材质感要求重构语言。")
        if continue_from:
            lines.append(f"续写边界：{continue_from}")
    elif polishing_mode == "local_repair":
...
def _require_word_count(value: object) -> None:
    if not isinstance(value, str) or not re.fullmatch(r"\d+字", value.strip()):
        raise ValueError("豆包返回 frontmatter 字段 `字数` 必须使用 `XXX字` 格式。")
...
def _validate_generated_markdown(text: str, chapter_num: int, expected_previous_ref: str = "") -> None:
    ...
    if payload.get("润色模型") != EXPECTED_POLISHING_MODEL:
        raise ValueError(f"豆包返回 frontmatter 字段 `润色模型` 必须是 `{EXPECTED_POLISHING_MODEL}`。")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Polish chapter manuscript via AnyFast Doubao")
    ...
    parser.add_argument(
        ...
        default="auto",
        help="polishing mode; auto polishes from 3-初稿 and blocks writeback over existing 4-润色 chapters",
    )
    parser.add_argument(
        "--instruction",
        action="append",
    ...
    parser.add_argument("--continue-from", help="legacy boundary note; prefer --instruction for polishing constraints")
...
def _resolve_polishing_mode(args: argparse.Namespace, chapter_path: Path) -> str:
    target_exists = chapter_path.exists()
    if args.mode == "auto":
        if target_exists and not (args.dry_run or args.no_writeback):
            raise SystemExit(
                "目标章已存在，auto 模式禁止直接覆盖。请显式传入 "
                "--mode polish_rewrite / local_repair，并加 --force；"
                "或使用 --dry-run / --no-writeback。"
            )
        return "polish_rewrite" if target_exists else "chapter_polish"

    if args.mode == "chapter_polish" and target_exists and not (args.dry_run or args.no_writeback):
        raise SystemExit("目标润色章已存在，不能用 chapter_polish 覆盖；请改用 polish_rewrite/local_repair 并加 --force。")
    if args.mode in {"polish_rewrite", "local_repair"} and not target_exists:
        raise SystemExit(f"目标润色章不存在，不能执行 {args.mode}；请改用 chapter_polish。")
...
def main() -> int:
    ...
    if output_dir:
        ...
        if messages_path is None:
            messages_path = run_dir / f"chapter_{chapter_num}_doubao_messages.json"
            messages_path.write_text(json.dumps(messages, ensure_ascii=False, indent=2), encoding="utf-8")
        ...
        with tempfile.TemporaryDirectory(prefix="story-polishing-doubao-") as temp_output_dir:
    ...
    if not args.no_writeback:
        ...
        if chapter_path.exists() and output_dir:
