...
| 角色演技、微表情、情绪动机、身体联动、环境声音和微动态限制 | `references/actor-performance-control-contract.md` |
...
- 上游心理、潜台词、信任变化、权力压迫、沉默反应必须按 `references/psychological-reaction-contract.md`、`references/performance-and-scene-craft-contract.md` 与 `references/actor-performance-control-contract.md` 转成主角内心独白、可执行演员任务、身体行为、微表情、呼吸、停顿、道具动作、空间关系、环境声音、群体反应或环境/道具状态差；不得停留在解释性结论或单一情绪标签。关键情绪 beat 必须避免“开心/愤怒/难过/紧张”等模板化情绪词空转，优先形成 `trigger -> surface_emotion / suppressed_emotion / hidden_motive -> micro_expression -> body_linkage -> ambient_support -> micro_dynamics` 的表演控制证据，并内嵌到既有字段。`心理反应` 字段必须“可 GET”：观众应能通过演员表演、画面或声音接收到反应，不能写成抽象想象、内心散文、因果解释或离开文字无法感知的主角心理。描写人物动作时只写镜头可实拍的客观动作、神态和语气，不得写“试图、想要、打算、意图”等主观预判或心理意图；“感到恶心/难受/愤怒”等主观情绪必须转成微表情、肢体动作、生理反应、环境声音或主角内心独白。视线只是候选变量，不得把多个转场、潜台词或未出口对白连续写成“避开对方 / 看向远方 / 顺着视线望去”的固定模板。
...
- 执行报告包含 `thinking_action_node_ledger`、`novel_expression_transform_evidence`、`psychological_reaction_evidence`、`actor_performance_control_evidence`、`protagonist_inner_voice_evidence`、`objective_action_purity_evidence`、`director_substance_evidence`、`visual_aesthetic_evidence.episode_visual_spine`、`visual_aesthetic_evidence.scene_items` 和 `episode_final_image_evidence`；若本轮新增或显著修改学习型合同，还必须包含 `learning_integration_review_evidence`，区分真实样例通过、等价 smoke、`static_only` 与残余风险；若本轮是机械修复或特例降级，必须说明哪些创作证据未补齐以及原因。
...
2. `N6-REVIEW` 按 `review/review-contract.md` 审计保真、对白冻结、声画配对、slugline、字段纯度、小说表述二次画面化、心理反应可感知化、角色演技控制、具像化、声音本体、编导创作内核、高潮画面、场景状态差、演员任务、场面调度内嵌、画面美学组织、controlled enrichment 和 LLM-first 边界。
...
flowchart TD
    ...
    B -->|"潜台词/权力关系/沉默反应/强情绪"| I["scene turn + actor performance control -> 微表情 / 身体联动 / 环境声 / 反应余波"]
...
- 角色演技细化以 `references/actor-performance-control-contract.md` 为准；关键情绪 beat 不得只写情绪标签，必须把上游触发点、情绪动机、微表情、非面部生理联动、环境声音和微动态限制转成既有字段中的可表演材料，不得新增剧情、对白或摄影方案。
...
| `FIELD-DIRECT-11` | 表演与场景工艺 | 关键场景有状态差；心理、潜台词、权力关系、强情绪和沉默反应被转成主角内心独白、可执行表演/调度/反应证据；关键情绪 beat 有上游触发点、情绪动机、微表情、身体联动、环境声或微动态限制；`心理反应` 必须可见、可听或可演，动作字段只写客观可拍动作/神态/语气，并内嵌到对应剧本句段 | `FAIL-DIRECT-11` |
...
| `FIELD-DIRECT-16` | 创作证据 | 执行报告包含 `thinking_action_node_ledger`、`novel_expression_transform_evidence`、`psychological_reaction_evidence`、`actor_performance_control_evidence`、`director_substance_evidence`、`episode_visual_spine`、`visual_aesthetic_evidence` 和 `episode_final_image_evidence`，证明思维·执行节点、小说转译、心理反应、角色演技控制、编导判断、画面美学和终结画面尾钩不是只停留在文档规则层 | `FAIL-DIRECT-16` |
| `FIELD-DIRECT-19` | 学习集成验证 | 新增或显著修改学习型合同时，执行报告包含 `learning_integration_review_evidence`，说明静态接入点、真实样例或 smoke 状态、残余风险和下一次生产观察点；没有真实样例时只能标注 `static_only` | `FAIL-DIRECT-19` |
...
| `PASS-DIRECT-07` | 表演与场景工艺 | 场景表、字段路由、编导创作内核、高点计划、顾问包 | 是否把场景状态差、潜台词、演员任务、权力关系、强情绪和沉默反应转成主角内心独白、客观可拍执行计划或角色演技控制计划，并清除动作字段中的主观意图词 | `scene_dramatic_map` / `performance_task_map` / `actor_performance_control_evidence` / `objective_action_purity_evidence` |
...
| `PASS-DIRECT-10` | 验收回写 | 编导稿、校验结果、`thinking_action_node_ledger`、学习集成验证证据 | 是否满足保真、声画、场景、高潮画面、编导创作干货、表演工艺、controlled enrichment、思维·执行节点和输出门禁 | `review_result` / `thinking_action_node_ledger` / `learning_integration_review_evidence` |
...
| `PASS-DIRECT-07` | 心理、潜台词、权力关系、强情绪和沉默反应被转成主角内心独白、可执行表演/调度计划或角色演技控制计划，并要求终稿内嵌；动作字段无主观意图词，不新增对白或摄影方案 | `FAIL-DIRECT-11` | `references/performance-and-scene-craft-contract.md` + `references/actor-performance-control-contract.md` |
...
| `PASS-DIRECT-10` | 输出路径、执行报告、思维·执行节点 ledger 和 review gate 齐全 | `FAIL-DIRECT-07` / `FAIL-DIRECT-19` | `review/review-contract.md` + `steps/directing-workflow.md#Thinking-Action Node Contract` |
...
- 思维·执行节点只写流程动作或 checklist，没有判断问题、证据字段、出口路由、gate 状态、source owner 或失败回路。
- 学习型合同新增后只完成静态文档引用，没有在执行报告中说明真实样例、等价 smoke、`static_only` 状态、残余风险和后续生产观察点。
