...
- 若目标文本来自书项目，必须先确认 canonical runtime 仍是 `projects/story/<项目名>/`，并锁定真实目标文件；不得在技能目录里冒充业务正文。
...
- 若目标是 `projects/story/<项目名>/3-Drafting/第N集.md`：
  - 必须先确认真实 `project_root`
  - 若 `N > 1` 且用户要求强连续性润色，可补读上一集终稿
