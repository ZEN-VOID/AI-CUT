...
- 回答：`validation-report.md` 的 closure 四段到底该怎么写才可复核
- 不替代阶段报告真源，只负责 closure 的书写提示与反例护栏
...
6. 若本轮以知行合一方式结案，`validation-report.md` 还必须写出 `思考过程 / 关键证据 / 风险/例外 / 下一入口`，以便创作评审与阶段验收共用同一 closure 载体。
