...
| --- | --- | --- | --- |
| frame visual reference | 用户提到分镜画面图、镜级图、多张图、四段式 `分镜ID`、`7-图像/A-分镜画面` | `A-分镜画面参照` | 核对每个 `shot_id` 是否来自目标分镜组；缺图由叶子处理，不在父级补图 |
| storyboard reference | 用户提到分镜故事板、组级 storyboard、整张故事板图、`7-图像/B-分镜故事板` | `B-分镜故事板参照` | 核对 `group_id` 与故事板图唯一对应；没有故事板图时由叶子决定 text-only |
| subject reference | 用户提到角色、场景、道具、主体参照、组底 YAML、`6-设计/*/3-生成` | `C-主体参照` | 以组底 YAML 为主体槽位真源；不要用正文泛词猜主体 |
...
| video filename drift | 下载视频使用 sessionId、provider id 或非 group_id 文件名，导致 `9-审片` 无法回推分镜组 | A/B/C/D 叶子 output contract | canonical 视频名改为 `<group_id>.mp4`；同组多变体用 `<group_id>-a.mp4`、`<group_id>-b.mp4`，sessionId 写 queue/report |
| duplicate LibTV canvas | 同一 `projects/aigc/<项目名>/` 多次调用 `$libTV` 却生成多个 projectUrl / 画布 | 项目级画布 registry 缺失或未读 | 先读 `projects/aigc/<项目名>/8-视频/libtv-canvas-registry.json`；缺失时从 A/B/C/D queue/results/report 反建；已有 `canonical_sessionId` 时复用 |
...
4. 若用户只说“把第 N 集生成视频”，先看是否已有明确视频策略或既有 `8-视频/<叶子>/` 产物；仍不明确时询问最小澄清。
...
8. 下载视频进入可审片状态前，必须确认文件名能直接回推 `5-分组` 的 `group_id`；同组变体只能使用小写字母后缀。
...
- 视频阶段的上游事实主源通常是 `5-分组`；`6-设计` 和 `7-图像` 提供参照资产，不应反向改写分组正文。
...
- 视频生成未显式指定模型时，父级和叶子都应按 `$libTV` 后端默认路由理解；具体模型选择不在 `8-视频` 本地硬编码。除非用户显式要求其他规格，否则基础输出默认 720P、15 秒、16:9；用户显式指定模型、时长、比例、分辨率或质量档时，保留原话进入 LibTV 任务正文。
- 视频文件名是 `8-视频` 和 `9-审片` 之间的主接口；不要把审片依赖绑到 sessionId 或下载顺序上。
