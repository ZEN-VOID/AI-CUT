...
| --- | --- | --- |
| 默认 LibTV 画布视频 | 用户说“根据 10-分组生成视频节点”、“创建画布”、“上传主体参照图” | 路由 `libTV画布流` |
| LibTV / AIGC 层级映射 | 用户说明“libTV项目名=projects/aigc/项目名”或“项目名.画布名=项目名/第N集” | 父级锁定映射：LibTV 项目空间名对应 AIGC 项目名，LibTV 画布名对应第N集；本地 `projects/aigc/<项目名>/第N集` 只是语义范围，不替代 `10-分组/第N集.md` 或证据目录 |
...
- 源文件路径仍优先是 `projects/aigc/<项目名>/10-分组/第N集.md`；画布证据路径仍是 `projects/aigc/<项目名>/13-画布/libTV画布流/第N集/`。
