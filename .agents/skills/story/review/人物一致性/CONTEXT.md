...
| 台词语法通顺但角色身份不对，高文化角色突然市井化，或底层角色突然讲成论文腔 | sociolect drift | 回到角色阶层、地域、教育程度、职业经验、即时情绪和关系位置，重判是否有伪装、失控或策略性变声理由 | 在人物维度输出中固定 `sociolect_violations`，并把无因声口漂移路由到 `5-对白优化` | 报告能指出哪句台词违背了哪项身份/背景证据 |
...
4. 声口检查要继续拆到社会身份：阶层、地域、教育、职业经验、即时情绪和关系位置；若有突变，必须判断正文是否给出伪装、失控或策略性变声理由。
5. 若问题主要出在说话方式，优先回 `5-对白优化`；若是内心活动像作者评论或 POV 漂移，优先回 `6-心理活动描写`；若是行为/动机失真，再回 `4-角色形象刻画`。
6. `drafting_inline` 下若当前是 `Step 4`，对白问题默认记为 Step 5 待处理项，不得把 Step 5 的工序债务提前当成 Step 4 的阻塞失败。
...
- 声口一致性不是给每个人贴口头禅；更可靠的依据是身份压力、教育背景、地域经验、职业语汇和当下关系位置共同塑造出的句法与省略方式。
