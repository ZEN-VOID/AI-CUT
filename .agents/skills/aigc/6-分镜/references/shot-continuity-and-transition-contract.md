...
本文件融合旧 `4-摄影/references/intra-shot-transition-contract.md` 与 `shot-continuity-contract.md`，改写为 `6-分镜` 的连贯性细则。它约束字段内相邻分镜和字段间相邻画面点的可剪辑连续性，不替代下游创意转场或 `9-图像` / `10-画布` 生成。

## Core Rule

每个画面点内的相邻分镜之间必须存在物理、注意力、声音或光色过渡锚点。相邻画面点之间必须保留可消费的姿态、轴线、动作方向、视线、声音尾巴、光色变化、空间出口或注意力落点。同一画面点内多条分镜还必须保持前景、中景、后景和主体站位的连续性；具体解析由 `start-frame-spatial-continuity-contract.md` 约束。
...
| `screen_direction_axis_map` | 多人、对峙、追逐、进出门或转场承接时，谁在 screen left/right、主体朝向、入口/出口方向、视线轴线和上一镜交出点 |
| `movement_policy` | 顺接上一运动方向、减速停住、反向但给动机，还是用静止切断 |
| `action_anchor` | 人物站/坐/蹲/跪/靠墙/牵手/搀扶等持续姿态是否继承 |
| `within_point_spatial_continuity_map` | 同一画面点内多条分镜是否共享 base_space，前景/中景/后景/主体站位如何合法变化 |
...
- 多人、对峙、追逐、进出门或转场承接时，内部计划必须写出 `screen_direction_axis_map`：谁在画面左侧、谁在右侧、主体朝向哪里、从哪里入画/出画、视线轴线指向谁、上一镜把注意力交到哪里。
- 若 screen left/right 影响理解，内部计划必须重复谁在画面左侧、谁在右侧，中间空间锚点是什么；成稿里至少压缩出可读的方位或连续性锚点。
...
```text
分镜1（0-1.5秒）：中景，深景深，走廊纵深构图，门框压住前景左侧，主角沿中景左墙后退，后景冷白灯管一盏盏退远，主角贴左墙扶门框后退。
分镜2（1.5-3秒）：近景，浅景深，前景遮挡构图，门框继续压在前景左侧，右手占中景贴住门框，后景走廊虚成一条压低亮线，主角仍停在左墙边只把手推到画面中央。
```
...
| 触发多人、对峙、追逐、进出门或转场承接时，是否有 screen direction / 入口出口 / 视线轴线证据？ | `GATE-SB-18` | `FAIL-SB-SCREEN-DIRECTION` | `N5/N7` | `screen_direction_axis_map` |
| 同一画面点内多条分镜的前景、中景、后景和主体站位是否连续？ | `GATE-SB-22` | `FAIL-SB-SPATIAL-CONTINUITY-WITHIN-POINT` | `N5/N7` | `within_point_spatial_continuity_map` |
