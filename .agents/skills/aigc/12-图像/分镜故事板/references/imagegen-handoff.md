...
本文件定义 step3：根据任务执行前缀、`10-分组` 对应分镜组完整内容、storyboard frame-unit plan、panel 描述、角色头顶名称标注、annotation plan、主体参照、style lock、layout aspect decision、visual prompt atoms 和可选 `spatial_handoff`，调用 `.agents/skills/cli/imagegen` 以分镜组为单位生成组级 storyboard sheet 图片。
...
- 默认不设置后台并行要求；执行者应按 `.agents/skills/cli/imagegen` 当前能力顺序执行或受控批量执行。
...
| --- | --- | --- | --- | --- |
| 执行前是否加载 `.agents/skills/cli/imagegen/SKILL.md + CONTEXT.md`，默认遵循 built-in `image_gen` 路由，并实际完成 imagegen 调用？ | `G9-HANDOFF` | `FAIL-SHEET-IMAGEGEN` | `N7-IMAGEGEN` / `references/imagegen-handoff.md#skill-dependency` | imagegen plan 记录 dependency loaded、`mode` 和未启用 CLI/API fallback 的原因；results/report 记录 `imagegen_called` 与生成图路径 |
