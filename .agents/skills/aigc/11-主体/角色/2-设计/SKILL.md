...
- 上游角色入口固定为 `projects/aigc/<项目名>/11-主体/角色/1-清单/角色清单.md`；清单字段至少包含 `名称`、`首次登场`、`原文描述（关键词式）`。
...
- 审美吸引力约束：角色设计必须强化容貌和服装的审美属性，而不是手术式还原清单关键词。女性角色默认朝美丽、动人、有辨识度的方向设计，男性角色默认朝英俊、不凡、有辨识度的方向设计；主角必须进一步强化颜值、气质和服装完成度。正派、反派和功能角色都应充满个性化魅力，允许好看、锋利、危险、阴郁、病态或怪诞，但不得平庸、模板脸或只靠职业标签区分。
- 明星脸灵感约束：可在 `Visual Drivers` 中使用 1 到 2 个公开明星、演员或模特作为脸型、骨相、气质、妆发或镜头魅力的灵感来源，但必须转译为原创组合，不得写成精确复刻、换脸、同款肖像或让角色被识别为现实本人；若项目、用户或安全边界不允许真实人物参考，则改用“明星级镜头脸 / editorial model face / cinematic leading-face quality”等泛化描述。
...
- 推荐运行时路径：主 agent 读取 `team.yaml.init_synthesis.stage_seed_summary."11-主体"`、`init_handoff.design_seed` 与 `north_star.yaml.创作阶段不变量.设计`，筛出与角色、服装、美术、摄影、导演相关的可执行约束、启发和风险，压缩为 `init_team_synthesis_context`。
...
- 已存在的 `projects/aigc/<项目名>/11-主体/角色/1-清单/角色清单.md`。
...
| --- | --- | --- | --- | --- |
| `knowledge-base/character-design-corpus.md` | 任意角色细目设计、批量设计、增量补缺或 repair 中需要强化容貌、妆容、服装吸引力、主角美丽/英俊、正反派个性魅力、明星脸原创转译或 prompt 审美短语 | 作为 LLM 创作前的语料、词库、转译模式、妆容化策略和服装时代语境护栏 | 逐字套用；覆盖角色清单、项目时代、用户禁区或 `north_star.yaml`；把真实人物灵感写成精确复刻；让服装风格化脱离时代母体 | `GATE-CHAR-DESIGN-20`、`GATE-CHAR-DESIGN-19`、`GATE-CHAR-DESIGN-06`、`GATE-CHAR-DESIGN-12`、`GATE-CHAR-DESIGN-13` |
...
flowchart TD
    ...
    M --> N
    N --> O{"通过门禁?"}
    O -->|"yes"| P["写入 11-主体/角色/2-设计/C###-<角色名>.md"]
    O -->|"no"| K
```
...
2. 读取上游 `角色清单.md` 和可选 `projects/aigc/<项目名>/11-主体/角色/design-manifest.yaml`，锁定待设计角色主体、首次登场和原文描述关键词；不得新增清单外角色作为 canonical 输出。
...
11. 使用 `templates/output-template.md` 为每个角色生成唯一 markdown，写入 `projects/aigc/<项目名>/11-主体/角色/2-设计/<主体ID>-<角色名>.md`，并可更新 `design-manifest.yaml` 的 `design_file` 与 `design_gaps`。
...
| `FIELD-CHAR-DESIGN-10` | Init team synthesis | 已按 `team.yaml.init_synthesis.stage_seed_summary."11-主体"`、`init_handoff.design_seed` 或 `north_star.yaml.创作阶段不变量.设计` 形成 `init_team_synthesis_context`，并把节点级判断、执行取舍、局部 patch 或风险提示作为创作前上下文；缺失时有明确记录 | `FAIL-CHAR-DESIGN-10` |
...
- 角色设计只做“原文关键词还原”，缺少容貌、妆发、骨相、身形和服装审美强化；主角不够美丽/英俊，反派或配角缺少个性化视觉魅力；明星脸灵感被写成现实人物精确复刻或换脸。
...
5. `解构` 必须包含审美吸引力判断：`Beauty / Handsomeness Target`、`Face / Bone Aesthetic`、`Costume Appeal Strategy`，以及可选 `Celebrity Face Inspiration`。明星脸灵感只能作为原创设计参考，不得精确复刻现实人物。
...
9. 可选更新 `projects/aigc/<项目名>/11-主体/角色/design-manifest.yaml`，记录 `design_file` 和剩余 `design_gaps`；manifest 不替代设计稿真源。
...
| --- | --- |
| `OUTPUT-CHARACTER-DESIGN` | `projects/aigc/<项目名>/11-主体/角色/2-设计/C###-<角色名>.md` |
| `OUTPUT-CHARACTER-DESIGN-REPORT` | `projects/aigc/<项目名>/11-主体/角色/2-设计/执行报告.md` |
| `OUTPUT-CHARACTER-MANIFEST` | `projects/aigc/<项目名>/11-主体/角色/design-manifest.yaml` |
...
- 已完成审美吸引力投影：容貌、妆发、骨相、身形、服装廓形、材质和色彩均有明确美感策略；女性角色美丽动人，男性角色英俊不凡，主角更强，正反派和功能角色均有个性化魅力；如使用明星脸灵感，已转译成原创组合，不精确复刻现实人物。
