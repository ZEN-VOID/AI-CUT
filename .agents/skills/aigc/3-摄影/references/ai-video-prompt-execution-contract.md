...
本文件把 `reports/transcripts/audio/AI视频镜头与提示词学习稿.md` 中可复用的方法论转译为 `3-摄影` 的摄影细则。它不要求本阶段直接生成最终视频提示词；它要求每条 `分镜N` 在成稿时具备可被下游 `7-视频` 稳定改写为提示词的执行顺序、方向参照、光线结果和表演微动态控制。

## Core Rule

AI 视频生成更像顺序执行提示词，而不是自动理解导演意图。因此 `3-摄影` 的分镜明细必须采用“镜头包裹动作”的设计思路：先建立镜头、运动、构图和参照方向，再让人物动作、表演变化、光线结果和环境声在该镜头内部发生。
...
- `3-摄影` 仍输出自然中文 `分镜明细`，不输出完整视频提示词模板。
