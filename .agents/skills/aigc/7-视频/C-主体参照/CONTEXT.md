...
| 远端消息里混入 `@projects/...` 本地图片路径 | 远端可读性层 | 远端提交只保留 `参照图N <uploaded_url>`，本地路径只留在审核 prompt / plan / manifest | `libtv-handoff.md` 区分 local prompt 与 remote handoff | `*-libtv-submission.txt` 本地路径关键词扫描无命中 |
...
10. 修复后按 `review/review-contract.md` 复核，并在执行报告写清楚 skipped / failed / next_action。
