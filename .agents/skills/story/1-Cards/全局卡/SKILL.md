...
- 势力/力量/世界规则相关判断，优先回读既有 `1-Cards/0-全局卡/**/*.json`、`1-Cards/5-类型卡/**/*.json` 与 `0-Init` seeds；这些内容只提供 craft support，不替代本技能的本地 canonical truth。
...
5. `1-Cards/5-类型卡/**/*.json`
6. `0-Init` 中与世界观/力量体系直接相关的题材 seed
...
| analysis_slot | 当前结论 |
| --- | --- |
| `business_goal` | 把整书级世界设定、规则系统、年代边界、文化艺术、势力格局、科技/武功与金手指压成可长期复用的全局卡。 |
...
| `success_criteria` | 全局卡能回答世界怎么运作、有哪些硬规则、时代与文化气味是什么、谁掌规则/资源、力量/科技边界在哪、金手指如何服务主线且付代价。 |
...
flowchart TD
    ...
    D --> E["收束 faction_topology + power_or_technology + golden_finger"]
...
flowchart LR
    ...
    D --> E["faction_topology"]
    E --> F["power_or_technology"]
    F --> G["golden_finger"]
```
...
| `G4` | 收束势力格局、力量边界与金手指合同 | `faction_topology + power_or_technology + golden_finger` | `FAIL-CD-GLOBAL-POWER` | 回势力/力量/金手指 |
...
| `faction_topology` | 谁制定规则、谁控制资源、主要势力如何分层和互动、主角怎样被卷入这张网？ |
...
4. 再锁 `faction_topology`，把势力层级、资源控制、关系型与主角卷入路径写清。
5. 再锁 `power_or_technology`，把升级路径、资源分布、越级上限与膨胀阀门写清。
6. 最后锁 `golden_finger`，用功能、触发、代价、限制、反制、成长闭合外挂合同。
...
3. `culture_and_arts + faction_topology + power_or_technology`
4. `golden_finger`
5. 模板映射
...
| `FIELD-CD-GLOBAL-03` | `G4` | 势力、力量与金手指合同成立 | `faction_topology + power_or_technology + golden_finger` | `FAIL-CD-GLOBAL-POWER` | 回势力/力量/金手指 |
...
- `culture_and_arts`、`faction_topology` 与 `power_or_technology` 不是空名词，而能约束后续写作。
