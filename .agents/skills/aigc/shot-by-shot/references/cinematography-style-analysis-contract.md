...
`摄影风格解析.md` 是 `shot-by-shot` 输出给 `2-美学/摄影风格` 的摄影语法 side context；生产阶段若需要摄影桥接，应输出 `摄影解析.md` 给 `7-摄影`，显式 legacy 光影桥接才输出 `光影解析.md` 给 archived `backup/9-光影`。它可以讨论景别、视角、焦点、运镜、构图、光影和节奏，但不得改写 `4-编剧`、`5-导演`、`6-分镜` 或 `7-摄影` canonical 正文，也不得固定照抄参考片镜头数量或顺序。
...
| --- | --- | --- | --- | --- |
| `摄影风格解析.md` 是否只作为 `2-美学/摄影风格` side context，或 `摄影解析.md` 是否只作为 `7-摄影` side context，不改写上游 canonical 正文？ | `GATE-SBS-ADAPT-01` | `FAIL-SBS-ADAPT-SIDE-CONTEXT` | `N5-BRIDGE` | 使用边界与未改写上游正文证据 |
...
| `camera_grammar_plan_seed`、`functional_projection_payload` 和 `shot_detail_style_seed` 是否能被 `7-摄影` 消费并转成自然中文运镜注入？ | `GATE-SBS-CINE-04` | `FAIL-CINE-PAYLOAD` | `N5-BRIDGE` | camera plan、payload、shot detail style seed |
