...
它不再在根层维护对象私有 `references/` 或 `templates/`，而是直接治理四个直连子技能包：
...
- 把四个子技能再压回根层 `references/` 或 `templates/`
...
| `角色卡` | 角色对象真源、关系边、成长时间线、`exclusive_item_hooks` 输入接口 | `Cards/2-角色卡/**/*.json` |
| `场景卡` | 场景对象真源、规则与风险、`scene_links`、复用策略 | `Cards/3-场景卡/**/*.json` |
| `物品卡` | 物品对象真源、归属链、使用规则、代价、专属适配 | `Cards/4-物品卡/**/*.json` |

硬规则：

1. 没有 `水月 / 镜花` 这种中间 parent；`1-Cards` 直接调四个子技能。
2. 四个子技能都必须输出正式 `.json` card payload，不输出平行 Markdown 真源。
...
| analysis_slot | 当前结论 |
| --- | --- |
| `business_goal` | 把 `0-Init` 交出的对象种子与风格真源收敛为可长期维护的 cards 体系，并通过四个直连子技能把风格/角色/场景/物品正式落盘。 |
| `business_object` | `projects/story/<项目名>/0-Init/north_star.yaml`、`projects/story/<项目名>/0-Init/init_handoff.yaml`、`projects/story/<项目名>/Cards/**/*.json`、cards writer/validator/tests、四个 child skill package。 |
...
| `success_criteria` | 四个子技能都能独立解释自己的对象成立条件，writer/validator/test 全部识别新子技能路径，cards 系统可通过定向 gate。 |
...
| `step_strategy` | 父层只做总线与闭环，详细对象判断下沉到四个直连 child skills。 |
...
flowchart LR
    ...
    B --> E["场景卡"]
    B --> F["物品卡"]
    C --> G["风格卡/templates/style-card.json"]
    D --> H["角色卡/templates/character-card.json"]
    E --> I["场景卡/templates/scene-card.json"]
    F --> J["物品卡/templates/item-card.json"]
    G --> K["Cards/1-风格卡/**/*.json"]
    H --> L["Cards/2-角色卡/**/*.json"]
    I --> M["Cards/3-场景卡/**/*.json"]
    J --> N["Cards/4-物品卡/**/*.json"]
    K --> O["cards-check"]
    L --> O
    M --> O
    N --> O
...
5. `team.yaml`
6. `风格卡/SKILL.md + CONTEXT.md`
7. `角色卡/SKILL.md + CONTEXT.md`
8. `场景卡/SKILL.md + CONTEXT.md`
9. `物品卡/SKILL.md + CONTEXT.md`
10. 命中的子技能包本地 `templates/*.json`
11. 既有 `Cards/**/*.json`
...
| 多个彼此独立的单对象请求 | `风格卡 / 角色卡 / 场景卡 / 物品卡` 可并发 | 互不共享 writeback 决议时允许并发 |
| mixed 请求 | `风格卡 || (角色卡 -> 场景卡 -> 物品卡)` | 风格卡可并发，后三者存在依赖链 |
| full-build | `风格卡 || (角色卡 -> 场景卡 -> 物品卡)` | 风格卡独立；后三者存在强上游依赖 |
...
| --- | --- | --- |
| `child-skill topology` | 风格/角色/场景/物品四个模块已升格为直连 child skills | 仍存在 active `references/*-module` 业务真源 |
...
| `FIELD-CD-MAT-00` | MAT | `Cards/1-风格卡/**/*.json` | `风格卡` | `FAIL-CD-MAT-00` |
| `FIELD-CD-MAT-01` | MAT | `Cards/2-角色卡/**/*.json` | `角色卡` | `FAIL-CD-MAT-01` |
| `FIELD-CD-MAT-02` | MAT | `Cards/3-场景卡/**/*.json` | `场景卡` | `FAIL-CD-MAT-02` |
| `FIELD-CD-MAT-03` | MAT | `Cards/4-物品卡/**/*.json` | `物品卡` | `FAIL-CD-MAT-03` |
...
| `S4` | `FIELD-CD-MAT-00/01/02/03` | 进入具体 child skill 产出正式 payload | 回 child skill |
...
| `FIELD-CD-MAT-00` | style completeness | 风格卡具备承诺/审美轴/风格系统 | 回 `风格卡` |
| `FIELD-CD-MAT-01` | character completeness | 角色卡具备关系/成长/接口 | 回 `角色卡` |
| `FIELD-CD-MAT-02` | scene completeness | 场景卡具备功能/规则/复用 | 回 `场景卡` |
| `FIELD-CD-MAT-03` | item completeness | 物品卡具备归属/代价/专属适配 | 回 `物品卡` |
...
- 风格/角色/场景/物品四个模块已升格为直连 child skills。
