---
name: aigc-video
description: Use when the `aigc` workflow reaches `projects/<项目名>/视频/` and needs to turn canonical director-group material, subject anchors, or existing images into video request JSON, reference packs, and model-ready execution entries.
...
`6-视频` 是 `aigc` 主链里承接 `编导/第N集.json`、`4-主体` 与 `5-画面` 的视频执行前置阶段真源。
...
4. 产物应落到 `projects/<项目名>/视频/` 的哪里

当前已建可执行子路径位于 `1-提示词蒸馏` 与 `3-视频生成` tranche：
...
- 需要把 `projects/<项目名>/编导/第N集.json` 的分镜组内容蒸馏为视频工具入参 JSON。
- 需要在正式调用 `dreamina` 或其他视频 API 前，先整理主体参照、prompt、画幅与质量参数。
- 需要说明 `6-视频` 与 `5-画面`、`dreamina-cli` 的边界。
- 用户只说“做视频参照 / 做视频请求 JSON / 从编导转视频入参”，但还没进入实际提交命令。

## When Not to Use

- 任务仍在补 `编导/第N集.json` 的组级或镜级事实，应回到 `2-组间` 或 `3-明细`。
- 任务仍在生成参考图、故事板或单帧图，应回到 `5-画面`。
...
- 工具配置与上游真源之间的对位规则
- `projects/<项目名>/视频/` 阶段落点

### `6-视频` 不拥有

- 重新改写 `编导/第N集.json` 的剧情或镜头事实
- 替代 `5-画面` 生成图片资产
...
```mermaid
flowchart TD
    A["编导/第N集.json"] --> B{"识别 6-视频 任务类型"}
    C["4-主体 / 5-画面 资产"] --> B
    B -->|"分镜组 -> 视频请求 JSON"| D["subtypes/1-提示词蒸馏/全能参照"]
    B -->|"单帧主导"| E["subtypes/1-提示词蒸馏/首帧参照"]
    B -->|"稳定请求 -> 真实生成入口"| J["subtypes/3-视频生成"]
    B -->|"首尾帧双锚点"| F["subtypes/首尾帧参照（待补）"]
    B -->|"多图故事线"| G["subtypes/多图参照（待补）"]
    D --> H["projects/<项目名>/视频/全能参照/"]
    E --> I["projects/<项目名>/视频/首帧参照/"]
    J --> K["projects/<项目名>/视频/生成任务/<provider>/"]
```
...
- 当前阶段的第一事实源是 `projects/<项目名>/编导/第N集.json`。
- shared schema 固定为 `.agents/skills/aigc/_shared/director_episode_output.schema.json`。
- 阶段级产物统一写回 `projects/<项目名>/视频/`，由命中的子路径承载请求对象或 handoff 包。
- 详细输入合同、canonical landing 与 handoff 见 `references/execution-flow.md`。

## Output Summary

- 当前阶段默认交付不是视频文件，而是“请求 JSON + manifest/说明 + 下一步执行入口”。
- 首个 canonical 主产物为 `projects/<项目名>/视频/全能参照/第N集/第N集.json`。
- 首个 canonical 文本副产物为 `projects/<项目名>/视频/全能参照/第N集/第N集.txt`。
- 帧级 canonical 主产物为 `projects/<项目名>/视频/首帧参照/第N集/第N集.json`。
- 帧级 canonical 文本副产物为 `projects/<项目名>/视频/首帧参照/第N集/第N集.txt`。
- 生成入口 canonical 计划文件为 `projects/<项目名>/视频/生成任务/<provider>/第N集/submit-plan.json`。
- 生成入口 canonical 简报为 `projects/<项目名>/视频/生成任务/<provider>/第N集/submit-brief.md`。
- 当前共享入参模板真源为 `.agents/skills/aigc/6-视频/_shared/video-generation-input.template.json`，供多个视频子技能包共用。
- 当前共享文本模板真源为 `.agents/skills/aigc/6-视频/_shared/视频生成入参.template.txt`，供多个视频子技能包共用。
...
当出现以下症状时，必须先修 `6-视频` 父级合同：

- 执行者直接跳到视频模型命令，绕过请求 JSON 整理层
- 上游还没锁定 `编导/第N集.json`，却提前生成视频入参
...
- `Rule Source`
  ...
  - `.agents/skills/aigc/6-视频/subtypes/*/SKILL.md`
- `Meta Rule Source`
...
- 进入叶子子路径时，再加载对应 `subtypes/<子路径>/SKILL.md + CONTEXT.md`。
