...
| 节点 | 思维焦点 | 执行动作 | 产物 |
| --- | --- | --- | --- |
| `CAM-01 主收益锁定` | 这段运镜最该放大什么观看变化 | 从 `1-水月 + shot_spine + cinematography_patch` 提取主动作、主情绪、主视线和揭示点 | `movement_intent_note` |
| `CAM-02 默认路线裁决` | 静止是否已经足够；若不足，最小有效运动是什么 | 在静止、最小必要运动、明确移动方式之间择一，并写明不这样会损失什么 | `default_route_note` |
| `CAM-03 叶子并发细化` | 变化、组合、速度各自怎样服务同一目标 | 并发下钻 `变化 / 组合 / 速度`，吸收兼容的上游运动提示 | `movement_variation + shot_combination + speed_profile` |
