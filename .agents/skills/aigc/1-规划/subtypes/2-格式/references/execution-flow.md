...
6. 若命中 `storyboard_script / hybrid_story_text + scene_boundary lock`，则在正文顶部显式写出来源画像，并只在锁轴或上游显式提供时保留 `镜号范围 / 锚点继承`。
7. 若上游含明确镜头语言或可解析场景块标题，先锁定“优先保留镜头语言 + 优先复用原场景结构”的特别处理，不得退回普通叙事源补齐模式。
8. 进入目标子技能，按被选变体为原文添加 `场景标题 + 字段标题`，并生成 `projects/<项目名>/规划/2-格式/第N集.md`；对分镜源应优先整理既有 `*画面` 与已显式给出的 `镜头语言预设`，而不是默认补造新画面或条件字段占位。
...
- 是否已经产出 `2-格式/第N集.md`
- 是否只附加了字段标题，而未改动原文措辞
- 若命中混合源/分镜源，是否显式写出来源画像
- 若 `镜号范围 / 锚点继承` 出现，是否都有上游显式证据或锁轴依据
- 若命中分镜源且上游已有镜头语言，是否优先保留并紧跟相关 `*画面` 字段整理；若上游没有，是否完全省略该字段
