...
def estimate_duration_seconds(text: str) -> float:
    ranges = list(BRACKET_DURATION_RE.finditer(text))
    if not ranges:
        return 0.0
...
def validate_group_baseline_ranges(prefix: str, text: str, errors: list[str]) -> None:
    ranges = [
        ...
        for match in BRACKET_DURATION_RE.finditer(text)
    ]
    if not ranges:
        return
...
def find_first_time_range_index(body: str) -> int | None:
    for index, line in enumerate(body.splitlines()):
        if BRACKET_DURATION_RE.search(line):
            return index
    return None
...
def validate_file(path: Path) -> ValidationResult:
    ...
    for group_index, group in enumerate(groups):
        ...
        if STORYBOARD_PICTURE_LABEL in group.body:
            errors.append(
                f"{prefix} must inherit original visual field titles from 4-摄影 and must not introduce standalone {STORYBOARD_PICTURE_LABEL}"
            )
        ...
        if visual_style_index is None:
            ...
            if first_time_range_index is not None and visual_style_index >= first_time_range_index:
                errors.append(
                    f"{prefix} {VISUAL_STYLE_LABEL} must appear before the first [起始秒-结束秒] timecoded visual line"
                )
        ...
        if legacy_duration_matches:
            errors.append(
                f"{prefix} contains legacy 分镜N（约X秒） durations; return to 4-摄影 and migrate to inherited visual fields with [起始秒-结束秒] ranges"
            )
        duration_seconds = estimate_duration_seconds(countable_text)
        validate_group_baseline_ranges(prefix, countable_text, errors)
        if duration_seconds == 0:
            warnings.append(
                f"{prefix} contains no [起始秒-结束秒] time ranges; semantic review should return to 4-摄影 to restore canonical timing"
            )
