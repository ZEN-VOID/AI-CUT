...
| `risk_focus` | `readiness`、`dialogue_fidelity`、`format_preservation`、`quantization`、`reference_sync` | 主要风险 |
...
| 用户要求“2-剧本/标准剧/对白旁白独白整理/剧本主稿” | `script_format` | `references/script-format-contract.md` | `scripts/validate_script_output.py` | script validator |
...
1. 若用户只说“执行/继续/完成规划阶段”或直接点名 `.agents/skills/aigc/1-规划`，且没有显式要求在分集、格式或分组处停住，则判为 `full_chain`：从最早缺口开始，连续跑到 `grouping + stage_validation`。
...
- 不得因为 `1-分集 / 2-格式 / 3-分组` 名称仍是项目 runtime 子目录，就把它们重新判为三个 skill 包；其中 `2-格式` 在业务口径中称为 `2-剧本`。
