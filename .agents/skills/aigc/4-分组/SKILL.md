...
`4-分组` 负责把 `3-摄影` 的逐集摄影稿切分为可供后续设计、图像和视频阶段消费的完整分镜组。它不改写上游剧本正文和原有镜头语言，只裁决组边界、补入组间衔接用的入场/出场画面、拼接项目 `north_star.yaml` 的风格字段，使用内部空间锚点在首次锁定或发生变化的分镜补入站位/位移辅助行，最后在组底部附加统计 YAML。
...
flowchart TD
    ...
    P2 --> P3["PASS-GROUP-03 空间一致性账本"]
    P3 --> P4["PASS-GROUP-04 边界裁决"]
    P4 --> P5["PASS-GROUP-05 补位画面设计"]
    P5 --> P6["PASS-GROUP-06 统计抽取"]
    P6 --> P7{"PASS-GROUP-07 落盘与审查"}
    P7 -->|"PASS"| O["4-分组/第N集.md + 执行报告.md"]
    P7 -->|"FAIL-GROUP-03 站位位移无证据或不连续"| P3
    P7 -->|"FAIL-GROUP-05/06 边界或 atomic unit 错误"| P4
    P7 -->|"FAIL-GROUP-07 桥接不连续"| P5
    P7 -->|"FAIL-GROUP-08 YAML 统计错误"| P6
```
...
4. 按 `references/spatial-consistency-contract.md` 为每个分镜组提取 1 个或多个内部空间锚点，并建立不输出的空间连续性账本：本组入场状态、上一条分镜状态、当前分镜证据、允许发生的空间变化和本组出场状态。随后在首次锁定或发生变化的 `分镜N:` 明细下方补入 `站位和位移：` 辅助行，格式为 `[明确角色名][站在/坐在等表示位置关系的动词][空间参照][方位]，[如有移动，其方向如何；涉及多角色时明确前后、左右、内外、近远或动作先后的顺位关系]`。主语必须是明确角色名或上游已命名的稳定群体称谓，不得使用 `画面主体`、`主体`、`人物`、`角色`、`主角`、`他/她/他们/她们` 等含混指代。该辅助行必须关联到具体分镜明细，不得只挂在整个 `镜头语言：` 容器下，也不得删改原有镜头语言；有移动时必须能回指当前分镜原文、上一条分镜状态或入场/出场补位画面，不得把心理变化、镜头运动或句式变化误写成角色位移。若连续多个分镜的站位、位移、朝向和多角色顺位没有变化，只需保留首次锁定，不必在每个分镜头重复输出。
...
| `scripts/validate_storyboard_groups.py` | 可选机械校验：检查分镜组 ID、风格字段、分镜级站位位移首次锁定、禁止输出 `空间锚点：` 字段、入场/出场标题、YAML 统计、字数上限和编号连续性；不能替代站位位移的剧情证据、空间连续性和多角色顺位审查 |
...
| `FIELD-GROUP-03` | 空间一致性 | 每组内部提取空间锚点但不输出 `空间锚点：` 字段；建立不输出的空间连续性账本；首次锁定或发生变化的 `分镜N:` 明细下方含非空 `站位和位移：` 辅助行，站位位移需以明确角色名为主语，包含空间参照、方位/运动方向和多角色顺位关系，并能从上一条分镜、本组入场画面或当前分镜证据连续推导；连续分镜完全无变化时不重复输出 | `FAIL-GROUP-03` |
...
| `FIELD-GROUP-07` | 原文保真 | `3-摄影` 划定正文同步原换行，不删改字段、对白、原有镜头语言或场景顺序；只允许在首次锁定或发生变化的对应 `分镜N:` 下方新增 `站位和位移：` 辅助行 | `FAIL-GROUP-07` |
...
flowchart LR
    ...
    E2 --> T3{"站位位移能否由空间账本连续推导"}
    T3 -->|"是"| E3["生成 spatial_continuity_plan"]
    T3 -->|"否"| R3["回 spatial-consistency-contract 重建账本"]
    E3 --> T4{"边界是否满足对白/字数/atomic unit"}
    T4 -->|"是"| E4["生成 group_boundary_plan"]
    T4 -->|"否"| R4["移动完整 atomic unit"]
    E4 --> T5{"出场与下一入场是否同画面且可接续"}
    T5 -->|"是"| E5["生成 bridge_shot_pair"]
    T5 -->|"否"| R5["成对重写补位并回查首条站位"]
    E5 --> T6{"统计是否可回指正文证据"}
...
| `PASS-GROUP-03` | 空间一致性 | 场景、画面字段、镜头语言、分镜明细、补位画面 | 内部空间参照、上一组出入场状态和当前分镜证据是否能连续推导首次锁定及变化分镜的站位、方位、运动方向和多角色顺位 | `spatial_continuity_plan` |
...
| `PASS-GROUP-03` | 提取内部空间锚点，建立不输出的空间连续性账本，并为首次锁定或发生变化的分镜明细补入站位位移 | 内部空间锚点列表、入场/上一分镜/当前证据/允许变化/出场状态、首次锁定及变化 `分镜N:` 的 `站位和位移：` 辅助行 | `references/spatial-consistency-contract.md` |
...
- 输出了独立 `空间锚点：` 字段，或内部空间锚点不是空间位置参照点，或缺少首次站位锁定，或站位位移未使用明确角色名、未说明方位/运动方向/多角色顺位，或站位位移不能从上一条分镜、本组入场/出场画面和当前分镜证据连续推导，或在连续无变化分镜中重复堆叠站位位移。
...
3. 每个分镜组必须包含：`[分镜组ID]`、组头三项 north_star 风格纯内容、从 `3-摄影` 划定的分镜剧本正文、首次锁定及变化分镜的 `站位和位移：`、`出场画面：`、组底 YAML 统计；第 2 组起还必须包含 `入场画面：`。不得输出独立 `空间锚点：` 字段。
4. 每集第 1 个分镜组直接省略 `入场画面：` 段，不写 `无`，不占用字数；后续组的 `入场画面：` 必须与上一组 `出场画面：` 为同一画面内容。
5. 分镜剧本正文必须同步原换行，不改写 `3-摄影` 的字段、对白、原有镜头语言或场景顺序；`站位和位移：` 只能作为新增空间一致性辅助行补入到首次锁定或发生变化的对应 `分镜N:` 明细下方。
...
- 每组不输出独立 `空间锚点：` 字段；首次锁定及变化分镜下方包含非空 `站位和位移：` 辅助行，主语为明确角色名，并自然包含空间参照；涉及多角色时明确顺位关系；所有角色移动都能回指上游动作、上一条分镜状态或入场/出场补位画面，连续无变化的分镜不重复输出。
