...
description: Use when the `4-Design/2-设计/道具` leaf skill needs to turn the prop catalog plus `0-Init` and `2-Global` presets into canonical per-prop Markdown design cards, while keeping `道具设计.json` and `prop_design_prompt.json` only as optional compatibility projections.
...
- `projects/aigc/<项目名>/4-Design/道具/2-设计/第N集/<prop_id>-<canonical_name>.md`
- `projects/aigc/<项目名>/4-Design/道具/2-设计/第N集/<prop_id>-<canonical_name>.<ext>`
- `projects/aigc/<项目名>/4-Design/道具/2-设计/第N集/_manifest.json`
- `projects/aigc/<项目名>/4-Design/道具/2-设计/第N集/道具设计.json`（仅在显式兼容导出时）
- `projects/aigc/<项目名>/4-Design/道具/2-设计/第N集/prop_design_prompt.json`（仅在显式兼容导出时）
...
| 载体 | 位置 | 作用 |
| --- | --- | --- |
| 第一输入根 | `projects/aigc/<项目名>/4-Design/道具/1-清单/第N集/道具清单.json` | 本阶段唯一默认 design-source truth |
| 兼容研究 | `projects/aigc/<项目名>/4-Design/道具/1-清单/第N集/道具研究.json` | 仅在存在时补证，不反客为主 |
| 兼容 bridge | `projects/aigc/<项目名>/4-Design/道具/1-清单/第N集/prop_design_bridge.json` | 仅在存在时补证，不反客为主 |
...
flowchart TD
    ...
    F --> G["调用 nano-banana general 生成同目录同名图片"]
...
python3 .agents/skills/aigc/4-Design/2-设计/_shared/scripts/run_design_auto_image.py \
  --design-file "projects/aigc/<项目名>/4-Design/道具/2-设计/第N集/<prop_id>-<canonical_name>.md"
```
...
6. 同目录同 stem 的 `<prop_id>-<canonical_name>.<ext>` 已由 nano-banana general 生成。
7. 逐道具 Markdown、图片与 `_manifest.json` 已同轮落到 `4-Design/道具/2-设计/第N集/`。
...
- `<prop_id>-<canonical_name>.<ext>`
  - 由 `full_generation_prompt` 调用 nano-banana general 生成；是单主体概念图派生产物，不是业务真源
...
- `projects/aigc/<项目名>/4-Design/道具/2-设计/第N集/*.<ext>`
- `projects/aigc/<项目名>/4-Design/道具/2-设计/第N集/_manifest.json`
...
- `.agents/skills/aigc/4-Design/2-设计/道具/templates/prop_masterprompt.structured.v2.md` 如何作为唯一 Markdown 模板真源
...
- 默认进入 `.agents/skills/aigc/4-Design/3-面板/道具`
...
- `Rule Source`
  - `.agents/skills/aigc/4-Design/2-设计/道具/SKILL.md`
  - `.agents/skills/aigc/4-Design/2-设计/道具/CONTEXT.md`
  - `.agents/skills/aigc/4-Design/2-设计/道具/_shared/IO_CONTRACT.md`
  - `.agents/skills/aigc/4-Design/2-设计/道具/templates/prop_masterprompt.structured.v2.md`
