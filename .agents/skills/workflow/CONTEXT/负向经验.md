...
35. 引流素材若已经模糊或低清，不要再放大。CTA 段优先用原始比例证据窗、并列 PiP 或局部面板，并在 plan/DOM 里留下 no-upscale/native-scale 证据。
36. PiP 数量不是只看全片总数。至少要有一组同屏多窗，网格对齐并有可读尺寸；单个很小的 PiP 即使 cue 绑定正确，也不能算严格社媒视觉通过。
37. 大字报/左上角标题先看它服务谁：默认服务短视频平台 C 端用户。标题应从匹配文案 cue 的利益点、反差、结果或行动中提炼；“工作流程”“内部学习交流”“项目复盘”只进报告或工程元数据，不进画面。
