...
`2-编导` 优先消费 `2-编剧` 的逐集剧本，继续转成可拍、可演、可听的编导稿；若项目尚未生成 `2-编剧` 且用户明确直接编导，可回退读取 `1-分集`。旧 `3-导演`、`4-表演` 仍只作为兼容触发词。
...
- 输入：优先 `projects/aigc/<项目名>/2-编剧/第N集.md`；缺失且用户明确直接编导时回退 `projects/aigc/<项目名>/1-分集/第N集.md`
