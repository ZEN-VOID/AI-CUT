...
3. 每个分镜组必须以 `## x-y-z` 作为分镜组标题，并在标题后先写当前场景标题行，例如 `场景1：外景 扶桑战船外舷与黑礁 - 夜`；即便新的分镜组没有切换场景，也必须重复同一个场景标题；随后写 `定场镜头：` 自然描述，先覆盖场景身份、环境身份、氛围、光影、色彩和声音底色；再写 `画面构图：`，并用 `左侧：`、`中间：`、`右侧：`、`前景：`、`中景：`、`背景：` 等至少两个有内容的构图分区，把全部可见或需要生成角色、群体、关键道具和场景锚点布置成带形容属性、状态属性、站位属性、空间方位关系和朝向的主体账本，不另列 `角色：` / `场景：` / `道具：` 组头清单；每集第二组起必须在保留上述站位关系的基础上，增量添加上一组尾帧还原入场，具体复现上一组末帧的可见状态，并说明本组如何从同一状态以不同景别、机位、焦点、构图或观看距离进入；再写画面属性自然语句（从组内镜头设计提炼，覆盖构图布局核心选择、构图方式关键子维度、光源效果、色彩基调和关键摄影技术参数）、组头三项 north_star 风格纯内容、从 `5-摄影` 划定的分镜剧本正文和组底 YAML 统计。
...
- 每组标题后先写当前场景标题行，未切换场景的新分镜组也重复同一场景标题；随后写 `定场镜头：`，先覆盖场景身份、环境身份、氛围、光影、色彩和声音底色，再写 `画面构图：` 和至少两个有内容的构图分区，把全部可见或需要生成角色、群体、关键道具和场景锚点布置成带形容属性、状态属性、站位属性、空间方位关系和朝向的主体账本，且不退化为人物动作摘要、不新增无互动道具氛围镜头、不另列 `角色：` / `场景：` / `道具：` 组头清单；每集第二组起定场镜头必须在保留站位关系的基础上增量添加上一组尾帧还原入场，具体复现上一组末帧可见状态，并把尾帧作为本组开端牵引整组空间关系；再写画面属性语句，位于定场镜头之后、north_star 风格行之前，覆盖构图布局核心选择、构图方式关键子维度、光源效果、色彩基调和关键摄影技术参数；随后投影 `全局风格.全局风格提示词`、`类型元素.类型元素提示词`、`细分风格.画面风格`，第 1 行风格行以固定前置词 `视频生成的画面风格，光影和氛围与场景参照图保持一致。需要生成现场物理互动音效、氛围感音效、环境声、自然现象声、动作声，不要生成任何字幕，不要生成背景音乐。` 开头，后接全局风格原词，且组头不显示标题字段、中文括号或装饰性连接符。
