#!/usr/bin/env python3
"""Run the prop design pipeline for 4-Design/道具."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 4-Design/道具 pipeline，默认以 `道具清单.json` 为第一输入根。"
    )
    ...
    parser.add_argument(
        "--output-dir",
        help="输出目录；默认推断到 `projects/aigc/<项目名>/4-Design/`",
    )
...
def infer_output_dir(catalog_path: Path) -> Path:
    return infer_project_root(catalog_path) / "4-Design"
