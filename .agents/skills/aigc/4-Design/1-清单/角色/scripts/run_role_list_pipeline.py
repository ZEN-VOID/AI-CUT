#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run the role list pipeline for 4-Design/角色/1-清单."""
...
EXTRACT_SCRIPT = SCRIPT_DIR / "extract_episode_roles.py"
RESEARCH_SCRIPT = SCRIPT_DIR / "build_role_research.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="从 `projects/aigc/<项目名>/3-Detail/第N集.json` 生成角色三真源。"
    )
    parser.add_argument("--input", required=True, help="输入 episode JSON")
    parser.add_argument("--output-dir", help="输出目录；默认推断到 `projects/aigc/<项目名>/4-Design/角色/1-清单/第N集/`")
    ...
    parser.add_argument("--bridge-name", default="role_design_bridge.json", help="桥接 JSON 文件名")
...
def resolve_output_dir(input_path: Path, explicit: Optional[str]) -> Path:
    ...
    try:
        ...
        return project_root / "4-Design" / "角色" / "1-清单" / input_path.stem
    except ValueError:
        pass

    return input_path.parent / "4-Design-角色-1-清单" / input_path.stem
...
def write_manifest(args: argparse.Namespace, input_path: Path, output_dir: Path) -> Path:
    ...
    manifest = {
        ...
        "output_files": [
            ...
            (output_dir / args.report_name).as_posix(),
            (output_dir / args.manifest_name).as_posix(),
        ],
        "statistics": catalog.get("statistics", {}),
        "notes": [
            "业务三真源为 `角色清单.json + 角色研究.json + role_design_bridge.json`；`validation-report.md` 为 domain 验收摘要，`_manifest.json` 仅为审计侧车。",
        ],
    }
...
def run_dry_run(args: argparse.Namespace, input_path: Path) -> int:
    with tempfile.TemporaryDirectory(prefix="aigc-role-list-") as tmp_dir_str:
...
def run_normal(args: argparse.Namespace, input_path: Path) -> int:
    ...
    print(f"- 角色清单: {(output_dir / args.catalog_name).as_posix()}")
    print(f"- 角色研究: {(output_dir / args.research_name).as_posix()}")
    print(f"- 设计桥接: {(output_dir / args.bridge_name).as_posix()}")
    print(f"- 验收报告: {(output_dir / args.report_name).as_posix()}")
