...
本文件定义 step1：以 `projects/aigc/<项目名>/5-分组` 为主要信息来源，获取每个分镜内容，并理清其关联剧情桥段信息。

## Source Priority

1. `projects/aigc/<项目名>/5-分组/第N集.md` 是镜级 prompt 的剧情与镜头主要真源。
2. `projects/aigc/<项目名>/0-初始化/north_star.yaml` 只提供风格直引，不替代分镜内容。
3. `6-设计/*/3-生成` 只提供主体参照候选，不改写剧情。
4. `4-摄影` 仅在用户要求上溯修复 `5-分组` 时读取。

## Frame Landing ID Mapping

`5-分组` 中的标题 `## x-y-z` 表示三段式分镜组 ID：

- `x`: episode number
- `y`: scene number
- `z`: group number inside the scene

本技能生成的是 `分镜帧 / storyboard frame`，不是上游视频运镜单元的一对一翻译。组内 `分镜1`、`分镜2` 等标签是 `5-分组` 阶段围绕视频运镜中心形成的处理结果，只能作为 frame landing 的证据之一，不得直接等同于本阶段的 `shot_id`。
...
- `## x-y-z~x-y-z` 是 `5-分组` 的组间连接件标题，不是三段式分镜组，也不映射四段式 `shot_id`。
- `7-图像/A-分镜画面` 默认完全忽略连接件：不进入 `story_beat`、`shot_detail`、`bridge_context`、英文 prompt、reference manifest、imagegen plan 或生成图片。
...
| `source_episode_path` | `5-分组/第N集.md` | 可回读 |
...
| --- | --- | --- | --- | --- |
| 是否把 `5-分组/第N集.md` 作为剧情与镜头主要真源，并把 north_star、6-设计生成目录和 4-摄影限制在各自辅助边界内？ | `G1-SOURCE` | `FAIL-FRAME-ID` | `N2-CONTEXT` / `N3-SHOT-INDEX` | `input manifest` 记录源文件路径、north_star 字段用途、6-设计目录只作参照候选；报告说明未回到 4-摄影改写分组。 |
