...
GLOBAL_APP_DIR = "story2026"
LEGACY_GLOBAL_APP_DIR = "webnovel-writer"
...
def _preferred_global_env_path() -> Path:
    return _get_user_claude_root() / GLOBAL_APP_DIR / ".env"


def _legacy_global_env_path() -> Path:
    return _get_user_claude_root() / LEGACY_GLOBAL_APP_DIR / ".env"


def _migrate_legacy_global_env_if_needed(legacy_env: Path, preferred_env: Path) -> None:
    if not legacy_env.exists() or preferred_env.exists():
        return
    try:
        preferred_env.parent.mkdir(parents=True, exist_ok=True)
        preferred_env.write_text(legacy_env.read_text(encoding="utf-8"), encoding="utf-8")
    except Exception:
        return
...
def _load_dotenv():
    ...
    约定：
    - 项目级 `.env`（当前工作目录下）优先；
    - 全局 `.env` 作为兜底：优先 `~/.claude/story2026/.env`，兼容旧 `~/.claude/webnovel-writer/.env`
    """
    # 1) 当前目录（常见：用户从项目根目录执行）
    _load_dotenv_file(Path.cwd() / ".env", override=False)

    # 2) 用户级全局（常见：skills/agents 全局安装，API key 放这里最省心）
    preferred_global_env = _preferred_global_env_path()
    legacy_global_env = _legacy_global_env_path()
    if _load_dotenv_file(preferred_global_env, override=False):
        return
    if _load_dotenv_file(legacy_global_env, override=False):
        _migrate_legacy_global_env_if_needed(legacy_global_env, preferred_global_env)
