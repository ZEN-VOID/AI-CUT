...
source_scene_list: `projects/aigc/<项目名>/3-主体/场景/1-清单/场景清单.md`
source_type_style: `projects/aigc/<项目名>/2-美学/类型风格.md`
source_visual_tone: `projects/aigc/<项目名>/2-美学/画面基调/全局风格协议.md`
source_scene_style: `projects/aigc/<项目名>/2-美学/第N集/场景风格/场景风格协议.md` 或 `projects/aigc/<项目名>/2-美学/场景风格/场景风格协议.md`
...
- 本模板只定义结构和证据槽位，不生成研究、物语、Scene Design、Cinematography、prompt evidence chain 或英文提示词。
- 禁止脚本批量生成、批量插入、正则套句或映射投影创作正文；每个场景必须由 LLM 逐条理解上游清单、项目资料和 `type_profile` 后落盘。
...
research_brief:
  ...
  source_posture:
    project_source:
      - "{{来自场景清单 / 2-美学 / MEMORY.md / CONTEXT/ 的强约束}}"
    user_source:
      - "{{用户本轮补充；无则写 none}}"
    common_knowledge:
...
- 场景风格引用：{{优先引用 projects/aigc/<项目名>/2-美学/第N集/场景风格/场景风格协议.md 中的 Scene Style Prompt；缺失时回退 projects/aigc/<项目名>/2-美学/场景风格/场景风格协议.md，并记录 fallback}}
- 主体 ID 号：{{场景主体 ID，例如 S###；若上游清单已有 ID 则原样沿用}}
- 时间与地域引用：{{引用 research_brief / type_profile / 上游清单 / 项目资料中的时间与地域锚点；若具体信息不确定，写入有来源姿态的保守英文锚点，不得编造具体年代、城市、国家、族群或宗教}}
...
| style_anchor | {{2-美学/类型风格 / 2-美学/画面基调 / 2-美学/场景风格 / MEMORY.md / user_source}} | {{类型风格、画面基调、场景风格与项目记忆如何转为可见色彩、质感、构图或氛围}} | {{无 / 保守化 / 非特指化}} |
| period_region_tokens | {{research_brief.source_posture / type_profile / user_source}} | {{最终英文 prompt 中必须显式出现的时间与地域英文 token}} | {{无 / 保守化 / 非特指化}} |
| space_style_token | {{type_profile.scene_type / Scene Style Prompt / research_brief / scene_inference}} | {{按场景类型选择建筑 / 室内 / 街区 / 自然地理 / 材质系统 / 超现实规则 / 交通空间风格 token}} | {{无 / 保守化 / 非特指化；非建筑场景不得强行写建筑流派}} |
...
```text
{{英文整合提示词必须以主体 ID 号开头，格式为 "<主体ID>: ..."; 整合对象是 4. 解构的全部有效信息，而不是只拼接主体 ID、风格、时间地域和负向词；必须把 Scene Design 与 Cinematography 中的空间结构、尺度边界、材质表面、色彩陈设、动线、镜头距离、构图、光线、焦段、景深和氛围节奏蒸馏组织为自然流畅、可生成画面的英文整合提示词，2000 字符以内；必须显式包含有来源姿态的时间 token、地域 token，并按场景类型包含建筑/室内/街区/自然地理/材质系统/超现实规则/交通空间等风格 token；非建筑场景不得强行写建筑流派；必须包含 pure empty shot / no people / no human figures 约束。}}
```
...
| Output path | canonical path 为 `projects/aigc/<项目名>/3-主体/场景/2-设计/S###-<场景名>.md`。 |
| Naming convention | `S###` 来自上游清单顺序，场景名使用清单 canonical 名称并替换非法文件名字符。 |
| Completion gate | 场景来自上游清单；已消费 `2-美学/类型风格.md`、`2-美学/画面基调/全局风格协议.md`、当前集优先/项目级回退的 `2-美学/场景风格/场景风格协议.md` 与 `MEMORY.md`/`project_memory_init_context`；正文由 LLM 创作；研究层包含 research_brief / source_posture / uncertainty_register / visual_translation；`## 4. 解构` 下的主体 ID、`## 5. 提示词设计` 的主体 ID 和英文 prompt 开头三者一致；英文 prompt 以主体 ID 号开头，有 prompt_evidence_chain，已整合 `## 4. 解构` 的 Scene Design 与 Cinematography 全部有效信息，融合 `画面基调.Global Style Prompt + 场景风格.Scene Style Prompt`、有来源姿态的时间与地域锚点、按场景类型选择的空间风格 token，且不超过 2000 字符；画面固定为纯空镜，无人物、人体局部、剪影或人群。 |
