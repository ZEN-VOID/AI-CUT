...
| --- | --- | --- | --- | --- |
| prompt 只有主体参照，缺少故事板总参照绑定 | prompt 组装层 | 回到 source-first 两阶段，final 按生成槽位在 fenced YAML 写入 `故事板参照.reference_index / uploaded_url / image_token`；故事板用途说明放在远端 `【直接生成请求】` | `hybrid-prompt-assembly-contract.md` 固定 source-first YAML 绑定 | prompt 以原组标题起笔，YAML 中出现故事板总参照绑定 |
...
| 主体参照只进了 manifest，没有进入对应 YAML 主体项 | prompt 绑定层 | 回到 source-first YAML，给对应角色/场景/道具补 `name + reference_index + uploaded_url + image_token` | review gate 检查 bound subject YAML binding | 每个 bound subject 在 fenced YAML 中可定位 |
...
| OSS 上传和视频图N槽位被混为一谈 | 上传身份层与生成槽位层混淆 | 将 `asset_uploads: 故事板/主体身份 -> uploaded_url` 和 `generation_slots: 图N/mixedList[n-1] -> uploaded_url -> 故事板/主体身份` 分开；用实际 `mixedList` 顺序回刷 YAML `reference_index` | `hybrid-prompt-assembly-contract.md` 与 `libtv-handoff.md` 固定 OSS 上传不承载图N顺序真源 | `reference_index` 可由 `generation_slots` 反查，不依赖上传顺序 |
...
15. 若当前调用面不能在生成前直接控制或验证 `params.enableSound`，记录 `audio_preflight_unverified_non_blocking`，不要阻断官方 `$libTV` Agent-IM 提交流程；生成后必须用 `task_result.audios` 或 `ffprobe` 做硬验收。
...
- 主体参照在本地审核 prompt 中必须落在 final fenced YAML 对应主体列表项里，写成 `name + reference_index + uploaded_url + image_token`；远端提交复用该 YAML，不要人工预设 `参照图N`，这样 LibTV 和后续审查都能知道每张图锁定谁且不和真实导入编号冲突。
- D 路线必须分清上传身份和生成槽位：`asset_uploads` 只回答“这张 OSS 图是故事板总参照还是哪个主体”，`generation_slots` 才回答“图N/mixedList[n-1] 是哪张 OSS 图”。最终 YAML 的 `reference_index` 以 `generation_slots` 为准。
- 若视频生成框 UI 缩略图顺序可观测，UI 图N / `Image N` 优先级高于 OSS 上传顺序和旧 query 回显；`prompt.md` 初始应保持 draft，不提前写 `reference_index / uploaded_url`。
...
- “参照图 URL”这个说法容易让远端把故事板和主体图当成匿名素材。D 远端请求应始终让 `【分镜组源文本】` 的 final fenced YAML 同时持有故事板参照和主体的 `reference_index / uploaded_url / image_token` 绑定；不得另起混合参照说明段，也不得人工预设 `参照图1/2/N` 编号。
