...
- 本文件是 `4-Design/4-道具/1-清单` 的经验层知识库，不是过程日志。
...
4. 再检查研究层是否把材质、功能、镜头路由和物理性格一起交给桥接层。
5. 最后检查输出目录是否稳定落到 `4-Design/4-道具/1-清单/第N集/`。
...
- outcome: 为空的 `.agents/skills/aigc/4-Design/4-道具/1-清单/` 建立了首份可执行叶子合同与脚本链。
...
- final_fix_or_heuristic: 保留“抽取 -> 研究 -> bridge”三段式结构，但把上游改为 `3-Detail/第N集.json`，把下游改为 `4-Design/4-道具/1-清单/第N集/`，并在 runner 中兼容 `3-Detail/第N集.json`。
