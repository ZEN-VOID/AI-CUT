...
主体参照流是 `libTV画布流` 的默认路线。本文件只定义计划层合同：从 `6-分组` 抽取分镜组、绑定主体参照图、生成 CLI handoff plan、建立审查证据。真实 LibTV 画布操作由最新版 `.agents/skills/cli/libTV` 执行。
...
1. 按 canonical reference order 生成 `left_input_edges[]`。
2. 用 `libtv node <视频节点> --left <主体图节点>` 或 `--left-add <主体图节点>` 把主体图节点连到视频节点左侧。
3. 在 `video_node.params.prompt` 中用 `{{Image 1}}`、`{{Image 2}}` 这类官方占位引用已连线图片。
4. `{{Image N}}` 的主体语义必须由 `主体绑定表.image_index=N`、`left_input_edges[N-1].node_key` 和执行后查询到的左侧连线顺序共同证明。
...
2. 读取项目级设计清单，建立 `YAML 主体名 -> 设计 ID / canonical_name / source_aliases / design_file` 映射：
   - `projects/aigc/<项目名>/7-设计/角色/design-manifest.yaml`
   - `projects/aigc/<项目名>/7-设计/场景/design-manifest.yaml`
   - `projects/aigc/<项目名>/7-设计/道具/design-manifest.yaml`
...
5. 只有画布节点和 active registry 都不能唯一命中，才进入本地 `7-设计/*/3-生成` 兜底上传流程。
...
预算裁决先决定哪些主体进入本轮参考图；排序再按上述 YAML 展示顺序保留选中主体的相对位置。`subject_bindings`、CLI handoff 的参考绑定表、`left_input_edges[]`、`image_placeholder_map[]`、`source_node_keys`、`source_node_url_mapping`、`imageList/mixedList` 必须一致采用该顺序。

`{{Image N}}` 编号来自左侧连线顺序，不是自然语言推断。计划层必须把 canonical order 投影为：
...
主体语义仍以 `yaml_name + category + node_key + URL + image_index` 绑定为准；`Image N` 只有在左侧连线顺序已验证时才可作为 prompt 内引用。
...
1. `cli_handoff_plan`
   ...
   - 参考数组按 canonical reference order 传入，即 YAML `角色` 原顺序 -> YAML `场景` 原顺序 -> YAML `道具` 原顺序。
   - 新建视频节点时优先在同一命令或同一计划中按顺序传入全部 `--left <node_key>`；更新既有视频节点时，必须先清理会造成编号漂移的旧左侧输入，再按 canonical order 使用 `--left-add` 顺序追加，并查询节点确认连线顺序。
2. `video_node.params.prompt`
   - 只能包含 `## x-y-z` 下的 Markdown 正文 + 底部完整 fenced YAML 内容 + 已验证 `{{Image N}}` 占位。
   - 不包含 `YAML主体清单`、`主体绑定表`、参考图清单、执行锁、生成参数、文件路径、missing/excluded 诊断或审计说明。
   - 底部 YAML 必须完整保留原字段和值，包括 `字数统计`、`时长估算`、`角色`、`场景`、`道具` 等；不得只摘要成主体清单。
   - 每个已绑定 YAML 主体在分镜组正文第一次出现时，在主体名后插入对应占位，例如 `令狐冲 {{Image 1}}`。底部 YAML 中对应主体名也插入同一占位，以便工具参数和 prompt 文本双重对齐。
   - `{{Image N}}` 必须来自 `image_placeholder_map[]`，不得手写未登记编号；占位插入只服务参考图绑定，不改变原文剧情、句序、镜头顺序、动作结果或风格描述。
   - 不得追加 `其中，...` 复述段。
...
- 不得把缺失主体改写成泛化描述来继续提交。
- 不得让执行层依据上传顺序、未验证图片编号或 `imageList` 顺序推断主体身份；编号只能来自已查询验证的左侧连线顺序。
...
- CLI handoff 必须逐项列出每张进入 LibTV 的参考图：`image_index / placeholder / yaml_name / category / node_key / URL / 用途`。
- CLI handoff 必须把所有参考图节点作为视频节点左侧输入连接；`{{Image N}}` 编号必须与左侧输入顺序一致。
...
- 未 opt-in 时，CLI handoff 必须要求 `video_node.params.prompt` 直接使用 `6-分组` 现有组正文 + 完整 YAML，不得要求远端重排、摘要、压缩或改写剧情事实。
...
- `subject-reference-manifest.json`：本组主体绑定、候选、本地路径、消歧证据、预算排除、`left_input_edges[]` 和进入 LibTV 的 `images[] / mixedList` 真源。
- `libtv-submit-plan.json`：本组 prompt 来源、duration/spec、下载策略、远端优化授权、`{{Image N}}` 占位映射和 CLI handoff 计划。
- `queue-record.json`：提交状态、`projectUuid`、`groupNodeKey`、`video_node_key`、执行 skill、命令摘要、下载状态和阻断原因。
...
| prompt 主体是否直接采用 `6-分组` 组正文，没有回到早期阶段重写剧情、镜头顺序、台词或动作结果？ | `REV-LIBTVCANVAS-09` | `FAIL-SOURCE-FIDELITY` | `N2 Group Extraction` / `N3e Prompt Assembly` | source fidelity diff、params.prompt 与组正文对照、无上游重写记录 |
...
| `主体绑定表` 是否包含 `image_index / placeholder / yaml_name / category / canvas_node_name / node_key / URL / usage`，且只有 `{{Image N}}` 占位进入 `video_node.params.prompt`？ | `REV-LIBTVCANVAS-03` | `FAIL-BINDING` | `N3 Subject Binding` / `N3e Prompt Assembly` | handoff plan 绑定表、manifest `subject_bindings`、prompt hygiene check |
| 参考图数组、绑定表、左侧连线和 `{{Image N}}` 映射是否统一使用 canonical reference order？ | `REV-LIBTVCANVAS-04` | `FAIL-ORDER-SAFETY` | `N3 Subject Binding` | canonical order、`left_input_edges[]`、`image_placeholder_map[]`、queried node inputs 对照 |
