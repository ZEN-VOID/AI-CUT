...
- 同步 `.agents/skills/cli/imagegen` 更新口径：分镜故事板生成阶段默认且唯一调用该技能的内置 `image_gen` 路线，CLI/API/provider 专属控制不作为默认或 fallback 路径。
- 批量一集或多组 storyboard sheet 时默认遵循 imagegen subagents 并发模式，最大并发数为 `10`；只有用户显式要求时才主线程逐一执行。
- 将旧 `gpt-image-2` 尺寸合法性描述收束为 imagegen 4K 目标、`layout_aspect_decision`、`panel_geometry_blueprint` 与可读性门禁。
...
- 将分镜平面图职责迁移到 `9-图像/分镜平面图` 独立 Skill 2.0 叶子；`分镜故事板` 不再生成、验收或维护平面图，也不再输出 floor-plan manifest / floor-plans 目录。
...
- 将项目输出根从旧 `9-图像/B-分镜故事板` 收敛为与叶子技能包名一致的 `9-图像/分镜故事板`，并同步 `SKILL.md`、模板、README、CONTEXT、references、types、agents metadata 与父级/registry/query 引用。
...
- 调整 prompt / imagegen 逻辑：以 `8-分组` 对应分镜组完整内容为生图基础，添加本技能任务执行前缀，并按组底 YAML 绑定角色、场景、道具参照图。
...
- 调整 storyboard panel 落点规则：新增 `storyboard_frame_units`，要求基于 `8-分组` 当前组正文中的视觉节拍识别 panel，而不是把原始 `分镜1`、`分镜2` 机械映射为 storyboard 格子；允许 split/merge，但每个 panel 必须能回指源正文。
...
- 将主信息源固定为 `projects/aigc/<项目名>/8-分组`。
