...
`道具/1-清单` 负责从 `4-分组` 逐集分镜组底部 YAML 的 `道具` 字段中提取、归并并生成项目级道具清单。它不重新创作道具设定，不替代后续 `2-设计` 的造型设计，只裁决同一叙事道具在不同分镜组中的别名、代称和首次登场证据。
...
- 上游唯一准确信息来源为 `projects/aigc/<项目名>/4-分组/第N集.md` 每个分镜组底部 YAML 的 `道具` 字段；必要时只允许回查同一分镜组正文作为证据。
...
- 用户要求“道具清单”“从 4-分组提取道具”“生成/修复 5-设计/道具/1-清单”等任务。
- 已完成或部分完成的 `4-分组` 逐集稿；默认处理 `4-分组/` 中全部可读 `第N集.md`。
...
- 上游 `4-分组/第N集.md` 不存在、不可读，且用户没有提供替代分组稿。
...
| mode | 触发信号 | 输出 |
| --- | --- | --- |
| `project_inventory` | 指定项目或默认处理全部 `4-分组/第N集.md` | `5-设计/道具/1-清单/道具清单.md` |
...
2. 锁定上游 `projects/aigc/<项目名>/4-分组/第N集.md`，只从每个分镜组底部 YAML 的 `道具` 字段采集候选项。
...
| --- | --- | --- | --- | --- |
| `PASS-PROP-LIST-01` | 输入锁定 | 项目路径、目标集号、`4-分组/第N集.md` | 是否具备组底 YAML `道具` 字段和分镜组 ID | `input_manifest` |
| `PASS-PROP-LIST-02` | 候选采集 | 逐组 YAML `道具` 字段 | 候选是否只来自上游 YAML，正文是否仅作同组补证 | `prop_candidates` |
| `PASS-PROP-LIST-03` | 道具归并 | 候选项、同组正文关键词、项目记忆 | 别名、代称、状态称呼是否指向同一叙事道具 | `canonical_prop_map` |
| `PASS-PROP-LIST-04` | 杂物过滤 | canonical 候选与正文证据 | 是否具备叙事、规则、视觉钩子或生成锁定价值 | `filtered_prop_map` |
| `PASS-PROP-LIST-05` | 表格落盘 | canonical 映射与关键词证据 | 三列是否固定且无新增道具设定 | `道具清单.md` |
| `PASS-PROP-LIST-06` | 验收回查 | 清单与上游文件 | 来源、归并、过滤、字段和路径是否通过 review gate | `review_result` |

## Pass Table

| pass_id | must_do | evidence | Rework Entry |
| --- | --- | --- | --- |
| `PASS-PROP-LIST-01` | 读取本技能与项目上下文，锁定 `4-分组` 输入 | input manifest | `references/prop-list-contract.md` |
| `PASS-PROP-LIST-02` | 只从组底 YAML `道具` 字段采集候选 | 候选清单与分镜组 ID | `steps/prop-list-workflow.md` |
| `PASS-PROP-LIST-03` | 由 LLM 裁决别名、代称和状态称呼归并 | canonical prop map | `types/prop-type-map.md` |
| `PASS-PROP-LIST-04` | 过滤无生成锁定价值的背景杂物 | filtered prop map | `knowledge-base/prop-list-heuristics.md` |
| `PASS-PROP-LIST-05` | 输出固定三列表格 | `道具清单.md` | `templates/output-template.md` |
| `PASS-PROP-LIST-06` | 执行人工或等价机械验收 | review result | `review/review-contract.md` |
...
`Symptom -> Direct Script/Prompt Overreach -> 道具/1-清单 Section Owner -> 4-分组 YAML Contract -> AGENTS.md LLM-first / Skill 2.0 Rule`
...
- 每个候选道具都可回指到 `4-分组/第N集.md` 的组底 YAML `道具` 字段。
