...
Canonical structured template: `.agents/skills/aigc/11-主体/场景/2-设计/templates/scene_masterprompt.structured.v2.md`
...
| --- | --- | --- | --- | --- |
| 是否使用 `.agents/skills/aigc/11-主体/场景/2-设计/templates/scene_masterprompt.structured.v2.md` 作为结构真源，而不是另起字段顺序、临时模板或脚本生成正文？ | `GATE-SCENE-DESIGN-05` | `FAIL-SCENE-DESIGN-05` | `N6-DESIGN` | 记录采用的 canonical template、输出 section 顺序、任何字段偏离及其修复位置。 |
...
| 最终英文整合 prompt 是否真正吸收 `## 4. 解构` 的全部有效 Scene Design 与 Cinematography 信息，而不是只追加主体 ID、画面基调、场景风格、时间地域、pure empty shot / no people 等前缀或后缀？ | `GATE-SCENE-DESIGN-06` | `FAIL-SCENE-DESIGN-06` | `N6-DESIGN` | 提供 prompt 字符数、prompt token 到解构槽位的覆盖摘要，以及未覆盖项的 blocking finding。 |
