...
INNER_MONOLOGUE_PRONOUN_PATTERNS = [
    "他心想",
    "她心想",
    "他暗想",
    "她暗想",
...
def validate(path: Path) -> tuple[bool, list[str]]:
    ...
    for number, title, scene_lines in scenes:
        ...
        if not any(field_base(field) in VISUAL_FIELDS for field in fields):
            ...
            ok = False
        # Check if last or second-to-last field is a summary-only block
        if len(fields) >= 1 and field_base(fields[-1]) in SUMMARY_ONLY_FIELDS:
            findings.append(
            ...
            ok = False
        if len(fields) >= 2 and field_base(fields[-2]) in SUMMARY_ONLY_FIELDS:
            findings.append(
                f"[ERROR] Scene {number} has summary-only field '{fields[-2]}' as second-to-last field. Integrate performance/blocking details into the relevant beat fields."
            )
            ok = False
            ...
            if base == "心理反应":
                for pattern in DIRECT_EMOTION_PATTERNS:
                    if pattern in value:
                        ...
                        ok = False
            if base == "内心独白":
                for pattern in INNER_MONOLOGUE_PRONOUN_PATTERNS:
                    if pattern in value:
                        findings.append(
                            f"[WARN] Scene {number} inner monologue field '{field}' may use third-person self-reference instead of first-person voice: {pattern}"
                        )
            if base in CONCRETE_CHECK_FIELDS:
                for pattern in HABITUAL_SUMMARY_PATTERNS:
                    if pattern in value:
                        findings.append(
                            f"[WARN] Scene {number} field '{field}' may contain habitual/summary pattern that should be converted to present-tense filmable beat: {pattern}"
                        )
