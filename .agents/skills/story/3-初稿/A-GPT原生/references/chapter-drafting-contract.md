...
8. 若上一章存在，`context pack` / dry-run summary 必须能追溯实际加载的上一章路径；正文 frontmatter 仍不得写入 `previous_chapter_ref`。
...
15. actual creative step 默认由当前 GPT/LLM 会话直接完成；若需要外部 provider，应显式改走对应 provider skill。
16. 正式写作默认启动隔离 team supervision subagents；同为 GPT 时也必须分离主写作上下文与监制上下文。具体模式为读取项目 `team.yaml -> roles.production.members`，把已指定监制组成员作为资深创作顾问逐一请教，并把其创意脑洞、个人风格判断和可执行指导汇流为写作前额外上下文。
17. `scripts/write_chapter_gpt_native.py` 只能装配上下文、注入 `supervision_packet`、输出 sidecar、校验已创作稿与写回，不得直接生成正文。
18. 若 GPT 原生输出不含完整 YAML frontmatter、`写作模型` 不等于 `GPT`、缺少 `字数: XXX字`、缺少 `# 第N章｜章标题` 标题行，或句式重复门禁失败，必须判定为 output invalid，禁止直接写回业务真源。
...
- GPT 原生流正文 frontmatter 保持极简，只记录写作模型与字数；上一章路径、连续性桥和上下文引用由 context pack、GPT-native sidecar 与 dry-run summary 追溯。
