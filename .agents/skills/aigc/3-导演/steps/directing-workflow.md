...
| `constraint_profile` | 不改写剧情事实、不修改对白、不改变场景顺序和字段标签；只在既有剧本字段内部组织和强化导演判断；导演创作内核、高潮画面识别、视觉主轴、画面美学、氛围意境、终结画面、受控增强留证、跨集连续性、LLM-first、顾问与复核流程 监制顾问上下文沉淀 |
...
| `topology_fit` | 串行主干 + 类型分支 + 顾问与复核流程的顾问分支 + B 路线受控增强分支 + review 回路 |
...
| `../_shared/team-advisor-consultation-contract.md` | `N5-DIR-ADVISOR` / `N10-DIR-REVIEW` | `advisor_consultation_packet`、`advisor_routeback_targets`、`local_checklist` | `FAIL-ADVISOR-CONSULT` |
...
| `N4-DIR-PEAK` | 高潮画面识别、强化与反高潮计划 | `director_substance_plan`、`audience_psychology_map`、上游段落、字段映射、`references/climax-visual-treatment-contract.md`、`references/anticlimax-strategy-contract.md`、`references/hollywood-quality-spec.md` | 识别 1-3 个上游高点或最强 `micro_payoff`，锁 `source_evidence / audience_desire / promise_source / character_anchor / payoff_mode / build_up / delivery_action / satisfaction_delta / visual_payload / audio_payload / cost_or_aftershock`；判断是否采用 `deliberate_anticlimax / delayed_gratification / pyrrhic_payoff / false_payoff / interrupted_payoff / reversed_payoff`，并记录残余张力 | `peak_visual_plan`、`peak_visual_candidates`、`micro_payoff`、`anticlimax_directive` | `N5-DIR-ADVISOR` | 高点可回指上游，强化或反高潮只改变满足方式、画面/声音/表演/空间/余波密度和残余张力，不新增事实、对白、胜负、死亡、反转或因果 |
| `N5-DIR-ADVISOR` | 顾问与复核流程 导演监制参谋汇流 | `team.yaml`、共享顾问合同、上游编剧稿、场景表、`director_substance_plan`、高潮画面计划、项目 `MEMORY.md`、`north_star.yaml`、相关 `CONTEXT/`、当前 `node_id/pass_id/gate_id` | 启动或按不可用说明处理 `team.yaml.roles.supervision.stage_profiles."3-导演"` 或共享合同回退路径中的导演监制顾问；主 agent 将当前思维·执行节点的 judgment、actions、evidence、route_out、gate 与失败回路转化为顾问任务；顾问代入角色意识、创作风格和专业水准参与节点判断、执行取舍、证据补强、回修建议和风险提示；主 agent 汇流为后续任务上下文 | `advisor_consultation_packet`、`advisor_routeback_targets` 或本地 checklist 结果 | `N3-DIR-SUBSTANCE` / `N4-DIR-PEAK` / `N6-DIR-ENRICH` / `N7-DIR-AESTHETIC` | packet 已包含 roster 来源、`node_ref/pass_ref/gate_ref/role_lens`、可执行指导、风险提示、`routeback_targets` 和 `execution_brief`；若顾问发现前置节点证据不成立，必须先回修对应节点 |
...
| `N9-DIR-DRAFT` | LLM 直出逐集导演稿 | 场景表、字段映射、`director_substance_plan`、`audience_psychology_map`、`performance_style_directive`、`peak_visual_plan`、`anticlimax_directive`、`advisor_consultation_packet`、`controlled_enrichment_ledger`、`episode_visual_spine`、`emotional_rhythm_map`、`sound_design_directive`、`visual_aesthetic_plan`、`atmosphere_mood_evidence`、`episode_final_image_plan`、`concrete_visual_language_evidence`、`cross_episode_continuity_profile`、上游编剧稿正文、`../_shared/concrete-visual-language-contract.md` | 在完整保留上游编剧稿结构、字段标签、对白和顺序的前提下，把导演创作内核、观众心理、表演风格基调、高潮/反高潮处理、声音设计、情绪节奏、顾问参谋、受控增强、视觉主轴、画面美学、氛围意境和终结画面尾钩拆入对应句段的既有字段；落笔前清除抽象审美词、概念化主题句、人物状态标签和解释性导演意图，把它们改成看得见的物、动作、空间方向、光/材质变化、可听声音、可演停顿或余波；模板占位和内部规则只能指导写作，不得输出到正文 | `第N集.md` 导演稿草稿、`faithful_directing_trace`、`concrete_visual_language_evidence` | `N10-DIR-REVIEW` | 不改写上游剧情事实、对白、场景标题和字段顺序；导演判断以具像创作干货内嵌，不以"已执行XX pass"或抽象概念总结形式泄露；无第二字段体系，无场景末尾总结块 |
| `N10-DIR-REVIEW` | 保真、戏剧实质、观众心理、表演风格、高潮/反高潮、声音设计、情绪节奏、具像画面语言、视觉主轴、画面美学、氛围意境、终结画面、受控增强边界、顾问汇流、思维·执行节点与质量门禁 | candidate 导演稿、上游编剧稿正文、`review/review-contract.md`、各节点证据、`concrete_visual_language_evidence`、`thinking_action_node_ledger`、`learning_integration_review_evidence` | 运行机械校验或人工 review；逐项执行 `GATE-DIR-01..17`；检查每个关键节点是否具备 `judgment_question / actions_taken / evidence_keys / route_out / gate_status / source_owner`；检查导演稿、顾问意见、报告证据和最终字段是否把上游画面化成果重新抽象化；把 finding 映射到最早责任节点和 source owner | 校验结果、问题清单、`thinking_action_node_ledger`、`learning_integration_review_evidence`、`gate_to_node_repair_map`、repair targets | `N10R-DIR-REPAIR` 或 `N11-DIR-WRITEBACK` | 无阻断项才可写回；质量建议不得掩盖保真、戏剧实质、观众心理、表演风格、高潮/反高潮、声音设计、情绪节奏、具像画面语言、视觉主轴、画面美学、氛围意境、终结画面、受控增强越权或节点退化问题 |
...
- 若执行顾问与复核流程，`N5-DIR-ADVISOR` 必须在 `N6-DIR-ENRICH`、`N7-DIR-AESTHETIC` 与 `N9-DIR-DRAFT` 前完成；顾问参谋必须绑定当前思维·执行节点并只转化为 `advisor_consultation_packet` 上下文，不直接写正文，不替换上游事实、对白或事件顺序。
...
- 若 `concrete_visual_language_evidence` 或 `conceptual_regression_risk_map` 发现导演判断、顾问意见、报告证据或终稿字段出现抽象审美词、概念化主题句、人物状态标签或解释性导演意图，必须回到最早责任节点：戏剧实质空泛回 `N3-DIR-SUBSTANCE`，画面/氛围/声音/终结画面抽象回 `N7-DIR-AESTHETIC` 或 `N8-DIR-FINAL-IMAGE`，投影落盘抽象回 `N9-DIR-DRAFT`。
...
| 顾问与复核流程 启用但缺 team.yaml 监制顾问请教、节点锚点、个人风格参谋或上下文沉淀 | `N5-DIR-ADVISOR` |
| 顾问指出戏剧实质或高点前置证据不成立，但流程仍继续下游 | 最早责任节点：`N3-DIR-SUBSTANCE` / `N4-DIR-PEAK` |
| 导演判断、视觉主轴、氛围意境、高潮承托、终结画面、顾问意见或执行报告把已画面化内容重新写成抽象审美词、概念标签、人物状态总结或导演意图解释 | 最早责任节点：`N3-DIR-SUBSTANCE` / `N7-DIR-AESTHETIC` / `N8-DIR-FINAL-IMAGE` / `N9-DIR-DRAFT` |
