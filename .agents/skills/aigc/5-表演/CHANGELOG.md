...
- 接入 `../_shared/upstream-context-application-contract.md`，要求表演稿证明 `4-导演` 批注意图和 `3-美学` 三类协议如何融合为同字段表演正文。
...
- 固化核心输出口径：默认消费 `4-导演` 导演批注稿和 `3-美学` 的画面基调、角色风格、场景风格；保留原字段标题，删除导演批注，将批注意图融合为画面化表演正文。
