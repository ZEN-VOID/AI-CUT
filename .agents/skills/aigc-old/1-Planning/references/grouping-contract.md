---
name: aigc-planning-reference-grouping
description: Preserved and enhanced reference contract for the `aigc-planning` grouping mode.
governance_tier: full
---

# aigc 3-分组

> Fusion notice: 本文件已从旧 sibling `SKILL.md` 迁为 `aigc-planning` 的 `grouping` reference。文中“本技能”在当前包内均指 `grouping` mode；运行入口统一由父级 `SKILL.md` 负责。
...
`3-分组` 是 `1-Planning` 单包内的 `grouping` mode。旧版 stage-local parent skill 的完整执行细则在本文件中保留为 mode contract。

当前固定口径：

- 本包 `grouping` mode 自己持有拓扑、量化、validator、写回与节奏复核 gate
...
7. `3-分组` 必须逐行继承 `2-剧本` 的正式影视剧本字段体系；不得把 `环境描写/角色动作/音效/音效画面/道具特写/角色造型/群像画面/系统画面/规则显影/现实灾难画面/心理反应/表演提示` 重新压回 `动作画面`，也不得摘要化改写为剧情梗概。
...
```mermaid
flowchart LR
    A["2-剧本 runtime: 2-格式/第N集.md"] --> Q["grouping_quantizer.py"]
...
- 上游还没有稳定的 `2-剧本` 主稿（runtime: `2-格式/第N集.md`）
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `N1-INPUT-LOCK` | `S1` | `FIELD-GROUP-01` | 锁定唯一上游主稿与当前集范围 | 读取 `2-剧本` 主稿（runtime: `2-格式/第N集.md`）、`episode-split-plan.json`、`north_star.yaml`、`init_handoff.yaml`、manifest | 输入清单、episode scope、上游锚点 | 成功 -> `N2`；输入漂移 -> 回到 `S1` | 输入真源唯一后方可继续 |
...
7. `.agents/skills/aigc/1-规划/references/planning-io-contract.md`
...
- `### 场景N：...` 是从 `2-剧本` 主稿继承的上游场景单位锚点；`3-分组` 只能原样搬运，不得改名、重编号或用分组标题替换。
...
### Upstream Field Inheritance Contract

- grouped script 的正文来源是新版 `2-剧本` 主稿；分组阶段只能插入 `## 【episode-scene-group】` 组标题、必要 frontmatter 与尾钩隐藏标记。
- 除尾钩借入段外，组内字段行应保持 `2-剧本` 的字段标题、文本顺序与正式影视剧本化表达；不得重新把多字段体系合并成单一 `动作画面`。
- 组标题负责表达 beat 切口，不能替代或改写 `2-剧本` 已确定的 `环境描写/音效/音效画面/角色动作/系统画面/规则显影` 等字段。
- 量化器必须把上述正式画面字段纳入 `visual_field` 权重计算；不能因字段标题扩展而落入无语义 fallback。
...
2. 借入内容只允许取下一组开端的首个叙事拍点：
   - `动作画面 / 角色动作 / 环境描写 / 音效 / 音效画面 / 道具特写 / 角色造型 / 群像画面 / 系统画面 / 规则显影 / 现实灾难画面 / 心理反应 / 表演提示` 等正式画面字段取单行；若借入 `音效`，必须尽量连带其紧随的 `音效画面`
...
- grouped script 结构
- 三段式 `分镜组ID`
- 输出中的 `### 场景N：...` 必须能回读 `上游主稿` 并与 `2-剧本` 的场景编号和标题逐项一致，不得由分组阶段生成第二套场景标题
- 默认尾钩策略下，每个非末组必须存在 `尾钩借焰`；缺失即失败
...
1. 锁定 `2-剧本` 主稿（runtime: `2-格式/第N集.md`）为唯一上游主稿。
...
| --- | --- | --- | --- | --- | --- |
| `FIELD-GROUP-01` | 输入锚点 | 锁定 `2-剧本` 主稿（runtime: `2-格式/第N集.md`）与相关索引 | `S1` | 输入真源一致性 | `FAIL-GROUP-01` |
...
- `Rule Source`
  ...
  - `.agents/skills/aigc/1-规划/references/scene-order-duration-strategy.md`
  - `.agents/skills/aigc/1-规划/scripts/grouping_quantizer.py`
  - `.agents/skills/aigc/1-规划/scripts/validate_grouping_output.py`
- `Meta Rule Source`
  - `AGENTS.md`
...
- 当前文件是 `aigc-planning` 单包内 `grouping` mode 的 reference 真源，不再是独立 `SKILL.md` 入口。
- 调用入口统一为 `.agents/skills/aigc/1-规划/SKILL.md` 与 `$aigc-planning`。
- 原 sibling `CONTEXT.md` 已迁入 `knowledge-base/grouping-heuristics.md`。
- 原 grouping 脚本与模板已迁入父级 `scripts/` 与 `templates/`。
