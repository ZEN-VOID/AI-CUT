...
1. **Step 1 / 输入收束**
   ...
   - 统一抽取 `prompt / project_name / task_kind / aspect_ratio / image_size / images / request_id`
2. **Step 2 / 默认值补齐**
   - 保留显式合法值
...
5. **Step 5 / 调用与落盘**
   ...
   - 将输出落到 `output/影片/[项目名]/5-API/image/nano-banana/`
...
- 默认输出目录：`output/影片/[项目名]/5-API/image/nano-banana/`
- 子技能输出目录：默认与输入图（原图）同目录
  ...
  - 若显式传 `--output-dir`，允许覆盖默认目录；推荐项目化路径 `output/影片/[项目名]/5-API/image/nano-banana/[子技能包名]/`
  - 永不覆盖原则：原始文件保持不变
- 目录映射规则：
...
`Symptom/Failure`
-> `Direct Cause`：请求字段不符原生格式、`aspectRatio / imageSize` 缺失、参考图未转 `inline_data`、多任务仍串行、并发值未钳制到 `100` 以内、输出目录未落到项目化路径
