...
| `motion_subject` | 运动主体 |
| `reference_group_id` | source 中的场景段、连续动作段、临时参照簇 ID，或输入源显式自带的分镜组/group_id；不得凭空生成下游分镜编号 |
| `primary_reference_frame` | 同一场景或连续动作段优先继承的主参照系 |
| `reference_frame` | 参照系 |
...
- 同一场景或连续动作段默认继承 `primary_reference_frame`；只有场景切换、参照不可见、主体离开参照范围或动作重心转移时才切换。若输入源显式已有分镜组/group_id，只继承源内边界，不生成下游分镜编号。
...
| 同一场景或连续动作段是否在时间轴上继承主参照系，切换时是否有原因；输入源显式已有分镜组时是否只继承源内边界？ | `GATE-MOTION-04A` | `FAIL-MOTION-REFERENCE-GROUP` | `N3-MOTION-STATE-LEDGER`、`N5R-MOTION-REPAIR` | `group_reference_profile` 与 `reference_switch_reason` |
