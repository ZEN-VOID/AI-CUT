...
- 必须读取上游设计文档：`projects/aigc/<项目名>/7-设计/角色/2-设计/<角色名>.md`；本技能只消费相关设计文档，不重新设计主体。
...
- 已存在的上游角色设计文档目录：`projects/aigc/<项目名>/7-设计/角色/2-设计/`。
...
3. 读取上游 `角色/2-设计` 目标设计文档和可选 `projects/aigc/<项目名>/7-设计/角色/design-manifest.yaml`，抽取角色名称、设计锚点与 `4. 解构` 内容；不得再把 `提示词设计` 的英文整合 prompt 作为导入给 gpt-image-2 的源文本，不得重写角色设定。
...
7. 所有输出落入 `projects/aigc/<项目名>/7-设计/角色/3-生成/`，按命名合同写入；可更新 `design-manifest.yaml` 的 `generation_assets` 与 `generation_gaps`。
...
4. 可选更新 `projects/aigc/<项目名>/7-设计/角色/design-manifest.yaml`，记录 `generation_assets` 和剩余 `generation_gaps`；manifest 不替代生成资产真源。
...
| --- | --- |
| `OUTPUT-CHARACTER-*` | `projects/aigc/<项目名>/7-设计/角色/3-生成/` |
| `OUTPUT-CHARACTER-GENERATION-REPORT` | `projects/aigc/<项目名>/7-设计/角色/3-生成/执行报告.md` |
