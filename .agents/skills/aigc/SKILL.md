...
- 数字序号阶段包（`0-初始化` -> `1-分集` -> `2-编导` -> `3-摄影` -> `4-分组` -> `5-设计` -> `6-图像` -> `7-视频` -> `8-审片`）默认按数字升序串行推进，前一阶段 canonical 产物自动作为后一阶段输入。
- 无序号同级子技能包默认全选并发执行，由所属父级汇总、裁决和写回唯一 canonical 输出；例如 `5-设计` 整体调用时由其父级并发调度 `场景`、`角色`、`道具`。
...
| `7-视频` | `.agents/skills/aigc/7-视频/libTV画布流/SKILL.md` | 用户只说进入 `7-视频`、生成视频阶段、下一步生视频、LibTV 画布流，且未明确指定旧 A/B/C/D 任一路线 | 用户点名 `A-分镜画面参照`、`B-分镜故事板参照`、`C-主体参照`、`D-主板混合参照` 旧路线，或要求多路线对比 |
...
flowchart TD
    ...
    S --> P["Reference imitation packet for 2-编导 / 3-摄影"]
...
| `7-视频` | `.agents/skills/aigc/7-视频/` | `projects/aigc/<项目名>/7-视频/` | active；默认叶子 `libTV画布流`；旧 A/B/C/D 位于 `7-视频-backup` 兼容路径 |
| `8-审片` | `.agents/skills/aigc/8-审片/` | `projects/aigc/<项目名>/8-审片/` | active；审查 `7-视频` 实际素材并回写 `4-分组` 修复 |
...
| current video stage | `7-视频/SKILL.md + CONTEXT.md`；未显式指定旧叶子时默认继续加载 `7-视频/libTV画布流/SKILL.md + CONTEXT.md` |
| current footage review stage | `8-审片/SKILL.md + CONTEXT.md`；对照 `7-视频` 素材和 `4-分组` 真源，必要时回写分镜组修复 |
| query / resume / review side channels | `query/`, `resume/`, `review/` skill pairs |
| reference imitation / shot-by-shot analysis | `shot-by-shot/SKILL.md + CONTEXT.md`；按需再加载 `2-编导` 与 `3-摄影` 阶段合同，输出临摹参照包而非阶段 canonical 主稿 |
...
3. 选择唯一主入口；若主入口为 `6-图像` 且用户未显式指定叶子，默认进入 `6-图像/B-分镜故事板`；若主入口为 `7-视频` 且用户未显式指定旧叶子，默认进入 `7-视频/libTV画布流`；若主入口为 `8-审片`，必须定位实际视频素材和对应 `4-分组` 分镜组。
