...
本文件是 `story/1-设定/角色卡` 与 `story/2-卷章规划` 之间的共享桥接真源。

它负责回答三件事：

1. 角色卡哪些字段允许进入规划文档。
2. 哪些字段必须继续留在角色卡侧，不得复制进规划文档。
3. `2-卷章规划` 父层与 `1-部级 / 2-卷级 / 3-章级` 各自能写什么、只能引用什么。

## Canonical Source Design

| carrier | truth_role | owner |
| --- | --- | --- |
| `1-Cards/2-角色卡/**/*.json` | 角色对象完整真源 | `1-设定/角色卡` |
| `1-Cards/2-角色卡/角色关系图谱.md` | 关系图谱 side output | `1-设定/角色卡` |
| `2-卷章规划/整体规划.md` | 宏观角色群、主题承担者与卷级分布摘要 | `2-卷章规划` 父层 |
| `2-卷章规划/第N卷/卷规划.md` | 卷级人物、关系摘要、任务钩子 | `2-卷章规划/2-卷级` |
| `2-卷章规划/第N卷/第N章.md` | 章级出场、任务线、线索/伏笔关联 | `2-卷章规划/3-章级` |
...
| --- | --- | --- |
| `2-卷章规划` 父层 | 最小角色/关系投影 | 宏观角色群、主题承担者、跨卷角色分布摘要 |
...
2. `2-卷章规划` 只写最小摘要、refs 与 hooks，不写角色完整事实。
