...
| `success_criteria` | 设计稿可回指上游，包含 research_brief / source posture / uncertainty / visual translation / 物语 / 解构 / prompt evidence chain，且可进入图像生成 |
...
| `N6-DESIGN` | LLM 直出设计正文 | research brief、north star、team、type profile | 写物语、Scene Design、Cinematography、prompt 和 `prompt_evidence_chain` | draft markdown | `N7-REVIEW` | 核心正文非脚本生成，prompt token 可回指 |
| `N7-REVIEW` | 质量门禁与 reviewer 汇流 | draft markdown、review contract | 执行 subagents 或本地 checklist，修复阻断项 | review verdict | `N8-WRITE` 或 `N6-DESIGN` | verdict 非阻断 |
...
- `prompt_evidence_chain` 至少记录 style、spatial、material、light/camera 和 empty-shot token group 的证据来源。
- `review verdict` 至少记录字段完整性、research brief 完整性、prompt 字符数、LLM-first 边界和写入路径。
