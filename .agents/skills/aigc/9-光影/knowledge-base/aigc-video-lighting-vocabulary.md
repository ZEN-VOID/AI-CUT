...
本知识库服务 `.agents/skills/aigc/9-光影/SKILL.md`，用于把逐分镜光影描述写成 AIGC 视频模型更容易实现的具象词库。它不是第二执行合同，不拥有输出路径、完成门或主链裁决权；调用时必须回到主入口 `N6-LIGHT-DESIGN` 和 `GATE-LIGHT-09-CONCRETE-AIGC`。
...
1. 词库只提供可见光影词，不替代 `source`、`3-美学` 和分镜事实。
