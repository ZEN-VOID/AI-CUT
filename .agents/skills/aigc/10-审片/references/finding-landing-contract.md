...
| --- | --- | --- |
| `review_only` | 信息不足、低置信、或仅记录素材内容 | `projects/aigc/<项目名>/10-审片/第N集/` |
| `rerun_only` | 单次生成瑕疵，不需要改上游 | 审片报告 + `9-视频` rerun note |
| `group_repair` | 分镜组正文、节奏、焦点、首尾状态导致可复现偏差 | `projects/aigc/<项目名>/6-分组/第N集.md` |
...
| `group_repair` 是否以“让下一次视频生成更稳定”为目标，降低主体数量、明确唯一焦点/镜头动作/停点，并把后续 beat 留给下一组？ | `GATE-REVIEW-10` / `GATE-REVIEW-11` | `FAIL-REVIEW-PATCH-SCOPE` | 本文件 `Patch Rule`、`projects/aigc/<项目名>/6-分组/第N集.md` 对应组 | 修前/修后组摘要、被移走或保留的 beat、生成稳定性说明 |
