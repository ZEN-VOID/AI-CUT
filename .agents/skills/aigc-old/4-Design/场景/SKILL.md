...
description: Use when the 4-Design stage needs one scene-domain Skill 2.0 package that runs scene list, scene design, and scene panel prompt generation in order, while writing final scene artifacts directly under `projects/aigc/<项目名>/4-Design/`.
...
- 本技能是 `.agents/skills/aigc/4-Design/` 下的场景域子技能包；清单、设计、面板不再作为独立 skill 入口展开。
- 冲突优先级：用户显式请求 > 根 `AGENTS.md` > `.agents/skills/aigc/SKILL.md` > `.agents/skills/aigc/4-Design/SKILL.md` > 本 `SKILL.md` > 本目录 `references/*` / `steps/*` / `review/*` / `types/*` > 项目 `MEMORY.md` > 项目 `CONTEXT/` > 本 `CONTEXT.md`。
...
1. 锁定项目根、集数、输出根：`projects/aigc/<项目名>/4-Design/`。
...
1. 输出不在 `projects/aigc/<项目名>/4-Design/` 根：回到本 `Output Contract` 与 `references/processing-order.md`。
...
| --- | --- | --- |
| `SCENE-PASS` | 清单、设计文档、面板 JSON 都在 4-Design 根目录且同 stem 对齐 | done |
...
- Output path: `projects/aigc/<项目名>/4-Design/` 根目录。
