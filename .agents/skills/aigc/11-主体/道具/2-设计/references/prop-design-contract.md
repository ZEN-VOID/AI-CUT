...
| `Prop Corpus Usage Trace` | 触发道具审美、文化元素、工艺装饰、功能结构、使用痕迹或 prompt 短语时，记录 `knowledge-base/prop-design-corpus.md` 的加载、语料种子、原创转译、时代/文化 guardrail 与 prompt 证据 |
...
flowchart TD
    ...
    C["3-美学/画面基调"] --> D["Global Style Prompt"]
    C2["3-美学/道具风格"] --> D2["Prop Style Prompt"]
    C3["north_star.yaml"] --> D3["主题 / 北极星 / 禁区"]
...
- 每条研究结论必须落到至少一个可见或可生成字段：形制、材料、工艺、设计细节、文化元素、年代、使用痕迹、功能逻辑、风险/不确定性、prompt evidence token。
- 道具设计必须主动追求审美吸引力：每个道具都要具备独特轮廓、材质记忆点、工艺/装饰细节、文化/身份符号、使用痕迹和功能结构的组合；关键道具必须有可被一眼记住的 `signature detail`。
- 文化元素不得随机堆砌，必须绑定项目时代、地域、阶层、职业、宗教/族群禁区、叙事功能和 `north_star`；不得错置现代奢侈品、街头潮牌、战术风、赛博朋克、哥特奇幻等脱离语境的装饰。
- 当任务进入单道具设计、批量设计、增量补缺或修复，并涉及审美、文化元素、工艺装饰、功能结构、使用痕迹或 prompt 短语时，必须加载 `knowledge-base/prop-design-corpus.md`，只做原创转译，不照搬为第二规则源。
...
| `visual translation` | 转成形制、材料、工艺、设计细节、文化元素、年代、使用痕迹、功能逻辑或安全边界 |
...
| `design_detail_culture` | 独特轮廓、材质记忆点、工艺/装饰细节、纹样、铭文、徽记、封缄、器型、地域/时代/阶层/职业符号和 signature detail |
...
- 英文 prompt 中的设计细节、文化元素、纹样、铭文、徽记、封缄、工艺装饰和 signature detail，应能回指 `Prop Design` 或 `Prop Corpus Usage Trace`。
...
- 道具为什么好看、哪里有设计感、它的 `signature detail` 是什么、文化元素如何与时代/地域/阶层/职业/功能绑定。
- 文化元素、纹样、铭文、徽记、封缄、器型、材质和装饰必须符合项目语境；不得为了“风格化”加入脱离时代的现代奢侈品、战术、赛博、哥特奇幻或街头潮流符号。
...
- 最终英文整合提示词的整合对象是 `## 4. 解构` 的全部有效信息，包括 `Photography` 与 `Prop Design` 中的全貌构图、45 度角度、完整轮廓、形制、线条、体积、材料、纹理、装饰、年代、磨损、功能逻辑、尺度和固定画面约束；不得只把主体 ID、画面基调、道具风格、固定画面词或负向词作为前缀/后缀拼接后宣布完成。
- prompt 必须纳入与当前道具相符的设计细节、文化元素、工艺/装饰细节、材质记忆点、signature detail 或功能结构 token；不得输出简单平凡的通用物件描述。
...
- 若画面基调或道具风格缺失，必须写明缺失路径与字段，例如 `Global Style Prompt: missing 3-美学/画面基调 source` 或 `Prop Style Prompt: missing 3-美学/道具风格 source`，不得从 `north_star.yaml` 补造最终风格提示词。
...
| 道具是否好看、有设计感、有文化元素和可见细节，包含独特轮廓、材质记忆点、工艺/装饰细节、文化/身份符号、使用痕迹、功能结构和关键道具 signature detail，而不是简单和平凡的功能物？ | `GATE-PROP-DESIGN-13` | `FAIL-PROP-DESIGN-DETAIL-CULTURE` | `N5-RESEARCH-CHAIN` / `N6-DESIGN` | `Design Appeal Target`、`Signature Detail`、`Cultural Element Strategy`、`Craft / Ornament Detail`、prompt token |
| 触发审美、文化元素、工艺装饰、功能结构、使用痕迹或 prompt 短语时，是否加载 `knowledge-base/prop-design-corpus.md`，并说明语料种子、原创转译、时代/文化 guardrail 和 prompt 证据？ | `GATE-PROP-DESIGN-14` | `FAIL-PROP-DESIGN-CORPUS-MISSING` | `N5-RESEARCH-CHAIN` / `N6-DESIGN` | `Prop Corpus Usage Trace`、语料种子、原创转译说明、period/culture guardrail |
...
| `3-美学/画面基调` 是否转译为 `Global Style Prompt`，`3-美学/道具风格` 是否转译为 `Prop Style Prompt`，`north_star.yaml` 是否转译为主题、时代/材质/色彩/镜头禁区和项目美术位置；`team.yaml.init_synthesis` 是否转译为至少一条可见设计决策，而不是成员名字装饰或伪顾问问答？ | `GATE-PROP-DESIGN-04` | `FAIL-PROP-DESIGN-04` | `N2-UPSTREAM` / `N5-RESEARCH-CHAIN` | `project_design_context`、init synthesis source、设计决策证据 |
...
| prompt 关键名词、材质、年代、磨损、工艺、形制和禁止项是否能回指研究、物语、解构、`visual_tone` 或 `prop_style`，且未为证据链新增场景、人物或手持 token？ | `GATE-PROP-DESIGN-10` | `FAIL-PROP-DESIGN-09` | `N5-RESEARCH-CHAIN` / `N6-DESIGN` | `prompt_evidence_chain`、token 来源对照、禁用 token 检查 |
