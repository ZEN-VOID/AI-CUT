...
| 定场镜头场景/环境/角色身份、动作链牵引、上一组末尾承接、空间可达性和无互动道具准入 | `../_shared/scene-shot-identity-contract.md`、`../_shared/action-first-continuity-contract.md`、`references/group-establishing-shot-contract.md` |
...
flowchart TD
    ...
    P6 -->|"FAIL-GROUP-11 定场镜头缺失、空间关系错误或未承接上一组"| P2A
...
4.25. 按 `references/group-establishing-shot-contract.md`、`../_shared/scene-shot-identity-contract.md` 和 `../_shared/action-first-continuity-contract.md` 为每个已裁定的分镜组生成 `定场镜头：` 自然描述，位于场景标题行之后、画面属性和 north_star 风格行之前；必须先锁定场景身份和环境身份，包括年代/时代、社会语境、空间功能、场景内子区域、固定锚点、材质新旧、光线结果和不新增事件的声音底色，再交代本组全部可见或需要生成角色、每个角色站/坐/蹲/躺/靠等姿态、角色之间的前后左右内外远近关系、身体朝向或视线方向。每集第二个及之后的分镜组必须先回顾上一分镜组结束状态，并用 `承接上一组结束状态`、`承接上组末尾` 或等价短语说明本组开端如何继承上一组末尾的角色位置、姿态、朝向、视线、身体接触、注意力落点或未完成动作；同场景不得突然重置空间，跨场景必须与组间连接件抵达后的首态一致。连续同场景的新分镜组也必须重新写定场镜头；定场镜头只交代身份明确的空间起始状态，不写成单纯人物动作摘要，不新增剧情、人物、道具互动或无互动道具氛围镜头。
...
7. 按 `references/statistics-yaml-contract.md` 在每组底部附加 YAML 统计；每组标题后的场景标题行、定场镜头和画面属性语句计入 YAML `字数统计`，但不计入组内 `时长估算`；统计 YAML、组头 north_star、组标题和组间连接件均不计入组内 `时长估算`、1680 目标上限、1980 硬上限或 `字数统计`；`道具` 抽取必须先识别同一物品并归一合并，去除普通环境背景物、重复项、同物异名、状态拆分和部件滥列，只保留重要叙事道具、规则道具、视觉钩子或后续生成必须锁定的 canonical 道具名。
...
| `scripts/validate_storyboard_groups.py` | 必跑机械校验：检查分镜组标题、场景标题行、定场镜头、第二组起定场镜头承接上一组结束状态标记、风格字段、YAML 统计、计入场景标题行、定场镜头和画面属性后的正文字数、显式时长累计、编号连续性和组间连接件结构；不能替代分组边界、连接件创意、原文保真和统计证据判断 |
...
| `FIELD-GROUP-02A` | 定场镜头 | 每组场景标题行后必须写 `定场镜头：`，用一段自然描述先交代场景身份和环境身份，再交代全部可见或需要生成角色、站坐蹲躺靠等姿态、角色之间的空间方位关系和朝向；每集第二组起必须显式承接上一组结束状态，并以该状态作为本组开端牵引整组空间关系和人物动作链；同场景连续组也重复输出；不得写成单纯人物动作摘要，不得新增人物、剧情、无互动道具氛围镜头或道具互动事实 | `FAIL-GROUP-11` |
...
| `FIELD-GROUP-08` | YAML 统计 | 每组底部含 `字数统计`、`时长估算`、`角色`、`场景`、`道具`；分镜组标题后的场景标题行、定场镜头和画面属性计入 `字数统计` 但不计入组内时长，统计块不计入字数或组内时长；`道具` 先做同物识别与归一合并，同一物品只列一次，不把状态、持有人、镜头景别或普通部件拆成重复道具 | `FAIL-GROUP-08` |
...
flowchart LR
    ...
    E4 --> T3{"每组是否已重建定场镜头"}
    T3 -->|"是"| E3["生成 group_establishing_shot"]
    T3 -->|"否"| R3["从本组正文重建角色站位、姿态和朝向"]
...
| `PASS-GROUP-02A` | 定场镜头裁决 | 每组正文、上一组结束状态、场景标题、角色动作、对白主体、空间证据、`references/group-establishing-shot-contract.md`、`../_shared/scene-shot-identity-contract.md`、`../_shared/action-first-continuity-contract.md` | 是否为每组先重新交代场景身份和环境身份，再交代全部可见或需要生成角色、姿态、空间方位和朝向；第二组起是否显式承接上一组结束状态并以它作为本组开端和动作链牵引；是否未退化为人物动作摘要，且未新增无证据道具镜头 | `group_establishing_shot`、`scene_group_identity_profile`、`action_first_continuity_check` |
...
| `PASS-GROUP-06` | 落盘与审查 | 分组稿 | ID、时长估算、计入场景标题行/定场镜头/画面属性后的正文字数、组间连接件、north_star、YAML 是否通过 | `review_result` |
...
| `PASS-GROUP-02A` | 为每个分镜组开头生成定场镜头 | 每组 `定场镜头：`、场景身份/环境身份、第二组起承接上一组结束状态标记、角色姿态、空间方位、朝向和动作链牵引证据 | `references/group-establishing-shot-contract.md`、`../_shared/scene-shot-identity-contract.md`、`../_shared/action-first-continuity-contract.md` |
...
- 分镜组开头缺少 `定场镜头：`，或定场镜头未先建立场景身份/环境身份，退化为“谁在做什么”的人物动作摘要，未覆盖本组全部可见/需要生成角色的站位、姿态、空间方位和朝向，或第 N+1 组没有承接第 N 组结束状态，或新增无互动道具氛围镜头。
- 只按字数硬切，或只为了精确卡 15 秒切断同一画面句子、对应对白/画面承托或其 `分镜明细` 多分镜。
- 场景标题行、定场镜头、画面属性加纯分镜剧本正文超过 1680 字却未做拆分复核，或超过 1980 字硬上限，或把 YAML 统计、north_star 组头或组间连接件误计入正文统计。
...
3. 每个分镜组必须以 `## x-y-z` 作为分镜组标题，并在标题后先写当前场景标题行，例如 `场景1：外景 扶桑战船外舷与黑礁 - 夜`；即便新的分镜组没有切换场景，也必须重复同一个场景标题；随后写 `定场镜头：` 自然描述，先覆盖场景身份与环境身份，再覆盖全部可见或需要生成角色、站坐蹲躺靠等姿态、空间方位关系和朝向；每集第二组起必须显式承接上一组结束状态，把上一组末尾作为本组开端牵引整组空间关系；再写画面属性自然语句（从组内镜头设计提炼，覆盖构图布局核心选择、构图方式关键子维度、光源效果、色彩基调和关键摄影技术参数）、组头三项 north_star 风格纯内容、从 `5-摄影` 划定的分镜剧本正文和组底 YAML 统计。
...
6. 每组边界以 `分镜N（约X秒）` 显式时长累计为主，优先接近约 15 秒，通常约 12-18 秒可接受；单组显式时长累计不得超过 18 秒，超过 18 秒即为阻断错误，必须通过移动完整 atomic unit、重组边界或回退 `5-摄影` 修复单个超长 atomic unit 后再落盘。低于约 10 秒必须在执行报告说明回填复核结果。字数和对白句数是辅助风险指标；每组标题后的场景标题行、定场镜头和画面属性语句计入 YAML `字数统计`，但组标题、north_star、组间连接件和统计 YAML 均不计入组内时长累计，也不计入 `字数统计`。
...
- 每组标题后先写当前场景标题行，未切换场景的新分镜组也重复同一场景标题；随后写 `定场镜头：`，先覆盖场景身份和环境身份，再覆盖全部可见或需要生成角色、站坐蹲躺靠等姿态、空间方位关系和朝向，且不退化为人物动作摘要、不新增无互动道具氛围镜头；每集第二组起定场镜头必须显式承接上一组结束状态，并把上一组末尾作为本组开端牵引整组空间关系；再写画面属性语句，位于定场镜头之后、north_star 风格行之前，覆盖构图布局核心选择、构图方式关键子维度、光源效果、色彩基调和关键摄影技术参数；随后投影 `全局风格.全局风格提示词`、`类型元素.类型元素提示词`、`细分风格.画面风格`，第 1 行风格行以固定前置词 `视频生成的画面风格，光影和氛围与场景参照图保持一致。需要生成现场物理互动音效、氛围感音效、环境声、自然现象声、动作声，不要生成任何字幕，不要生成背景音乐。` 开头，后接全局风格原词，且组头不显示标题字段、中文括号或装饰性连接符。
...
- 每组底部 YAML 统计含 `字数统计`、`时长估算`、`角色`、`场景`、`道具`；每组标题后的场景标题行、定场镜头和画面属性语句计入 `字数统计`，但不计入 `时长估算`；分镜组标题、YAML 统计块、组头 north_star 和连接件未被计入字数或组内时长累计；`道具` 已先归一合并，同一物品不重复列名，不因颜色、位置、持有人、状态、镜头或普通部件差异拆成多个道具。
