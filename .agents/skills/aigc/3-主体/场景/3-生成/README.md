...
Skill 2.0 package for `aigc/3-主体/场景/3-生成`.
...
```text
projects/aigc/<项目名>/3-主体/场景/2-设计/
```
...
`test-prompts.json` 用于 runtime-spine dry-run / regression prompts，覆盖完整生成、main_only 和缺 JSON repair。
...
- `SKILL.md 的 Thinking-Action Node Map` owns the detailed node graph, sequence diagram, branch matrix and failure loop.
