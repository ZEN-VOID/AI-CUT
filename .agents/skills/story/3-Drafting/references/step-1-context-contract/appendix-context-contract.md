# Step 1 Context Contract Template

> Canonical module route: `references/step-1-context-contract/module-spec.md`
> 当前角色：Step 1 模块 appendix。根 `SKILL.md` 不再直接路由到本文件。
...
- 是否过渡章（true/false，必须按章节规划节点判定，不按字数判定）
- 追读力设计（钩子类型/强度、微兑现清单、爽点模式）

过渡章判定规则（强制）：
- 依据 `Planning/8-全息地图.json` 章节节点中的功能标签与目标（铺垫/转场/承接/回收等）。
- 若全息地图未显式标注，由“本章核心目标是否以推进主冲突为主”判定；仅在缺失时回退 legacy outline。
- 禁止使用字数阈值判定过渡章。
...
执行：
1. 从 `writing-craft-catalog/leaf-notes/genre-hook-payoff-library.md` 选 1 条期待锚点（优先章末，也可后段）。
