...
| `5-Image` 实际没有阶段根 `SKILL.md`，但根入口仍把它写成已建阶段根合同                                                    | 根阶段投影层           | 在根 `SKILL.md` 明确 `5-Image` 当前只有 `1-提示词蒸馏 / 2-参照引用 / 3-图像生成` 三个真实入口                                            | 把“阶段桶存在”和“阶段根合同存在”分开表述；对 tranche-first 阶段禁止回指不存在的根 `SKILL.md`                                                                                                            | 根入口不再试图进入不存在的 `.agents/skills/aigc/5-Image/SKILL.md`                                      |
...
| 根技能把 `.agents/skills/aigc/benchmark-suite.yaml` 写成现成证据源，但磁盘上并不存在                                     | 质量证据同步层         | 把根 `SKILL.md` 的 Quality Evidence Source 改成当前真实证据，并显式标注 benchmark suite 待补                                             | 对根级 evidence source 做“文件存在性 + contract existence”双检查；未落盘前不写成已存在真源                                                                                                              | 根入口不再引用不存在的 benchmark 文件                                                                  |
...
3. 若某阶段只有逻辑阶段桶、没有阶段根合同，必须在根层显式写“直达真实入口”，不得伪造根 `SKILL.md`。
...
- 对像 `5-Image` 这种 tranche-first 阶段，根入口最稳的写法不是虚构阶段根合同，而是直接说明“进入哪个真实父/叶入口”。
