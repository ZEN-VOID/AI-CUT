...
6. `组间设计.全局风格 / 类型元素 / 导演意图` 必须直接提取自 Markdown 中的同名字段；写入 JSON 时只允许剥离字段标题与空白。`组间设计.出场角色及穿搭` 为组级服装摘要槽，格式建议 `角色名-服装简述`。
7. `组间设计.全局风格 / 类型元素 / 导演意图` 的默认字数窗固定为 `220 / 50 / 100` 个字符以内；`出场角色及穿搭` 在 `2-Global` 阶段允许先留空字符串，待 `3-Detail` 回填。
