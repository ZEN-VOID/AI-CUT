...
- 需要为 `4-Design/1-场景/2-设计` 提供唯一场景对象池，避免下游重新从镜头文本猜场景。
...
- `projects/<项目名>/4-Design/1-场景/1-清单/第N集/第N集.json` 的 canonical 写出。
...
- `actions`
  ...
  4. 保证输出路径固定到 `projects/<项目名>/4-Design/1-场景/1-清单/第N集/`。
- `evidence`
  - `第N集.json`
  - 可选 `_manifest.json`
- `route_out`
...
- `projects/<项目名>/4-Design/1-场景/1-清单/第N集/_manifest.json` 仅在显式要求追溯时输出
