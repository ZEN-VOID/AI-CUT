...
| --- | --- | --- | --- | --- |
| 查询到章节规划、任务、线索、伏笔时仍只读 `STATE.json` 或旧 `大纲/` | skill contract / data-flow doc | 把规划类问题默认入口切到 `整体规划.md / 第N卷/卷规划.md / 第N卷/第N章.md` | 在 `SKILL.md` 与 `system-data-flow.md` 固定三层规划优先 | 规划类查询说明明确写出三层规划为优先数据源 |
| query 仍把兼容 `全息地图` 当成主真源，而不先看三层规划文档 | planning truth layering | 先读三层规划，再视需要回退兼容投影 | 在 query 合同与 data-flow 文档固定“fractal-first, holomap-fallback” | 查询具体卷章规划时不再误把兼容投影当主真源 |
| 把 `planned_state` 当成“已经发生的事实” | truth-role contract | 改为同时检查 `actualization + context-return artifact + validation PASS` | 在 `SKILL.md` 固定 `planned/current/validated_actual` 三分法 | 查询“已经发生了吗”时不会再拿计划冒充结果 |
...
| 实绩查询忽略 `context-return` 的 PASS-only gate | stage contract | 把 `actualization` 与 `第V卷.context-return.json` 设为必须校验项 | 在 query 规则中写明“无 PASS 证据只能回答尚无 validated actual” | 实绩类回答会带 `validation_ref` 或明确缺口 |
...
3. 若涉及“已经发生了吗”，先检查 `MAP.actualization` 与 context-return artifact，再看 `validation_ref`。
...
- 凡是带“原计划、安排、落在哪章”的问法，优先看三层规划文档，兼容项目再看 `MAP planned_state`。
- 凡是带“现在、当前、默认有效”的问法，优先看 `Cards.current_state` 与 `STATE.json`。
- 凡是带“已经、实际、最终、兑现到哪”的问法，必须去找 `actualization + 上下文回流 + PASS 证据`。
