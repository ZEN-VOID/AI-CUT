...
`story-plan-volume-level` 是 `2-Planning` 的受治理子技能，负责把 `整体规划.md` 下钻为单卷中观规划。它只产出规划，不写正文，不越权重写部级总纲或章级执行蓝图。
...
- 必须读取项目内 `2-Planning/整体规划.md` 后，才允许生成、补写或修订 `2-Planning/第N卷/卷规划.md`。
...
- 部级总纲：`projects/story/<项目名>/2-Planning/整体规划.md`。
- 目标卷序号或目标卷目录：`第N卷`。

### Optional Input

- 已存在的 `projects/story/<项目名>/2-Planning/第N卷/卷规划.md`。
- `0-Init/north_star.yaml`、`0-Init/init_handoff.yaml`。
- `1-Cards/类型卡`、`1-Cards/角色卡`、`1-Cards/场景卡`、`1-Cards/物品卡` 中与本卷相关的局部事实。
