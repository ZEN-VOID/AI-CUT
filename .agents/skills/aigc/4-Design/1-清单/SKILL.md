...
description: Use when the `4-Design/1-清单` tranche needs to hold the shared `3-Detail` consumption contract and route the currently landed scene/prop list leaves under `projects/aigc/<项目名>/4-Design/`.
...
`1-清单` 是 `4-Design` 阶段承接 `3-Detail`、连接 `2-设计` 的阶段父 skill。
...
| `服装` | pending-migration | `4-Design/角色/1-清单/第N集/角色清单.json` | 暂不声明 active runtime；不得在初始化时预建 `4-Design/服装/*` |
...
- 强制读取：`.agents/skills/aigc/4-Design/1-清单/_shared/list-output-contract.md`
- 强制读取：`道具/SKILL.md`
- 条件读取：`场景/SKILL.md`
- 条件读取：`角色/SKILL.md`
- 条件读取：`服装/SKILL.md`
