...
prompt_source:
  prompts_path: "projects/aigc/<项目名>/7-图像/A-分镜画面/第1集/第1集-分镜画面-prompts.md"
...
scene_visual_style_lock:
  ...
  scene_reference_path: "projects/aigc/<项目名>/6-设计/场景/3-生成/<场景名>.png"
previous_frame_context:
  status: "visible_in_conversation_context"
  previous_shot_id: "1-1-1-1"
  previous_image_path: "projects/aigc/<项目名>/7-图像/A-分镜画面/第1集/images/1-1-1-1.png"
  continuity_role: "spatial_blocking_reference"
output_image_path: "projects/aigc/<项目名>/7-图像/A-分镜画面/第1集/images/1-1-1-2.png"
reference_input_status: "visible_in_conversation_context"
```

## Output Persistence

- 生成结果必须复制或持久化到 `projects/aigc/<项目名>/7-图像/A-分镜画面` 根路径下；逐集图片默认放入 `projects/aigc/<项目名>/7-图像/A-分镜画面/第N集/images/`。
...
| 生成结果是否持久化到项目内 `7-图像/A-分镜画面/第N集/images/`，而不是只留下 `$CODEX_HOME/generated_images` 路径？ | `G6-PERSIST` | `FAIL-FRAME-IMAGEGEN` | `N8-PERSIST` | `imagegen-results.json` 记录项目内 `output_image_path` 和源路径；项目内文件存在。 |
