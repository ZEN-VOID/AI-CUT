...
| `complexity_source` | 类型分流、监制上下文转译、冷门考据、研究到 prompt 的证据链、subagent 汇流 |
...
| `N6-DESIGN` | 完成单道具 LLM-first 设计 | 清单项、north_star、team、type profile、research evidence chain | 写物语、`## 4. 解构` 下的 `主体ID号：<主体ID>`、Photography、Prop Design、prompt evidence chain、英文 prompt；prompt 必须整合 `## 4. 解构` 全部有效信息并补 `deconstruction_coverage` | design draft | `N7-REVIEW` | 必填章节齐全，prompt 英文、以同一主体 ID 号开头且 1300 characters 内，使用自然语言负向约束且不含 `--no`，并包含 full prop in view、prop only、no people、no background elements |
| `N7-REVIEW` | 执行质量门禁和 subagent 汇流 | design draft、review contract | reviewer subagent 或降级 review 检查来源、研究转译、字段、路径、prompt | review verdict | `N8-WRITE` 或 `N5-RESEARCH-CHAIN` / `N6-DESIGN` | verdict 非阻断 |
...
- `N7-REVIEW` 汇流时只聚合已调度主体；未调度主体不得补空文件或默认占位。
