...
- `首帧参照` 的帧级提示词正文属于内容创作型输出，必须由 LLM 直接完成。
- `第N集.txt` 的一镜版蒸馏正文同样属于内容创作型输出，必须由 LLM 直接完成；脚本不得代替 LLM 对句子质量、镜头语言连贯性与审美压缩做主创裁决。
...
`第N集.txt` 在 `首帧参照` 中仍以集为输出单位，但每个 block 只承载一个目标分镜，固定模板为：
...
- 每个目标分镜 block 末尾必须追加 `字数统计：xxx`，其中 `xxx` 统计的是该 block 主稿正文字符数，不把统计行本身计入。
- `剧本正文` 必须原样保留，不得摘要、裁断、改写或重述。
- `主体锚定.场景` 与 `主体锚定.角色` 必须原样保留，不得压缩。
- `主体锚定.道具` 可轻量压缩，但不得改变事实。
- `分镜明细` 必须展开到下一层字段后再吸收进镜头语言：
  - `分镜构图.构图形式 / 景别景深 / 镜头类型`
  - `运镜手法.变化 / 速度 / 组合`
  - `角色表现.动作戏 / 对话戏 / 内心戏`
  - `氛围表现.层次 / 空间诗学 / 意境`
  - `摄影表现.光影 / 色彩 / 质感`
  - `转场特效.特效 / 组内 / 组间`
- 上述二级字段只能作为内容来源，最终 `分镜明细` 必须写成镜头语言优先的整段 prose，不得保留明显字段顺序痕迹。
- `全局风格 / 类型元素 / 导演意图` 只允许轻量压缩。
- 当字数紧张时，优先压缩 `分镜明细` 与 `主体锚定.道具`，但仍须保持自然句或自然短语。

## When To Use

- 需要从 `projects/aigc/<项目名>/3-Detail/第N集.json` 中锁定单一 `分镜ID`，且该 episode root 已形成 canonical detail 壳、并经 compat projection 判定为 ready，生成帧级视频请求对象。
- 需要优先通过 `正文切分参考[] -> 目标分镜.正文回指` 把目标分镜所属组的 `剧本正文` 融写为当前帧的剧情桥段，而不是直接照搬整组剧情。
- 需要把输出组织成“组级设计块 + 单镜融写行”的 `BC` 结构，而不是保留独立 A 段。
- 需要把目标分镜 `时间段.开始秒 / 结束秒` 落成当前分镜组内的 `xx秒-xx秒｜分镜<组内序号>：` 时间锚点，不写成 `分镜 <ID> 的 xx秒-xx秒`。
...
- 上游 `3-Detail/第N集.json` 尚未形成合法 canonical `groups[]` / compat group view，或目标 `分镜ID` 不存在。
...
- `正文切分参考[] -> 正文回指 -> 对应分镜帧剧情桥段` 的提取、保守压缩与例外说明规则。
- 组级设计块、单镜融写行、字段标题隐藏与字数窗控制。
...
- 关键结构：canonical `groups[].detail.分镜列表`（必要时经 compat projection 暴露旧式 `分镜明细[]` helper）
- handoff gate：canonical detail root 已成立，且目标分镜所属组经 compat projection 后满足 `ready + 分镜切换 == len(分镜明细[])`
- 关键组级字段：`分镜组ID`、`剧本正文`、`正文切分参考[]`、`组间设计.全局风格`、`组间设计.类型元素`、`组间设计.导演意图`、`组间设计.出场角色及穿搭`
- 关键镜级字段：目标 `分镜明细` 下的 `分镜ID`、`时间段.开始秒 / 结束秒`、`角色表现`、`运动表现`、`氛围表现`、`视觉强化`、`分镜构图`、`摄影美学`、`运镜手法`、`镜头视角`，以及存在时的 `镜头速度 / 道具及状态 / 镜头类型兼容 / 镜头框架 / 镜头类型`
...
- `全局风格` 必须原文保留，剩余空间只能压缩 `P1/P2/P3` 非核心块。
...
- `剧情桥段` 只对应目标分镜帧，最终 prompt 不再保留独立 A 段整组 `剧本正文`。
- `全局风格` 与上游逐字一致。
- 单镜行都能读出当前分镜组内的 `xx秒-xx秒｜分镜<组内序号>：`，且不写成 `分镜 <ID> 的 xx秒-xx秒`。
- branch-owned 主镜级事实必须在最终 prompt 中保持清晰可辨，尤其是由 `运动表现 / 氛围表现 / 分镜构图 / 运镜手法` 投影出的空间关系与镜头起势；若上游存在 `镜头速度`，也不得被静默压没。
- 在相同总字数与字段集合下，当前目标分镜的动作、空间、镜头控制与氛围细节应比组级模式更丰满，而不是沿用组级多镜分摊后的稀薄表达。
- prompt 中除 `xx秒-xx秒｜分镜<组内序号>：` 镜级标签外，不残留显式字段标题，也不泄露完整四段式 `分镜ID`。
- `第N集.json / 第N集.txt / _manifest.json` 三件套可继续 handoff，并对例外情况给出可追溯备注；其中 `第N集.txt` 必须是固定模板下的一镜版同构主稿，而不是 prompt 展示页。
...
- canonical 输入文件：`projects/aigc/<项目名>/3-Detail/第N集.json`
- compat projection 后的 `final_output.main_content.分镜组列表[]`
...
- 目标分镜 `镜头框架`
- 目标分镜 `镜头类型`
- legacy `角色背景面 / 角色站位走位 / 摄影美学 / 运镜手法 / 分镜表现`

### 输入处理原则

1. 一切剧情与镜头事实以上游 `3-Detail/第N集.json` 为准。
2. compat projection 只作为 leaf helper 视图；canonical `meta + groups[].global/detail.分镜列表` 仍是唯一业务真源。
3. 只有 compat projection 后 `metadata.document_phase=ready` 且目标分镜所属组 `分镜切换 == len(分镜明细[])` 时，才允许首帧蒸馏继续。
4. `全局风格` 只允许原文直贴，不做净化、重命名或压缩。
5. `剧情桥段` 只负责把组级剧本切成目标帧可见事实，不负责重写剧情。
6. `时间段` 只允许使用目标分镜所在组内的相对秒位，不得回退成全集累积时间线，也不得改写成模糊时间语。
7. prompt 中不再保留独立 A 段整组 `剧本正文`；完整四段式 `分镜ID` 只允许留在结构化回链字段，正文显示只保留组内序号。
8. 图片字段只保留共享模板骨架，不准编造真实图片信息。

### 输入完整性门禁

以下任一缺失都必须停机并回报上游缺口：

- compat projection 后 `分镜组列表[]` 缺失
- compat projection 后 `metadata.document_phase` 不是 `ready`
- 目标 `分镜ID` 不存在
- 无法锁定所属 `分镜组ID`
...
- 目标 `分镜明细` 缺 `分镜ID / 时间段.开始秒 / 时间段.结束秒 / 角色表现 / 运动表现 / 氛围表现 / 视觉强化 / 分镜构图 / 摄影美学 / 运镜手法 / 镜头视角`
...
2. 由于本技能只服务单一目标分镜，在 `G1 + P0` 落定后，剩余预算应优先转化为当前分镜的细节丰满度，优先把 `P1/P2/P3` 写得更完整、更具体，而不是机械继承组级多镜分摊后的稀疏压缩心智。
3. 当预算吃紧时，只能按 `P3 -> P2 -> P1` 的顺序压缩，不得先牺牲 `P1`。
4. 目标分镜的时间锚点必须写成当前分镜组内的 `xx秒-xx秒｜分镜<组内序号>：`，不得写成 `分镜 <ID> 的 xx秒-xx秒`。
5. 镜级信息可优先按“`镜头类型兼容 -> 景别/运镜手法/镜头速度/镜头视角 -> 运动表现/分镜构图 -> 角色表现/氛围表现 -> 道具及状态/摄影美学/视觉强化`”的语义顺序组织，但表层表达不强制套固定句式；若固定骨架让句子发硬，应优先改写成更自然流畅的句子。
6. 除镜级序号标签外，不得暴露字段标题，尤其不得写成 `字段标题：字段值`。

### 镜头类型兼容术语合同

- `镜头类型兼容` 是叙事/观看功能上的兼容专有命名，不是 `景别 / 运镜 / 视角` 的同义改写。
- 优先沿用上游 `镜头类型兼容` 原词；若上游缺失，不得为了凑模板硬造术语。
...
flowchart TD
    ...
    B -->|"direct_match"| C["按 正文回指 / beat_refs 直取目标帧桥段"]
    B -->|"ambiguous"| D["保守压到最小可见剧情事实"]
    C --> E["N6 组装组级设计块与镜级压缩块"]
    D --> F["manifest 记录保守退化"]
    F --> E
```
...
- `任务归位 -> 真源装配 -> 锁定目标分镜 -> 输入完整性体检 -> 桥段策略判型 -> 剧情桥段提取 -> 组级设计块/镜级压缩块装配 -> prompt 合成 -> 三件套写回 -> 汇流验收`

### 条件分支

- `bridge_mode=direct_match`
  - 可稳定提取目标分镜对应桥段，按 `正文回指.beat_refs[]` 正常裁切
- `bridge_mode=ambiguous`
  - 桥段边界模糊，保守压缩到“目标帧可见的最小剧情事实”，并写 manifest 说明
- `budget_state=normal`
  - 非核心块保持连贯自然句，并尽量覆盖 `P1 + P2 + P3`，把单分镜剩余预算优先转化为当前镜头的细节密度
- `budget_state=tight`
...
3. 单镜行显式写出当前分镜组内的 `xx秒-xx秒｜分镜<组内序号>：`，且不写成 `分镜 <ID> 的 xx秒-xx秒`。
4. 除镜级序号标签外，没有字段标题泄露，也没有完整四段式 `分镜ID` 泄露。
...
- `actions`:
  ...
  5. 读取共享 JSON/TXT 模板，并锁定其中“自然句优先、`tight` 才局部短语化、镜级显示使用 `xx秒-xx秒｜分镜<组内序号>：`、禁用 `分镜 <ID> 的 xx秒-xx秒`”的模板语义
  6. 记录可用输入清单与路径合法性
...
- `actions`:
  ...
  4. 记录目标分镜 `时间段.开始秒 / 结束秒`，作为后续 `xx秒-xx秒｜分镜<组内序号>：` 时间锚点
...
- `actions`:
  ...
  2. 通过 compat helper 校验上游 `metadata.document_phase = ready`
  3. 校验目标分镜所属组 `分镜切换 == len(分镜明细[])`
  4. 校验目标分镜是否具备 `分镜ID / 时间段.开始秒 / 时间段.结束秒 / 角色表现 / 运动表现 / 氛围表现 / 视觉强化 / 分镜构图 / 摄影美学 / 运镜手法 / 镜头视角`
  5. 若 `镜头速度 / 道具及状态 / 镜头类型兼容 / 镜头框架 / 镜头类型 / 转场特效` 缺失，则登记为 `soft risk`；legacy `人物表演锚点 / 动作路径 / 空间氛围 / 视觉抓手 / 构图骨架 / 分镜表现 / 角色背景面 / 角色站位走位` 只作 fallback 风险提示；更老 `人物表演 / 动作调度 / 视觉焦点 / 构图策略 / 摄影策略 / 运镜策略 / 转场策略` 已退出 shared schema 主兼容层
...
- `inputs`: `剧本正文`、组内镜头数、目标镜头 `时间段.开始秒 / 结束秒 / 角色表现 / 视觉强化`
- `着手方面`:
  1. `正文回指.beat_refs[]` 是否足以直接定位目标桥段
  2. 目标分镜是否能从组级剧情中直接定位
  3. 边界模糊时如何保守退化
- `actions`:
  1. 判定 `bridge_mode=direct_match/ambiguous`
...
- `objective`: 把组级 `剧本正文` 按 `正文切分参考[] -> 正文回指` 切成只对应目标分镜帧的剧情桥段
- `inputs`: `剧本正文`、`bridge_mode`、目标镜级事实
- `着手方面`:
  1. `beat_refs[]` 指向的原文片段如何回收为目标镜头剧情桥段
  2. 直配模式下哪些句子或事实真正属于目标镜头
  3. 模糊模式下最小可见剧情事实是什么
- `actions`:
  1. `direct_match` 时按 `正文回指.beat_refs[]` 抽出目标分镜直接对应的事件阶段、动作节点或状态变化
  2. `ambiguous` 时压缩到目标帧可见的最小剧情事实，不虚构过渡
  3. 写出 `bridge_strategy` 与 `exception_note` 候选内容
- `evidence`: 最终 `剧情桥段`、`bridge_strategy`、保守退化说明
- `route_out`: 提取成功进入 `N6`；若提取结果漂移则回到 `N4`
- `gate`: 禁止整段照搬组级 `剧本正文`；必须通过 `正文回指` 回链到目标桥段

### N6. 组装组级设计块与镜级压缩块

- `node_id`: `N6-block-assembly`
- `objective`: 保住组级设计块与镜级桥段，同时把其余组级与镜级字段压进剩余字数预算
- `inputs`: `剧情桥段`、`全局风格`、`类型元素`、`导演意图`、`出场角色及穿搭`、目标镜级字段
- `着手方面`:
  1. 哪些内容属于 `G1` 组级设计块与 `P0` 核心剧情块
  2. 哪些内容属于 `P1/P2/P3`，以及压缩优先级如何执行
  3. 当前预算压力是 `normal` 还是 `tight`
- `actions`:
  1. 冻结 `G1 组级设计块` 与 `P0 剧情桥段`
  2. 估算 `G1 + P0` 后的剩余空间
  3. 先按连贯自然语句组织 `P1 + P2 + P3`
  4. 将目标分镜 `时间段.开始秒 / 结束秒` 规范成 `xx秒-xx秒｜分镜<组内序号>：`
  5. 显式消费 `P1`：`运动表现 / 分镜构图 / 景别 / 运镜手法 / 镜头视角`；若存在 `镜头速度` 也应一并消费，不得回退成旧字段心智
...
- `evidence`: `G1/P0/P1/P2/P3` 组装结果、预算判定、例外备注、时间锚点写法、当前分镜细节密度判断
- `route_out`: 预算合适进入 `N7`；若组级设计块、桥段被改写或预算失衡则回退本节点
- `gate`: `G1 + P0` 不得被改写；非核心块不得被静默遗漏

### N7. 合成 prompt 与计数

- `node_id`: `N7-prompt-synthesis`
- `objective`: 合成最终 prompt，隐藏字段标题，并同步生成 `prompt_char_count`
- `inputs`: 组级设计块、镜级压缩块、共享 JSON/TXT 模板
- `着手方面`:
  1. 组级设计块与镜级序号标签是否显式且足够
  2. 除允许标签外是否仍有字段标题或完整四段式 `分镜ID` 泄露
  3. 模板建议的语义顺序是否被遵守，但未被误用成表层硬模板
  4. `prompt_char_count` 是否与实际 prompt 一致
- `actions`:
  1. 只保留 `组级设计块 + xx秒-xx秒｜分镜<组内序号>：` 镜级标签
  2. 确保目标分镜写成 `xx秒-xx秒｜分镜<组内序号>：`，不得写成 `分镜 <ID> 的 xx秒-xx秒`
  3. 按模板建议语义顺序融合组级设计块与镜级压缩块；若固定骨架让句子发硬，优先改写成更自然流畅的表达
  4. 若存在 `镜头类型兼容`，优先沿用上游原词并直接落术语本体，不机械补“为”
...
- `actions`:
  ...
  6. 确保 `第N集.txt` 命中固定模板：`分镜组ID / 全局风格-类型元素-导演意图 / 分镜N，x-y秒 / 剧本正文 / 主体锚定 / 分镜明细`
...
| --- | --- | --- | --- | --- | --- |
| V-VID-FFR-01 | 输入 | 目标分镜 handoff 是否已稳定可消费 | `ready/incomplete` | 检查 `metadata.document_phase=ready`，目标分镜所属组 `分镜切换 == len(分镜明细[])`，且 `分镜组ID / 剧本正文 / 正文切分参考[] / 组间设计（含出场角色及穿搭） / 目标分镜明细（含正文回指、时间段开始秒/结束秒、角色表现/运动表现/氛围表现/视觉强化/分镜构图/摄影美学/运镜手法/镜头视角，若存在则含镜头速度）` 成立 | P0 |
| V-VID-FFR-02 | 桥段判定 | `正文切分参考[] -> 正文回指` 与目标分镜的对应清晰度 | `direct_match/ambiguous` | 先读 `正文回指.beat_refs[]`，再结合覆盖模式与动作状态 | P0 |
| V-VID-FFR-03 | 字数预算 | 非核心字段压缩压力 | `normal/tight` | 估算 `G1 组级设计块 + P0 剧情桥段` 后的剩余字数；只要不超过 1900 字就保持连贯自然语句，仅在超限风险下进入 `tight` | P1 |
...
| C-VID-FFR-02 | `V-VID-FFR-02=direct_match` | 从 `正文回指.beat_refs[]` 抽取与目标分镜直接对应的剧情桥段 | 不引入无关分镜事实 | 无 |
| C-VID-FFR-03 | `V-VID-FFR-02=ambiguous` | 在 `正文回指` 非 direct 覆盖时，保守压缩到目标分镜可见的最小剧情事实，并在 manifest 记录例外 | 不虚构过渡；manifest 备注原因 | 无 |
| C-VID-FFR-04 | `V-VID-FFR-03=normal` | 以“组级设计块 + 单镜融写行”的 `BC` 结构压缩非核心字段，并把剩余预算优先转成当前分镜的细节密度 | `prompt_char_count <= 1900`，且 `P1` 可清晰读出，当前分镜细节不呈现稀疏缩写感 | 无 |
| C-VID-FFR-05 | `V-VID-FFR-03=tight` | 只在超 1900 风险下按 `P3 -> P2` 顺序压缩；必要时局部短语化，但继续高保留 `P0/P1` | 组级设计块稳定，整体被压回 `1900` 以内，且 `xx秒-xx秒｜分镜<组内序号>：` 与 `P1` 仍清晰可辨 | 无 |

## Convergence Contract

- 汇流点固定在 `N9`，前面任一节点失败都不得绕过。
- `FAIL-VID-FFR-01`：来源定位或输入完整性失败，回到 `N2` 或 `N3`。
- `FAIL-VID-FFR-02`：桥段提取漂移、`G1/P0` 改写、字段标题残留或预算失衡，回到 `N4-N7`。
...
2. `思考过程`
   - 简明说明本轮 `bridge_mode`、`budget_state`、`G1/P0` 与 `P1/P2/P3` 处理方式及汇流判断
...
2. `第N集.txt` 是一镜版同构模板下的 derived display view，展示固定模板编排后的蒸馏正文，而不是简单回显 `prompt`。
...
6. `全局风格` 必须原文保留，不得改写。
7. `剧情桥段` 必须转换为对应分镜帧的剧情桥段；若组内只有 1 个分镜，也应通过 `正文回指` 确认其覆盖整段后再自然等同整段直贴。
8. `时间段` 必须使用当前分镜组内的 `开始秒 / 结束秒`，并写成 `xx秒-xx秒｜分镜<组内序号>：`，不得写成 `分镜 <ID> 的 xx秒-xx秒`，也不得误写成全集时间线。
9. 默认目标字数上限为 `1900` 中文字符；若用户或父级显式给出其他范围，以显式约束覆盖。
10. 除 `xx秒-xx秒｜分镜<组内序号>：` 镜级标签外，不得出现 `字段标题：字段值` 结构，也不得泄露完整四段式 `分镜ID`。
11. 单分镜模式下，同样总字数应优先体现为当前分镜的细节丰满度；不得机械沿用组级多镜 prompt 的稀疏表达心智。
12. 只有当预算进入 `tight` 时，才允许把部分镜级内容收束为更精炼的自然短语；其余情况下应保持连贯自然语句。
13. `prompt_char_count` 必须与实际 `prompt` 内容一致；若 `第N集.txt` 记录附加统计，必须与 JSON 同步。
...
16. `第N集.txt` 不得退化成“提示词 + 字数统计”展示页；它必须保持固定模板编排，且不承载结构化参数区块。
...
7. `shots[].group_id`
8. `shots[].shot_id`
9. `shots[].shot_display_index`
10. `shots[].prompt_char_count`
11. `shots[].bridge_strategy`
12. `shots[].within_target_range`
13. `shots[].exception_note`

## Field Master

| field_id | 输出位置/字段 | 内容要求 | 证据来源 | 默认责任 Step | 质量维度 | 失败码 |
| --- | --- | --- | --- | --- | --- | --- |
| FIELD-VID-FFR-01 | `prompt_style.type / prompt_style.language / prompt_style.char_limit / meta.shot_level / meta.group_id / meta.source_shot_ids / meta.shot_display_index` | 以独立 `prompt_style` 声明帧级提示词约束，锁定组级归属、单一目标 `分镜ID` 与组内显示序号，并确认上游 `document_phase=ready` 且所属组 `分镜切换 == len(分镜明细[])` | episode root、目标 shot 绑定结果 | N2-N3 | 输入覆盖完整度 | FAIL-VID-FFR-01 |
| FIELD-VID-FFR-02 | `prompt / prompt_char_count` | prompt 必须覆盖目标分镜的剧情桥段、组级设计块和压缩后的上下文；压缩块需显式覆盖 `类型元素 / 导演意图 / 出场角色及穿搭 / 目标镜级字段`，并按 `P1 高保留 / P2 重要 / P3 补充` 顺序压缩；目标镜级条目需保留当前分镜组内的 `xx秒-xx秒｜分镜<组内序号>：`，且隐藏字段标题与完整四段式 `分镜ID`；在单分镜模式下，同等预算应优先体现为当前分镜细节更丰满 | `剧本正文`、`组间设计.*`、目标镜级字段 | N4-N7 | Prompt 蒸馏稳定性 | FAIL-VID-FFR-02 |
...
| N6 | FIELD-VID-FFR-02 | 哪些内容属于 `G1/P0/P1/P2/P3`，以及压缩优先级如何执行 | 冻结 `G1 + P0`，按顺序压缩其余组级与目标镜级字段，并补齐 `xx秒-xx秒｜分镜<组内序号>：` 时间锚点；非 `tight` 时优先补足当前分镜细节 | 组级设计块或桥段被改写、`P1` 被先吞掉、时间锚点缺失或当前分镜细节仍过薄 |
| N7 | FIELD-VID-FFR-02 / FIELD-VID-FFR-03 | 如何按模板语义顺序生成自然句，同时隐藏字段标题并保持模板骨架 | 合成 prompt、校准计数，并保留图片字段骨架 | 字段标题残留、时间写成 `分镜 <ID> 的 xx秒-xx秒`、完整 `分镜ID` 泄露、自然句退化成碎短语、当前分镜像组级缩小版、模板字段被删除或伪造 |
...
| FIELD-VID-FFR-02 | Prompt 蒸馏稳定性 | prompt 满足桥段提取、组级设计块保留、隐藏标题与字数窗，且 `P1` 高保留项与 `xx秒-xx秒｜分镜<组内序号>：` 保持清晰可辨；非 `tight` 档不退化成碎短语清单，且当前分镜细节在同等预算下明显比组级模式更丰满 | FAIL-VID-FFR-02 | N4 |
...
- compat projection 后 `metadata.document_phase` 是否已经到 `ready`
- 目标分镜所属组是否满足 `分镜切换 == len(分镜明细[])`
- `剧情桥段` 是否只包含目标分镜可见事实
- `全局风格` 是否与上游逐字一致
- `出场角色及穿搭` 是否已进入帧级 prompt 的压缩块
- 是否保留当前分镜组内的 `xx秒-xx秒｜分镜<组内序号>：`，且未写成 `分镜 <ID> 的 xx秒-xx秒`
- `运动表现 / 分镜构图 / 景别 / 运镜手法 / 镜头视角` 是否已被消费，而非继续按旧字段名理解
...
- 除镜级序号标签外，是否仍有字段标题或完整四段式 `分镜ID` 泄露
...
- prompt 没写出目标分镜所属组内的 `xx秒-xx秒｜分镜<组内序号>：`，写成了 `分镜 <ID> 的 xx秒-xx秒`，或把时间误当成全集时间线
- prompt 里仍然残留旧字段标题，或因 schema / 模板升级后漏掉 `出场角色及穿搭 / 角色表现 / 运动表现 / 分镜构图 / 运镜手法 / 镜头视角`
