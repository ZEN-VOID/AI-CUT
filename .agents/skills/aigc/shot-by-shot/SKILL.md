...
本技能不替代 AIGC 主链任何 canonical 真源：不改写 `2-编剧` 正文、`3-美学` 六类风格协议、`4-导演` 批注稿、`5-表演` 表演稿、`7-分镜` 分镜稿、`8-摄影` 摄影稿、`9-光影` 光影稿、`11-主体` 设计稿，也不替代 `分镜脚本.md` 的 19 列表格合同。参考片中的剧情、台词、角色、构图、镜头顺序、具体美术表达和受版权保护的独特表达不得进入项目 canonical 产物。

## Context Loading Contract

- 每次调用 `$aigc-shot-by-shot` 时，必须同时加载本目录 `SKILL.md + CONTEXT.md`。
- 每次调用本技能时，必须同时加载同目录 `CONTEXT.md`。
- 若任务绑定 `projects/aigc/<项目名>/`，必须先加载项目根 `MEMORY.md`，再加载项目 `CONTEXT/` 中与参考片、美学、编剧、导演、表演、分镜、摄影、光影、主体设计或制作限制相关的文件；`0-初始化/north_star.yaml`、`team.yaml` 仅在旧项目已存在且本轮需要回读时按 legacy supporting context 加载。
- 若本轮输出服务 `3-美学`，必须按任务目标加载 `.agents/skills/aigc/3-美学/SKILL.md + CONTEXT.md`，并按需加载六个子技能的 `SKILL.md + CONTEXT.md`：`画面基调`、`角色风格`、`场景风格`、`道具风格`、`摄影风格`、`分镜风格`。
- 若本轮输出服务 `2-编剧`、`4-导演`、`5-表演`、`7-分镜`、`8-摄影`、`9-光影` 或 `11-主体`，必须按需加载对应 owning stage 的 `SKILL.md + CONTEXT.md` 并遵守字段边界。
...
- 生成供 `3-美学` 六子技能消费的风格解析 side context：`画面基调解析.md`、`角色风格解析.md`、`场景风格解析.md`、`道具风格解析.md`、`摄影风格解析.md`、`分镜风格解析.md`。
- 生成供 `2-编剧`、`4-导演`、`5-表演`、`7-分镜`、`8-摄影`、`9-光影` 或 `11-主体` 消费的非 canonical 解析材料。
- 生成标准表格式 `分镜脚本.md`；表头、列顺序和编排继续完全沿用 `references/storyboard-script-contract.md`。
- 审查或修复既有拉片包中的证据不足、参考污染、输出路径漂移、3-美学字段不对齐或分镜脚本表头错误。

Non-goals:

- 不把参考片具体镜头序列、构图、台词、角色脸、服装纹样、场景构图、道具纹章、地图文字或独特符号迁入项目。
- 不直接生成或覆盖 `3-美学` 的六类正式协议：`全局风格协议.md`、`角色风格协议.md`、`场景风格协议.md`、`道具风格协议.md`、`摄影风格协议.md`、`分镜风格协议.md`。
...
| --- | --- | --- | --- |
| `business_goal` | 明确本轮是拉片研究、3-美学 side context、编剧/导演/表演/分镜/摄影/光影/主体桥接、分镜脚本投影、审查或修复 | 用户请求、目标阶段、输出文件清单 | `FAIL-SBS-BUSINESS-GOAL` |
...
- 至少一个目标用途：`3-美学` 风格解析、`2-编剧`、`4-导演`、`5-表演`、`7-分镜`、`8-摄影`、`9-光影`、`11-主体`、`分镜脚本`、项目风格库、单次研究报告、审查或修复。

Optional input:

- 参考片名、导演/摄影师、时间码范围、镜头粒度、目标项目、目标集/场/分镜组、下游模型限制、用户长期审美偏好。
- 用户指定“只看摄影”“只看角色风格”“只输出分镜脚本”“只做 3-美学六路解析”等限制。
...
| `aesthetic_bridge` | 明确服务 `3-美学`、风格解析、全套美学或任一美学子技能 | 主报告 + 六类或点名类 3-美学 side context |
| `targeted_stage_bridge` | 明确服务 `2-编剧`、`4-导演`、`5-表演`、`7-分镜`、`8-摄影`、`9-光影`、`11-主体` 或 `分镜脚本` | 主报告 + 被点名 stage 解析 + 可选 `分镜脚本.md` |
...
| `repair` | 既有包缺证据、字段不对齐、路径漂移、3-美学拆分错误或分镜表头错误 | 最小修复后的拉片包与修复报告 |
...
| `aesthetic_bridge` | 命中 `3-美学`、六子技能、风格解析、画面基调、角色/场景/道具/摄影/分镜风格 | `Aesthetic Bridge Path` | `N1,N2,N3,N4,N5A,N6,N7,N8` | `references/aesthetic-style-analysis-contract.md`, `.agents/skills/aigc/3-美学/SKILL.md` | `FAIL-SBS-TYPE-AESTHETIC` |
| `targeted_stage_bridge` | 明确服务 `2-编剧`、`4-导演`、`5-表演`、`7-分镜`、`8-摄影`、`9-光影`、`11-主体` 或 `分镜脚本` | `Targeted Stage Bridge Path` | `N1,N2,N3,N4,N5B,N6,N7,N8` | `references/adaptation-output-contract.md`, `references/storyboard-script-contract.md` | `FAIL-SBS-TYPE-STAGE` |
| `stage_bridge` | 命中 `2-编剧`、`4-导演`、`5-表演`、`7-分镜`、`8-摄影`、`9-光影`、`11-主体` 或分镜脚本 | `Stage Bridge Path` | `N1,N2,N3,N4,N5B,N6,N7,N8` | `references/adaptation-output-contract.md`, `references/storyboard-script-contract.md` | `FAIL-SBS-TYPE-STAGE` |
...
| `N5A-AESTHETIC-BRIDGE` | 对齐 `3-美学` 六子技能关注点 | N4 输出、3-美学合同 | 生成点名或六路 side context：画面基调、角色风格、场景风格、道具风格、摄影风格、分镜风格；只给候选证据和可迁移原则 | `aesthetic_bridge_matrix`，每个启用子技能 1 行，含 source refs、allowed seeds、do_not_import | `N6` | 不得替代六个正式协议；不得把参考具体内容写入 prompt 终稿 |
...
flowchart TD
    ...
    E --> F1["N5A-AESTHETIC-BRIDGE: 3-美学 six-way side context"]
...
flowchart LR
    ...
    P --> M["摄影风格解析"]
    P --> B["分镜风格解析"]
    T -. "side context only" .-> T2["3-美学/画面基调"]
    C -. "side context only" .-> C2["3-美学/角色风格"]
    S -. "side context only" .-> S2["3-美学/场景风格"]
    O -. "side context only" .-> O2["3-美学/道具风格"]
    M -. "side context only" .-> M2["3-美学/摄影风格"]
    B -. "side context only" .-> B2["3-美学/分镜风格"]
```

## Multi-Subskill Continuous Workflow

- 命中 `3-美学整体`、`全套美学` 或等价请求时，风格解析默认覆盖六个 aesthetics side context；命中单个或部分子技能时只输出被点名解析，不补空未点名子技能。
...
| `references/aesthetic-style-analysis-contract.md` | `aesthetic_bridge` 或 `FAIL-SBS-AESTHETIC-BRIDGE` | 3-美学六子技能 side context 字段细则 | 替代六子技能正式协议 | `N5A-AESTHETIC-BRIDGE` |
| `references/adaptation-output-contract.md` | `stage_bridge` 或既有非美学桥接修复 | 编剧/导演/表演/分镜/摄影/光影/主体/分镜脚本 side context 汇流细则 | 改写 stage canonical 文件 | `N5B-STAGE-BRIDGE` |
| `references/screenwriter-style-analysis-contract.md` | 输出 `编剧风格解析.md` | `2-编剧` side context 细则 | 写导演批注、表演稿、摄影、分镜编号或镜头参数 | `N5B-STAGE-BRIDGE` |
| `references/cinematography-style-analysis-contract.md` | 输出摄影解析给 `8-摄影`、`9-光影` 或 `3-美学/摄影风格` | 摄影/光影语法 side context 细则 | 固定照抄参考镜头顺序 | `N5A-AESTHETIC-BRIDGE` / `N5B-STAGE-BRIDGE` |
| `references/global-style-analysis-contract.md` | legacy 包修复或旧 `全局风格解析.md` 兼容审查 | 旧全局风格 side context 细则 | 作为新 3-美学输出真源 | `R2-SYNC-REPAIR` |
...
| `3-美学` 风格解析是否按画面基调、角色、场景、道具、摄影、分镜六子技能拆分，并只作为 side context？ | `GATE-SBS-05A` | `FAIL-SBS-AESTHETIC-BRIDGE` | `N5A-AESTHETIC-BRIDGE` | aesthetic bridge matrix、enabled subskills |
...
| 摄影风格解析是否对齐 `3-美学/摄影风格` 的构图秩序、景别、机位、运动、连续性，而不写具体镜头正文？ | `GATE-SBS-CINE-AES` | `FAIL-SBS-CINE-BRIDGE` | `N5A-AESTHETIC-BRIDGE` | camera grammar profile |
...
| 3-美学风格解析仍按旧聚合包输出 | `N5A-AESTHETIC-BRIDGE` |
...
| `CHK-SEMANTIC` | 定稿 3-美学六路 side context 或版权边界 | 检查六子技能边界、copy 禁区、side context 状态 | aesthetic bridge matrix、do_not_import | `FAIL-SBS-CHECKPOINT-SEMANTIC` |
...
- `aesthetic-six-way-bridge`: 参考视频拉片后输出 3-美学六子技能 side context。
- `storyboard-script-preserved`: 只要求分镜脚本时保持 Numbers 示例 19 列不变。
- `repair-legacy-style-pack`: 修复旧 `全局风格解析.md` / `设计风格解析.md` 聚合包，使其对齐 3-美学拆分。
...
- `画面基调解析.md` 对齐 `3-美学/画面基调`：媒介、渲染、光影范式、美学哲学、大师/作品锚点候选、全局 prompt 候选和污染审计；不得导入具体内容。
- `角色风格解析.md` 对齐 `3-美学/角色风格`：轮廓语言、妆发纪律、服装结构倾向、体态张力、年龄质感和表演外观边界；不得写具体角色卡。
- `场景风格解析.md` 对齐 `3-美学/场景风格`：空间类型、空间压力、材质/光影秩序、空镜规则、世界地理视觉信号；不得写具体场景清单。
- `道具风格解析.md` 对齐 `3-美学/道具风格`：道具功能层级、材质体系、细节密度、符号边界和生成禁区；不得写具体道具清单。
- `摄影风格解析.md` 对齐 `3-美学/摄影风格`：构图秩序、景别体系、机位高度、镜头运动、运动速度、视角关系、节奏密度和稳定性；不得写具体分镜正文。
- `分镜风格解析.md` 对齐 `3-美学/分镜风格`：节奏密度、景别流转、镜头组合、转场逻辑、动作承接、信息揭示、段落推进和下游容错；不得替代 `分镜脚本.md`。
...
- `Rule Evidence Map`：把证据、边界、3-美学六路、stage handoff、分镜脚本表头、版权和输出路径映射到正文位置。
...
- `3-美学` 风格解析对齐六子技能关注点，且只作为 side context。
...
| `FIELD-SBS-05A` | aesthetic bridge | 3-美学 side context 对齐画面基调、角色、场景、道具、摄影、分镜风格六路或用户点名子路由 | `FAIL-SBS-AESTHETIC-BRIDGE` |
...
| `PASS-SBS-04A` | `FIELD-SBS-05A` | 哪些结果能服务 `3-美学` 六个子技能 | 投影 aesthetic side context | aesthetic bridge matrix |
...
| `PASS-SBS-04A` | 风格解析对齐 `3-美学` 六子技能关注点，且不替代正式协议 | `FAIL-SBS-AESTHETIC-BRIDGE` | `references/aesthetic-style-analysis-contract.md` |
...
- 把风格解析继续写成旧 `全局风格解析.md` + `设计风格解析.md` 聚合包，而不是对齐 `3-美学` 六子技能关注点。
- 给 `3-美学` 解析写成正式协议正文、prompt 终稿、角色卡、场景清单、道具清单、具体镜头正文或图片/视频生成 prompt。
- 给 `分镜脚本.md` 漏掉 Numbers 示例列、改变列顺序、把多镜合并成剧情段落，或照搬示例具体表达。
- 脚本替代 LLM 做逐镜审美判断或临摹策略归纳。

必经链路：

`Symptom -> Direct Analysis/Bridge/Rights Failure -> shot-by-shot Section Owner -> 3-美学 subskill / stage bridge / storyboard script Contract -> AGENTS.md LLM-first / Skill 2.0 Rule`
