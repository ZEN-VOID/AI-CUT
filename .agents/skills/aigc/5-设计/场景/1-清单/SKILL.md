...
`场景清单` 负责从 `4-分组` 逐集分镜组稿的组底 YAML 中抽取并归并场景主体，输出供后续场景设计阶段使用的 table 式 Markdown 清单。它只建立“哪些场景需要设计”的清单真源，不扩写场景设定、不生成视觉方案、不替代后续场景设计稿。
...
- 上游唯一准确信息来源为 `projects/aigc/<项目名>/4-分组/第N集.md` 每个分镜组底部 YAML 的 `场景` 字段；必要时只允许回查同一分镜组正文和场景标题作为证据。
...
- 用户要求“场景清单”“从分组稿提取场景”“生成 5-设计/场景/1-清单”等任务。
- 已完成或部分完成的 `4-分组` 逐集稿；默认按集读取 `第N集.md`。

Required input:

- 可定位、可读取的 `projects/aigc/<项目名>/4-分组/第N集.md`。
- 每个可消费分镜组底部存在 YAML 统计块，且包含 `场景` 字段。
- 至少一个目标集号，或允许默认处理 `4-分组/` 中全部 `第N集.md`。
...
- 上游 `4-分组/第N集.md` 不存在、不可读，且用户没有提供替代文件。
...
| --- | --- | --- | --- |
| `FIELD-SCENE-LIST-01` | 输入取证 | 项目路径、目标集号、上游 `4-分组/第N集.md` 可回指 | `FAIL-SCENE-LIST-01` |
...
`Symptom -> Direct Script/Prompt Overreach -> 场景清单 Section Owner -> 4-分组 YAML Contract -> AGENTS.md LLM-first / Skill 2.0 Rule`
