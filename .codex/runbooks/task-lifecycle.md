...
4. `执行`
   ...
   - 对 `aigc` 项目工作流，canonical runtime 为 `projects/aigc/<项目名>/`
...
- `aigc` 项目型工作流：`projects/aigc/<项目名>/`，并以此作为运行时唯一真源
...
- 当 `aigc` 进入 `bootstrap_compat` 改造窗口时，保留 `projects/aigc/<项目名>/`、治理工件 carriers、卫星技能入口与 review gate；阶段内部合同与深层 runtime 检查可按审计脚本的兼容模式降级
