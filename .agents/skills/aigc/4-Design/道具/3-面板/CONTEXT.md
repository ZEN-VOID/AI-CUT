...
- outcome: 为 `.agents/skills/aigc/4-Design/道具/3-面板` 建立了可执行 skill 包，并把参考仓的 prop panel contract 映射到当前 `projects/<项目名>/4-Design/...` runtime。
...
- final_fix_or_heuristic: 保留 reference 的 panel dossier 模板和门禁语义，但将输入锁定为 `2-设计` 产物，输出收口到 `projects/<项目名>/4-Design/道具/3-面板/第N集/`，并在本轮只生成 layout JSON + manifest。
