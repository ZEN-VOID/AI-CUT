...
`4-摄影` 将 `projects/aigc/<项目名>/3-运动/第N集.md` 的逐集运动强化稿升级为带大师级分镜、摄影、运镜、光影、色彩和边界交出锚点的摄影分镜明细稿；用户明确跳过运动强化时可回退 `2-编导/第N集.md`。组间与跨场景创意转场由下游 `5-分组` 连接件承接。
...
- 分镜落盘格式：每条分镜固定写成 `分镜N（约X秒）:`，默认应用短剧·AIGC 时值压缩；对白、旁白和画外音台词量必须进入时值预算；非 `slow_burn/hold` 的 `约3秒` 以上镜头必须有台词、读秒、表演变化、复杂调度、空间重置或高点证据，情绪类 `slow_burn/hold` 镜头必须有可见微动态、静止压力、极慢运动或框内变化。
