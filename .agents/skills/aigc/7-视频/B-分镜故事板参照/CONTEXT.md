...
| 有故事板图却走 `image2video` 把故事板误当首帧 | provider 路由层 | 改为 `multimodal2video` 并在 prompt 中声明 `@图1` 是 storyboard reference | `dreamina-handoff-contract.md` 固定参照图语义 | command preview 为 `multimodal2video --image` |
| 无参照图时仍调用 `multimodal2video` 导致必填输入失败 | provider 路由层 | 改为 `text2video` | YAML 根据 `reference_images` 判定 command_type | 无图 job 的 command_type 是 `text2video` |
| 批量并发提交后 submit_id 丢失 | 队列治理层 | 从终端输出、`list_task` 或 tasks.db 回填 queue ledger | 每个 worker 写独立临时结果，主流程汇流 | ledger 每组有 queue row 和 next_action |
| `query_result` 已成功但下载文件不完整 | 下载层 | 删除半截文件后重试下载，必要时用媒体直链 | 采用 `dreamina-cli` 的下载超时经验 | 本地 MP4 可被读取且大小非零 |
...
5. 若问题在 Dreamina 命令，回 `references/dreamina-handoff-contract.md` 和 `.agents/skills/cli/dreamina-cli/SKILL.md`，先确认子命令、模型、duration、poll 和图片路径。
6. 若问题在批量执行，先恢复 queue ledger，再查询 Dreamina 远端状态，不把终端滚屏当唯一状态来源。
7. 修复后按 `review/review-contract.md` 复核所有 gate，并在最终报告中列出非阻断事项。

## Reusable Heuristics

- 本技能的关键不是“重新写一个视频故事”，而是把已经在 `4-分组` 中裁好的完整组内容稳定交给 Dreamina。
- 故事板图是参照，不是第一帧。除非用户明确要求首帧图生视频，默认不要把 storyboard sheet 交给 `image2video`。
- 缺故事板图时不应停机；这类任务天然可以降级为 `text2video`，但报告要写清没有图片参照。
- `第N集-dreamina-batch.yaml` 是提交前的可审查真源；CLI 命令是它的投影，不要让手写命令成为第二真源。
- 并发只发生在“提交/查询每个独立 group job”层；报告、结果总表和 queue ledger 的最终收敛必须单线程完成。
- Dreamina `--poll` 不是完成承诺；批量视频默认应以 `submit_id + query_result + queue ledger` 收口。
