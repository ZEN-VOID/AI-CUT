...
| DeepSeek provider 返回 markdown 但缺 YAML 头 | provider output validation | 阻断写回，调整 prompt 或增大 `max_tokens` 后重试 | 写回前固定校验 `润色模型: Deepseek`、`初稿来源` 与 `字数` | 非法输出不落到 `第N章.md` |
...
| DeepSeek 最小修补却继承既有整章润色稿的短句化、规整分段 | distribution collapse | 在 prompt 中声明以 `3-初稿` 文本分布为主，不继承既有 `4-润色` 的短句化重排 | system prompt 和父级 gate 固定“保骨架、少动、只修坏处” | 平均句长、句长波动和段落密度更接近初稿而非整章润色稿 |
| 用户反馈 AI 检测人工成分下降 | anti-ai detector regression | 做同模型对照：DeepSeek 整章润色 vs DeepSeek 最小局部修补；默认采用后者 | C lane 作为润色默认 lane，但任务语义固定为最小局部修补 | 检测样本同结构处理后，最小修补稿优先作为候选 accepted manuscript |
...
4a. 若正文比初稿明显短句化、分段规整化或通用顺滑化，回到最小局部修补约束；不要继续追加“去 AI 味”泛化口号。
5. 若上下章像各写各的，优先检查 messages pack 是否只截取了上一章开头；承接判断应优先看上一章末尾，而不是章节开篇。
6. 若 YAML 头缺字段，只检查并补正 `润色模型: Deepseek`、`初稿来源` 与 `字数`；上下文缺口回到 context pack / sidecar，而不是补进正文头。
...
- DeepSeek 流在润色阶段的价值不是输出更统一的章节，而是用强推理判断哪里该少量修补、哪里应保留初稿不动。
...
- AI 检测对照表明，同一 DeepSeek 下“最小局部修补”比“整章润色”更能保留初稿文本分布；后续默认按这个方向执行。
