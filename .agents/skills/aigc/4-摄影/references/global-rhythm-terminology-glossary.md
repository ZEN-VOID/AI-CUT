...
`节拍` 在 `4-摄影` 中等价于“有效分镜触发点”。快节奏短视频平台默认采用 `trigger-first` 口径：`BT-01~BT-16` 命中的有效触发点默认 1:1 落为一个 `分镜N（约X秒）:`。只有当多个触发点能在同一镜头内清楚完成，且不损失平台节奏、观看结果、下游 payload、人物动作连续性或 AIGC 执行稳定性时，才合并。
