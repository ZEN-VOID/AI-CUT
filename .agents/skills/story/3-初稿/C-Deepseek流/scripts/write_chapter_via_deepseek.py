#!/usr/bin/env python3
"""
Bridge script for `story/3-初稿/C-Deepseek流`.
...
def _validate_generated_markdown(text: str, chapter_num: int) -> None:
    ...
    if not body_after_heading.strip():
        raise ValueError("DeepSeek 返回正文为空，禁止写回。")
    ...
    _validate_no_meta_narrative_terms(body_after_heading, "DeepSeek 返回")
...
def main() -> int:
    ...
    if output_dir:
        ...
        with tempfile.TemporaryDirectory(prefix="story-drafting-deepseek-") as temp_output_dir:
