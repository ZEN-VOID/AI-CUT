...
| screenwriting | `projects/aigc/<项目名>/2-编剧/第N集.md`、`2-编剧/执行报告.md` | legacy `2-编导` / `3-Detail` 只在旧项目或用户点名时兼容回读 |
| aesthetic | `projects/aigc/<项目名>/3-美学/**` | 视觉基调、角色/场景/道具/分镜/摄影风格协议 |
| director | `projects/aigc/<项目名>/4-导演/第N集.md`、`4-导演/执行报告.md` | 导演批注事实 |
| performance | `projects/aigc/<项目名>/5-表演/第N集.md`、`5-表演/执行报告.md` | 表演融合稿事实 |
| atmosphere | `projects/aigc/<项目名>/6-氛围/第N集.md`、`6-氛围/执行报告.md` | 氛围增强事实 |
| storyboard | `projects/aigc/<项目名>/7-分镜/第N集.md`、`7-分镜/执行报告.md` | 分镜拆分与动作连续性 |
| cinematography | `projects/aigc/<项目名>/8-摄影/第N集.md`、`8-摄影/执行报告.md` | 分镜明细与摄影稿事实 |
...
rg --files "$PROJECT_ROOT/2-编剧" | rg '第[0-9]+集\\.md$'
rg --files "$PROJECT_ROOT/3-美学"
rg --files "$PROJECT_ROOT/4-导演" | rg '第[0-9]+集\\.md$'
rg --files "$PROJECT_ROOT/5-表演" | rg '第[0-9]+集\\.md$'
rg --files "$PROJECT_ROOT/6-氛围" | rg '第[0-9]+集\\.md$'
rg --files "$PROJECT_ROOT/7-分镜" | rg '第[0-9]+集\\.md$'
rg --files "$PROJECT_ROOT/8-摄影" | rg '第[0-9]+集\\.md$'
rg --files "$PROJECT_ROOT/9-光影" | rg '第[0-9]+集\\.md$'
rg --files "$PROJECT_ROOT/10-分组" | rg '第[0-9]+集\\.md$'
rg --files "$PROJECT_ROOT/11-主体"
rg --files "$PROJECT_ROOT/12-图像"
rg --files "$PROJECT_ROOT/13-画布"
rg --files "$PROJECT_ROOT/14-审片"
...
| 当前 2-10 阶段查询是否默认读取 `2-编剧 / 3-美学 / 4-导演 / 5-表演 / 6-氛围 / 7-分镜 / 8-摄影 / 9-光影 / 10-分组`，而不是默认回到旧 `2-编导 / 3-运动 / 4-摄影`？ | `GATE-QUERY-07` | `FAIL-QUERY-CARRIER` | `N3-carrier-read` | 报告当前阶段 carrier 路径、集号匹配方式、执行报告路径和未命中的 canonical 路径。 |
...
| 角色、场景、道具等主体资产查询是否默认读取 `11-主体/{场景,角色,道具}/`，并把旧 `4-Design/` 仅作为兼容证据而非默认资产真源？ | `GATE-QUERY-06` | `FAIL-QUERY-LEGACY` | `N3-carrier-read` | 报告命中的 `11-主体` 子目录、旧 `4-Design` 命中情况和 canonical/legacy 差异。 |
| 分镜画面、故事板、视频、参照视频与审片报告查询是否默认读取 `12-图像`、`13-画布` 与 `14-审片`，并把旧 `5-Image/6-Video/7-Cut` 明确降级为 legacy fallback？ | `GATE-QUERY-06` | `FAIL-QUERY-LEGACY` | `N3-carrier-read` | 报告当前图像/视频/审片 carrier、旧路径 fallback、是否存在 provider 输出、阶段工件与审片结论差别。 |
