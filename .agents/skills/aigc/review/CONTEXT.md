...
- 新 `aigc` 文本主链阶段名以 `0-初始化 / 1-分集 / 2-美学 / 3-主体 / 4-编剧 / 5-导演 / 6-分镜 / 7-摄影 / 8-分组` 为主；`2-编导 / 3-运动 / 4-摄影` 与 archived `5-表演 / 6-氛围 / 9-光影` 只作为 legacy/迁移审计线索；`9-图像 / 10-画布` 作为下游生成链审计，审片只作为 review evidence；旧英文 stage 只作为兼容线索。
