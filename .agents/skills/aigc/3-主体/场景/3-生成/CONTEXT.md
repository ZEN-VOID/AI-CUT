...
8. 批量生成的 Skill 2.0 包若只有表格没有图，优先补 `SKILL.md` 的总览图，再补 `SKILL.md` runtime spine 的执行拓扑图；不要把完整细则倒灌回 `SKILL.md`。
...
- 场景生成 skill 升级时优先守住 source deconstruction、主图 JSON、多视图 `view_image` 和项目持久化 gate；旧正文可以保留，但执行主链必须回到 `SKILL.md`。
