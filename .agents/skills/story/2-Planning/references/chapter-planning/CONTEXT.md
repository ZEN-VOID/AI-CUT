...
- 本文件是 `chapter-planning` 子模块的局部经验层，只服务 `2-Planning` 的 Step 2。
- 加载顺序固定为：先读同目录 `module-spec.md`，再按需读取本文件。
- 跨模块的挂章规律、holomap 收束经验仍回写到 `2-Planning/CONTEXT.md`。
...
| --- | --- | --- | --- | --- |
| 章节规划只有数量没有功能槽 | step contract | 补齐 chapter / volume blocks 与 function slots | 在 `module-spec.md` 固化章节容器优先级 | Step 3-8 能稳定挂入章节容器 |
| 体量与对象密度失衡 | density contract | 回修 density contract 的区间带 | 在 Step 2 统一裁决密度，不让后序重发明 | holomap 能回指各类对象负荷 |
| drafting 无法挂章 | handoff contract | 回到本模块修章节容器，而非直接补 holomap | 固化“挂章失败先回 Step 2” | 下游不再出现临时拼章板 |
