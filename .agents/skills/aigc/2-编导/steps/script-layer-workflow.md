...
| `constraint_profile` | 原文信息量保真、对白冻结、长对白节拍拆分、声画配对、slugline 稳定、小说表述二次画面化、主角内心独白保留、动作客观可拍、关键面部 beat 有 `表情特写` 或相邻字段承托、环境字段纯化、环境氛围承托、LLM-first、初始化团队综合上下文沉淀 |
...
| `complexity_source` | 场景解析、字段分流、声画配对、对白冻结、长对白节拍拆分、小说表述二次画面化、主角内心独白人称转换、动作客观性、环境纯度、环境氛围承托、保真与质量的优先级协调 |
...
| `N5-SCRIPT-DRAFT` | LLM 直出逐集编剧稿 | 场景表、字段映射、对白锁、`long_dialogue_beat_map`、`scene_rhythm_profile`、`information_asymmetry_map`、`audience_knowledge_state`、`audience_psychology_seed`、`conflict_legacy_seed`、`dialogue_subtext_map`、`novel_expression_transform_evidence`、`init_team_synthesis_context`（如有）、上游正文 | 写入 frontmatter、`【剧本正文】`、场景标题和字段化正文；把长对白节拍、场景节奏、信息差、观众心理基线、对白戏剧动作、小说表述转译和初始化综合上下文拆入对应句段但不改写上游真源或对白；模板占位和内部规则只能指导写作，不得输出到正文 | `第N集.md` 草稿、`faithful_projection_trace`、`long_dialogue_beat_map` | `N6-SCRIPT-REVIEW` | 小说转译上下文未越权；长对白拆分后拼回保真；信息释放、观众心理基线和对白戏剧动作可被下游消费；无第二字段体系，无场景末尾总结块，无占位/规则说明泄露 |
| `N6-SCRIPT-REVIEW` | 保真、对白、长对白节拍、声画、slugline、小说转译、信息差、观众心理、场景节奏、对白潜台词、思维·执行节点与质量门禁 | candidate 草稿、上游正文、`review/review-contract.md`、各节点证据、`thinking_action_node_ledger` | 运行机械校验或人工 review；逐项执行 `GATE-SCRIPT-01..23`；检查长对白拆分是否逐字拼回、是否每个 beat 有画面/反应承托；检查每个关键节点是否具备 `judgment_question / actions_taken / evidence_keys / route_out / gate_status / source_owner`；把 finding 映射到最早责任节点和 source owner | 校验结果、问题清单、`thinking_action_node_ledger`、`gate_to_node_repair_map`、repair targets | `N6R-SCRIPT-REPAIR` 或 `N7-SCRIPT-WRITEBACK` | 无阻断项才可写回；质量建议不得掩盖保真、对白、长对白节拍、声画、小说转译、信息差、观众心理、节奏、潜台词或节点退化问题 |
...
- 若用户要求"C 路线"新增对白、新桥段、新因果、新规则或新事件结果，必须阻断 canonical 写回并另行授权为候选稿，不得混入 `2-编导` script layer 默认主线。
