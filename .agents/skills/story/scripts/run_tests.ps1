...
$env:TMP = $tmpRoot
$env:TEMP = $tmpRoot
$scriptsRoot = Join-Path $ProjectRoot ".agents\\skills\\story2026\\scripts"
...
if ($Mode -eq "smoke") {
    python -m pytest -q `
        .agents/skills/story/scripts/data_modules/tests/test_extract_chapter_context.py `
        .agents/skills/story/scripts/data_modules/tests/test_rag_adapter.py `
        --basetemp $baseTemp `
        --no-cov `
        -p no:cacheprovider
    exit $LASTEXITCODE
...
python -m pytest -q `
    .agents/skills/story/scripts/data_modules/tests `
    --basetemp $baseTemp `
    -p no:cacheprovider
exit $LASTEXITCODE
