...
description: Use when `5-设计/3-面板/场景` needs to consume scene design outputs, quote only their prompt section, write 16:9 3x3 scene panel layout JSON, and automatically call `built-in imagegen` unless JSON-only is requested.
...
`5-设计/3-面板/场景` 是场景设计后的面板化 leaf。它参照 `/Volumes/AIGC/AIGC-ZEN-VOID/.agents/skills/aigc2026/3-设定/4-面板/场景面板` 的成熟配置，但按当前仓 runtime 重定输入、落点与 SMART 生图规则。

核心职责：

1. 消费 `5-设计/场景/2-设计` 的各种设计产物。
...
- `.agents/skills/aigc/5-设计/3-面板/SKILL.md`
- `.agents/skills/aigc/5-设计/3-面板/_shared/smart-image-handoff-contract.md`
- `.agents/skills/aigc/5-设计/3-面板/_shared/panel_auto_generate.py`
- `.agents/skills/aigc/_shared/image-generation-execution-contract.md`
- `.agents/skills/aigc/5-设计/2-设计/场景/SKILL.md`
...
- `projects/aigc/<项目名>/4-设计/场景/2-设计/第N集/scene_design.json`
- 同目录逐场景 Markdown（可选）
...
- `projects/aigc/<项目名>/4-设计/场景/3-面板/第N集/`
...
```json
{
  ...
  "output_dir": "projects/aigc/<项目名>/4-设计/场景/3-面板/第N集/generated/SCN-001-ScenePanel",
...
```bash
python3 .agents/skills/aigc/5-设计/3-面板/场景/scripts/generate_scene_panels.py \
  --project "<项目名>" \
  --episode "第1集" \
...
flowchart TD
    ...
    B -->|"default batch from 5-设计 series"| C["continuous-batch"]
