...
当前执行真源是 `2-美学/画面基调/全局风格协议.md`。题材类型和跨阶段 handoff 由 `2-美学/类型风格.md`、`2-美学/美学总览.md` 和对应风格协议补充；旧初始化风格载体不再作为本阶段上下文来源。
...
`全局风格：` 是字段标题，必须可见。字段内容只保留单行场景化整理，不是摘要删减，也不是完整直引。不得输出 `Visual Slogan`、`Design Principle`、`Visual Gene Profile` 或 `Negative Traits` 的独立行；不得把项目 `MEMORY.md`、`CONTEXT/` 或旧初始化载体的风格偏好替代 `2-美学/画面基调.Global Style Prompt`。项目记忆只能作为当前组匹配和取舍的辅助约束。
...
1. 读取 `2-美学/画面基调/全局风格协议.md` 的 `Global Style Prompt`；必要时读取 `2-美学/类型风格.md`、当前集优先/项目级回退的 `2-美学/<风格>/风格协议.md` 和项目 `MEMORY.md` 作为边界辅助。
