#!/usr/bin/env python3
"""
Bridge script for `story/4-润色/C-Deepseek流`.

It assembles the polishing context pack from the canonical 3-初稿 chapter,
then delegates the actual prose polish to `.agents/skills/api/deepseek`.
"""
...
SYSTEM_PROMPT_PATH = POLISHING_DIR / "templates" / "deepseek-system-prompt.md"
...
def _resolve_polishing_mode(args: argparse.Namespace, chapter_path: Path) -> str:
    ...
    if args.mode == "chapter_polish" and target_exists and writeback_enabled:
...
def main() -> int:
    ...
    required_files = [book_plan_path, volume_plan_path, chapter_plan_path, north_star_path, memory_path, source_draft_path]
