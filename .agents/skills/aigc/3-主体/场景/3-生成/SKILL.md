...
`$aigc-scene-generation` 消费上游 `$aigc-scene-design` 已完成的单场景设计文档，以关联 libTV 项目和画布上的图片节点生成为核心，调用 `.agents/skills/cli/libTV` 生成场景主图与场景多视图主体设计图。本阶段只执行图像生成、libTV 画布节点落位、提示词 JSON 落盘、路径归档与质量复核，不重新设计场景主体、不改写上游设计真源。
...
- 执行 Step2 多视图前，作为 reference image 的场景主图必须已经是同一 libTV 画布中的图片节点，且多视图 JSON 记录 `reference_node_name`、可选 `reference_node_key` 与 `reference_context_status: linked_in_libtv_canvas`。
...
5. 按 `types/scene-generation-type-map.md` 形成 `generation_profile`，决定 `main_only`、`multiview_only`、`single_scene`、`batch_scene`、`incremental_fill`、`reuse_existing_asset`、`state_variant_generation` 或 `repair`；已有主图、多视图和 JSON 默认跳过，覆盖必须有明确授权。
...
7. Step2：套用 `templates/scene-multiview-prompt.json`，以对应主图节点或状态变体主图节点作为同画布参照；普通新主体拼接场景 Midjourney 后缀，状态变体继承参考图风格与比例并使用 `Lib Image`，创建或更新对应 `-多视图` 节点，并落同名 JSON 提示词记录。
...
- Completion gate: 已加载 `SKILL.md + CONTEXT.md`、项目记忆、上游设计、libTV 合同、Midjourney 共享后缀和主体图复用规则；prompt JSON 由 LLM 基于上游 `4. 解构` 裁决；目标画布 UUID 与普通新主体 Midjourney V8.1 modelKey 已解析；已扫描项目 `3-主体` 既有主体图；同主体同状态复用或上传；画布节点已下载或确认保存到项目 `场景/3-生成/`；状态变体使用 `Lib Image`、既有参考图和状态后缀；主图先生成/复用并持久化；多视图引用同一画布主图节点；无脚本化伪差异；review gate 通过。
...
4. 多视图生成必须以同一主体的主图节点作为同画布参照；真实生成模式下，该主图节点已在目标 libTV 画布中可引用，并记录 `reference_context_status: linked_in_libtv_canvas`。
...
| `OUTPUT-SCENE-MULTIVIEW-IMAGE` | libTV 画布 `image` 节点产物，模型默认 Midjourney V8.1，后缀含 `--ar 16:9 --hd --style raw` |
...
- 状态变体命名为 `<主体ID>-<主体名称>-<状态后缀>-主图` 与 `<主体ID>-<主体名称>-<状态后缀>-多视图`。
...
| `constraint_profile` | 不重设计；默认 libTV 画布 `image` 节点；生成前先跨集扫描既有主体图；同主体同状态复用或上传；同主体新状态改用 `Lib Image`；主图先持久化；多视图前必须引用同画布主图节点；场景后缀固定含 `--ar 16:9 --hd --style raw`；无覆盖许可则版本化 | 本合同、libTV 合同、共享 Midjourney YAML、主体图复用规则、review | `FAIL-SCENE-GEN-BUSINESS-CONSTRAINT` |
...
| `topology_fit` | 串行 `source -> main node -> JSON -> same-canvas reference -> multiview node -> JSON -> review` 保护证据链；repair 节点处理持久化和版本化；review_only 避免写入 | Visual Maps、节点表、Type Routing Matrix | `FAIL-SCENE-GEN-TOPOLOGY-FIT` |
...
| `multiview_only` | 已有主图且只要求 Step2 | 同画布主图节点 + 多视图 + 多视图 JSON | `N1,N2,N3,N6,N7,N8` | `templates/scene-multiview-prompt.json`, `review/review-contract.md` | `FAIL-SCENE-GEN-TYPE-MULTIVIEW` |
...
| `state_variant_generation` | 同主体出现昼夜、季节、战后废墟、修复后等新状态 | 使用 `Lib Image`、既有参考图和状态后缀命名生成变体 | `N1,N2,N3,N4,N5,N6,N7,N8` | `../../_shared/主体图复用与状态变体规则.md`, `../../_shared/midjourney风格参数.yaml`, `review/review-contract.md` | `FAIL-SCENE-GEN-STATE-VARIANT` |
...
| `N3-PROFILE` | 判定生成模式、资产缺口和复用/变体分流 | source manifest、existing assets、用户模式 | 建立 `generation_profile`，处理复用、上传、覆盖/版本化和状态变体策略 | `generation_profile`、`canvas_action`、`generation_model_policy` | `N4-MAIN` / `N6-MULTIVIEW` / `N9-REPAIR` / `N8-REVIEW` | 不重设计主体；冲突有许可或版本化；状态变体使用 Lib Image |
...
| `N6-MULTIVIEW` | 生成并确保多视图本地存在 | 主图节点、多视图模板、上游解构 | 先锁定同画布主图节点，再创建/运行同名多视图 `image` 节点；若 canonical 已有同 stem 文件则记录 `already_present`，否则用 `libtv download` 持久化到本地 canonical 目录 | multi-view image path、reference_node_name、local_sync_status、download_stdout_path | `N7-MULTIVIEW-JSON` | `reference_context_status=linked_in_libtv_canvas`；本地 canonical 存在 |
...
| `../../../../cli/libTV/commands/download.md` | 任意真实生图、画布已有节点本地缺图或状态变体完成后 | `libtv download` 画布节点资源到项目 `场景/3-生成/` 的执行合同 | 替代画布生成、改变命名或下载到第二目录 | `N4-MAIN` / `N6-MULTIVIEW` |
...
| `multiview_only` / `FAIL-SCENE-GEN-TYPE-MULTIVIEW` | `templates/scene-multiview-prompt.json`, `review/review-contract.md` | `N6-MULTIVIEW -> N7-MULTIVIEW-JSON` | `C4-REFERENCE-VISIBLE` | reference_node_name + JSON pair |
...
| `evidence_count` | 每个目标至少 1 个 source design、1 个 generation_profile、每张图 1 个同名 JSON；多视图还需 1 个同画布 `reference_node_name` 证据 | `N2-SOURCE` 至 `N8-REVIEW` | `FAIL-SCENE-GEN-QUANT-EVIDENCE` |
...
| `ATTE-S20-02` | 注意力转移规则 | source 后看 mode；main 后立刻写 JSON；multiview 前先锁定同画布主图节点；review 后才 closeout | `Thinking-Action Node Map` |
| `ATTE-S20-03` | 注意力漂移检测 | 出现重新设计、用旧英文 prompt 替代 `4. 解构`、缺 JSON、未引用同画布主图节点或脚本套 prompt 即判漂移 | `Review Gate Binding` |
...
`test-prompts.json` 至少覆盖 single_scene 完整生成、main_only/multiview_only、repair/review 三类任务。评估默认 `eval_mode=dry_run`，检查来源锁定、Step1/Step2 顺序、同画布主图节点 gate、JSON 配对、项目持久化和 anti-script gate。
...
| `C4-REFERENCE-VISIBLE` | 多视图前主图节点已在同一 libTV 画布可引用并记录状态 | 只有路径未建立画布引用 | `reference_node_name`, `reference_context_status` | `N6-MULTIVIEW` |
...
| 场景主体图是否已确保存在于项目 `场景/3-生成/`？ | 本地 canonical 缺同 stem 文件、`local_sync_status` 非 already_present/synced/copied、需要下载时缺 `libtv download` 证据、或本地文件 stem 与节点名不一致即失败 | `FAIL-SCENE-GEN-LOCAL-SYNC` | `N4-MAIN` / `N6-MULTIVIEW` | `local_sync_required`、`local_sync_action`、`local_sync_status`、`local_asset_path`、`download_command`、`download_stdout_path` |
...
| 多视图是否先锁定同画布主图节点？ | `reference_context_status` 不是 `linked_in_libtv_canvas` 即失败 | `FAIL-SCENE-GEN-REFERENCE` | `N6-MULTIVIEW` | reference_node_name evidence |
...
- 要求脚本批量生成 prompt、静默覆盖旧资产、跳过同画布主图节点或改上游设计的输入必须转为返工或澄清。
