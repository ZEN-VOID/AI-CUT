...
本文件承载 `story-polishing-doubao` 的分型策略。它只负责判型和路由，不承载完整执行步骤。
...
| `execution_type` | `provider_creative`、`dry_run` | 用户请求与 CLI 参数 |
...
| `output_type` | `canonical_draft`、`messages_pack`、`provider_sidecar`、`repair_patch` | mode 与 gate |
...
| `polish_rewrite` | 目标章存在且用户显式要求重写/大修 | 现有 `第N章.md`、用户约束、正式写回时的 `--force` | `N5B-REPOLISH-PROMPT` | minimal repair stays source-anchored and aligns with planning |
| `local_repair` | 目标章存在且用户要求续写/补全 | 现有 `第N章.md`、续写边界或补充约束、正式写回时的 `--force` | `N5C-REPAIR-PROMPT` | continuity bridge and chapter exit hook |
| `local_repair` | 用户或 review 指出局部问题 | finding / issue / rework target / instruction、正式写回时的 `--force` | `N5D-REPAIR-PROMPT` | local fix preserves source distribution and does not rewrite upstream truth |
| `subagent_review_optimize` | 用户显式要求启用 subagents、按审计点分维度审查后直接优化 | 审计点、`story/review` registry、命中的 review 子技能 packets 或降级报告、正式写回时的 `--force` | `N3R-REVIEW-SUBAGENT-AUDIT` -> `N5D-REPAIR-PROMPT` | dimension findings have been injected into Doubao messages and directly optimized |
...
3. 若目标章存在且用户没有说明处理方式，正式写回必须阻断；仅 `--dry-run` 或 `--no-writeback` 可装配上下文包。
