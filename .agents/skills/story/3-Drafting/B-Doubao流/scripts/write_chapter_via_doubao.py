#!/usr/bin/env python3
"""
Bridge script for `story/3-Drafting/B-Doubao流`.
...
def _validate_generated_markdown(text: str, chapter_num: int, expected_previous_ref: str = "") -> None:
    ...
    if int(payload.get("chapter_num") or 0) != chapter_num:
        raise ValueError(f"豆包返回 chapter_num 与目标章不一致：{payload.get('chapter_num')}")
    if payload.get("写作模型") != EXPECTED_WRITING_MODEL:
        raise ValueError(f"豆包返回 frontmatter 字段 `写作模型` 必须是 `{EXPECTED_WRITING_MODEL}`。")

    if not isinstance(payload.get("global_card_refs"), list) or not payload["global_card_refs"]:
        raise ValueError("豆包返回 global_card_refs 必须是非空列表。")
    if not isinstance(payload.get("style_card_refs"), list) or not payload["style_card_refs"]:
        raise ValueError("豆包返回 style_card_refs 必须是非空列表。")
    if expected_previous_ref and payload.get("previous_chapter_ref") != expected_previous_ref:
        raise ValueError(
            "豆包返回 previous_chapter_ref 与已加载上一章不一致："
            f"{payload.get('previous_chapter_ref')!r} != {expected_previous_ref!r}"
        )
