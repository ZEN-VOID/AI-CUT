...
| `resume/` 与 `context-return` actualization 职责混淆 | stage contract | 在 `resume/SKILL.md` 明确其是 satellite recovery skill | 在 `context-return` 与 `resume` 同时固定边界 | 不再把恢复动作描述成 truth writeback |
...
| `workflow detect` 一旦看不到 `current_task` 就直接输出“无中断任务”，忽略 validation/review/context-return/写作日志等业务产物链 | no-interrupt artifact fallback contract | 为 `resume` 增加 artifact fallback：按 `上下文回流 -> validation -> review -> writing_log` 顺序继续判定唯一下一入口 | 在 `workflow_manager.py`、`resume/SKILL.md` 与 `workflow-resume.md` 同步写死 fallback 证据顺序与解释规则，并补回归测试 | 无 tracked 中断但有稳定业务证据时，`detect` 返回 `artifact_fallback` 而不是“无中断任务” |
...
5. 修完后必须验证：
   ...
   - 是否在 `上下文回流 / validation / review / writing_log` 已存在时仍错误返回“无中断任务”
