...
| `frontmatter` | YAML 头是否极简且只保留 `写作模型: Doubao`，未重复灌入上下文引用或摘要 |
...
finding:
  ...
  dimension: context_loading | source_alignment | supervision_packet | continuity | frontmatter | prose_quality | provider_evidence | path_contract | script_boundary | review_handoff
  symptom: ""
  direct_cause: ""
  source_contract: ""
  rework_target: ""
...
- 输出不是 canonical path。
- frontmatter 缺 `写作模型: Doubao`，重复写入 planning/cards/context 引用与 global/style/north-star 摘要，或 YAML 不可解析。
