...
`场景清单` 负责从 `6-分组` 逐集分镜组稿的组底 YAML 中抽取并归并场景主体，输出供后续场景设计阶段使用的 table 式 Markdown 清单。它只建立“哪些场景需要设计”的清单真源，不扩写场景设定、不生成视觉方案、不替代后续场景设计稿。
...
- 上游唯一准确信息来源为 `projects/aigc/<项目名>/6-分组/第N集.md` 每个分镜组底部 YAML 的 `场景` 字段；必要时只允许回查同一分镜组正文和场景标题作为证据。
...
- 用户要求“场景清单”“从分组稿提取场景”“生成 7-设计/场景/1-清单”等任务。
- 已完成或部分完成的 `6-分组` 逐集稿；默认按集读取 `第N集.md`。

Required input:

- 可定位、可读取的 `projects/aigc/<项目名>/6-分组/第N集.md`。
- 每个可消费分镜组底部存在 YAML 统计块，且包含 `场景` 字段。
- 至少一个目标集号，或允许默认处理 `6-分组/` 中全部 `第N集.md`。
...
- 上游 `6-分组/第N集.md` 不存在、不可读，且用户没有提供替代文件。
...
| `all_grouped_episodes` | 未指定集号但 `6-分组/` 下有 `第N集.md` | 全部可读集的场景清单 |
| `incremental_merge` | 既有 `场景清单.md` 存在，且 `6-分组` 新增/更新了部分 `第N集.md` | merge 更新清单、执行报告与可选 `design-manifest.yaml` |
...
2. 锁定输入集号与 `projects/aigc/<项目名>/6-分组/第N集.md` 文件；若既有 `场景清单.md` 或 `design-manifest.yaml` 存在，先读取并建立本轮 `reconcile_delta`。
...
8. 写入 `projects/aigc/<项目名>/7-设计/场景/1-清单/场景清单.md`，并按需写入 `projects/aigc/<项目名>/7-设计/场景/1-清单/执行报告.md`；可同步更新 `projects/aigc/<项目名>/7-设计/场景/design-manifest.yaml` 的 source/subject 映射。
...
| --- | --- | --- | --- |
| `FIELD-SCENE-LIST-01` | 输入取证 | 项目路径、目标集号、上游 `6-分组/第N集.md` 可回指 | `FAIL-SCENE-LIST-01` |
...
| --- | --- | --- | --- | --- |
| `PASS-SCENE-LIST-01` | 输入锁定 | 项目路径、目标集号、`6-分组/第N集.md` | 是否具备组底 YAML `场景` 字段和分镜组 ID | `input_manifest` |
...
| --- | --- | --- | --- |
| `PASS-SCENE-LIST-01` | 读取本技能与项目上下文，锁定 `6-分组` 输入 | input manifest | `references/source-and-merge-contract.md` |
...
`Symptom -> Direct Script/Prompt Overreach -> 场景清单 Section Owner -> 6-分组 YAML Contract -> AGENTS.md LLM-first / Skill 2.0 Rule`

## Output Contract

### Required output

1. 场景清单固定写入 `projects/aigc/<项目名>/7-设计/场景/1-清单/场景清单.md`。
...
7. 可选执行报告写入 `projects/aigc/<项目名>/7-设计/场景/1-清单/执行报告.md`，用于记录输入范围、归并风险、待核项和校验结果。
8. 可选增量状态索引写入 `projects/aigc/<项目名>/7-设计/场景/design-manifest.yaml`；它只是 sidecar，不替代 `场景清单.md`。
...
| --- | --- |
| `OUTPUT-SCENE-LIST` | `projects/aigc/<项目名>/7-设计/场景/1-清单/场景清单.md` |
| `OUTPUT-SCENE-LIST-REPORT` | `projects/aigc/<项目名>/7-设计/场景/1-清单/执行报告.md` |
| `OUTPUT-SCENE-MANIFEST` | `projects/aigc/<项目名>/7-设计/场景/design-manifest.yaml` |
...
- 分镜组 ID 沿用上游 `6-分组` 的 `x-y-z`：`集-场-组`，不改写为四段式分镜帧 ID。

### Completion gate

- 已读取本 `SKILL.md + CONTEXT.md`，并在项目任务中加载项目 `MEMORY.md` 与相关项目 `CONTEXT/`。
- 已读取目标 `6-分组/第N集.md`，并能回指每个主体来自组底 YAML `场景` 字段。
