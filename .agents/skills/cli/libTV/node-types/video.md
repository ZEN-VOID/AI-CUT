...
| ----------------------------------- | ------------------------------------------------------------------------------------------ |
| **`prompt`**                        | 提示词。                                                                                   |
| **`model`**                         | 视频模型 **`modelKey`**；未显式指定时默认使用 **`star-video2`**。`star-video2-fast` 只在用户/上游计划明确要求 fast / 极速 / 草稿预览时使用。 |
...
`star-video2` 是本 skill 对视频生成节点的默认模型 key。创建或更新 `video` 节点时，如果调用方没有指定 `model`，应显式补上 `-s model=star-video2`，避免继承画布旧节点、历史任务或示例中的其它模型。
...
| **`duration`**       | `settings`         | **`duration`**                                       | **4–15** 秒（默认 **`5`**）；schema `step=1` 是表单步长，CLI 可保存小数值（如 `12.5`） |
