...
本文件定义 `sword10` 的 subagent 派发、隔离、降级和主窗口边界。
...
| --- | --- | --- | --- | --- |
| 是否做到逐集阶段一集一个后台隔离 subagent，且主窗口只追踪状态？ | `GATE-SWORD10-02` | `FAIL-SWORD10-SUBAGENT` | `SKILL.md#Thinking-Action Node Map` | dispatch packet 清单、subagent status table |
| subagent runtime 不可用时是否停止或显式降级，而不是本地伪装执行？ | `GATE-SWORD10-02` | `FAIL-SWORD10-SUBAGENT` | 本文件 `Degradation Protocol`、`SKILL.md#Runtime Guardrails` | completion report 中的 runtime_mode 与 blocking_layer |
| 每个 subagent 是否只获得本集、本阶段和必要上下文？ | `GATE-SWORD10-03` | `FAIL-SWORD10-CONTEXT` | `templates/stage-dispatch-packet-template.md` | packet 中的 context_bundle 与 input/output path |
