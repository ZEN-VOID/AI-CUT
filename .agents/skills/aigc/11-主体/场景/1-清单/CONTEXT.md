...
| --- | --- | --- | --- | --- |
| 正文新增未在 YAML 出现的场景 | 来源真源层 | 删除该主体，除非回查到组底 YAML `场景` 字段 | 在取证表中固定 `yaml_scene_value` | 每个主体都有 YAML 来源 |
...
| 2.0 目录完整但缺可视拓扑 | Skill 2.0 表达层 | 在 `SKILL.md`、`steps/`、`types/`、`review/` 补 Mermaid 图与返工路由 | 后续新增或升级分区时同步检查关键拓扑图是否覆盖入口、执行、类型和验收 | `rg "mermaid" <skill-dir>` 能命中关键分区 |

## Repair Playbook

1. 先列出所有输入 `10-分组/第N集.md`，按集号与文件顺序建立候选扫描顺序。
