...
- 必须读取可用的 `3-美学` 产物，优先级为 `画面基调`、当前集 `角色风格`、当前集 `场景风格`。`画面基调` 始终读取全局 singleton；若能从 source 推断 `第N集`，角色/场景风格必须先读 `3-美学/第N集/<风格>/...`，缺失时回退项目级 `3-美学/<风格>/...`。若任一仍缺失，可使用用户提供的等价风格文本，但必须在报告记录降级来源和 N/A 原因。
