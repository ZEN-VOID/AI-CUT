...
- 正式写作调用必须读取 `_shared/supervised-drafting-review-loop-contract.md`，并把默认 subagents 监制、team 角色选择、卷级 `review + code-reviewer` 返工闭环传递给被选 lane。
...
| lane | 状态 | 触发信号 | 入口 |
| --- | --- | --- | --- |
| `C-Deepseek流` | default active | 用户只说 `$story-drafting`、章节正文、按章创作、从 planning 直接写小说正文，或没有点名其它 provider | `C-Deepseek流/SKILL.md` |
| `A-GPT原生` | active explicit | 用户明确要求 GPT 原生、本地会话直写、不要外部 provider，或需要先由当前 GPT/LLM 产出 canonical draft 再由脚本校验落盘 | `A-GPT原生/SKILL.md` |
| `B-Doubao流` | active explicit | 用户明确要求豆包、Doubao、doubao-seed-2.0-pro、`$story-drafting-doubao`，或要求本阶段内容创作部分调用 `.agents/skills/api/anyfast/llm/doubao-seed-2.0-pro` | `B-Doubao流/SKILL.md` |
...
flowchart TD
    ...
    Kind -->|"润色 / 二次改写 / 中文表达强化 / 去 AI 味 / 题材质感校准"| Polishing["移交 4-润色"]

    Drafting --> DIntent{"3-初稿是否显式指定 provider / lane?"}
    DIntent -->|"GPT 原生 / 本地会话直写"| A["3-初稿/A-GPT原生"]
    DIntent -->|"豆包 / Doubao / story-drafting-doubao"| B["3-初稿/B-Doubao流"]
    ...
    PIntent -->|"DeepSeek / story-polishing-deepseek"| PC["4-润色/C-Deepseek流"]
...
- 未明确 provider / lane 时，默认选择 `C-Deepseek流`。
...
- 若用户诉求是“润色 / 二次改写 / 中文表达强化 / 去 AI 味 / 题材质感校准”，则不在 `3-初稿` 内部执行正文生成，而是移交 `4-润色`。
...
| 场景 | 读取文件 |
| --- | --- |
| 需要默认 provider 执行 | `C-Deepseek流/SKILL.md` + `C-Deepseek流/CONTEXT.md` |
...
| 需要润色且未显式指定 provider | `../4-润色/SKILL.md` + `../4-润色/CONTEXT.md`，再加载 `../4-润色/B-Doubao流/SKILL.md` + `../4-润色/B-Doubao流/CONTEXT.md` |
| 需要润色且显式指定 GPT 原生 / Doubao / DeepSeek | `../4-润色/SKILL.md` + `../4-润色/CONTEXT.md`，再加载对应 `../4-润色/<lane>/SKILL.md` + `CONTEXT.md` |
| 需要默认 subagents 监制、team 视角与卷级审计返工闭环 | `_shared/supervised-drafting-review-loop-contract.md` |
