...
混合拓扑：项目上下文串行锁定，角色主体可并行分发给 顾问与复核流程，最后统一汇流和 review。
...
flowchart TD
    ...
    R --> E{"N6-ADVISOR-REVIEW<br/>advisor + worker dispatch"}
...
| `N5-RESEARCH-PROFILE` | 把研究转化为设计证据链 | `character_intake_table`、`project_design_context`、`type_profile`、必要外部来源 | LLM 生成身份、职业、阶层、地域年代、服饰工艺、身体姿态、禁区、不确定性和 prompt evidence chain；搜索只作辅助证据 | `research_profile` | `N6-ADVISOR-REVIEW` | 每个研究镜头都有设计转化 |
| `N6-ADVISOR-REVIEW` | 启动默认 顾问与复核流程 或使用本地流程 | `type_profile`、`research_profile`、`references/workflow-supervision-contract.md`、runtime 能力 | 先按当前 `node_id / pass_id / gate_id` 绑定项目监制参谋，再分发研究证据、物语、视觉服装、摄影 patch；不可用时本地顺序执行并按监督合同记录 | `workflow_supervision_record`、`advisor_node_coverage` | `N7-MERGE-DRAFT` | 不静默跳过 顾问与复核流程，supervision 记录非空，顾问问题不退化为固定字段清单 |
...
| `N8-REVIEW-GATE` | 审查字段、风格、研究证据链、prompt 和 LLM-first | draft、review 合同、`references/design-slot-review-contract.md`、`references/workflow-supervision-contract.md` | 检查清单锚点、项目风格、研究镜头、解构主体 ID、解构字段、prompt 长度、脚本边界；解析 `ROLE-BUNDLE-01` 并记录缺槽或通过结论 | `review_result`、`slot_bundle_review` | `N9-WRITE-OUTPUT` 或 `N7-MERGE-DRAFT` | 无阻断 finding，slot bundle 无缺槽 |
...
| `workflow_supervision_record` | 执行模式、阻断层级、顾问/worker/reviewer 路径、本地流程、`advisor_node_coverage` | `N8-REVIEW-GATE` 的 reviewer 汇流和最终报告 |
...
| `FAIL-ADVISOR-REVIEW-SKIPPED` | 默认 顾问与复核流程 路径被静默跳过 | 回到 `N6-ADVISOR-REVIEW` 并补本地 checklist 结果 |
