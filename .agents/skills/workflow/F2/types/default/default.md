...
| --- | --- | --- |
| 命中 F2 且有内容、主时钟和素材池，用户未明确禁止渲染 | `full_hyperframes_edit` | F2 普通完成态是最终成片 |
| 命中 F2 且用户未指定比例 | `full_hyperframes_edit` + `aspect_ratio=16:9` | F2 普通视频默认 16:9、1920x1080 |
| 命中 F2 且包含台词字幕 | `full_hyperframes_edit` + `dialogue_sync_validator=required` | 台词字幕 final 前必须通过机械同步校验 |
...
- 缺音频主时钟且无 TTS 授权：默认请求补充音频或 TTS 授权；不得无声降级后宣称 F2 完成。
