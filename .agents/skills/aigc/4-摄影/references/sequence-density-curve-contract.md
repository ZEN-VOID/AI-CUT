...
本文件定义 `4-摄影` 的段落级时间段密度曲线机制。它不替代 `visual-sequence-alignment-contract.md` 的逐画面点归属，也不替代 `beat-analysis-contract.md` 对单个 `visual_unit` 的节拍判断；它负责在相邻画面单位进入逐句分镜前，先判断整段观看节奏应该如何变速：哪里省镜头，哪里加密，哪里停顿，哪里硬切，哪里交出给下游。
...
每当相邻 `visual_unit` 构成一个连续观看段落，必须先形成内部 `sequence_density_curve`。该曲线只约束时间段密度、时值倾向、峰值槽位、恢复槽位和交出锚点；最终落盘仍按正上方原画面性字段标题逐点写连续时间段，不得把多个画面句子合并成一条失主镜头。
...
- 同一场中 `shot_count_distribution` 呈现固定 2 段、平均铺满、连续高密度或连续低密度风险；机械校验提示 2 段集中时，应建立或复核 `sequence_density_curve`，但 2 段集中本身只作为节奏复核信号，不自动视为失败。
...
| `density_ramp` | 每个阶段的时间段密度倾向：`conserve / measured / build / burst / hold / release` |
| `peak_slots` | 可加密的峰值槽位；必须说明峰值证据和允许的最高时间段数 |
| `recovery_slots` | 爆点后需要停住、反压、读秒或普通人反应的槽位 |
| `set_piece_chain_slots` | 允许 5-6 段的连续动作/声画打点槽位；必须逐段有独立起点、撞点、结果和节奏声 |
...
| `set-piece-chain` | 连续命中、连续反弹、连续物件结果或声画五拍 | 允许 5-6 个短镜，但每段必须一击一结果，不许随机堆切 |
...
通常关键块扩展到 3-4 段已经足够；但当当前画面句子明确包含连续动作结果、连续命中、连续反弹、连续物件变化或连续声画打点时，可启动 `set_piece_chain_slots`，允许单个 `visual_unit` 写到 5-6 段。

启动条件：

- 每一段都有新的动作相位、撞击点、物件状态、身体结果、声音打点或反应落点。
- 删掉任一段都会少一个必要结果或少一个清楚节奏拍。
...
5. 若存在连续动作/声画打点，判断是否进入 `set_piece_chain_slots`；若进入，允许 5-6 段，但必须逐段可删减测试。
6. 形成 `density_budget`：检查全段是否过于平均、全满、全空或固定两段；若固定两段风险被机械校验提示，必须抽样低信息、关键揭示、群像、高点和边界交出块，证明该收敛的已收敛、该加密的已加密。
7. 形成 `handoff_anchors` 与 `ownership_guard`：曲线只给当前阶段可见锚点，不写 `5-分组` 的创意连接件。
8. 将 `sequence_density_curve` 交给 `N4-BEAT`、`N5-RHYTHM`、`N5.2-DURATION`、`N5.5-PEAK-SHOT` 和 `N6.5-SHOT-PLAN` 消费；单个 `visual_unit` 的最终时间段数仍必须由本句有效触发点、观看结果或叙事节奏价值证明。

## Review Questions

1. 整段是否有清楚的速度变化，而不是每句同样密度？
2. 低信息处是否被省掉了不必要的镜头？
3. 关键高点是否获得足够时间段密度、时值和结果钉镜？
4. 高密度之后是否有恢复、反压、普通人反应或交出锚点？
5. 5-6 段链条是否每段都有独立动作结果或声音打点？
...
| --- | --- | --- | --- | --- |
| 命中连续观看段落、固定两段风险、set-piece 链条或初始化综合变速建议时，是否建立 `sequence_density_curve`？ | `GATE-CINE-04A2` | `FAIL-CINE-03D` | `N3.6-DENSITY-CURVE` | `density_curve_summary`、触发信号、未建立曲线的返工记录 |
| 曲线是否包含 `tempo_beats / density_ramp / peak_slots / recovery_slots / density_budget / handoff_anchors`，而不是只有时间段数量统计？ | `GATE-CINE-04A2` | `FAIL-CINE-03D` | `N3.6-DENSITY-CURVE` | 段落密度曲线结果、峰值/恢复槽位和整体预算 |
| 低信息处是否收敛，关键处是否加密，高密度后是否有恢复、反压、普通人反应或交出锚点？ | `GATE-CINE-05` | `FAIL-CINE-05D` | `N5-RHYTHM` + `N3.6-DENSITY-CURVE` | 张弛抽样、低信息收敛与关键信息发散证据 |
| 机械校验提示 2 段集中时，是否抽样证明第二段具有有效触发点、观看结果或叙事节奏价值？ | `GATE-CINE-04A` | `FAIL-CINE-03A` | `N3.6-DENSITY-CURVE` + `N4-BEAT` + `N6.5-SHOT-PLAN` | 时间段数量分布与 2 段集中抽样复核结果 |
| 5-6 段 `set_piece_chain_slots` 是否每段都有独立动作相位、撞点、声音打点、结果或反应，删掉任一段都会损失节奏拍？ | `GATE-CINE-04A3` | `FAIL-CINE-03E` | `N3.6-DENSITY-CURVE` + `N6.5-SHOT-PLAN` | set-piece 不可删证明、每段起点/撞点/结果抽样 |
