...
| `0-初始化` | 项目骨架、`MEMORY.md` 初始化写回、项目 `CONTEXT/` 入口 | 初始化目录、项目记忆或上下文入口错误先修此层 |
| `1-分集` | 原文分集、集边界、零改写源文本 | 漏段、错集、原文边界问题先修此层 |
| `2-美学` | 类型风格、画面基调、角色/场景/道具/分镜/摄影风格协议 | 题材类型、视觉纪律或风格继承错误先修此层 |
| `3-主体` | 主体注册表、场景/角色/道具清单、设计、生成请求 | 主体命名、资产身份、造型、物件逻辑和引用错误先修此层 |
| `4-编剧` | 剧情事实字段化、对白保真、slugline、声画配对 | 可拍事实和对白错误先修此层 |
| `5-导演` | 戏剧问题、视觉主轴、导演意图、高潮与终结画面 | 导演判断、场面调度和观看焦点错误先修此层 |
| `6-分镜` | 动作路径、心理反应可见化、分镜明细和前后状态承接 | 演员可执行性、动作连续性和分镜拆分错误先修此层 |
| `7-摄影` | 景别、运镜、焦点、光影、时长、连续性锚点 | 镜头和 AI 视频可执行性错误先修此层 |
| `8-分组` | 组边界、组间桥接、入场/出场、统计、组级 prompt 基础 | 分镜组消费单位错误先修此层 |
| `9-图像` | 帧图、故事板、图像 prompt、reference manifest、生成报告 | 图像任务与成图证据由对应 leaf 持有 |
| `10-画布` | 视频任务、reference 绑定、生成结果、MP4 证据 | 视频生成证据由对应 leaf/provider 持有 |
...
2. 最早 canonical source owner：`0-初始化` 到 `8-分组` 中对应最早层。
3. 同层相邻或投影：同集前后场、同分镜组、同一资产 alias 或同一阶段报告。
4. 下游文本产物：后续阶段文本、prompt、handoff、manifest。
5. 生成资产状态：保留、失效、重建任务或重跑 provider。
6. review aggregate、审片 finding、`STATE.json`、`governance-state.yaml`。
...
| --- | --- | --- | --- | --- |
| 是否按 Truth Owners 表定位项目记忆、项目上下文、`0-初始化` 到 `10-画布`、review/state 的 canonical owner，而不是让 repair 技能直接夺取业务真源？ | `PASS-REPAIR-03` | `FAIL-AIGC-REPAIR-OWNER` | `N4-OWNER-ROUTE` | `canonical_owner`、owner decision、引用的 source rule |
...
| 执行型修复是否完整记录 `source_rules_reviewed` 的 skill/context/references/types/review 与 owner_decision？ | `PASS-REPAIR-02` | `FAIL-AIGC-REPAIR-SOURCE-RULE` | `N2-SOURCE-RULE-REVIEW` | `source_rules_reviewed` 结构化记录、loaded partition 列表、rule summary |
