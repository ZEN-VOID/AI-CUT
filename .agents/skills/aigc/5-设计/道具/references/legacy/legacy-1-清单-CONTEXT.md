...
- 本文件是 `5-设计/道具/1-清单` 的经验层知识库，不是过程日志。
...
| 道具链 runner 在相对路径输入下把输出误落到 `3-Detail/5-设计-道具-1-清单/` | 输出路径解析层 | 改为按 `Path.parts` 直接定位 `projects/aigc/<项目名>` 项目根，而不是依赖 `\"/projects/\"` 字符串命中 | 默认输出根统一从项目根推断到 `5-设计/道具/1-清单/第N集/`，并保留误落盘清理步骤 | 相对路径与绝对路径输入都落到同一 canonical 输出目录 |
...
6. 最后检查输出目录是否稳定落到 `5-设计/道具/1-清单/第N集/`。
...
- 对 `5-设计` 来说，研究不是终点；研究必须在同一轮里折叠成 `design_context`，否则下游仍然会退回手工理解。
