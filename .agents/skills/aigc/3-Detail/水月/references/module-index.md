...
本文件是 `水月` 内部四条并发树形维度链的总索引。

它回答三个问题：

1. 四条主链各自负责什么
2. 12 个叶子子模块各自补什么，不该补什么
3. 四条主链如何汇流成 `group_design_patch + beat_patches[]`
...
5. 最后只写：
   - `group_design_patch.出场角色及穿搭`（若证据充分）
   - `beat_patches[].角色背景面`
   - `beat_patches[].角色站位走位`
   - `beat_patches[].道具及状态`
   - `beat_patches[].镜头消费提示`
...
- `水月` 的标准输出不是 prose，而是 factual evidence sidecar 中的局部字段。
- 四条主链都必须服务同一件事：让父层能稳定回填 `组间设计.出场角色及穿搭 + 分镜明细[]` 的 factual 槽位。
- `镜头消费提示` 只负责提供 pre-shot 抓手；最终 shot-level `分镜表现` 由父层投影生成。
- 未命中的字段不得补空洞默认值。
