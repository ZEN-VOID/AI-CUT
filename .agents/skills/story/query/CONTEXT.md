...
| --- | --- | --- | --- | --- |
| 查询到章节规划、任务、线索、伏笔时仍只读 `state.json` 或旧 `大纲/` | skill contract / data-flow doc | 把规划类问题默认入口切到 `Planning/8-全息地图.json` | 在 `SKILL.md` 与 `system-data-flow.md` 固定 holomap-first | 规划类查询说明明确写出 holomap 为优先数据源 |
...
| 任务进度、恢复点、最近 run 查询仍只读 `workflow_state.json` 或完全无入口 | execution truth contract | 把执行态问题切到 `execution_state.json + task_log.jsonl` | 在共享 data-flow 文档明确 execution truth 三件套分工 | 能回答“当前跑到哪了 / 最近哪个 stage 卡住了 / 最新恢复点在哪” |
...
- `workflow_state.json` 是当前 run 兼容断点；`execution_state.json` 是全阶段执行真源；`task_log.jsonl` 是事件证据链。
