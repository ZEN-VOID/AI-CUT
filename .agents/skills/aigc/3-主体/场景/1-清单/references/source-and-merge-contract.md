...
- 唯一准确信息来源：`projects/aigc/<项目名>/3-主体/subject-registry.yaml` 的 `subjects.scenes` 条目。
- 证据回查来源：registry `source_anchors` 指向的 `1-分集` 故事源、`2-美学/场景风格` 协议、项目已确认命名约束。
- 已有 `8-分组` 稿只作为后置命名对齐证据，不能新增场景主体。
- 禁止来源：未被 registry 支持的剧情推断、后续设计想象、脚本自动补全。

## Source Trust Map

```mermaid
flowchart TD
    A["subject-registry subjects.scenes"] -->|"唯一主体来源"| B["候选场景"]
    C["source_anchors / first_appearance"] -->|"首次登场证据"| B
    D["1-分集故事源"] -.->|"命名消歧"| E["证据关键词"]
    F["2-美学/场景风格"] -.->|"风格证据"| E
...
| field | meaning |
| --- | --- |
| `registry_id` | registry 中的场景 ID |
| `source_anchor` | 来源集号、段落或文件锚点 |
| `canonical_name` | registry 的 `canonical_name` |
| `evidence_keywords` | source anchor、故事源或风格协议中的消歧关键词 |
...
5. 跨场景、移动路线、组合地点应拆成可制作空间，除非 registry 明确只表达一个泛称空间。
...
| `原文描述（关键词式）` | registry 原值、别名证据、source anchor 消歧关键词；不扩写设计 |

## Review Gate Mapping

| Review Question | Review Gate | Fail Code | Rework Target | Report Evidence |
| --- | --- | --- | --- | --- |
| 每个最终场景主体是否都能回指 `subject-registry.yaml` 的 `subjects.scenes` 条目，而不是剧情推断、后续设计想象或脚本补全？ | `GATE-SCENE-LIST-01` | `FAIL-SCENE-SOURCE` | `N2-REGISTRY-SCAN` | `candidate_records` 列出 `registry_id`、`canonical_name`、`source_anchor`；报告删除或修正的 registry 外主体。 |
| source anchor、故事源、项目 `MEMORY.md` / `CONTEXT/` 是否只用于命名消歧和证据关键词，没有变成新增主体来源？ | `GATE-SCENE-LIST-02` | `FAIL-SCENE-EVIDENCE` | `N3-EVIDENCE` | `evidence_keywords` 标明故事源/项目约束来源，并注明其只服务消歧。 |
| 每个候选记录是否至少保留 `registry_id`、`canonical_name`、`source_anchor` 与必要 `evidence_keywords`，足以让 LLM 复核归并？ | `GATE-SCENE-LIST-03` | `FAIL-SCENE-EVIDENCE` | `N2-REGISTRY-SCAN` / `N3-EVIDENCE` | 候选记录抽样或完整表，缺字段项与补证动作写入执行报告。 |
...
| 跨场景、移动路线、组合地点是否拆成可制作空间；若保留泛称空间，是否有 registry 证据支持？ | `GATE-SCENE-LIST-04` | `FAIL-SCENE-MERGE` | `N3-EVIDENCE` / `N5-MERGE` | `merge_decision` 或待核项说明路线拆分、泛称保留依据和涉及 source anchor。 |
