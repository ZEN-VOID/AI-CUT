...
`8-视频` 是 AIGC 项目的视频阶段父级入口。它只负责判定视频生成路线、加载目标叶子技能、约束项目 runtime 与上游真源边界；不直接主创视频 prompt，不直接调用 LibTV，不直接改写 `5-分组`、`6-设计` 或 `7-图像` 产物。
...
- LibTV 画布复用是 `8-视频` 父级共享治理合同：绑定 `projects/aigc/<项目名>/` 后，所有 A/B/C/D 叶子调用 `$libTV` 前必须先读取或重建 `projects/aigc/<项目名>/8-视频/libtv-canvas-registry.json`；同一项目默认复用 registry 中的 `canonical_sessionId` / `projectUuid` / `projectUrl`，不得为同一项目无故新建平行画布。
- LibTV 远端调用锁是 A/B/C/D 共同门禁：所有叶子生成的 `*-libtv-submission.txt` 必须以 `【LibTV 调用锁定】` 开头；父级只检查首行存在和路线归属，具体 `modeType` 与参照字段由对应叶子合同锁定。
- 视频文件命名必须服务下游 `9-审片`：下载或整理后的 canonical 视频命名为 `<分镜组ID>.mp4`；同一分镜组多变体命名为 `<分镜组ID>-a.mp4`、`<分镜组ID>-b.mp4`。sessionId、provider task id、路线名写入 queue / results / report，不写入 canonical 视频文件名。
- `5-分组` 中的 `## x-y-z~x-y-z` 组间连接件在 `8-视频` A/B/C/D 路线中默认全部忽略：不进入视频 prompt、参照 manifest、LibTV job 或 canonical 视频命名。未来连接件视频由单独手动视频连接 skill 处理，父级不得默认调度。
...
- 连续调度不得绕过本技能的阻断门：缺少必需输入、视频参照路线无法唯一判断、叶子技能缺失或路线歧义会造成错误 canonical 写回时，必须先停下并给出最小澄清或不可用说明。
...
- 用户命中 `8-视频`、视频阶段、生视频、LibTV、分镜参照、故事板参照、主体参照、主板混合参照或批量视频生成。
- 来自 `projects/aigc/<项目名>/5-分组/` 的分镜组稿，需要转为组级视频任务。
- 来自 `projects/aigc/<项目名>/7-图像/A-分镜画面/` 的镜级图像参照，或 `7-图像/B-分镜故事板/` 的组级故事板参照。
- 来自 `projects/aigc/<项目名>/6-设计/*/3-生成/` 的角色、场景、道具主体资产参照。
- 已有 `projects/aigc/<项目名>/8-视频/*/` 的 prompt、manifest、LibTV batch、queue ledger 或生成结果需要 query / download / repair / review / rerun。
...
| mode | trigger | route |
| --- | --- | --- |
| `frame_visual_reference` | 单镜图、多张分镜画面图、四段式 `分镜ID`、`7-图像/A-分镜画面`、镜级图参照出视频 | `A-分镜画面参照/SKILL.md` |
| `storyboard_reference` | 分镜故事板、组级 storyboard 图、`7-图像/B-分镜故事板`、用整张故事板图参照出视频 | `B-分镜故事板参照/SKILL.md` |
| `subject_reference` | 角色/场景/道具主体参照、组底 YAML、`6-设计/*/3-生成`、按主体图片出视频 | `C-主体参照/SKILL.md` |
...
| LibTV 项目级画布复用 | 先读取 `projects/aigc/<项目名>/8-视频/libtv-canvas-registry.json`；缺失时从 A/B/C/D 既有 queue / results / report 反建；仍缺失时由首个成功提交写入 |
...
```mermaid
flowchart TD
    A["8-视频 request"] --> B["Load SKILL.md + CONTEXT.md"]
    ...
    E --> I["projects/aigc/<项目名>/8-视频/A-分镜画面参照"]
    F --> J["projects/aigc/<项目名>/8-视频/B-分镜故事板参照"]
    G --> K["projects/aigc/<项目名>/8-视频/C-主体参照"]
    L --> M["projects/aigc/<项目名>/8-视频/D-主板混合参照"]
    H --> E
    H --> F
    H --> G
    H --> L
...
2. 读取项目级 LibTV 画布 registry：`projects/aigc/<项目名>/8-视频/libtv-canvas-registry.json`。若缺失，先扫描 A/B/C/D 既有 `第N集-libtv-queue.md`、`第N集-libtv-results.json`、`执行报告.md` 中的 `sessionId`、`projectUuid`、`projectUrl` 并反建 registry；仍缺失时标记为 `needs_initial_session`，由第一个成功的叶子提交写入。
...
```text
projects/aigc/<项目名>/8-视频/libtv-canvas-registry.json
```
...
4. 既有产物无法回指：按 `projects/aigc/<项目名>/8-视频/<叶子名>/` 目录、文件名、ledger 和 report 重建所属关系。
...
- Output path: 父级不直接落业务产物；A/B/C/D 分别写入 `projects/aigc/<项目名>/8-视频/A-分镜画面参照/`、`B-分镜故事板参照/`、`C-主体参照/`、`D-主板混合参照/`。
