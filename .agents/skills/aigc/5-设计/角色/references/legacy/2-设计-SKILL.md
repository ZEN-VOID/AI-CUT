...
description: Use when `5-设计/2-设计/角色` needs to consume `角色清单.json`, `role_design_bridge.json`, `角色研究.json`, plus `0-Init` and `2-Global` truths, then emit machine-first role design truth and per-role Markdown under `projects/aigc/<项目名>/4-设计/角色/2-设计/`.
...
`5-设计/2-设计/角色` 是当前 `aigc` 技能树里角色主体设计的统一入口。

它承接：

- `5-设计/角色/1-清单/第N集/*.json`
- `0-Init/*.yaml`
- `2-Global/*.md`
...
2. `[角色名].md`
3. `[角色名].<ext>`（built-in imagegen 单主体图片）
4. `_manifest.json`
...
- `角色设计` 属于内容创作型任务；`character_design.json`、`[角色名].md`、`prompt整合` 与其他创作性角色设计正文，必须由 LLM 直接完成。
- `run_character_design_pipeline.py / build_character_design_packets.py` 不得再被视为默认主创入口；它们只允许用于受控兼容迁移、既有 LLM 真源的模板投影、批量落盘或结构校验前的机械辅助。
...
- 当前 skill 是 `5-设计/2-设计` 下的角色 leaf。
- 上游对象识别真源属于 `1-清单/角色`。
...
- `.agents/skills/aigc/5-设计/2-设计/_shared/design-input-contract.md`
...
- `.agents/skills/aigc/5-设计/1-清单/角色/SKILL.md`
...
3. `.agents/skills/aigc/5-设计/SKILL.md + CONTEXT.md`
4. `.agents/skills/aigc/5-设计/2-设计/SKILL.md + CONTEXT.md`
5. `.agents/skills/aigc/5-设计/2-设计/_shared/design-input-contract.md`
6. `.agents/skills/aigc/5-设计/2-设计/_shared/design-output-contract.md`
7. `.agents/skills/aigc/5-设计/2-设计/_shared/design-slot-review-contract.md`
8. `.agents/skills/aigc/_shared/council-runtime/module-spec.md`
9. `.agents/skills/aigc/5-设计/2-设计/_shared/subagent-supervision-contract.md`
10. 本 `SKILL.md + CONTEXT.md`
11. `references/character-design-assembly.md`
12. `templates/character_masterprompt.structured.v2.md`
13. `projects/aigc/<项目名>/4-设计/角色/1-清单/第N集/role_design_bridge.json`（若存在）
14. `projects/aigc/<项目名>/4-设计/角色/1-清单/第N集/角色研究.json`（若存在）
15. `projects/aigc/<项目名>/4-设计/角色/1-清单/第N集/角色清单.json`
...
```bash
python3 .agents/skills/aigc/5-设计/2-设计/角色/scripts/validate_character_design_projection.py \
  --output-dir "projects/aigc/<项目名>/4-设计/角色/2-设计/第N集"
```

legacy 兼容投影入口：

```bash
python3 .agents/skills/aigc/5-设计/2-设计/角色/scripts/run_character_design_pipeline.py \
  --catalog "projects/aigc/<项目名>/4-设计/角色/1-清单/第N集/角色清单.json" \
  --allow-legacy-script-authorship
```

脚本硬规则：

1. `run_character_design_pipeline.py` 与 `build_character_design_packets.py` 仅可在受控兼容模式下执行，不得再作为默认主创入口。
2. legacy builder 若被调用，仍必须读取 `templates/character_masterprompt.structured.v2.md` 并只做投影/落盘。
...
| `business_object` | `projects/aigc/<项目名>/4-设计/角色/1-清单/第N集/*.json`、`projects/aigc/<项目名>/0-Init/*.yaml`、`projects/aigc/<项目名>/2-Global/*.md`、`projects/aigc/<项目名>/4-设计/角色/2-设计/第N集/character_design.json`。 |
...
- `projects/aigc/<项目名>/4-设计/角色/2-设计/第N集/`
...
- `projects/aigc/<项目名>/4-设计/角色/2-设计/thinking/第N集/[角色名].md`
...
5. `full_generation_prompt` 必须等于统一 `global_style_prefix + prompt_integration`，并作为 built-in imagegen 的唯一 prompt 入参。
...
flowchart TD
    ...
    H --> I["Call built-in imagegen same-dir same-stem"]
```
...
| `FIELD-CHAR-11` | `[角色名].<ext> / _manifest.json.auto_image` | 自动生图使用含全局风格前缀的完整 prompt，默认内置 imagegen 请求准备，图片与设计文件同目录同 stem | `design-output-contract.md`、`image-generation-execution-contract.md`、`全局风格.md`、`[角色名].md` | `S10` | auto image completeness | `FAIL-CHAR-AUTO-IMAGE` |
...
| `FIELD-CHAR-13` | current-round outputs / post-write audit boundary | 输出后必须读取项目根 `team.yaml`，明确 `roles.supervision` 不再承担当前轮 closeout；若需记录后置问题，只写角色输出的 audit note | `team.yaml`、`council-runtime`、`subagent-supervision-contract.md`、`design-slot-review-contract.md` | `S11` | audit boundary | `FAIL-CHAR-SUPERVISION-REVIEW` |
...
| `S10` | 单主体自动生图 | 读取 `[角色名].md` 与全局风格前缀；自动生图前复验纯色背景锚句，再默认内置 imagegen 请求准备 built-in imagegen，同目录同名图片由后续验收复核 | `auto_image_packet + reference_cleanliness_note` | `S11` | `S7-S10` |
| `S11` | 输出后审计边界说明 | 读取 `team.yaml` 与共享占位合同，明确当前轮角色输出不再进入 `监制` closeout；若需记录问题，只把 `ROLE-BUNDLE-*` 写入 audit note 与 handoff | `post_write_audit_note` | `done` | `S7-S11` |
...
| `N10-AUTO-IMAGE` | 生成单角色概念图 | `[角色名].md`、`全局风格.md`、`prompt_integration` | 生成 `full_generation_prompt`；复验纯色背景锚句与背景污染禁令后默认内置 imagegen 请求准备 built-in imagegen，目标输出 `[角色名].<ext>` | `auto_image_packet + reference_cleanliness_note` | `N11` | 不得只传局部 prompt；内置 imagegen request_ready不得伪装为已产图；图片必须同目录同名；纯色背景门禁失败不得生图 |
| `N11-POST-WRITE-AUDIT-NOTE` | 输出后审计占位 | 当前轮 `character_design.json + [角色名].md + _manifest.json`、`team.yaml`、`subagent-supervision-contract.md`、`design-slot-review-contract.md` | 按共享占位合同确认当前轮 closeout 不再由 `监制` 执行；如需后置问题记录，只把当前轮角色输出解析成 `ROLE-BUNDLE-01~04` 并写入 audit note / handoff | `post_write_audit_note` | `done` | 不得把 `source_skill_refs` 误当 runtime 授权或 post-write reviewer |
...
| `FIELD-CHAR-11` | 默认提交态含 `request_ready/request_batch_path/default_model`；最终验收时 `[角色名].<ext>` 已由 `full_generation_prompt` 生成，且 stem 与 `[角色名].md` 一致 | `FAIL-CHAR-AUTO-IMAGE` | `S10` |
...
| `FIELD-CHAR-13` | `team.yaml` 已读取，且已明确当前轮角色输出的 closeout 不再由 `监制` 执行；如需后置问题，仅写 audit note | `FAIL-CHAR-SUPERVISION-REVIEW` | `S11` |
...
当前轮若需记录后置问题，只围绕以上文件及其对应 `ROLE-BUNDLE-*` 写 audit note，不再以 `监制强化` 名义做 patch。
...
8. `.agents/skills/aigc/5-设计/2-设计/_shared/subagent-supervision-contract.md`
...
1. `S10/N10` 完成后仍进入 `S11/N11`，但其职责已改为写审计边界说明，不再执行 `监制强化`。
2. `_shared/subagent-supervision-contract.md` 当前只作为停用占位真源；本 leaf 不再声明角色型 closeout 补选。
3. 若需后置问题记录，只能写当前轮 `character_design.json / [角色名].md / _manifest.json` 的 audit note，并按需补阶段 `validation-report.md`。
...
6. 已使用含统一全局风格前缀的 `full_generation_prompt` 默认内置 imagegen 请求准备 `[角色名].<ext>` 请求；最终验收时图片同 stem 存在。
7. `validate_character_design_projection.py` 已通过，且 `_manifest.json.template_validation.status` 非 `failed`。
8. 已按 `team.yaml + _shared/subagent-supervision-contract.md` 明确当前轮角色输出的 closeout 不再由 `监制` 执行。
