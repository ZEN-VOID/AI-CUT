...
| mode | trigger | action |
| --- | --- | --- |
| `global_style_review` | 每次阶段 PASS 前 | 检查每个分镜组是否在场景标题行下方立即输出 `全局风格：`；字段内是否有三行内容；第 1 行是否以固定前置词开头并接 300 字以内当前组全局风格整理句；第 2、3 行是否来自 `3-美学/画面基调` 的 `Visual Slogan / Design Principle` 与 `Visual Gene Profile / Negative Traits`；是否没有完整照抄全局母稿或新增无证据风格 |
| `visual_style_review` | 每次阶段 PASS 前 | 检查每组是否在 `全局风格：` 三行内容之后、第一个时间码分镜行之前输出 `画面风格：`；该语句是否从组内镜头设计中提炼，并覆盖构图布局、构图方式、光源、色彩、关键摄影参数和观看位置 |
...
| `mechanical_check` | 每次落盘前或修复后 | 运行 `scripts/validate_storyboard_groups.py`，检查分镜组标题、场景标题行、`全局风格：`、`画面风格：`、禁用 `分镜画面：` / `增补首帧：` / `回龙帧：` 与其他旧字段、字数统计、分镜总时长、组内时间码连续累加、YAML、禁用连接件块；warning 进入语义复核，不直接等于 PASS |
| `boundary_review` | 每次阶段 PASS 前 | 检查每组是否先按上游每个分镜行最后时间段结束秒累计裁决边界，落盘后是否改写为当前分镜组基准下连续递增的 `[N-N秒]`，且后一个时间段起点等于前一个终点；组内分镜总时长是否接近 15 秒、通常是否落在约 12-18 秒、且必须不超过 18 秒；atomic unit 与对白/画面承托必须完整，但不能作为超 18 秒放行理由 |
...
| `GATE-GROUP-02` | `全局风格` 整理 | 每组在场景标题行下方立即输出 `全局风格：`；字段内三行纯内容；第 1 行以固定前置词开头，后接当前组全局风格整理句；整理句依据 `画面基调.Global Style Prompt` 和当前证据抽取匹配部分，保持自然单段且 300 字以内；第 2、3 行分别来自 `Visual Slogan / Design Principle` 与 `Visual Gene Profile / Negative Traits` |
| `GATE-GROUP-05` | ID 真实 | 每组 ID 为 `x-y-z`，匹配真实集、场、场内序号 |
| `GATE-GROUP-06` | 组内时长 | 每组以分镜行总时长为主，裁决时按上游 `分镜N（起始秒-结束秒）：` 或兼容 `[起始秒-结束秒]` 行累计，落盘后改写为当前分镜组基准下连续递增的 `分镜N（N-N秒）：` 或 `[N-N秒]`；优先接近约 15 秒，通常约 12-18 秒可接受；低于约 10 秒必须有回填复核或短场景例外理由；超过 18 秒为硬失败 |
...
| `GATE-GROUP-12` | `画面风格` | 每组在 `全局风格：` 三行内容之后、第一个时间码分镜行之前有 `画面风格：` 自然语句；从组内镜头设计中提炼，覆盖构图布局、构图方式、光源、色彩、关键摄影技术参数和必要的观看位置 |
...
| --- | --- | --- |
| `FAIL-GROUP-02` | `全局风格：` 缺失、位置错误、三行内容缺失、固定前置词缺失、整理句超过 300 字、完整照抄全局母稿、未匹配当前组证据或画面基调必需字段缺失 | `references/north-star-projection-contract.md` |
...
| `FAIL-GROUP-05` | 组内分镜总时长明显偏离 15 秒且未复核、低于约 10 秒未回填、超过 18 秒、单个 atomic unit 超 18 秒但未回退 `9-光影` 修复，或字数/对白风险失控 | `references/group-boundary-contract.md` |
| `FAIL-GROUP-DIRECT-SCREENPLAY` | 直接接手剧本时仍要求用户补 `9-光影`、未自动按约 15 秒/组拆组、未声明 direct screenplay 源路径与时间码来源、或把本阶段规划时间码伪称为 `9-光影` 真源 | `references/group-boundary-contract.md#direct-screenplay-intake-rule` |
...
| `FAIL-GROUP-10` | `画面风格：` 缺失、位置错误、与镜头设计不一致、退化为参数清单或标签列表，或遗漏关键机位/前景/透视/手持/发现路径导致空间感漂移 | `references/group-visual-tone-contract.md`、`SKILL.md#N5-VISUAL-STYLE` |
