...
本文件定义 `道具/3-生成` 的验收门禁。review 不拥有业务主真源改写权；修复建议必须回流到 `SKILL.md`、`references/`、`SKILL.md` runtime spine 或对应模板。
...
| `REV-PROP-GEN-03` | 多视图未被创建、补齐、下载或验收；历史 `-多视图` 缺失不计失败 | `FAIL-PROP-GEN-REFERENCE` | `Multiview Cancellation Contract` |
| `REV-PROP-GEN-04` | 图像与 JSON 同 stem，文件名包含主体 ID，并包含 `-主图` | `FAIL-PROP-GEN-NAMING` | `N7-REVIEW` |
...
| `REV-PROP-GEN-07` | 多视图 reference gate 已取消；不得把 reference 缺失作为失败 | `FAIL-PROP-GEN-REFERENCE-CONTEXT` | `Multiview Cancellation Contract` |
...
| `REV-PROP-GEN-10` | 真实生成模式下主图图片和同名 JSON 已产出；多视图不再必填 | `FAIL-PROP-GEN-MAIN-OUTPUT` | `P4-MAIN-IMAGE` |
| `REV-PROP-GEN-11` | 主图 JSON 有来源、主体 ID、输出路径、libTV 画布 UUID、节点名、Midjourney V8.1 modelKey、后缀和可复跑证据链 | `FAIL-PROP-GEN-JSON` | `N3-MAIN-PROMPT` |
...
| `REV-PROP-GEN-14` | 道具主图已确保存在于 `projects/aigc/<项目名>/3-主体/道具/3-生成/`；本地 canonical 已有时可跳过下载并记录 `already_present`，本地缺失时需下载或复制补齐 | `FAIL-PROP-GEN-LOCAL-SYNC` | `P4-MAIN-IMAGE` |
...
| `REV-PROP-GEN-03` | 多视图取消 | 多视图未被创建、补齐、下载或验收 | `Multiview Cancellation Contract` |
| `REV-PROP-GEN-04` | 命名 | 图像与 JSON 同 stem，文件名包含主体 ID，且包含 `-主图` | `SKILL.md Output Contract` |
...
| `REV-PROP-GEN-07` | 参照上下文 | 多视图 reference gate 已取消；reference 缺失不构成失败 | `SKILL.md 的 Thinking-Action Node Map` |
...
| `REV-PROP-GEN-10` | 主图产物 | 真实生成模式下主图图片和同名 JSON 存在 | `templates/single-subject-prompt.json` |
| `REV-PROP-GEN-11` | JSON 证据链 | 主图 JSON 记录来源、主体 ID、输出路径、画布 UUID、节点名、模型 key、Midjourney 后缀和可复跑证据 | `templates/single-subject-prompt.json` |
...
checked_outputs:
  - subject: ""
    ...
    main_image: ""
    main_prompt_json: ""
    multiview_image: "disabled"
    multiview_prompt_json: "disabled"
    libtv_canvas_uuid: ""
    libtv_node_name: ""
    ...
    reference_node_name: ""
    reference_context_status: "disabled_multiview"
findings: []
next_action: ""
```
