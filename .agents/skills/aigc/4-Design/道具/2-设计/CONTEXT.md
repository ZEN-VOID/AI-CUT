...
- outcome: 为 `.agents/skills/aigc/4-Design/道具/2-设计` 建立了父 skill、shared I/O、team、prompt sidecar 和最小 runner。
...
### Case-20260412-AIGC-PROP-DESIGN-NARRATIVE-SIGNIFICANCE

- milestone_type: source_contract_change
- outcome: 将上游 bridge 的 `narrative_significance` 正式接入 `2-设计` 的 brief、specialists、canonical、prompt 与 manifest 审计链。
- root_cause_or_design_decision: 脚本层虽然已有零散 `narrative_function`，但主合同没有要求把“特殊叙事意义”转成设计事实，导致关键剧情道具有被平均化处理的风险。
- final_fix_or_heuristic: 让 `道具设计.json.design_thesis.narrative_significance` 成为稳定真源，并在 `prop_design_prompt.json.narrative_focus` 与 manifest coverage 中同步记录叙事重点。
- prevention_or_replication_checklist:
  - [x] `2-设计/SKILL.md` 已把叙事意义写进 brief、specialists、canonical、prompt、audit 节点
  - [x] `build_prop_design_packets.py` 已把该字段落到 design/prompt/manifest
  - [x] `_shared/IO_CONTRACT.md` 已增加不得静默丢失叙事意义的硬规则
- evidence_paths:
  - `.agents/skills/aigc/4-Design/道具/2-设计/SKILL.md`
  - `.agents/skills/aigc/4-Design/道具/2-设计/CONTEXT.md`
  - `.agents/skills/aigc/4-Design/道具/2-设计/scripts/build_prop_design_packets.py`
  - `.agents/skills/aigc/4-Design/道具/2-设计/_shared/IO_CONTRACT.md`
- user_feedback_or_constraint: 用户明确要求为 `.agents/skills/aigc/4-Design/道具` 技能包的思维·执行节点加入“是否具有特殊叙事意义”的考虑，并允许优化成更稳定的合同表达。

### Case-20260412-AIGC-PROP-DESIGN-TYPE-ELEMENTS-ALIAS

- milestone_type: source_contract_change
- outcome: 将道具设计链对 `2-Global` 类型文档的 canonical 引用统一为 `类型元素.md`，并在 runner / builder 中保留 `--type-guide` 兼容别名。
- root_cause_or_design_decision: 上游导演链已把 canonical 名收束为 `类型元素`；若 `4-Design` 仍在主示例和 CLI help 中使用 `类型指导`，会继续把旧命名扩散到新项目。
- final_fix_or_heuristic: 主示例命令改用 `--type-elements "projects/<项目名>/2-Global/类型元素.md"`，脚本层新增 `--type-elements` 并把 `--type-guide` 降为兼容别名。
- prevention_or_replication_checklist:
  - [x] 主示例命令已改用 `--type-elements`
  - [x] `build_prop_design_packets.py` 已支持 `--type-elements`
  - [x] `run_prop_design_pipeline.py` 已支持 `--type-elements`
  - [x] 经验层已记录“canonical 名与兼容别名分层”规则
- evidence_paths:
  - `.agents/skills/aigc/4-Design/道具/2-设计/SKILL.md`
  - `.agents/skills/aigc/4-Design/道具/2-设计/references/execution-flow.md`
  - `.agents/skills/aigc/4-Design/道具/2-设计/scripts/build_prop_design_packets.py`
  - `.agents/skills/aigc/4-Design/道具/2-设计/scripts/run_prop_design_pipeline.py`
- user_feedback_or_constraint: 用户要求将 `schema / 2-Global 文档` 的命名漂移统一到 `类型元素`，避免下游继续沿用旧名。
