...
- 把事实转成可见词：材质、形制、年代、工艺、使用/保存状态、握持方式、尺度、重量感。
...
- 道具不能只满足“像这个东西”；它还要在轮廓、比例、材质、工艺、使用/保存状态、文化符号或功能结构上有可见设计。
- 关键道具的 `signature detail` 应该能被一句 prompt 锁定，例如特殊封缄、非对称轮廓、独特材质记忆点、铭文、徽记、修补痕或开合结构。
- 普通功能道具也要有克制设计：通过材质对比、维护状态、比例、接口、结构分区、闭合方式、缝线、铆钉、封缄或有依据的旧损/包浆/折痕变得有设计价值，而不是加随机装饰或默认做旧。
- 文化/身份符号必须有语境：纹样、铭文、徽记、器型、封缄和材质选择要能回到时代、地域、阶层、职业、宗教/族群禁区或功能逻辑；无依据时用极简、洁净、结构化或功能主导的细节替代。
- `knowledge-base/prop-design-corpus.md` 适合在设计前选取“设计轴”和“文化母题”，再原创转译；不要把语料词条当模板句直接塞进输出。

## Prompt Compression

- 先保留：single prop subject、material、signature detail、condition/history、lighting/camera、global style anchor、negative constraints。
...
- 如果 `project_memory_init_context` 没有明确设计相关综合，就使用 `2-美学` 和项目记忆，不强行编造顾问或监制角色。
