...
| 顾问与复核流程 未请教监制 | 顾问请教层 | 回到 `team.yaml.roles.supervision.stage_profiles."5-摄影"` 或共享合同回退路径，按节点派生问题；不可用时直接使用本地流程 | packet 或本地 checklist 能回指 node/pass/gate |
...
- 物理遮挡可以作为转场锚点，但 `5-摄影` 只负责当前画面内部的遮挡交出点：柱子、门框、树干、墙面、衣摆或黑场前景如何吞没画面。完整组间连接件的创意裁决仍交给 `6-分组`，不要在单条 `分镜明细` 中越权写完整跨组转场。
