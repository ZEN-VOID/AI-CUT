...
本文件承载 `story-drafting-gpt-native` 的章节正文细则。它不拥有入口路由权；调用时由根 `SKILL.md` 决定何时加载。

## Stage Position

- 当前技能是 `story2026` 主链 `3-初稿` 阶段的 `A-GPT原生` 路径，不再通过 `正文/` child 承接。
- 当 `3-初稿` 路由选择 `A-GPT原生` 时，当前技能拥有当前章正文根文件与 YAML 头的写权。
...
- `./templates/gpt-native-system-prompt.md`
- `./scripts/write_chapter_gpt_native.py`
...
- `supervision_packet`：正式写作时必须由 team supervision subagents 产出并进入 GPT-native messages；该包必须记录项目 `team.yaml` roster 来源、向各领域大师请教的问题、顾问回答摘要和最终可执行指导；若上层策略阻断真实 subagents，必须有降级报告。
...
| `business_object` | 当前章 planning、上游卷/整书规划、全局卡、风格卡、`north_star.yaml`、项目级 `MEMORY.md`、项目级 `CONTEXT/`、当前卷全部前序章正文。 |
...
| `topology_fit` | `source lock -> context pack -> minimal frontmatter markers -> continuity bridge -> GPT-native chapter generation -> final writeback` |
...
16. actual creative step 默认由当前 GPT/LLM 会话直接完成；若需要外部 provider，应显式改走对应 provider skill。
17. 正式写作默认启动隔离 team supervision subagents；同为 GPT 时也必须分离主写作上下文与监制上下文。具体模式为读取项目 `team.yaml -> roles.production.members`，把已指定监制组成员作为资深创作顾问逐一请教，并把其创意脑洞、个人风格判断和可执行指导汇流为写作前额外上下文。
18. `scripts/write_chapter_gpt_native.py` 只能装配上下文、注入 `supervision_packet`、输出 sidecar、校验已创作稿与写回，不得直接生成正文。
19. 若 GPT 原生输出不含完整 YAML frontmatter、`写作模型` 不等于 `GPT`、缺少 `字数: XXX字`、缺少 `# 第N章｜章标题` 标题行，或句式重复门禁失败，必须判定为 output invalid，禁止直接写回业务真源。
...
- GPT 原生流正文 frontmatter 保持极简，只记录写作模型与字数；同卷前文路径、连续性桥和上下文引用由 context pack、GPT-native sidecar 与 dry-run summary 追溯。
