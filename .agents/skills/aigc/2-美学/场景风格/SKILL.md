...
`场景风格` 是 AIGC 影片项目的场景层级视觉协议制定技能。它从上游剧本、`画面基调`、参考图/视频和项目设定中提取空间尺度/秩序、空间光影、色彩/明度纪律、氛围介质、生态/人工系统组织、环境层次、时代质地和场景视觉统一性，形成可被后续具体场景设计继承的风格协议。
...
- 默认上游来源包括 `projects/aigc/<项目名>/1-分集/`、`projects/aigc/<项目名>/2-美学/画面基调/全局风格协议.md`、用户指定参考图/视频和项目设定。用户显式指定其他来源时，以用户输入为本轮来源并标记来源类型。
- 若输入来源或任务目标明确为 `第N集`，正式输出对象是该集场景风格覆盖，写入 `projects/aigc/<项目名>/2-美学/第N集/场景风格/`；只有整季、多集汇总或用户明确项目级基线时才写入非逐集 `场景风格/`。
- 多模态参考只允许提供场景层风格事实，例如空间层次、环境光结果、氛围介质/粒子层、生态系统组织、人工/科技系统组织、时代质地和统一性策略；不得把参考中的具体地点、建筑、道具、镜头构图或生成参数迁入协议。
...
- 建立空间尺度/秩序、空间光影、色彩/明度纪律、氛围介质/粒子层、生态系统组织、人工/科技系统组织、环境层次、时代质地和场景视觉统一性矩阵。
...
- `projects/aigc/<项目名>/2-美学/画面基调/全局风格协议.md` 或用户粘贴的全局视觉协议。
- 项目初始化资料、世界观设定、时代背景、用户美术偏好、禁区说明。
- 参考图、参考视频、参考作品名称、环境概念图说明、导演/美术指导/作品锚点。
- 已有逐集 `projects/aigc/<项目名>/2-美学/第N集/场景风格/场景风格协议.md`、项目级 `projects/aigc/<项目名>/2-美学/场景风格/场景风格协议.md` 或候选 `scene_style_prompt`。
...
- 用户指定的时代背景、环境参考、生态系统或人工/科技系统偏好、禁用元素、模型平台、输出语言。
...
| --- | --- | --- | --- | --- | --- |
| `script_scene_analysis` | 指定 `1-分集` 文件/目录或粘贴剧本文本 | `Script Scene Style Path` | `N1,N2,N4,N5,N6,N7,N8` | `CONTEXT.md` | `FAIL-SS-TYPE-SCRIPT` |
...
| --- | --- | --- |
| `single_episode_scene_seed` | 基于单集剧本建立场景风格覆盖 | `projects/aigc/<项目名>/2-美学/第N集/场景风格/场景风格协议.md` |
| `project_scene_protocol` | 基于多集、项目资料和画面基调建立正式项目级协议 | `projects/aigc/<项目名>/2-美学/场景风格/场景风格协议.md` |
...
| `N2-SCRIPT-SCENE-EVIDENCE` | 从剧本/设定抽取场景风格证据 | `1-分集`、项目设定或文本 | 提取空间秩序、时代背景、环境压力、生态系统组织、人工/科技系统组织、氛围介质、场景统一需求；禁止输出具体地点清单 | `script_scene_evidence_map`，至少 6 条证据 | `N3` / `N4` | 每条证据能回指输入片段或项目资料 |
...
| `N4-SCENE-STYLE-DIMENSIONS` | 汇流九维场景风格属性 | N2/N3 证据 | 建立 9 个维度：空间尺度/秩序、空间光影、色彩/明度纪律、氛围介质/粒子层、生态系统组织、人工/科技系统组织、环境层次、时代质地、视觉统一性 | `scene_style_dimension_matrix` | `N5` | 9 维齐全，且只保留场景层级属性 |
...
| `N6-PROMPT-DISTILL` | 蒸馏场景风格 prompt | N4/N5 输出 | 生成约 100 字中文 `scene_style_prompt`；包含空间光影、氛围介质/粒子层、环境层次、时代质地、生态/人工系统组织和统一性边界；过滤具体地点、构图摄影和生成请求 | `candidate_scene_style_prompt`、`contamination_scan` | `N7` | 80-130 个中文字；禁用类别残留 0 个 |
...
| 是否覆盖空间尺度/秩序、空间光影、色彩/明度纪律、氛围介质、生态/人工系统组织、环境层次、时代质地和视觉统一性？ | `GATE-SS-02-DIMENSION-COVERAGE` | `FAIL-SS-DIMENSION-MISSING` | `N4-SCENE-STYLE-DIMENSIONS` | 九维矩阵完整性 |
...
- 可保留的场景层级类别：空间尺度/秩序、空间光影结果、色彩/明度纪律、氛围介质/粒子层、生态/人工系统组织方式、环境层次、时代质地和跨场景视觉统一性。

## Output Contract

正式写回路径：

- `episode_scoped`: `projects/aigc/<项目名>/2-美学/第N集/场景风格/场景风格协议.md`
- `episode_scoped`: `projects/aigc/<项目名>/2-美学/第N集/场景风格/执行报告.md`
- `series_baseline`: `projects/aigc/<项目名>/2-美学/场景风格/场景风格协议.md`
- `series_baseline`: `projects/aigc/<项目名>/2-美学/场景风格/执行报告.md`
...
当 `2-美学` 父级调度多个同级风格子技能时，`场景风格` 参与父级并发 fan-out，不要求等待 `画面基调` 实时先跑；若已有 `画面基调` 则继承其媒介和渲染纪律，若缺失则标记 `candidate/dependency_gap`。它向后续具体 `场景设计`、`分镜风格`、`摄影风格` 提供场景层级约束，只拥有场景风格协议和 `Scene Style Prompt` 的真源权，不拥有具体地点清单、单场景设计、构图、摄影或生成请求真源，不得反向补写或覆盖全局画面基调。
