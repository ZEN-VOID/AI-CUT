...
- 若改动涉及正文主创或润色，必须继续加载 owning stage 的 `SKILL.md + CONTEXT.md`；不再加载旧模型分流合同。
- 若用户指定的目标文档头部含 legacy `写作模型` 或 `润色模型` 字段，只能把该字段作为历史执行环境线索读取，不得作为默认正文调整 lane。
...
- 缺少项目根、目标局部、改动意图、破坏性写回授权或 canonical owner 判定时必须阻断，先输出最小缺口报告。
...
- 不绕过 `3-初稿` 或 `4-润色` 根技能直接把 repair brief、验收 finding 或脚本 patch 当成 canonical creative truth。
...
1. 先锁定项目根、目标局部、改动意图、写回权限和 owning stage。
2. 若目标是用户指定的正文或润色文档，先读取文档头部 legacy `写作模型` / `润色模型`；存在时只记录为 `creative_engine_note`，不得驱动修复路由。
...
6. 若需要创作性改写，输出 repair brief 并路由到 owning stage 根技能；本技能只拥有诊断、计划、约束、汇流和验收权。
...
| `FIELD-REPAIR-04` | authorship | `owning_stage` and LLM-first creative authorship rule | `FAIL-REPAIR-AUTHORSHIP` |
...
- Completion gate: 已完成影响图、源层 owner、写回顺序、创作权归属、本技能 review/code-reviewer 审计和 residual risk 说明；执行型任务还必须列出实际改动文件与复验结果。
