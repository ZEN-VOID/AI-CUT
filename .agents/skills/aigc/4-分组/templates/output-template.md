...
- 每组包含三项 north_star 风格投影、正文、逐 `分镜N:` 的 `站位和位移：`、出场画面和 YAML 统计；其中第 1 行全局风格必须在原词基础上追加固定词 `不生成字幕，不生成BGM，要生成物理互动音效与环境音。`，第 2 组起包含入场画面。
- 每条 `站位和位移：` 主语必须是明确角色名或上游已命名的稳定群体称谓，不使用 `画面主体`、`主体`、`人物`、`角色`、`主角`、代词等模糊指代。
