...
- 数字序号阶段包（`0-初始化` -> `1-分集` -> `2-编导` -> `3-运动` -> `4-摄影` -> `5-分组` -> `6-设计` -> `7-图像` -> `8-视频` -> `9-审片`）默认按数字升序串行推进，前一阶段 canonical 产物自动作为后一阶段输入。
...
| `8-视频` | `.agents/skills/aigc/8-视频/libTV画布流/SKILL.md`  | 用户只说进入 `8-视频`、生成视频阶段、下一步生视频、LibTV 画布流，且未明确指定旧 A/B/C/D 任一路线 | 用户点名 `A-分镜画面参照`、`B-分镜故事板参照`、`C-主体参照`、`D-主板混合参照` 旧路线，或要求多路线对比 |
...
| `2-编导`   | `.agents/skills/aigc/2-编导/`   | `projects/aigc/<项目名>/2-编导/`   | active；整合旧 `2-编剧` / `3-导演` / `4-表演`                               |
...
| `8-视频`   | `.agents/skills/aigc/8-视频/`   | `projects/aigc/<项目名>/8-视频/`   | active；默认叶子 `libTV画布流`；旧 A/B/C/D 位于 `backup/` 兼容路径 |
| `9-审片`  | `.agents/skills/aigc/9-审片/`  | `projects/aigc/<项目名>/9-审片/`  | active                                                                       |

旧 `3-导演` / `4-表演` 技能目录已移除；这些名称只作为自然语言兼容触发词，直接路由到 `2-编导` 的 director / performance 内部层。
...
| current video stage                         | `8-视频/SKILL.md + CONTEXT.md`；未显式指定旧叶子时默认继续加载 `8-视频/libTV画布流/SKILL.md + CONTEXT.md`               |
...
3. 选择唯一主入口；若主入口为 `7-图像` 且用户未显式指定叶子，默认进入 `7-图像/B-分镜故事板`；若主入口为 `8-视频` 且用户未显式指定旧叶子，默认进入 `8-视频/libTV画布流`；若主入口为 `9-审片`，必须定位实际视频素材和对应 `5-分组` 分镜组。
