...
flowchart TD
    ...
    J --> K["steps/character-design-workflow.md"]
    K --> L["LLM-first 创作与汇流"]
    L --> M["templates/output-template.md"]
    M --> N
    N --> O{"通过门禁?"}
...
3. `解构` 必须包含字段：`Identity & Story Pressure`、`Visual Drivers`、`Detailed Character Design`、`Detailed Costume Design`、`Cinematography`。
4. `提示词设计` 必须为英文、引用全局风格提示词与服装风格，并控制在 2000 字符内。
5. 画面固定为纯色背景全身定妆照，不得置身具体场景、建筑空间、街景、室内陈设或复杂环境。
