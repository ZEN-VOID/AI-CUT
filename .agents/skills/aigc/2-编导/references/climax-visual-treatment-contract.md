...
本文件将 `story/2-卷章/3-章级` 的章级爽点设计，投影为 `aigc/2-编导` 的高潮画面处理机制。

`2-编导` 只负责从上游逐集正文中识别已经存在的读者满足、冲突释放、认知翻转、关系暖点或体验高点，并把它们强化为可拍、可听、可执行的剧本字段。它不得新增剧情事实、对白、事件结果或人物动机。
...
执行 `2-编导` 时，应在字段分流后、LLM 草稿前完成一次 `peak_visual_pass`。若上游存在明确或隐性的高潮画面成分，每集至少锁定 1 个、最多 3 个 `peak_visual_candidate`；若上游没有真正高点，只锁定最强 `micro_payoff`，不得硬造大高潮。
