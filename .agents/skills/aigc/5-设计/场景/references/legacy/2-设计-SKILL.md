...
description: Use when `5-设计/2-设计/场景` needs to consume `场景清单.json + 场景研究.json + scene_design_bridge.json`, plus `0-Init` and `2-Global` truths, then emit `scene_design.json` and per-scene Markdown design cards.
...
`5-设计/2-设计/场景` 是当前 `aigc` 技能树里的统一场景主体设计 leaf。
...
- 当前第一输入根固定为 `projects/aigc/<项目名>/4-设计/场景/1-清单/第N集/` 下的清单产物 JSON。
...
1. `scene_design.json`
2. `[场景名].md`
3. `[场景名].<ext>`（built-in imagegen 单主体图片）
4. `_manifest.json`
...
- 当前 skill 是 `5-设计/2-设计` 下的场景 leaf。
- 上游对象池与场景研究真源属于 `5-设计/1-清单/场景`。
...
- `.agents/skills/aigc/5-设计/1-清单/场景/SKILL.md`
...
10. `.agents/skills/aigc/5-设计/1-清单/_shared/list-output-contract.md`
11. `.agents/skills/aigc/5-设计/1-清单/场景/SKILL.md + CONTEXT.md`
12. 本 `SKILL.md + CONTEXT.md`
13. `references/scene-design-assembly.md`
14. `templates/scene_masterprompt.structured.v2.md`
15. `projects/aigc/<项目名>/4-设计/场景/1-清单/第N集/场景清单.json`
16. `projects/aigc/<项目名>/4-设计/场景/1-清单/第N集/场景研究.json`（若存在）
17. `projects/aigc/<项目名>/4-设计/场景/1-清单/第N集/scene_design_bridge.json`（若存在）
...
- `projects/aigc/<项目名>/4-设计/场景/1-清单/第N集/场景清单.json`
...
- `projects/aigc/<项目名>/4-设计/场景/2-设计/第N集/`
...
- `projects/aigc/<项目名>/4-设计/场景/2-设计/thinking/第N集/[场景名].md`
...
7. `full_generation_prompt` 必须等于统一 `global_style_prefix + design_prompt`，并作为 built-in imagegen 的唯一 prompt 入参。
...
```bash
python3 .agents/skills/aigc/5-设计/2-设计/场景/scripts/validate_scene_design_projection.py \
  --output-dir "projects/aigc/<项目名>/4-设计/场景/2-设计/第N集"
```

legacy 兼容投影入口：

```bash
python3 .agents/skills/aigc/5-设计/2-设计/场景/scripts/build_scene_design_packets.py \
  --catalog "projects/aigc/<项目名>/4-设计/场景/1-清单/第N集/场景清单.json" \
...
flowchart TD
    ...
    M --> N["Call built-in imagegen same-dir same-stem"]
```
...
| `FIELD-SCN-DES-12` | `[场景名].<ext> / _manifest.json.auto_image` | 自动生图使用含全局风格前缀的完整 prompt，默认内置 imagegen 请求准备，图片与设计文件同目录同 stem | `design-output-contract.md`、`image-generation-execution-contract.md`、`全局风格.md`、`[场景名].md` | `S10` | auto image completeness | `FAIL-SCN-AUTO-IMAGE` |
...
| `S10` | 单主体自动生图 | 读取 `[场景名].md` 与全局风格前缀；自动生图前复验空镜锚句，再默认内置 imagegen 请求准备 built-in imagegen，同目录同名图片由后续验收复核 | `auto_image_packet + reference_cleanliness_note` | `S11` | `S7-S10` |
| `S11` | 输出后审计边界说明 | 读取 `team.yaml` 与共享占位合同，明确当前轮场景输出不再进入 `监制` closeout；若需记录问题，只把 `SCENE-BUNDLE-01~04` 写入 audit note 与 handoff | `post_write_audit_note` | `done` | `S7-S11` |
...
| `N10-AUTO-IMAGE` | 生成单场景概念图 | `[场景名].md`、`全局风格.md`、`design_prompt` | 生成 `full_generation_prompt`；复验空镜锚句与污染主体禁令后默认内置 imagegen 请求准备 built-in imagegen，目标输出 `[场景名].<ext>` | `auto_image_packet + reference_cleanliness_note` | `N11` | 不得只传局部 prompt；内置 imagegen request_ready不得伪装为已产图；图片必须同目录同名；空镜门禁失败不得生图 |
| `N11-POST-WRITE-AUDIT-NOTE` | 输出后审计占位 | 当前轮 `scene_design.json + [场景名].md + _manifest.json`、`team.yaml`、`subagent-supervision-contract.md`、`design-slot-review-contract.md` | 按共享占位合同确认当前轮 closeout 不再由 `监制` 执行；如需后置问题记录，只把当前轮场景输出解析成 `SCENE-BUNDLE-01~04` 并写入 audit note / handoff | `post_write_audit_note` | `done` | 不得把 `source_skill_refs` 误当 runtime 授权或 post-write reviewer |
...
| `FIELD-SCN-DES-12` | 默认提交态含 `request_ready/request_batch_path/default_model`；最终验收时 `[场景名].<ext>` 已由 `full_generation_prompt` 生成，且 stem 与 `[场景名].md` 一致 | `FAIL-SCN-AUTO-IMAGE` | `S10` |
...
| `FIELD-SCN-DES-14` | `team.yaml` 已读取，且已明确当前轮场景输出的 closeout 不再由 `监制` 执行；如需后置问题，仅写 audit note | `FAIL-SCN-SUPERVISION-REVIEW` | `S11` |
...
当前轮若需记录后置问题，只围绕以上文件及其对应 `SCENE-BUNDLE-*` 写 audit note，不再以 `监制强化` 名义做 patch。
...
3. `5-设计/1-清单/_shared/list-output-contract.md`
...
10. `.agents/skills/aigc/5-设计/2-设计/_shared/subagent-supervision-contract.md`
...
2. `_shared/subagent-supervision-contract.md` 当前只作为停用占位真源；本 leaf 不再声明场景型 closeout 补选。
3. 若需后置问题记录，只能写当前轮 `scene_design.json / [场景名].md / _manifest.json` 的 audit note，并按需补阶段 `validation-report.md`。
...
7. 已使用含统一全局风格前缀的 `full_generation_prompt` 默认内置 imagegen 请求准备 `[场景名].<ext>` 请求；最终验收时图片同 stem 存在。
8. 已运行 `validate_scene_design_projection.py`，并且 `_manifest.json.template_validation.status == success`。
9. 已按 `team.yaml + _shared/subagent-supervision-contract.md` 明确当前轮场景输出的 closeout 不再由 `监制` 执行。
