...
description: Use when story2026 needs chapter-native novel drafting through the repo-local DeepSeek provider flow, turning planning truth, cards, north_star, project MEMORY/CONTEXT, and continuity into `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`.
governance_tier: full
---

# 3-Drafting / C-Deepseek流
...
`C-Deepseek流` 是 `story2026` 主链 `3-Drafting` 阶段的 DeepSeek provider 路径。它负责在路由选择 DeepSeek 流时，把当前章 planning 义务、全局卡、风格卡、`north_star.yaml`、项目记忆、项目上下文与上一章承接，转成可落盘的中文小说章节。
...
2. `.agents/skills/api/deepseek/scripts/deepseek_chat.py` 调用固定 `deepseek-v4-pro`，默认 `thinking=enabled` 与 `reasoning_effort=high`，负责实际生成完整章节 Markdown 文件。
...
- YAML 头必须显式包含 `写作模型`、`global_context`、`style_context` 与 `north_star_chapter_brief`；`写作模型` 固定为 `Deepseek`。
- 正文主体必须是小说 prose，不得把 planning 中的标题、任务线或规避条目原样复制成正文段落。
- 输出路径固定为 `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`。
- provider 返回内容缺完整 YAML frontmatter、必需字段、`写作模型: Deepseek` 或 `# 第N章｜章标题` 标题行时，禁止写回业务真源。
- DeepSeek provider artifacts 可落到项目 `reports/3-Drafting/deepseek/.../`，但它们不是业务真源。
...
| `FIELD-DSD-05` | `review/` | 质量门禁层 | verdict model、finding shape、provider evidence gate | `FAIL-DSD-REVIEW` |
...
```bash
python3 .agents/skills/story/3-Drafting/C-Deepseek流/scripts/write_chapter_via_deepseek.py \
...
| Output path | 业务真源固定写入 `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`；provider artifacts 可写入 `projects/story/<项目名>/reports/3-Drafting/deepseek/.../`。 |
