...
| `N5-CHAPTER-PAYOFF` | 设计章级爽点 | `chapter_spine`、`chapter_timeline`、`conflict_profile`、卷级 promise、`genre_payoff_profile`、任务欠账、角色/关系/信息欠账、近邻章爽点、角色最小投影或 `card_path` | 锁 `reader_desire / promise_source / genre_payoff_profile / character_anchor / payoff_mode / payoff_variation_axis / build_up / delivery_action / satisfaction_delta / exaggeration_logic / cost_or_aftershock / aftertaste_hook`；若是高超对决，额外锁 `duel_variation_axis`；并按类型画像及动能式、势能式或浪能式裁定主爽点形态 | `payoff_profile` | `N6-CHAPTER-RHYTHM` | 爽点能回指类型画像、读者期待、上游 promise 和角色个性；与近邻高潮点有差异轴；有可验证兑现动作，有状态差与余味牵引；夸张但合情理，未写成正文 |
| `N6-CHAPTER-RHYTHM` | 绘制章级节奏 handoff | `chapter_spine`、`chapter_timeline`、`conflict_profile`、`payoff_profile`、shared rhythm contract | 锁 `selected_pack / selected_mode / mode_selection_reason / payoff_type / rhythm_intensity / previous_next_contrast / 七步职责映射 / 规划义务 / 义务段位 / 建议写法` 并附 Mermaid 图 | `rhythm_handoff` | `N7-CHAPTER-ELEMENTS` 与 `N8-INFO-LAYER` | handoff slots 齐全，mode 选择有证据，payoff 与爽点设计一致，强度可复核，前后章波形有承接，建议写法未变正文 |
| `N7-CHAPTER-ELEMENTS` | 收束人物、场景、道具与任务线 | `rhythm_handoff`、`payoff_profile`、`chapter_timeline`、Cards 真源、卷级任务线 | 输出本章登场人物、主要场景、关键道具、主支线任务、支流角色、汇聚动作和未汇聚去向 | `chapter_resources` | `N9-CHAPTER-CLOSE` | 任务可上溯卷级，支流不悬空 |
| `N8-INFO-LAYER` | 分离线索与伏笔 | `chapter_spine`、`chapter_timeline`、`rhythm_handoff`、`payoff_profile`、上游信息承诺 | 写本章可见信息推进、伏笔铺设、伏笔兑现判断 | `info_layer` | `N9-CHAPTER-CLOSE` | 线索与伏笔不混写，铺设/兑现槽位存在 |
| `N9-CHAPTER-CLOSE` | 锁章末达成与规避 | 所有节点产物 | 汇总章末达成、禁飞区，按模板落盘或输出 patch | `chapter_plan` | `review/chapter-planning-review.md` | 必填标题齐全，无正文越界 |

## Branch And Merge Rules

- `N1 -> N2 -> N3 -> N4 -> N5 -> N6` 是固定串行主干，不得并行绕过。
- `N7` 与 `N8` 可以交叉校正，但必须在 `N9` 汇流。
