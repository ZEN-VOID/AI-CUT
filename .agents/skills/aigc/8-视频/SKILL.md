...
description: "Use when routing AIGC 8-video work from grouped storyboard prompts to LibTV canvas control, generation handoff, query, repair, or review."
governance_tier: router
---

# AIGC 8-视频

`8-视频` 是 AIGC 项目的视频生成阶段父入口。本级只负责路由、边界和回接；具体 LibTV 画布项目创建、素材上传、YAML 回刷、视频节点创建和连线由叶子技能持有。
...
- 从 `projects/aigc/<项目名>/5-分组/第N集.md` 生成 LibTV 画布视频节点。
...
- 可定位的分组稿：通常是 `projects/aigc/<项目名>/5-分组/第N集.md`。
...
| mode | trigger | route |
| --- | --- | --- |
| `canvas_flow` | 创建画布项目、上传主体参照、回刷 YAML、生成/连线视频节点 | `.agents/skills/aigc/8-视频/libTV画布流/SKILL.md` |

## Output Contract

- Required output: 唯一路由、项目根、分组稿路径、目标叶子技能或阻断原因。
- Output format: 简短路由说明；具体证据文件由叶子技能生成。
- Output path: 本父入口不写业务证据；叶子默认写 `projects/aigc/<项目名>/8-视频/libTV画布流/第N集/`。
...
`Symptom -> Direct Cause -> 8-视频 route owner -> selected video leaf SKILL.md -> AGENTS.md`
