...
| analysis_slot | 当前结论 |
| --- | --- |
| `business_goal` | 把 `3-Detail` 的 `角色背景面 + 分镜表现 + 摄影美学 + 时间段 + 导演意图` 收束成可追溯、可研究、可桥接的场景三真源，供 `4-Design/2-设计/场景` 直接消费。 |
| `business_object` | `projects/aigc/<项目名>/3-Detail/第N集.json`、`projects/aigc/<项目名>/4-Design/场景/1-清单/第N集/{场景清单.json,_manifest.json}`。 |
| `constraint_profile` | 上游 `3-Detail` JSON 是唯一事实真源；`角色背景面` 是主场景锚点；`分镜表现 / 摄影美学 / 时间段 / 导演意图 / 剧本正文` 只做补证；`清单/研究/bridge` 各管自己的字段边界，不得互相复制抢真源。 |
...
| `complexity_source` | 当前仓主输入已经从旧仓分镜 markdown 转成 `3-Detail` 结构化 shot fields；难点在于既要沿用旧仓高密度场景研究配置，又要避免把 `角色背景面` 整句硬升格成新的场景主键。 |
...
2. `分镜明细[].角色背景面` 是主场景实体的一号锚点。
3. `分镜表现 / 摄影美学 / 时间段 / 组间设计.导演意图 / 剧本正文` 只做 evidence augmentation。
4. 方位、门禁、边界、气氛、时间状态默认归入 `scene_variant / design_context.variable_state_layer`，不能直接扩张 canonical scene family。
5. 若无法稳定提取主场景实体，允许保守输出 `unknown` 或 `needs_manual_review`，不得把整句背景描述直接升格成稳定主键。
...
flowchart TD
    ...
    B --> C["Extract shot-level 场景锚 from 角色背景面"]
...
flowchart LR
    ...
    B -->|"No"| D["回退 分镜表现 / 剧本正文 / 标记 unknown"]
    C --> E{{"其余描述属于什么?"}}
...
| --- | --- | --- | --- | --- | --- | --- |
| `FIELD-SCENE-01` | `场景清单.json.scenes[]` | canonical 场景家族稳定，`scene_id` 与 `scene_name` 不被背景整句污染 | `角色背景面`、`剧本正文` | `S2-S4` | identity stability | `FAIL-SCENE-CATALOG` |
...
| `S2` | 扫描镜级场景锚 | 从 `角色背景面` 提取主场景家族候选与 raw variant | `scene_anchor_scan` | `S3` | `S2` |
...
| `N2-SCENE-ANCHOR` | 建立 shot-level 场景锚 | `分镜明细[].角色背景面` | 抽取主场景候选、保留 raw 背景句与 shot evidence | `scene_anchor_scan` | `N3` | `role_background_face` 不得丢失 |
| `N3-NORMALIZE` | 主场景/变体裁决 | `N2` + `分镜表现 / 摄影美学 / 时间段 / 导演意图` | 把主场景、方位、状态、气氛拆层，建立 `scene_id` | `normalized_scene_map` | `N4` | 不得把整句背景句直接变成 canonical name |
...
| `FIELD-SCENE-02` | `group_scene_map[]` 对每条记录都能回链到 `group_id / shot_id / role_background_face` | `FAIL-SCENE-MAP` | `S3-S4` |
...
1. 先 `角色背景面`，再 `分镜表现 / 摄影美学 / 时间段 / 导演意图`，最后才回退 `剧本正文`。
