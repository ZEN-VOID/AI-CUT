...
def build_role_research_payload(catalog: dict) -> Tuple[dict, dict, dict, str]:
    ...
    report_lines.extend(
        [
            ...
            "- `role_design_bridge.json`：供 `4-Design/2-设计/角色` bridge-first 消费 `role_identity / appearance_bridge / costume_bridge / continuity_bridge / prompt_ready / quality_flags`。",
            "",
            "## 已执行校验",
            "",
