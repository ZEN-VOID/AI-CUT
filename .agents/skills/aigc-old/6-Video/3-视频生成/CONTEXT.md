...
| `Assets/` 已有可用图片，但空引用视频请求被当作 prompt-only 提交 | 引用模式分流层 | 停止 handoff，先回 `2-参照引用` 绑定并严检 Assets 引用 | 在 `When Not to Use` 与 Workflow 固化“Assets 非空 + 未显式 no_reference = unresolved” | 只有通过绑定的请求或显式 no-reference 请求才能进入 provider 计划 |

## Repair Playbook

1. 先确认本轮目标是否真的属于“提交前组织”。
2. 再确认上游请求 JSON 是否稳定可提交。
3. 再查 `Assets/` 是否非空；若非空且没有显式 `prompt_only / no_reference`，空引用先回 `2-参照引用`。
4. 再判断 provider 是否已明确，还是只输出推荐与对比。
5. 产出 `submit-plan.json` 与 `submit-brief.md`。
6. 最后给唯一下一入口：外部 provider skill、人工提交，或回上游补请求对象。
7. 收尾复扫是否仍残留 `2-视频生成` 或 `references/` 作为规则真源。
...
- 判断 prompt-only 不能只看 `reference_images` 为空；还要看项目 `Assets/` 是否已有可用图，以及用户/上游是否明确声明本轮不使用参照。
