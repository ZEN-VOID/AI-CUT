...
`flash` 是 `.agents/skills/aigc` 的聊天窗口迷你工作流。它把少量源内容，通常是用户自然语言、一个分镜组、参照图、参照视频、首尾帧或图生视频要求，当作一小段故事源，快速串联吸收 `2-编剧 -> 3-美学 -> 4-导演 -> 5-表演 -> 6-氛围 -> 7-分镜 -> 8-摄影 -> 9-光影 -> 10-分组` 的核心判断，并在当前聊天窗口输出统一视频提示词。
...
| `B3` | Type Routing Matrix | 文本源、参照图、首尾帧、参照视频、repair/review 路由 |
...
| `B11` | Quantifiable Execution Criteria Contract | 时长、镜头数、证据量、阈值、重试和停止 |
...
- 用户给出一小段故事、分镜组、自然语言场景、画面想法、短视频脚本、广告片段、角色动作或 11.5 秒左右的视频需求，要求输出视频提示词。
...
- 压缩串联 2-10 阶段判断：
  1. `2-编剧`：把故事源转成可拍的冲突、动作、声画和尾钩。
  2. `3-美学`：提炼画面基调、角色/场景/道具风格、分镜和摄影风格。
  3. `4-导演`：明确观看焦点、场面调度、镜头意图。
  4. `5-表演`：明确动作、微表情、反应节奏和潜台词外显。
  5. `6-氛围`：补声画氛围、环境压迫、物理特效或心理余韵。
  6. `7-分镜`：拆成 1-4 个可生成镜头段；默认 11.5 秒不超过 4 段。
...
- 至少一种短片段源：自然语言故事、分镜组描述、图片、首帧/尾帧、视频、截图序列、时间码描述或用户对参考素材的文字说明。
...
| --- | --- | --- | --- | --- | --- | --- |
| `F1-INTAKE` | 锁定源、模式、时长、目标模型和注意力锚点 | 用户请求、素材、文字源 | 判定 mode；默认 `duration=11.5s`；建立 `source_manifest`、`target_profile`、`business_profile`、`attention_anchor` | 至少 1 个 source、1 个 target、1 个 duration decision | `F2` / `F3` / `V1` / `R1` | 无源或无目标不得继续；素材不可见须标注 insufficient |
...
| `F3-SCREENPLAY-COMPRESS` | 2-编剧压缩：把源变成可拍短冲突 | source、F2 | 提炼人物、目标、阻力、动作链、声画承托、尾钩；保留用户事实 | `mini_screenplay_profile` | `F4` | 不得扩成长剧本；新增事实必须标注为 prompt adaptation |
| `F4-AESTHETIC-DIRECTOR` | 3-美学 + 4-导演压缩 | F3、项目偏好 | 定义全局风格、色彩、材质、场面调度、观看焦点、镜头意图 | `style_director_profile` | `F5` | 风格必须具体到画面、空间、材质或摄影，不用空泛词 |
| `F5-PERFORMANCE-ATMOSPHERE` | 5-表演 + 6-氛围压缩 | F3-F4 | 明确动作、微表情、反应节奏、潜台词、环境氛围、物理特效、声音/空气/光尘 | `performance_atmosphere_profile` | `F6` | 表演与氛围必须服务故事动作，不得无关堆料 |
| `F6-STORYBOARD-MINI` | 7-分镜压缩 | F3-F5、duration | 拆 1-4 个镜头段；默认 11.5 秒使用 2-3 段，复杂动作最多 4 段；每段有起止秒、主体、构图、动作 | `mini_shot_plan` | `F7` | 总时长必须等于目标时长；每段可生成、连续、无跳轴 |
...
| `F9-PROMPT-ASSEMBLE` | 10-分组压缩并汇成统一提示词 | F3-F8、mode | 写 `Flash Prompt Pack`：正向 prompt、镜头段、运动/表演/氛围/光影、首尾帧或图生视频策略、负面约束、模型注意事项 | `candidate_prompt_pack` | `F10` / `R1` | 输出必须是一个统一 prompt pack；不得写项目文件路径 |
| `F10-REVIEW-CLOSE` | 审查并在聊天窗口交付 | candidate、Review Gate | 执行 gate；必要时最小返工一次；输出最终聊天答复 | `review_verdict`、`final_prompt_pack` | done | `GATE-FLASH-01..10` 阻断项为 0 |
| `R1-REWORK` | 最小返工 | fail code、用户反馈 | 回到证据、故事压缩、镜头、摄影、光影或 prompt 组装层修复；同一 fail code 最多 2 轮 | `repair_log` | `F9` / `F10` | 不得泛化重写无关内容 |
...
```mermaid
flowchart TD
    A["short story / image / first-last frames / video reference"] --> B["F1-INTAKE"]
    B --> C{"visual source?"}
...
| `.agents/skills/aigc/2-编剧/SKILL.md + CONTEXT.md` | 用户要求对齐正式剧本规则或 `FAIL-FLASH-SCREENPLAY` | 声画、动作链、尾钩边界 | 触发正式写回或长篇改编 | `F3-SCREENPLAY-COMPRESS` |
...
| `.agents/skills/aigc/5-表演/SKILL.md + CONTEXT.md` | 表演动作空泛或潜台词不可拍 | 表演外显与反应节奏 | 写正式表演稿 | `F5-PERFORMANCE-ATMOSPHERE` |
| `.agents/skills/aigc/6-氛围/SKILL.md + CONTEXT.md` | 氛围承托不足或物理特效需要边界 | 氛围画面和声画余韵 | 写正式氛围稿 | `F5-PERFORMANCE-ATMOSPHERE` |
...
| `evidence_count` | 文本源至少 1 个 `mini_screenplay_profile`；视觉源至少 1 个 `media_evidence_table`；最终至少 1 个 `mini_shot_plan`、1 个 `camera_plan`、1 个 `lighting_plan`、1 个 `final_prompt_pack` | `Thinking-Action Node Map.evidence` | `FAIL-FLASH-EVIDENCE` |
| `pass_threshold` | `GATE-FLASH-01..10` 阻断项为 0；文件写回 0；强推断未标注 0；参考复制风险未处理 0；镜头段时长不等于目标时长 0 | `F10.gate` | `FAIL-FLASH-THRESHOLD` |
...
| `ATTE-FLASH-03` | 漂移检测 | 开始写长剧本、写项目路径、生成报告、照搬参考片、堆空泛形容词、忘记 11.5 秒 | `Review Gate Binding` |
| `ATTE-FLASH-04` | 再集中机制 | 回到最近有效节点：素材漂移回 F2，故事漂移回 F3，prompt 汇流漂移回 F9 | `R1-REWORK` |
...
| `CHK-FLASH-SEMANTIC` | 定稿 prompt 前 | 检查 2-10 压缩链均有对应字段 | `candidate_prompt_pack` | `FAIL-FLASH-SEMANTIC` |
| `CHK-FLASH-VALIDATION` | Review Gate 失败 | 回 R1 最小返工，最多 2 轮 | `repair_log` | `FAIL-FLASH-VALIDATION` |

## Evaluation Prompt Contract

- `test-prompts.json` 至少覆盖：纯文本故事源、图生视频、首尾帧、参考视频、prompt repair。
...
Completion gate: `GATE-FLASH-01..10` 通过，最终答复只包含一个 prompt pack 和必要的简短使用说明。
...
`Prompt Symptom -> Flash Node Failure -> Referenced AIGC Stage Compression Boundary -> aigc root router -> AGENTS.md LLM-first / Skill 2.0`
