...
- `0-初始化 -> 1-Cards -> 2-卷章 -> 3-初稿 -> 4-Polish -> 5-Validation -> 6-Loopback`
...
- `2-卷章` 默认只有一个对外入口：组级 `SKILL.md`。
- 所有 leaf 子目录默认视为内部 planning pass，不应在用户只说“进入 2-卷章”时被直接当成首入口。
...
- `story2026` 的 `1-设定`、`2-卷章`、`3-初稿`、`4-Polish`、`5-Validation` 与 `6-Loopback` 没有被误写成过期阶段链或旧中文 stage。
