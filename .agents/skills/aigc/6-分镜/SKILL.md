...
description: "Use when splitting AIGC episode scripts into inline storyboard shots from 5-导演 or a user-specified draft."
...
`6-分镜` 负责把上游逐集剧本文稿拆成可下游消费的内联分镜稿。默认 source 是 `projects/aigc/<项目名>/5-导演/第N集.md`；若用户显式指定文稿、粘贴文本或要求跳过某些上游环节，则以用户指定文稿为 source，并在报告中记录 `source_override=true`。本技能必须同时消费 `2-美学` 中的 `画面基调/全局风格协议.md` 与当前集优先的 `分镜风格/分镜风格协议.md`；若存在当前集 `摄影风格/摄影风格协议.md`、`角色风格/角色风格协议.md` 或 `场景风格/场景风格协议.md`，作为辅助约束读取，缺失时回退项目级基线。
...
阶段职责边界：`6-分镜` 拥有静态画面结构的定义权，负责确定起始状态帧里的主体位置、前中后景、左右方位、遮挡关系、空间纵深和构图层次；`7-摄影` 只能基于这些既有结构设计摄影机如何进入、移动、对焦和使用空间，不应反向改写本阶段的空间布局。

本技能不是 `9-图像`，不生成图片、storyboard sheet、图像 prompt 或视频请求；不是旧 `4-摄影` 的覆盖替代，也不把原字段正文改写为 `[起始秒-结束秒]` 时间段。本阶段只做逐画面点的分镜拆分和起始状态帧设计，为后续图像、视频、分组或审查提供稳定镜头清单。
...
- 当上游 source、`2-美学` 画面基调/分镜风格、项目记忆共同影响分镜拆分方向时，执行报告必须追加 `Storyboard Direction Inheritance Matrix`，说明每类上下文如何引导 `beat_decision`、`composition_constraint`、`duration_rationale`、`continuity_policy` 与 `boundary_check`。
...
- 冲突优先级：用户显式请求 > 根 `AGENTS.md` / meta 规则 > `.agents/skills/aigc/SKILL.md` > 本 `SKILL.md` > 本 `Module Loading Matrix` 授权模块 > 上游 source 文稿 > `2-美学` 产物 > 项目 `MEMORY.md` > 项目 `CONTEXT/` > 本 `CONTEXT.md`。
...
- 用户要求 `6-分镜`、`分镜拆分`、`内联注入分镜`、`按画面点加分镜`、`从 5-导演 到 6-分镜`，或要求在剧本字段下写 `分镜1（N-N秒）：...`。
- 输入是 `5-导演/第N集.md`、用户指定的单集剧本文稿、粘贴文本或已有候选分镜稿，且需要结合 `2-美学` 的画面基调与分镜风格完成逐画面分镜。
...
- 不改写剧情事实、对白原意、角色关系、场景顺序、上游字段标题或 `2-美学` 协议。
...
| --- | --- | --- | --- |
| `business_goal` | 将单集上游文稿拆成保留原剧本结构的内联分镜稿 | 用户请求、source 文稿、`2-美学` source、输出路径 | `FAIL-SB-BUSINESS-GOAL` |
...
| `complexity_source` | 复杂度来自逐画面点归属、节拍数量裁决、构图起始帧设计、时值量化、2-美学继承和连续性 | 类型路由、节点证据、reference execution matrix | `FAIL-SB-COMPLEXITY` |
...
- `2-美学/画面基调/全局风格协议.md`、当前集优先的 `2-美学/第N集/分镜风格/分镜风格协议.md`，缺失时回退 `2-美学/分镜风格/分镜风格协议.md`；以及可选且当前集优先的 `摄影风格/摄影风格协议.md`、`角色风格/角色风格协议.md`、`场景风格/场景风格协议.md`。
...
- 当前集优先的 `2-美学/第N集/摄影风格`、`角色风格`、`场景风格`，缺失时回退项目级对应风格；项目 `MEMORY.md` 的长期审美要求、分镜密度偏好、下游视频限制。
...
| --- | --- | --- |
| `single_episode_storyboard_split` | 指定单个 `第N集.md`、单集 source 或单集粘贴文本 | 项目内 `6-分镜/第N集.md` 分镜稿 |
...
| `N2-SB-UNDERSTAND` | 理解题材、情节、剧本正文和 2-美学上下文 | source、美学协议、项目上下文 | 摘要题材机制、主要冲突、场景节奏、情绪曲线、画面基调、大师/作品参照、分镜风格、下游限制和美学禁区 | `storyboard_context_profile`、`aesthetic_context_map`、`master_reference_map`、`scene_rhythm_map` | `N3` / `R1` | 不能只写类型标签；必须说明分镜方向、禁用构图和美学继承边界 |
...
| `N9-SB-REVIEW-REPAIR` | 审查并最小修复候选稿 | candidate、review gates | 执行 `GATE-SB-01..25`；阻断项回到 N2-N8 或 R2 最小修复，最多 3 轮；无法修复时进入阻断收束 | `review_verdict`、`repair_log`、`reference_execution_matrix`、`upstream_context_application_map`、`storyboard_direction_inheritance_matrix`、`rule_evidence_map`、`beat_calculation_audit` | `N10` / `R1` | review 未通过不得写回 canonical |
| `N10-SB-WRITEBACK-CLOSE` | 写回唯一输出并生成报告 | passed candidate、output contract | 写入 `projects/aigc/<项目名>/6-分镜/第N集.md` 与 `执行报告.md`；报告记录 source、美学继承、Scene Axis Dictionary、reference matrix、rule map、N/A、修复和残余风险 | `output_manifest`、`execution_report` | done | 输出路径唯一；报告证据完整；正式写回不得缺执行报告 |
...
```mermaid
flowchart TD
    A["5-导演/第N集.md 或用户指定文稿"] --> B["N1-SB-INTAKE"]
    C["2-美学: 画面基调"] --> B
    D["2-美学: 分镜风格"] --> B
...
| `fallback_evidence` | 若 `5-导演` 缺失但用户指定 source，报告 `source_override=true`；若画面基调或分镜风格缺失，必须有等价替代文本并标记降级；若某字段语义不可判定，保持原文不注入并在报告列 N/A | `Review Gate Binding.report_evidence` | `FAIL-SB-QUANT-FALLBACK` |
...
- 若同轮同时命中 `5-导演` 和 `6-分镜`，必须先完成 `5-导演` 输出，再以其 `5-导演/第N集.md` 作为本技能 source。
- `2-美学` 输出只作为上下文和约束，不参与本技能主稿聚合，不被反向改写。
- `9-图像` 是下游图像阶段；本技能完成后可作为 `9-图像` 的上游文本依据，但不得自动调用图像生成。
...
| `../_shared/upstream-context-application-contract.md` | 任意正式生成、repair、review，或 `FAIL-SB-UPSTREAM-CONTEXT` / `FAIL-SB-UPSTREAM-DIRECTION-MATRIX` | 规定上游文稿与美学上下文如何被分镜稿应用、保真和举证，要求 `Upstream Context Application Map` 与 `Storyboard Direction Inheritance Matrix` | 替代分镜主创、改写上游正文、重画剧情画面或把美学协议复制成分镜正文 | `N1-SB-INTAKE` / `N3-VISUAL-POINTS` / `N9-SB-REVIEW-REPAIR` |
...
| 上游分镜方向继承、`FAIL-SB-UPSTREAM-DIRECTION-MATRIX` | `../_shared/upstream-context-application-contract.md` | `N2/N3/N4/N5/N6/N7/N9` | `GATE-SB-25` | `N2-SB-UNDERSTAND` / `N4-BEAT-SPLIT` / `N5-COMPOSITION-DESIGN` / `N9-SB-REVIEW-REPAIR` | `Storyboard Direction Inheritance Matrix` maps source/director/aesthetic/memory signals to beat, composition, duration and continuity choices |
...
| `CP-SB-REPORT` | 正式写回时，执行报告包含 `Execution Decision Trace`、`Beat Calculation Audit`、`Scene Axis Dictionary`、`Reference Execution Matrix`、`Upstream Context Application Map`、`Storyboard Direction Inheritance Matrix`、`Rule Evidence Map`、`N/A Justification`、`Repair Log`、`Coverage Stats` 和 `Source Override Note` | 报告证据缺少 upstream context application map、storyboard direction matrix、beat calculation audit、scene axis dictionary、reference matrix、rule map、N/A 或 repair log | `execution_report` | `N9-SB-REVIEW-REPAIR` |
...
| 前中后景、框架、遮挡或纵深是否是可供 7-摄影使用的运动条件，而非“有层次”的空话？ | `GATE-SB-19` | `FAIL-SB-SPATIAL-LAYER` | `N5-COMPOSITION-DESIGN` | `spatial_layer_payload`、`start_frame_spatial_layout` 抽样 |
...
| 上游文稿与 `2-美学` 上下文是否明确投影为分镜拆分、节拍、构图和时值决策，并记录 source anchor、local decision 和 preservation check，而非只写“已读取/已参考”？ | `GATE-SB-24` | `FAIL-SB-UPSTREAM-CONTEXT` | `N1-SB-INTAKE` / `N3-VISUAL-POINTS` / `N5-COMPOSITION-DESIGN` / `N9-SB-REVIEW-REPAIR` | `upstream_context_application_map` |
| 上游 source、导演批注、美学画面基调/分镜风格与项目记忆是否分别说明如何引导分镜方向，并落到节拍、构图、起始状态帧、时值或连续性取舍？ | `GATE-SB-25` | `FAIL-SB-UPSTREAM-DIRECTION-MATRIX` | `N2-SB-UNDERSTAND` / `N4-BEAT-SPLIT` / `N5-COMPOSITION-DESIGN` / `N9-SB-REVIEW-REPAIR` | `storyboard_direction_inheritance_matrix` |
...
- `FAIL-SB-UPSTREAM-DIRECTION-MATRIX`：回到 `N2/N4/N5/N6/N7`，补 `Storyboard Direction Inheritance Matrix`，说明 source / 导演批注 / 美学 / 项目记忆分别如何改变节拍、构图、时值或连续性。
...
| `CHK-SB-SCOPE` | 覆盖既有 `6-分镜/第N集.md`、批量写回、多 source 冲突、启用/复制 references、同步 registry | 形成 scope/diff checkpoint，或引用用户明确授权 | 影响路径、写回策略、不可逆风险说明 | `FAIL-SB-CHECKPOINT-SCOPE` |
...
- Storyboard episode path template: project runtime `6-分镜/第N集.md`.
- Execution report path template: project runtime `6-分镜/执行报告.md`.
...
- 分镜稿：`projects/aigc/<项目名>/6-分镜/第N集.md`
- 执行报告：`projects/aigc/<项目名>/6-分镜/执行报告.md`
...
- 分镜稿路径模板为 `projects/aigc/<项目名>/6-分镜/第N集.md`；执行报告路径模板为 `projects/aigc/<项目名>/6-分镜/执行报告.md`。
- 单集文件沿用上游集号命名 `第N集.md`；批量任务逐集独立写入同一 `6-分镜/` 目录；报告文件固定命名为 `执行报告.md`，必要时在报告内按集号分 section。
...
- 执行报告必须包含 `Storyboard Direction Inheritance Matrix`，证明 `5-导演` 或指定 source、`2-美学/画面基调`、`2-美学/分镜风格`、项目记忆分别如何引导节拍、构图、起始状态帧、空间连续和时值方向。
...
- 只允许在用户授权的项目路径或候选输出范围内写回 `6-分镜` 产物。
- 不修改 `2-美学`、`5-导演` 或 `9-图像` 的 canonical 真源，除非用户另行明确要求进入对应 owning skill。
- 脚本只做机械辅助，不生成核心分镜正文。
...
| `FIELD-SB-08` | 输出落盘 | `6-分镜/第N集.md` 与 `执行报告.md` 可复查 | `FAIL-SB-OUTPUT` |
| `FIELD-SB-09` | 报告证据 | Reference matrix、Upstream Context Application Map、Storyboard Direction Inheritance Matrix、Rule map、N/A、Repair log、Coverage stats 完整 | `FAIL-SB-REPORT` / `FAIL-SB-UPSTREAM-DIRECTION-MATRIX` |
