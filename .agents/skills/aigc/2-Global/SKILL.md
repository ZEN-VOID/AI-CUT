...
6. 本阶段必须把 `组间设计` seed 写入 `projects/<项目名>/3-Detail/第N集.json`，并同步维护 `分镜组列表[].分镜组ID / 总时长 / 剧本正文 / 分镜明细=[]` 的组壳；`剧本正文` 必须完整整理自 `1-Planning/3-分组/第N集.md` 的命中组正文，除组号标题外不得二次摘要。
7. `组间设计.全局风格 / 类型元素 / 导演意图` 的 JSON 文本必须分别直接提取自三份 Markdown 中已确认的同名字段；写入 JSON 时只允许剥离字段标题与空白，不允许现场重写。
8. `组间设计.全局风格 / 类型元素 / 导演意图` 的默认字符窗固定为 `220 / 50 / 100` 个字符以内；其中 `全局风格` 允许为 AIGC 统一风格前缀做适度放宽，但必须先在 Markdown 中确认。
...
| `group_design_distill_engine` | 把三份 Markdown 中已确认字段与完整分组正文提取到 shared episode root 的分镜组壳 | `group_design_seed_plan`、`group_design_seed_patch`、`episode_seed_patch` | 风格/类型/导演意图都稳定且 Markdown 提取字段已确认后强触发 |
...
| `N6-GROUP-DESIGN-DISTILL` | S10 | `FIELD-GLOBAL-08` | 把三份 Markdown 中已确认字段与完整剧本正文直接提取到 shared episode root 的组级壳，并控制长度窗 | 按 `group_design_seed_contract` 对照字段标题，直接提取 `全局风格.md` 的项目级 `全局风格`、`类型元素.md` 的命中组 `类型元素`、`导演意图.md` 的命中组 `导演意图`；同步将 `1-Planning/3-分组/第N集.md` 命中组全文去掉组号标题后完整写入 `分镜组列表[].剧本正文`，并生成 `episode_seed_patch` | `group_design_seed_plan`、`group_design_seed_patch`、`episode_seed_patch` | pass -> `N7`；fail -> 回 `N3A/N3B/N5` | 三个字段必须来自已确认 Markdown，且 `剧本正文` 必须是完整组正文 |
...
1. `projects/<项目名>/2-Global/全局风格.md`
   - 项目级风格底座唯一真源
2. `projects/<项目名>/2-Global/类型元素.md`
   - 按集、按组组织的类型化导演协议唯一真源
3. `projects/<项目名>/2-Global/导演意图.md`
   - 按集、按组沉淀的导演构思唯一真源
4. `projects/<项目名>/3-Detail/第N集.json`
   - shared episode root；本阶段写 `分镜组ID / 总时长 / 剧本正文 / 组间设计 / 分镜明细=[]` 的分镜组壳与相关 metadata
...
4. shared episode root 的 `剧本正文 + 组间设计` 也由当前 skill 聚合写入，但本阶段不得发明 shot-level `分镜明细[]`。
...
| `FIELD-GLOBAL-08` | JSON 提取与组壳写回 | shared episode root 的 `剧本正文 / 全局风格 / 类型元素 / 导演意图` 已按正确字段位置写回，且 `组间设计` 字段来自 Markdown 直接提取并满足 `220 / 50 / 100` 字符窗 | S10 | 提取可消费性 | FAIL-GLOBAL-08 |
...
| `S10` | `FIELD-GLOBAL-08` | Markdown 已确认字段与完整组正文能否被正确提取进 shared episode root，并保持 `220 / 50 / 100` 字符窗 | 生成 `group_design_seed_patch` 与 `episode_seed_patch` | 字段错引、剧本正文被摘要化、跨组混写或超窗 |
...
| `FIELD-GLOBAL-08` | `剧本正文 + 组间设计` 已按正确字段位置写入分镜组壳，且三条组级上下文来自 Markdown 直接提取 | FAIL-GLOBAL-08 | S10 |
