...
1. 先查当前任务对象到底是 `分镜组`、`单一分镜ID` 还是应转交 repo-local `comic` workflow 的 `漫画页`。
...
- 对 `5-Image` 来说，`分镜故事板` 是最宽容的默认入口；`分镜帧` 只在对象信号足够明确时切入；漫画页诉求则应转交 repo-local `comic` workflow，而不是继续挂在 `5-Image` 阶段内。
