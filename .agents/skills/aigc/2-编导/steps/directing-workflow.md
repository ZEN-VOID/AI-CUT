...
| `constraint_profile` | 原文信息量保真、对白冻结、声画配对、slugline 稳定、LLM-first、subagents 监制顾问上下文沉淀 |
| `success_criteria` | 输出能完整承接上游，并可被下游分组/摄影/设计消费 |
| `non_goals` | 不做分镜组切分、不生成图像提示词、不重写剧情 |
| `complexity_source` | 场景解析、字段分流、声画配对、高潮画面识别、监制顾问参谋汇流、保真与质量的优先级协调 |
...
| `N4.6-ADVISOR` | subagents 编导监制参谋汇流 | `team.yaml`、共享顾问合同、上游正文、场景表、字段映射、高潮画面计划、项目 `MEMORY.md` 与相关 `CONTEXT/` | 启动或按阻断报告处理 team.yaml 中明确的监制组相关智能顾问团；要求顾问代入专业视角和个人风格，对结构投影、场景目的、表演任务、声画承托、高潮兑现和保真风险提出参谋指导；主 agent 汇流为后续任务上下文 | `advisor_consultation_packet` 或降级报告 | `N5-DRAFT` | packet 已包含 roster 来源、问题类型、可执行指导、风险提示和 `execution_brief` |
| `N5-DRAFT` | LLM 直出逐集编导稿 | 场景表、字段映射、高潮画面计划、质量规范、`advisor_consultation_packet` | 写入 frontmatter、`【剧本正文】`、场景标题和字段化正文，并吸收顾问参谋上下文但不改写上游真源 | `第N集.md` 草稿 | `N6-REVIEW` | 未使用脚本主创；顾问上下文未越权 |
...
- 若启动 subagents 模式，`N4.6-ADVISOR` 必须在 `N5-DRAFT` 前完成；顾问参谋只转化为 `advisor_consultation_packet` 上下文，不直接写正文，不替换上游事实、对白或事件顺序。
