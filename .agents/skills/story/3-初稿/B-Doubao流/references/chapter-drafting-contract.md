...
| `constraint_profile` | planning 只给义务和约束，不能原样照抄进正文；global/style 摘要必须短而可用；`north_star` 只提炼本章相关摘要；同卷前文是增强输入不是阻塞门；若同卷前文存在，必须形成显式连续性桥。 |
...
16. actual creative step 必须调用 `scripts/write_chapter_via_doubao.py`，并由它继续调用 `doubao_seed_chat.py`。
16a. `scripts/write_chapter_via_doubao.py` 必须把 `north_star.yaml.global_contract/style_contract/genre_contract`、`../../../_shared/genre-trope-quality-filter.md`、命中的 `types/网文/<题材>/` 类型包、项目 `team.yaml` 监制 roster、项目 `MEMORY.md`、项目 `CONTEXT/`、角色声纹、角色关系图谱与同卷前文证据写入 messages pack；若实体全局卡/风格卡目录存在，也必须并入摘录。
...
| --- | --- | --- | --- | --- |
| 是否先锁定当前章 planning、north_star、global/style/genre 真源，再进入 Doubao 正文生成？ | `source_alignment` | `FAIL-DRAFT-REFERENCE` | `references/chapter-drafting-contract.md` + `steps/chapter-drafting-workflow.md` N3 | planning/north_star/card loaded manifest |
| frontmatter 是否只保留 `写作模型: Doubao` 与 `字数: XXX字`，没有资料转储或执行层路径？ | `frontmatter` | `FAIL-DRAFT-REFERENCE` | `references/chapter-drafting-contract.md` Frontmatter Contract + `templates/output-template.md` | parsed frontmatter、extra field list |
| 正文是否完成小说 prose 转换，未复制 planning 标题、任务线或规避条目？ | `prose_quality` | `FAIL-DRAFT-REFERENCE` | `references/chapter-drafting-contract.md` Hard Rules + `templates/doubao-system-prompt.md` | offending excerpt、provider prompt constraints |
| 正文是否保持叙事内视角，没有 `上一章`、`本章`、`provider`、`frontmatter`、`sidecar`、`supervision_packet` 等执行层标签？ | `narrative_perspective` | `FAIL-DRAFT-REFERENCE` | `references/chapter-drafting-contract.md` Hard Rules + `scripts/write_chapter_via_doubao.py` validator | blocked term list、offending excerpt |
| 同卷前文存在时，是否加载当前卷所有早于目标章的前序章并形成连续性桥？ | `continuity` | `FAIL-DRAFT-REFERENCE` | `references/chapter-drafting-contract.md` Continuity Bridge Contract + `steps/chapter-drafting-workflow.md` N3 | previous chapter refs、bridge summary |
| 正式写作是否真实接收 `supervision_packet` 或上层阻断降级报告，并将其进入 Doubao messages？ | `supervision_packet` | `FAIL-DRAFT-REVIEW` | `SKILL.md` Core Gates + `../_shared/supervised-drafting-review-loop-contract.md` | packet path、consultation summary、degradation report |
| 当前章正文、续写、重写或局部修复是否仍由 Doubao provider 主创，而不是 GPT/subagents 或脚本拼接替代？ | `provider_evidence` / `script_boundary` | `FAIL-DRAFT-SCRIPT` | `scripts/write_chapter_via_doubao.py` + `SKILL.md` Actual Creative Engine | messages pack、raw output、provider report |
| 输出是否只写入 `projects/story/<项目名>/3-初稿/第N卷/第N章.md`，并遵守已有章 `--force` 安全门？ | `path_contract` / `overwrite_safety` | `FAIL-DRAFT-ENTRY` | `SKILL.md` Output Contract + `scripts/write_chapter_via_doubao.py` | expected path、actual path、force/mode evidence |
