...
| 项目运行时缺少 `5-设计/角色/2-设计/第N集/`，批量验证无法启动 | runtime precondition layer | 改用显式 `--prompt-file` 或先执行角色 `2-设计` | CLI 对缺失设计目录返回 `FAIL-RP-INPUT`，避免静默生成空面板 | 错误信息指向缺失目录，fixture dry-run 可继续验证脚本 |
...
- 当前 `5-设计/角色` 可能只有逐角色 Markdown 和 `_manifest.json`，不要假设一定存在 `character_design.json`。
