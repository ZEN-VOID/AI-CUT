...
description: Use when `2-Planning` needs the macro whole-book plan that locks the book title, total outline, volume partition, task topology, conflict axis, Save the Cat book-level rhythm, and global avoidances.
governance_tier: full
---

# 2-Planning / 1-部级

`story-plan-book-level` 是 `2-Planning` 的部级子技能，负责把立项输入、类型卡与父层分形规划合同收束为唯一的整书规划真源：`projects/story/<项目名>/2-Planning/整体规划.md`。
...
- 进入正式创作前，必须读取项目输入：`0-Init/north_star.yaml`、`0-Init/init_handoff.yaml` 与 `1-Cards/5-类型卡/**/*.json`。
- 冲突优先级：用户显式请求 > 根 `AGENTS.md` > 父层 `2-Planning/SKILL.md` > 本 `SKILL.md` > 本技能分区文件 > 项目 `MEMORY.md` > 项目 `CONTEXT/` > 本 `CONTEXT.md`。

## Input Contract

- Accepted input: 生成、补写、修订或审查整书级规划；输入可来自用户设定、`0-Init`、类型卡、已有 `整体规划.md`、父层 planning 合同和项目记忆。
- Required input: 项目根、`0-Init/north_star.yaml`、`0-Init/init_handoff.yaml`、至少一个可用类型卡或用户明确给出的类型承诺。
- Optional input: 已存在的 `2-Planning/整体规划.md`、角色/场景/物品卡摘要、用户指定卷数、题材禁区、长期偏好、阶段性 review 结论。
...
| mode | 触发信号 | 主要动作 |
| --- | --- | --- |
| `create_book_plan` | 项目尚无 `2-Planning/整体规划.md` | 从 `0-Init` 与类型卡生成整书规划 |
...
```mermaid
flowchart TD
    A["项目输入<br/>0-Init + 类型卡 + MEMORY"] --> B["部级输入锁定"]
    B --> C["书名与读者承诺"]
    C --> D["整体故事大纲"]
    D --> E["卷划分与卷职责"]
    E --> F["整部任务关系"]
...
1. 锁定项目根和输入真源，确认 `0-Init`、类型卡、项目记忆与父层 planning 合同均已加载。
...
- Required output: 唯一部级规划文件 `projects/story/<项目名>/2-Planning/整体规划.md`；审查模式可额外输出本轮 findings，但不得替代规划真源。
- Output format: Markdown，必须包含 `书名 / 整体故事大纲 / 卷划分 / 整部任务关系 / 整体冲突 / 整体节奏曲线 / 规避`，其中 `整体节奏曲线` 必须包含 Mermaid 图。
- Output path: `projects/story/<项目名>/2-Planning/整体规划.md`。
- Naming convention: 文件名固定为 `整体规划.md`；卷名、任务节点和 Mermaid 节点可使用中文，但任务 ID 或模板要求的机器字段必须保持 ASCII 安全字符。
- Completion gate: 通过 `review/book-level-review-contract.md` 的部级门禁；父层校验时还应可运行 `python3 .agents/skills/story/2-Planning/scripts/validate_planning_outputs.py --help`。
