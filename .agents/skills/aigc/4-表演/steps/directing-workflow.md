...
| `constraint_profile` | 不改写剧情事实、不修改对白、不改变场景顺序和字段标签；只在既有字段中增加表演密度、微表情、身体联动、环境声承托和空间关系；心理反应 GETability、五层表演控制、潜台词行为化、场景状态差、场面调度内嵌、沉默反应、主角内心独白第一人称、动作客观可拍、转场多样性、LLM-first、subagents 只作表演监制顾问上下文 |
...
| `complexity_source` | 心理反应可感知化、五层表演控制（触发点/情绪动机/微表情/身体联动/环境声）、潜台词行为化、场景戏剧映射、场面调度/权力关系、沉默反应余波、主角内心独白人称转换、动作字段客观可拍、转场多样性、表演监制顾问参谋汇流、保真与表演密度的优先级协调 |
...
| `N2-PERF-TYPE` | 形成表演类型路由与关键 beat 标记 | 导演稿字段行、上游正文、`types/type-map.md`、`types/performance-evidence-type-map.md`、`references/psychological-reaction-contract.md`、`references/performance-and-scene-craft-contract.md`、`references/actor-performance-control-contract.md` | 判断当前内容属于心理反应、情绪触发、潜台词行为、场面调度、沉默反应还是主角内心独白；形成 `performance_type_profile`；标记哪些场景需要五层表演控制，并锁定后续证据字段 owner | `performance_type_profile`、`performance_task_map` | `N3-PERF-PSYCHOLOGICAL` | 类型路由不改变上游事实和字段标签；证据字段 owner 不漂移 |
...
| `N7-PERF-DRAFT` | LLM 直出逐集表演稿 | 场景表、字段映射、`psychological_reaction_evidence`、`actor_performance_control_evidence`、`protagonist_inner_voice_evidence`、`objective_action_purity_evidence`、`scene_dramatic_map`、`performance_task_map`、`blocking_power_map`、`integration_targets`、`advisor_consultation_packet`、上游导演稿正文 | 在完整保留上游导演稿结构、字段标签、对白和顺序的前提下，把心理反应可感知化、五层表演控制、潜台词行为化、场景状态差、场面调度权力关系、沉默反应余波和主角内心独白拆入对应句段的既有字段；吸收顾问参谋上下文但不改写上游真源；模板占位和内部规则只能指导写作，不得输出到正文 | `第N集.md` 表演稿草稿、`faithful_performance_trace` | `N8-PERF-REVIEW` | 不改写上游剧情事实、对白、场景标题和字段顺序；表演判断以创作干货内嵌，不以"已执行XX pass"形式泄露；无场景末尾总结块；无第二字段体系；顾问上下文未越权 |
| `N8-PERF-REVIEW` | 心理反应、五层表演控制、潜台词行为化、场景状态差、场面调度、沉默反应、内心独白、动作纯度、顾问汇流、原文保真与质量门禁 | candidate 表演稿、上游导演稿正文、`review/review-contract.md`、各节点证据、`advisor_consultation_packet`、`thinking_action_node_ledger`、`learning_integration_review_evidence` | 运行机械校验或人工 review；逐项执行 `GATE-PERF-01..03`；检查每个关键节点是否具备 `judgment_question / actions_taken / evidence_keys / route_out / gate_status / source_owner`；把 finding 映射到最早责任节点和 source owner | 校验结果、问题清单、`thinking_action_node_ledger`、`learning_integration_review_evidence`、`gate_to_node_repair_map`、repair targets | `N8R-PERF-REPAIR` 或 `N9-PERF-WRITEBACK` | 无阻断项才可写回；质量建议不得掩盖心理反应、五层控制、潜台词、场面调度、沉默、内心独白、动作纯度、顾问汇流或节点退化问题 |
...
| `N9-PERF-WRITEBACK` | 落盘、报告和下游 handoff | 最终表演稿、校验证据、所有 planning evidence、`advisor_consultation_packet`、`thinking_action_node_ledger` | 写入 `4-表演/第N集.md` 和 `执行报告.md`；报告记录 `advisor_consultation_packet` 或降级说明、`thinking_action_node_ledger`、`learning_integration_review_evidence`、`psychological_reaction_evidence`、`actor_performance_control_evidence`、`protagonist_inner_voice_evidence`、`objective_action_purity_evidence`、`scene_dramatic_map`、`performance_task_map`、`blocking_power_map`、`integration_targets`、review/repair/re-review | 文件路径、verdict、handoff status、node ledger status | done | 输出路径、节点 ledger、报告证据和下游准入状态完整 |
