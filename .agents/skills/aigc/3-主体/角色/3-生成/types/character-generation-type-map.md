...
| `reference_context_status` | `pending_libtv_node_reference` / `linked_in_libtv_canvas` / `no_reference_image` | 角色主图节点是否已在同一 libTV 画布中可被多视图节点引用 |
...
- 多视图永远消费对应角色主图节点，不跨角色复用参照；真实生成模式下主图节点必须与多视图节点位于同一 libTV 画布，且 `reference_context_status=linked_in_libtv_canvas`。
