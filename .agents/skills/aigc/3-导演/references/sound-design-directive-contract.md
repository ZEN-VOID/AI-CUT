...
本合同定义 `3-导演` 的声音设计策略。它不是 BGM 推荐清单、音效标签列表或配乐风格指南，而是声音作为叙事工具的导演判断：什么声音在、什么声音不在、声音从谁的视角听到、沉默何时出现、沉默被什么打破、声音母题如何跨集呼应。

声音设计是导演控制观众感知的第二把钥匙——第一把是画面。当画面展示"发生了什么"，声音决定"观众怎么感受它"。一个好的声音设计决策不是"这里配紧张的音乐"，而是"此刻环境声突然消失，只剩下角色自己的呼吸声，让观众被迫进入角色的主观空间"。

本文件不授权新增剧情事实、对白、事件结果、人物动机、摄影方案或具体音效制作参数。它只负责声音设计的叙事策略判断，供 `4-表演` 和后续音频阶段消费。
...
- `../../4-表演/references/actor-performance-control-contract.md` 负责五层表演控制中的 `ambient_support`；本文件的声音策略是其上层约束。
- `../../4-表演/references/performance-and-scene-craft-contract.md` 负责场景中的声音表演变量；本文件不干涉具体的对白表演细节。
...
├── consumed_by: 4-表演 全局 (ambient_support 层的上层约束)
├── produces:
...
**诊断**：`3-导演` 把声音设计等同于配乐选择，没有把声音当成场景世界的一部分。
...
**诊断**：`3-导演` 把沉默当成了"没有声音"的默认状态，没有把它当成需要设计的叙事工具。
...
**诊断**：`3-导演` 没有把声音与 `scene_identity_seed`（场景身份种子）关联，声音脱离了场景的具体身份。
...
**诊断**：`3-导演` 认为声音母题"越多越好"，没有控制母题出现的节奏。
...
**诊断**：`3-导演` 为了"增加主观感"而随意切换声音现实状态，没有追踪切换的叙事触发。
...
**诊断**：`3-导演` 没有把打破沉默的声音限制在场景世界内，用配乐替代了场景声音。
