# Context: aigc 3-主体/场景

## Purpose & Loading Contract

- 本文件是 `aigc-design-scene` 场景域组根的经验层，不是场景设定库，也不是执行流水日志。
- 调用 `.agents/skills/aigc/3-主体/场景/SKILL.md` 时，必须同时加载本文件。
...
| `SCENE-GROUP-TM-07` | 父包或叶子有旧 workflow/steps 载体，执行者不知道哪个是主节点真源 | 以 `SKILL.md` 的 `Thinking-Action Node Map`、`Module Trigger Matrix` 和 `Review Gate Binding` 为当前主真源，旧载体只作兼容参考 | `rg` 能在 SKILL 中命中 runtime-spine 必需控制块 |
...
- runtime-spine 升级优先补主入口控制块和可执行表格；旧语义先迁入或回指主 `SKILL.md`，不做无限整包重写。
