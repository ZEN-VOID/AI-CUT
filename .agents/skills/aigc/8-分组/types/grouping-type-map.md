...
| `source_state` | `complete_camera`、`partial_camera`、`direct_screenplay`、`legacy_grouped`、`broken_markup` | 上游输入状态 |
...
| --- | --- | --- | --- |
| `source_state=complete_camera` | 直接进入 `N2-SOURCE-INVENTORY` 和 `N3-GROUP-BOUNDARY` | 读取 boundary | 标准 review |
| `source_state=partial_camera` | 标记缺时间码、缺场景锚点或部分分镜行不完整的 atomic unit，不替上游补写；只按已有正文裁决分组边界 | boundary contract 保真优先 | 报告上游缺口 |
| `source_state=direct_screenplay` | 不阻断到 `7-摄影`；由 LLM 直接从剧本字段、对白承托、动作落点和物件证据规划约 14.5 秒/组，补写连续时间码分镜行，并让最终累计结束秒以 `.5` 结尾 | direct screenplay intake rule | 报告源路径、时间码为本阶段规划、`.5` 收束、无法做 `7-摄影` 分镜行级 diff |
| `source_state=legacy_grouped` | 仅作为 repair evidence，不当作新真源 | output contract 防平行真源 | 检查是否需迁移到 `8-分组/` |
...
| `duration_signal=bracket_explicit` | 兼容 `[起始秒-结束秒]` 行，以完整画面/声音 atomic unit 累计时长；报告声明不是默认摄影稿格式 | group duration band | 审查兼容格式与 source override 说明 |
| `duration_signal=llm_planned_from_screenplay` | 直接以剧本声画 atomic unit 规划组内连续时间码；每组优先约 14.5 秒、通常 10-11.5 秒、不得超过 14.5 秒，最终累计结束秒以 `.5` 结尾 | direct screenplay intake rule | 审查是否保留剧本事实、英文对白和声画承托，且报告时间码来源与 `.5` 收束 |
...
- 若正文含 `stage: 7-摄影`、`source_camera_path` / `source_camera_path`，且存在 `分镜N（起始秒-结束秒）：` 行，通常为 `complete_camera`。
- 若正文含 `stage: 2-编导`、`source_episode_path`、剧本正文、场景标题、对白、动作、音效、转场等字段，但缺少分镜时间码，且用户要求由 `8-分组` 接手，判为 `direct_screenplay`。
- 若只有部分分镜行存在时间码，或只有兼容 `[起始秒-结束秒]` 行而非摄影稿格式，为 `partial_camera` 或 `specified_source_override`，本阶段不补上游正文。
- 若输入来自 `1-Planning/3-分组/`、`8-分组/` 或含旧式 `【分组正文】`，为 `legacy_grouped`，不得覆盖本技能 canonical 输入。
