...
| FIELD-DETAIL-05 | 业务字段.空间与构图 | 锁定 `角色背景面`、`分镜表现` 与 shot-level 镜头描述子槽，并用结构化密度预算明确主陪背景层级、构图布局、构图方式、`景别节奏预判` 与 `POV 策略预判` | skeleton、导演意图、全局风格、参考锚点、`pace_tier`、故事记忆与情绪引导 | S4 | 空间可读性 | FAIL-DETAIL-05 |
...
| S4 | FIELD-DETAIL-05 | 空间、构图、节拍、景别、镜头属性/框架/类型/视角、主陪背景层级、构图布局/方式与焦点是否成立；quantizer 生成的 `candidate beat segments / recommended_shot_baseline / preferred_shot_count / shot_budget_floor / shot_budget_ceiling` 是否共同支撑当前镜数；当前段该选哪种 `景别节奏模板` 与 `POV 策略`，并如何在命中镜头窗口里验证心理距离 | 锁 `动作阶段点 / 台词气口点 / 焦点切换点 / 结构转折点`，调用 quantizer 生成密度预算，再锁主陪背景关系、构图布局、构图方式、镜头描述子槽、焦点、空间锚点、主体重心，并写出 `景别节奏预判 / POV 策略预判 / 节奏曲线验证 / 心理距离审视 / 反直觉检验` | 画面主重心、层级关系和空间关系模糊，镜数无法解释为何不是更少/更多，或景别/视点只服务信息传递却没有服务情感目标 |
...
| FIELD-DETAIL-05 | 空间可读性 | 构图、节拍、景别、镜头属性/框架/类型/视角、主陪背景层级、构图布局/方式与空间锚点明确，镜数已通过结构化预算校验，且 `景别节奏模板 / POV 策略 / 心理距离` 已完成预判与验证，可支撑后续链路 | FAIL-DETAIL-05 | S4 |
