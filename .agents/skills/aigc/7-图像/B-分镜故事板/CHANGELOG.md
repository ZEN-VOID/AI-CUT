...
- 调整 storyboard panel 落点规则：新增 `storyboard_frame_units`，要求基于 `5-分组` 当前组正文中的视觉节拍识别 panel，而不是把原始 `分镜1`、`分镜2` 机械映射为 storyboard 格子；允许 split/merge，但每个 panel 必须能回指源正文。
...
- 将主信息源固定为 `projects/aigc/<项目名>/5-分组`。
