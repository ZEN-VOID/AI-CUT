...
- outcome: 为 `.agents/skills/aigc/4-Design/角色` 建立了类目父级合同，并把 `1-清单` 固定为首个 active 入口。
...
- evidence_paths:
  ...
  - `.agents/skills/aigc/4-Design/角色/SKILL.md`
- user_feedback_or_constraint: 用户要求以 `skill-subagents + brainstorming + senior-prompt-engineer` 为基础，系统性重构 `4-Design/角色/2-设计`。
...
- evidence_paths:
  ...
  - `.agents/skills/aigc/4-Design/角色/SKILL.md`
  - `.agents/skills/aigc/4-Design/角色/CONTEXT.md`
- user_feedback_or_constraint: 用户本轮要求直接完善 `.agents/skills/aigc/4-Design/角色/3-面板`，并参照前代 `角色面板` 技能。

### Case-20260412-AIGC-DESIGN-ROLE-ZXY-REPACK

- milestone_type: source_contract_change
- outcome: 将 `2-角色` 父级与 `1-清单 / 2-设计 / 3-面板` 子技能统一重排为知行合一口径，并明确 `复杂链路的骨架 / 细则分层=false`。
- root_cause_or_design_decision: 现有角色链内容已经具备，但 `1-清单` 仍把字段/步骤拆到 `references/`，`2-设计` 与 `3-面板` 也保留并列细则载体，导致主合同不是真正唯一思行真源。
- final_fix_or_heuristic: 保持业务路径、共享 I/O、模板、script 与 subagent/team 机制不变，只把业务分析、拓扑、思行节点、汇流门、类型策略与一次性输出合同收回主 `SKILL.md`；旧 `references/` 统一降级为迁移 stub。
- prevention_or_replication_checklist:
  - [x] 父级合同已写明 `复杂链路的骨架 / 细则分层=false`
  - [x] `1-清单 / 2-设计 / 3-面板` 主合同已内收关键细则
  - [x] 旧 `references/` 已降级为非规范 stub
  - [x] 经验层已记录本次真源治理策略
- evidence_paths:
  - `.agents/skills/aigc/4-Design/角色/SKILL.md`
  ...
  - `.agents/skills/aigc/4-Design/角色/2-设计/SKILL.md`
  - `.agents/skills/aigc/4-Design/角色/3-面板/SKILL.md`
- user_feedback_or_constraint: 用户明确要求“内容和机制上全量参照现有配置，但根据知行合一的规范进行编排”，并指定 `复杂链路的骨架 / 细则分层=false`。
