...
<context>
本文件只定义 `resume/` 的恢复原则与安全边界，不替代 `3-初稿`、`4-润色`、`return` 自身的 stage 合同。
</context>
...
2. `4-润色/第N卷/第N章.acceptance.json`
3. `3-初稿/第N卷/第N章.acceptance.json`
...
- 命中 `4-润色` acceptance PASS + return handoff，但无 上下文回流：
  - 下一稳定入口：`story-return`
- 命中 `3-初稿` acceptance PASS + polish handoff，但无润色：
  - 下一稳定入口：`4-润色`
- 命中 `candidate_volume_draft` 写作日志，但无 acceptance 包：
  - 下一稳定入口：`3-初稿` 内置验收或重跑当前章
- 命中 `acceptance FAIL`：
  - 按 `rework_targets` 回 owning stage 或 source contract owner

禁止：

- 把 artifact fallback 当成 tracked interruption 伪装输出
- 在已存在 `acceptance / 上下文回流` 业务证据时仍简单打印“无中断任务”
...
- 风险：`low` 到 `medium`
- 适用：`Step 4 / Step 5 / Step 6 / Step 7`，或 stage acceptance 中后段
- 动作：
  - 核对输入文件仍存在且未被外部改坏
...
- ❌ 自动替用户选择恢复策略
- ❌ 未备份即删除正文
- ❌ 把 `resume/` 冒充 `return` actualization 主流程
...
<example>
<input>workflow detect 显示无中断，但项目里已有 `4-润色/第1卷/第1章.acceptance.json`，还没有 `context-return/第1卷.context-return.json`</input>
<output>
说明：
- 当前没有 tracked interruption
- 但 artifact fallback 已命中：`acceptance PASS + return handoff + 上下文回流 pending`

建议：
A) 进入 `story-return` 执行第1章 actualization（推荐）
B) 先人工核对 acceptance packet 与 accepted manuscript refs，再执行 actualization
</output>
</example>
