...
```text
projects/aigc/<项目名>/8-视频/C-主体参照/第N集/groups/<分镜组ID>/libtv-submit-plan.json
```
...
```json
{
  "project_name": "诡校-测试版",
  "episode_id": "第1集",
  "package_dir": "projects/aigc/诡校-测试版/8-视频/C-主体参照/第1集/groups/1-1-1",
  "parallelism": 3,
  "libtv_self_check": {
  ...
  "tasks": [
    {
      ...
      "prompt_path": "projects/aigc/诡校-测试版/8-视频/C-主体参照/第1集/groups/1-1-1/prompt.md",
      "remote_submission_path": "projects/aigc/诡校-测试版/8-视频/C-主体参照/第1集/groups/1-1-1/libtv-submission.txt",
      ...
      "images": [
        {
          ...
          "name": "林寂",
          "category": "character",
          "path": "projects/aigc/诡校-测试版/6-设计/角色/3-生成/林寂-多视图.png",
      ...
      "projectUrl": "",
      "canvasMarkdown": "",
      "download_dir": "projects/aigc/诡校-测试版/8-视频/C-主体参照/第1集/groups/1-1-1",
      "expected_output": "projects/aigc/诡校-测试版/8-视频/C-主体参照/第1集/groups/1-1-1/1-1-1.mp4"
    }
  ]
}
```
...
python3 .agents/skills/cli/libTV/scripts/upload_file.py "projects/aigc/<项目名>/6-设计/角色/3-生成/林寂-多视图.png"
python3 .agents/skills/cli/libTV/scripts/upload_file.py "projects/aigc/<项目名>/6-设计/场景/3-生成/永夜私立中学二年级A班教室-多视图.png"
python3 .agents/skills/cli/libTV/scripts/create_session.py "$(cat projects/aigc/<项目名>/8-视频/C-主体参照/第N集/groups/1-1-1/libtv-submission.txt)"
python3 .agents/skills/cli/libTV/scripts/query_session.py "<sessionId>" --project-id "<projectUuid>"
python3 .agents/skills/cli/libTV/scripts/download_results.py "<sessionId>" --output-dir "projects/aigc/<项目名>/8-视频/C-主体参照/第N集/groups/1-1-1" --filename "1-1-1.mp4"
ffprobe -v error -select_streams a -show_entries stream=index,codec_type,codec_name -of json "projects/aigc/<项目名>/8-视频/C-主体参照/第N集/groups/1-1-1/1-1-1.mp4"
```
...
```bash
python3 .agents/skills/cli/libTV/scripts/query_session.py "<sessionId>" --project-id "<projectUuid>"
python3 .agents/skills/aigc/8-视频-backup/C-主体参照/scripts/detect-libtv-stall.py "<query-json>"
python3 .agents/skills/aigc/8-视频-backup/C-主体参照/scripts/validate-post-submit-reference-order.py "<package_dir>" "<query-json>"
```
...
| --- | --- | --- | --- | --- |
| 提交或查询前是否加载并遵守 `.agents/skills/cli/libTV/SKILL.md`，且未绕过官方 skill 私接 OpenAPI 或写出密钥？ | `G10-SELF-CHECK` | `FAIL-VIDSUBJ-LIBTV` | `N7-LIBTV-CHECK` | LibTV self-check 记录、命令来源、`LIBTV_ACCESS_KEY` 不落盘扫描 |
| 有主体参照图时是否按 `change_project -> upload_file -> create_session -> query_session -> download_results -> ffprobe` 顺序执行或规划？ | `G7-PROVIDER-ROUTE` / `G18-AUDIO-ACCEPTANCE` | `FAIL-VIDSUBJ-LIBTV` / `FAIL-VIDSUBJ-AUDIO-MISSING` | `N7-LIBTV-CHECK` / `N8-DISPATCH` / `N9-QUEUE` | `libtv-results.json.attempts[]`、submit plan 命令序列、下载和 ffprobe 证据 |
| 每个任务的 `projectUuid/projectUrl/canvas_link` 是否在 submit plan、queue 和 report 中一致，且 Markdown 可点击？ | `G11-QUEUE` / `G13-REPORT` | `FAIL-VIDSUBJ-LIBTV` / `FAIL-VIDSUBJ-REPORT` | `N9-QUEUE` / `N11-CLOSE` | `queue.md`、`projectUrl`、`canvasMarkdown`、执行报告链接 |
| 所有 uploaded URL 的 `/claw/<projectUuid>/` 是否与当前 submit plan / queue / session 画布一致，且没有先上传再切换画布？ | `G15-SAME-CANVAS-REUSE` | `FAIL-VIDSUBJ-REFERENCE-PROJECT-SCOPE` | `N5-REF-BIND` / `N8-DISPATCH` | URL scope 检查、`asset_uploads`、`projectUuid`、跨画布阻断记录 |
| 命令选择是否匹配参照状态：bound 或 visual resolved 用 mixed2video，有 no_images 用 text2video，ambiguous / auth_or_script_failed 阻断？ | `G7-PROVIDER-ROUTE` | `FAIL-VIDSUBJ-LIBTV` | `N6-REVIEW` / `N8-DISPATCH` | submit plan `command`、reference state、blocked reason |
| 远端 handoff 是否默认 `strict_original + transport_only`，并在未 opt-in 时阻断 LibTV 优化、摘要、重排、台词/音效改写或主体绑定改变？ | `G2-CONTENT` / `G14-POST-SUBMIT` | `FAIL-VIDSUBJ-PROMPT` / `FAIL-VIDSUBJ-LIBTV-STALL` | `N4-PROMPT` / `N9-QUEUE` | `prompt_fidelity_mode`、`allow_libtv_prompt_optimization`、query drift finding |
| `slot_binding_phase=draft` 是否不创建最终远端提交；`final` 是否已用 `generation_slots` 回刷 YAML、submit plan、submission 和 mixedList？ | `G16-REF-PROMPT-INTEGRITY` | `FAIL-VIDSUBJ-REF-PROMPT-INTEGRITY` | `N8-DISPATCH` | phase 字段、`generation_slots`、`build-upload-ledger.py --sync` 或等价同步证据 |
| 有图时远端 opening 是否明确 `provider=seedance2.0 / taskType=video / modeType=mixed2video / mixedList=[{url,type:image}]`，且 mixedList 为真实 URL 严格 JSON、<=9 张？ | `G7-PROVIDER-ROUTE` / `G6-REFERENCE-BUDGET` | `FAIL-VIDSUBJ-LIBTV` / `FAIL-VIDSUBJ-REF-BUDGET` | `N8-DISPATCH` | `libtv-submission.txt` opening、`mixedList` JSON 解析、图片预算记录 |
| final 相位的 YAML、`images[]`、`mixedList`、`generation_slots` 是否同槽一致，未声明共享时无重复 URL，且缺图/未入预算主体未进入远端文本？ | `G16-REF-PROMPT-INTEGRITY` | `FAIL-VIDSUBJ-REF-PROMPT-INTEGRITY` / `FAIL-VIDSUBJ-REFERENCE-SLOT-REGISTRY` | `N8-DISPATCH` | 引用一致性 gate、缺图列表扫描、重复 URL 检查 |
| UI 图N / Portrait N 或 post-submit `mixedList` 顺序是否被用作最终槽位证据；实际顺序错位时是否反查 URL 回刷 YAML 后重提？ | `G19-REMOTE-REFERENCE-ORDER` | `FAIL-VIDSUBJ-REMOTE-REFERENCE-ORDER` / `FAIL-VIDSUBJ-PROMPT-REFERENCE-BINDING` | `N8-DISPATCH` / `N9-QUEUE` | post-submit reference-order gate、UI 槽位证据、回刷和 rerun 记录 |
| 远端提交是否无本地路径、无人工 `参照图N` 映射、无裸图片 token/编号/URL 列表，主体名与 reference_index / token / URL 邻近绑定？ | `G5-LOCAL-ASSET-EVIDENCE` / `G9-REMOTE-NUMBERING` | `FAIL-VIDSUBJ-PROMPT` / `FAIL-VIDSUBJ-PROMPT-REFERENCE-BINDING` | `N4-PROMPT` / `N8-DISPATCH` | submission 禁词扫描、query prompt 邻近绑定检查 |
| post-submit gate 是否执行，并把 `ask_user`、明确 tool error、无生成节点、未授权优化、主体名绑定丢失、顺序错位分别写成非 pending 状态？ | `G14-POST-SUBMIT` / `G19-REMOTE-REFERENCE-ORDER` | `FAIL-VIDSUBJ-LIBTV-STALL` / `FAIL-VIDSUBJ-REMOTE-REFERENCE-ORDER` | `N9-QUEUE` | `detect-libtv-stall.py`、`validate-post-submit-reference-order.py`、`libtv-results.json.attempts[]` |
| 平台审核拦截是否触发安全重提，而不是改写 `5-分组` 或把空画布视为完成？ | `G14-POST-SUBMIT` | `FAIL-VIDSUBJ-LIBTV-STALL` | `N9-QUEUE` / `N8-DISPATCH` | `platform_policy_rejected`、`moderation_repair`、安全版提交路径和结果 |
| 远端提交是否请求 `enableSound:on`，不可预检时仅记录风险；生成后是否用 `task_result.audios`、音频 URL 或 ffprobe 做音频验收？ | `G17-AUDIO-PREFLIGHT` / `G18-AUDIO-ACCEPTANCE` | `WARN-VIDSUBJ-AUDIO-PREFLIGHT-UNVERIFIED` / `FAIL-VIDSUBJ-AUDIO-MISSING` | `N8-DISPATCH` / `N9-QUEUE` | audio preflight 字段、query 音频证据、ffprobe JSON |
| 队列和持久化是否按 group package 原子写入，compact 模式不在组根平铺 latest/corrected/followup/post-submit 临时文件？ | `G11-QUEUE` / `G12-PERSIST` | `FAIL-VIDSUBJ-LIBTV` | `N9-QUEUE` / `N10-WRITE` | group package 文件清单、`queue.md`、`libtv-results.json.attempts[]` |
| 并发提交是否只写各自 group package，集级 summary 和最终报告是否在汇流阶段串行重建？ | `G11-QUEUE` / `G13-REPORT` | `FAIL-VIDSUBJ-LIBTV` / `FAIL-VIDSUBJ-REPORT` | `N9-QUEUE` / `N11-CLOSE` | per-group queue/result、集级 summary 回指、最终执行报告 |
