...
| preserve_scope | 身份、构图、姿态、服装、背景等保留项清楚；涉及角色形象保留时必须明确原角色形象和妆容不变；涉及服装保留或替换时必须明确服装样式和版型 |
...
| --- | --- | --- |
| `多视图` | 场景、道具、服装、角色 | 模板路径正确；ID/name/desc 与短 ASCII ID 注入；顶左身份牌显示短 ID 或保留叠字区；完整名称进入 prompt plan / JSON 记录；layout grammar 明确；主体不变量与跨视图身份/形态一致；角色多视图必须保留原角色形象和妆容不变；服装多视图必须锁定服装样式和版型；对应空场景/无人/无强角色护栏成立；场景模板额外要求每个 panel 左下角有视角标签或叠字预留条 |
