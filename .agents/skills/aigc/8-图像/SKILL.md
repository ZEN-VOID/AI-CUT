...
`8-图像` 是 AIGC 项目的图像阶段父级入口。它负责把来自 `6-分组` 与 `7-设计` 的信息路由到叶子技能，不直接主创 prompt 正文，也不直接替代 `.agents/skills/cli/imagegen` 生成图像。

`6-分组` 中的 `## x-y-z~x-y-z` 组间连接件在 `8-图像` A/B 叶子路线中默认全部忽略：不进入镜级 prompt、组级 storyboard prompt、参照 manifest、imagegen plan 或生成图片。未来连接件视频由单独手动视频连接 skill 处理，父级不得默认调度。
...
- 用户命中 `8-图像`、分镜画面、生图提示词、AIGC 生图、分镜故事板或图像阶段批量生成。
- 来自 `projects/aigc/<项目名>/6-分组/` 的分镜组稿。
- 来自 `projects/aigc/<项目名>/7-设计/*/3-生成` 的主体生成资产。
...
- 任务实际是视频首帧、视频参照或运动提示词，应转入 `9-视频` 或对应视频技能。
...
```mermaid
flowchart TD
    A["8-图像 request"] --> B{"target"}
    B -->|"single frame / 分镜ID"| C["A-分镜画面"]
    B -->|"storyboard sheet / 分镜组"| D["B-分镜故事板"]
    C --> E["projects/aigc/<项目名>/8-图像/A-分镜画面"]
    D --> F["projects/aigc/<项目名>/8-图像/B-分镜故事板"]
```
...
4. 父级不直接写镜级 prompt、不直接生成图片、不改写 `6-分组`。
...
- Output path: 父级不直接落业务产物；`A-分镜画面` 叶子技能固定写入 `projects/aigc/<项目名>/8-图像/A-分镜画面`。
