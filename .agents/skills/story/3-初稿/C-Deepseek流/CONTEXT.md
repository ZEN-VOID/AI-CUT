...
| DeepSeek provider 返回 markdown 但缺 YAML 头 | provider output validation | 阻断写回，调整 prompt 或增大 `max_tokens` 后重试 | 写回前固定校验 `写作模型: Deepseek` | 非法输出不落到 `第N章.md` |
...
6. 若 YAML 头缺字段，只检查并补正 `写作模型: Deepseek`；上下文缺口回到 context pack / sidecar，而不是补进正文头。
