...
8. 角色多视图、换装、换角色、换脸等涉及保留角色形象不变的任务，必须明确写入“保留原角色形象和妆容不变”，不能只写身份不变。
9. 服装多视图、换装等涉及服装保留或替换的任务，必须明确写入“服装样式和版型”，不能只写服装或衣服不变。
10. 提示词正文由 LLM 直接创作；脚本只允许保存、校验或转发 prompt plan。
...
| --- | --- |
| `多视图` | 固定对象类型、设计页 layout、身份徽章、跨面板一致性和非目标护栏；角色多视图必须保留原角色形象和妆容不变；服装多视图必须锁定服装样式和版型 |
