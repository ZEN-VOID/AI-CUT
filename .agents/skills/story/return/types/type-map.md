...
`return` has lightweight type handling. It classifies the request and aggregate before loading the workflow branch.
...
| --- | --- | --- | --- |
| PASS + handoff to `review/` and `return` + `accepted_manuscript_stage=4-润色` | `actualize_volume` | enter `steps/context-return-workflow.md` at `N2-GATE` | full completion checklist |
| PASS + handoff to `review/` and `return` + explicit skip-polish acceptance | `actualize_volume` | enter with `accepted_manuscript_type=draft_explicitly_accepted` | note exception in artifact |
