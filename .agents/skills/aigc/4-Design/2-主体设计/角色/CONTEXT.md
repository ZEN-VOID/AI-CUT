...
- 本文件是 `.agents/skills/aigc/4-Design/2-主体设计/角色` 的经验层知识库，不是过程日志。
...
| --- | --- | --- | --- | --- |
| `2-设计` 跳过 `1-清单` 直接发明角色设计 | 输入锚点层 | 强制回到 `角色清单.json` 锁角色 canonical identity | 在 `SKILL.md`、`1-主体清单/角色` 合同与 `_shared/IO_CONTRACT.md` 固化“先清单、再设计” | 不再出现无对象池设计 |
| 四个 specialist 各写一整份角色稿，父 skill 无法收束 | 编排边界层 | 收回写回权到父 skill，只允许返回 `agents_plan + patch / note / report` | 在主 `SKILL.md` 与 `_shared/IO_CONTRACT.md` 固化 agents-plan-aware handoff | canonical 输出只由父 skill 写回 |
...
| 主合同仍强依赖不存在的 `.codex/agents/aigc/设计组/角色设计/*` | source-layer 装配层 | 把外置 team 依赖改为本技能内的能力镜面真源 | 让 `SKILL.md`、`CONTEXT.md` 与 `_shared/IO_CONTRACT.md` 只回链真实存在的文件 | preload 与 rule source 不再断链 |
| `2-设计` 仍把 `2-Global/全局风格.md`、`导演意图.md` 当第一输入，但 runtime 已改为目录化风格文件 + detail 组级导演意图 | 输入真源层 | 改读 `2-Global/全局风格/全局风格设计.md`，并从 `3-Detail/第N集.json` 的 `组间设计.导演意图` 补导演指导 | 在 `SKILL.md` 与 `_shared/IO_CONTRACT.md` 固化真实 runtime 路径，不再虚构平铺投影 | preload 路径与项目真实文件一致 |
...
- 若 source-layer 目录已经改成 `4-Design/2-主体设计/角色`，则 preload、root-cause trace 和 `_shared` 回链也必须同步切换到该真实路径，不能继续把旧 `4-Design/角色/2-设计` 当作文档真源。
- 若 `2-Global` 只有目录化风格文件而没有平铺 `导演意图.md`，角色设计链应把风格真源固定到 `全局风格/全局风格设计.md`，把导演人物指导回收到 `3-Detail` 命中分组的 `组间设计.导演意图`。
