...
- 当前技能是 `story2026` 主链 `3-Drafting` 阶段的 `A-GPT原生` 路径，不再通过 `正文/` child 承接。
- 当 `3-Drafting` 路由选择 `A-GPT原生` 时，当前技能拥有当前章正文根文件与 YAML 头的写权。
...
| `topology_fit` | `source lock -> context pack -> frontmatter synthesis -> continuity bridge -> GPT-native chapter generation -> final writeback` |
...
13. `scripts/write_chapter_gpt_native.py` 只能装配上下文、输出 sidecar、校验已创作稿与写回，不得直接生成正文。
14. 若 GPT 原生输出不含完整 YAML frontmatter、缺少必需字段、`写作模型` 不等于 `GPT`、缺少 `# 第N章｜章标题` 标题行，必须判定为 output invalid，禁止直接写回业务真源。
