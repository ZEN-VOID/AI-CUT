...
| --- | --- | --- | --- |
| `business_goal` | 将图像阶段请求路由到唯一叶子技能：分镜画面或分镜故事板 | 用户请求、项目阶段产物、Mode Selection | `FAIL-IMG-STAGE-BUSINESS-GOAL` |
...
| `complexity_source` | 复杂度来自分镜画面/故事板分流、旧版/新版分组输入适配、主体图绑定和 provider 返回形态差异 | Type Map、Mode Selection、Pass Table | `FAIL-IMG-STAGE-COMPLEXITY` |
| `topology_fit` | 父级路由适配业务：1) 保持叶子主创边界；2) 避免分镜画面与故事板混淆；3) 在 provider 返回 contact sheet 时能阻断错误完成 | Visual Maps、Execution Contract、Pass Table | `FAIL-IMG-STAGE-TOPOLOGY-FIT` |
...
- 用户命中 `12-图像`、分镜画面、生图提示词、AIGC 生图、分镜故事板或图像阶段批量生成。
- 来自 `projects/aigc/<项目名>/10-分组/` 的分镜组稿。
- 来自旧版 `projects/aigc/<项目名>/5-分组/` 的分组稿可作为测试、迁移或显式 legacy adapter 输入，但不得静默伪装为新版 `10-分组` canonical 真源。
- 来自 `projects/aigc/<项目名>/11-主体/*/3-生成` 的主体生成资产。
...
- 任务实际是视频首帧、视频参照或运动提示词，应转入 `13-画布` 或对应视频技能。
...
| --- | --- | --- | --- | --- | --- | --- |
| `N1-INTAKE` | 锁定项目、任务类型、比例和目标范围 | 用户请求、项目根 | 加载父级 `SKILL.md + CONTEXT.md`；识别 frame/storyboard/repair；未显式指定时设置 `aspect_ratio=16:9` | route note、aspect_ratio note | `N2` / `N4` | 项目或目标无法唯一判断时不得继续 |
| `N2-LEAF-SELECT` | 选择唯一叶子技能 | `N1` route note | 按 Type Routing Matrix 选择 `分镜画面` 或 `分镜故事板` | selected_leaf | `N3` | 叶子必须唯一，不能同时把分镜画面当故事板 |
...
4. 父级不直接写镜级 prompt、不直接生成图片、不改写 `10-分组`。
5. 未显式指定比例时向叶子传递 `aspect_ratio=16:9`；显式指定时传递 `aspect_ratio_override`。
6. 叶子或 provider 返回图像后，父级审查返回形态：分镜画面路线必须是多张单独文件；一张 contact sheet/grid/storyboard 即使包含全部画面，也必须按失败处理。
...
| `FIELD-IMG-STAGE-05` | output shape | 分镜画面路线返回多张单独图片；故事板路线才允许单张多格板 | `FAIL-IMG-STAGE-MULTI-IMAGE-SHAPE` |
