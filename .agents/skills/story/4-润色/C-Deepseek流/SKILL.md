...
description: "Use when polishing or locally repairing an existing 3-初稿 story chapter through DeepSeek provider flow with canonical 4-润色 writeback."
...
`C-Deepseek流` 是 `4-润色` 阶段的默认 DeepSeek provider 路径。它通过 `.agents/skills/api/deepseek` 固定调用 `deepseek-v4-pro`，对 `3-初稿` 正文做高推理最小局部修补，并写回 `4-润色` canonical path。

## Input Contract

- Accepted input: 用户要求默认润色、DeepSeek 润色、局部修复、重润，或基于 `story/review` findings 生成 DeepSeek repair brief 后直接优化已有 `3-初稿` 章节。
- Required input: `projects/story/<项目名>/`、卷号、章号、当前章 `3-初稿/第N卷/第N章.md`、上游 planning、`north_star.yaml`、同目录 `CONTEXT.md`、命中的 `types/` 类型包、DeepSeek provider 可用环境。
- Optional input: 已有 `4-润色` 目标章、上一章正文、项目 `MEMORY.md`、项目 `CONTEXT/`、supervision packet、review subagent packets、用户指定局部修复点。
- Reject or clarify when: 源章缺失、项目根或卷章不唯一、用户未授权覆盖既有目标章、DeepSeek provider 不可用且用户未允许阻断报告，或要求静默回退到本地 GPT 直写。
...
| `polish_rewrite` | `4-润色` 目标章已存在，用户明确要求重润/覆盖/整章重写 | 回读初稿与既有润色稿后重润；正式覆盖需 `--force` |
| `local_repair` | 用户或审查指出局部语言/质感/AI 检测规整化问题 | 只修复指定问题，不扩大改写 |
| `subagent_review_optimize` | 用户显式要求启用 subagents、按审计点并行审查并直接优化 | 按 `../SKILL.md` 的 `Subagent Review-Optimize Contract` 调度 `story/review` 维度子技能，形成 repair brief 后仍由 DeepSeek provider 执行最小优化 |
...
3. DeepSeek provider、最小修补或 provider evidence 失败：回到 `Base Polishing Rules`、`steps/chapter-polishing-workflow.md` 与 `N5/N6`。
...
| --- | --- | --- | --- |
| `FIELD-DSP-01` | `SKILL.md` | 入口、Input Contract、连续调度、动态引用、Root-Cause、Output Contract、Runtime Guardrails | `FAIL-DSD-SOURCE` |
| `FIELD-DSP-02` | `CONTEXT.md` | Type Map、Repair Playbook、Reusable Heuristics | `FAIL-DSD-CONTEXT` |
| `FIELD-DSP-03` | `references/` | 章节润色强规则、provider 边界与 Review Gate Mapping | `FAIL-DSD-PROMPT` |
| `FIELD-DSP-04` | `steps/` | 思行节点、分支、汇流、失败回路 | `FAIL-DSD-CONTINUITY` |
| `FIELD-DSP-05` | `types/` | 类型包索引、guardrail setup、润色模式判定 | `FAIL-DSD-TYPE` |
| `FIELD-DSP-06` | `review/` | 质量门禁、provider/reviewer 降级、convergence 标准 | `FAIL-DSD-REVIEW-HANDOFF` |
| `FIELD-DSP-07` | `templates/` | 输出骨架与 Output Contract Alignment | `FAIL-DSD-WRITEBACK` |
| `FIELD-DSP-08` | `scripts/` | provider bridge、context pack、校验、写回等机械辅助 | `FAIL-DSD-PROVIDER` |
| `FIELD-DSP-09` | `knowledge-base/` | 可复用润色经验与按需检索材料 | `FAIL-DSD-CONTEXT` |
| `FIELD-DSP-10` | `agents/openai.yaml` | 产品侧 display_name、short_description、default_prompt | `FAIL-DSD-SOURCE` |
| `FIELD-DSP-11` | `guardrails/` | Forbidden Actions、Permission Boundaries、Anti-Injection Rules、Violation Response | `FAIL-DSD-WRITEBACK` |
...
- MUST NOT 通过脚本拼接、模板灌字或启发式补句替代 DeepSeek provider 主创正文。
- MUST NOT 在 provider 失败时静默回退到本地 GPT 直写并冒充 DeepSeek 产物。
...
- Output format: YAML frontmatter、空行、`# 第N章｜章标题`、章节润色稿；frontmatter 至少包含 `润色模型: Deepseek`、`初稿来源` 与 `字数`。
- Output path: `projects/story/<项目名>/4-润色/第N卷/第N章.md`。
- Naming convention: 卷目录 `第N卷`，章节文件 `第N章.md`。
- Completion gate: `3-初稿` 源章已读取；DeepSeek provider 真实命中；显式 subagents 模式下已按 `story/review` 维度子技能完成审计并把 findings 注入 DeepSeek repair brief 直接优化正文；输出通过 frontmatter、heading、最小修补、中文表达、题材质感与 guardrails 门禁；正式正文已写回 canonical path。
