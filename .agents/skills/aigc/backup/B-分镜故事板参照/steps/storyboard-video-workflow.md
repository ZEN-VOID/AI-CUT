...
| `N3-GROUP-INDEX` | 从 `5-分组` 建立组级索引 | `第N集.md` | 解析 `## x-y-z`、完整组正文、分镜数量和 `时长估算` | `group-index.json` | `N4` | 每个 ID 唯一可回指，时长估算可追溯 |
| `N4-REF-BIND` | 保守绑定对应故事板图 | group index、`7-图像/B-分镜故事板` | 按 `group_id` 查真实图片；无图置空 | reference manifest | `N5` | 无猜测路径 |
