...
description: Use when the `4-Design/3-面板` tranche needs to route stable design outputs into panel layout JSONs and SMART built-in imagegen generation under `projects/aigc/<项目名>/4-Design/<域>/3-面板/`.
...
`3-面板` 是 `4-Design` 的展示与派生生图 tranche。它只消费 `2-设计` 已稳定的设计产物，把其中可直接下游使用的 `full_generation_prompt / prompt整合` 收束为面板 layout JSON，并在默认路径下桥接内置 `$imagegen` / `image_gen` 生图，默认模型口径为 `GPT-IMAGE-2`。
...
| 载体 | 位置 | 作用 |
| --- | --- | --- |
| SMART bridge | `.agents/skills/aigc/4-Design/3-面板/_shared/panel_auto_generate.py` | layout JSON -> built-in imagegen request |
