...
本文件承载 `story-polishing-doubao` 的章节修补稿细则。它不拥有入口路由权；调用时由根 `SKILL.md` 决定何时加载。

## Stage Position

- 当前技能是 `story2026` 主链 `4-润色` 阶段的 `B-Doubao流` provider 路径，不再通过 `正文/` child 承接。
- 当 `4-润色` 路由选择 `B-Doubao流` 时，当前技能拥有当前章修补稿根文件与 YAML 头的写权。
...
- `review_subagent_packets`：用户显式要求 subagents 模式时必须按 `.agents/skills/story/review` 维度子技能生成，并转成 Doubao provider 可执行的 repair brief。
...
| `topology_fit` | `source lock -> context pack -> minimal frontmatter marker -> continuity bridge -> doubao minimal local repair generation -> final writeback` |
...
4. YAML 头只保留 `润色模型: Doubao`、`初稿来源` 与 `字数`；`字数` 按正文去除 frontmatter 与章节标题后的非空白字符数统计，格式为 `XXX字`；禁止重复写入 planning 路径、cards 路径、项目上下文路径或 global/style/north-star 摘要。
...
9. 若上一章存在，provider 上下文包与 sidecar 必须记录实际加载路径；正文 YAML 不再重复承载 `previous_chapter_ref`。
