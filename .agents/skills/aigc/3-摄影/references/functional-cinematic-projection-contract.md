...
本文件定义 `3-摄影` 的功能性底线：`分镜明细` 不是好看的镜头句子，也不是随机摄影术语拼接；它必须把上游画面句子转化为下游 `4-分组`、`6-图像`、`7-视频` 可消费的影视事实，尤其要给出可执行的运镜策略。

## Core Rule

每个 `分镜N` 必须先回答“这一镜在影视上解决什么问题”，再回答“摄影机为什么这样动、怎么动、何时停、如何交接”，最后回答“它如何被图像/视频工具执行”。若一条分镜删掉后不影响主体、动作、信息、空间、情绪、节奏、连续性、运镜策略或下游生成锚点，它就是伪分镜，必须删除或重写。

## Rule Stack Projection

`3-摄影` 的种种细则不是可选装饰，而是同一条分镜的决策栈。每个 `分镜N` 必须能回指下列至少一组有效来源：
...
| `transition-design-contract.md` | 若发生场景变化或其他转场动机，如何处理交出点、进入点和转场强度 |
...
- **视频可动性**：能生成连续镜头。必须让下游知道镜头如何开始、如何移动或保持、何时停、焦点转到哪里、如何接下一镜。
- **分组可切性**：能让 `4-分组` 判断该镜是否和前后画面同组；不得把空间、主体和动作写得漂浮。
- **连续性可追踪**：能让 `6-图像` 和 `7-视频` 继承上一镜的主体、空间轴线、光色母题和注意力落点。
- **转场可执行性**：场景变化、空间重置或高能转场必须让下游知道上一画面从哪个声音、动作、光色、文字、空间出口或反应交出，下一画面从哪里进入。
...
- 摄影术语与当前剧情功能无关，例如普通对话硬加复杂环绕、低信息动作硬加高能转场。
- 场景变化没有交出点和进入点，或只写“转场到下一场”但下游无法判断画面如何衔接。
- 下游图像阶段无法判断该画面该画谁、站哪里、看什么、光从哪里来。
- 下游视频阶段无法判断摄影机怎么动、为什么这样动、何时停、焦点转到哪里、如何接下一镜。
...
2. 抽取 `visible_subject / action_phase / camera_movement_plan / composition_anchor / light_color_material / continuity_handoff`。
3. 按 Rule Stack Projection 回查节拍、节奏、峰值、连续性、技法库和动态运镜细则，重新选择最小充分的摄影与运镜策略。
