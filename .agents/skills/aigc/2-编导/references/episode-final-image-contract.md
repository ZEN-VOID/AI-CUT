...
本细则定义 `2-编导` 的每集终结画面：它是剧本层面的迷你彩蛋尾钩，用最后一个可见/可听/可感受的落点吸引观众继续追更下一集。
...
每集进入 LLM 草稿前，必须先按 `types/episode-final-image-type-map.md` 形成 `final_image_type_profile`，再建立 `episode_final_image_plan`，并由 `N5-DRAFT` 把它内嵌到本集最后一组既有字段中。
...
- 尾钩必须能被下游 `3-摄影` 和 `4-分组` 消费，但 `2-编导` 只写导演/演员/声画可执行材料，不写机位、景别、镜头运动或分镜编号。
