...
- 必须回读父层 `../SKILL.md` 与根级 `../../SKILL.md`，先锁定 `3-Drafting` 的阶段边界，再进入当前 chapter-native 直写 lane。
...
`正文/` 是 `3-Drafting` 下的 chapter-native 直写 child skill。
...
- 代替 `4-Validation` 做 PASS/FAIL 判定。
- 代替 `5-Loopback` 回写 validated actualization。
- 把上一章缺失伪装成“无法开写”的硬失败。

## Parent Positioning

- 当前 child 是显式 chapter-native lane，只在用户明确要求“按章写正文 / 输出到 `第N卷/第N章.md` / 命中 `正文`”时进入。
- 它不抹除父层既有的卷级 orchestrator 说明，但当前 child 自身拥有本章正文根文件的写权。
- 当前 child 默认不强制生成 `第V卷.写作日志.yaml`；如上游父层同时运行批次治理，可把本 child 的结果视为 chapter-root side input，而不是让当前 child 越权接管卷级账本。
...
- “LLM-first creative authorship” 在本 child 上的 owning provider 固定为 `doubao-seed-2.0-pro`。
- 未经用户显式改口，不得把本地 GPT 直写、手工改写或其他 provider 伪装成当前 child 的正常主路径。
...
stateDiagram-v2
    [*] --> SourceLocked
    SourceLocked --> ContextLoaded
    ContextLoaded --> FrontmatterReady
    FrontmatterReady --> Drafting
...
1. 当前 child 的最小业务单元是“章”，不是“集”或“卷批次”。
...
12. 当前 child 的 actual creative step 必须调用 `scripts/write_chapter_via_doubao.py`，并由它继续调用 `doubao_seed_chat.py`；不得把“本地会话直接写正文”当作等价执行。
...
- 当前 child 的 provider artifacts（messages pack、raw model output、provider report）可落到项目 `reports/3-Drafting/正文/.../doubao/`，但它们不是业务真源。

## Standard Invocation

```bash
python3 .agents/skills/story/3-Drafting/正文/scripts/write_chapter_via_doubao.py \
...
- 本次正文的实际创作 provider 已真实命中 `doubao-seed-2.0-pro`；若未命中，不得宣称当前 child 已按合同完成。
