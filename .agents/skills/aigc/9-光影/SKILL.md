...
description: "Use when injecting cinematic lighting aesthetics into 8-摄影 episode drafts or user-specified storyboard/camera drafts."
...
`9-光影` 是逐集分镜光影美学注入阶段。它默认消费 `projects/aigc/<项目名>/8-摄影/第N集.md`，也允许用户显式指定文稿、粘贴文本或要求跳过上游部分环节；指定 source 优先于默认路径。执行时必须加载 `3-美学` 的主要风格产物，尤其是：
...
这三类上下文用于提取画面基调、场景空间光影、摄影风格、参照大师及作品的可继承光影原则。核心文本动作是在原 `8-摄影` 稿或用户指定稿基础上内联增量注入：保留每条 `分镜N（N-N秒）：原有内容（包含摄影）` 的编号、秒数和原有内容，在原有内容后追加一段光影美学描述。
...
- 正式写回时，必读美学上下文为 `3-美学/画面基调/全局风格协议.md`、当前集优先的 `3-美学/第N集/场景风格/场景风格协议.md` 与 `3-美学/第N集/摄影风格/摄影风格协议.md`；逐集风格缺失时回退项目级对应风格。三者缺失时，可使用用户提供的等价文本；若完全没有等价画面基调、场景风格或摄影风格，不得判定 canonical pass。
- 可选上下文包括当前集优先的 `3-美学/第N集/分镜风格/分镜风格协议.md`、`角色风格/角色风格协议.md`，缺失时回退项目级对应风格；项目长期审美偏好、参考图/视频说明和视频模型限制。
- 正式生成、repair 或 review 时，必须加载 `../_shared/upstream-context-application-contract.md`，并在执行报告中记录 `Upstream Context Application Map`：说明 `8-摄影` 分镜/运镜、`3-美学` 画面基调/场景风格/摄影风格如何被投影为光源、受光、阴影、色温、材质、空气介质、动态光和光影连续性。
...
- 用户要求 `9-光影`、`aigc-lighting-aesthetic-injection`、光影美学注入、给摄影稿加光影、从 `8-摄影` 到 `9-光影`、逐分镜补光影、电影光影美学处理。
- 输入是 `8-摄影/第N集.md`、用户指定分镜/摄影稿、粘贴文本或已有候选 `9-光影` 稿，且需要结合 `3-美学` 的画面基调、场景风格和摄影风格完成逐分镜光影处理。
...
| --- | --- | --- | --- |
| `business_goal` | 将 `8-摄影` 单集稿升级为逐分镜具备电影光影美学描述的 `9-光影` 稿 | 用户请求、source 摄影稿、美学 source、输出路径 | `FAIL-LIGHT-BUSINESS-GOAL` |
...
| `topology_fit` | 先取 source 与 3-美学上下文，再理解题材和整集节奏，再逐分镜分析，再设计光影，再内联注入，再审查写回：1) 防止泛化光影词堆叠；2) 保证三类美学继承；3) 保证原文保真；4) 支持动态和多光源设计 | Visual Maps、节点表、报告证据 | `FAIL-LIGHT-TOPOLOGY-FIT` |

## Input Contract

Accepted input:

- 项目名、项目路径、单个或多个 `projects/aigc/<项目名>/8-摄影/第N集.md`。
- 用户指定分镜/摄影稿、粘贴文本、已有候选 `9-光影` 稿或修复目标。
- `3-美学/画面基调/全局风格协议.md`、当前集优先的 `3-美学/第N集/场景风格/场景风格协议.md` 与 `3-美学/第N集/摄影风格/摄影风格协议.md`，缺失时回退项目级对应风格；以及当前集优先、缺失回退项目级的可选分镜风格、角色风格、参考图/视频说明。
...
```mermaid
flowchart TD
    A["8-摄影/第N集.md 或用户指定文稿"] --> B["N1-LIGHT-INTAKE"]
    C["3-美学: 画面基调"] --> B
    D["3-美学: 场景风格"] --> B
    E["3-美学: 摄影风格"] --> B
...
| `../_shared/upstream-context-application-contract.md` | 任意正式生成、repair、review，或 `FAIL-LIGHT-UPSTREAM-CONTEXT` | 规定摄影稿与三类美学上下文如何被光影稿应用、保真和举证，要求 `Upstream Context Application Map` | 替代光影主创、改写 `8-摄影` 原文、无源新增光源/事件、复制全局 prompt | `N1-LIGHT-INTAKE` / `N5-LIGHT-SHOT-FUNCTION` / `N8-LIGHT-REVIEW-REPAIR` |
...
| `C2A-upstream-context-applied` | `8-摄影` 分镜/运镜和三类美学协议已投影为光源、受光、阴影、色温、材质、空气介质和动态光决策 | 只说明已读取摄影稿/美学，光影无法回指 source anchor、光源依据或 preservation check | `upstream_context_application_map` | `N1-LIGHT-INTAKE` / `N5-LIGHT-SHOT-FUNCTION` / `N6-LIGHT-DESIGN` |
...
| `8-摄影` 和三类 `3-美学` 上下文是否明确投影为光影决策，并记录 source anchor、local decision 和 preservation check，而非只写“已读取/已参考”？ | `GATE-LIGHT-09-UPSTREAM-CONTEXT` | `FAIL-LIGHT-UPSTREAM-CONTEXT` | `N1-LIGHT-INTAKE` / `N5-LIGHT-SHOT-FUNCTION` / `N6-LIGHT-DESIGN` | `upstream_context_application_map` |
