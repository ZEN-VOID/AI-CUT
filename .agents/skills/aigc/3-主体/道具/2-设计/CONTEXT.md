...
| `TM-PROP-DESIGN-02` | 研究考据像百科摘抄，无法转成可生成视觉 | 研究转译层 | 把考据压缩为材质、年代、工艺、使用状态/保存状态和功能逻辑 | 在 `references/` 固定“考据必须服务设计”，并声明磨损只是条件状态 | 考据之后能直接支撑解构字段 |
...
| `TM-PROP-DESIGN-11` | 道具只是“普通匕首、普通盒子、普通信件、普通令牌”，功能准确但缺少可见设计判断 | 审美设计层 | 回到 `N5-RESEARCH-CHAIN / N6-DESIGN`，补独特轮廓、材质记忆点、工艺/结构细节、条件性文化/身份/功能符号或克制设计理由、使用/保存状态、功能结构和 signature detail | 在 `SKILL.md`、模板、types、review gate 固定 `design_detail_culture`、`symbol_applicability_decision` 和 `condition_state_guardrail` | `Design Appeal Target`、`Signature Detail`、`Cultural Element Strategy or Minimal Function-Led Rationale`、状态证据和 prompt token 均可见 |
| `TM-PROP-DESIGN-12` | 文化元素像随机贴花，或把现代奢牌、战术、赛博、哥特奇幻符号硬套进历史/地域语境 | 文化语境层 | 删除错置元素，按项目时代、地域、阶层、职业、宗教/族群禁区和功能逻辑重做原创转译；无依据时改为极简/功能主导细节 | `knowledge-base/prop-design-corpus.md` 只做启发，必须配 `Period Context Guardrail` 与符号适用性判断 | `Prop Corpus Usage Trace` 说明语料种子、原创转译、适用性和 period/culture guardrail |
...
| `TM-PROP-DESIGN-14` | 道具设计稿字段完整但像批量模板换名、换材质或换纹样 | 反模板伪差异层 | 废弃候选设计稿，回到研究、解构和 prompt 节点重做道具专属裁决 | `SKILL.md` 固定 `FAIL-PROP-DESIGN-PSEUDO-DIFF`，禁止用字段完整、长度合规、语料库加载或“细节很多”抵消 | 每个道具有不可互换的形制、材质工艺、使用/保存状态、功能结构或文化语境裁决证据 |
| `TM-PROP-DESIGN-15` | 道具设计因为“磨损/使用痕迹”字段被系统性写成有旧损、折旧、污渍、包浆或锈蚀 | 状态语义层 | 把候选稿中的旧化词逐项回查上游证据、功能逻辑和项目风格；无依据则改为全新、洁净、封存、抛光维护、展陈级完好或其他更合适状态 | 在 `SKILL.md`、模板、references、types、review、corpus 和入口提示统一使用 `使用状态/保存状态`，并声明 `wear_trace` 只是条件子类 | prompt 中的 worn/scratched/dirty/patinated/rusted/damaged token 均有证据；新物/洁净物没有被强行旧化 |
| `TM-PROP-DESIGN-16` | `SKILL.md 的 Thinking-Action Node Map` 继续承载运行时节点，和 `SKILL.md` 的节点表漂移 | runtime spine 漂移层 | 将节点、路由、gate、汇流和 fail code 以 `SKILL.md` 为准；`SKILL.md` runtime spine 只作 legacy read-only reference | Module Loading Matrix 固定 `SKILL.md` runtime spine 禁止第二执行链 | `SKILL.md` 可独立跑通 intake -> worklist -> context -> design -> prompt -> review -> write |
| `TM-PROP-DESIGN-17` | test prompt 缺失导致达尔文或 dry-run 只能口头推断 | 评估资产层 | 新增 `test-prompts.json` 覆盖 single / incremental / repair / reject-script | 后续标准变更同步 prompts | JSON schema 可解析且至少 3 条 |
...
8. 若道具看起来平凡，先不加剧情，先加可见设计：轮廓比例、材料对比、工艺/结构细节、状态证据、功能结构和 signature detail；封缄/铭文/徽记、旧损、包浆、划痕或污渍都必须有依据。
9. 若需要文化/身份符号，先判断时代、地域、阶层、职业、宗教/族群禁区和道具功能，再从 `knowledge-base/prop-design-corpus.md` 取启发做原创转译；无依据时写极简/洁净/功能主导的设计理由。
...
15. 维护 Skill 2.0 合同时，优先更新 `SKILL.md` 的节点表、模块触发和 review gate；references、templates、scripts 只能跟随，不反向定义完成门。

## Reusable Heuristics

- 单道具设计的好坏不在信息量，而在“能否让生成和美术执行者稳定复现同一个物件”。
- 上游 `原文描述（关键词式）` 是约束，不是天花板；可以深化材质和视觉逻辑，但不得新增与上游冲突的叙事事实。
- 冷门考据应以“可见特征”收束：形制、材料、工艺、年代、使用/保存状态、携带方式和功能逻辑。
- 道具“好看”不是加一堆装饰，也不是默认做旧，而是让轮廓、材质、工艺、文化符号、使用/保存状态和功能结构形成统一设计；普通工具也可以通过比例、手作痕迹、材料记忆点、维护状态或使用证据变得有设计感。
...
- `Photography` 关注镜头如何让物件被看见；`Prop Design` 关注物件本身如何被制造、持握、保存、维护、使用、老化和辨认。
...
- 物语不应替代剧情扩写；它只说明该道具的叙事压力、象征和使用/保存状态。
- 文化符号和装饰不是“设计感”的默认答案；低证据、洁净、无菌、高科技、工业或极简道具优先通过比例、材质、结构、维护状态和功能逻辑建立辨识度。
...
- 设计叶子的最小可执行 spine 是“单主体 worklist + 设计上下文包 + LLM 研究/解构 + prompt 门禁 + review/write”；若时间不足，也必须先保留这些控制块标记。
