  ...
  asset_uploads:
    - command: "libtv upload"
      args:
        ...
        file: "projects/aigc/<项目名>/6-设计/角色/3-生成/CHAR-005-令狐冲.png"
      expected_evidence:
        - node_key
        - url
        - canvas_node_name
...
主体绑定表中 `canvas_node_name` 应与画布节点显示名一致；`yaml_name` 应与 `5-分组` 组底 YAML 主体名一致。
...
主体身份必须由 `主体绑定表` 的 `stable_subject_id + yaml_name + category + node_key + assetId/URL` 决定。`Image 1/2/3` 只有在执行层已按 canonical order 用 `--left/--left-add` 连接图片节点，并在最终节点参数、左侧输入和 prompt 写定后查询确认 runtime `data.params.imageList[]` 顺序，才具备 prompt 引用语义。
...
source_path: projects/aigc/<项目名>/6-设计/角色/3-生成/CHAR-005-令狐冲.png
last_verified_at: ""
executor_skill: ".agents/skills/cli/libTV"
```
...
- submit plan 的 `planned_left_input_edges[]` 能说明计划连接哪些 `node_key`，运行前查询得到的 `runtime_image_placeholder_map[]` 能说明每个 `{{Image N}}` 实际对应哪个 `node_key / assetId / URL`。
- 决定性远端查询固定在最终节点参数、左侧输入和 prompt 写定之后、`--run` 之前；最终执行前的 `data.params.prompt` 不得含 `{{Portrait N}}`、绑定表、参考图清单、执行锁、路径或诊断文本。
...
| 主体绑定是否只依赖已验证 runtime `imageList` 下的 `{{Image N}}`，不依赖上传顺序、UI 缩略图、`{{Portrait N}}` 或计划层下标？ | `REV-LIBTVCANVAS-04` / `REV-LIBTVCANVAS-16` | `FAIL-ORDER-SAFETY` / `FAIL-RUNTIME-IMAGE-MAP` | `N3 Subject Binding` / `N5 CLI Handoff` | `主体绑定表`、`planned_left_input_edges[]`、`runtime_image_placeholder_map[]`、唯一决定性 queried `data.params.imageList[] + data.params.prompt` |
