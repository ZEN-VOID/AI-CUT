...
- `OUTPUT-GROUP-EPISODE`: `projects/aigc/<项目名>/8-分组/第N集.md`
- `OUTPUT-GROUP-REPORT`: `projects/aigc/<项目名>/8-分组/执行报告.md`
...
- 每组以连续分镜行 / 声画 atomic unit 总时长为主：裁决边界时优先累计 `分镜N（起始秒-结束秒）：` 行的时长，兼容 `[起始秒-结束秒]` 行；落盘到分镜组后，时间码必须整合为当前分镜组基准下连续递增的 `分镜N（N-N秒）：` 或 `[N-N秒]`，后一个时间段起点必须等于前一个时间段终点，YAML `时长估算` 取最终结束秒。目标约 14.5 秒，通常约 10-11.5 秒可接受；最终累计结束秒必须以 `.5` 结尾，若自然相加不是 `.5` 结尾则在组尾上调 0.5 秒；单组时长不得超过 14.5 秒，超过 14.5 秒必须拆分、重组或回退 source owner 修复，不能写完整性例外理由放行；字数和对白只作辅助风险复核。
- 不输出 `## <上一个分镜组ID>~<下一个分镜组ID>` 组间连接件，也不输出 `连接类型：`、`连接方法：`、`变化过程：`、`主体运动：`、`运镜设计：`、`透视适应：`、`避免元素：` 等连接件字段；相邻组承接只通过下一组第一个普通 `[0-N秒]` 分镜行完成。
- 正文同步 `7-摄影` 或用户指定 source 原换行且未无授权改写。
- 执行报告同时记录初始化综合消费来源与采纳点、`Upstream Context Application Map`、`Upstream Grouping Direction Matrix`、每组 `时长估算`、回龙帧尾帧状态锚点五项元素复现、镜头调整和声音承托同步检查、validator error/warning 与 `boundary_review`、`global_style_review`、`first_storyboard_continuity_review`、`connector_removal_review`、`faithfulness_review`、`statistics_evidence_review` 语义结论。

## Upstream Context Application Map

| upstream_context | source_anchor | used_in_node | application_summary | output_anchor | preservation_check |
| --- | --- | --- | --- | --- | --- |
...
| upstream_context | direction_role | used_as | stage_decision | stage_landing | boundary_check | evidence_map |
| --- | --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |  |
