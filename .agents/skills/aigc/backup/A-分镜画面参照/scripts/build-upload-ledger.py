#!/usr/bin/env python3
"""Build and optionally sync frame-reference upload and generation-slot ledgers."""
...
def _task_images(task: dict[str, Any]) -> list[dict[str, Any]]:
    images = task.get("images") or task.get("reference_images") or []
    if images:
...
def _identity(image: dict[str, Any], index: int) -> tuple[str, str]:
    shot_id = str(image.get("shot_id") or image.get("frame_id") or image.get("id") or image.get("name") or f"frame-{index}")
    source_label = str(image.get("source_label") or image.get("label") or image.get("shot_label") or shot_id)
    return shot_id, source_label
...
def _build_slots(plan: dict[str, Any], manifest: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    frame_uploads: list[dict[str, Any]] = []
    generation_slots: list[dict[str, Any]] = []
    errors: list[str] = []
    project_uuid = _project_uuid(plan)
    images = _slot_images(plan, manifest, errors)
    for position, image in enumerate(images, start=1):
        uploaded_url = str(image.get("uploaded_url") or image.get("url") or "")
        if not uploaded_url:
            errors.append(f"frame slot {position}: missing uploaded_url")
            continue
        shot_id, source_label = _identity(image, position)
        ...
        if project_uuid and f"/claw/{project_uuid}/" not in uploaded_url:
            errors.append(f"{shot_id}: uploaded_url project scope does not match projectUuid={project_uuid}")
        frame_uploads.append({
            "shot_id": shot_id,
            ...
            "identity_lock": f"{shot_id} -> {uploaded_url}",
        })
        generation_slots.append({
            ...
            "slot_lock": f"slot={reference_index} imageList[{reference_index - 1}] {uploaded_url} -> {shot_id}",
        })
    return frame_uploads, generation_slots, errors
...
def _rewrite_image_list(text: str, slots: list[dict[str, Any]]) -> str:
    ...
    return text.replace("modeType: image2video", "modeType: image2video\n" + replacement, 1)
...
def build(package_dir: Path, *, sync: bool) -> dict[str, Any]:
    ...
    frame_uploads, generation_slots, errors = _build_slots(plan, manifest)
    warnings: list[str] = []

    if sync and not errors:
        manifest["frame_uploads"] = frame_uploads
        manifest["generation_slots"] = generation_slots
        if manifest_path:
            _write_doc(manifest_path, manifest)
        tasks = _tasks(plan)
        if tasks:
            tasks[0]["frame_uploads"] = frame_uploads
    ...
    return {
        ...
        "sync_rule": "generation_slots projects frame refs into manifest, plan, prompt YAML, and imageList",
        "errors": errors,
        "warnings": warnings,
    }
