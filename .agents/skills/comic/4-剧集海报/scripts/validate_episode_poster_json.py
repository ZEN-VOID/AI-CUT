...
IMAGEGEN_SKILL_PATH = ".agents/skills/cli/imagegen"
...
def validate(data: dict[str, Any]) -> list[str]:
    ...
    expect("grouped_script" in loaded_roles, "upstream_context.loaded_artifacts must include grouped_script", errors)
    ...
    imagegen_handoff = data.get("imagegen_handoff", {})
    expect(isinstance(imagegen_handoff, dict), "imagegen_handoff must be an object", errors)
    if isinstance(imagegen_handoff, dict):
        expect(
            imagegen_handoff.get("tool_skill_path") == IMAGEGEN_SKILL_PATH,
            f"imagegen_handoff.tool_skill_path must be {IMAGEGEN_SKILL_PATH}",
            errors,
        )
        expect(
            imagegen_handoff.get("render_prompt_source") == "prompt_package.positive_prompt",
            "imagegen_handoff.render_prompt_source must be prompt_package.positive_prompt",
            errors,
        )
        expect(bool(imagegen_handoff.get("suggested_output_dir")), "imagegen_handoff.suggested_output_dir is required", errors)
        expect(bool(imagegen_handoff.get("mode_policy")), "imagegen_handoff.mode_policy is required", errors)
        expect(bool(imagegen_handoff.get("persist_policy")), "imagegen_handoff.persist_policy is required", errors)
