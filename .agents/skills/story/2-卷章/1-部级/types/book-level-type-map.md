...
| `revision_scope` | `whole_file`、`field_patch`、`suspense_only`、`suspense_thread_only`、`rhythm_only`、`handoff_only` | 修订范围 |
...
| --- | --- | --- | --- |
| `request_type=create` | 从 `N1-INPUT-LOCK` 串行执行到 `N11-REVIEW` | 加载全部必需 references 与模板 | 完整 review gate |
