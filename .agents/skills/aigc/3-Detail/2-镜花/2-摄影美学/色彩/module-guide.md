...
| --- | --- | --- | --- | --- | --- | --- |
| `CLR-N1-INHERIT` | 回收色彩锚 | `visual_control_line`、`cinematography_strategy_note` | 提炼色彩首先服务的对象与约束 | `color_anchor` | 服务对象不清 -> 回上游；通过 -> `CLR-N2` | 先回答色彩服务谁 |
| `CLR-N2-HUE` | 锁主导色相 | `color_anchor`、空间/人物关系 | 写主色域和点状辅助色 | `hue_bias` | 主色域漂移 -> 重做本节点；通过 -> `CLR-N3` | 主导色相必须稳定 |
...
| `CLR-N5-TEMP` | 锁色温方向 | 前述色彩关系、情绪张力 | 明确整体色温和局部对照策略 | `color_temperature` | 冷暖逻辑自相矛盾 -> 回到 `CLR-N2~N4`；通过 -> `CLR-N6` | 色温必须服务关系张力 |
| `CLR-N6-RELATION` | 定色彩关系与心理 | 完整色板信息、当前组主情绪 | 汇总成可执行色板说明 | `palette_relation + emotional_color_gain` | 只剩抽象感受 -> 回到前序节点；通过 -> done | 色彩必须能被下游执行 |
