...
3. 能定位明确 source owner：`5-摄影`、`6-分组`、`9-视频`、`10-审片` 或对应叶子技能。
...
| --- | --- |
| 文件名无法回推 group_id | `9-视频` 输出合同 |
| 15 秒视频总是塞入太多 beat | `6-分组` 边界/密度合同 |
| 镜头不可动、只有气氛形容 | `5-摄影` 分镜明细合同 |
| 审片无法稳定落盘 | `10-审片` landing contract |
| 多路线视频都错绑主体参照 | `9-视频` 叶子 reference binding |
...
| 是否定位了明确 source owner，例如 `5-摄影`、`6-分组`、`9-视频`、`10-审片` 或对应叶子技能，而不是笼统写“上游问题”？ | `GATE-REVIEW-10` | `FAIL-REVIEW-SOURCE-ESCALATION` | 本文件 `Common Source Owners`、`steps/video-review-workflow.md#N5 Landing` | `source_owner`、owner path、症状到 owner 的因果说明 |
