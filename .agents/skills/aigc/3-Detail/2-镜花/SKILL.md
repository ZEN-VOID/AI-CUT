...
description: Use when `3-Detail` needs a stage-local child skill to take `1-水月/第N集.md` as the primary formatted screenplay, load `0-Init` and `2-Global` outputs as added context, then complete director/cinematography-oriented enrichment and write `projects/aigc/<项目名>/3-Detail/2-镜花/第N集.md`.
...
- `projects/aigc/<项目名>/3-Detail/1-水月/第N集.md` 中已经完成剧本维度优化的 grouped screenplay
- `0-Init` 的项目基线、题材约束、north star 与 preset
- `2-Global` 的 `类型元素 / 全局风格 / 设计元素` 项目级或分组级证据

进一步压成一份以导演调度、摄影执行、运镜组织和转场节奏为主的组级融写稿：

- canonical 输出：`projects/aigc/<项目名>/3-Detail/2-镜花/第N集.md`
...
- `2-摄影美学` 在进入叶子维度前，必须先串行完成 `摄影底座 -> visual_control_line -> cinematography_strategy_note`，不得跳过中层收束直接并发 `光影 / 色彩 / 质感`。
- 所有模块只产出局部导演/摄影 patch，不得长出第二份主稿。
- 所有镜头语言组织都必须建立在 `1-水月` 已写出的动作、情绪、空间、关系和视觉线索之上，不得脱离上游另起一套镜头叙事。
- 每个分镜组最终 `镜花` 正文总字数不得超过 `1800` 字。
...
- 写回 `projects/aigc/<项目名>/3-Detail/2-镜花/第N集.md`
...
- `projects/aigc/<项目名>/3-Detail/1-水月/第N集.md` 已稳定存在
...
- 第一输入真源固定为：`projects/aigc/<项目名>/3-Detail/1-水月/第N集.md`
...
- 当前模块包分层真源统一回链：`references/module-index.md`
- 最终真源只允许落在：`projects/aigc/<项目名>/3-Detail/2-镜花/第N集.md`
...
5. 每个分镜组最终 `镜花` 正文可包含多个 `[分镜N ...]` 段，但总字数固定 `<= 1800`。
...
| `constraint_profile` | `1-水月` 是主输入；`0-Init + 2-Global` 是附加上下文；顶层编号模块必须串行；叶子模块只产 patch；镜头语言必须有 `1-水月` 证据依托；每组成稿不超过 `1800` 字 |
...
8. `projects/aigc/<项目名>/3-Detail/1-水月/第N集.md`
9. `projects/aigc/<项目名>/1-Planning/3-分组/第N集.md`（仅用于组界复核，若存在）
10. `projects/aigc/<项目名>/2-Global/类型元素/全集设计.md`（若存在）
11. `projects/aigc/<项目名>/2-Global/类型元素/分组设计.md`（若存在）
12. `projects/aigc/<项目名>/2-Global/全局风格/全局风格设计.md`（若存在）
13. `projects/aigc/<项目名>/2-Global/设计元素/设计元素.md`（若存在）
14. `references/module-index.md`
15. `references/route-profile.yaml`
16. 按需读取各分类 `module-spec.yaml` 与命中叶子 `module-spec.yaml`
17. 若需要解释、反例或审美尺度，再读同目录 `module-guide.md`
18. `references/examples.md`
19. `references/creative-review-rubric.md`
20. `templates/episode-jinghua.template.md`
...
| `N4-SHOT-STAGE` | `S4` | `FIELD-JH-04` | 完成 `1-分镜表现` 的严格串行阶段 | 依次执行 `切换 -> 插入 -> 构图`，形成含密度预算、景别节奏预判、POV 预判、descriptor lock 与下游可抽镜抓手的 shot spine | `shot_spine_patch` | 成功 -> `N5`；散乱 -> 回 `S3/S4` | 未形成分镜骨架、预算/视点预判与可抽镜抓手前不得进入后续阶段 |
| `N5-CINEMATOGRAPHY-STAGE` | `S5` | `FIELD-JH-05` | 完成 `2-摄影美学` 的“先收束、后分发、再汇流”阶段 | 先锁 `visual_control_line`（主体控制 / 空间剥离 / 质感方向 / 观看压力），再写 `cinematography_strategy_note`，随后分发 `光影 / 色彩 / 质感` 并压成单一摄影 patch | `cinematography_patch` | 成功 -> `N6` | 未先锁视觉控制线与摄影执行策略前不得进入叶子；摄影信息必须依附 shot spine |
...
| `N9-BUDGET` | `S9` | `FIELD-JH-09` | 把每组 `镜花` 正文裁进 `1800` 字预算 | 先删重复修辞，再删次级说明，保留主执行收益 | `budget_report` | 成功 -> `N10`；超限 -> 回 `S8` | 每组 visible chars 必须 <= 1800 |
| `N10-WRITEBACK` | `S10` | `FIELD-JH-10` | 按模板一次性写回整集 markdown | 落盘 `projects/aigc/<项目名>/3-Detail/2-镜花/第N集.md` | `writeback_note` | -> `N11` | 只写一个 canonical 文件 |
...
| 模块 | 作用 | 真源文件 |
| --- | --- | --- |
| 模块总索引 | 规定顶层顺序、模块 ownership、叶子回链与模块包分层真源 | `references/module-index.md` |
| 创作引导集合 | 规定 route 偏置、正反例与创作验收尺度 | `references/route-profile.yaml`、`references/examples.md`、`references/creative-review-rubric.md` |
...
- 按 `1-水月` 的分镜组顺序逐组展开
- 保留上游分镜组标题与组内格式化标题字段
- 最终落盘形态是分镜剧本，不新增 `锚点 / 镜花 / 水月承接` 等外露区块
- 组内必须出现分镜标记，例如：`[分镜1 0秒-4秒]`

### 输出质量门槛

1. 组序与 `1-水月` 上游一致。
2. 每组都仍然读起来像分镜剧本，而不是分析表单或模块拼贴。
...
| `FIELD-JH-04` | shot spine patch | 分镜数、预算区间、插入点、构图骨架、景别节奏、POV、descriptor lock 与下游可抽镜抓手稳定 | `S4` | 分镜组织力 | `FAIL-JH-04` |
| `FIELD-JH-05` | 摄影美学 patch | 先锁视觉控制线与摄影执行策略，再让光影、色彩、质感服务故事与风格 | `S5` | 摄影可执行性 | `FAIL-JH-05` |
...
| `FIELD-JH-09` | 字数预算 | 每组 `镜花` 正文 <= 1800 字 | `S9` | 预算控制 | `FAIL-JH-09` |
...
| `S4` | `FIELD-JH-04` | 这组要切几镜、何处插入、每镜如何构图，以及景别/POV/心理距离怎样形成组级节奏 | 形成 shot spine | 仍像整段 prose，或镜头预算/descriptor 仍未锁定 |
| `S5` | `FIELD-JH-05` | 这组的主体控制、空间剥离、质感方向与观看压力是什么，哪些判断必须稳定贯穿后续光位和色彩分支 | 先收束 `visual_control_line + cinematography_strategy_note`，再生成摄影 patch | 跳过中层策略，直接堆光色形容词 |
...
| `S9` | `FIELD-JH-09` | 如何在 `1800` 字内保住导演/摄影收益 | 裁剪与去重 | 超字数或只剩空骨架 |
...
| `FIELD-JH-04` | 每组都有可追踪的分镜标记、预算区间、景别/POV 预判、构图意图与后续可抽镜抓手 | `FAIL-JH-04` | `S4` |
| `FIELD-JH-05` | 摄影设计先有控制线与策略，再具体、克制、能被执行 | `FAIL-JH-05` | `S5` |
...
| `FIELD-JH-09` | 每组 `镜花` 正文不超过 `1800` 字 | `FAIL-JH-09` | `S9` |
