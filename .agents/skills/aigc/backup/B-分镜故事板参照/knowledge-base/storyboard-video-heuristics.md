...
- 分镜组视频最容易漂移的是“把组正文重写成短视频广告语”；本技能应优先保留 `5-分组` 的完整内容。
- 多格故事板图对视频模型最有价值的是画面连续性、构图节奏和角色位置，不是首帧精确复制。
- B 路线有故事板 sheet 时必须锁定 Seedance `singleImage2video`，故事板 URL 进入 `imageList[0]`；不要再用泛化的 `image2video` 口径描述这条路线。
- 缺故事板图时用 `libtv_session_text_only` 是正常降级；报告清楚即可，不要为了凑参照去绑定主体图或旧资产图。
