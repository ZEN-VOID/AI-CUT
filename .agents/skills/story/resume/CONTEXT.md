...
| `resume/` 与 `5-上下文回流` actualization 职责混淆 | stage contract | 在 `resume/SKILL.md` 明确其是 satellite recovery skill | 在 `5-上下文回流` 与 `resume` 同时固定边界 | 不再把恢复动作描述成 truth writeback |
