...
| FIELD-PROP-DESIGN-04 | `道具设计.json` | 必须只保留稳定设计事实，并保留 `design_thesis.narrative_significance` | FAIL-PROP-DESIGN-04 |
| FIELD-PROP-DESIGN-05 | `prop_design_prompt.json` | 必须只保留 prompt sidecar，并响应叙事重点 | FAIL-PROP-DESIGN-05 |
| FIELD-PROP-DESIGN-06 | 聚合写回 | 必须由父 skill 独占最终写回 | FAIL-PROP-DESIGN-06 |
| FIELD-PROP-DESIGN-07 | 审计报告 | 必须产出 coverage、`special_narrative_prop_count` 与 drift flags | FAIL-PROP-DESIGN-07 |
| FIELD-PROP-DESIGN-08 | 下游回接 | 必须明确 `5-Image / multiview-prop` 入口 | FAIL-PROP-DESIGN-08 |

## Thought Pass Snapshot

1. 先锁定 bridge 是否存在。
2. 再锁定本轮是否需要 design master、prompt sidecar 或二者都要，并识别哪些道具具特殊叙事意义。
3. 并行收集结构 / 材质 / 痕迹 patch，把叙事约束转成可执行字段。
4. 父 skill 先聚合 canonical design，再让 prompt 架构师消费。
5. 最后由审计角色检查 coverage、`special_narrative_prop_count`、path normalization 与 drift。
...
4. 不得静默丢失上游 `narrative_significance`。
