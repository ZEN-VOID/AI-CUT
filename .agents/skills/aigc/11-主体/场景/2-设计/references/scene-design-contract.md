...
| `提示词设计` 是否记录 `画面基调.Global Style Prompt`、`场景风格.Scene Style Prompt`、时间与地域引用，且时间/地域无法确认时采用有来源姿态的保守英文锚点而不是省略？ | `GATE-SCENE-DESIGN-06` | `FAIL-SCENE-DESIGN-06` | `N6-DESIGN` | 报告列出 visual tone anchor、scene style、period token、region token 与来源姿态。 |
