...
字段缺失时视为空数组并记录缺口。不得从正文、对白、镜头语言或道具描述中自动扩展主体清单。
...
4. 若多视图与主图都不存在，槽位保持空、列入 `missing`，从 Dreamina 图片数组中移除，并不得在主体信息后添加空 `@` 后缀。
...
matched_by: "exact"
subject_inline: "林寂 @projects/aigc/<项目名>/4-设计/角色/3-生成/林寂-多视图.png"
```
...
2. 所有绑定路径存在且是图片文件。
3. 多视图优先于主图。
4. 缺图主体不保留空路径，不进入 Dreamina reference images，不生成空 `subject_inline`。
