...
| 摄影稿中的 `动作画面：` 被漏识别为普通正文 | source inventory 层 | 将 `动作画面：` 与 `对白画面：`、`音效画面：` 同等视为可独立承载时间码的声画 atomic unit header，重建清单后再裁边界 | inventory 扫描字段头时不得只列对白、旁白、音效、角色动作；凡字段下包含 `分镜N（起始秒-结束秒）：` 的动作/画面类字段都应进入 atomic unit 清单 | validator 不再出现由漏段导致的非连续时间码、超 14.5 秒或 YAML 时长不一致 |
| 按字数硬切截断分镜画面 | 完整性层 | 把被截断的分镜行和全部连续时间段放回同组 | 将分镜行视为不可拆 atomic unit | 任一分镜行不跨组 |
| 仍按字数中心裁决 14.5 秒组 | 时长裁决层 | 回到 `7-摄影` 的分镜行，取每字段最后时间段结束秒并累计组内分镜总时长；字数和对白只做风险复核 | boundary contract 固定“分镜总时长为主、atomic unit 完整性优先、字数对白为辅、最终累计结束秒以 `.5` 结尾” | 每组有 `时长估算`；边界接近 14.5 秒但不为卡点拆断同一画面，最终结束秒以 `.5` 结尾 |
...
| source override 首镜为 `0-0.5秒` 时 validator 漏识别首段时间码 | 时间码兼容层 | 落盘到 `8-分组` 时把首个 0.5 秒镜头投影为 1 秒基线，并整体顺延组内后续时间码，组尾仍按 `.5` 收束；不得回改 source 真源或删减该镜头 | 当前 validator 不识别 `0-0.5秒` / `0.5-N秒` 作为标准首段；在落盘层做兼容投影，并在报告 `Repair Log` 记录 | validator 不再报 first storyboard time range 错误；报告声明该兼容只影响 `8-分组` 累计时间码，不改变 source 事实 |
...
| 动作组反应镜头过载或多线混战空间断裂 | 动作设计/空间调度层 | 把纯表情、器物反应或氛围镜头改写为攻击起点、路线、身体换位、拆招撞点、结果后果和宽景复位；多线战场先固定门缝/池沿/墙根等空间锚点 | repair/review 增加 `action-chain audit` 与 `melee-layout pass`：反应镜头必须改变战术选择，群战必须有可读运动前线 | 每个动作组能复述“起点 -> 路径 -> 身体 -> 撞点 -> 后果”；群战镜头能看清至少三个空间锚点、主动威胁线和阵线推进方向 |
...
3b. 若 source override 中某组首个普通分镜行是 `0-0.5秒`，不要让该 0.5 秒首段原样进入 `8-分组` 落盘稿；当前 validator 会漏识别该首段并误判首个时间码不是 0。应在落盘层把首段投影为 `0-1秒`，后续时间码整体顺延，最终组尾仍以 `.5` 收束，并在执行报告中声明这是 validator 兼容投影，不是 source 改写。
4. 时长接近 14.5 秒时，不为精确卡点或制造 `.5` 结尾拆断同一画面；低于约 10 秒优先回填完整 atomic unit；超过 14.5 秒必须向前或向后移动完整 atomic unit，无法降到 14.5 秒以内时回退 `7-摄影` 修复单个超长 atomic unit；自然累计不以 `.5` 结尾时只允许在组尾上调 0.5 秒。
...
7. 组内镜头、构图、光源、色彩、摄影参数或观看位置需要保留时，保留在普通时间码分镜正文里。若上游本身无动机正面平视或空间感不足，应回退 `7-摄影`。
...
15a. 若用户反馈“连贯性不足、反应镜头太多、过招太少、多线混战乱”，先做动作链审计：每个动作段必须有攻击起点、运动路线、身体换位、拆招撞点、结果后果和宽景复位；表情/眼神/器物特写只有在它改变下一拍战术选择时保留。多线混战先建立固定空间锚点和运动前线，例如门缝、池沿、墙根、阵线右前角，再让人物或阵形沿锚点推进，不用孤立特写堆出混战感。
...
- 分镜组不是平均字数块，也不是情绪/话题转折块；它是一个可独立送入后续生成的完整声画单元。主轴是组内分镜总时长接近 14.5 秒，通常 10-11.5 秒，最终累计结束秒以 `.5` 结尾。
- 分镜行是上游摄影稿对画面句子的承载单元，分组时必须连同其全部时间段整体移动，不得替换为 `分镜画面：`。
...
- 直接接手剧本是正式兼容路径，不是异常绕过；核心差异是时长来源从 `7-摄影` 显式时间段变为本阶段 LLM 对剧本声画单元的约 14.5 秒规划，报告必须把这个来源和 `.5` 收束说清楚。
...
- 借鉴逐镜参考片时只迁移动作机制，不复制表层元素：优先抽取攻击前的轨迹锚点、软兵器改变身体路线、低角度前景武器压迫、快速剪辑的“发现-锁定-变招/接触-结果”节拍、落地/撞击反馈和尾帧物件结果，再翻译到当前项目已有场景锚点与道具。
- 参照图校准不是“补更多细节”，而是收束自由想象：角色只写图片已有的服装颜色、发型、护具、武器轮廓和可见道具；场景只写图片已有的入口、木门、盐袋、湿路、木桩、灯影和空间层次。若剧情需要使用未在图中明确出现的物件，应让它作为剧情道具出现，避免把它误写成参照图固有元素。
