...
- 调用本技能时，应在父级 `5-设计/2-设计`、共享输入合同、`1-清单/场景` 之后加载本文件。
...
| --- | --- | --- | --- | --- |
| 直接把旧仓 `3-设定/2-设计/场景设计` 路径复制进当前仓 | runtime 迁移层 | 改为 `projects/aigc/<项目名>/4-设计/场景/2-设计/第N集/` | 在 `SKILL.md` 的 Parent Positioning 与 Output Contract 固化当前仓 runtime | 输出不再指向 `output/影片/.../3-设定` |
...
- 场景设计迁移最稳的做法不是复刻旧仓路径，而是复刻旧仓字段密度与质量门，再把输入输出接到当前仓 `5-设计` runtime。
