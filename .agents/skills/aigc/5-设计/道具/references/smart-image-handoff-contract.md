...
本文件是 `5-设计/3-面板` 到内置 `$imagegen` / `image_gen` 的共享 handoff 真源。
...
| --- | --- | --- | --- |
| `continuous-batch` | 由 `5-设计` 系列批量调度，或只给 `--project + --episode` 批量处理设计目录 | 自动扫描 layout 中声明的 `continuity_source_roots`、设计目录同级图片与 `projects/aigc/<项目名>/Assets/<域>/` 中匹配主体的已有图片 | 有图则 I2I，无图则 T2I |
  ...
  "tasks": [
    {
      ...
      "output_dir": "projects/aigc/<项目名>/4-设计/角色/3-面板/第1集/generated/ROLE-001",
      ...
      "prompt_reference": {
        ...
        "source_layout_json": "projects/aigc/<项目名>/4-设计/角色/3-面板/第1集/ROLE-001-角色名-baseline-CharacterPanel-layout.json"
      }
    }
  ]
}
...
4. request sidecar 默认落到 `projects/aigc/<项目名>/4-设计/<域>/3-面板/第N集/generated/requests/panel_auto_generate_batch.json`。
