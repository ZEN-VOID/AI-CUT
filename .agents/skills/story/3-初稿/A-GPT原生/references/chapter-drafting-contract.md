...
- `supervision_packet`：正式写作时必须由 team supervision subagents 产出并进入 GPT-native messages；若上层策略阻断真实 subagents，必须有降级报告。
...
| `topology_fit` | `source lock -> context pack -> minimal frontmatter marker -> continuity bridge -> GPT-native chapter generation -> final writeback` |

## Hard Rules

1. 当前技能的最小业务单元是“章”，不是“集”或“卷批次”。
2. 必须先锁定当前章 planning，再读取 global/style/north-star；不得反过来凭风格或世界观猜当前章该写什么。
3. YAML 头只保留 `写作模型: GPT`；禁止重复写入 planning 路径、cards 路径、项目上下文路径或 global/style/north-star 摘要。
...
12. actual creative step 默认由当前 GPT/LLM 会话直接完成；若需要外部 provider，应显式改走对应 provider skill。
13. 正式写作默认启动隔离 team supervision subagents；同为 GPT 时也必须分离主写作上下文与监制上下文。
14. `scripts/write_chapter_gpt_native.py` 只能装配上下文、注入 `supervision_packet`、输出 sidecar、校验已创作稿与写回，不得直接生成正文。
15. 若 GPT 原生输出不含完整 YAML frontmatter、`写作模型` 不等于 `GPT`、缺少 `# 第N章｜章标题` 标题行，必须判定为 output invalid，禁止直接写回业务真源。
...
- GPT 原生流正文 frontmatter 保持极简；上一章路径、连续性桥和上下文引用由 context pack、GPT-native sidecar 与 dry-run summary 追溯。

## File Rules

- frontmatter 只记录 `写作模型`，不承载上下文引用或摘要。
- 正文主体不得保留“本章故事概要 / 本章冲突 / 规避”之类 planning 标题。
- 章末必须对齐当前章 planning 的 `exit_hook / 对下章的直接推动 / 章末达成` 中至少一项强义务。
- 当前技能的 GPT-native artifacts 不是业务真源。
