...
| 下游仍按旧 detail markdown sidecar 读取上游 | 输入真源层 | 切换到 `projects/aigc/<项目名>/3-Detail/第N集.json` 并按 canonical `groups[].detail.分镜列表` 锁定目标分镜；旧 helper 只经 compat projection 读取 | 在 `SKILL.md` 固化 `canonical first, compat helper fallback` | 单帧条目能回链 shared detail template |
| 单镜融写行混入整组剧情或大段对白 | 内容归纳层 | 收缩到当前目标分镜与其必要组级上下文 | 把单镜融写行定义为只服务当前帧的镜级块，而非整组剧情摘要 | 内容块不再复述整段台词 |
| prompt 没有固定英文前缀，或重新恢复独立 A 段整组 `剧本正文` | Prompt 合同层 | 重新按固定前缀 + 组级设计块 + 单镜融写行拼接 | 在 `SKILL.md` 与 `prompt-assembly-spec.md` 固化前缀逐字保留与 `BC` 结构 | prompt 开头逐字一致，且不再单列整组剧本文本 |
...
| `3-Detail` phase 未就绪却直接锁镜 | 阶段就绪层 | 在 `N1` 输入门先查 canonical detail root 经 compat adapter 推断的 readiness | 在 `SKILL.md` 固化 `detail_in_progress | ready` 才允许进入 `N2` | 不再从未完成 detail 根文件取镜 |
| 单帧上下文漏掉 `出场角色及穿搭`、`正文回指` 或镜级 canonical 字段 | 上下文打包层 | 回到 `N3` 补齐组级穿搭、`正文切分参考[] -> 正文回指` 桥接与镜级字段 | 在 `SKILL.md` 固化 frame context pack 最低覆盖面 | 单镜融写行不再只有抽象镜头描述 |
...
1. 先查 canonical detail root 经 compat adapter 推断的 readiness 是否已到 `detail_in_progress | ready`，再锁唯一 `分镜ID`。
2. 再检查单镜融写行是否只服务当前目标分镜与其必要组级上下文。
3. 再检查 `prompt` 是否严格等于“固定英文前缀 + 组级设计块 + 单镜融写行”。
...
- 先锁 ID，再走 `正文切分参考[] -> 正文回指 -> 单镜融写行`，否则所有单帧内容块都会失真。
- `分镜帧` 与 `首帧参照` 现在共享同一桥接心智：不再保留独立 A 段整组 `剧本正文`，而是把原剧本信息融到镜级行里。
- 当上游 detail 真源已切到 `.agents/skills/aigc/3-Detail/_shared/episode_detail.json` 后，`分镜帧` 必须先从 canonical `groups[].detail.分镜列表` 取镜；若脚本桥接仍需旧 helper，只能经 `detail_root_adapter` 投影后读取。
