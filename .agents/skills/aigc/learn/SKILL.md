...
本技能不替代 `0-初始化` 到 `10-审片` 的阶段主创权，也不把外部材料直接提升为仓库真源。它只拥有学习对象解析、可信度裁决、跨技能差距诊断、改进落点规划、协调写回和交付审计权。所有落盘改进必须回到 owning skill 的 `SKILL.md + CONTEXT.md`、分区合同、registry/routes 与审计脚本。
