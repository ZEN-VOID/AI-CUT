...
| `N3R-REVIEW-SUBAGENT-AUDIT` | 显式 subagents 模式下按审计点拆分维度审查 | messages pack、源初稿、既有润色稿、审计点、`.agents/skills/story/review` registry、命中的 review 子技能 | 分别调用结构兑现、连续性、逻辑自洽校验、人物一致性、时间线、任务汇聚等子技能输出 packets，并汇成 Doubao repair brief | dimension packets、review repair brief、降级报告 | `N4-DRAFT-BRANCH` | 每个审计点有对应 review 子技能结果；同轮必须进入 provider 优化分支 |
