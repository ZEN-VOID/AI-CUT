...
| mode | trigger | action |
| --- | --- | --- |
| `mechanical_check` | 落盘前或修复时 | 检查分镜组 ID、必填标题、逐分镜站位位移、禁止输出 `空间锚点：` 字段、字数、YAML、组间桥接一致性 |
| `boundary_review` | 分组边界争议时 | 检查对白数、1980 字上限、atomic unit 完整性 |
| `spatial_review` | 空间连续性风险高时 | 检查 `站位和位移：` 中的空间参照是否为真实位置参照点，每个 `分镜N:` 是否都有对应 `站位和位移：`，并以明确角色名说明方位和运动方向；不得输出独立 `空间锚点：` 字段 |
...
| `GATE-GROUP-02` | north_star 投影 | 每组组头含三行纯内容；第 1 行在直引 `全局风格.全局风格提示词` 原词基础上追加 `不生成字幕，不生成BGM，要生成物理互动音效与环境音。`，第 2、3 行分别直引 `类型元素.类型元素提示词`、`细分风格.画面风格`；不显示 `[全局风格]`、`[类型元素]`、`[画面风格]` 标题字段 |
| `GATE-GROUP-03` | 空间参照 | 不输出独立 `空间锚点：` 字段；空间参照只内化到逐分镜 `站位和位移：` 中 |
| `GATE-GROUP-04` | 站位位移 | 每个 `分镜N:` 明细下方含非空 `站位和位移：` 辅助行，以明确角色名或上游已命名的稳定群体称谓为主语，不使用 `画面主体`、`主体`、`人物`、`角色`、`主角`、`他/她/他们/她们` 等含混指代，并说明空间参照、方位和运动方向 |
| `GATE-GROUP-05` | ID 真实 | 每组 ID 为 `x-y-z`，匹配真实集、场、场内序号 |
| `GATE-GROUP-06` | 字数 | 每组完整构成 <= 1980 字，不含 YAML，包含逐分镜站位位移 |
...
| `GATE-GROUP-12` | 原文保真 | 划定正文同步原换行，未改写 `3-摄影` 字段、对白、原有镜头语言；只允许在对应 `分镜N:` 下方新增 `站位和位移：` 辅助行 |
...
| --- | --- | --- |
| `FAIL-GROUP-02` | north_star 三项缺失、被改写，或全局风格固定追加词缺失 | `references/north-star-projection-contract.md` |
| `FAIL-GROUP-03` | 输出独立空间锚点字段、空间参照抽象化、任一分镜明细缺站位位移，或站位位移使用模糊主语 | `references/spatial-consistency-contract.md` |
