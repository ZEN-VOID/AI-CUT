...
| `complexity_source` | 场景类型分型、冻结初始化综合、研究可靠性、来源姿态、不确定性处理、视觉翻译和批量一致性 |
...
- `workflow_supervision_record` 至少记录执行模式、阻断层级、初始化综合消费状态、本地流程、`init_synthesis_node_coverage` 和汇流裁决。
