...
description: "Use when running an AIGC project episode chain through 2-编剧 to 6-分组 with per-episode subagents and isolated stage handoffs."
...
`sword6` 是 AIGC 影视项目的多阶段 subagent 编排技能。它面向 `projects/aigc/<项目名>/` 下已完成 `1-分集` 的特定分集集合，依次调度 `2-编剧 -> 3-导演 -> 4-表演 -> 5-摄影 -> 6-分组`。主窗口只负责派发、追踪、汇流和触发下一阶段；阶段正文由后台隔离 subagent 按对应阶段技能完成。
...
整体调用 `$aigc-workflow-sword6` 时，视为用户已授权本编排器在输入、安全门和 subagent runtime 成立后连续完成 `2-编剧` 到 `6-分组`。阶段之间不再逐步询问“是否继续下一步”。

- 数字序号阶段按 `2-编剧 -> 3-导演 -> 4-表演 -> 5-摄影 -> 6-分组` 串行执行；上一阶段所有目标分集通过汇流门后，才启动下一阶段。
- 无序号同级子技能包默认全选并发执行；本技能没有无序号业务子包时，该规则仅作为未来扩展边界，不改变当前 2-6 阶段链。
...
- 阶段 subagent 的输入固定为上一阶段 canonical 产物；例如 `3-导演` 消费 `2-编剧/第N集.md`，`6-分组` 消费 `5-摄影/第N集.md` 与 `0-初始化/north_star.yaml`。
...
- 指定 AIGC 影视项目和一个或多个分集，要求从 `2-编剧` 连续推进到 `6-分组`。
- 指向 `projects/aigc/<项目名>/1-分集/第N集.md` 的分集源文件，或已存在的中间阶段产物并要求从某个阶段续跑到 `6-分组`。
...
- `start_stage`: 默认 `2-编剧`；续跑时允许为 `3-导演`、`4-表演`、`5-摄影` 或 `6-分组`。
- `end_stage`: 默认且通常固定为 `6-分组`。
- `subagent_runtime`: 必须可真实启动后台隔离 subagents；否则阻断或降级报告，不执行主创正文。

Reject or clarify when:

- 项目根无法唯一定位，或分集选择会匹配到多个项目。
- 请求跨越 `7-设计`、`8-图像`、`9-视频` 或 `10-审片`，但仍要求使用 `sword6` 作为唯一编排器。
...
| mode | trigger | route |
| --- | --- | --- |
| `bounded_episode_chain` | 单集或少量明确集数从 `2-编剧` 推到 `6-分组` | 加载 `types/bounded-episode-chain/bounded-episode-chain.md` |
| `episode_batch_chain` | 多集批处理，每阶段一集一个 subagent 并发 | 加载 `types/episode-batch-chain/episode-batch-chain.md` |
| `retry_from_stage` | 某阶段失败后从指定阶段续跑到 `6-分组` | 加载 `types/retry-from-stage/retry-from-stage.md` |
...
flowchart TD
    ...
    G2 -->|"Yes"| S3["Stage 3 导演: one subagent per episode"]
    S3 --> G3{"all episode gates pass?"}
    G3 -->|"No"| R3["stop or retry failed episodes"]
    G3 -->|"Yes"| S4["Stage 4 表演: one subagent per episode"]
    S4 --> G4{"all episode gates pass?"}
    G4 -->|"No"| R4["stop or retry failed episodes"]
    G4 -->|"Yes"| S5["Stage 5 摄影: one subagent per episode"]
    S5 --> G5{"all episode gates pass?"}
    G5 -->|"No"| R5["stop or retry failed episodes"]
    G5 -->|"Yes"| S6["Stage 6 分组: one subagent per episode"]
    S6 --> G6{"final gate pass?"}
    G6 -->|"Yes"| O["workflow ledger + completion report"]
    G6 -->|"No"| R6["repair route or blocked report"]
```
...
3. 运行 `steps/sword6-workflow.md#N1-PREFLIGHT`：确认每集上游输入存在，确认 `2-编剧` 到 `6-分组` 阶段技能可加载。
...
| `FIELD-SWORD6-03` | stage handoff | `references/stage-handoff-contract.md` | 2->3->4->5->6 输入输出链 | `FAIL-SWORD6-HANDOFF` |
...
- Required output: workflow run ledger、每阶段 dispatch packet、completion report；阶段 subagents 另按阶段合同写入 `2-编剧` 到 `6-分组` 的逐集 canonical 产物。
...
- Completion gate: 所有目标分集在 `6-分组` 产物存在且通过对应阶段 gate；若未全部通过，completion report 必须列出失败集、失败阶段、阻断原因和 retry/repair route。
