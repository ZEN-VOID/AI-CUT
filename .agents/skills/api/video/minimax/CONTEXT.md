...
| --- | --- | --- | --- | --- | --- |
| `TM-MINIMAX-DEFAULT-MODEL-DRIFT` | 新增更高 Hailuo 版本后默认模型仍停在旧值 | 默认值治理层 | 把默认值改回父级 `../runbooks/default-model-policy.md` 的 `highest-available-general` 规则族 | 只在 `KNOWN_MODELS` 维护模型集合，脚本通过 `../shared/default_model_policy.py` 统一推导 `DEFAULT_MODEL`，MiniMax 子技能只声明“Hailuo 家族参与默认值解析”这一过滤条件 | 新增更高版本模型后，无需改 CLI 默认值常量即可自动前进 |
...
- 当同一家族模型会持续涨版本时，默认值不该再写成单点常量；更稳的做法是先回到父级共享 runbook，再由共享 helper + Hailuo 家族过滤条件解析当前最高版本，文档只同步当前解析结果。
