#!/usr/bin/env python3
"""
Man-Tui Grok async video CLI.

Supports:
- create: submit a video task via multipart/form-data
- status: query task status, optionally wait until terminal state
- content: fetch final video binary, following redirects when needed
"""
...
try:
    ...
    print(
        "❌ 缺少依赖，请先执行: "
        "pip install -r .agents/skills/api/man-tui/video/grok/requirements.txt"
    )
    sys.exit(1)
...
def _redact_url(url: str) -> str:
    ...
    for chunk in split.query.split("&"):
        ...
        if key.lower() in {"key", "api_key", "token"}:
...
def _load_grok_connection_json() -> Dict[str, Any]:
    raw = os.getenv("MAN_TUI_GROK_CONNECTION_JSON") or os.getenv("MAN_TUI_GROK_CHANNEL_CONN")
    if not raw:
        return {}
    try:
        parsed = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise RuntimeError("MAN_TUI_GROK_CONNECTION_JSON 不是合法 JSON") from exc
    if not isinstance(parsed, dict):
        raise RuntimeError("MAN_TUI_GROK_CONNECTION_JSON 必须是 JSON 对象")
    return parsed


def _load_api_key(explicit: Optional[str]) -> str:
    conn = _load_grok_connection_json()
    key = (
        explicit
        or os.getenv("MAN_TUI_GROK_API_KEY")
        or os.getenv("MAN_TUI_GROK_KEY")
        or (str(conn.get("key")).strip() if conn.get("key") else None)
        or os.getenv("MAN_TUI_API_KEY")
    )
    if not key:
        raise RuntimeError(
            "缺少 API Key，请在根目录 .env 中配置 "
            "MAN_TUI_GROK_API_KEY / MAN_TUI_GROK_CONNECTION_JSON / MAN_TUI_API_KEY，"
            "或显式传 --api-key"
        )
    return key
...
def _load_base_url(explicit: Optional[str]) -> str:
    ...
    base = (
        explicit
        or os.getenv("MAN_TUI_GROK_API_BASE_URL")
        or os.getenv("MAN_TUI_GROK_BASE_URL")
        or (str(conn.get("url")).strip() if conn.get("url") else None)
        or os.getenv("MAN_TUI_API_BASE_URL")
        or DEFAULT_BASE_URL
    )
    return base.rstrip("/")
...
def _resolve_output_dir(
    ...
    return DEFAULT_OUTPUT_ROOT / resolved_project / "5-API" / "video" / "man-tui" / "grok"
...
def _default_report_path(command: str, output_dir: Path) -> Path:
    ...
    return output_dir / f"man_tui_grok_{command}_report_{_now_stamp()}.json"
...
def _normalize_quality(value: str) -> str:
    ...
    if value not in QUALITY_CHOICES:
        raise ValueError(f"quality 非法，允许值: {sorted(QUALITY_CHOICES)}")
    return value
...
def _normalize_image_reference(
    ...
    url_list = [item.strip() for item in urls if item and item.strip()]
    if url_list and raw_json:
        raise ValueError("--image-reference-url 与 --image-reference-json 不能同时使用")
    if raw_json:
        parsed = json.loads(raw_json)
        if not isinstance(parsed, list) or not all(isinstance(item, str) and item.strip() for item in parsed):
            raise ValueError("--image-reference-json 必须是非空字符串 URL 数组")
        return json.dumps(parsed, ensure_ascii=False, separators=(",", ":"))
    if url_list:
        return json.dumps(url_list, ensure_ascii=False, separators=(",", ":"))
    return None


def _create_request_summary(args: argparse.Namespace, image_reference: Optional[str], output_dir: Path) -> Dict[str, Any]:
    return {
        "model": args.model,
        "prompt": args.prompt,
        ...
        "input_reference": str(Path(args.input_reference).resolve()) if args.input_reference else None,
...
def _build_content_output_path(
    task_id: str,
    output: Optional[str],
    output_dir: Path,
) -> Path:
...
def create_task(
    ...
    image_reference = _normalize_image_reference(args.image_reference_url, args.image_reference_json)
    if args.input_reference and image_reference:
        raise ValueError("input_reference 与 image_reference 互斥，请二选一")
    ...
    if args.input_reference:
        input_path = Path(args.input_reference)
        if not input_path.exists():
            raise FileNotFoundError(f"input_reference 不存在: {input_path}")
        file_handle = input_path.open("rb")
        files["input_reference"] = (input_path.name, file_handle, "application/octet-stream")

    request_summary = _create_request_summary(args, image_reference, output_dir)
    if args.print_request or args.dry_run:
        print(json.dumps(_redact_obj(request_summary), ensure_ascii=False, indent=2))
    if args.dry_run:
    ...
    try:
        ...
        raise RuntimeError(f"HTTP {exc.response.status_code if exc.response else 'ERR'}: {body}") from exc
    finally:
        if file_handle is not None:
            file_handle.close()
...
def wait_for_task(
    ...
    while True:
        payload = fetch_status(base_url, api_key, task_id, timeout=timeout)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Man-Tui Grok async video CLI")
    ...
    create.add_argument("--quality", default=DEFAULT_QUALITY, help="清晰度档位")
    create.add_argument("--input-reference", help="本地参考图路径")
    create.add_argument("--image-reference-url", action="append", default=[], help="远程参考图 URL，可重复传入")
    create.add_argument("--image-reference-json", help="远程参考图 URL 数组 JSON 字符串")
    ...
    content = subparsers.add_parser("content", help="获取生成的视频内容")
    content.add_argument("task_id", help="任务 ID")
    content.add_argument("--api-key", help="显式 API Key")
    content.add_argument("--api-base-url", help="显式 API 基础地址")
    content.add_argument("--timeout", type=int, default=DEFAULT_TIMEOUT, help="HTTP timeout 秒数")
    content.add_argument("--project-name", help="项目名，用于默认输出目录")
    content.add_argument("--task-kind", default="test", help="project / test / temp")
    content.add_argument("--output-dir", help="输出目录")
    content.add_argument("--output", help="输出视频路径")
    content.add_argument("--report-json", help="报告 JSON 路径")
    ...
    try:
        if args.command == "create":
            ...
            if args.wait:
                ...
                if args.download_on_complete and wait_result.get("ok"):
                    output_path = _build_content_output_path(task_id, args.output, output_dir)
                    download_result = download_content(
                        base_url,
                        api_key,
                        task_id=task_id,
        ...
        if args.command == "status":
            ...
            if args.wait:
                ...
                payload = fetch_status(base_url, api_key, args.task_id, timeout=args.timeout)
                result = {
                    "ok": True,
                    "command": "status",
                    "task_id": args.task_id,
            ...
            if args.download_on_complete:
                ...
                if status != "completed":
                    raise RuntimeError("任务尚未 completed，不能自动下载")
                output_path = _build_content_output_path(args.task_id, args.output, output_dir)
                result["download_result"] = download_content(
                    base_url,
                    api_key,
                    task_id=args.task_id,
                    output_path=output_path,
        ...
        if args.command == "content":
            ...
            report_path = Path(args.report_json) if args.report_json else _default_report_path("content", output_dir)
            _write_json(report_path, result)
            print(json.dumps(_redact_obj(result), ensure_ascii=False, indent=2))
            print(f"📄 report: {report_path}")
            return 0
