...
groups:
  - group_id: x-y-z
    ...
    registry_path: projects/aigc/<项目名>/7-视频/libTV画布流/libtv-canvas-active-registry.json
    manifest_path: projects/aigc/<项目名>/7-视频/libTV画布流/第N集/x-y-z-subject-reference-manifest.json
    submit_plan_path: projects/aigc/<项目名>/7-视频/libTV画布流/第N集/x-y-z-libtv-submit-plan.json
    queue_record_path: projects/aigc/<项目名>/7-视频/libTV画布流/第N集/x-y-z-queue-record.json
...
- Output path: 默认不落本地视频生成物；证据工件进入 `projects/aigc/<项目名>/7-视频/libTV画布流/第N集/`，active registry 位于 `projects/aigc/<项目名>/7-视频/libTV画布流/`；显式下载时进入同集目录。
