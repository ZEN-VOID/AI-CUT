...
| 输出像影评，没有下游可消费字段 | 阶段桥接层 | 把观察拆成 `0-初始化 visual style bridge`、`2-编导 bridge`、`3-运动 bridge`、`4-摄影 bridge` 与 `6-设计 bridge` | 模板固定五份项目解析字段 | 下游可直接作为上下文喂给 owning stage |
...
| 给 `4-摄影` 的包只有参数堆叠 | 摄影消费层 | 转成 `visual_unit -> beat_map -> camera_grammar_plan -> functional_projection_payload` | 模板固定 shot design seed，而不是参数清单 | 可改写为自然中文 `分镜明细：` |
...
6. 对 `0-初始化` 只给 north star 风格块的补强语法；对 `2-编导` 只给戏剧、表演、调度和可拍承托；对 `3-运动` 给起点、路径、终点、参照系和前后画面状态承接；对 `4-摄影` 再给景别、运镜、构图、光色和下游 payload；对 `6-设计` 只给角色/场景/道具可迁移视觉原则和画面合同边界。
...
- `0-初始化` 需要的是“全片共同画面基调如何稳定继承”；`2-编导` 需要的是“角色在空间中如何被迫行动”；`3-运动` 需要的是“角色从哪里、沿什么路径、到哪里、相对什么运动”；`4-摄影` 需要的是“摄影机如何把行动变成观看路径”；`6-设计` 需要的是“资产如何承载身份、空间和物证压力”。
- 新输出命名里，`全局风格解析.md` 承接 global-style 字段逻辑，`编剧风格解析.md` 承接戏剧/表演/调度，`运动风格解析.md` 承接运动五要素和连续性，`摄影风格解析.md` 承接镜头语法，`设计风格解析.md` 承接角色/场景/道具资产，`分镜脚本.md` 承接 Numbers 表格。
