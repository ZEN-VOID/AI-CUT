...
| `scene-design-reviewer` | Scene Design 字段、空间结构、材质、按类型选择的空间风格、可制作资产 | 缺空间结构、缺类型匹配风格、非建筑场景被强行建筑化、不可制作 |
| `cinematography-reviewer` | 摄影字段、构图、光线、镜头逻辑 | 与场景无关、泛电影感、缺光线/镜头 |
| `prompt-reviewer` | 英文 prompt 主体 ID 开头、画面基调、场景风格、有来源姿态的时间与地域锚点、类型匹配空间风格 token、`prompt_evidence_chain`、字符数 | 非英文、未以主体 ID 号开头、超 2000 characters、缺引用、缺时间或地域、编造具体锚点、关键 token 无证据链 |
...
| prompt | 英文、以主体 ID 号开头、<= 2000 characters、承接 `画面基调.Global Style Prompt + 场景风格.Scene Style Prompt`、有来源姿态的时间和地域、类型匹配空间风格 token，并显式固定纯空镜/no people；prompt 前缀与解构主体 ID、提示词设计主体 ID 完全一致；最终英文整合 prompt 已吸收 `## 4. 解构` 的全部有效 Scene Design 与 Cinematography 信息，而不是只补前缀/后缀 |
...
| `GATE-SCENE-DESIGN-06` | `prompt` / `design_output_contract` | 最终英文 prompt 必须以同一主体 ID 开头，显式包含 `画面基调.Global Style Prompt + 场景风格.Scene Style Prompt`、有来源姿态的时间锚点和地域锚点、类型匹配空间风格 token，<= 2000 characters，并整合 `## 4. 解构` 的全部有效 Scene Design 与 Cinematography 信息；不得编造具体锚点或强行建筑化 | `FAIL-SCENE-DESIGN-06` | `N6-DESIGN` | prompt 字符数、开头 ID、时间/地域 token、space_style_token、解构槽位覆盖记录 |
...
- 英文提示词没有以主体 ID 号开头。
- 英文提示词缺少显式时间 token、地域 token 或类型匹配空间风格 token，或时间/地域/空间风格 token 不能通过 `prompt_evidence_chain` 回指来源姿态、推断或不确定性处理。
- 英文提示词为满足显式 token 要求编造具体年代、地点、族群、宗教、建筑流派或建筑大师名，或给自然/超现实/交通/抽象空间强行套建筑风格。
