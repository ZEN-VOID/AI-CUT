...
| `boundary_review` | 每次阶段 PASS 前 | LLM 或人工检查每组显式 `分镜N（约X秒）` 累计是否接近 15 秒、通常是否落在约 12-18 秒、atomic unit 与对白/画面承托是否完整、过长组拆分复核和短组回填/例外理由；字数和对白数只作为辅助风险检查 |
...
| `GATE-GROUP-06` | 组内时长 | 每组以 `分镜N（约X秒）` 累计为主，优先接近约 15 秒，通常约 12-18 秒可接受；低于约 10 秒必须有回填复核或短场景例外理由，超过约 18 秒必须有拆分复核或完整 atomic unit 例外理由；超过约 22 秒仅允许单个 atomic unit 本身过长 |
...
| `FAIL-GROUP-05` | 组内显式时长累计明显偏离 15 秒且未复核、低于约 10 秒未回填、高于约 18 秒未拆分/声明完整性例外、超过约 22 秒硬风险线，或字数/对白风险失控 | `references/group-boundary-contract.md` |
