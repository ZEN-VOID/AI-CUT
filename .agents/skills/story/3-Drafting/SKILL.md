---
name: story-drafting
description: Use when story2026 needs a guided 3-Drafting stage router for chapter-native novel drafting, selecting the active execution lane before writing `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`.
governance_tier: lite
---

# 3-Drafting

`3-Drafting` 是 `story2026` 主链的章节正文阶段导引层。它不直接承载某个 provider 的长执行细则，而是负责把“写章节、续写、重写、局部修复、dry-run context pack”类请求路由到当前可执行子路径，并统一守住正文真源、加载顺序和输出门禁。

## Context Loading Contract

- 每次调用本技能时，必须同时加载同目录 `CONTEXT.md`。
- 必须回读 story 根层 `../SKILL.md` 与 `../CONTEXT.md`，先锁定 `story2026` 总线边界，再进入 `3-Drafting` 阶段路由。
...
1. 当前 drafting 请求是否属于 `3-Drafting`。
2. 当前请求应该进入哪个可执行 lane。
3. 当前章正文 canonical truth 是否仍是 `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`。
4. 进入子路径前必须先加载哪些上游 truth。
5. 子路径完成后如何把结果回接到 story 主链与 `4-Review`。
...
| lane | 状态 | 触发信号 | 入口 |
| --- | --- | --- | --- |
| `B-Doubao流` | default active | 用户只说 `$story-drafting`、`$story-drafting-doubao`、章节正文、按章创作、从 planning 直接写小说正文，或没有点名其它 provider | `B-Doubao流/SKILL.md` |
| `A-GPT原生` | active explicit | 用户明确要求 GPT 原生、本地会话直写、不要外部 provider，或需要先由当前 GPT/LLM 产出 canonical draft 再由脚本校验落盘 | `A-GPT原生/SKILL.md` |
| `C-Deepseek流` | active explicit | 用户明确要求 DeepSeek、deepseek-v4-pro、DeepSeek 思考模式、高推理 DeepSeek 写章，或要求本阶段内容创作部分调用 `.agents/skills/api/deepseek` | `C-Deepseek流/SKILL.md` |

默认规则：

- 未明确 provider 时，继续按 registry 当前配置选择 `B-Doubao流`。
...
- `projects/story/<项目名>/3-Drafting/第N卷/第N-1章.md`：存在时作为承接增强；不存在不得阻塞起稿。
...
- 输出路径被要求降格到平铺 `3-Drafting/第N章.md`、`正文/`、`Drafting/chNNNN/chapter-root.md` 或临时 sibling 文件。
- 用户点名某 lane，但该 lane 缺少可执行合同、脚本、模板或 provider key。
- 子路径生成内容缺完整 YAML frontmatter、必需字段或 `# 第N章｜章标题` 标题行，却要求静默写回。
...
1. 锁定项目根、卷号、章号与用户意图。
2. 读取 story 根层与本目录 `CONTEXT.md`，确认 canonical output 仍是 `3-Drafting/第N卷/第N章.md`。
...
6. 子路径完成后，确认业务真源写回 `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`，过程 artifacts 只作为证据链。
7. 若本章需要继续主链，下一跳进入 `4-Review`；若失败，按 Root-Cause 合同回到最早拥有该问题的层。

## Core Gates

- `3-Drafting` 只能消费 planning、cards、north-star、项目记忆与项目上下文，不得替上游真源补写设定。
- YAML frontmatter 必须包含 `写作模型`、`global_context`、`style_context` 与 `north_star_chapter_brief`；`写作模型` 只能取 `GPT`、`Doubao`、`Deepseek`，并必须与所选 lane 一致。
- 正文主体必须是中文小说 prose，不得把 planning 标题、任务线或规避条目原样贴成正文。
- 业务真源路径固定为 `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`。
- lane artifacts 可落到 `projects/story/<项目名>/reports/3-Drafting/<lane>/.../`，但它们不是业务真源。
- `4-Review` 以前的 drafting 完成只表示 candidate draft 完成，不等于 validated final draft。
...
| Output path | 业务真源固定写入 `projects/story/<项目名>/3-Drafting/第N卷/第N章.md`；lane artifacts 仅可作为证据链写入 `projects/story/<项目名>/reports/3-Drafting/<lane>/.../`。 |
