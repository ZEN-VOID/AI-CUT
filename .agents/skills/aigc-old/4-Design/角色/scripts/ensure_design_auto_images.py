#!/usr/bin/env python3
"""Ensure 4-Design domain Markdown files have same-stem built-in imagegen requests."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="扫描 4-Design 域级输出目录，补齐每个 Markdown 设计文件的同 stem 自动图。"
    )
    parser.add_argument("--project", help="项目名；用于自动发现 projects/aigc/<项目名>/4-Design/")
    ...
    parser.add_argument("--design-dir", action="append", default=[], help="直接指定 4-Design 输出目录，可重复传入")
...
def discover_design_dirs(args: argparse.Namespace) -> list[Path]:
    ...
    if args.project:
        root = REPO_ROOT / "projects" / "aigc" / args.project / "4-Design"
        if root.exists():
            dirs.append(root)
    deduped: list[Path] = []
    seen: set[str] = set()
...
def markdown_files(design_dir: Path) -> list[Path]:
    return sorted(
        ...
        if path.is_file() and not path.name.startswith("_") and not path.name.endswith("清单.md")
    )
