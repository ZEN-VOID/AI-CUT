...
| --- | --- | --- | --- | --- | --- | --- |
| `group-multi-image` | `types/group-multi-image/group-multi-image.md` | 三段式 group_id、四段式 shot_id、GPT-IMAGE-2 多图、一次生成多张单独分镜画面 | default | `types/group-multi-image/group-multi-image.md` | none | none |

## Default Package Rule

- 默认加载 `group-multi-image`，因为本技能的业务对象固定为 `10-分组` 普通分镜组。
...
| `single_group_generate` | 指定一个 `group_id` 或组内 `shot_id` 出图 | 项目名、目标组 | yes | 一组一次多图、`n == shot_count`、输出映射 |
| `episode_group_generate` | 整集批量出图 | 项目名、集号 | yes | 每个普通组一个 multi_image_task，连接件跳过 |
...
| `execution_type` | `prompt_only` / `generate` / `review_only` | 决定是否进入 GPT-IMAGE-2 handoff |
| `group_source_state` | `complete_group_block` / `missing_group` / `connector_only` / `ambiguous_group` | 决定执行或阻断 |
| `shot_count_state` | `valid_1_to_provider_cap` / `zero` / `over_provider_cap` / `ambiguous` | 决定 `n` 和是否阻断 |
...
| `handoff_route` | `gpt_image_2_multi_image_group` / `prompt_only` / `blocked_provider_cap` | 决定执行路径 |
...
- 不要把 provider cap、输出路径或 completion gate 只写在类型包里。
