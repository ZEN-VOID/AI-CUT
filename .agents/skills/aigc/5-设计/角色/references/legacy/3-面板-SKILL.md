...
description: Use when `5-设计/3-面板/角色` needs to consume role design artifacts under `projects/aigc/<项目名>/4-设计/角色/2-设计/`, directly reuse the prompt portion, emit CharacterPanel layout JSON, and automatically call `内置 $imagegen / image_gen`.
...
`5-设计/3-面板/角色` 是角色设计产物的面板化与自动生图入口。

它完全参照 `/Volumes/AIGC/AIGC-ZEN-VOID/.agents/skills/aigc2026/3-设定/4-面板/角色面板` 的核心配置口径，并按当前仓库 `5-设计` 新 runtime 改写落点：

- 上游：`projects/aigc/<项目名>/4-设计/角色/2-设计/第N集/`
- 输出：`projects/aigc/<项目名>/4-设计/角色/3-面板/第N集/`
...
- `.agents/skills/aigc/5-设计/2-设计/角色/SKILL.md`
...
- `projects/aigc/<项目名>/4-设计/角色/3-面板/第N集/`
...
1. `continuous-batch`
   - 触发：只提供 `--project + --episode`、或父级 `5-设计` 批量调度。
...
3. `.agents/skills/aigc/5-设计/3-面板/_shared/smart-image-handoff-contract.md`
4. 本 `SKILL.md`
