...
| --- | --- | --- | --- | --- | --- |
| `N1-SCOPE` | 项目路径、集号范围 | 是否能定位 `10-分组/第N集.md` | 建立输入 manifest | `source_manifest` | 输入文件可读 |
...
sequenceDiagram
    ...
    participant Source as 10-分组 YAML
...
若 YAML `道具` 字段缺失或格式异常，停止生成 canonical 清单，输出风险报告或要求先修复 `10-分组`。
