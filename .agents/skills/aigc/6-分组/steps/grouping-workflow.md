...
| `success_criteria` | 每组 ID 真实、每组开头有定场镜头，且定场先建立场景身份和环境身份，再把氛围、光影、色彩、角色、群体、关键道具和场景锚点整理为带属性和站位的构图分区主体账本；第二组起定场镜头在保留站位关系的基础上增量添加上一组尾帧还原入场并牵引人物动作链、风格投影含置顶于第 1 行最前的全局固定前置词、正文保真、组间连接自然、统计 YAML 可复查 |
...
| `N5-GROUP-PLAN` | 裁决组边界 | scene unit table、style projection | 按显式时长累计形成组计划：优先接近 15 秒，通常约 12-18 秒可接受；低于约 10 秒做回填复核，超过 18 秒必须拆分、重组或回退 `5-摄影` 修复，不能例外放行；字数/对白只作辅助风险检查 | group boundary plan | `N5E-SUBJECT-EVIDENCE` | 每组时长接近 15 秒、完整且 `<=18` 秒 |
...
| `N5A-ESTABLISHING-SHOT` | 为每组重建开头定场镜头并落盘构图分区主体账本 | subject layout evidence table、composition_axis_profile、group boundary plan、上一组末帧、组间连接件预期首态、`references/group-establishing-shot-contract.md`、`../_shared/scene-shot-identity-contract.md`、`../_shared/action-first-continuity-contract.md` | 为每个分镜组写 `定场镜头：`，先覆盖场景身份和环境身份，包括年代/时代、社会语境、空间功能、固定锚点、材质、氛围、光影、色彩和声音底色；随后写 `画面构图：` 概述起始画面的主体布局逻辑，再按实际画面使用 `左侧：`、`中间：`、`右侧：`、`前景：`、`中景：`、`背景：` 等分区落盘主体。每个重要主体必须带形容属性、状态属性、站位属性、姿态、朝向/视线和与其他主体或关键道具的关系；不另列 `角色：` / `场景：` / `道具：` 组头清单。每集第二组起在保留上述站位关系的基础上增量添加上一组尾帧还原入场，具体复现上一组最后一个原始分镜或连接件抵达后的主体位置、姿态、朝向、视线、身体接触、道具关系、光线、烟雾、水纹、碎片、衣袍或发丝等可见状态，并说明本组如何从同一状态以不同景别、机位、焦点、构图或观看距离进入；连续同场景组也重新输出；不写成单纯人物动作摘要，不新增剧情、人物、道具互动或无互动道具氛围镜头 | group establishing shot list、composition_partition_ledger、tail_frame_replay_entry、scene_group_identity_profile、action_first_continuity_check | `N4-VISUAL-TONE` | 每组都有定场镜头、`画面构图：` 和至少两个有内容的构图分区；主体位置能回指上游、本组正文、上一组末帧或组间连接件证据 |
...
| `N7-ASSEMBLE` | 组装分组稿 | group plan、group_establishing_shot list、composition_partition_ledger、group_visual_tone list、connector list、style projection、scene title list | 每个分镜组标题后先写当前场景标题行，再写定场镜头、画面构图和构图分区，再写画面属性语句（从 `group_visual_tone` 提炼），再写组头（north_star 三项风格行）、原正文、YAML 统计，并把组间连接件物理夹放在对应上下两个相邻分镜组之间；场景标题行、定场镜头、画面构图、构图分区和画面属性语句计入 YAML `字数统计`，不计入 `时长估算`；YAML fenced block 本身不计入字数；YAML `道具` 先做同物识别、归一合并和去重 | episode group draft | `N8-REVIEW` | 正文同步原换行；道具统计不过细、不重复 |
...
| 定场镜头缺失、未先建立场景/环境身份、主体信息退化为 `角色/场景/道具` 清单、缺少 `画面构图：` 或构图分区、主体未带形容属性/状态属性/站位属性、未覆盖全部角色站位姿态朝向、第二组起未在保留站位关系的基础上增量添加上一组尾帧还原入场、未牵引人物动作链，或新增无互动道具氛围镜头 | `N5E-SUBJECT-EVIDENCE` -> `N5A-ESTABLISHING-SHOT`，回上一组末帧、本组正文和上游分镜重建身份明确的连续空间起始状态 |
