...
description: Use when `5-设计/1-清单/场景` needs to consume `projects/aigc/<项目名>/3-Detail/第N集.json`, normalize canonical scene families plus shot-level evidence, and emit `场景清单.json + 场景研究.json + scene_design_bridge.json`.
...
`5-设计/1-清单/场景` 是 `5-设计` 阶段承接 `3-Detail` 的场景清单 leaf。
...
2. 服从当前仓 `5-设计/1-清单/_shared/list-output-contract.md` 的单真源治理。
...
- 当前 skill 是 `5-设计/1-清单` 下的场景 leaf。
...
5. `.agents/skills/aigc/5-设计/1-清单/_shared/list-output-contract.md`
...
11. 已存在的 `projects/aigc/<项目名>/4-设计/场景/1-清单/第N集/场景清单.json`（若存在）

## Business Requirement Analysis Contract (Mandatory)

| analysis_slot | 当前结论 |
| --- | --- |
| `business_goal` | 把 `3-Detail` canonical `detail.分镜列表[].氛围表现 + 分镜构图 + 摄影表现` 收束成可追溯、可研究、可桥接的场景三真源，并仅在必要时回退 compat `角色背景面 + 分镜表现 + 时间段 + 导演意图`，供 `5-设计/2-设计/场景` 直接消费。 |
| `business_object` | `projects/aigc/<项目名>/3-Detail/第N集.json`、`projects/aigc/<项目名>/4-设计/场景/1-清单/第N集/{场景清单.json,_manifest.json}`。 |
...
- 已存在的 `projects/aigc/<项目名>/4-设计/场景/1-清单/第N集/场景清单.json`
...
- `projects/aigc/<项目名>/4-设计/场景/1-清单/第N集/`
