...
| produced downstream | 已产出的后续章、润色稿、sidecar 是否要同步 | `3-初稿/`、`4-润色/`、provider reports |
| future constraints | 后续未产出但 planning/handoff 已预设的内容是否要改 | 后续章规划、卷末兑现、下一卷 handoff |
| review and actualization | 已 PASS 或已回流的事实是否要失效或重验 | `review/`、`return/`、`STATE.json` |
...
| 角色动机 / 关系 / 状态 | 角色卡、关系字段、人物历史、相关场景/物品卡、章级人物任务 | 角色最近出场章、关系转折章、上一章情绪余波 | 后续互动章、阵营变化、润色稿、人物一致性 review | 声纹、动机、关系动作和状态历史一致 |
| 物品 / 技能 / 规则机制 | 物品卡、技能卡、世界规则、north_star 边界、章级任务 | 首次出现、规则解释点、最近使用点 | 后续使用/兑现章、战斗/解谜/交易场景、卷末机制 payoff | 机制不变成外挂；代价、限制、持有人和证据链一致 |
| 场景 / 地点 / 空间动线 | 场景卡、地图/路线、章级场景义务、历史地理边界 | 上一次到达/离开、当前章前一场、空间封锁或入口点 | 后续移动、追逃、战场/旅程节点、时间线 review | 人物位置、路线耗时、空间关系成立 |
| 章节事件 / 情节事实 | 章规划、卷规划、整体规划、上一章结尾、相关对象卡 | 事件因果前置、同卷前序章、冲突触发点 | 后续承接章、章节钩子、review aggregate、return actualization | 事件因果、时间线、任务兑现和章末牵引一致 |
...
| 叙事结构 / 卷章拓扑 | 整体规划、卷规划、章级规划、STATE、handoff、review aggregate | 前后章任务分配、卷内节奏、已完成章节窗口 | 后续章节编号、卷末兑现、return carryover、下一卷起点 | 章节数、卷标签、任务分配和 validated actual 不漂移 |
| 已 PASS 终稿事实 / accepted actual | review aggregate、return actualization、accepted manuscript refs、STATE | 被验收卷内的相关章节、终稿引用链 | 项目 CONTEXT carryover、下一卷 handoff、后续正文/润色稿 | 必须重验、失效化或明确保留旧验收；不得静默覆盖 |
| 局部语体 / 风格 / 表达分布 | MEMORY、north_star.style_contract、风格卡、provider lane 合同 | 同章前后句群、同卷气口、角色声纹表 | provider prompt/messages、润色稿、AI 检测风险上下文 | 不破坏事实；不把局部润色扩大成整章重写；provider authorship 保持 |
...
3. 若改动会改变已经 PASS 的终稿事实，必须把 review aggregate 和 return actualization 纳入修复。
4. 若改动只影响表达风格而不影响事实，仍需检查 `MEMORY.md`、风格卡和 provider prompt 是否会把旧风格带回。
...
| --- | --- | --- | --- | --- |
| 本次 repair 是否输出了覆盖 upstream truth、same-layer predecessor、current locality、downstream existing、future constraints、review/actualization 的 impact map？ | `impact_scope` | `FAIL-REPAIR-SCOPE` | `steps/repair-workflow.md#N2-IMPACT-MAP` | repair packet 中的 `impact_map` 六类 surface 与路径/状态/理由 |
...
| 已 PASS 或 return actualized 的事实被改动时，是否处理 review aggregate、return actualization 与 STATE？ | `accepted_truth` | `FAIL-REPAIR-AUDIT` | `steps/repair-workflow.md#N6-DOWNSTREAM-SYNC`、`steps/repair-workflow.md#N8-REVIEW-GATE` | review/return/state action、重验/失效/保留理由 |
