...
本文件是 `8-视频/B-分镜故事板参照` 的经验层知识库，不是过程日志。调用本技能时，应在父级 `8-视频` 合同之后加载本文件。
...
| --- | --- | --- | --- | --- |
| 视频 prompt 被脚本拼接改写，偏离 `5-分组` 原文 | LLM 主创层 | 停止脚本主创，改为 LLM 保真组织并直接嵌入组正文 | `SKILL.md` 和 `references/group-source-contract.md` 固定“现有内容为主体” | prompt 可回放完整组正文 |
...
| OSS 上传和视频图1槽位被混为一谈 | 上传身份层与生成槽位层混淆 | 将 `storyboard_uploads: group_id/storyboard_sheet -> uploaded_url` 和 `generation_slots: 图1/imageList[0] -> uploaded_url -> 故事板总参照` 分开；用实际 `imageList[0]` 回刷 YAML `reference_index` | `libtv-handoff-contract.md` 固定 OSS 上传不承载图N顺序真源 | `reference_index=1` 可由 `generation_slots` 反查，不依赖上传动作 |
| 视频生成框 UI 图1与本地预期或旧 query 回显不一致 | UI 槽位真源层 | 以 UI 图1 / `Image 1` 为最终 `generation_slots`，用 URL 反查 `storyboard_uploads` 后回刷 final YAML | `libtv-handoff-contract.md` 固定 UI 槽位优先级 | final YAML 的 `reference_index=1 / uploaded_url` 与 UI 图1一致 |
| 初始 prompt 提前写入故事板 `reference_index / uploaded_url`，后续回刷困难 | 槽位相位混淆层 | draft prompt 只保留原 YAML；确认 UI 图1或最终 imageList 后再回刷 `reference_index / uploaded_url / image_token` | `SKILL.md` 固定 draft/final 两相位 | draft 无空 URL/占位 URL；final 按 `generation_slots` 排序 |
...
| 所有分镜组都被固定提交为 15 秒，短组动作拖长或节奏失真 | 时长投影层 | 回到 `5-分组` 当前组 `时长估算`，重建 `duration_estimate_seconds` 与 `duration_hint`；按 `clamp(估算, 4, 15)` 写入 batch 和远端提交 | `group-source-contract.md` 与 `libtv-handoff-contract.md` 固定组级时长估算与 4-15 秒 clamp | batch 有 `duration_source / duration_estimate_seconds / duration_hint`，远端 `duration` 与 `duration_hint` 一致 |
...
| 生成结果有视频 URL 但无音频证据 | 音频控制面缺失层 | 若 CLI 路径无法在生成前验证 `create_generation_task.params.enableSound`，记录 `audio_preflight_unverified_non_blocking` 并继续提交；生成后无音频证据则 `audio_missing` | `libtv-handoff-contract.md` 固定非阻断音频预检和下载后 `ffprobe` 验收 | 远端提交写 `enableSound=on`；生成后 `task_result.audios` 非空或 `ffprobe` 检出 audio stream |
...
2. 再查 `projects/aigc/<项目名>/5-分组/第N集.md` 是否可读且目标 `group_id` 唯一。
...
8. 若远端代理把 B 任务解释成“先生成故事板图 / 拆分 panel / image2video / 多段合成”，先修 `*-libtv-submission.txt` 的 `singleImage2video` 调用锁，再重新提交，不要补跑 `7-图像/B-分镜故事板`。
9. 若远端 `params.prompt` 只剩裸图片 token 或裸图片编号，说明 fenced YAML 的故事板绑定没有进入 prompt 完整体；必须把 `【直接生成请求】` 改成基于下方 `【分镜组源文本】`，并要求原正文 + final YAML `故事板参照.reference_index / uploaded_url / image_token` 一起进入 prompt。
...
11. 若 batch 或远端提交把所有组固定为 15 秒，先查 group index 是否保留 `时长估算`；缺失时回 `5-分组` 重新提取。小于等于 4 秒统一用 4 秒，大于等于 15 秒统一用 15 秒，中间值用估算值。

12. 若远端提交包含缺故事板、多候选未裁决、未入预算或空槽说明，先从 manifest 重投影提交文本；这些说明只留本地报告，不进入 LibTV prompt。
13. 若当前调用面不能在生成前直接控制或验证 `params.enableSound`，记录 `audio_preflight_unverified_non_blocking`，不要阻断官方 `$libTV` Agent-IM 提交流程；生成后必须用 `task_result.audios` 或 `ffprobe` 做硬验收。

## Reusable Heuristics

- 本技能的关键不是“重新写一个视频故事”，而是把已经在 `5-分组` 中裁好的完整组内容稳定交给 LibTV。
...
- “参照图 URL”这个说法容易让远端把故事板当成匿名图片。B 远端请求应始终让 `【分镜组源文本】` 的 final fenced YAML 持有 `故事板参照.reference_index / uploaded_url / image_token` 绑定；不得另起故事板参照说明段，也不得人工预设 `参照图1/2/N` 编号。
- B 路线也必须分清上传身份和生成槽位：`storyboard_uploads` 只回答“这张 OSS 图是哪一组的故事板总参照”，`generation_slots` 才回答“图1/imageList[0] 是哪张 OSS 图”。最终 YAML 的 `reference_index=1` 以 `generation_slots` 为准。
- 若视频生成框 UI 缩略图顺序可观测，UI 图1 / `Image 1` 优先级高于 OSS 上传动作和旧 query 回显；`prompt.md` 初始应保持 draft，不提前写 `reference_index / uploaded_url`。
...
- 当前 `$libTV` Agent-IM CLI 没有独立音频开关；`enableSound` 必须写入远端提交文本，生成前无法验证时只记录 `audio_preflight_unverified_non_blocking`，交付仍以后验音频证据为硬门。
