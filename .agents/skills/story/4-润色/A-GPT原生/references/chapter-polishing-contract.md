...
本文件承载 `story-polishing-gpt-native` 的章节润色稿细则。它不拥有入口路由权；调用时由根 `SKILL.md` 决定何时加载。

## Stage Position

- 当前技能是 `story2026` 主链 `4-润色` 阶段的 `A-GPT原生` 路径，不再通过 `正文/` child 承接。
- 当 `4-润色` 路由选择 `A-GPT原生` 时，当前技能拥有当前章润色稿根文件与 YAML 头的写权。
...
- `./scripts/polish_chapter_gpt_native.py`
...
3. YAML 头只保留 `润色模型: GPT` 与 `初稿来源`；禁止重复写入 planning 路径、cards 路径、项目上下文路径或 global/style/north-star 摘要。
...
14. `scripts/polish_chapter_gpt_native.py` 只能装配上下文、注入 `supervision_packet`、输出 sidecar、校验已创作稿与写回，不得直接生成正文。
15. 若 GPT 原生输出不含完整 YAML frontmatter、`润色模型` 不等于 `GPT`、缺少 `# 第N章｜章标题` 标题行，必须判定为 output invalid，禁止直接写回业务真源。
