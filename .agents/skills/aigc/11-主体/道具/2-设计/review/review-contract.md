...
- 初始化综合消费按 `../../../_shared/team-advisor-consultation-contract.md` 执行：只读消费项目 `team.yaml.init_synthesis.stage_seed_summary."11-主体"`、`init_handoff.design_seed` 或 `north_star.yaml.创作阶段不变量.设计`，形成 `init_team_synthesis_context` 后再进入单道具设计汇流；不得在本阶段解析监制 roster、请教顾问、调用 team 身份或补造顾问问答。
...
| context | 是否读取并消费 `north_star.yaml`、`team.yaml.init_synthesis`、项目记忆和相关上下文 |
...
| design_detail_culture | 道具是否好看、有设计感、有文化元素和可见细节；是否具备独特轮廓、材质记忆点、工艺/装饰细节、文化/身份符号、使用痕迹和功能结构；关键道具是否有 signature detail，而不是简单和平凡 |
| corpus_usage | 触发道具审美、文化元素、工艺装饰、功能结构、使用痕迹或 prompt 短语时，是否加载 `knowledge-base/prop-design-corpus.md`，并完成符合时代语境的原创转译 |
...
| scope | 是否只写入 `11-主体/道具/2-设计`，未触碰 registry、父级或其他技能 |
...
| `GATE-PROP-DESIGN-11` | init_team_synthesis | `FAIL-PROP-DESIGN-10` | 初始化综合存在但未形成 `init_team_synthesis_context`，或采纳内容没有绑定当前 `node_id / pass_id / gate_id` 并转成节点级判断、取舍、patch 或风险提示，或误触发 team 身份 / 旧 stage profile / 伪顾问问答 | `N5-RESEARCH-CHAIN` / `N7-REVIEW` | `init_team_synthesis_context`、`init_synthesis_node_coverage`、缺失原因 |
...
| `GATE-PROP-DESIGN-13` | design_detail_culture | `FAIL-PROP-DESIGN-DETAIL-CULTURE` | 道具只是普通功能物、缺少审美设计、文化元素、工艺/装饰细节、signature detail、材质记忆点或时代语境，导致画面平凡无辨识度 | `N5-RESEARCH-CHAIN` / `N6-DESIGN` | `design_detail_culture_evidence`、`Design Appeal Target`、`Signature Detail`、`Cultural Element Strategy`、prompt token |
| `GATE-PROP-DESIGN-14` | corpus_usage | `FAIL-PROP-DESIGN-CORPUS-MISSING` | 已进入单道具设计、批量设计、增量补缺或修复，且涉及审美、文化元素、工艺装饰、功能结构、使用痕迹或 prompt 短语，但未加载 `knowledge-base/prop-design-corpus.md` 或未说明原创转译与时代语境约束 | `N5-RESEARCH-CHAIN` / `N6-DESIGN` | `prop_corpus_usage_trace`、语料种子、原创转译说明、period/culture guardrail |
...
flowchart TD
    ...
    C --> A1["检查 init team synthesis packet"]
...
- [ ] 道具好看、有设计感、有文化元素和可见细节，包含独特轮廓、材质记忆点、工艺/装饰细节、文化/身份符号、使用痕迹和功能结构，不是简单功能还原或平凡物件。
- [ ] 文化元素、纹样、铭文、徽记、器型、封缄、材质和装饰符合项目时代、地域、阶层、职业、宗教/族群禁区与功能逻辑，没有错置现代奢侈品、街头潮牌、战术风、赛博朋克、哥特奇幻等元素。
- [ ] 触发审美、文化元素、工艺装饰、功能结构、使用痕迹或 prompt 短语时，已加载 `knowledge-base/prop-design-corpus.md`，并在 `Prop Corpus Usage Trace` 中说明原创转译。
...
- [ ] 初始化综合存在时，`init_team_synthesis_context` 已从 `team.yaml.init_synthesis` 中提炼出节点级可执行指导、局部 patch 或风险提示；缺失时已记录 `not_applicable` / `blocked`。
- [ ] 输出路径在 `projects/aigc/<项目名>/11-主体/道具/2-设计/`。
...
- 道具只是简单和平凡的功能物，缺少审美吸引力、设计细节、文化元素、signature detail 或时代语境。
- 触发道具审美、文化元素、工艺装饰、功能结构、使用痕迹或 prompt 短语时，未加载 `knowledge-base/prop-design-corpus.md`，或只是照搬语料而未完成原创转译。
