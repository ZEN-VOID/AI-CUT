...
| `全局风格` 用“空间/时间处理”“统一画面气压”这类抽象汇总词组织前缀，导致风格字段虽然正确但不够像工具可执行语言 | 风格组织层 | 把最终字段收束成“媒介属性 + 参照锚点 + 渲染技术栈 + 视觉美学范式 + 可执行视觉构词”，优先使用具象短语 | 在 `SKILL.md`、`IO_CONTRACT.md`、模板和示例项目中同步固化“参照锚点 + concrete lexicon”规则 | `全局风格` 提取字段更像可直接投喂工具的前缀，而不是人工总结句 |
| `全局风格` 仍偏“广义视觉风格”，没有把摄影层面的总体属性放在主位，导致它不够适合统摄全局 | 摄影统摄层 | 将最终字段收束成“媒介属性 + 摄影参照锚点 + 渲染技术栈 + 摄影级总体属性 + 可执行摄影构词”，优先写照明、反差、颗粒、皮肤层次、反射与暗部可读性 | 在 `SKILL.md`、`IO_CONTRACT.md`、模板和示例项目中同步固化“photography-first global attributes”规则 | `全局风格` 更像全项目摄影总则，而不是泛视觉说明 |
| team 模式已启用，但 `全局风格` 里看不出已锁定顾问阵容，导致 team 只在会审阶段可见、不会沉淀成风格真源 | team 风格锚点层 | 把 `team.yaml` 前移到 `N3A-GLOBAL-STYLE`，优先将已启用成员改写成“参照 XXX 的 XXX”式视觉锚点，并下钻成 AIGC 构词 | 在 `SKILL.md`、`IO_CONTRACT.md`、模板和示例项目里同步固化“team anchor -> concrete lexicon -> core prefix”路径 | team 模式一旦启用，`全局风格.md` 能自然留下 lineup 痕迹，而不是只在会审日志中出现 |
...
- `全局风格` 的叙事相关信息只能以视觉结果存在，例如空间气压、时间处理、湿度、反射、颗粒、光比与统一质感；不能把“人物关系结论”原句搬进统一前缀。
- `全局风格` 的最终字段宁可多用“潮湿夜色、玻璃反射、走廊遮挡、真实皮肤层次、低饱和暖金室内、轻颗粒”这类具体词，也不要用“空间/时间处理”“统一画面气压”这类二次概括词。
- 若用户强调“统摄全局的总体属性”，优先写摄影层的恒定属性：照明质感、反差、高光控制、暗部可读性、皮肤层次、颗粒纯度、反射处理、空间光比；这些比情绪形容词更稳。
- 面向 `Midjourney` 这类工具时，最稳的做法是把 canonical `全局风格` 维护成 tool-agnostic core prefix，再在 Markdown 里给出“前缀先放 core，再补渲染与质感修饰词”的投影建议；不要把 `--ar`、`--stylize` 之类参数写进真源字段。
- 若项目启用了 team 模式，`全局风格` 不应把 team 只当隐形背景；最稳的做法是直接用“参照《作品》的某种处理 / 参照某导演的某种组织 / 参照某摄影的某种光影与可拍性”来留下自然痕迹。
- team 痕迹不能停在只报名字。必须再下钻成 AIGC 工具能读懂的词组，例如“都市夜景反射层次”“时间回声停顿留白”“内容优先写实摄影”“真实皮肤层次与可拍空间光比”。
