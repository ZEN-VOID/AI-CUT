...
- 强情绪不是动作越多越真实；保留少数能改变状态差的外显动作，并让角色弧线、生理残留和声线变化延续到下一拍。
- 群戏层次先分功能再写动作。行动者推动事件，反应者承接冲击，压力源制造张力，背景参与者维持空间；所有人同强度表演会让镜头失焦。
...
- 表演阶段的 顾问与复核流程 只适合当监制顾问：它们可以指出表演风险、潜台词策略、身体变量和局部 patch，但 canonical 表演稿仍由主 agent 的 LLM 主创写回。
- 角色变化不靠旁白说'他变了'，靠的是同一种动作在不同集里做出不同选择——这就是弧线。
