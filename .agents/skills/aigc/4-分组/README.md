...
`4-分组` 将 `projects/aigc/<项目名>/3-摄影/第N集.md` 的逐集摄影稿切成完整分镜组，并为每组重复当前场景标题、拼接 north_star 风格字段和 YAML 统计；相邻分镜组之间额外生成 3-4 秒组间首尾帧连接件，连接件同样先标明同场景或跨场景标题关系，再继承三项 north_star 风格行，并把固定音频/字幕约束置顶于第 1 行风格行最前。
