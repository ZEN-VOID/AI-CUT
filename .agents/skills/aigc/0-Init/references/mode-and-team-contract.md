...
| --- | --- | --- | --- | --- |
| `策划` | `0-Init` | first direct-answer packet before synthesis | converge story core, emotional core, boundaries, and stage seeds | does not own later-stage canonical truth |
| `监制` | `2-Global`, `3-Detail`, `4-Design` | stage-front advisory | style, type, director intent, feasibility, design continuity | does not replace stage canonical writeback |
| `评审` | `5-Image`, `6-Video` | around validation reports | image/video consistency, reference binding, provider risk, delivery gate | not a default early creative expansion role |
...
3. Lock governance role ownership: `策划 -> 0-Init`, `监制 -> 2-Global/3-Detail/4-Design`, `评审 -> 5-Image/6-Video`.
