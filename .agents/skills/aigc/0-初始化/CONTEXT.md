...
| --- | --- | --- | --- | --- |
| 初始化继续生成 `north_star / init_handoff / story-source-manifest / team / STATE` 等旧五件套或治理载体 | scaffold-only 合同漂移 | 回到 `0-初始化/SKILL.md`，只创建当前 `0-初始化` 到 `10-画布` 目录、项目根 `MEMORY.md` 和项目根 `CONTEXT/README.md` | 在 `scope-and-runtime.md`、`steps/init-workflow.md`、`review/init-review-gate.md` 与审计脚本同步 denylist 旧初始化产物，并把 `CONTEXT/` 保留为允许项 | 新项目初始化后只有当前 0-10 目录、`MEMORY.md` 与 `CONTEXT/README.md` 是本轮写回产物 |
...
| north-star 风格字段仍按旧短前缀压缩 | 输出约束层 | 将 `全局风格提示词` 调整为通常 300-500 字；`类型元素提示词` 仍短；服装/建筑/物品保持简明域级摘要 | 在模板注释、artifacts 规则、review gate 和 audit marker 同步声明 | 初始化产物既能表达总风格全貌，又能被 `8-分组` 摘取为 300 字以内当前组风格语句 |
...
- `细分风格.画面风格` 是全片统一的画面摘要，不是唯一的场景别摄影表；可复用场景化光色策略优先放在 `全局风格`，下游 `8-分组` 再按当前分镜组摘取相关部分。
...
- 对创作起盘来说，最小闭环应先保证当前 0-10 scaffold、项目 `MEMORY.md` 和项目 `CONTEXT/README.md`；其余治理载体只有在复杂执行或卫星技能真正需要时再补。
