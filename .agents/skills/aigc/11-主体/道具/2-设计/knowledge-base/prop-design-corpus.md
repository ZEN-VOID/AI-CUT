...
本文件是 `道具/2-设计` 的高质量语料库，用于把“道具也要好看、充满设计细节和文化元素，而不是简单平凡”的要求转化为可执行的形制、材料、工艺、装饰、文化符号、使用痕迹和 prompt 词组。它是知识库，不替代 `SKILL.md` 的入口、门禁和输出合同。
...
usage_boundary:
  - "语料用于启发和原创转译，不逐字套用。"
  - "文化元素必须受项目时代、地域、阶层、职业、宗教/族群禁区和 north_star.yaml 约束。"
...
| --- | --- | --- |
| 不做普通物件 | 每个道具至少有一个独特轮廓、一个材质记忆点、一个工艺/装饰细节 | 写 `unique silhouette`, `signature detail`, `crafted surface` 等可见词 |
| 文化元素可见 | 把地域、时代、信仰、家族、职业、阶层转成纹样、铭文、符号、器型或工艺 | 写具体 motif，不只写 `cultural elements` |
| 细节服务功能 | 装饰、磨损、接口、开合、封缄、绑带、铆钉都应解释功能或叙事 | prompt 中保留 functional detail，不堆无意义 ornament |
| 美感与旧痕并存 | 道具可破损、古旧、危险，但仍要有设计秩序和视觉吸引力 | `aged but elegant`, `weathered crafted detail` |
...
| `cultural_motif` | 文化元素来自哪里？纹样、铭文、图腾、封印、家徽、祭祀符号是否合规？ | cloud-and-thunder motif, lotus engraving, seal-script inscription, family crest |
| `wear_trace` | 使用历史如何可见？磨损、包浆、划痕、修补、污渍、焦痕、折痕是否有秩序？ | polished worn edges, repaired crack, patina in grooves, ink stains |
...
| --- | --- | --- |
| 远看可识别，近看有信息 | 独特外轮廓、核心符号、隐藏结构、材料反差、手工痕迹、专属磨损 | 只写“神秘物件”“重要道具”，没有可见 signature |
...
| design target | details | avoid |
| --- | --- | --- |
| 美术外观强，现实操作弱 | 独特刃形/护手/鞘口/铆钉/包缠/磨损/仪式标记 | 可复现伤害结构、制造步骤、现实教程 |
...
| 信息可见但不泄密 | 局部铭文、符号化文字、封缄结构、磨损角 | 真实个人信息或长段可读文本 |
...
| design target | details | avoid |
| --- | --- | --- |
| 开合逻辑可见 | 锁扣、铰链、嵌片、边框、暗格暗示、磨损集中点 | 光滑盒子没有结构 |
...
| 功能决定美感 | 重心、把手、接口、磨损位置、便携方式 | 只堆装饰，不说明用途 |
...
| design target | details | avoid |
| --- | --- | --- |
| 小物件需要轮廓和材质记忆 | 吊坠外形、链节、镶嵌、流苏、缝线、佩挂方式、磨损点 | 只写漂亮首饰 |
...
| design target | details | avoid |
| --- | --- | --- |
| 机制暗示而非工程教程 | 外壳、接口、灯点、刻度、旋钮、封闭结构、磨损区 | 真实可复现电路/武器/危险机制 |
...
| 商贸/城市 | 编号、票据、封条、磨损金属、运输划痕、商号标识 | 避免现代品牌 logo |
| 宗教/仪式 | 对称、供奉痕迹、香灰、封印、象征纹样、礼制尺度 | 不随机混搭真实宗教符号 |
| 异域/边地 | 几何纹样、皮革、骨角、金属片、编绳、风沙磨损 | 避免刻板化族群符号 |
| 科幻/未来 | 模块接缝、哑光复合材料、发光指示、编号、维护磨损 | 不脱离项目技术水平 |
...
| 民国/近代 | 金属铆钉、皮革、纸张印刷、机械表面、旧封条、城市磨损 | 过度古风纹样或现代潮牌 |
| 现代现实 | 工业设计、品牌去标识化、塑料/玻璃/金属复合、使用磨损 | 空泛“未来感”或无来源古代符号 |
...
| 一个盒子 | 漆木盒匣，四角有铜包边，锁片周围有细小云纹和反复开合磨损 | lacquered wooden box, brass corner guards, fine cloud motif around the lock plate, worn opening marks |
...
2. 至少写入四类设计细节：形制、材质、工艺、文化元素、使用痕迹、功能结构、尺度中的四项。
3. 文化元素必须具体：纹样、铭文、徽记、封缄、器型、工艺来源或地域暗示，不使用空泛 `cultural elements`。
...
| --- | --- | --- |
| “一个普通盒子，木头材质。” | 平凡、无轮廓、无工艺、无文化元素 | 补盒匣器型、锁片、边框、漆面、纹样、开合磨损 |
