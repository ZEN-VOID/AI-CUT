...
description: Use when the `4-Design` stage needs scene-side artifacts, especially routing scene catalog, scene design, and scene panel work under `projects/<项目名>/4-Design/场景/`.
...
- 场景类设计源的父级路由。
- `projects/<项目名>/4-Design/场景/` 下的目录约定。
- `清单 -> 设计 -> 面板` 的默认消费顺序。
...
- 当前类目产物统一写到 `projects/<项目名>/4-Design/场景/`。
...
- `Rule Source`
  ...
  - `.agents/skills/aigc/4-Design/场景/1-清单/`
  - `.agents/skills/aigc/4-Design/场景/2-设计/`
  - `.agents/skills/aigc/4-Design/场景/3-面板/`
- `Meta Rule Source`
