...
| old `1-Planning` mixed planning/grouping/report facts | 旧规划目录同时承载规划、格式、分组、节奏和报告线索 | `references/project-runtime-layout.md`、`types/query-type-map.md`、对应阶段执行报告 | split and annotate | medium | query maps episode split to `1-分集`、grouping to `10-分组`、validation/report facts to `执行报告.md` 或 legacy fallback |
| old `3-Detail` / `2-编导` 主文件查询 | 旧编导/detail root | `references/project-runtime-layout.md` | adapt by intent to `2-编剧` / `4-导演` / `5-表演` with legacy fallback | medium | output marks current owner and legacy fallback |
| old `4-Design/5-Image/6-Video/7-Cut` | 旧资产阶段 | `references/project-runtime-layout.md` | adapt to `11-主体/12-图像/13-画布` | medium | no default answer points to legacy |
...
| 旧 `3-Detail` / `2-编导` 主文件查询是否按意图改查 `2-编剧`、`4-导演` 或 `5-表演`，只有证据需要时才标注 legacy fallback？ | `GATE-QUERY-06` | `FAIL-QUERY-LEGACY` | `N3-carrier-read` | 输出 current owner 路径、legacy 路径及 fallback 原因。 |
| 旧 `4-Design/5-Image/6-Video/7-Cut` 是否只作为 legacy compatibility，而不冒充当前 `11-主体/12-图像/13-画布` 的默认产物根？ | `GATE-QUERY-06` | `FAIL-QUERY-LEGACY` | `N3-carrier-read` | 报告当前资产 carrier 优先级，并列出被降级的旧英文阶段名。 |
