...
| 问断点或下一入口时只读 `STATE.json`，忽略结构化断点 | governance snapshot contract | 优先读取 `governance-state.yaml`，再补读 `STATE.json` | 在 query 合同中固定“断点治理优先读 governance-state” | 查询会给出结构化 checkpoint 证据 |
| 把轻量初始化态误判为治理故障 | init layering contract | 先确认 `STATE.json` 与核心初始化工件是否齐全，再决定是否建议补治理快照 | 在 query 合同中固定“core first, governance lazy” | 查询会明确回答“轻量初始化态”而不是泛化成异常 |
...
- 当问题带有“现在停在哪、从哪继续、还缺什么治理工件”时，`governance-state.yaml` 的优先级应高于 `STATE.json`。
- 若 `governance-state.yaml` 缺失但 `STATE.json` 与 `Init/` 核心工件都在，优先判为“轻量初始化态”，不要直接说成治理失效。
