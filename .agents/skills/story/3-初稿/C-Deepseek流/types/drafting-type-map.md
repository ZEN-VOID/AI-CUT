...
本文件承载 `story-drafting-deepseek` 的分型策略。它只负责判型和路由，不承载完整执行步骤。
...
| `messages_pack` | `projects/story/<项目名>/reports/3-初稿/deepseek/.../*messages.json` | provider 输入证据 |
| `deepseek_sidecar` | `projects/story/<项目名>/reports/3-初稿/deepseek/.../` | provider 过程证据 |
