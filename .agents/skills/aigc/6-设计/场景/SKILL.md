...
metadata:
  short-description: Route the 6-设计 scene skill group
---

# aigc 6-设计/场景

`场景` 是 6-设计阶段的域级组根导引。它只负责判断当前场景任务应进入 `1-清单`、`2-设计` 还是 `3-生成`，并维护场景清单、单场景设计与场景生成资产之间的交接边界；它不直接生成场景清单正文、场景设计稿或图像提示词。

## Context Loading Contract

- 每次调用本技能时，必须同时加载同目录 `CONTEXT.md`。
- 每次调用 `$aigc-design-scene` 或直接命中 `.agents/skills/aigc/6-设计/场景/SKILL.md` 时，必须同时加载同目录 `CONTEXT.md`。
...
- Accepted input: 场景清单、场景设计、场景生成、场景面板、scene panel、场景九宫格、场景域修复、或泛称“处理 6-设计/场景”的任务。
- Required input: 可定位的 `projects/aigc/<项目名>/`，或足以判断目标叶子技能的文件路径、集号、场景名、清单/设计/生成缺口。
- Optional input: 指定叶子阶段、指定场景范围、已有参考图、项目 `MEMORY.md` / `CONTEXT/`、上游 `5-分组` 文件。
...
| --- | --- | --- | --- |
| `scene_list` | 场景清单、从 `5-分组` YAML 提取场景、修复场景清单 | `1-清单/SKILL.md` | `场景清单.md` |
...
| `domain_reconcile` | 上游 `5-分组` 后续新增/更新集数，或既有场景清单、设计稿、生成资产已存在 | 先执行增量对账，再按最早缺口路由 | `reconcile_delta` / `design-manifest.yaml` |
| `domain_repair` | 路径、registry、输出目录或叶子顺序漂移 | 按症状选择对应叶子 | 修复报告或最小 patch |
| `domain_closeout` | 检查场景域是否可交给 7-图像/8-视频 | 已完成叶子输出的验收回查 | 域级状态摘要 |
...
1. 锁定项目根 `projects/aigc/<项目名>/` 与场景域输出根 `projects/aigc/<项目名>/6-设计/场景/`。
2. 读取本 `SKILL.md + CONTEXT.md`；项目任务继续加载项目 `MEMORY.md` 与相关 `CONTEXT/`。
3. 若 `5-分组` 存在新增/更新集数或场景域已有产物，先按 `../references/incremental-reconciliation-contract.md` 建立 `reconcile_delta`，必要时更新 `projects/aigc/<项目名>/6-设计/场景/design-manifest.yaml`。
...
- Output path: 叶子输出固定在 `projects/aigc/<项目名>/6-设计/场景/{1-清单,2-设计,3-生成}/`；增量状态 sidecar 可写入 `projects/aigc/<项目名>/6-设计/场景/design-manifest.yaml`。
