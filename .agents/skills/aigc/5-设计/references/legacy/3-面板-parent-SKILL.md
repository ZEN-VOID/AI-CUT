...
`3-面板` 是 `5-设计` 的展示与派生生图 tranche。它只消费 `2-设计` 已稳定的设计产物，把其中可直接下游使用的 `full_generation_prompt / prompt整合` 收束为面板 layout JSON，并在默认路径下桥接内置 `$imagegen` / `image_gen` 生图，默认模型口径为 `GPT-IMAGE-2`。
...
- `.agents/skills/aigc/5-设计/3-面板/场景`
- `.agents/skills/aigc/5-设计/3-面板/角色`
- `.agents/skills/aigc/5-设计/3-面板/道具`
...
| `constraint_profile` | 不改写 `2-设计` 真源；layout JSON 是面板层第一派生真源；PNG 是再派生资产；SMART 参照只在批量 5-设计 调度时自动发现 |
...
4. 批量 `5-设计` 调度默认启用 `continuous-batch`，自动扫描 `2-设计` 目录中同主体同 stem 图片作为参照。
...
```mermaid
flowchart LR
    A{"执行上下文"} -->|"批量 5-设计 调度"| B["continuous-batch"]
...
| 载体 | 位置 | 作用 |
| --- | --- | --- |
| SMART bridge | `.agents/skills/aigc/5-设计/3-面板/_shared/panel_auto_generate.py` | layout JSON -> built-in imagegen request |
| image execution contract | `.agents/skills/aigc/_shared/image-generation-execution-contract.md` | 内置 imagegen 默认执行模式 |
| design output contract | `.agents/skills/aigc/5-设计/2-设计/_shared/design-output-contract.md` | `full_generation_prompt` 与同目录同名单主体图真源 |
| 场景 leaf | `.agents/skills/aigc/5-设计/3-面板/场景/SKILL.md` | 当前 active leaf |
| 角色 leaf | `.agents/skills/aigc/5-设计/3-面板/角色/SKILL.md` | 当前 active leaf |
| 道具 leaf | `.agents/skills/aigc/5-设计/3-面板/道具/SKILL.md` | 当前 active leaf |
...
4. 用户要求整个 `5-设计` 批量推进，且对应 `2-设计/<域>` 已产出，允许自动进入已 active 的 panel leaf。
...
- `objective`: 判断本轮是批量 5-设计 panel、单 leaf panel、单文件生图还是自然语言直生图。
