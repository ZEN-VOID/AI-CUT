...
当前技能采用 hybrid 拓扑：前段串行锁源与判型，中段按起草/续写/重写/修复分支，后段统一汇流到 DeepSeek provider 与 review gate。
...
| `N6-DEEPSEEK-DRAFT` | 通过 DeepSeek 生成完整章节文件 | provider prompt、系统提示、输出模板 | 调用 `write_chapter_via_deepseek.py` 及 DeepSeek bridge | provider report、raw output | `N7-VALIDATE-WRITEBACK` | provider 真实命中 |
