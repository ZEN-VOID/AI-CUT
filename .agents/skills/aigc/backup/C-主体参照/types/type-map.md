...
| `source_state` | `group_source_ready / group_source_partial / group_source_missing` | `5-分组` 可用性 |
...
- 单一四段式 `分镜ID` 或单帧首帧视频请求：转 `8-视频/A-分镜画面参照`。
- 需要基于组级 storyboard 图片做视频：转 `8-视频/B-分镜故事板参照`。
- 需要重写 `5-分组` 内容：转 `5-分组` 修复，不在本技能内改写。
