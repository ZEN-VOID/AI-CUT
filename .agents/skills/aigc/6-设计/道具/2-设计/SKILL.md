...
`道具/2-设计` 负责消费上游 `道具/1-清单` 的汇总式道具清单，并结合项目 `north_star.yaml` 与 `team.yaml.init_synthesis` 中的设计相关初始化综合上下文，为每个需要进入生成锁定的单个道具主体输出细目设计 Markdown。它不重新抽取清单，不批量改写父级 registry，也不代替 `3-生成` 产出图像。
...
- 必须读取上游 `projects/aigc/<项目名>/6-设计/道具/1-清单/道具清单.md`；缺失时不得凭空生成完整道具设计，应回到 `1-清单` 或请求用户提供替代清单。
- 必须读取 `projects/aigc/<项目名>/0-初始化/north_star.yaml` 与 `projects/aigc/<项目名>/team.yaml.init_synthesis`，抽取全局风格、主题、媒介、禁区和设计相关初始化约束、启发、风险。
- 初始化综合存在时，必须读取 `../../../_shared/team-advisor-consultation-contract.md`，优先消费 `team.yaml.init_synthesis.stage_seed_summary."6-设计"`、`init_handoff.design_seed` 与 `north_star.yaml.创作阶段不变量.设计`；不得在本阶段调用项目监制成员、解析叶子专属 profile、派生新顾问问题或代入顾问角色意识，只能在 LLM 道具设计前形成 `init_team_synthesis_context`。
...
- 默认复核路径必须先按共享初始化综合消费合同读取 `team.yaml.init_synthesis.stage_seed_summary."6-设计"`、`init_handoff.design_seed` 与 `north_star.yaml.创作阶段不变量.设计`；输出只能作为 `init_team_synthesis_context` 中的节点级可执行指导、局部 patch、risk note 或 inspiration，不得直接替代单道具设计稿。
...
- 用户要求“道具设计”“道具细目设计”“从道具清单生成道具设定”“配置或执行 6-设计/道具/2-设计”等任务。
- 单个道具名称、多个道具名称、或默认处理 `道具清单.md` 中全部需要进入设计的主体。

Required input:

- 可定位的 `projects/aigc/<项目名>/6-设计/道具/1-清单/道具清单.md`。
- 清单中每项至少包含 `名称`、`首次登场`、`原文描述（关键词式）`。
- 可读取的 `projects/aigc/<项目名>/0-初始化/north_star.yaml` 与 `projects/aigc/<项目名>/team.yaml.init_synthesis`；若缺失，必须在输出中标注缺口，不得伪造初始化综合上下文。
...
2. 锁定上游 `1-清单/道具清单.md` 的道具主体，并读取可选 `projects/aigc/<项目名>/6-设计/道具/design-manifest.yaml`；只对被指定、被调度或 manifest 标记为 `design_gaps` 的主体生成细目，不为空置主体补占位文件。
...
11. 写入 canonical 路径 `projects/aigc/<项目名>/6-设计/道具/2-设计/<主体ID>-<安全文件名>.md`，并可更新 `design-manifest.yaml` 的 `design_file` 与 `design_gaps`；不改写父级 registry、`1-清单` 或 `3-生成`。
...
- 未读取 `north_star.yaml` / `team.yaml.init_synthesis` 却声称已经使用全局风格和初始化设计约束。
...
7. 可选更新 `projects/aigc/<项目名>/6-设计/道具/design-manifest.yaml`，记录 `design_file` 和剩余 `design_gaps`；manifest 不替代设计稿真源。
...
| --- | --- |
| `OUTPUT-PROP-DESIGN` | `projects/aigc/<项目名>/6-设计/道具/2-设计/PROP-###-<安全文件名>.md` |
| `OUTPUT-PROP-DESIGN-REPORT` | `projects/aigc/<项目名>/6-设计/道具/2-设计/执行报告.md` |
| `OUTPUT-PROP-MANIFEST` | `projects/aigc/<项目名>/6-设计/道具/design-manifest.yaml` |
...
- `north_star.yaml` 与 `team.yaml.init_synthesis` 的全局风格、项目主题和设计相关初始化上下文已被实际消费；缺失项已显式标注。
