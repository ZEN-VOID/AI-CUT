...
| dynamic_reference | `SKILL.md` 只做入口、路由、动态引用和门禁；分区细则不回堆根文件 | `FAIL-INIT-DYNAMIC-REFERENCE` |
...
| steps | `steps/init-workflow.md` 覆盖 `N0 -> N7`、分支、汇流和返工入口 | `FAIL-INIT-STEPS` |
...
| integration | validator、smoke test、Reference Loading Guide 和路径引用完整 | `FAIL-INIT-INTEGRATION` |
...
| --- | --- | --- | --- | --- |
| `references/mode-and-team-contract.md` | `mode` / `team` / `subagents` | `FAIL-INIT-MODE` / `FAIL-INIT-TEAM` / `FAIL-INIT-SUBAGENT` | `references/mode-and-team-contract.md`、`steps/init-workflow.md` | init_mode、team_lineup_mode、team provenance、subagent dispatch evidence |
...
| `references/prompt-packet-contract.md` | `subagents` / `handoff` | `FAIL-INIT-SUBAGENT` / `FAIL-INIT-HANDOFF` | `references/prompt-packet-contract.md`、`steps/init-workflow.md` N4-N5 | planning packet answers、unknowns、sources_breakdown |
