...
`B-分镜故事板参照` 负责把 `projects/aigc/<项目名>/5-分组/` 中的分镜组转为 LibTV 视频生成任务：step1 直接使用现有分镜组内容作为生视频提示词主体；step2 检查 `7-图像/B-分镜故事板/` 是否存在对应分镜组 ID 的图像，有则写入 YAML 参照图路径，没有则保持空引用；step3 调用 `.agents/skills/cli/libTV`，以分镜组为单位默认后台多线程批量并发提交视频任务。
...
- `5-分组` 是本技能的主要信息来源；不得回到 `4-摄影` 或更早阶段改写分镜组内容，除非用户显式要求修复上游。
- 分镜组视频 prompt 主体直接采用 `5-分组` 的现有分镜组正文；LLM 只负责保真组织、LibTV 指令化封装、缺口说明和审查，不得扩写或改写剧情事实。
- 故事板参照图只来自 `projects/aigc/<项目名>/7-图像/B-分镜故事板/` 中与 `group_id` 对应的真实本地图片；不存在时引用为空，不猜测、不补占位、不改用无关图片。
...
- 项目名、项目路径、单集或多集范围，要求从 `5-分组` 批量生成组级视频。
- 用户指定一个或多个三段式分镜组 ID，例如 `1-1-1`。
- 已有 `8-视频/B-分镜故事板参照/` 的 prompt、YAML、LibTV 计划、queue ledger 或生成结果需要 repair / review / rerun / query。
...
- 调用 LibTV 前必须能确定项目内输出目录，默认 `projects/aigc/<项目名>/8-视频/B-分镜故事板参照/第N集/`。
...
- `5-分组` 缺失、目标分镜组 ID 无法唯一追溯，或用户要求改变分镜组剧情核心、镜头顺序、角色事实、动作结果或组边界。
- 用户要求脚本主创视频 prompt 正文、自动扩写剧情或用模板补写未知画面。
- 用户要求生成分镜故事板图本体，应转入 `7-图像/B-分镜故事板`。
- 用户指定的图片参照路径不存在、位于项目外且未明确授权，或同一 `group_id` 命中多个同优先级图片候选。

## Positioning

本技能是 `8-视频` 阶段的组级 LibTV 视频入口，向上承接 `5-分组`，横向读取 `7-图像/B-分镜故事板` 的已生成故事板图，向下调用 `.agents/skills/cli/libTV`。它拥有视频 prompt 包、故事板参照 YAML、LibTV 提交计划、queue ledger、结果下载记录和执行报告的裁决权；它不拥有上游分组改写权，也不拥有故事板图生成权。
...
- `prompt.md` 必须采用 source-first YAML 两阶段处理：draft 阶段直接保留 `5-分组` 原文和原始 fenced YAML，不提前写死 `reference_index / uploaded_url`、空 URL 或占位 URL；final 阶段只在 fenced YAML 的 `故事板参照` 对象中注入最终 `reference_index: 1`、真实 `uploaded_url` 和可选 `image_token`。
...
| 场景 | 必读文件 |
| --- | --- |
| 从 `5-分组` 提取组级正文 | `references/group-source-contract.md` |
| 匹配 `7-图像/B-分镜故事板` 的故事板图 | `references/storyboard-image-binding-contract.md` |
...
```mermaid
flowchart TD
    A["projects/aigc/<项目名>/5-分组/第N集.md"] --> B["step1 提取分镜组完整内容"]
    B --> C["直接作为 LibTV prompt 主体"]
    D["projects/aigc/<项目名>/7-图像/B-分镜故事板/第N集/images/<group_id>.*"] --> E["step2 绑定故事板参照图"]
    E --> F{"图片存在?"}
...
3. step1：以 `projects/aigc/<项目名>/5-分组` 为主要信息来源，解析每个 `## x-y-z` 分镜组，完整提取组正文；同步提取组底 YAML 的 `时长估算`，形成 `duration_estimate_seconds`；若缺失则按组内 `分镜明细` 秒数求和估算，区间时长优先取上限，仍无法确定才回退 15 秒并记录 `duration_source=fallback_default`；`## x-y-z~x-y-z` 组间连接件默认忽略，不进入视频 prompt、故事板参照绑定、LibTV job 或视频文件命名；视频 prompt 主体直接使用现有组内容，不进行剧情改写。
...
5. step2：检查 `projects/aigc/<项目名>/7-图像/B-分镜故事板/第N集/` 下是否存在与 `group_id` 对应的图片；优先 `images/<group_id>.*`，其次同集目录内 `<group_id>.*`，允许常见扩展名 `png/jpg/jpeg/webp`。
...
10. 按官方 `$libTV` 轮询策略查询画布进展；超时后必须保存 `sessionId/projectUuid/projectUrl`，把状态写入 queue ledger，并使用 `python3 .agents/skills/cli/libTV/scripts/query_session.py <sessionId> --project-id <projectUuid>` 后续查询。生成完成后必须自动执行 `download_results.py`，下载默认写入 `projects/aigc/<项目名>/8-视频/B-分镜故事板参照/第N集/`。
...
| --- | --- | --- | --- |
| `FIELD-SBVID-01` | input manifest | 项目根、集号、`5-分组`、目标组范围可追溯 | `FAIL-SBVID-INPUT` |
...
| `FIELD-SBVID-04` | storyboard reference manifest | 只绑定真实 `7-图像/B-分镜故事板` 图片；缺图为空引用 | `FAIL-SBVID-REF` |
...
| --- | --- | --- | --- | --- |
| `FIELD-SBVID-01` | input lock | `第N集-group-index.json` / report | 项目根、集号、`5-分组`、视频输出根 | `FAIL-SBVID-INPUT` |
...
| `PASS-SBVID-02` | `FIELD-SBVID-02` | 如何从 `5-分组` 保真提取组正文 | 解析 `## x-y-z` 与组边界 | group index |
| `PASS-SBVID-03` | `FIELD-SBVID-03` | 如何保证 prompt 直接承接现有组内容 | 添加固定视频约束，接入完整组正文 | prompt markdown |
| `PASS-SBVID-04` | `FIELD-SBVID-04` | 该组是否有对应故事板图 | 按 group_id 查 `7-图像/B-分镜故事板` | reference manifest |
...
- Output path: `projects/aigc/<项目名>/8-视频/B-分镜故事板参照/第N集/`，其中 prompt、manifest、batch YAML、queue、result、报告与自动下载视频均在该集目录下。
