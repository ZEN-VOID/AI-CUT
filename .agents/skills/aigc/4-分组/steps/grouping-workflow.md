...
| `constraint_profile` | 组内显式时长累计优先接近 15 秒，通常约 12-18 秒可接受；画面句子多分镜和对应对白承托不可截断；字数/对白只作辅助风险复核；相邻组具备 3-4 秒首尾帧连接件 |
...
| `N5-GROUP-PLAN` | 裁决组边界 | scene unit table、style projection | 按显式时长累计形成组计划：优先接近 15 秒，通常约 12-18 秒可接受；低于约 10 秒做回填复核，超过约 18 秒做拆分复核；字数/对白只作辅助风险检查 | group boundary plan | `N6-CONNECTOR` | 每组时长接近 15 秒且完整；例外有理由 |
...
| 组内累计时长低于约 10 秒、高于约 18 秒且无例外理由，或超过约 22 秒硬风险线 | `N5-GROUP-PLAN`，移动完整 atomic unit |
| 纯正文超 1680 字、对白过载或旧稿缺少显式秒数 | `N5-GROUP-PLAN`，做密度风险复核或回到 `3-摄影` 补时长 |
