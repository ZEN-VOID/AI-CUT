...
- 接入 `../_shared/upstream-context-application-contract.md`，要求每条选择性 `氛围画面` 证明来自 `5-表演` 触发点、表演焦点和 `2-美学` 三类协议。
...
- 为动作破坏点新增显式 `destruction_type_route` 证据链，挂钩用户信号、`2-美学`、`5-表演` 场景/动作/材质和项目 `MEMORY.md`；新增 `FAIL-ATM-DESTRUCTION-TYPE-ROUTE`，防止只靠关键词误判类型。
...
- 明确灵光、法阵、能量、护盾、粒子或科技光效不是统一禁词；只有在当前类型、上游 source、`2-美学` 或项目 `MEMORY.md` 未授权时才构成越界。
...
- 固化核心输出口径：默认消费 `5-表演` 表演稿和 `2-美学` 的画面基调、角色风格、场景风格；只在渲染、烘托、增强触发点新增 `氛围画面：XXX`，不得每点硬加。
