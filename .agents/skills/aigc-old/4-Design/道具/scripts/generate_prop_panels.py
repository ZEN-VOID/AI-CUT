...
def _load_panel_auto_generate_module() -> Any:
    module_path = Path(__file__).resolve().parent / "panel_auto_generate.py"
...
def _default_output_dir(project_name: str, episode_id: str) -> Path:
    return _repo_root() / "projects" / "aigc" / project_name / "4-Design"
...
def build_panels(
    *,
    ...
    input_root = project_root / "4-Design"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="从 4-Design/道具 域级设计输出生成道具面板 layout JSON，并默认自动生图。")
