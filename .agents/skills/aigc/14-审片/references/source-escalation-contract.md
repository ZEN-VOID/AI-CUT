...
| --- | --- | --- | --- | --- |
| 源层升级是否满足“至少两个素材/变体/跨分镜组重复”或“单个问题明确来自技能合同/模板文字”的高置信入口？ | `GATE-REVIEW-10` | `FAIL-REVIEW-SOURCE-ESCALATION` | `steps/video-review-workflow.md#N5 Landing And Operation Design`、本文件 `High-Confidence Gate` | 多例素材清单、变体/分镜组 refs，或直接源层文字证据 |
...
| 是否定位了明确 source owner，例如 `4-摄影`、`5-分组`、`8-视频`、`9-审片` 或对应叶子技能，而不是笼统写“上游问题”？ | `GATE-REVIEW-10` | `FAIL-REVIEW-SOURCE-ESCALATION` | 本文件 `Common Source Owners`、`steps/video-review-workflow.md#N5 Landing And Operation Design` | `source_owner`、owner path、症状到 owner 的因果说明 |
