...
| execution_report_evidence | 报告包含可审计决策链、references 执行矩阵、规则证据、N/A 说明和返工记录 |
...
| --- | --- | --- | --- | --- |
| `GATE-SCR-01` | 输出只写 `projects/aigc/<项目名>/2-编剧/第N集.md` 和 `执行报告.md`，不改上游 | `FAIL-SCR-PATH` | `N8-SCR-WRITEBACK-CLOSE` | `output_path_check` |
...
- `pass_with_followups`: 阻断项为 0，非阻断 followup 不超过 3 项，且不影响保真、路径、声画、节奏、高潮或尾钩。
