...
description: Use when `5-设计/3-面板/道具` must turn prop design outputs into `PropPanel-layout.json` files, then automatically invoke built-in imagegen; batch 5-设计 runs should reuse existing design images as references, while single file or natural-language generation defaults to no references.
...
- runtime 固定为 `projects/aigc/<项目名>/4-设计/道具/`
...
- `projects/aigc/<项目名>/4-设计/道具/2-设计/第N集/prop_design_prompt.json`
- `projects/aigc/<项目名>/4-设计/道具/2-设计/第N集/道具设计.json`
...
- `projects/aigc/<项目名>/4-设计/道具/3-面板/第N集/<prop_id>-<prop_name>-PropPanel-layout.json`
- `projects/aigc/<项目名>/4-设计/道具/3-面板/第N集/_manifest.json`
- `projects/aigc/<项目名>/4-设计/道具/3-面板/第N集/generated/requests/*.json`
...
| 载体 | 位置 | 作用 |
| --- | --- | --- |
| Markdown 主输入 | `projects/aigc/<项目名>/4-设计/道具/2-设计/第N集/*.md` | 默认 prompt 真源 |
| prompt compat | `projects/aigc/<项目名>/4-设计/道具/2-设计/第N集/prop_design_prompt.json` | 兼容 prompt 输入 |
| design compat | `projects/aigc/<项目名>/4-设计/道具/2-设计/第N集/道具设计.json` | 兼容 fallback |
| layout template | `.agents/skills/aigc/5-设计/3-面板/道具/templates/道具面板-提示词.json` | 版式唯一真源 |
| runner | `.agents/skills/aigc/5-设计/3-面板/道具/scripts/generate_prop_panels.py` | 执行入口 |
| SMART bridge | `.agents/skills/aigc/5-设计/3-面板/_shared/panel_auto_generate.py` | 生图桥接唯一真源 |

## SMART Reference Rule

1. 批量调度 `.agents/skills/aigc/5-设计` 系列任务时：
...
- `actions`:
  1. `--project` 默认定位 `5-设计/道具/2-设计/第N集/`。
...
```bash
python3 .agents/skills/aigc/5-设计/3-面板/道具/scripts/generate_prop_panels.py \
  --project "<项目名>" \
  --episode "第1集"
```
...
python3 .agents/skills/aigc/5-设计/3-面板/道具/scripts/generate_prop_panels.py \
  --project "<项目名>" \
  --prompt-file "projects/aigc/<项目名>/4-设计/道具/2-设计/第1集/prop-001-某道具.md"
```
...
| --- | --- | --- | --- | --- |
| `S1` | `FIELD-PROP-PANEL-01` | 当前是不是批量 5-设计 上下文 | 判定 input + pipeline context | SMART 模式默认错 |
...
`Symptom -> Direct Technical Cause -> Rule Source (本 SKILL / runner / template / _shared bridge) -> Meta Rule Source (5-设计 parent / AGENTS.md / skill-知行合一 / built-in imagegen) -> Fix Landing Points`
