...
| `N5-BRIDGE` | 投影阶段对接解析 | `imitation_principle_map`、全局风格/2/3/5/分镜脚本合同 | 生成 `全局风格解析.md`、`编剧风格解析.md`、`摄影风格解析.md`、`设计风格解析.md`、`分镜脚本.md` | `stage_bridge_analysis_docs`、`storyboard_script_table` | `N6-REVIEW` | 字段不越权，可被 owning stage 和分镜生产消费，路径统一在 `shot-by-shot/<reference_slug>/` |
| `N6-REVIEW` | 验收与直接修复 | 全部候选输出 | 执行证据、边界、阶段、AIGC 可行性 review | `review_verdict` | `N7-WRITE` 或责任节点 | 阻断项已修复或明确阻断 |
| `N7-WRITE` | 写回唯一拉片包 | 通过 review 的候选包 | 写主报告、五份解析、`分镜脚本.md` 和执行报告，统一落点在 `shot-by-shot/<reference_slug>/` | output paths | done | 路径稳定统一，报告含 `思考过程` |

## Workflow Notes

- `N3-OBSERVE` 可以并行分析多个维度，但必须在 `N4-PRINCIPLE` 汇流为唯一临摹原则口径。
- `N5-BRIDGE` 是阶段适配门，不是内容改写门；正式主创仍由全局风格 owning stage、`2-编导`、`3-运动`、`4-摄影` 或 `6-设计` 执行；`分镜脚本.md` 只承接表格式输出，不改变上游主真源。
