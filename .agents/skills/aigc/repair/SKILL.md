...
本技能不取代 `0-初始化` 到 `9-审片` 的阶段主创权；它拥有诊断、影响图、repair brief、豆包执行调度、中文润色增强、创意激发、汇流和验收权。任何创作性正稿修复都必须回到 owning stage 的合同，默认通过 `.agents/skills/api/anyfast/llm/doubao-seed-2.0-pro/` 执行文本分析、拆解、润色和创意扩展。
...
- `review/` 或 `9-审片` 已产生 repair route，需要把 finding 转成源层修复、局部重写、批量重做或后续生成 guardrail。
- 多个子技能包输出物不一致，例如 `4-摄影`、`5-分组`、`6-设计`、`7-图像`、`8-视频` 对同一角色、空间、道具、镜头意图或风格锚点表达冲突。
...
2. 剧情事实、对白、顺序错误：回到 `1-分集` 或 `2-编导` script layer 的保真与字段合同。
3. 导演意图、氛围、美学错误：回到 `2-编导` director layer。
4. 表演动作、心理可见化、场面调度错误：回到 `2-编导` performance layer。
5. 镜头语言、时长、连续性错误：回到 `4-摄影`。
6. 分镜组边界、入场/出场、统计或组内连续性错误：回到 `5-分组`。
7. 场景/角色/道具资产错误：回到 `6-设计` 对应叶子。
8. 图像/视频生成结果错误：回到 `7-图像`、`8-视频` 或 `9-审片` 的 finding route；必要时失效资产并重建。
