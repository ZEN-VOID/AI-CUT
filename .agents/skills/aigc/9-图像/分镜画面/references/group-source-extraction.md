...
本文件定义 `N2-GROUP-SOURCE` 与 `N3-SHOT-POINT-MAP`：从 `projects/aigc/<项目名>/8-分组/第N集.md` 中锁定对应普通分镜组的完整内容，并把组内 `分镜N` 点位映射为四段式 `shot_id`。

## Source Priority

1. `projects/aigc/<项目名>/8-分组/第N集.md` 是分镜画面生成的剧情、镜头、声音和组内连续性主真源。
2. 对每个目标 `## x-y-z`，必须直接引用或完整保存该组从标题到下一个标题前的全部正文，作为 `group_full_content`。
3. `2-美学/类型风格.md`、`2-美学/画面基调/全局风格协议.md`、当前集优先/项目级回退的分镜/画面相关风格协议、项目 `MEMORY.md` 与相关 `CONTEXT/` 只提供项目级风格边界，不替代组稿内容。
4. `3-主体/*/3-生成` 只提供主体参照图候选，不改写剧情或分镜点。
5. `7-摄影`、archived `backup/9-光影` 仅在用户显式要求上溯修复 `8-分组` 或 legacy 光影证据时读取。
...
本技能现在按 `8-分组` 内已经形成的普通 `分镜N` 点位生成图片。四段式 ID 的最后一段必须与组内普通分镜点的源顺序一一对应：
...
| --- | --- |
| `episode_id` | 集号 |
| `source_episode_path` | `8-分组/第N集.md` 路径 |
| `group_id` | 三段式普通组 ID |
| `group_heading` | 原始标题行 |
...
| --- | --- | --- | --- | --- |
| 是否把 `8-分组/第N集.md` 的目标普通组完整正文作为 prompt 基础，而不是摘要或单镜片段？ | `G1-GROUP-SOURCE` | `FAIL-FRAME-GROUP-SOURCE` | `N2-GROUP-SOURCE` | `group-index.json` 记录 `group_full_content` 或全文块引用、源路径、起止行/hash。 |
...
| `shot_count` 是否与 prompt sections、`expected_image_count`、result returned_count 保持一致？ | `G8-RESULT-MAP` | `FAIL-FRAME-RESULT-MAP` | `N3-SHOT-POINT-MAP` / `N9-PERSIST-MAP` | report 记录 `shot_count`, `prompt_image_section_count`, `expected_image_count`, `returned_count`。 |
