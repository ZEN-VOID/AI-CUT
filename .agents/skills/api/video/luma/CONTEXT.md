...
| `TM-LUMA-MODEL-ALIAS` | `ray-2` 或 `ray-v2` 在某些网关下不被接受 | 模型漂移层 | 默认先按父级 `../runbooks/default-model-policy.md` 的 `highest-available-general` 规则族解析最高 Ray 版本（当前为 `ray-2`），再回退 `ray-v2`；旧模型链路继续保留 `ray-v1 -> ray-1.6` | 把共享算法骨架沉到 `../shared/default_model_policy.py`，子技能只保留 Luma 的别名偏好与代理旧别名回退，避免默认值再次散落成硬编码 | 默认名失败后别名候选可继续尝试 |
...
- 当官方模型命名已经切到 `ray-2`，而代理样例还停留在 `ray-v2` 时，默认值应先回到父级共享 runbook 的 `highest-available-general` 规则族，再由 Luma 子技能补 alias fallback 兜底，而不是继续把旧别名写成默认真源。
