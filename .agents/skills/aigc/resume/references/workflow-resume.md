...
| --- | --- | --- |
| `lightweight_init_continue` | 当前 scaffold、`MEMORY.md`、`CONTEXT/` 与可选 `STATE.json` 足够，但 `governance-state.yaml` 尚未生成 | 允许低风险回根或下一阶段；复杂恢复前再补治理快照 |
| `governance_rebuild` | `STATE.json` 缺失且无法从真实阶段产物唯一恢复，或高风险 gate 所需治理工件缺失 | 回根 `aigc` 或 owning governance workflow 补治理链 |
...
test -f "$PROJECT_ROOT/MEMORY.md" && sed -n '1,180p' "$PROJECT_ROOT/MEMORY.md"
test -d "$PROJECT_ROOT/CONTEXT" && find "$PROJECT_ROOT/CONTEXT" -maxdepth 2 -type f | sort
...
| --- | --- | --- | --- | --- |
| 恢复裁决是否按 `governance-state.yaml`、`STATE.json`、初始化核心证据、根治理工件、阶段产物、Git 辅助证据的优先级建立 evidence chain？ | `GATE-RESUME-EVIDENCE-CHAIN` | `FAIL-RESUME-EVIDENCE` | `N2-TRUTH-LOCK` | 报告列出每一类证据的存在性、读取路径、优先级和用于裁决的字段。 |
| `STATE.json` 或 `governance-state.yaml` 中的 recommended entry 是否已与真实阶段产物或初始化核心证据交叉验证，而不是直接免检采用？ | `GATE-RESUME-EVIDENCE-CHAIN` | `FAIL-RESUME-EVIDENCE` | `N2-TRUTH-LOCK` | 报告记录 recommended entry、交叉验证文件、缺口和是否降级为 blocker/gate。 |
