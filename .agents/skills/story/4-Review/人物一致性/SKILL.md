...
description: Use when `4-Review` needs the governed child skill that checks character behavior, motivation, relationship pressure, visible arc continuity, and dialogue/persona consistency.
...
- 当本章存在世界规则变化、巨型装置、灾难系统或文明压力时，检查人物是否真的在承担这种压力，而不是退化成设定讲解员或旁观者
...
- 硬规则：
  ...
  - 若当前章命中证词冲突或人物自证场，必须检查不同角色是否在保护不同的体面/利益；若都像作者统一发言，优先回 Step 4。
  - 若当前章命中世界规则变化或系统性压力，必须检查人物是否把这种压力转成了职责、风险判断或代价承担；若只剩设定说明，优先回 Step 4。
