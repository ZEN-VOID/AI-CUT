...
| `cultural_context_policy` | `strict_period` / `regional_symbolic` / `class_profession_marked` / `inspired_by_only` / `avoid_sensitive` | 决定文化元素如何绑定时代、地域、阶层、职业、禁区和功能逻辑 |
| `evidence_mode` | `source_fact` / `inference` / `inspired_by` / `unknown` | 决定研究结论能否进入确定性设计锁定 |
| `research_axis` | `form_factor` / `material_system` / `craft_process` / `design_detail_culture` / `period_logic` / `wear_trace` / `function_logic` / `risk_uncertainty` | 决定研究必须转译到哪些可见设计字段 |

## Routing Matrix

| type signal | route | required emphasis | review focus |
| --- | --- | --- | --- |
| 关键剧情道具、规则物、反复出现 | `key_prop` | 物语、识别点、生成锁定、signature detail、文化/身份符号 | 是否可跨镜头稳定复现，是否好看且一眼可识别 |
| 普通功能道具 | `support_prop` | 材质、使用痕迹、功能逻辑、克制但明确的设计细节 | 是否过度扩写剧情；是否平凡到缺少设计感 |
...
| `tool` | `function_logic`、`material_system`、`wear_trace`、`design_detail_culture`；突出可见使用磨损与工匠细节，不写操作教程 |
| `weapon` | `form_factor`、`material_system`、`design_detail_culture`、`risk_uncertainty`；只保留美术外观和文化/身份符号，避免现实伤害指导 |
| `document` | `material_system`、`period_logic`、`design_detail_culture`、`wear_trace`；关注纸张、封缄、折痕、印迹、铭文和阶层/机构符号 |
| `container` | `form_factor`、`craft_process`、`design_detail_culture`、`function_logic`；关注开合结构、接口、封缄、纹样和内部不可见逻辑的外部暗示 |
| `costume_adjacent` | `material_system`、`craft_process`、`design_detail_culture`、`wear_trace`；仍按单道具呈现，不让人物入镜 |
| `device` | `form_factor`、`function_logic`、`design_detail_culture`、`risk_uncertainty`；保留外观机制与时代技术语境，不输出可复现工程步骤 |
| `misc` | 至少覆盖 `form_factor`、`material_system`、`design_detail_culture`、`wear_trace` 四项 |

## Design Detail And Cultural Context Rules

| profile | application |
| --- | --- |
| `signature` | 关键道具必须有可复现的独特轮廓、材质记忆点或文化/身份符号，能在纯色背景全貌图中一眼识别。 |
| `crafted` | 工具、器皿、容器、饰件等应突出工艺痕迹、连接件、缝线、铆钉、刻痕、镶嵌、漆层、封缄或修补。 |
| `cultural` | 仪式物、文书、令牌、族群/地域相关物件应写明纹样、铭文、徽记、器型、封缄与文化语境，不随机贴花。 |
| `functional_beauty` | 普通功能道具也要有材质、比例、磨损和结构上的设计美感，但不扩写成清单外剧情真源。 |
| `strict_period` | 时代、地域、阶层、职业与技术水平优先于装饰冲动；风格化不得脱离项目语境。 |
...
| `support_prop` | 保持紧凑，聚焦功能逻辑、材质、年代、可见使用痕迹和一到两个克制设计细节，避免平凡通用物件 |
