...
- 正式写回时，父级 `执行报告.md` 必须包含 `Execution Decision Trace`、`Subagent Dispatch Matrix`、`Reference Execution Matrix`、`Rule Evidence Map`、`N/A Justification` 和 `Repair Log`。
...
Completion gate: `overall_parallel` 模式必须有 6 行 dispatch、6 行 result、父级 cross-check、父级 handoff 和完整报告证据；任一子技能 `blocked` 时父级不得判定为 `pass`。
...
| `pass_threshold` | 6 个 subagents 全部返回 `pass` 或可接受的 `candidate`；任何 `blocked` 必须使父级 verdict 不得为 pass | `Convergence Contract` | `FAIL-AES-QUANT-THRESHOLD` |
