...
- 已 PASS、终稿、accepted manuscript、return actualization、validated actual、stage acceptance packet、上下文回流事实。

## When X Then Check X

| when | must check |
| --- | --- |
| 改已 PASS 终稿事实 | owning stage `第N章.acceptance.json`、accepted manuscript refs、`4-润色/` 或 accepted `3-初稿/` |
...
| 只做局部说明不改 accepted truth | stage acceptance packet 是否仍有效，是否需要 residual risk 标注 |

## Required Impact Additions

- `acceptance_packet_refs`
