...
`2-编导` performance layer 的 review gate 验证逐集编导稿中的表演层是否到位：心理反应可演可感知、演员演技控制精准、每段对白都有语气情绪和气口断句等台词表演锚点、主角内心独白人称正确、动作字段客观可拍、道具承托通过互动准入、表演提示和场面调度已拆入对应剧本句段而非末尾总结，并能被下游摄影和视频链路稳定消费。
...
- review 必须加载 `../references/performance-style-directive-contract.md`，检查 `performance_style_directive` 是否进入角色外放度、身体性、声线、面具/真实轴、风格转变触发和具体表演取舍。
...
- review 必须加载 `../../_shared/concrete-visual-language-contract.md`，检查表演阶段自身的心理反应、演员控制、台词表演、潜台词行为、场面调度、初始化综合、执行报告和最终落盘是否使用具像表演语言；不得把 `2-编导` script layer / `2-编导` director layer 已完成的画面化成果重新抽象成情绪标签、心理解释、关系概念、权力概念或表演意图总结。
- review 还必须加载 `../../_shared/team-advisor-consultation-contract.md` 与 `../SKILL.md#Init Team Synthesis Consumption`，检查本阶段是否只从 `team.yaml.init_synthesis.stage_seed_summary."2-编导"`、`init_handoff.writing_directing_seed` 或 `north_star.yaml.创作阶段不变量.编导` 消费冻结初始化综合，是否形成 `init_team_synthesis_context`，并且没有调用 team 成员身份、解析旧 stage profile 或把本地判断伪装成顾问回答。
...
- `needs_rework` 必须回到 `steps/directing-workflow.md` 的 `N8R-PERF-REPAIR`，由 `2-编导` performance layer 本阶段直接做最小修复并复审；复审未通过不得写入 canonical `2-编导/第N集.md`。
...
- 禁止直接修复的范围：新增或删减剧情事实、改写对白、改变事件顺序、替上游 `2-编导` script layer 或 `2-编导` director layer 修剧情。遇到这类问题必须输出 source owner 和不可用说明。
...
| `GATE-PERF-03` | `init_team_synthesis_context` 仅消费初始化冻结综合，记录来源、采纳约束、风险和被拒/延后点；未调用 team 身份技能、未执行旧 stage profile、未伪造本阶段顾问回答 | `FAIL-PERF-13A` |
...
| `GATE-PERF-15` | 表演阶段自身的心理反应、演员控制、台词表演、潜台词行为、场面调度、初始化综合、执行报告和最终落盘均通过具像表演语言检查：心理、情绪、关系、权力和表演判断落到可见身体、可听声音、可演停顿、空间距离、道具互动或对手反应；没有重新抽象成情绪标签、心理解释、概念关系或表演意图总结；所有投影仍保持上游保真 | `FAIL-CONCRETE-PERFORMANCE-LANGUAGE` |
