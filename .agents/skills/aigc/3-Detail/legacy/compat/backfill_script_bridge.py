#!/usr/bin/env python3
"""Backfill `正文切分参考` and `正文回指` for a legacy 3-Detail projection.

This compatibility repair script is meant for projects whose shared
legacy director-style episode root already carries canonical `剧本正文 + 分镜明细[]`,
but the bridge layer between them was never landed.

Under the new baseline, canonical `3-Detail/第N集.json` is `meta + groups[].global/detail`.
This script must not rewrite the new canonical file back to the old shell.
"""
...
def backfill_episode(episode_path: Path, group_ids: set[str] | None, force: bool, dry_run: bool) -> dict[str, Any]:
    ...
    if isinstance(data.get("meta"), dict) and isinstance(data.get("groups"), list) and "final_output" not in data:
        raise ValueError(
            "检测到新的 canonical 3-Detail detail root (`meta + groups[].global/detail`)。"
            "该 compat 脚本不会把新 baseline 改写回旧的 `final_output.main_content.分镜组列表` 壳。"
        )
    groups = require_list(
