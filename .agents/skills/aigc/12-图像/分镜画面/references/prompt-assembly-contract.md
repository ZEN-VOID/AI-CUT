...
本文件定义 `N5-GROUP-CONSISTENCY` 与 `N6-PROMPT-PACKAGE`：把单个完整分镜组组织为 GPT-IMAGE-2 一次多图长提示词。多图输出必须是多张单独图片，不是故事板拼图。

## Mandatory Source Use

每个 `group_id` 的 prompt block 必须包含或可审计引用：

1. `group_full_content`：来自 `10-分组` 对应 `## x-y-z` 的完整正文。
...
- 默认按当前仓库 GPT-IMAGE-2 CLI/API 口径审查单次 `n` 上限为 `10`。
- 若 `shot_count > provider_cap`，不得静默拆分；应报告 `blocked_provider_image_count_cap`，或在用户显式授权后改为拆组/重组方案。
- 若用户提供可信 provider 上限覆盖值，必须写入 `imagegen-plan.json.provider_cap_override` 和执行报告。

## Word / Length Guidance

- 组级长提示词可以很长，但每个 Image section 应避免无关重复。
- 若 provider 对 prompt 长度报错，优先压缩重复风格词和已由 `Group Consistency Contract` 统一声明的内容，不得删除 `Source Group Full Content` 的审计引用或 Image section 的核心画面状态。
