...
`video-to-manifest` 是 workflow 共享卫星技能，用于从指定视频或视频目录生成、更新、修复或校验 F1、F2 等视频工作流可消费的 `视频说明.yaml`。
...
- validate an existing manifest before F1 editing or F2 asset-evidence ingestion.
...
`operation_demo` requires operation/action-state evidence, `tool_display` requires screen-state evidence, and `aigc_content` requires semantic/visual rhythm evidence for downstream selection. F1 consumes the manifest directly in visual planning; F2 may use it only as optional input evidence before rebuilding `asset_evidence.json`.
