...
- `5-上下文回流` 只承接 PASS 且明确 handoff 的 validated actualization，不回写规划正文。
...
flowchart TD
    ...
    E --> F["3-初稿<br/>章节正文唯一业务真源"]
    F --> G["review<br/>validation_fact_pack / 第V卷.validation.json"]
    G -->|"FAIL / 修复分流"| E
    G -->|"PASS + handoff: 4-润色"| H["4-润色<br/>二次改写与表达强化稿"]
    H --> G
