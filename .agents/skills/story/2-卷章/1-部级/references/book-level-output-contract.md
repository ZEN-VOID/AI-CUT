...
7. `整体节奏曲线` 必须默认采用 `Save the Cat 15 步`，并附 Mermaid 图。
8. `整体节奏曲线` 必须显式回答：长线 promise 如何分配到各卷、哪段卷级走廊承担改规、哪段卷级走廊承担见底与终局收束。
9. `整体节奏曲线` 必须包含 `book_wave_map`，并写清 `volume_intensity_map / volume_role_map / respite_corridor / payoff_distribution`。
10. `规避` 必须是创作层禁飞区，而不是空泛提醒。
