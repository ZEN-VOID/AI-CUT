...
| `default_rework_targets` | `10-画布/B-分镜故事板参照`, `10-画布/C-主体参照`, `10-画布/D-主板混合参照`, `review` |
| `source_owners` | `3-主体`, `8-分组`, `9-图像`, `10-画布`, `review` |

## Scope

本维度检查 `10-画布/B-分镜故事板参照` 请求对象是否稳定、`10-画布/C-主体参照` 是否可信、`10-画布/D-主板混合参照` 是否同时满足故事板总参照与主体参照绑定，provider handoff pack 是否完整，review evidence 是否形成可执行缺陷和修复路由，以及 motion、duration、continuity readiness 是否达到交付门。
...
| --- | --- | --- | --- | --- |
| 是否锁定同一 `review_fact_pack` 下的 `10-画布/B-分镜故事板参照`、`C-主体参照`、`D-主板混合参照` 与 provider handoff refs，而不是凭远端任务或本地视频文件存在判断交付就绪？ | `GATE-DIM-VD-01` | `FAIL-VD-01` | `N1-VIDEO-READ` | `video_note` 记录视频请求、storyboard refs、subject refs、hybrid refs、handoff refs 与缺失项。 |
...
| `C-主体参照` 是否以 `3-主体` 注册表及分组 YAML 引用为基准，主体槽位、reference order、9 图预算、混合主体互斥和本地路径禁区均可追溯？ | `GATE-DIM-VD-03` | `FAIL-VD-01` | `N2-MOTION-CHECK` | 维度报告列出主体 slot、reference asset、预算状态、缺失/排除原因。 |
...
| 视频交付问题是否归因到 `10-画布` 的 B/C/D 路线、review evidence、`9-图像` 参照资产、`3-主体` 主体资产或 `8-分组` 源文本，而不是笼统写成 provider 不稳？ | `GATE-DIM-VD-07` | `FAIL-VD-03` | `N4-PACKET-WRITE` | `dimension_packet.issues[*].source_layer_owner`、default_rework_targets 与 evidence refs 完整。 |
...
- `10-画布` 的 review 要比图像多看一层 motion/duration readiness；review evidence 还要看实际视频缺陷是否回到可执行 repair route。
- 先锁 request、binding、handoff 三段，再判问题在 motion/duration 还是 provider pack。

## Root-Cause Rule

若本维度失效，先修 `10-画布` 的 motion、duration、reference readiness、provider pack 与 review repair route，不要把问题压成泛化“视频还不稳”。
