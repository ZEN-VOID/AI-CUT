...
description: Use when the `4-Design` stage needs costume-side artifacts, especially routing role-list costume facts into wardrobe list, costume design, or costume panel outputs under `projects/项目名/4-Design/服装/`.
...
- 需要确保服装阶段所有产物稳定落到 `projects/<项目名>/4-Design/服装/`。

## When Not to Use

- 当前任务仍在抽角色 canonical identity，应先回到 `4-Design/角色`。
...
5. `projects/<项目名>/4-Design/角色/1-清单/第N集/角色清单.json`
...
1. 全链第一输入根固定为 `projects/<项目名>/4-Design/角色/1-清单/第N集/角色清单.json`。
...
| `RL4` | 输出根是否唯一 | 锁 `projects/<项目名>/4-Design/服装/1-清单/第N集/` | 返回三份 JSON 主产物 |
...
- `Rule Source`
  ...
  - `.agents/skills/aigc/4-Design/服装/CONTEXT.md`
  - `.agents/skills/aigc/4-Design/服装/1-清单/SKILL.md`
  - `.agents/skills/aigc/4-Design/服装/2-设计/SKILL.md`
  - `.agents/skills/aigc/4-Design/服装/3-面板/SKILL.md`
- `Meta Rule Source`
