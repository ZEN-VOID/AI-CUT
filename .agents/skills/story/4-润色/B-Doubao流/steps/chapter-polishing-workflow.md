...
本文件承载 `story-polishing-doubao` 的思行网络。节点必须同时表达判断、动作、证据、路由和 gate。

## Topology

当前技能采用 hybrid 拓扑：前段串行锁源与判型，中段先启动 team supervision subagents；若用户显式要求 subagents 模式，则追加按 `story/review` 维度子技能拆分的审计点 subagents，并把 findings 汇成 provider repair brief；随后按起草/续写/重写/修复分支汇流到 Doubao provider，后段接 writeback 与卷级 review handoff。
...
flowchart TD
    ...
    N3S --> N3R{"N3R-REVIEW-SUBAGENT-AUDIT?"}
    N3R -->|"explicit subagents"| N3RP["story/review 维度审计包"]
    N3RP --> N4{"N4-DRAFT-BRANCH"}
    N3R -->|"default"| N4
...
| `N2-TYPE-PROFILE` | 判定起草、续写、重写、修复或 dry-run | 目标章是否存在、用户意图、`types/polishing-type-map.md` | 生成 `type_profile`；目标章已存在时要求显式 mode，正式写回要求 `--force` | `type_profile` | `N3-CONTEXT-PACK` | mode 唯一且不冲突 |
...
| `N3S-SUPERVISION-PACKET` | 启动 GPT 监制组 | messages pack、`team/SKILL.md + CONTEXT.md`、被选 team 成员技能 | 真实启动 subagents，汇流 narrative/structure/character/style/continuity 约束 | supervision packet、roster refs 或降级报告 | `N3R-REVIEW-SUBAGENT-AUDIT` 或 `N4-DRAFT-BRANCH` | 有真实 subagent 证据；被上层阻断时有降级说明 |
| `N3R-REVIEW-SUBAGENT-AUDIT` | 显式 subagents 模式下按审计点拆分维度审查 | messages pack、源初稿、既有润色稿、审计点、`.agents/skills/story/review` registry、命中的 review 子技能 | 分别调用结构兑现、连续性、逻辑自洽校验、人物一致性、时间线、任务汇聚、文体读感等子技能输出 packets，并汇成 Doubao repair brief | dimension packets、review repair brief、降级报告 | `N4-DRAFT-BRANCH` | 每个审计点有对应 review 子技能结果；同轮必须进入 provider 优化分支 |
| `N4-DRAFT-BRANCH` | 按类型选择 prompt 约束 | `type_profile`、现稿状态、用户约束 | 路由到新章、重写、续写或修复分支 | branch decision | `N5A/B/C/D` | 分支与用户请求一致 |
| `N5A-POLISH-PROMPT` | 为新章起稿生成 provider prompt | context pack、输出模板 | 保持 planning 义务，生成完整章请求 | prompt section | `N6-PROVIDER-DRAFT` | 没有依赖现稿 |
| `N5B-REPOLISH-PROMPT` | 为重写生成 provider prompt | context pack、现有正文、用户重写约束 | 保留成立事实，重构正文 | prompt section | `N6-PROVIDER-DRAFT` | 已回读现稿 |
| `N5C-REPAIR-PROMPT` | 为续写/补全生成 provider prompt | context pack、现有正文、章末目标、续写边界或补充约束 | 延续既有文气并补足未完成义务 | prompt section | `N6-PROVIDER-DRAFT` | 承接点明确 |
...
| `N6-PROVIDER-DRAFT` | 通过豆包生成完整最小局部修补章节文件 | provider prompt、系统提示、输出模板、supervision packet | 调用 `polish_chapter_via_doubao.py` 及豆包 bridge | provider report、raw output | `N7-VALIDATE-WRITEBACK` | provider 真实命中 |
...
| `FAIL-DRAFT-SUPERVISION` | subagents 未真实启动、未说明降级，或监制包未进入 provider messages | `N3S-SUPERVISION-PACKET` |
| `FAIL-DRAFT-REVIEW-SUBAGENTS` | 显式 subagents 模式未按 `story/review` 维度子技能拆分审计，或只有审计包没有通过 Doubao 直接优化 | `N3R-REVIEW-SUBAGENT-AUDIT`、`N5D-REPAIR-PROMPT` |
| `FAIL-DRAFT-CONTINUITY` | 上一章存在但本章开篇像重新开局、漏接事实/位置/情绪/悬念 | `N3-CONTEXT-PACK`、`N5*` |
...
- 显式 subagents 模式还必须产生 `story/review` 维度 packets 或降级报告、review repair brief，以及 Doubao 输出已按 findings 直接优化的证据。
